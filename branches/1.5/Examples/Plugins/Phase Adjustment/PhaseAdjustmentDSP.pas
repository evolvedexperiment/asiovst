...
procedure TPhaseAdjustmentModule.ParameterPhaseChange(Sender: TObject;
...
begin
  ...
  if EditorForm is TFmPhaseAdjustment then
    with TFmPhaseAdjustment(EditorForm) do
      UpdatePhaseDial;
end;
...
begin
  if Assigned(FImpulseResponse) and Assigned(FFilterFreq) and Assigned(FFft)
  then
  begin
    ...
    begin
      ...
      for Index := 1 to FIRSize - 1 do
        FFilterFreq[Index] := ComplexValue;
    end;
    ...
    case FFft.DataOrder of
      doPackedRealImaginary:
        FFft.PerformiFFTPackedReIm(PDAVSingleFixedArray(FFilterFreq),
          FImpulseResponse);
      doPackedComplex:
        FFft.PerformiFFTPackedComplex(FFilterFreq, FImpulseResponse);
    else
      raise Exception.Create('not supported');
    end;

    Move(FImpulseResponse^[0], FImpulseResponse^[FIRSize div 2],
      (FIRSize div 2) * SizeOf(Single));
    Move(FImpulseResponse^[3 * FIRSize div 2], FImpulseResponse^[0],
      (FIRSize div 2) * SizeOf(Single));

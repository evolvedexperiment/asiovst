...
uses Windows, uEditor, DAEffect, DAEffectX, ActiveX,
     ...
     dmoParamEQ, dmoWavesReverb, dmoEcho,
     dmoGargle, DVSTUtils, dvstextra;
...
// MIDI only plugins, but generally this should be quite okay
 ...
 private
  ...
  {$IFDEF EQ}
  _eq:array[0..15] of tdcdmoparameq;
  {$ENDIF}

  {$IFDEF Echo}
  _echo:array[0..15] of tdcdmoecho;
  {$ENDIF}

  {$IFDEF WAVESREVERB}
  _rv:array[0..15] of tdcdmowavesreverb;
  {$ENDIF}

  {$IFDEF Gargle}
  _gargle:array[0..15] of tdcdmogargle;
  {$ENDIF}
 public
...
begin
 ...
 begin
  _gargle[i]:=tdcdmogargle.create(nil);
  _gargle[i].Init(round(samplerate),32,1,true);
  _gargle[i].Enabled:=true;
 end;
 {$ENDIF}
 ...
 for i:=0 to numChannels-1 do
 begin
  _eq[i]:=tdcdmoparameq.create(nil);
  _eq[i].Init(round(samplerate),32,1,true);
  _eq[i].Enabled:=true;
 end;
 {$ENDIF}
 ...
 for i:=0 to numChannels-1 do
 begin
  _echo[i]:=tdcdmoecho.create(nil);
  _echo[i].Init(round(samplerate),32,1,true);
  _echo[i].Enabled:=true;
 end;
 {$ENDIF}
 ...
 for i:=0 to numChannels-1 do
 begin
  _rv[i]:=tdcdmowavesreverb.create(nil);
...
begin
 inherited Destroy;
 {$IFDEF Gargle}
 for i:=0 to numchannels-1 do _gargle[i].free;
 {$ENDIF}

 {$IFDEF Echo}
 for i:=0 to numchannels-1 do _echo[i].free;
 {$ENDIF}

 {$IFDEF EQ}
 for i:=0 to numchannels-1 do _eq[i].free;
 {$ENDIF}
...
begin
 ...
 for i:=0 to numchannels-1 do
 case index of
 0:_gargle[i].RateHz:=round(fpar[0]*999+1);
 1:_gargle[i].WaveShape:=round(fpar[1]);
 else
 end;
 {$ENDIF}
 ...
 0:if (not odd(i)) then
 begin
  _echo[i].leftdelay:=fpar[0]*1999+1;
  _echo[i].rightdelay:=fpar[0]*1999+1;
 end;
 1:if (odd(i)) then
 begin
  _echo[i].leftdelay:=fpar[1]*1999+1;
  _echo[i].rightdelay:=fpar[1]*1999+1;
 end;
 2:_echo[i].feedback:=fpar[2]*100;
 else
 end;
 {$ENDIF}
 ...
 for i:=0 to numchannels-1 do
 case index of
 0:_eq[i].Gain:=fpar[0]*30-15;
 1:_eq[i].center:=fpar[1]*(16000-80)+80;
 2:_eq[i].bandwidth:=fpar[2]*35+1;
 else
 end;
 {$ENDIF}
 ...
 for i:=0 to numchannels-1 do
 case index of
 0:_rv[i].inGain:=fpar[0]*96-96;
 1:_rv[i].reverbmix:=fpar[1]*96-96;
 2:_rv[i].reverbtime:=fpar[2]*(3000-0.001)+0.001;
 3:_rv[i].highfreqrtratio:=fpar[2]*(0.999-0.001)+0.001;
 else
 end;
 {$ENDIF}
...
begin
 {$IFDEF Gargle}
 for i:=0 to numchannels-1 do
  _gargle[i].Process(Ins[i],sampleFrames*4);
 {$ENDIF}

 {$IFDEF Echo}
 for i:=0 to numchannels-1 do
  _echo[i].Process(Ins[i],sampleFrames*4);
 {$ENDIF}

 {$IFDEF EQ}
 for i:=0 to numchannels-1 do
  _eq[i].Process(Ins[i],sampleFrames*4);
 {$ENDIF}
 ...
 for i:=0 to sampleFrames-1 do
  ...
  begin
   ...
   if not odd(j) then
   begin
    Outs[j,i]:=Outs[j,i]+Ins[j,i]*(1-fpar[3])+fpar[3]*Ins[j+1,i];
    Outs[j+1,i]:=Outs[j+1,i]+Ins[j+1,i]*(1-fpar[3])+fpar[3]*Ins[j,i];
   end;
   {$ELSE}
...
begin
 ...
 for i:=0 to sampleFrames-1 do
  ...
  begin
   ...
   if not odd(j) then
   begin
    Outs[j,i]:=Ins[j,i]*(1-fpar[3])+fpar[3]*Ins[j+1,i];
    Outs[j+1,i]:=Ins[j+1,i]*(1-fpar[3])+fpar[3]*Ins[j,i];
   end;
   {$ELSE}
   Outs[j,i]:=Ins[j,i];
   {$ENDIF}
...
begin
 ...
 1:StrPCopy(text,'Center Frequency');
 2:StrPCopy(text,'Bandwidth');
 {$ENDIF}

 {$IFDEF Gargle}
 0:StrPCopy(text,'Rate');
 1:StrPCopy(text,'Waveform');
 {$ENDIF}

 {$IFDEF WAVESREVERB}
 0:StrPCopy(text,'Gain');
 1:StrPCopy(text,'Reverb Mix');
 2:StrPCopy(text,'Reverb Time');
 3:StrPCopy(text,'High Frequency Ratio');
 {$ENDIF}

 {$IFDEF Echo}
 0:StrPCopy(text,'Delay Time Left');
 1:StrPCopy(text,'Delay Time Right');
 2:StrPCopy(text,'Feedback');
 3:StrPCopy(text,'Crossover');
 {$ENDIF}
...
begin
 ...
 begin
  ...
  {$IFDEF Gargle}
  Editor.SetBounds(0,0,Editor.Width,199);
  {$ELSE}
 ...
 {$IFDEF EQ}
 editor.label3.caption:='DX9 DMO ParamEQ';
 editor.label4.caption:='DX9 DMO ParamEQ';
 editor.label7.visible:=false;
 editor.label9.visible:=false;
 editor.label12.visible:=false;
 editor.par3.visible:=false;
 {$ENDIF}

 {$IFDEF Echo}
 editor.label3.caption:='DX9 DMO Echo';
 editor.label4.caption:='DX9 DMO Echo';
 editor.label3.left:=editor.label3.left+10;
 editor.label4.left:=editor.label4.left+10;
 editor.label9.visible:=false;
 editor.label12.visible:=false;
 {$ENDIF}

 {$IFDEF WAVESREVERB}
 editor.label3.caption:='DX9 DMO WavesReverb';
 editor.label4.caption:='DX9 DMO WavesReverb';
 editor.label3.left:=editor.label3.left-20;
 editor.label4.left:=editor.label4.left-20;
 editor.label9.visible:=false;
 editor.label12.visible:=false;
 {$ENDIF}

 {$IFDEF Gargle}
 editor.label3.caption:='DX9 DMO Gargle';
 editor.label4.caption:='DX9 DMO Gargle';
 editor.label13.visible:=false;
 editor.par1.visible:=false;
 editor.par2.visible:=false;
 editor.par3.visible:=false;
 editor.label5.visible:=false;
 editor.label6.visible:=false;
 editor.label7.visible:=false;
 editor.DIBImage1.visible:=false;
 editor.DIBImage2.top:=94;
 editor.label13.top:=130;
 editor.clientHeight:=199;
 {$ENDIF}
...
begin
 ...
 // Here you can access the plugin and editor threads and properties
 // at the same time, which is useful if you want to update a
 // control in the editor to reflect for example the state of a
 // variable inside the algorithm
 // example:
 // if Plugin.fpar[2]>0.5 then Editor.button.visible:=true
 // else Editor.button.visible:=false;
end;

...
type
  ...
  public
    ...
    procedure PerformFFTCCS(const FrequencyDomain: PDAVComplexSingleFixedArray;
      const TimeDomain: PDAVSingleFixedArray);
    procedure PerformiFFTCCS(const FrequencyDomain: PDAVComplexSingleFixedArray;
      const TimeDomain: PDAVSingleFixedArray);
...
procedure TFftReal2ComplexCuda32.PerformFFTCCS(const FrequencyDomain
...
begin
  ...
  if CudaFftResult <> cfrSuccess then
    raise Exception.Create(CCudaFftResultStrings[CudaFftResult]);

  // copy memory from host to device
  CudaError := CudaMemcpy(FGPUTimeDomain, TimeDomain, FFftSize * SizeOf(Single),
    cmkHostToDevice);
  if CudaError <> ceSuccess then
    raise Exception.Create(CudaErrorToString(CudaError));

  // perform FFT
  CudaFftResult := CudaFftExecR2C(CudaFftHandle, FGPUTimeDomain,
    FGPUFreqDomain);
  if CudaFftResult <> cfrSuccess then
    raise Exception.Create(CCudaFftResultStrings[CudaFftResult]);

  // copy memory from device to host
  CudaError := CudaMemcpy(FrequencyDomain, FGPUFreqDomain,
    FFftSize * SizeOf(Single), cmkDeviceToHost);
  if CudaError <> ceSuccess then
    raise Exception.Create(CudaErrorToString(CudaError));
...
procedure TFftReal2ComplexCuda32.PerformiFFTCCS(const FrequencyDomain
  : PDAVComplexSingleFixedArray; const TimeDomain: PDAVSingleFixedArray);
var
  CudaFftHandle: TCudaFftHandle;
  CudaFftResult: TCudaFftResult;
...
begin
  ...
  // copy memory from host to device
  CudaError := CudaMemcpy(FGPUFreqDomain, FrequencyDomain,
    FFftSize * SizeOf(Single), cmkHostToDevice);
  if CudaError <> ceSuccess then
    raise Exception.Create(CudaErrorToString(CudaError));

  // perform IFFT
  CudaFftResult := CudaFftExecR2C(CudaFftHandle, FGPUFreqDomain,
    FGPUTimeDomain);
  if CudaFftResult <> cfrSuccess then
    raise Exception.Create(CCudaFftResultStrings[CudaFftResult]);
...
begin
  if Assigned(FGPUTimeDomain) then
    CudaFree(FGPUTimeDomain);
  if Assigned(FGPUFreqDomain) then
    CudaFree(FGPUFreqDomain);
  inherited;
end;
...
begin
  ...
  // Allocate new memory
  CudaError := CudaMalloc(FGPUFreqDomain, (FFftSize div 2 + 1) *
    SizeOf(TComplex32));
  if CudaError <> ceSuccess then
    raise Exception.Create(CudaErrorToString(CudaError));
...
begin
  ...
  case AutoScaleType of
    astDivideFwdByN:
      Rescale(FrequencyDomain);
    astDivideBySqrtN:
      RescaleSqrt(FrequencyDomain);
  end;
end;

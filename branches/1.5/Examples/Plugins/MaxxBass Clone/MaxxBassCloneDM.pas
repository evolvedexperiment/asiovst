...
uses
  Math, DAV_Common, DAV_Convert, DAV_Approximations, MaxxBassCloneGUI;
...
begin
  for Channel := 0 to Length(FHarmonicBass) - 1 do
    FreeAndNil(FHarmonicBass[Channel]);
end;

procedure THarmonicBassModule.ParameterHighpassDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  case Round(Parameter[Index]) of
    0:
      PreDefined := '16 Hz DC Filter';
    1:
      PreDefined := '12 dB/oct';
    2:
      PreDefined := '24 dB/oct';
  end;
end;

procedure THarmonicBassModule.ParameterListenDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  case Round(Parameter[Index]) of
    0:
      PreDefined := 'Audio';
    1:
      PreDefined := 'Original Bass';
    2:
      PreDefined := 'HarmonicBass';
  end;
end;

procedure THarmonicBassModule.ParameterdBDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  if Parameter[Index] = 0 then
    PreDefined := '-oo'
  else
    PreDefined := AnsiString(FloatToStrF(RoundTo(Amp_to_dB(Parameter[Index]), -2), ffGeneral, 3, 3));
end;

procedure THarmonicBassModule.ParameterHighpassSelectChange(Sender: TObject;
  ...
  try
    for Channel := 0 to Length(FHarmonicBass) - 1 do
      if Assigned(FHarmonicBass[Channel]) then
        FHarmonicBass[Channel].HighpassSelect := THighpassSelect(Round(Value));
  finally
    FCriticalSection.Release;
  end;
end;

procedure THarmonicBassModule.ParameterInputChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  ...
  try
    for Channel := 0 to Length(FHarmonicBass) - 1 do
      if Assigned(FHarmonicBass[Channel]) then
        FHarmonicBass[Channel].InputLevel := Value;
  finally
    FCriticalSection.Release;
  end;
end;

procedure THarmonicBassModule.ParameterDecayChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  ...
  try
    for Channel := 0 to Length(FHarmonicBass) - 1 do
      if Assigned(FHarmonicBass[Channel]) then
        FHarmonicBass[Channel].Decay := 0.5 * dB_to_Amp(Value);
  finally
    FCriticalSection.Release;
  end;
end;

procedure THarmonicBassModule.ParameterResponseChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
const
...
begin
  ...
  try
    for Channel := 0 to Length(FHarmonicBass) - 1 do
      if Assigned(FHarmonicBass[Channel]) then
        FHarmonicBass[Channel].Response := Value * CScale;
  finally
    FCriticalSection.Release;
  end;
end;

procedure THarmonicBassModule.ParameterRatioChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  ...
  try
    for Channel := 0 to Length(FHarmonicBass) - 1 do
      if Assigned(FHarmonicBass[Channel]) then
        FHarmonicBass[Channel].Ratio := Value;
  finally
    FCriticalSection.Release;
  end;
end;

procedure THarmonicBassModule.ParameterOriginalBassChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  CalculateGains;
end;

procedure THarmonicBassModule.ParameterMaxxbassChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  CalculateGains;
end;

procedure THarmonicBassModule.ParameterListenChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  CalculateGains;
end;
...
procedure THarmonicBassModule.ParameterFrequencyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  ...
  try
    for Channel := 0 to Length(FHarmonicBass) - 1 do
      if Assigned(FHarmonicBass[Channel]) then
        FHarmonicBass[Channel].Frequency := Value;
  finally
    FCriticalSection.Release;
  end;
end;
...
begin
  ...
  try
    if Abs(SampleRate) > 0 then
      for Channel := 0 to Length(FHarmonicBass) - 1 do
        if Assigned(FHarmonicBass[Channel]) then
          FHarmonicBass[Channel].SampleRate := Abs(SampleRate);
  finally
    FCriticalSection.Release;
  end;
end;
...
begin
  ...
  try
    for Channel := 0 to Length(FHarmonicBass) - 1 do
      FHarmonicBass[Channel].ResetStates;
  finally
    FCriticalSection.Release;
  end;
end;
  ...
  try
    for Channel := 0 to Length(FHarmonicBass) - 1 do
      for Sample := 0 to Min(FBlockSize, SampleFrames) - 1 do
        if IsNaN(Inputs[Channel, Sample]) then
          Outputs[Channel, Sample] := FHarmonicBass[Channel].ProcessSample32(0)
        else
          Outputs[Channel, Sample] := FHarmonicBass[Channel].ProcessSample32

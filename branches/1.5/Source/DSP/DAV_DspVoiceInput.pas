...
type
  ...
  public
    ...
    property VoicedUnvoicedRatio: Single read FVoicedUnvoicedRatio
      write SetVoicedUnvoicedRatio;
  end;
...
begin
  FMinPitch := Power(16, 0.5 - (FMaximumFrequency - 45) / 48) *
    SampleRate / 440;
...
begin
  if (FTracking = ttOff) then
    FPitchStep := 110.0 * FPitchMult * FInvSampleRate;
end;
...
begin
  ...
  FLowpassState[1] := FLowpassState[1] - FLowFreq *
  ...
  begin
    if ((FLowpassState[1] > 0) and (FLowBuffer[0] <= 0)) then
    // found +ve zero crossing
    begin
      if ((FLowBuffer[1] > FTempMinPitch) and (FLowBuffer[1] < FMaxPitch)) then
      // ...in allowed range
      begin
        FTempMinPitch := 0.6 * FLowBuffer[1];
        // new max pitch to discourage octave jumps!
        FLowBuffer[0] := FLowpassState[1] / (FLowpassState[1] - FLowBuffer[0]);
        // fractional period...
        FPitchStep := FPitchMult / (FLowBuffer[1] - FLowBuffer[0]);
        // new period

        if (FTracking = ttQuantized) then // quantize pitch
        begin
          FPitchStep := CRootM * (Log10(FPitchStep) - FRoot);
          FPitchStep := Power(1.0594631, trunc(FPitchStep + 0.5) +
            CRootM * FRoot);
        end;
      end;

...
constructor TCustomSETunerModule.Create(SEAudioMaster: TSE2audioMasterCallback;
  Reserved: Pointer);
begin
...
procedure TCustomSETunerModule.SubProcessStatic(const BufferOffset,
  SampleFrames: Integer);
begin
...
class procedure TCustomSETunerModule.GetModuleProperties
...
function TCustomSETunerModule.GetPinProperties(const Index: Integer;
...
// describe the pins (plugs)
function TSETunerStaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
...
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSETunerPins(index) in [pinMinimum .. pinDSBandwidth] then
    with Properties^ do
      Direction := drIn;
end;

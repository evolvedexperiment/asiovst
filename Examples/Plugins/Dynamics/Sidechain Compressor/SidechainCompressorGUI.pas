...
function TFmSidechainCompressor.GetFilterGain(Sender: TObject;
  const Frequency: Single): Single;
begin
  with TSidechainCompressorDataModule(Owner) do
    Result := EvaluateFrequencyResponse(Frequency);
end;
...
begin
  ...
  begin
    ...
    begin
      if VstPlugIn.EditVisible then
        VstPlugIn.GUIControl.BringToFront
      else
        VstPlugIn.ShowEdit;
    end;
  end;
...
begin
  ...
  begin
    ...
    if not SliderMakeUpGain.Enabled then
      UpdateMakeUp;
  end;
end;
...
begin
  ...
  begin
    ...
    if Frequency <> SliderLowcutFrequency.Value then
      SliderLowcutFrequency.Value := Frequency;
    LbLowcutFrequencyValue.Caption := ParameterDisplay[0] + ' ' +
      ParameterLabel[0];
...
begin
  ...
  begin
    ...
    if Order <> Round(SliderLowcutOrder.Value) then
      SliderLowcutOrder.Value := Order;
...
begin
  ...
  begin
    ...
    if Frequency <> SliderHighcutFrequency.Value then
      SliderHighcutFrequency.Value := Frequency;
    LbHighcutFrequencyValue.Caption := ParameterDisplay[2] + ' ' +
      ParameterLabel[2];
    GuiEQGraph.UpdateGraph;
  end;
end;
...
begin
  ...
  begin
    ...
    if Order <> Round(SliderHighcutOrder.Value) then
      SliderHighcutOrder.Value := Order;
    LbHighcutOrderValue.Caption := ParameterDisplay[3] + ' ' +
      ParameterLabel[3];
    GuiEQGraph.UpdateGraph;
  end;
end;
...
begin
  ...
  begin
    Attack := Parameter[4];
    if Attack <> SliderAttack.Value then
...
begin
  ...
  begin
    Release := Parameter[6];
    if Release <> SliderRelease.Value then
      SliderRelease.Value := Release;
...
begin
  ...
  begin
    ...
    if Threshold <> SliderThreshold.Value then
...
begin
  ...
  begin
    Ratio := Parameter[8];
    if Ratio <> SliderRatio.Value then
...
begin
  ...
  begin
    Knee := Parameter[9];
    if Knee <> SliderKnee.Value then
      SliderKnee.Value := Knee;
...
begin
  ...
  begin
    ...
    if MakeUp <> SliderMakeUpGain.Value then
      SliderMakeUpGain.Value := MakeUp;
    LbMakeUpGainValue.Caption := ParameterDisplay[10] + ' ' +
      ParameterLabel[10];
    GuiGraphXY.UpdateGraph;
  end;
end;
...
begin
  ...
  begin
    ...
    if Brightness <> LEDSideChain.Brightness_Percent then
      LEDSideChain.Brightness_Percent := Brightness;
  end;
end;
...
begin
  ...
  begin
    if VstPlugIn.Active then
      LbVstPluginValue.Caption := VstPlugIn.GetFriendlyNameString(45)
    else
    begin
      if VstPlugIn.numInputs <> 2 then
        LbVstPluginValue.Caption := 'input channel mismatch'
      else if VstPlugIn.numOutputs <> 2 then
        LbVstPluginValue.Caption := 'output channel mismatch'
      else
        LbVstPluginValue.Caption := '(double click to load)';
    end;
  end;

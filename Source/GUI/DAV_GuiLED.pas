...
{$ENDIF}
  ...
  public
    ...
    property BorderStrength_Percent: Single read GetBorderStrength
      write SetBorderStrength;
...
uses
  ExtCtrls, Math, DAV_Math, DAV_Common, DAV_Complex, DAV_Approximations,
  DAV_GuiBlend;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            CombColor := CombinePixel(LEDColor, CBlack,
              Round(FBorderFactor * Scale * $FF))
          else
          begin
        ...
        begin
          ...
          begin
            ...
            Scale := (Temp + (1 - Temp) * FBorderFactor) * 0.5 *
              FBorderWidth * Scale;
            CombAlpha := Round($FF * (Radius - FastSqrtBab2(SqrDist)));
            CombColor := CombinePixel(CBlack, CombColor,
              Round((1 - Scale) * $FF));
            CombColor.A := CombAlpha;
...
procedure TCustomGuiLED.CMColorChanged(var Message:
...
procedure TCustomGuiLED.CMEnabledChanged(var Message:
  {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF});
begin
  inherited;
  BufferChanged;

...
type
  ...
  TProcessSampleSplit32 = procedure(out Low, High: Single;
    Input: TDAV2SingleArray) of object;
  TProcessSampleSplit64 = procedure(out Low, High: Double;
    Input: TDAV2DoubleArray) of object;
  ...
  private
    ...
    procedure ProcessSampleSplit1(out Low, High: Single;
      Input: TDAV2SingleArray);
    procedure ProcessSampleSplit2(out Low, High: Single;
      Input: TDAV2SingleArray);
    procedure ProcessSampleSplit3(out Low, High: Single;
      Input: TDAV2SingleArray);
    procedure ProcessSampleSplit4(out Low, High: Single;
      Input: TDAV2SingleArray);
    procedure ProcessSampleSplitLarge(out Low, High: Single;
      Input: TDAV2SingleArray);
  protected
  ...
  public
    ...
    procedure ProcessBlockSplit(const OutputL, OutputH,
      Input: PDAVSingleFixedArray; SampleFrames: Integer);
    ...
    property ProcessSampleSplit: TProcessSampleSplit32
      read FProcessSampleSplit32;
  end;
  ...
  private
    ...
    procedure ProcessSampleSplit1(out Low, High: Double;
      Input: TDAV2DoubleArray);
    procedure ProcessSampleSplit2(out Low, High: Double;
      Input: TDAV2DoubleArray);
    procedure ProcessSampleSplit3(out Low, High: Double;
      Input: TDAV2DoubleArray);
    procedure ProcessSampleSplit4(out Low, High: Double;
      Input: TDAV2DoubleArray);
    procedure ProcessSampleSplitLarge(out Low, High: Double;
      Input: TDAV2DoubleArray);
  protected
    procedure AssignTo(Dest: TPersistent); override;
  ...
  public
    ...
    procedure ProcessBlock(const Input, Output: PDAVDoubleFixedArray;
      SampleFrames: Integer);
    procedure ProcessBlockSplit(const OutputL, OutputH,
      Input: PDAVDoubleFixedArray; SampleFrames: Integer);
    ...
    property ProcessSampleSplit: TProcessSampleSplit64
      read FProcessSampleSplit64;
  end;
...
constructor TCustomPolyphaseDownsampler.Create(const NumberOfCoefficients
  : Integer; const Transition: Double);
begin
  inherited;
  AllocateBuffer;
  ChooseProcedures;
...
begin
  ...
  if Dest is TPolyphaseDownsampler32 then
    ...
    begin
      ...
      Move(Self.FStateStack^, FStateStack^, 2 * FNumberOfCoeffs *
        SizeOf(Single));
    end
  else if Dest is TPolyphaseDownsampler64 then
...
function TPolyphaseDownsampler32.ProcessSample1(const Input
  : TDAV2SingleArray): Single;
{$IFDEF PUREPASCAL}
begin
  // calculate filter
...
function TPolyphaseDownsampler32.ProcessSample2(const Input
  : TDAV2SingleArray): Single;
{$IFDEF PUREPASCAL}
var
...
function TPolyphaseDownsampler32.ProcessSample3(const Input
  : TDAV2SingleArray): Single;
{$IFDEF PUREPASCAL}
var
...
function TPolyphaseDownsampler32.ProcessSample4(const Input
  : TDAV2SingleArray): Single;
{$IFDEF PUREPASCAL}
var
...
function TPolyphaseDownsampler32.ProcessSampleLarge
...
begin
  ...
  begin
    LocalY[Index] := (LocalY[Index - 2] - LocalY[Index]) * Coeffs[Index] +
      LocalX[Index];
    LocalX[Index] := LocalY[Index - 2];
  end;

  // calculate result
  Result := CHalf32 * (LocalY[FNumberOfCoeffs - 1] +
    LocalY[FNumberOfCoeffs - 2]);
{$ELSE}
asm
  PUSH    EBX
  PUSH    ESI
...
procedure TPolyphaseDownsampler32.ProcessSampleSplit1(out Low, High: Single;
  Input: TDAV2SingleArray);
begin
  FY[0] := (Input[1] - FY[0]) * FCoefficients[0] + FX[0];
  FX[0] := Input[1];
...
procedure TPolyphaseDownsampler32.ProcessSampleSplit2(out Low, High: Single;
  Input: TDAV2SingleArray);
begin
  FY[0] := (Input[1] - FY[0]) * FCoefficients[0] + FX[0];
  FX[0] := Input[1];
...
procedure TPolyphaseDownsampler32.ProcessSampleSplit3(out Low, High: Single;
  Input: TDAV2SingleArray);
begin
  ...
  PDAV4SingleArray(FY)^[2] := (PDAV4SingleArray(FY)^[0] - PDAV4SingleArray(FY)
    ^[2]) * PDAV4DoubleArray(FCoefficients)^[2] + PDAV4SingleArray(FX)^[2];
...
procedure TPolyphaseDownsampler32.ProcessSampleSplit4(out Low, High: Single;
  Input: TDAV2SingleArray);
begin
  FY[0] := (Input[1] - FY[0]) * PDAV4DoubleArray(FCoefficients)^[0] + FX[0];
  FX[0] := Input[1];
  ...
  PDAV4SingleArray(FY)^[2] := (PDAV4SingleArray(FY)^[0] - PDAV4SingleArray(FY)
    ^[2]) * PDAV4DoubleArray(FCoefficients)^[2] + PDAV4SingleArray(FX)^[2];
  PDAV4SingleArray(FX)^[2] := PDAV4SingleArray(FY)^[0];
  PDAV4SingleArray(FY)^[3] := (PDAV4SingleArray(FY)^[1] - PDAV4SingleArray(FY)
    ^[3]) * PDAV4DoubleArray(FCoefficients)^[3] + PDAV4SingleArray(FX)^[3];
...
procedure TPolyphaseDownsampler32.ProcessSampleSplitLarge(out Low, High: Single;
  Input: TDAV2SingleArray);
var
  Index: Integer;
begin
...
procedure TPolyphaseDownsampler32.ProcessBlockSplit(const OutputL, OutputH,
  Input: PDAVSingleFixedArray; SampleFrames: Integer);
var
  Index: Integer;
begin
  ...
  Index := 0;
  repeat
    ProcessSampleSplit(OutputL[Index], OutputH[Index],
      PDAV2SingleArray(@Input[Index * 2])^);
    Inc(Index);
...
procedure TPolyphaseDownsampler32.ProcessBlock(const Input,
  Output: PDAVSingleFixedArray; SampleFrames: Integer);
var
  Index: Integer;
begin
  for Index := 0 to SampleFrames - 1 do
    Output[Index] := FProcessSample32(PDAV2SingleArray(@Input[Index * 2])^);
end;

function TPolyphaseDownsampler32.ProcessSample32(const Input
  : TDAV2SingleArray): Single;
begin
...
begin
  ...
  else if Dest is TPolyphaseDownsampler32 then
    with TPolyphaseDownsampler32(Dest) do
    begin
      inherited;
      Assert(FNumberOfCoeffs = Self.FNumberOfCoeffs);
...
function TPolyphaseDownsampler64.ProcessSample1(const Input
  : TDAV2DoubleArray): Double;
{$IFDEF PUREPASCAL}
begin
  FY[0] := (Input[1] - FY[0]) * FCoefficients[0] + FX[0];
  FX[0] := Input[1];
...
function TPolyphaseDownsampler64.ProcessSample2(const Input
  : TDAV2DoubleArray): Double;
{$IFDEF PUREPASCAL}
var
...
function TPolyphaseDownsampler64.ProcessSample3(const Input
  : TDAV2DoubleArray): Double;
{$IFDEF PUREPASCAL}
var
...
function TPolyphaseDownsampler64.ProcessSample4(const Input
  : TDAV2DoubleArray): Double;
{$IFDEF PUREPASCAL}
var
...
procedure TPolyphaseDownsampler64.ProcessBlock(const Input,
  Output: PDAVDoubleFixedArray; SampleFrames: Integer);
var
  SampleIndex: Integer;
begin
  for SampleIndex := 0 to SampleFrames - 1 do
    Output[SampleIndex] := FProcessSample64
      (PDAV2DoubleArray(@Input[SampleIndex * 2])^);
end;
...
procedure TPolyphaseDownsampler64.ProcessSampleSplit1(out Low, High: Double;
  Input: TDAV2DoubleArray);
begin
  FY[0] := (Input[1] - FY[0]) * FCoefficients[0] + FX[0];
  FX[0] := Input[1];
...
procedure TPolyphaseDownsampler64.ProcessSampleSplit2(out Low, High: Double;
  Input: TDAV2DoubleArray);
begin
  FY[0] := (Input[1] - FY[0]) * FCoefficients[0] + FX[0];
  FX[0] := Input[1];
...
procedure TPolyphaseDownsampler64.ProcessSampleSplit3(out Low, High: Double;
  Input: TDAV2DoubleArray);
begin
  FY[0] := (Input[1] - FY[0]) * FCoefficients[0] + FX[0];
  FX[0] := Input[1];
  ...
  PDAV4DoubleArray(FY)^[2] := (PDAV4DoubleArray(FY)^[0] - PDAV4DoubleArray(FY)
    ^[2]) * PDAV4DoubleArray(FCoefficients)^[2] + PDAV4DoubleArray(FX)^[2];
...
procedure TPolyphaseDownsampler64.ProcessSampleSplit4(out Low, High: Double;
  Input: TDAV2DoubleArray);
begin
  FY[0] := (Input[1] - FY[0]) * FCoefficients[0] + FX[0];
  FX[0] := Input[1];
  ...
  PDAV4DoubleArray(FY)^[2] := (PDAV4DoubleArray(FY)^[0] - PDAV4DoubleArray(FY)
    ^[2]) * PDAV4DoubleArray(FCoefficients)^[2] + PDAV4DoubleArray(FX)^[2];
  PDAV4DoubleArray(FX)^[2] := PDAV4DoubleArray(FY)^[0];
  PDAV4DoubleArray(FY)^[3] := (PDAV4DoubleArray(FY)^[1] - PDAV4DoubleArray(FY)
    ^[3]) * PDAV4DoubleArray(FCoefficients)^[3] + PDAV4DoubleArray(FX)^[3];
...
procedure TPolyphaseDownsampler64.ProcessSampleSplitLarge(out Low, High: Double;
  Input: TDAV2DoubleArray);
var
  Index: Integer;
begin
...
procedure TPolyphaseDownsampler64.ProcessBlockSplit(const OutputL, OutputH,
  Input: PDAVDoubleFixedArray; SampleFrames: Integer);
var
  Pos: Integer;
begin
  ...
  Pos := 0;
  repeat
    ProcessSampleSplit(OutputL[Pos], OutputH[Pos],
      PDAV2DoubleArray(@Input[Pos * 2])^);
    Inc(Pos);
  until (Pos >= SampleFrames);
end;

function TPolyphaseDownsampler64.ProcessSample64(const Input
  : TDAV2DoubleArray): Double;
begin

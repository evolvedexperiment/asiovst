...
{$IFDEF DELPHI10_UP} {$REGION 'Dial Renderer'} {$ENDIF}
  ...
  published
    ...
    property Transparency: TGuiDialPrimitiveImageTransparency read FTransparency
      write SetTransparency default itAlphaLayer;
    property IntelligentAlpha: Byte read FIntelligentAlpha
      write SetIntelligentAlpha default 1;
    property AutoFitToRect: Boolean read FAutoFitToRect write SetAutoFitToRect
      default False;
  ...
  published
    ...
    property LightDirection: Single read FLightDirection
  ...
  protected
    ...
    procedure SetItem(Index: Integer;
      const Value: TGuiDialLayerCollectionItem); virtual;
    procedure Notify(Item: TCollectionItem;
      Action: TCollectionNotification); override;
    property Items[Index: Integer]: TGuiDialLayerCollectionItem read GetItem
  ...
  published
    ...
    property PrimitiveClassName: string read GetPrimitiveClassName
      write SetPrimitiveClassName;
    property Primitive: TCustomGuiDialPrimitive read FPrimitive
      write SetPrimitive;
    property PrimitiveClassChanged: TNotifyEvent read FPrimitiveClassChanged
      write FPrimitiveClassChanged;
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
  end;
  ...
  protected
    property Items[Index: Integer]: TGuiDialLayerCollectionItem
...
function TGuiDialLayerCollection.GetItem(Index: Integer)
  : TGuiDialLayerCollectionItem;
begin
  Result := TGuiDialLayerCollectionItem(inherited GetItem(Index));
end;

function TGuiDialLayerCollection.Insert(Index: Integer)
  : TGuiDialLayerCollectionItem;
begin
...
procedure TGuiDialLayerCollectionItem.SetPrimitive
  (const Value: TCustomGuiDialPrimitive);
begin
  FPrimitive.Assign(Value);
end;

procedure TGuiDialLayerCollectionItem.SetPrimitiveClassName
  (const Value: string);
var
  PrimitiveClass: TCustomGuiDialPrimitiveClass;
begin
  if (Value <> '') and (FPrimitive.ClassName <> Value) and
    Assigned(PrimitiveClassList) then
  begin
    PrimitiveClass := TCustomGuiDialPrimitiveClass
      (PrimitiveClassList.Find(Value));
...
procedure TGuiDialPrimitiveImage.SetTransparency(const Value
  : TGuiDialPrimitiveImageTransparency);
begin
...
function TGuiDialImageRenderer.GetItems(Index: Integer)
  : TGuiDialLayerCollectionItem;
begin
...
begin
  if not Assigned(PrimitiveClassList) then
    PrimitiveClassList := TClassList.Create;
...
begin
  if Assigned(FRenderer) then
    FRenderer.RenderDial(FDialBitmap, GlyphCount)
  else
    with FDialBitmap.Canvas do
      FillRect(ClipRect);
end;

...
begin
  for Channel := 0 to Length(FRenaissanceBass) - 1 do
    FreeAndNil(FRenaissanceBass[Channel]);
end;
...
begin
  if Parameter[Index] = 0 then
    ...
    PreDefined := FloatToStrF(RoundTo(Amp_to_dB(Parameter[Index]), -2), ffGeneral, 3, 3);
end;
...
begin
  ...
  try
    for Channel := 0 to Length(FRenaissanceBass) - 1 do
      if Assigned(FRenaissanceBass[Channel]) then
        FRenaissanceBass[Channel].AddOriginalBass := Boolean(Round(Value));
  finally
    FCriticalSection.Release;
  end;
end;

procedure TResurrectionBassCloneModule.ParameterIntensityChange(Sender: TObject;
...
begin
  ...
  try
    for Channel := 0 to Length(FRenaissanceBass) - 1 do
      if Assigned(FRenaissanceBass[Channel]) then
        FRenaissanceBass[Channel].Intensity := Value;
  finally
    FCriticalSection.Release;
  end;
end;

procedure TResurrectionBassCloneModule.ParameterGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  ...
  try
    for Channel := 0 to Length(FRenaissanceBass) - 1 do
      if Assigned(FRenaissanceBass[Channel]) then
        FRenaissanceBass[Channel].Gain := Value;
  finally
    FCriticalSection.Release;
  end;
end;

procedure TResurrectionBassCloneModule.ParameterFrequencyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  ...
  try
    for Channel := 0 to Length(FRenaissanceBass) - 1 do
      if Assigned(FRenaissanceBass[Channel]) then
        FRenaissanceBass[Channel].Frequency := Value;
  finally
    FCriticalSection.Release;
  end;
end;

procedure TResurrectionBassCloneModule.VSTModuleSampleRateChange
...
begin
  ...
  try
    if Abs(SampleRate) > 0 then
      for Channel := 0 to Length(FRenaissanceBass) - 1 do
        if Assigned(FRenaissanceBass[Channel]) then
          FRenaissanceBass[Channel].SampleRate := Abs(SampleRate);
  finally
    FCriticalSection.Release;
  end;
end;
  ...
  try
    for Channel := 0 to Length(FRenaissanceBass) - 1 do
      for Sample := 0 to SampleFrames - 1 do
        Outputs[Channel, Sample] := FRenaissanceBass[Channel].ProcessSample32(Inputs[Channel, Sample]);
  finally
    FCriticalSection.Release;
  end;
end;

procedure TResurrectionBassCloneModule.VSTModuleProcessDoubleReplacing

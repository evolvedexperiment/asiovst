...
type
  ...
  TSEChorusPins = (pinInput, pinOutput, pinStages, pinDepth, pinSpeed,
    pinDrift, pinMix);
...
constructor TSEChorusModule.Create(SEAudioMaster: TSE2audioMasterCallback;
  Reserved: Pointer);
begin
...
begin
  ...
  for Sample := 0 to SampleFrames - 1 do
    Outp^[Sample] := FChorus.ProcessSample32(Inp^[Sample]);
end;

procedure TSEChorusModule.SubProcessStatic(const BufferOffset,
...
class procedure TSEChorusModule.GetModuleProperties
...
function TSEChorusModule.GetPinProperties(const Index: Integer;
...
begin
  ...
  case TSEChorusPins(CurrentPin.PinID) of
    ...
    pinStages:
      FChorus.Stages := FStages;
    pinDepth:
      FChorus.Depth := FDepth;
    pinSpeed:
      FChorus.Speed := FSpeed;
    pinDrift:
      FChorus.Drift := FDrift;
    pinMix:
      FChorus.Mix := FMix;
  end;
end;
...
constructor TSEStkChorusModule.Create(SEAudioMaster: TSE2audioMasterCallback;
  Reserved: Pointer);
begin
  inherited Create(SEAudioMaster, Reserved);
...
procedure TSEStkChorusModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
var
  Inp: PDAVSingleFixedArray;
...
procedure TSEStkChorusModule.SubProcessStatic(const BufferOffset,
  SampleFrames: Integer);
begin
  SubProcess(BufferOffset, SampleFrames);
  FStaticCount := FStaticCount - SampleFrames;
...
// describe your module
class procedure TSEStkChorusModule.GetModuleProperties
  (Properties: PSEModuleProperties);
begin
  with Properties^ do
  begin
...
// describe the pins (plugs)
function TSEStkChorusModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
  Result := True;
...
begin
  ...
  case TSEStkChorusPins(CurrentPin.PinID) of
    ...
    pinStkModDepth:
      FStkChorus.ModDepth := FModDepth;
    pinStkModFreq:
      FStkChorus.ModFrequency := FModFreq;
    pinStkMix:
      FStkChorus.EffectMix := FEffectMix;
  end;
end;

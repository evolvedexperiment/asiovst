...
type
  ...
  public
    ...
    procedure ProcessBlock32(const Data: PDAVSingleFixedArray;
      SampleCount: Integer); virtual; abstract;
  ...
  public
    ...
    property Inverted[Harmonic: Integer]: Boolean read GetInverted
      write SetInverted;
...
uses
  SysUtils, Math, DAV_Common, DAV_Strings, DAV_Math, DAV_Convert;
...
begin
  if Abs(Input) < Parameter then
    Result := Input
  else
  begin
    if Input > 0 then
      Result := Parameter + (1 - Parameter) *
        DAV_Math.Tanh((Input - Parameter) / (1 - Parameter))
    else
      Result := -(Parameter + (1 - Parameter) *
        DAV_Math.Tanh((-Input - Parameter) / (1 - Parameter)));
  end;
...
begin
  ...
  begin
    if Input > 0 then
      Result := Parameter + (1 - Parameter) *
        Sigmoid((Input - Parameter) / ((1 - Parameter) * 1.5))
    else
      Result := -(Parameter + (1 - Parameter) * Sigmoid((-Input - Parameter) /
        ((1 - Parameter) * 1.5)));
  end;
end;
...
begin
  Result := Input * (Abs(Input) + Parameter) /
    (Input * Input + (Parameter - 1) * Abs(Input) + 1);
end;
...
begin
  Result := sign(Input) * power(ArcTan(power(Abs(Input), Parameter)),
    (1 / Parameter));
end;
...
begin
  ...
  if b < Parameter then
    ...
    Result := sign(Input) * (Parameter + 1) * 0.5
  else
  begin
...
begin
  if (power < 0) or (Order < power) then
    Result := 0
  else
    case Order of
      0:
      ...
      Result := 2 * ChebyshevPolynomial(Order - 1, power - 1) -
        ChebyshevPolynomial(Order - 2, power);
    end;
end;
...
begin
  ...
  begin
    ...
    for Term := Order - 1 downto 0 do
      Temp := Temp * Data[Sample] + FChebyshevCoeffs^[Term];
    Data[Sample] := Temp;
  end;
end;
...
begin
  ...
  for Term := FCoeffCount - 2 downto 0 do
    Result := Result * Input + FChebyshevCoeffs^[Term];
{$ELSE}
asm
...
begin
  ...
  begin
    ...
    for Input := 1 to Order - 1 do
      if FGains[Input] <> 0 then
        FChebyshevCoeffs^[Index] := FChebyshevCoeffs^[Index] + FGains[Input] *
          ChebyshevPolynomial(1 + Input, Index);
  end;
  if FBypassDC then
    FChebyshevCoeffs^[0] := 0;
end;
...
begin
  if Value then
    Gain[Harmonic] := -Abs(Gain[Harmonic])
  else
    Gain[Harmonic] := Abs(Gain[Harmonic]);
end;
...
begin
  if FGains[Harmonic] < 0 then
    Gain[Harmonic] := -dB_to_Amp(Value)
  else
    Gain[Harmonic] := dB_to_Amp(Value);
end;
...
begin
  ...
  begin
    ...
    for Term := FCoeffCount - 2 downto 0 do
      Temp := Temp * Sqr(Data[Sample]) + FChebyshevCoeffs^[Term];
    Data[Sample] := Temp;
  end;
end;
...
begin
  Result := FChebyshevCoeffs^[FCoeffCount - 1];
  for Term := FCoeffCount - 2 downto 0 do
    Result := Result * Sqr(Input) + FChebyshevCoeffs^[Term];
  Result := Input * Result;
{$ELSE}
asm
  MOV     ECX, [Self.FCoeffCount]
...
begin
  ...
  begin
    ...
    for Input := 1 to FCoeffCount - 1 do
      if FGains[Input] <> 0 then
        Value := Value + FGains[Input] * ChebyshevPolynomial(2 * Input + 1,
          2 * Index + 1);
...
begin
  for Index := 0 to Order - 1 do
    case Index mod 4 of
      0:
        FGains[Index] := -1 / power(Index + 1, FShape);
      2:
        FGains[Index] := 1 / power(Index + 1, FShape);
    else
      FGains[Index] := 0;

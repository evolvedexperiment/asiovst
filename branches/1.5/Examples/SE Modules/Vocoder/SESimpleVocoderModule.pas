...
type
  ...
  TSESimpleVocoderPins = (pinInput, pinCarrier, pinOutput, pinBandwidth,
...
begin
  ...
  for Sample := 0 to SampleFrames - 1 do
    Outp^[Sample] := FVocoder.ProcessSample(Inp^[Sample], Carrier^[Sample]);
end;

procedure TCustomSESimpleVocoderModule.SubProcessStatic(const BufferOffset,
  SampleFrames: Integer);
begin
...
begin
  if (Pin[Integer(pinInput)].Status = stRun) and
    (Pin[Integer(pinCarrier)].Status = stRun) then
  begin
    OnProcess := SubProcess;
  ...
  begin
    FStaticCount := BlockSize + Round(0.001 * FVocoder.Release *
      FVocoder.SampleRate);
...
class procedure TCustomSESimpleVocoderModule.GetModuleProperties
...
function TCustomSESimpleVocoderModule.GetPinProperties(const Index: Integer;
...
begin
  ...
  case TSESimpleVocoderPins(CurrentPin.PinID) of
    pinInput, pinCarrier:
      ChooseProcess;
    pinBandwidth:
      FVocoder.SynthesisBandwidth := FBandwidth * sqrt(0.5);
  end;
end;
...
// describe the pins (plugs)
function TSESimpleVocoderStaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
...
begin
  ...
  case TSESimpleVocoderPins(CurrentPin.PinID) of
    pinAttack:
      FVocoder.Attack := FAttack;
    pinRelease:
      FVocoder.Release := FRelease;
  end;
end;
...
function TSESimpleVocoderControllableModule.GetPinProperties
  (const Index: Integer; Properties: PSEPinProperties): Boolean;
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSESimpleVocoderPins(index) in [pinBandwidth .. pinRelease] then
    with Properties^ do
      Direction := drIn;
end;
...
function TSESimpleVocoderAutomatableModule.GetPinProperties
  (const Index: Integer; Properties: PSEPinProperties): Boolean;
begin
  Result := inherited GetPinProperties(Index, Properties);
  case TSESimpleVocoderPins(index) of
  ...
  case TSESimpleVocoderPins(CurrentPin.PinID) of
    pinAttack, pinRelease:
      if (Pin[Integer(pinAttack)].Status <> stRun) and
        (Pin[Integer(pinRelease)].Status <> stRun) then
        OnProcess := SubProcess
      else
...
procedure TSESimpleVocoderAutomatableModule.SubProcessAutomated

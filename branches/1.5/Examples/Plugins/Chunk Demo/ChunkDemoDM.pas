...
procedure TChunkDemoDataModule.ParameterBetaChange(Sender: TObject;
...
begin
  ...
  if EditorForm is TFmChunkDemo then
    with TFmChunkDemo(EditorForm) do
      UpdateBeta;
end;

procedure TChunkDemoDataModule.ParameterGammaChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FMix[2] := 0.01 * Value;
  FMix[3] := 1 - FMix[2];
...
procedure TChunkDemoDataModule.ParameterDeltaChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
...
procedure TChunkDemoDataModule.ParameterAlphaChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to CNumChannels - 1 do
    if Assigned(FCrossover[Channel]) then

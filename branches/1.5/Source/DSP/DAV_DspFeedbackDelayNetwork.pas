...
type
  ...
  protected
    ...
    function GetFeedbackMatrix(InputIndex, OutputIndex: Integer): Double;
      virtual; abstract;
    ...
    procedure SetDelaySamples(Index: Integer; const Value: Integer);
      virtual; abstract;
    procedure SetFeedbackMatrix(InputIndex, OutputIndex: Integer;
      const Value: Double); virtual; abstract;
    procedure SetInputVector(Index: Integer; const Value: Double);
      virtual; abstract;
    procedure SetOutputVector(Index: Integer; const Value: Double);
      virtual; abstract;
  public
    constructor Create; virtual; abstract;
    property InputVector[Index: Integer]: Double read GetInputVector
      write SetInputVector;
    property OutputVector[Index: Integer]: Double read GetOutputVector
      write SetOutputVector;
    property DelaySamples[Index: Integer]: Integer read GetDelaySamples
      write SetDelaySamples;
    property FeedbackMatrix[InputIndex, OutputIndex: Integer]: Double
      read GetFeedbackMatrix write SetFeedbackMatrix;
  end;
  ...
  private
    ...
    procedure SetDelayTimes(Index: Integer; const Value: Single);
      virtual; abstract;
  ...
  public
    property DelayTimes[Index: Integer]: Single read GetDelayTimes
      write SetDelayTimes;
    property Samplerate: Single read FSampleRate write SetSamplerate;
  end;

  TFeedbackPathProcessEvent32 = procedure(var FeedbackVector: TDAVVector32)
    of object;
  ...
  protected
    ...
    function GetFeedbackMatrix(InputIndex, OutputIndex: Integer)
      : Double; override;
  ...
  public
    ...
    procedure ProcessStereo(const InLeft, InRight: Single;
      out OutLeft, OutRight: Single);

    property OnProcessFeedbackPath: TFeedbackPathProcessEvent32
      read FOnProcessFeedbackPath write FOnProcessFeedbackPath;
  end;
...
begin
  if Index in [0 .. 3] then
    ...
    begin
      ...
      FillChar(FDelayBuffers[Index]^[FDelaySamples[Index]],
        (Value - FDelaySamples[Index]) * SizeOf(Single), 0);
      FDelaySamples[Index] := Value;
    end
    else if FDelaySamples[Index] > Value then
    begin
      FDelaySamples[Index] := Value;
      ReallocMem(FDelayBuffers[Index], Value * SizeOf(Single));
      if FDelayPos[Index] >= FDelaySamples[Index] then
        FDelayPos[Index] := 0;
    end
    else
  else
...
begin
  ...
  if Assigned(FOnProcessFeedbackPath) then
    FOnProcessFeedbackPath(FeedbackInput);
  ...
  if FDelayPos[0] >= FDelaySamples[0] then
    FDelayPos[0] := 0;
  if FDelayPos[1] >= FDelaySamples[1] then
    FDelayPos[1] := 0;
  if FDelayPos[2] >= FDelaySamples[2] then
    FDelayPos[2] := 0;
  if FDelayPos[3] >= FDelaySamples[3] then
    FDelayPos[3] := 0;
end;
...
begin
  ...
  *)
  ...
  OutRight := FOutputVector[1] * FDelayBuffers[0]^[FDelayPos[0]] + FOutputVector
    [2] * FDelayBuffers[1]^[FDelayPos[1]] + FOutputVector[3] * FDelayBuffers[2]^
    [FDelayPos[2]] + FOutputVector[0] * FDelayBuffers[3]^[FDelayPos[3]];
  ...
  FDelayBuffers[0]^[FDelayPos[0]] := FInputVector[0] * InLeft + FInputVector[1]
    * InRight + FeedbackInput[0];
  FDelayBuffers[1]^[FDelayPos[1]] := FInputVector[1] * InLeft + FInputVector[2]
    * InRight + FeedbackInput[1];
  FDelayBuffers[2]^[FDelayPos[2]] := FInputVector[2] * InLeft + FInputVector[3]
    * InRight + FeedbackInput[2];
  FDelayBuffers[3]^[FDelayPos[3]] := FInputVector[3] * InLeft + FInputVector[0]
    * InRight + FeedbackInput[3];
...
begin
  case Index of
    0 .. 3:
      if FDelayTimes[Index] <> Value then
      begin

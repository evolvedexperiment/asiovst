...
type
  ...
  protected
    ...
    procedure PaintToGraphAntialias(const GraphXY: TCustomGuiGraphXY;
      const PixelMap: TGuiCustomPixelMap); virtual; abstract;
    procedure PaintToGraphDraft(const GraphXY: TCustomGuiGraphXY;
      const PixelMap: TGuiCustomPixelMap); virtual; abstract;
  public
    constructor Create; virtual;
  ...
  protected
    procedure PaintToGraphAntialias(const GraphXY: TCustomGuiGraphXY;
      const PixelMap: TGuiCustomPixelMap); override;
    procedure PaintToGraphDraft(const GraphXY: TCustomGuiGraphXY;
      const PixelMap: TGuiCustomPixelMap); override;
    procedure AssignTo(Dest: TPersistent); override;
  public
    property OnEvaluate: TFunctionEvaluateEvent read FOnEvaluate
      write SetOnEvaluate;
  end;
  ...
  public
    ...
    function AddPoint(Item: TDAVPointSingle): Integer; overload;
      virtual; abstract;
  ...
  published
    ...
    property SeriesClassName: string read GetSeriesClassName
      write SetSeriesClassName;
    property Series: TCustomGuiGraphXYSeries read FSeries write SetSeries;
    property SeriesClassChanged: TNotifyEvent read FSeriesClassChanged
      write FSeriesClassChanged;
  end;
  ...
  protected
    ...
    procedure SetItem(Index: Integer;
      const Value: TGuiGraphXYSeriesCollectionItem); virtual;
    procedure Notify(Item: TCollectionItem;
      Action: TCollectionNotification); override;
    property Items[Index: Integer]: TGuiGraphXYSeriesCollectionItem read GetItem
  ...
  private
    ...
    procedure SetSeriesCollectionItem(Index: Integer;
...
resourcestring
  RCStrCalculateRange = 'The upper value must not be equal to the lower value!'
    + #10#13 + 'If you need to set both values at the same time use' + #10#13 +
...
begin
  ...
  begin
    OldFlags := FFlags;
    FFlags := Value;
    if (cafAutoGranularity in FFlags) xor (cafAutoGranularity in OldFlags) then
      AutoGranularityFlagChanged;
    if (cafAutoExtendBounds in FFlags) xor (cafAutoExtendBounds in OldFlags)
    then
      AutoExtendBoundsFlagChanged;
  end;
end;
...
begin
  if cafAutoGranularity in Flags then
    if CalculateAutoGranularity then
      Changed;
end;
...
begin
  if not(cafAutoExtendBounds in Flags) then
  begin
    if FLower < FMinimum then
      FLower := FMinimum;
    if FUpper > FMaximum then
      FLower := FMaximum;
  end;
end;
...
begin
  ...
  if not(cafAutoExtendBounds in Flags) and (Lower < Minimum) then
    Lower := Minimum;
end;
...
begin
  ...
  if FRange <> 0 then
    ...
    raise Exception.Create(RCStrCalculateRange);
end;
...
begin
  // check, whether lower is outside new minimum and limit if necessary
  if not(cafAutoExtendBounds in Flags) and (Upper > Maximum) then
    Upper := Maximum;
end;
...
begin
  ...
  if (cafAutoExtendBounds in Flags) and (Lower < Minimum) then
    Minimum := Lower;
...
begin
  ...
  if (cafAutoExtendBounds in Flags) and (Upper < Maximum) then
    Maximum := Upper;
...
begin
  ...
  if (cafAutoExtendBounds in Flags) and (Value > Lower) then
    Value := Lower;
...
begin
  // check, whether lower exceed new minimum and limit if necessary
  if (cafAutoExtendBounds in Flags) and (Value < Upper) then
    Value := Upper;
...
begin
  ...
  if not(cafAutoExtendBounds in Flags) and (Value < Minimum) then
    Value := Minimum;
...
begin
  // check, whether new upper exceed maximum and extend if necessary
  if not(cafAutoExtendBounds in Flags) and (Value > Maximum) then
    Value := Maximum;
...
var
  ...
  begin
    if Lower < Upper then
    begin
      if Lower < SolidRange[0] then
        SolidRange[0] := Lower;
      if Upper > SolidRange[1] then
        SolidRange[1] := Upper;
    end;
  end;
...
begin
  if Visible and Assigned(FOnEvaluate) then
    ...
    begin
      ...
      for PtIndex := 0 to Length(YValues) - 1 do
        YValues[PtIndex] := ConvertToFixed24Dot8(Offset.y - Scale.y *
          FOnEvaluate(Self, Offset.X + (PtIndex - IntegerRadiusX) * Scale.X));

      for X := 0 to Width - 1 do
      begin
        // get next value
        YValues[Length(YValues) - 1] :=
          ConvertToFixed24Dot8(Offset.y - Scale.y * FOnEvaluate(Self,
          Offset.X + (X + IntegerRadiusX) * Scale.X));
        ...
        begin
          ...
          begin
            ...
            if y < SolidRange[0] then
              SolidRange[0] := y
            else
            begin
              y := FixedRound(FixedAdd(CurrentValue, Distance));
              if y > SolidRange[1] then
                SolidRange[1] := y;
            end;
          end;
        end;
        ...
        WidthScale := FixedSub(RadiusMinusHalf,
          ConvertToFixed24Dot8(Integer(IntegerRadiusX - 2)));

        if IntegerRadiusY = IntegerRadiusX then
          for LeftRightIdx := 0 to 1 do
          begin
            // set start/end values (left/right)
            YStartPos := PointPtr^[(2 * LeftRightIdx - 1) *
              (IntegerRadiusX - 2)];
            YEndPos := PointPtr^[(2 * LeftRightIdx - 1) * (IntegerRadiusX - 1)];

            // calculate split point
            Distance := FixedSub(YStartPos,
              FixedMul(WidthScale, FixedSub(YStartPos, YEndPos)));
            CurrentValue := FixedAdd(YEndPos,
              ((FixedMul(FixedSub(CFixed24Dot8Half, WidthScale),
              FixedSub(Distance, YEndPos)))));

            y := FixedRound(YStartPos);
            if YStartPos.Fixed <= YEndPos.Fixed then
              if FixedRound(CurrentValue) <= FixedRound(YEndPos) then
                AddToSolidRange(y, FixedRound(CurrentValue))
              else
                AddToSolidRange(y, FixedRound(YEndPos) - 1)
            else if FixedRound(CurrentValue) > FixedRound(YEndPos) then
              AddToSolidRange(FixedRound(CurrentValue) + 1, y)
            else
              AddToSolidRange(FixedRound(YEndPos), y);
          end;
        ...
        begin
          ...
          CurrentValue := FixedAdd(Distance, FixedMul(CFixed24Dot8Half,
            CurrentValue));

          y := FixedRound(YStartPos);
          if YStartPos.Fixed <= YEndPos.Fixed then
            if FixedRound(CurrentValue) < FixedRound(Distance) then
              AddToSolidRange(y, FixedRound(CurrentValue))
            else
              AddToSolidRange(y, FixedRound(Distance) - 1)
          else if FixedRound(CurrentValue) >= FixedRound(Distance) then
            AddToSolidRange(FixedRound(CurrentValue) + 1, y)
          else
            AddToSolidRange(FixedRound(Distance), y);
        end;
        ...
        begin
          ...
          YRange[1] := SolidRange[1];

        for y := YRange[0] to YRange[1] do
          BlendPixelInplace(PxColor, PixelPointer[X, y]^);
        EMMS;

        // shift y-values
        Move(YValues[1], YValues[0], (Length(YValues) - 1) *
          SizeOf(TFixed24Dot8));
      end;
    end;
end;
  ...
  if Visible and Assigned(FOnEvaluate) then
    ...
    begin
      ...
      try
        ...
        // fill additional points
        for PtIndex := 0 to Length(YValues) - 1 do
          YValues[PtIndex] := Offset.y - Scale.y *
            FOnEvaluate(Self, Offset.X + (Trunc(FBorderWidth) + PtIndex -
            IntegerRadiusX) * Scale.X);

        for X := Trunc(FBorderWidth) to Width - Trunc(FBorderWidth) - 1 do
        begin
          // get next value
          YValues[Length(YValues) - 1] := Offset.y - Scale.y *
            FOnEvaluate(Self, Offset.X + (X + IntegerRadiusX) * Scale.X);
          ...
          begin
            if PointPtr[PtIndex] > Mx then
              Mx := PointPtr[PtIndex];
            if PointPtr[PtIndex] < Mn then
              Mn := PointPtr[PtIndex];
            if PointPtr[-PtIndex] > Mx then
              Mx := PointPtr[-PtIndex];
            if PointPtr[-PtIndex] < Mn then
          ...
          begin
            ...
            if CurrentValue - RadiusMinusHalf < YBounds[0] then
              YBounds[0] := Trunc(CurrentValue - RadiusMinusHalf);
            if CurrentValue + RadiusMinusHalf > YBounds[1] then
              YBounds[1] := Ceil(CurrentValue + RadiusMinusHalf);
            CurrentValue := PointPtr[-PtIndex];
            if CurrentValue - RadiusMinusHalf < YBounds[0] then
              YBounds[0] := Trunc(CurrentValue - RadiusMinusHalf);
            if CurrentValue + RadiusMinusHalf > YBounds[1] then
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if Distance > RadiusMinusHalf then
                begin
            ...
            if PtIndex > 0 then
              ...
              begin
                ...
                begin
                  ...
                  else if ((YSrc < YDest) and (PointPtr[PtSgn * (PtIndex + 1)] >
                    YDest)) or
                    ((YSrc > YDest) and (PointPtr[PtSgn * (PtIndex + 1)] <
                    YDest)) then
                    Continue;
                end;
            ...
            if (((YDest >= y) and (YSrc <= y)) or ((YDest <= y) and (YSrc >= y))
              ) and (YSrc <> YDest) then
            begin
          ...
          begin
            for y := Max(0, Round(PointPtr[0])) to Height - 1 do
              if VertLine^[y] < FShadeAlpha then
                VertLine^[y] := FShadeAlpha;
            if Max(0, Round(PointPtr[0])) < YBounds[0] then
              YBounds[0] := Max(0, Round(PointPtr[0]));
            if Height - 1 > YBounds[1] then
              YBounds[1] := Height - 1;
          end;
          ...
          begin
            ...
            CombColor := ApplyAlpha(PixelColor32,
              Round($FF * (1 - Frac(FBorderWidth))));
            if YBounds[0] < Height then
            begin
              BlendPixelInplace(ApplyAlpha(CombColor, VertLine^[YBounds[0] - 1]
                ), PixelPointer[X, YBounds[0] - 1]^);
              EMMS;
            end;
            if YBounds[1] > 0 then
            begin
              BlendPixelInplace(ApplyAlpha(CombColor, VertLine^[YBounds[1]]),
                PixelPointer[X, YBounds[1]]^);
              EMMS;
            end;
            Dec(YBounds[1]);
          end
...
procedure TCustomGuiGraphXYFunctionSeries.SetOnEvaluate
  (const Value: TFunctionEvaluateEvent);
begin
...
class procedure TCustomGuiGraphXYDataSeries.Error(const Msg: string;
  Data: Integer);
{$IFDEF FPC}
begin
{$ELSE}
...
procedure TCustomGuiGraphXYDataSeries.Put(Index: Integer;
  Item: TDAVPointSingle);
var
...
begin
  Result := 0;
  while (Result < FCount) and (Item.X <> FData^[Result].X) and
    (Item.y <> FData^[Result].y) do
    Inc(Result);
...
begin
  repeat
    ...
    repeat
      while SortList^[I].X < P.X do
        Inc(I);
      while SortList^[J].X > P.X do
        Dec(J);
      if I <= J then
      begin
        T := SortList^[I];
...
begin
  Move(FData^[Index + 1], FData^[Index], (FCount - Index - 1) *
    SizeOf(TDAVPointSingle));
...
function TGuiGraphXYSortedDataSeries.AddPoint(Item: TDAVPointSingle): Integer;
var
  I: Integer;
begin
  Result := -1;
  ...
  begin
    ...
    while I < FCount do
    begin
      if FData^[I].X > Item.X then
        Break;
      Inc(I);
    end;
    ReallocMem(FData, (FCount + 1) * SizeOf(TDAVPointSingle));
    if I < FCount then
      System.Move(FData^[I], FData^[I + 1],
        (FCount - I) * SizeOf(TDAVPointSingle));
    FData^[I] := Item;
    Inc(FCount);
  end;
end;
...
begin
  if Assigned(FSeriesClassChanged) then
    FSeriesClassChanged(Self);
end;
...
procedure TGuiGraphXYSeriesCollectionItem.SetSeriesClassName
  (const Value: string);
var
  SeriesClass: TCustomGuiGraphXYSeriesClass;
begin
  if (Value <> '') and (FSeries.ClassName <> Value) and Assigned(SeriesClassList)
  then
  begin
...
function TGuiGraphXYSeriesCollection.GetItem(Index: Integer)
  : TGuiGraphXYSeriesCollectionItem;
begin
  Result := TGuiGraphXYSeriesCollectionItem(inherited GetItem(Index));
end;

function TGuiGraphXYSeriesCollection.Insert(Index: Integer)
  : TGuiGraphXYSeriesCollectionItem;
begin
...
function TCustomGuiGraphXY.GetSeriesCollectionItem(Index: Integer)
  : TGuiGraphXYSeriesCollectionItem;
begin
  if (Index >= 0) and (Index < FSeriesCollection.Count) then
    Result := FSeriesCollection[Index]
  else
    Result := nil;
end;
...
begin
  ...
  begin
    ...
    Radius := ConvertToFixed24Dot8(Min(FBorderRadius, 0.5 * Min(Width,
      Height)) + 1);
    ...
    if RadMinusOne.Fixed < 0 then
      RadMinusOne.Fixed := 0;

    RadMinusBorder.Fixed := Radius.Fixed - BorderWidthFixed.Fixed;
    if RadMinusBorder.Fixed < 0 then
      RadMinusBorder.Fixed := 0;
    SqrRadMinusBorder := FixedSqr(RadMinusBorder);

    SqrRadMinusBorderOne.Fixed := RadMinusBorder.Fixed - CFixed24Dot8One.Fixed;
    if SqrRadMinusBorderOne.Fixed < 0 then
      SqrRadMinusBorderOne.Fixed := 0
    else
      SqrRadMinusBorderOne := FixedSqr(SqrRadMinusBorderOne);

    SqrRadMinusOne.Fixed := Radius.Fixed - CFixed24Dot8One.Fixed;
    if SqrRadMinusOne.Fixed < 0 then
      SqrRadMinusOne.Fixed := 0
    else
      SqrRadMinusOne := FixedSqr(SqrRadMinusOne);
    ...
    begin
      ...
      if XStart.Fixed <= 0 then
        Continue
      else
        XStart.Fixed := FixedSqrt(XStart).Fixed - CFixed24Dot8Half.Fixed;
      ScnLne[0] := Scanline[y];
      ...
      XRange[1] := FixedRound
        (FixedSub(ConvertToFixed24Dot8(Integer(Width - 1)), Temp));
      for X := XRange[0] to XRange[1] do
      begin
        XFixed := ConvertToFixed24Dot8(X);

        // calculate squared distance
        if XFixed.Fixed < RadMinusOne.Fixed then
          SqrDist.Fixed := FixedSqr(FixedSub(XFixed, RadMinusOne)).Fixed +
            SqrYDist.Fixed
        else
        begin
          Temp.Fixed := ConvertToFixed24Dot8(Integer(Width - 1)).Fixed -
            RadMinusOne.Fixed;
          if XFixed.Fixed > Temp.Fixed then
            SqrDist.Fixed := FixedSqr(FixedSub(XFixed, Temp)).Fixed +
              SqrYDist.Fixed
          else
            SqrDist := SqrYDist;
        end;

        if SqrDist.Fixed < SqrRadMinusBorderOne.Fixed then
          CombColor := PanelColor
        else if SqrDist.Fixed < SqrRadMinusBorder.Fixed then
        begin
          Temp.Fixed := RadMinusBorder.Fixed - FixedSqrt(SqrDist).Fixed;
          Assert(Temp.Fixed >= 0);
          if Temp.Fixed > $FF then
            CombColor := PanelColor
          else
            CombColor := CombinePixel(BorderColor, PanelColor,
              Round($FF - Temp.Fixed));
        end
        else if SqrDist.Fixed < SqrRadMinusOne.Fixed then
    ...
    begin
      ...
      if (YFixed.Fixed < BorderWidthFixed.Fixed - CFixed24Dot8One.Fixed) or
        (YFixed.Fixed > ConvertToFixed24Dot8(Height).Fixed -
        BorderWidthFixed.Fixed) then
      begin
      ...
      if IsUpperLowerHalf then
        if y < Height div 2 then
          YBorderDistance.Fixed := BorderWidthFixed.Fixed - YFixed.Fixed
        else
          YBorderDistance.Fixed := YFixed.Fixed - Temp.Fixed +
            BorderWidthFixed.Fixed
      else
        YBorderDistance.Fixed := 0;
      ...
      begin
        ...
        begin
          if (XFixed.Fixed <= BorderWidthFixed.Fixed - CFixed24Dot8One.Fixed) or
            (XFixed.Fixed >= ConvertToFixed24Dot8(Width).Fixed -
            BorderWidthFixed.Fixed) then
            CombColor := BorderColor
          else if (XFixed.Fixed < BorderWidthFixed.Fixed) then
          begin
          ...
          else if (XFixed.Fixed >= WidthMinusOne.Fixed - BorderWidthFixed.Fixed)
          then
          begin
            Temp.Fixed := XFixed.Fixed + BorderWidthFixed.Fixed -
              WidthMinusOne.Fixed;
            Temp := FixedMul(Temp, FixedSub(CFixed24Dot8One, YBorderDistance));
            Temp.Fixed := YBorderDistance.Fixed + Temp.Fixed;
            Assert(Temp.Fixed >= 0);
        ...
        else if (XFixed.Fixed <= BorderWidthFixed.Fixed - CFixed24Dot8One.Fixed)
          or (XFixed.Fixed >= WidthMinusOne.Fixed - BorderWidthFixed.Fixed +
          CFixed24Dot8One.Fixed) then
          CombColor := BorderColor
        else if (XFixed.Fixed < BorderWidthFixed.Fixed) then
        begin
          Temp.Fixed := BorderWidthFixed.Fixed - XFixed.Fixed;
        ...
        then
        begin
          Temp.Fixed := XFixed.Fixed - WidthMinusOne.Fixed +
            BorderWidthFixed.Fixed;
          Assert(Temp.Fixed >= 0);
          Assert(Temp.Fixed <= $FF);
          CombColor := CombinePixel(BorderColor, PanelColor, Temp.Fixed);
...
begin
  ...
  begin
    ...
    while c < 1 do
    begin
      if Abs(c - FZeroPosition) < 0.1 * NormGran then
        PixelColor32 := ConvertColor(FFrameColor)
      else
        PixelColor32 := ConvertColor(FGridColor);

      PixelMap.VerticalLine(Rct.Left + Round(c * PixelRange), Rct.Top,
        Rct.Bottom, PixelColor32);

      CombColor32 := ApplyAlpha(PixelColor32,
        Round($FF * (1 - Frac(FBorderWidth))));
  ...
  with YAxis do
    ...
    begin
      ...
      begin
        ...
        PixelMap.HorizontalLine(Rct.Left, Rct.Right,
          Rct.Bottom - Round(c * PixelRange), PixelColor32);

        CombColor32 := ApplyAlpha(PixelColor32,
          Round($FF * (1 - Frac(FBorderWidth))));
        BlendPixelInplace(CombColor32, PixelMap.PixelPointer[Rct.Left - 1,
          Rct.Bottom - Round(c * PixelRange)]^);
        BlendPixelInplace(CombColor32, PixelMap.PixelPointer[Rct.Right,
          Rct.Bottom - Round(c * PixelRange)]^);
...
begin
  if FAntiAlias then
    for SeriesIndex := 0 to FSeriesCollection.Count - 1 do
      if Assigned(FSeriesCollection[SeriesIndex].FSeries) then
        FSeriesCollection[SeriesIndex].FSeries.PaintToGraphAntialias(Self,
          PixelMap)
      else
    else
      for SeriesIndex := 0 to FSeriesCollection.Count - 1 do
        if Assigned(FSeriesCollection[SeriesIndex].FSeries) then
          FSeriesCollection[SeriesIndex].FSeries.PaintToGraphDraft
            (Self, PixelMap)
end;
...
procedure TCustomGuiGraphXY.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, y: Integer);
begin
  inherited;
end;
...
begin
  ...
  begin
    OldFlags := FFlags;
    FFlags := Value;
    if (gfShowLabels in FFlags) xor (gfShowLabels in OldFlags) then
      ShowLabelsChanged;
  end;
end;
...
begin
  ...
  else if (Index = FSeriesCollection.Count) then
  begin
...
begin
  if not Assigned(SeriesClassList) then
    SeriesClassList := TClassList.Create;

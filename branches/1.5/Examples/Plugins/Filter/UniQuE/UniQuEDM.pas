...
uses
  DAV_Common, DAV_StringConvert, DAV_VSTCustomModule, UniQuEGUI;
...
procedure TUniQuEDataModule.ParamPowerDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := AnsiString(OnOff(Parameter[Index]));
end;

procedure TUniQuEDataModule.ParamPresChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Gain, Frequency: Single;
begin
  if Value > 0 then
    Gain := Value * 11.46 / CGainRange
  else
    Gain := Value * 12.96 / CGainRange;

  if Assigned(FPres[0]) then
    FPres[0].Gain := Gain;
  if Assigned(FPres[1]) then
    FPres[1].Gain := Gain;

  Frequency := 7278 + 108 * Value / CGainRange;

  if Assigned(FPres[0]) then
    FPres[0].Frequency := Frequency;
  if Assigned(FPres[1]) then
    FPres[1].Frequency := Frequency;

  // update GUI
  if Assigned(EditorForm) then
    with TFmUniQuE(EditorForm) do
      UpdatePres;
end;

procedure TUniQuEDataModule.ParamPhaseChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  UpdateVolume;
  if Assigned(EditorForm) then
...
procedure TUniQuEDataModule.ParamPowerChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FFade[0] := Value;
  FFade[1] := 1 - Value;
...
procedure TUniQuEDataModule.ParamHigh(Sender: TObject; const Index: Integer;
  var Value: Single);
var
  Gain, Frequency: Single;
begin
  if Value > 0 then
    Gain := Value * 15 / CGainRange
  else
    Gain := Value * 13.5 / CGainRange;

  if Assigned(FHigh[0]) then
    FHigh[0].Gain := Gain;
  if Assigned(FHigh[1]) then
    FHigh[1].Gain := Gain;

  Frequency := 4340 - 300 * Value / CGainRange;

  if Assigned(FHigh[0]) then
    FHigh[0].Frequency := Frequency;
  if Assigned(FHigh[1]) then
    FHigh[1].Frequency := Frequency;
...
procedure TUniQuEDataModule.ParamLowChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Gain: Single;
begin
  if Value > 0 then
    Gain := Value * 11.9 / CGainRange
  else
    Gain := Value * 12.3 / CGainRange;

  if Assigned(FLow[0]) then
    FLow[0].Gain := Gain;
  if Assigned(FLow[1]) then
    FLow[1].Gain := Gain;
...
procedure TUniQuEDataModule.ParamMidChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Gain, BW: Single;
begin
  if Value > 0 then
    Gain := Value * 11.42 / CGainRange
  else
    Gain := Value * 13.35 / CGainRange;

  if Assigned(FMid[0]) then
    FMid[0].Gain := Gain;
  if Assigned(FMid[1]) then
    FMid[1].Gain := Gain;
...
begin
  if Parameter[6] > 0 then
    HighAtt := 4.05 * Parameter[6] / CGainRange
  else
    HighAtt := 1.28 * Parameter[6] / CGainRange;

  if Parameter[2] > 0.5 then
    FVolume := -dB_to_Amp(-Parameter[1] - HighAtt)
  else
    FVolume := dB_to_Amp(-Parameter[1] - HighAtt);
end;

procedure TUniQuEDataModule.ParamPadChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  UpdateVolume;
...
procedure TUniQuEDataModule.ParamPadDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := AnsiString(IntToStr(Round(Parameter[Index])));
end;

procedure TUniQuEDataModule.ParamPhaseDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
...
begin
  for ChannelIndex := 0 to 1 do
  begin
    if Assigned(FLow[ChannelIndex]) then
      FLow[ChannelIndex].ResetStates;
    if Assigned(FMid[ChannelIndex]) then
      FMid[ChannelIndex].ResetStates;
    if Assigned(FPres[ChannelIndex]) then
      FPres[ChannelIndex].ResetStates;
    if Assigned(FHigh[ChannelIndex]) then
      FHigh[ChannelIndex].ResetStates;
  end;
end;
...
begin
  if Abs(SampleRate) > 0 then
    ...
    begin
      if Assigned(FLow[ChannelIndex]) then
        FLow[ChannelIndex].SampleRate := SampleRate;
      if Assigned(FMid[ChannelIndex]) then
        FMid[ChannelIndex].SampleRate := SampleRate;
      if Assigned(FPres[ChannelIndex]) then
        FPres[ChannelIndex].SampleRate := SampleRate;
      if Assigned(FHigh[ChannelIndex]) then
        FHigh[ChannelIndex].SampleRate := SampleRate;
    end;
end;
  ...
  for ChannelIndex := 0 to 1 do
    ...
    begin
      Outputs[ChannelIndex, SampleIndex] := FFade[0] * FVolume *
        ...
        FMid[ChannelIndex].ProcessSample64(FPres[ChannelIndex].ProcessSample64(

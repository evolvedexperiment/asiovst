  ...
  begin
    case FTerms of
      3:
        OnProcess := SubProcessOpt3asm;
      4:
        OnProcess := SubProcessOpt4asm;
      5:
        OnProcess := SubProcessOpt5asm;
      6:
        OnProcess := SubProcessOpt6asm;
      7:
        OnProcess := SubProcessOpt7asm;
    end;
  end;
end;

// The most important part, processing the audio
procedure TSETanhAproximationsModule.SubProcessOpt3asm(const BufferOffset,
...
{$IFDEF PUREPASCAL}
  for Sample := 0 to SampleFrames - 1 do
    Output^[Sample] := FastTanhOpt3Term(Input[Sample]);
{$ELSE}
  for Sample := 0 to SampleFrames - 1 do
    Output^[Sample] := FastTanhOpt3TermFPU(Input[Sample]);
{$ENDIF}
end;

procedure TSETanhAproximationsModule.SubProcessOpt4asm(const BufferOffset,
  SampleFrames: Integer);
var
  Input: PDAVSingleFixedArray;
  Output: PDAVSingleFixedArray;
...
{$IFDEF PUREPASCAL}
  for Sample := 0 to SampleFrames - 1 do
    Output^[Sample] := FastTanhOpt4Term(Input[Sample]);
{$ELSE}
  for Sample := 0 to SampleFrames - 1 do
    Output^[Sample] := FastTanhOpt4TermFPU(Input[Sample]);
{$ENDIF}
end;

procedure TSETanhAproximationsModule.SubProcessOpt5asm(const BufferOffset,
  SampleFrames: Integer);
var
  Input: PDAVSingleFixedArray;
  Output: PDAVSingleFixedArray;
...
{$IFDEF PUREPASCAL}
  for Sample := 0 to SampleFrames - 1 do
    Output^[Sample] := FastTanhOpt5Term(Input[Sample]);
{$ELSE}
  for Sample := 0 to SampleFrames - 1 do
    Output^[Sample] := FastTanhOpt5TermFPU(Input[Sample]);
{$ENDIF}
end;

procedure TSETanhAproximationsModule.SubProcessOpt6asm(const BufferOffset,
  SampleFrames: Integer);
var
  Input: PDAVSingleFixedArray;
  Output: PDAVSingleFixedArray;
...
{$IFDEF PUREPASCAL}
  for Sample := 0 to SampleFrames - 1 do
    Output^[Sample] := FastTanhOpt6Term(Input[Sample]);
{$ELSE}
  for Sample := 0 to SampleFrames - 1 do
    Output^[Sample] := FastTanhOpt6TermFPU(Input[Sample]);
{$ENDIF}
end;

procedure TSETanhAproximationsModule.SubProcessOpt7asm(const BufferOffset,
  SampleFrames: Integer);
var
  Input: PDAVSingleFixedArray;
  Output: PDAVSingleFixedArray;
...
{$IFDEF PUREPASCAL}
  for Sample := 0 to SampleFrames - 1 do
    Output^[Sample] := FastTanhOpt7Term(Input[Sample]);
{$ELSE}
  for Sample := 0 to SampleFrames - 1 do
    Output^[Sample] := FastTanhOpt7TermFPU(Input[Sample]);
{$ENDIF}
end;

// describe your module
class procedure TSETanhAproximationsModule.GetModuleProperties
...
constructor TSETanhAproxModule.Create(AudioMaster: TSE2AudioMasterCallback;
  Reserved: Pointer);
begin
  inherited;
  FTerms := 3;
...
class procedure TSETanhAproxModule.GetModuleProperties
  (Properties: PSEModuleProperties);
begin
  with Properties^ do
  begin
  ...
  begin
    case FTerms of
      3:
        OnProcess := SubProcess2d;
      4:
        OnProcess := SubProcess2c;
      5:
        OnProcess := SubProcess2b;
      6:
        OnProcess := SubProcess2a;
    end;
  end;
  inherited;
end;

procedure TSETanhAproxModule.SubProcess2a(const BufferOffset,
  SampleFrames: Integer);
var
  Input: PDAVSingleFixedArray;
  Output: PDAVSingleFixedArray;
...
begin
  ...
  Output := PDAVSingleFixedArray(@FOutputBuffer[BufferOffset]);
  for Sample := 0 to SampleFrames - 1 do
    Output^[Sample] := FastTanh2Like1Term(Input[Sample]);
end;

procedure TSETanhAproxModule.SubProcess2b(const BufferOffset,
  SampleFrames: Integer);
var
  Input: PDAVSingleFixedArray;
  Output: PDAVSingleFixedArray;
...
begin
  ...
  Output := PDAVSingleFixedArray(@FOutputBuffer[BufferOffset]);
  for Sample := 0 to SampleFrames - 1 do
    Output^[Sample] := FastTanh2Like2Term(Input[Sample]);
end;

procedure TSETanhAproxModule.SubProcess2c(const BufferOffset,
  SampleFrames: Integer);
var
  Input: PDAVSingleFixedArray;
  Output: PDAVSingleFixedArray;
...
begin
  ...
  Output := PDAVSingleFixedArray(@FOutputBuffer[BufferOffset]);
  for Sample := 0 to SampleFrames - 1 do
    Output^[Sample] := FastTanh2Like3Term(Input[Sample]);
end;

procedure TSETanhAproxModule.SubProcess2d(const BufferOffset,
  SampleFrames: Integer);
var
  Input: PDAVSingleFixedArray;
  Output: PDAVSingleFixedArray;
...
begin
  ...
  Output := PDAVSingleFixedArray(@FOutputBuffer[BufferOffset]);
  for Sample := 0 to SampleFrames - 1 do
    Output^[Sample] := FastTanh2Like4Term(Input[Sample]);
end;
...
class procedure TSETanhModule.GetModuleProperties
  (Properties: PSEModuleProperties);
begin
  with Properties^ do
  begin
...
begin
  ...
  Output := PDAVSingleFixedArray(@FOutputBuffer[BufferOffset]);
  for Sample := 0 to SampleFrames - 1 do
    Output^[Sample] := Tanh(Input[Sample]);
end;

...
type
  TFractionalDelayAllpass = class(TDspPersistent, IDspProcessor32,
    IDspProcessor64)
  protected
    FState: Double;
    FFractional: Single;
...
procedure TFractionalDelayAllpass.ProcessBlock32
...
procedure TFractionalDelayAllpass.ProcessBlock64
...
var
  ...
  StageOffset: Double;
begin
  with FPitchShifterStage[0] do
    BaseStage := FBufferOffset + Fractional;
...
begin
  ...
  begin
    ...
    begin
      FStages := Value;
      for i := Length(FPitchShifterStage) - 1 downto FStages do
        FreeAndNil(FPitchShifterStage[i]);
      SetLength(FPitchShifterStage, FStages);
    end
    else
    begin
      SetLength(FPitchShifterStage, Value);
      for i := FStages to Length(FPitchShifterStage) - 1 do
        FPitchShifterStage[i] := TGranularPitchShifterStage.Create;
      FStages := Value;
    end;
    StagesChanged;
  end;
...
begin
  ...
  else if Dest is TDspGranularPitchShifter64 then
...
begin
  ...
  if FBufferSize > OldBufferSize then
    FillChar(FBuffer32^[OldBufferSize], (FBufferSize - OldBufferSize) *
      SizeOf(Single), 0);
end;

procedure TDspGranularPitchShifter32.ProcessBlock32
  (const Data: PDAVSingleFixedArray; SampleCount: Integer);
var
  Sample: Integer;
begin
...
begin
  ...
  for i := 0 to Stages - 1 do
    ...
    begin
      ...
      if FBufferOffset >= FBufferSize then
        FBufferOffset := 0
      else if FBufferOffset < 0 then
        FBufferOffset := FBufferOffset + FBufferSize;
    end;
end;
...
begin
  ...
  if FBufferSize > OldBufferSize then
    FillChar(FBuffer64^[OldBufferSize], (FBufferSize - OldBufferSize) *
      SizeOf(Double), 0);
end;

procedure TDspGranularPitchShifter64.ProcessBlock64
  (const Data: PDAVDoubleFixedArray; SampleCount: Integer);
var
  Sample: Integer;
begin

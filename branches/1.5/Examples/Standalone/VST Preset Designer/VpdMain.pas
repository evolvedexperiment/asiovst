...
begin
  ...
  with TIniFile.Create(FIniFile) do
    try
      ...
      // ASIO driver
      with ASIOHost do
        DriverIndex := DriverList.IndexOf(ReadString('Program',
          'ASIO Driver', ''));
      MiPlayPreview.Enabled := (ASIOHost.DriverIndex >= 0);

      // VST Plugins
      VstFileName := ReadString('Program', 'Last VST Plugin', '');
      if FileExists(VstFileName) then
        LoadVSTPlugin(VstFileName);
...
begin
  ...
  begin
    ...
    with TIniFile.Create(FIniFile) do
      try
        InitialDir := ReadString('Program', 'Preview MIDI Directory',
          InitialDir);
      finally
        Free;
      end;
...
begin
  ...
  begin
    ...
    with TIniFile.Create(FIniFile) do
      try
        InitialDir := ReadString('Program', 'Preview Audio Directory',
          InitialDir);
      finally
        Free;
      end;
...
begin
  ...
  begin
    ...
    with TIniFile.Create(FIniFile) do
      try
        InitialDir := ReadString('Program', 'VST Preset/Bank Directory',
          InitialDir);
      finally
        Free;
      end;
...
begin
  for Param := 0 to VstHost[0].numParams - 1 do
    VstHost[0].Parameter[Param] := Random;
end;
...
begin
  ...
  if InputQuery('Rename', 'Please enter the new program name', ProgName) then
    VstHost[0].ProgramName := AnsiString(ProgName);
  UpdatePrograms;
end;
...
begin
  ...
  begin
    ...
    s := IntToStr(i);
    if (VstHost[0].numPrograms >= 100) and (i < 100) then
      s := '0' + s;
    if (VstHost[0].numPrograms >= 10) and (i < 10) then
      s := '0' + s;
...
begin
  ...
  with VstHost[0] do
  begin
    for Channel := 0 to Length(FInBuffer) - 1 do
      ReallocMem(FInBuffer[Channel], FBufferSize);
    for Channel := 0 to Length(FOutBuffer) - 1 do
      ReallocMem(FOutBuffer[Channel], FBufferSize);
  end;
end;
...
begin
  ...
  begin
    ...
    for Index := 0 to Min(2, ASIOHost.OutputChannelCount) - 1 do
      Move(FOutBuffer[Index]^, OutBuffer[Index]^, ASIOHost.BufferSize *
        SizeOf(Single));
  end
  else
    for Index := 0 to Min(2, ASIOHost.OutputChannelCount) - 1 do
      Move(FInBuffer[Index]^, OutBuffer[Index]^, ASIOHost.BufferSize *
        SizeOf(Single));
end;

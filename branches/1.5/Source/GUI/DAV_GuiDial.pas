...
{$IFDEF DELPHI10_UP} {$REGION 'Custom Dials'} {$ENDIF}
  ...
  public
    ...
    property DialImageIndex: Integer read GetDialImageIndex
      write SetDialImageIndex;
    property DialImageList: TGuiDialImageList read FDialImageList
      write SetDialImageList;
  ...
  protected
    ...
    property NormalizedPosition: Single read GetNormalizedPosition
      write SetNormalizedPosition;
  ...
  public
    ...
    property DefaultPosition: Single read FDefaultPosition
      write SetDefaultPosition;
    property Inertia: Single read FInertia write SetInertia;
    property Max: Single read FMax write SetMax;
    property Min: Single read FMin write SetMin;
    property PointerAngles: TGuiDialPointerAngles read FPointerAngles
      write SetPointerAngles;
    property Position: Single read FPosition write SetPosition;
    property RightMouseButton: TGuiDialRMBFunc read FRightMouseButton
      write FRightMouseButton default rmbfCircular;
    ...
    property OnQuantizeValue: TQuantizeValueEvent read FOnQuantizeValue
      write FOnQuantizeValue;
  end;
  ...
  public
    ...
    property DefaultGlyphNr: Integer read FDefaultGlyphNr
      write SetDefaultGlyphNr;
  ...
  public
    constructor Create(AOwner: TComponent); override;
    property IndicatorLineLength_Percent: Single read FIndLineLength
      write SetIndLineLength;
  end;
...
{$IFDEF DELPHI10_UP} {$REGION 'Dial Image List'} {$ENDIF}
  ...
  protected
    ...
    procedure SetItem(Index: Integer;
      const Value: TGuiDialImageCollectionItem); virtual;
    procedure Notify(Item: TCollectionItem;
      Action: TCollectionNotification); override;
    property Items[Index: Integer]: TGuiDialImageCollectionItem read GetItem
  ...
  protected
    property Items[Index: Integer]: TGuiDialImageCollectionItem
      read GetItems; default;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
  published
    property DialImages: TGuiDialImageCollection read FDialImageCollection
      write FDialImageCollection;
...
begin
  if (Value < 1) or (Value > 360) then
    raise Exception.Create(RCStrPointerAngleRange);

  FRange := Value;
  if FRange > Resolution then
    Resolution := FRange;
  Changed;
end;
...
begin
  if (Value < 0) or (Value > Range) then
    raise Exception.CreateFmt(RCStrPointerRangeResolution, [Range + 1]);
...
begin
  if (Value < 0) or (Value > 359) then
    raise Exception.Create(RCStrPointerAngleStart);
...
begin
  ...
  if (Width > 0) and (Height > 0) then
    ...
    begin
      ...
      then
        OwnerDraw := FDialImageItem.FDialBitmap.Empty;
...
{$IFNDEF FPC} if FTransparent then
        ...
        if (GlyphIndex >= FGlyphCount) then
          GlyphIndex := FGlyphCount - 1
        else if (GlyphIndex < 0) then
          GlyphIndex := 0;

        if Assigned(FDialImageItem) then
          Bmp := FDialImageItem.FDialBitmap
        else
          Bmp := DialBitmap;
        ...
        begin
          ...
          ImageList.Draw(FBuffer.Canvas, 0, 0, GlyphIndex);
      end;
      Unlock;
    end;
end;
...
begin
  ...
  if FDialBitmap.Empty or (FGlyphCount = 0) then
    exit;
...
begin
  ...
  if not Assigned(FDialImageList) then
    exit;

  // limit range to existing dial images
  if Value < 0 then
    Value := 0
  else if Value >= FDialImageList.Count then
    Value := FDialImageList.Count - 1;

  if DialImageIndex <> Value then
  begin
    if Value >= 0 then
      FDialImageList[Value].LinkStitchedControl(Self)
    else
      FDialImageItem.UnLinkStitchedControl(Self);
    FDialImageItem := FDialImageList[Value];
    Invalidate;
  end;
end;

procedure TCustomGuiStitchedControl.SetDialImageList
  (const Value: TGuiDialImageList);
begin
...
begin
  ...
  Filer.DefineProperty('Max', ReadMaxProperty, WriteMaxProperty, Max = 0);
end;
...
begin
  ...
  if Difference < 0 then
    Difference := -Power(Abs(Difference), FInertiaExp) * FInertiaScale
  else
    Difference := Power(Abs(Difference), FInertiaExp) * FInertiaScale;
...
const
  Pi180: Double = PI / 180;
begin
  Result := SafeAngle(PointerAngles.Start + (PointerAngles.Range *
    MapValue(NormalizedPosition))) * Pi180;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      PtsArray[0] := Point(Round(0.5 * Width + Val.Re),
        Round(0.5 * Height + Val.Im));
      ...
      begin
        ...
        Val.Re := tmp;
        PtsArray[i] := Point(Round(0.5 * Width + Val.Re),
          Round(0.5 * Height + Val.Im));
      end;
...
begin
  if Value < 0 then
    Result := -Power(Abs(Value), FCurveMappingExp)
  else
    Result := Power(Abs(Value), FCurveMappingExp);
end;
...
begin
  if FPosition > FMax then
    FPosition := FMax;
  if FDefaultPosition > FMax then
    FDefaultPosition := FMax;
  Invalidate;
end;
...
begin
  if FPosition < FMin then
    FPosition := FMin;
  if FDefaultPosition < FMin then
    FDefaultPosition := FMin;
  Invalidate;
end;
...
begin
  ...
  if Assigned(FOnQuantizeValue) then
    FOnQuantizeValue(Self, NewValue);
...
begin
  Range := Max - (Min - 1);
  Angle := SafeAngle(RelativeAngle(Width div 2, Height div 2, X, Y) -
    PointerAngles.Start);
...
procedure TCustomGuiDial.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  if Enabled then
  begin
    if ssCtrl in Shift then
      Position := FDefaultPosition;
    if (Button = mbRight) and (FRightMouseButton = rmbfReset) then
      Position := FDefaultPosition;
  end;
...
begin
  ...
  if ssShift in Shift then
    NewValue := MapValue(NormalizedPosition) + Difference * 0.1
  else
    NewValue := MapValue(NormalizedPosition) + Difference;
...
begin
  if FRightMouseButton = rmbfCircular then
    Position := CircularMouseToPosition(X, Y);
  inherited;
end;
...
begin
  if (Color and $000000FF) < $80 then
    if (((Color and $0000FF00) shr 8) < $80) or
      (((Color and $00FF0000) shr 16) < $80) then
      FCircleColor := $FFFFFF
    else if (((Color and $0000FF00) shr 8) < $80) and
      (((Color and $00FF0000) shr 16) < $80) then
      FCircleColor := $FFFFFF;
...
begin
  ...
  begin
    if (Button = mbLeft) then
      if FGlyphNr < FGlyphCount - 1 then
        GlyphNr := FGlyphNr + 1
      else
        GlyphNr := 0
...
begin
  inherited;
  if FDefaultGlyphNr >= FGlyphCount then
    DefaultGlyphNr := FGlyphCount - 1;
  if FGlyphNr >= FGlyphCount then
    GlyphNr := FGlyphCount - 1;
end;
...
begin
  ...
  begin
    ...
    if FGlyphNr < FStringList.Count then
      txt := FStringList[FGlyphNr]
    else
      txt := IntToStr(FGlyphNr);
...
begin
  ...
  if FDialBitmap.Empty then
    GlyphCount := Max(1, FStringList.Count);
  Invalidate;
end;
...
{$IFNDEF FPC}
    ...
    begin
      ...
      begin
        ...
        if Round(1.5 + Val) mod 3 = 0 then
          Val := Val * 50 - 99.5
        else
          Val := -Round(99.5 + Val * 50) mod 100;
        if Sqr(Steps - Center.Re) + Sqr(Rad - i) > Sqr(BW * Rad) then
          Val := -$90;
      ...
      Pnt := Point(Round(Center.Re + Cmplx.Re * BW * Rad),
        Round(Center.Im + Cmplx.Im * BW * Rad));
...
function TGuiDialImageCollection.GetItem(Index: Integer)
  : TGuiDialImageCollectionItem;
begin
  Result := TGuiDialImageCollectionItem(inherited GetItem(Index));
end;

function TGuiDialImageCollection.Insert(Index: Integer)
  : TGuiDialImageCollectionItem;
begin
...
procedure TGuiDialImageCollectionItem.LinkStitchedControl
  (Dial: TCustomGuiStitchedControl);
begin
...
procedure TGuiDialImageCollectionItem.UnLinkStitchedControl
  (Dial: TCustomGuiStitchedControl);
begin
...
begin
  for i := 0 to FLinkedDials.Count - 1 do
    TCustomGuiDial(FLinkedDials[i]).GlyphCount := GlyphCount;
end;
...
begin
  for i := 0 to FLinkedDials.Count - 1 do
    TCustomGuiDial(FLinkedDials[i]).StitchKind := StitchKind;
end;

procedure TGuiDialImageCollectionItem.SetGlyphCount(const Value: Integer);
begin
  if Value <= 0 then
    raise Exception.Create(RCStrGlyphCountMustBePositive);
...
procedure TGuiDialImageCollectionItem.SetStitchKind
  (const Value: TGuiStitchKind);
begin
  if FStitchKind <> Value then
  begin
    FStitchKind := Value;
...
function TGuiDialImageList.GetItems(Index: Integer)
  : TGuiDialImageCollectionItem;
begin

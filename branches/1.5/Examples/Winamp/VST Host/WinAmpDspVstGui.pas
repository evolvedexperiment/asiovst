    ...
    try
      if OpenKey(RegistryKey, True) then
        try
          ...
          if not ValueExists('Dispose Preset') then
            WriteBool('Dispose Preset', True);
        finally
          CloseKey;
        end;
      if Assigned(VstHost) and (VstHost[0].EditVisible) then
        VstHost[0].CloseEdit;
    finally
      Free;
    end;
...
begin
  ...
  if (not Assigned(TWinAmpObject(Owner).VstHost)) or
    (not TWinAmpObject(Owner).VstHost[0].EditVisible) then
    exit;
  ...
  begin
    ...
    with BMP do
      try
        ...
        // set font color
        if (R div BMP.Width) + (G div BMP.Width) + (B div BMP.Width) < 384 then
          LbPlugin.Font.Color := clWhite
        else
          LbPlugin.Font.Color := clBlack;
...
begin
  // make sure the VST host exists
  if not Assigned(TWinAmpObject(Owner).VstHost) then
    exit;
  ...
  begin
    if GetVendorString = '' then
      if GetEffectName = '' then
        Caption := ExtractFileName(DLLFileName)
      else
        Caption := GetEffectName
    else
    ...
    try
      ...
      if CBPreset.ItemIndex < 10 then
        s := '00' + s
      else if CBPreset.ItemIndex < 100 then
        s := '0' + s;
    ...
    try
      ...
      begin
        ...
        if CBEnhance.Visible then
          CBPreset.Width := ClientWidth - CBPreset.Left - 8 - CBEnhance.Width
        else
          CBPreset.Width := ClientWidth - CBPreset.Left - 8;
    ...
    begin
      ...
      EdVSTName.Text := Copy(EdVSTName.Text, 0,
        Pos('.dll', EdVSTName.Text) - 1);
    end
    else
      EdVSTName.Text := GetEffectName;
  end;
end;
        ...
        try
          if OpenKey(RegistryKey, False) then
            if ValueExists('Last Plugin') then
              InitialDir := ExtractFilePath(ReadString('Last Plugin'));
        finally
          CloseKey;
          Free;
        end;
        ...
        with TRegistry.Create do
          try
            ...
            begin
              ...
              while i < 10 do
              begin
                if not ValueExists('Recent ' + IntToStr(i)) then
                  break;
                if ReadString('Recent ' + IntToStr(i)) = FileName then
                  break;
                inc(i);
              end;
              if i <= 10 then
...
begin
  if TWinAmpObject(Owner).VstHost[0].DLLFileName <> '' then
...
procedure TFmWinAmpVST.CBPresetDrawItem(Control: TWinControl; Index: Integer;
  ...
  with Control As TComboBox do
    try
      if odSelected in State then
        Canvas.Brush.Color := FColorEdit + $00101010
      else
        Canvas.Brush.Color := FColorEdit;
...
begin
  with TWinAmpObject(Owner), TRegistry.Create do
    try
      if OpenKey(RegistryKey, False) then
        for i := 0 to 9 do
          if ValueExists('Recent ' + IntToStr(i)) then
            DeleteValue('Recent ' + IntToStr(i));
    finally
      CloseKey;
      Free;
    end;
...
begin
  ...
  begin
    ...
    begin
      if VstHost[0].DLLFileName <> '' then
        SetScheme
      else
      begin
        // set font color
        if (PanelControl.Color and $FF) + (PanelControl.Color shr 8 and $FF) +
          (PanelControl.Color shr 16 and $FF) < 384 then
          LbPlugin.Font.Color := clWhite
        else
          LbPlugin.Font.Color := clBlack;
        LbProgram.Font.Color := LbPlugin.Font.Color;

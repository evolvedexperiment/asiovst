...
procedure TCustomSEVocoderModule.SubProcessStatic(const BufferOffset,
  SampleFrames: Integer);
begin
...
class procedure TCustomSEVocoderModule.GetModuleProperties
...
function TCustomSEVocoderModule.GetPinProperties(const Index: Integer;
...
begin
  ...
  case TSEVocoderPins(CurrentPin.PinID) of
    ...
    pinOrder:
      FVocoder.AnalysisOrder := FAnalysisOrder;
  end;
end;
...
// describe the pins (plugs)
function TSEVocoderStaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
...
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSEVocoderPins(index) in [pinBandwidth .. pinRelease] then
    with Properties^ do
      Direction := drIn;
end;
...
procedure TSEVocoderAutomatableModule.PlugStateChange(const CurrentPin: TSEPin);
begin
  case TSEVocoderPins(CurrentPin.PinID) of
    pinAttack, pinRelease:

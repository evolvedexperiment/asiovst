...
procedure TLA1701DataModule.SKLInputChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FLA1701s) then
...
procedure TLA1701DataModule.SKLOutputChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FLA1701s) then
...
procedure TLA1701DataModule.SKLSKFBChange(Sender: TObject; const Index: Integer;
  var Value: Single);
begin
  if Assigned(FLA1701s) then
...
procedure TLA1701DataModule.SKLRatioChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FLA1701s) then
...
procedure TLA1701DataModule.SKLReleaseChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FLA1701s) then
...
procedure TLA1701DataModule.ParamOnOffChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Value < 0.5 then
  begin
...
procedure TLA1701DataModule.ParamAttackLabel(Sender: TObject;
...
begin
  if Assigned(FLA1701s) then
    Result := FLA1701s.GainReductionFactor
  else
    Result := 1;
end;
...
begin
  if Assigned(FLA1701s) then
    Result := FLA1701s.GainReduction_dB
  else
    Result := 0;
end;
...
procedure TLA1701DataModule.ParamVUMeterDisplayChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(EditorForm) then
    with EditorForm as TFmLA1701 do
...
procedure TLA1701DataModule.ParamHPOrderChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
...
procedure TLA1701DataModule.ParamHPOrderDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := AnsiString(IntToStr(Round(Parameter[Index])));
end;

procedure TLA1701DataModule.ParamHPFreqChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FHighpass) then
...
procedure TLA1701DataModule.ParamVUMeterDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
...
procedure TLA1701DataModule.ParamMixChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
...
procedure TLA1701DataModule.ParamAttackDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  if Parameter[Index] < 1 then
...
procedure TLA1701DataModule.SKLAttackChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FLA1701s) then
...
procedure TLA1701DataModule.VSTModuleProcess(const Inputs,
...
begin
  ...
  begin
    ...
    FInLevel := FLevelFallOffFactor *
      (FInLevel + SimpleDiode(Abs(d) - FInLevel));
...
procedure TLA1701DataModule.VSTModuleProcessDoubleReplacing(const Inputs,
...
procedure TLA1701DataModule.VSTModuleProcessBypass(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
begin
...
begin
  ...
  begin
    if Assigned(FLA1701s) then
      FLA1701s.SampleRate := SampleRate;
    if Assigned(FHighpass) then

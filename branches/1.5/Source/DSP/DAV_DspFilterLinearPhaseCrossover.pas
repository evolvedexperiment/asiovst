...
procedure TLinearPhaseCrossover.ProcessSample(const Input: Single;
  out Low, High: Single);
{$IFDEF PUREPASCAL}
var
  Tap: Integer;
begin
  ...
  begin
    ...
    if Tap = FFilterLength div 2 then
      FStates[1]^[Tap - 1] := FStates[1]^[Tap - 1] + Input
  end;
end;
{$ELSE}
asm
...
procedure TLinearPhaseCrossover.ProcessSample(const Input: Double;
  out Low, High: Double);
var
  Tap: Integer;
begin

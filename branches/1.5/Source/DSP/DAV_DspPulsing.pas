...
type
  TCustomPulsing = class(TDspSampleRatePersistent, IDspProcessor32,
    IDspProcessor64)
  private
    FPeriod_s: Single;
    FMaximum_dB: Single;
...
begin
  CalculateSlewRate;
  if FStateIndex = 0 then
    FCurrentSlewRate := FSlewRate
  else
    FCurrentSlewRate := 1 / FSlewRate;
end;
...
begin
  FSlewRate := Power(Exp(FSlewRate_dB_s * 0.11512925464970228420089957273422),
    1 / SampleRate);
end;
...
begin
  ...
  if FCurrentGain > FMaximumGain then
    FCurrentGain := FMaximumGain
  else if FCurrentGain < FMinimumGain then
...
begin
  if Value >= FPeriod_s then
    raise Exception.Create(RCStrLengthTooLong);

...
type
  ...
  public
    ...
    procedure Draw(PixelMap: TGuiCustomPixelMap; Alpha: Byte = $FF);
      overload; virtual;
    procedure Draw(PixelMap: TGuiCustomPixelMap; X, Y: Integer);
      overload; virtual;
    procedure Draw(PixelMap: TGuiCustomPixelMap; X, Y: Integer; Alpha: Byte);
      overload; virtual;
    procedure DrawTransparent(PixelMap: TGuiCustomPixelMap); overload; virtual;
    procedure DrawTransparent(PixelMap: TGuiCustomPixelMap; X, Y: Integer);
      overload; virtual;
{$IFDEF MSWINDOWS}
    procedure DrawByteMap(ByteMap: TGuiCustomByteMap; Color: TPixel32);
      overload; virtual;
    procedure DrawByteMap(ByteMap: TGuiCustomByteMap; Color: TPixel32;
      X, Y: Integer); overload; virtual;
    procedure DrawByteMap(ByteMap: TGuiCustomByteMap; Color: TPixel32;
      Rect: TRect); overload; virtual;
{$ENDIF}
    ...
    procedure HorizontalLine(FromX, ToX, Y: Integer;
      FromColor, ToColor: TPixel32); overload;
...
{$ENDIF}
  ...
  public
    ...
    procedure PaintTo(Canvas: TCanvas; X, Y: Integer); override;
    procedure PaintTo(Canvas: TCanvas; LRect: TRect; X: Integer = 0;
      Y: Integer = 0); override;
    procedure Draw(Bitmap: TBitmap; X, Y: Integer); override;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        NewColor.ARGB := (((ByteLine^[IX - X] * Alpha) shl 16) and
          $FF000000) or RGB;
...
begin
  ...
  begin
    ...
    begin
      ...
      for IX := Left to Right - 1 do
      begin
        NewColor.ARGB := (((ByteLine^[IX - Rect.Left] * Alpha) shl 16) and
          $FF000000) or RGB;
        MergePixelInplace(NewColor, ScnLn^[IX]);
      end;
    end;
...
{$IFDEF UsePixelMap}
        ...
        try
          ...
          IntersectClipRect(Canvas.Handle, 0, 0, Control.Parent.ClientWidth,
...
{$ENDIF}
                ...
                try
                  SaveIndex := SaveDC(Canvas.Handle);
                  SetViewportOrgEx(Canvas.Handle, Left + Pnt.X,
                    Top + Pnt.Y, nil);
...
begin
  ...
  begin
    if Size < SizeOf(TBitmapFileHeader) then
      raise Exception.Create('Invalid bitmap header found!');
...
{$ELSE}
  Result := False;
{$ENDIF}
  if Obj is TGuiCustomPixelMap then
    Result := Equal(TGuiCustomPixelMap(Obj));
end;
...
begin
  ...
  if HasData and (Filer.Ancestor <> nil) then
    HasData := not((Filer.Ancestor is TGuiCustomPixelMap) and
...
procedure TGuiCustomPixelMap.FillRect(Left, Top, Right, Bottom: Integer;
  Color: TPixel32);
var
  X, Y: Integer;
begin
...
procedure TGuiCustomPixelMap.VerticalLine(X, FromY, ToY: Integer;
  Color: TPixel32);
var
  Y: Integer;
begin
...
procedure TGuiCustomPixelMap.Line(FromX, FromY, ToX, ToY: Integer;
  Color: TPixel32);
var
  X, Y, t: Integer;
  dx, dy: Integer;
...
{$IFNDEF FPC}
    case Bitmap.PixelFormat of
      pf32bit:
        ...
        begin
          Move(Bitmap.ScanLine[IndexY]^,
            FDataPointer^[X + (Y + IndexY) * Width],
      ...
      pf24bit:
        ...
        begin
          ...
          begin
            FDataPointer^[X + IndexX + (Y + IndexY) * Width].R :=
              Data24^[IndexX].R;
            FDataPointer^[X + IndexX + (Y + IndexY) * Width].G :=
              Data24^[IndexX].G;
            FDataPointer^[X + IndexX + (Y + IndexY) * Width].B :=
              Data24^[IndexX].B;
...
{$ENDIF}
      if GetDIBits(Bitmap.Canvas.Handle, Bitmap.Handle, 0, Bitmap.Height,
        @FDataPointer^[X + Y * Width], FBitmapInfo, DIB_RGB_COLORS) = 0 then
...
begin
  ...
  begin
    if FOldBitmap <> 0 then
      SelectObject(Handle, FOldBitmap);
    H := Handle;
    Handle := 0;
    DeleteDC(H);
  end;
...
begin
  ...
  begin
    ...
    if FPixelMap.Handle <> 0 then
      FOldBitmap := SelectObject(H, FPixelMap.Handle)
    else
      FOldBitmap := 0;
    Handle := H;
  end;
...
{$IFDEF Darwin}
  ...
  if Status <> noErr then
    raise Exception.Create(RCStrCouldntCreateGenericProfile);

  // Creates a generic color space
  FColorSpace := CGColorSpaceCreateWithPlatformColorSpace(FProfile);

  if FColorSpace = nil then
    raise Exception.Create(RCStrCouldntCreateGenericColorSpace);
{$ENDIF}
end;
...
{$IFDEF MSWINDOWS}
    ...
    if FDataPointer = nil then
      raise Exception.Create(RCStrNoDibHandle);
...
{$IFDEF Darwin}
    ...
    FContext := CGBitmapContextCreate(FDataPointer, FWidth, FHeight, 8,
      4 * FWidth, FColorSpace, kCGImageAlphaNoneSkipFirst or
...
begin
  ...
  if Width * Height <> 0 then
    AllocateDeviceIndependentBitmap;
end;

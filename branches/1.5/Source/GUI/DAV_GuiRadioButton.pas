...
type
  ...
  protected
    ...
    procedure CMEnabledChanged(var Message:
      {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF}); message CM_ENABLEDCHANGED;
    ...
    procedure CMMouseEnter(var Message:
      {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF}); message CM_MOUSEENTER;
    procedure CMMouseLeave(var Message:
      {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF}); message CM_MOUSELEAVE;
    procedure CMParentColorChanged(var Message:
      {$IFDEF FPC}TLMCommand{$ELSE}TWMCommand{$ENDIF});
...
{$ENDIF}
    ...
    procedure BufferChanged(TextChanged: Boolean = True;
      CircleChanged: Boolean = True); virtual;
  ...
  published
    ...
    property ColorBorder: TColor index 3 read FBorderColor write SetColors
      default clWindowText;
    property ColorBackground: TColor index 4 read FBackgroundColor
      write SetColors default clWindow;
...
begin
  ...
  if 2 * FRadioButtonRadius > Height - 2 then
    FRadioButtonRadius := Round(0.5 * (Height - 2));
  if 2 * FRadioButtonRadius > Width - 2 then
    FRadioButtonRadius := Round(0.5 * (Width - 2));
end;
...
procedure TGuiControlsRadioButton.BufferChanged(TextChanged: Boolean = True;
  CircleChanged: Boolean = True);
begin
  if TextChanged then
    FTextChanged := True;
...
procedure TGuiControlsRadioButton.CMColorChanged(var Message:
...
procedure TGuiControlsRadioButton.CMMouseEnter(var Message:
  {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF});
begin
  inherited;
  MouseEnter;
...
procedure TGuiControlsRadioButton.CMMouseLeave(var Message:
  {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF});
begin
  inherited;
  MouseLeave;
...
procedure TGuiControlsRadioButton.CMEnabledChanged(var Message:
  {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF});
begin
  inherited;
...
procedure TGuiControlsRadioButton.CMFontChanged(var Message:
  {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF});
begin
  FGuiFont.Font.Assign(Font);
end;

procedure TGuiControlsRadioButton.CMTextChanged(var Message:
  {$IFDEF FPC}TLMessage{$ELSE}TWmNoParams{$ENDIF});
begin
  inherited;

  if not(csLoading in ComponentState) then
    BufferChanged(True, False);
end;
...
procedure TGuiControlsRadioButton.WMSetFocus(var Message:
  {$IFDEF FPC}TLMSetFocus{$ELSE}TWMSetFocus{$ENDIF});
begin
  inherited;
...
procedure TGuiControlsRadioButton.CMParentColorChanged(var Message:
  {$IFDEF FPC}TLMCommand{$ELSE}TWMCommand{$ENDIF});
begin
  inherited;

  if ParentColor and not FTransparent then
    BackBufferChanged;
end;
...
begin
  ...
  if FMouseIsDown and FMouseInControl then
    Checked := True;
  FFocused := True;
  BufferChanged(False);
end;
...
procedure TGuiControlsRadioButton.MouseDown(Button: TMouseButton;
...
procedure TGuiControlsRadioButton.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if (Button = mbLeft) and Enabled then
  begin
...
begin
  ...
  if FTransparent and (not(csLoading in ComponentState)) then
    BackBufferChanged;
end;

procedure TGuiControlsRadioButton.WMMove(var Message:
  {$IFDEF FPC}TLMMove{$ELSE}TWMMove{$ENDIF});
begin
  inherited;
...
procedure TGuiControlsRadioButton.WMPaint(var Message:
  {$IFDEF FPC}TLMPaint{$ELSE}TWMPaint{$ENDIF});
begin
  if FNative then
    inherited
  else
...
begin
  ...
  begin
    ...
    else if FMouseInControl or Focused then
      BackColor := ConvertColor(FFocusedColor)
    else
      BackColor := ConvertColor(FBackgroundColor);

    BorderWidth := ConvertToFixed24Dot8
      (Max(2.5, 1 + 0.15 * FRadioButtonRadius));

    // draw circle
    Radius := ConvertToFixed24Dot8(FRadioButtonRadius);
    if Radius.Fixed <= 0 then
      Exit;
    ...
    if Temp.Fixed < 0 then
      ...
      SqrRadMinusOne := FixedSqr(Temp);

    Temp := FixedSub(Radius, BorderWidth);
    if Temp.Fixed < 0 then
      SqrRadMinusBorder.Fixed := 0
    else
      SqrRadMinusBorder := FixedSqr(Temp);

    Temp := FixedAdd(Radius, CFixed24Dot8One);
    RadMinusBorderOne := FixedSub(Temp, BorderWidth);
    if RadMinusBorderOne.Fixed < 0 then
      RadMinusBorderOne.Fixed := 0;

    Temp := FixedSub(Radius, InnerOffset);
    if Temp.Fixed < 0 then
      SqrRadMinusInner.Fixed := 0
    else
      SqrRadMinusInner := FixedSqr(Temp);

    RadMinusInnerOne := FixedAdd(FixedSub(Radius, InnerOffset),
      CFixed24Dot8One);
    if RadMinusInnerOne.Fixed < 0 then
      RadMinusInnerOne.Fixed := 0;
...
{$ELSE}
    case Alignment of
      taLeftJustify:
        OffsetX := FixedSub(FixedSub(ConvertToFixed24Dot8(Width), Radius),
          CFixed24Dot8Half);
      taRightJustify:
        OffsetX := FixedAdd(Radius, CFixed24Dot8Half);
    else
      raise Exception.Create('Unknown justify');
    end;
{$ENDIF}
    OffsetY := FixedMul(ConvertToFixed24Dot8(Integer(Height - 1)),
      CFixed24Dot8Half);

    for Y := FixedRound(FixedSub(OffsetY, Radius))
      to FixedRound(FixedAdd(OffsetY, Radius)) do
    begin
      ...
      if XStart.Fixed < 0 then
        ...
        XStart := FixedSub(FixedSqrt(XStart), ConvertToFixed24Dot8(0.4999999));
      ...
      begin
        ...
        SqrDist := FixedAdd(FixedSqr(FixedSub(ConvertToFixed24Dot8(X1), OffsetX)
          ), SqrYDist);
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if not Enabled then
                CombColor.A := CombColor.A shr 1;
              BlendPixelLine(CombColor, @ScnLne[X1], X2 - X1 + 1);
              Break;
            end
            else if (SqrDist.Fixed <= FixedSqr(RadMinusInnerOne).Fixed) then
            begin
...
{$ELSE}
    case Alignment of
      taLeftJustify:
        TextSize.cx := Width - TextSize.cx - 2 * FRadioButtonRadius - 3;
      taRightJustify:
        TextSize.cx := 2 * FRadioButtonRadius + 3;
    end;
{$ENDIF}
...
begin
  ...
  else if FFlatChecked <> Value then
  begin
    FFlatChecked := Value;
    TabStop := Value;
{$IFNDEF FPC}
...
begin
  ...
  begin
    ...
    for Y := 0 to FBuffer.Height - 1 do
    begin
      Move(SrcPtr^, DestPtr^, (FBuffer.Width - (2 * FRadioButtonRadius + 1)) *
        SizeOf(TPixel32));
      SrcPtr := @SrcPtr^[FBuffer.Width];
      DestPtr := @DestPtr^[FBuffer.Width];
    end;

...
uses
  ...
  DAV_DspFilterChebyshevType1, DAV_StringConvert, DAV_DspBesselFilter;
...
{$ENDIF}
  ...
  try
    ...
    begin
      ...
      for Param := 0 to VstHost[0].numParams - 1 do
        with ParameterProperties[FBaseParCount + Param] do
          DisplayName := VstHost[0].GetParamName(Param);

      UniqueID := VstHost[0].UniqueID[1] + VstHost[0].UniqueID[2] +
        VstHost[0].UniqueID[3] + '²';
      EffectName := VstHost[0].EffectName + '²';
      ProductName := VstHost[0].ProductString + '²';
      VendorName := VstHost[0].VendorString +
        ' (powered by Delphi ASIO & VST Packages)';
      ...
      for Prog := 0 to VstHost[0].numPrograms - 1 do
        ...
        begin
          ...
          if PI.CurrentProgram <> Prog then
            PI.CurrentProgram := Prog;
          Parameter[0] := 0;
          Parameter[1] := 2;
          Parameter[2] := 4;
          Parameter[3] := 99;
          ...
          Parameter[6] := 99;
          DisplayName := str;
          for j := 0 to PI.numParams - 1 do
            Parameter[FBaseParCount + j] := PI.Parameter[j];
        end;
      ...
      if (effFlagsCanDoubleReplacing in VstHost[0].EffectOptions) then
      begin
  ...
  for ch := 0 to Length(FUpsampler) - 1 do
    FUpsampler[ch] := TDAVUpsampling.Create;
  SetLength(FDownsampler, numOutputs);
  for ch := 0 to Length(FDownsampler) - 1 do
    FDownsampler[ch] := TDAVDownsampling.Create;
...
{$ENDIF}
  for Channel := 0 to Length(FIn64) - 1 do
    Dispose(FIn64[Channel]);
  for Channel := 0 to Length(FOut64) - 1 do
    Dispose(FOut64[Channel]);

  for Channel := 0 to Length(FUpsampler) - 1 do
    FreeAndNil(FUpsampler[Channel]);
  for Channel := 0 to Length(FDownsampler) - 1 do
    FreeAndNil(FDownsampler[Channel]);
...
begin
  ...
  if Assigned(VstHost[0]) and VstHost[0].Active then
    ...
    begin
      ...
      if not VstHost[0].EditVisible then
        VstHost[0].ShowEdit(TForm(PnGui));
...
begin
  with VstHost[0] do
    if EditVisible then
      EditKeyDown(Char(keyCode.Character), keyCode.Virt,
        TVstModifierKeys(keyCode.Modifier));
end;
...
begin
  with VstHost[0] do
    if EditVisible then
      EditKeyUp(Char(keyCode.Character), keyCode.Virt,
        TVstModifierKeys(keyCode.Modifier))
end;
...
procedure TOversampleTemplateDataModule.VSTModuleOfflineNotify(Sender: TObject;
  const AudioFile: TVstAudioFile; const numAudioFiles: Integer;
  const start: Boolean);
begin
  if VstHost[0].Active then
    VstHost[0].OfflineNotify(AudioFile, numAudioFiles, start);
end;
...
begin
  ...
  begin
    ...
    for i := 0 to Length(OfflineTasks) - 1 do
      VstOfflineTaskRecords[i] := OfflineTasks[i].VstOfflineTaskRecord;
...
function TOversampleTemplateDataModule.VSTModuleOutputProperties
  (Sender: TObject; const Index: Integer; var vLabel, shortLabel: string;
  var SpeakerArrangement: TVstSpeakerArrangementType;
  var Flags: TVstPinPropertiesFlags): Boolean;
var
  PinProperties: TVstPinProperties;
...
procedure TOversampleTemplateDataModule.ParamPreTransBWChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to Length(FUpsampler) - 1 do
    FUpsampler[Channel].TransitionBandwidth := 0.01 * Value;
end;

procedure TOversampleTemplateDataModule.ParamOrderDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := ConvertOrderToString(Round(Parameter[Index]));
end;

procedure TOversampleTemplateDataModule.ParamPreFilterOrderValue
  (Sender: TObject; const Index: Integer; var Value: Single);
var
  ch: Integer;
begin
  for ch := 0 to Length(FUpsampler) - 1 do
    FUpsampler[ch].Order := Round(Value);
end;
...
procedure TOversampleTemplateDataModule.VSTModuleBlockSizeChange
...
{$ENDIF}
    ...
    with VstHost[0] do
      if Active then
      begin
        SetBlockSizeAndSampleRate(BlockSize * FOSFactor,
          SampleRate * FOSFactor);
...
procedure TOversampleTemplateDataModule.AudioMasterAutomate(Sender: TObject;
  Index: Integer; ParameterValue: Single);
begin
...
procedure TOversampleTemplateDataModule.VSTModuleSampleRateChange
  (Sender: TObject; const SampleRate: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to Length(FUpsampler) - 1 do
    FUpsampler[Channel].SampleRate := SampleRate;
  for Channel := 0 to Length(FDownsampler) - 1 do
    FDownsampler[Channel].SampleRate := SampleRate;
...
function TOversampleTemplateDataModule.VSTModuleVendorSpecific(Sender: TObject;
  const lArg1, lArg2: Integer; const ptrArg: Pointer;
  const floatArg: Single): Integer;
begin
  Result := 0;
  with VstHost[0] do
    if Active then
      Result := VendorSpecific(lArg1, lArg2, ptrArg, floatArg);
end;
...
procedure TOversampleTemplateDataModule.VSTModuleParameterChange
  (Sender: TObject; const Index: Integer; var Value: Single);
var
  n, pnr: Integer;
begin
  ...
  begin
    ...
    Inc(n);
    if n >= VstHost.Count then
      break;
  end;
  if (n < VstHost.Count) and VstHost[n].Active then
    VstHost[n].Parameter[Index - pnr] := Value;
end;

procedure TOversampleTemplateDataModule.CustomParameterDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
var
  n, pnr: Integer;
begin
...
procedure TOversampleTemplateDataModule.CustomParameterLabel(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
var
  n, pnr: Integer;
begin
...
procedure TOversampleTemplateDataModule.ParamPostCharChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ch: Integer;
begin
  for ch := 0 to Length(FUpsampler) - 1 do
    case Round(Value) of
      4, 5, 6:
        FUpsampler[ch].FilterClass := TChebyshev1LowpassFilter;
    else
      FUpsampler[ch].FilterClass := TButterworthLowpassFilter;
    end;
end;

procedure TOversampleTemplateDataModule.ParamPostFilterBWChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ch: Integer;
begin
  for ch := 0 to Length(FDownsampler) - 1 do
    FDownsampler[ch].TransitionBandwidth := 0.01 * Value;
end;

procedure TOversampleTemplateDataModule.ParamPostOrderChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ch: Integer;
begin
  for ch := 0 to Length(FDownsampler) - 1 do
    FDownsampler[ch].Order := Round(Value);
end;

procedure TOversampleTemplateDataModule.ParamCharChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ch: Integer;
begin
...
procedure TOversampleTemplateDataModule.ParamCharacterDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  case Round(Parameter[Index]) of
    4, 5, 6:
      PreDefined := 'Chebyshev';
  else
    PreDefined := 'Butterworth';
  end;
end;

procedure TOversampleTemplateDataModule.ParamOSFactorDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
...
begin
  case Order of
    0:
      Result := 'Off';
    1:
      Result := IntToStr(Order) + 'st';
    2:
      Result := IntToStr(Order) + 'nd';
    3:
      Result := IntToStr(Order) + 'rd';
  else
    Result := IntToStr(Order) + 'th';
  end;
end;

procedure TOversampleTemplateDataModule.ParamOversamplingChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
{$IFDEF UseCriticalSection}
  FCriticalSection.Enter;
...
{$ENDIF}
    ...
    if FOSActive = True then
      SetOSFactor(Round(ParameterByName['OS Factor']))
    else
      SetOSFactor(1);
{$IFDEF UseCriticalSection}
  finally
...
{$ENDIF}
  if EditorForm is TFmOversampler then
    TFmOversampler(EditorForm).UpdateOverSampling;
end;

procedure TOversampleTemplateDataModule.ParamOSFactorChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
{$IFDEF UseCriticalSection}
  FCriticalSection.Enter;
...
{$ENDIF}
  if EditorForm is TFmOversampler then
    TFmOversampler(EditorForm).UpdateOSFactor;
end;
...
begin
  ...
  for ch := 0 to Length(FDownsampler) - 1 do
    FDownsampler[ch].Factor := FOSFactor;
  for ch := 0 to Length(FUpsampler) - 1 do
    FUpsampler[ch].Factor := FOSFactor;
  TempBufferSize := FMaximumBlockSize * FOSFactor;
  PluginSampleRateChanged;
end;
...
var
  i: Integer;
begin
  if (FTempBufferSize <> Value) and ((Length(FIn64) > 0) or (Length(FOut64) > 0))
  then
  begin
...
begin
  if VstHost[0].Active then
    VstHost[0].ProcessEvents(Events);
end;

procedure TOversampleTemplateDataModule.CheckSampleFrames(const SampleFrames
  : Integer);
begin
  if SampleFrames > FMaximumBlockSize then
  begin
    FMaximumBlockSize := SampleFrames;
    if TempBufferSize < FMaximumBlockSize * FOSFactor then
      TempBufferSize := FMaximumBlockSize * FOSFactor;
  end;
end;
    ...
    begin
      ...
      for Channel := 0 to numInputs - 1 do
        for Sample := 0 to SampleFrames - 1 do
          FUpsampler[Channel].Upsample32(Inputs[Channel, Sample], @FIn32[Channel, Sample * FOSFactor]);

      (*
      for Channel := 0 to numInputs - 1 do
        for Sample := 0 to SampleFrames * FOSFactor - 1 do
          Assert(not IsNaN(FIn32[Channel, Sample]));
     ...
     // process serial chain
      if VstHost[0].Active then
        VstHost[0].Process32Replacing(@FIn32[0], @FOut32[0],
          SampleFrames * FOSFactor)
      else
        for Channel := 0 to min(numInputs, numOutputs) - 1 do
          Move(FIn32[Channel, 0], FOut32[Channel, 0],
            SampleFrames * SizeOf(Single) * FOSFactor);

      (*
       for Channel := 0 to numInputs - 1 do
         for Sample := 0 to SampleFrames * FOSFactor - 1 do
           Assert(not IsNaN(FOut32[Channel, Sample]));
      *)

      // downsample
      for Channel := 0 to numOutputs - 1 do
        for Sample := 0 to SampleFrames - 1 do
          Outputs[Channel, Sample] := FDownsampler[Channel].Downsample32(@FOut32[Channel, Sample * FOSFactor]);
    end
   else
    begin
      if VstHost[0].Active then
        ...
        for Channel := 0 to min(numInputs, numOutputs) - 1 do
          Move(Inputs[Channel, 0], Outputs[Channel, 0], SampleFrames * SizeOf(Single) * FOSFactor);
    end;
{$IFDEF UseCriticalSection}
 finally
  FCriticalSection.Leave;
...
procedure TOversampleTemplateDataModule.VSTModuleProcess64OversampleSingle
...
{$ENDIF}
    ...
    begin
      ...
      for Channel := 0 to numInputs - 1 do
        for Sample := 0 to SampleFrames - 1 do
          FUpsampler[Channel].Upsample64(Inputs[Channel, Sample], @FIn64[Channel, Sample * FOSFactor]);

      // process serial chain
      if VstHost[0].Active then
        VstHost[0].Process64Replacing(@FIn64[0], @FOut64[0], SampleFrames * FOSFactor)
      else
        for Channel := 0 to min(numInputs, numOutputs) - 1 do
          Move(FIn64[Channel, 0], FOut64[Channel, 0],
            SampleFrames * SizeOf(Single) * FOSFactor);

      // downsample
      for Channel := 0 to numOutputs - 1 do
        for Sample := 0 to SampleFrames - 1 do
          Outputs[Channel, Sample] := FDownsampler[Channel].Downsample64(@FOut64[Channel, Sample * FOSFactor]);
    end
    else if VstHost[0].Active then
      VstHost[0].Process32Replacing(@Inputs[0], @Outputs[0], SampleFrames * FOSFactor)
    else
      for Channel := 0 to min(numInputs, numOutputs) - 1 do
        Move(Inputs[Channel], Outputs[Channel, 0], SampleFrames * SizeOf(Double) * FOSFactor);
{$IFDEF UseCriticalSection}
  finally
    FCriticalSection.Leave;
  end;

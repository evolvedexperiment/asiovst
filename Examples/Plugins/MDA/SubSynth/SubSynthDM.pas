...
procedure TSubSynthDataModule.ParameterThresholdChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FThreshold := dB_to_Amp(Parameter[4]);
end;

procedure TSubSynthDataModule.ParameterTuneDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := IntToStr(Round(0.0726 * SampleRate * Power(10,
    -2.5 + (1.5 * Parameter[index]))));
end;

procedure TSubSynthDataModule.ParameterReleaseDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := IntToStr(Round(-301.03 / (SampleRate * log10(FRelease))));
end;

procedure TSubSynthDataModule.ParameterReleaseChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FRelease := 1 - Power(10, -2 - (3 * Parameter[5]));
end;

procedure TSubSynthDataModule.ParameterModeDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
...
procedure TSubSynthDataModule.ParameterDryChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FDry := 0.01 * Value;
end;
...
begin
  ...
  if FType = ptKeyOsc then
    FFilterIn := 0.018
  else
    FFilterIn := Power(10, -3 + (2 * Parameter[2]));
      ...
      ptKeyOsc:
        begin // Osc
          if (f2 > th) then
            ...
            en := en * rl;
          sub := (en * sin(phii));
          phii := FloatMod(phii + dph, 6.283185);
        end;
    end;

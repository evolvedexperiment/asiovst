...
begin
  Result := FLimiter.CharacteristicCurve_dB
    (FCompressor.CharacteristicCurve_dB(FGate.CharacteristicCurve_dB(Input)));
end;
...
begin
  ...
  // update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateGateAttack;
end;
...
begin
  ...
  // update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateGateRelease;
end;
...
begin
  if Assigned(FLimiter) then
    FLimiter.Attack := Value;

  // update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateLimiterAttack;
end;
...
begin
  if Assigned(FLimiter) then
    FLimiter.Release := Value;

  // update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateLimiterRelease;
end;
...
begin
  if Assigned(FLimiter) then
    FLimiter.Threshold_dB := Value;

  // update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateLimiterThreshold;
end;
...
begin
  if Assigned(FLimiter) then
    FLimiter.Knee_dB := Value;

  // update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateLimiterKnee;
end;
...
begin
  ...
  // update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateCompressorAttack;
end;
...
begin
  ...
  // Update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateCompressorRelease;
end;
...
begin
  ...
  // update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateCompressorThreshold;
end;
...
begin
  ...
  // update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateCompressorRatio;
end;
...
begin
  ...
  // update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateCompressorKnee;
end;
...
begin
  if Assigned(FGate) then
    FGate.Threshold_dB := Value;

  // update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateGateThreshold;
end;
...
begin
  ...
  // update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateGateRatio;
end;
...
begin
  ...
  // update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateGateKnee;
end;
...
begin
  if Assigned(FCompressor) then
    FCompressor.AutoMakeUp := Boolean(Round(Value));

  // update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateCompressorAutoMakeUpGain;
end;
...
begin
  ...
  // update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateCompressorMakeUp;
end;
...
begin
  ...
  // update GUI
  if EditorForm is TFmLightweightDynamics then
    TFmLightweightDynamics(EditorForm).UpdateLimiterSoftClip;
end;
...
procedure TLightweightDynamicsDataModule.ParameterTimeLabel(Sender: TObject;
...
procedure TLightweightDynamicsDataModule.ParameterTimeDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
var
  Val: Single;
begin
...
procedure TLightweightDynamicsDataModule.ParameterCompressorRatioDisplay
  (Sender: TObject; const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := '1 : ' + FloatToStrF(RoundTo(Parameter[Index], -2),
    ffGeneral, 3, 3);
end;
...
procedure TLightweightDynamicsDataModule.ParameterGateRatioDisplay
  (Sender: TObject; const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := FloatToStrF(RoundTo(Parameter[Index], -2), ffGeneral, 3,
    3) + ' : 1';
end;

procedure TLightweightDynamicsDataModule.ParameterKneeDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := FloatToStrF(RoundTo(Parameter[Index], -2), ffGeneral, 3, 3);
end;
...
begin
  ...
  begin
    if Assigned(FGate) then
      FGate.SampleRate := Abs(SampleRate);
    if Assigned(FCompressor) then
      FCompressor.SampleRate := Abs(SampleRate);
    if Assigned(FLimiter) then
      FLimiter.SampleRate := Abs(SampleRate);
  end;
end;

procedure TLightweightDynamicsDataModule.VSTModuleProcessGateCompressorLimiter
...
begin
  ...
  begin
    ...
    if AbsInp[0] > FInputPeak[0] then
      FInputPeak[0] := AbsInp[0]
    else
      FInputPeak[0] := FInputPeak[0] * FPeakRelease;
    if AbsInp[1] > FInputPeak[1] then
      FInputPeak[1] := AbsInp[1]
    else
      FInputPeak[1] := FInputPeak[1] * FPeakRelease;

    with FGate do
    begin
      if Abs(Inputs[0, Sample]) > Abs(Inputs[1, Sample]) then
        InputSample(Inputs[0, Sample])
      else
        InputSample(Inputs[1, Sample]);
    ...
    with FLimiter do
    begin
      if Abs(Outputs[0, Sample]) > Abs(Outputs[1, Sample]) then
        InputSample(Outputs[0, Sample])
      else
        InputSample(Outputs[1, Sample]);
...
procedure TLightweightDynamicsDataModule.
  VSTModuleProcessGateCompressorLimiterSoftClip(const Inputs,

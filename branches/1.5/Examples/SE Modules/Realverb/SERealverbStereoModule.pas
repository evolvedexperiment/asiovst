...
type
  ...
  TSERealverbPins = (pinInputLeft, pinInputRight, pinOutputLeft, pinOutputRight,
  ...
  with TResourceStream.Create(HInstance, 'Left', 'RAW') do
    try
      ...
      for i := 0 to Length(HFData) - 1 do
        IRData[i] := HalfFloatToSingle(HFData[i]);
...
procedure TSERealverbStereoModule.SubProcessStatic(const BufferOffset,
...
procedure TSERealverbStereoModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
begin
  // lock processing
  ...
  inc(FSemaphore);
  try
    FConvolver[0].ProcessBlock
      (PDAVSingleFixedArray(@FInputBuffer[0][BufferOffset]),
      PDAVSingleFixedArray(@FOutputBuffer[0][BufferOffset]), SampleFrames);
    FConvolver[1].ProcessBlock
...
begin
  ...
  if Pin[Integer(pinInputRight)].Status > Status then
    Status := Pin[Integer(pinInputRight)].Status;
  if Status = stRun then
    OnProcess := SubProcess
  else
  begin
...
class procedure TSERealverbStereoModule.GetModuleProperties
...
function TSERealverbStereoModule.GetPinProperties(const Index: Integer;
...
begin
  ...
  case TSERealverbPins(CurrentPin.PinID) of
    pinInputLeft, pinInputRight:
      begin
        Status := Pin[0].Status;
        if Pin[1].Status > Status then
          Status := Pin[1].Status;
        ChooseProcess;
    ...
    pinMaxFFTOrder:
      begin
        ...
        inc(FSemaphore);
        try
          if FMaxIRBlockOrder <= FConvolver[0].MinimumIRBlockOrder then
            FMaxIRBlockOrder := FConvolver[0].MinimumIRBlockOrder + 1;

...
type
  ...
  TSEVoiceSynthPins = (pinInput, pinMinimum, pinMaximum, pinSmooth,
    pinOneCrossingOnly, pinDSFilterOrder, pinDSBandwidth, pinAttack, pinRelease,
    pinThreshold, pinQuantize, pinOutput);
...
constructor TCustomSEVoiceSynthModule.Create(SEAudioMaster
...
begin
  ...
  if SampleRate > 0 then
    FVoiceSynth.SampleRate := SampleRate;
end;
...
begin
  ...
  for Sample := 0 to SampleFrames - 1 do
    Outp^[Sample] := FVoiceSynth.ProcessSample32(Inp^[Sample]);
end;

procedure TCustomSEVoiceSynthModule.SubProcessStatic(const BufferOffset,
  SampleFrames: Integer);
begin
...
class procedure TCustomSEVoiceSynthModule.GetModuleProperties
...
function TCustomSEVoiceSynthModule.GetPinProperties(const Index: Integer;
...
// describe the pins (plugs)
function TSEVoiceSynthStaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
...
begin
  ...
  case TSEVoiceSynthPins(CurrentPin.PinID) of
    pinMinimum:
      FVoiceSynth.MinimumFrequency := FMinimum;
    pinMaximum:
      FVoiceSynth.MaximumFrequency := FMaximum;
    pinSmooth:
      FVoiceSynth.SmoothFactor := FSmoothFactor;
    pinDSFilterOrder:
      FVoiceSynth.DownSampleFilterOrder := FDSFilterOrder;
    pinDSBandwidth:
      FVoiceSynth.DownSampleBandwidth := FDSBandwidth;
    pinOneCrossingOnly:
      FVoiceSynth.OneCrossingOnly := FOneCrossingOnly;
    pinAttack:
      FVoiceSynth.Attack := FAttack;
    pinRelease:
      FVoiceSynth.Release := FRelease;
    pinThreshold:
      FVoiceSynth.Threshold := FThreshold;
    pinQuantize:
      FVoiceSynth.QuantizeToNotes := FQuantize;
  end;
end;
...
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSEVoiceSynthPins(index) in [pinMinimum .. pinQuantize] then
    with Properties^ do
      Direction := drIn;
end;

...
begin
  for Band := 0 to Length(FFilterArray) - 1 do
    FreeAndNil(FFilterArray[Band]);

  inherited;
end;

procedure TCustomParametricEQ.Complex(const Frequency: Double;
...
begin
  for Band := 0 to Length(FFilterArray) - 1 do
    FFilterArray[Band].SampleRate := SampleRate
end;
...
var
  Band: Integer;
begin
  for Band := FBandCount to Length(FFilterArray) - 1 do
    FreeAndNil(FFilterArray[Band]);

...
begin
  with TLA4029DataModule(Owner) do
    Result := TLevelState(Round(Parameter[8]));
end;
...
begin
  ...
  begin
    if Parameter[0] < 0.5 then
      LEDOnOff.Brightness_Percent := 100
    else
      LEDOnOff.Brightness_Percent := 10;
  end;
end;
...
begin
  with TLA4029DataModule(Owner) do
  begin
    if DialInput.Value <> Parameter[1] then
      DialInput.Value := Parameter[1];
...
begin
  with TLA4029DataModule(Owner) do
  begin
    if DialKnee.Value <> Parameter[6] then
      DialKnee.Value := Parameter[6];
...
begin
  with TLA4029DataModule(Owner) do
  begin
    if DialMix.Value <> Parameter[7] then
      DialMix.Value := Parameter[7];
...
begin
  with TLA4029DataModule(Owner) do
  begin
    if DialOutput.Value <> Parameter[2] then
      DialOutput.Value := Parameter[2];
    LbOutputValue.Caption := FloatToStrF(DialOutput.Value, ffFixed, 3,
      1) + ' dB';
  end;
end;
...
begin
  ...
  if Value <   0 then Result := Round(43 + (1 + Value) * 5)
    ...
    Result := Round(48 + Value / 3 * (VUMeter.GlyphCount - 49));
end;
...
begin
  with TLA4029DataModule(Owner) do
    case LevelState of
      lsIn:
        VUMeter.GlyphIndex := VUMeterValueToPos(InLevel_dB);
      lsGR:
        VUMeter.GlyphIndex := VUMeterValueToPos(GRReduction_dB);
      lsOut:
        VUMeter.GlyphIndex := VUMeterValueToPos(OutLevel_dB);
    end;
end;
...
begin
  with TLA4029DataModule(Owner) do
  begin
    if Parameter[1] <> DialInput.Value then
      Parameter[1] := DialInput.Value;
    UpdateInput;
  end;
end;
...
begin
  with TLA4029DataModule(Owner) do
  begin
    if Parameter[2] <> DialOutput.Value then
      Parameter[2] := DialOutput.Value;
    UpdateOutput;
  end;
end;

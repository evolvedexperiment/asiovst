...
type
  ...
  protected
    ...
    procedure SetItem(Index: Integer;
      const Value: TGuiCustomStitchedCollectionItem); virtual;
    procedure Notify(Item: TCollectionItem;
      Action: TCollectionNotification); override;
    property Items[Index: Integer]: TGuiCustomStitchedCollectionItem
  ...
  protected
    ...
    property DefaultGlyphIndex: Integer read FDefaultGlyphIndex
      write SetDefaultGlyphIndex;
    property GlyphIndex: Integer read FGlyphIndex write SetGlyphIndex;
    property StitchedImageItem: TGuiCustomStitchedCollectionItem
      read GetStitchedImageItem;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;

    property ImageList: TGuiCustomStitchedList read GetStitchedImageList
      write SetStitchedImageList;
...
function TGuiStitchedImageCollection.GetItem(Index: Integer)
  : TGuiCustomStitchedCollectionItem;
begin
  Result := TGuiCustomStitchedCollectionItem(inherited GetItem(Index));
end;

function TGuiStitchedImageCollection.Insert(Index: Integer)
  : TGuiCustomStitchedCollectionItem;
begin
...
begin
  for Index := 0 to FLinkedControls.Count - 1 do
    ...
    begin
      if FDefaultGlyphIndex >= FGlyphCount then
        DefaultGlyphIndex := FGlyphCount - 1;
      if FGlyphIndex >= FGlyphCount then
...
procedure TGuiCustomStitchedCollectionItem.SetStitchKind
...
function TGuiCustomStitchedList.GetItems(Index: Integer)
...
begin
  ...
  begin
    ...
    if FImageIndex >= 0 then
      ImageItem := FImageList[FImageIndex];
    ImageIndexChanged;
  end;
  ...
  begin
    if Assigned(FImageItem) and
      (FDefaultGlyphIndex >= StitchedImageItem.FGlyphCount) then
      FDefaultGlyphIndex := -1;
  end;
end;
...
begin
  ...
  if Assigned(FImageItem) then
    Value := Limit(Value, 0, StitchedImageItem.GlyphCount - 1)
  else
    Value := -1;
...
begin
  if Assigned(FImageItem) then
    Result := StitchedImageItem.GlyphCount
  else
    Result := 0;
end;

function TGuiCustomStitchedControl.GetStitchedImageItem
  : TGuiCustomStitchedCollectionItem;
begin
...
begin
  if FImageList is TGuiCustomStitchedList then
    Result := TGuiCustomStitchedList(FImageList)
  else
    Result := nil;
end;
      ...
      case StitchKind of
        skHorizontal:
          ...
          begin
            ...
            begin
              BlendLine(PPixel32(DataPointer),
                PPixel32(FBuffer.ScanLine[LineIndex]), Self.Width);

...
constructor TSEEnvelopeModule.Create(SEAudioMaster: TSE2audioMasterCallback;
  Reserved: Pointer);
begin
...
procedure TSEEnvelopeModule.SubProcess(const BufferOffset,
...
class procedure TSEEnvelopeModule.GetModuleProperties
...
function TSEEnvelopeModule.GetPinProperties(const Index: Integer;
  ...
  case TSEEnvelopePins(CurrentPin.PinID) of
    pinInput:
      begin
        ...
        if (OutState < stRun) and (Pin[Integer(pinInput)].Value = 0) then
          OutState := stStatic;
        ChooseProcess;
        Pin[1].TransmitStatusChange(SampleClock, OutState);
      end;
    pinCoefficients:
      FHilbert.NumberOfCoefficients := FCoefficients;
    pinTransition:
      FHilbert.Transition := Limit(FTransition, 0.01, 0.499);
  end;
end;

{ TSEHilbertModule }

constructor TSEHilbertModule.Create(SEAudioMaster: TSE2audioMasterCallback;
  Reserved: Pointer);
begin
  inherited Create(SEAudioMaster, Reserved);
  FHilbert := TPhaseHalfPi32.Create;
...
// The most important part, processing the audio
procedure TSEHilbertModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
var
...
begin
  ...
  for Sample := 0 to SampleFrames - 1 do
    FHilbert.ProcessHilbertSample(Inp^[Sample], OutA^[Sample], OutB^[Sample]);
end;
...
// describe your module
class procedure TSEHilbertModule.GetModuleProperties
  (Properties: PSEModuleProperties);
begin
  with Properties^ do
  begin
...
// describe the pins (plugs)
function TSEHilbertModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
  Result := True;
  case Index of
  ...
  case TSEEnvelopePins(CurrentPin.PinID) of
    pinInput:
      begin
        ...
        if (OutState < stRun) and (Pin[0].Value = 0) then
          OutState := stStatic;

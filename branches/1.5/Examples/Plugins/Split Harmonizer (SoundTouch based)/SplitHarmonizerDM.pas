...
begin
  ...
  with Programs[1] do
    SetParameters([0, -9, 13.5, 52.5, 9, 14.1, 50.0, 1, 0, 1, 10, 10, 5]);
  with Programs[2] do
    SetParameters([1, -6, 23.1, 52.5, 6, 21.6, 50.0, 1, 0, 1, 10, 10, 5]);
  with Programs[3] do
    SetParameters([0, 16, 48.2, 13.5, -13, 79.1, 63.5, 1, 0, 1, 10, 10, 5]);
...
begin
  for Channel := 0 to Length(FSoundTouch) - 1 do
    FreeAndNil(FSoundTouch[Channel]);
...
procedure TSplitHarmonizerModule.ParameterSemiTonesAChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
    if Assigned(FSoundTouch[0]) then
      FSoundTouch[0].Pitch := Power(2, Value / 1200);
  finally
    FCriticalSection.Leave;
  end;

  if EditorForm is TFmSplitHarmonizer then
    TFmSplitHarmonizer(EditorForm).UpdateSemitones(0);
end;

procedure TSplitHarmonizerModule.ParameterSemiTonesBChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
    if Assigned(FSoundTouch[1]) then
  ...
  if EditorForm is TFmSplitHarmonizer then
    TFmSplitHarmonizer(EditorForm).UpdateSemitones(1);
end;

procedure TSplitHarmonizerModule.ParameterUseAntiAliasChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
...
procedure TSplitHarmonizerModule.ParameterUseQuickSeekChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  ...
  try
    for Channel := 0 to 1 do
      if Assigned(FSoundTouch[Channel]) then
        FSoundTouch[Channel].UseQuickSeek := Value > 0.5;
  finally
    FCriticalSection.Leave;
  end;
end;
...
procedure TSplitHarmonizerModule.ParameterLowpassAChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FLowpassFilter[0]) then
    FLowpassFilter[0].Frequency := Value;

  // update GUI
  if EditorForm is TFmSplitHarmonizer then
    TFmSplitHarmonizer(EditorForm).UpdateLowpassFilter(0);
end;

procedure TSplitHarmonizerModule.ParameterLowpassBChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FLowpassFilter[1]) then
    FLowpassFilter[1].Frequency := Value;

  // update GUI
  if EditorForm is TFmSplitHarmonizer then
    TFmSplitHarmonizer(EditorForm).UpdateLowpassFilter(1);
end;
...
procedure TSplitHarmonizerModule.ParameterAutoSettingsChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
...
begin
  ...
  try
    if not FAutoSettings then
      ...
      begin
        if Assigned(FSoundTouch[Channel]) then
          FSoundTouch[Channel].SequenceMs := Round(Value);
        if Assigned(FDelayLine[Channel, 0]) then
          FDelayLine[Channel, 0].BufferSize :=
            Round(0.001 * Value * SampleRate);
...
begin
  ...
  try
    if not FAutoSettings then
      for Channel := 0 to 1 do
        if Assigned(FSoundTouch[Channel]) then
          FSoundTouch[Channel].SeekWindow := Round(Value);
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TSplitHarmonizerModule.ParameterOverlapChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  ...
  try
    if not FAutoSettings then
      for Channel := 0 to 1 do
        if Assigned(FSoundTouch[Channel]) then
          FSoundTouch[Channel].OverlapMs := Round(Value);
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TSplitHarmonizerModule.ParameterDelayAChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
    if Assigned(FDelayLine[0, 1]) then
      FDelayLine[0, 1].BufferSize := Max(1, Round(Value * SampleRate * 1E-3));
  finally
    FCriticalSection.Leave;
  end;
  if EditorForm is TFmSplitHarmonizer then
    TFmSplitHarmonizer(EditorForm).UpdateDelay(0);
end;

procedure TSplitHarmonizerModule.ParameterDelayBChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
    if Assigned(FDelayLine[1, 1]) then
      FDelayLine[1, 1].BufferSize := Max(1, Round(Value * SampleRate * 1E-3));
  finally
    FCriticalSection.Leave;
  end;
  if EditorForm is TFmSplitHarmonizer then
    TFmSplitHarmonizer(EditorForm).UpdateDelay(1);
end;

procedure TSplitHarmonizerModule.ParameterMixAChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FMix[0] := 0.01 * Value;
  ...
  // update GUI
  if EditorForm is TFmSplitHarmonizer then
    TFmSplitHarmonizer(EditorForm).UpdateMix(0);
end;

procedure TSplitHarmonizerModule.ParameterMixRightChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FMix[1] := 0.01 * Value;
  ...
  // update GUI
  if EditorForm is TFmSplitHarmonizer then
    TFmSplitHarmonizer(EditorForm).UpdateMix(1);
end;

procedure TSplitHarmonizerModule.ParameterEncodeChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  case Round(Value) of
    0:
...
procedure TSplitHarmonizerModule.ParameterEncodeDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
...
begin
  ...
  try
    ...
    begin
      Outputs[0, Sample] := FLowpassFilter[0].ProcessSample32
        (Inputs[0, Sample]);
      Outputs[1, Sample] := FLowpassFilter[1].ProcessSample32
        (Inputs[1, Sample]);
    end;

    for Channel := 0 to NumInputs - 1 do
    begin
      Mix[Channel, 0] := CHalf32 * (1 - CHalf32 * abs(FMix[Channel]) -
        FMix[Channel] + CHalf32);
      Mix[Channel, 1] := CHalf32 * (1 - CHalf32 * abs(FMix[Channel]) +
        FMix[Channel] + CHalf32);
    ...
    for Sample := 0 to SampleFrames - 1 do
    begin
      Outputs[0, Sample] := Mix[0, 0] * FDelayLine[0, 0].ProcessSample32
        (Inputs[0, Sample]) + Mix[0, 1] * FDelayLine[0, 1].ProcessSample32
        (Outputs[0, Sample]);
      Outputs[1, Sample] := Mix[1, 0] * FDelayLine[1, 0].ProcessSample32
        (Inputs[1, Sample]) + Mix[1, 1] * FDelayLine[1, 1].ProcessSample32
        (Outputs[1, Sample]);
    end;
  finally
    FCriticalSection.Leave;
...
begin
  ...
  try
    ...
    begin
      Outputs[0, Sample] := FLowpassFilter[0].ProcessSample32
        (FDelayLine[0, 1].ProcessSample32(CHalf32 * (Inputs[0, Sample] +
        Inputs[1, Sample])));
      Outputs[1, Sample] := FLowpassFilter[1].ProcessSample32
        (FDelayLine[1, 1].ProcessSample32(CHalf32 * (Inputs[0, Sample] -
        Inputs[1, Sample])));
    end;
...
begin
  ...
  try
    ...
    for Sample := 0 to SampleFrames - 1 do
    begin
      Temp := Mix[0, 0] * FDelayLine[0, 0].ProcessSample32(Inputs[0, Sample]) +
        Mix[0, 1] * (Outputs[0, Sample] + Outputs[1, Sample]);
      Outputs[1, Sample] := Mix[1, 0] * FDelayLine[1, 0].ProcessSample32
        (Inputs[1, Sample]) + Mix[1, 1] *
...
begin
  ...
  try
    ...
    begin
      ...
      if Assigned(FDelayLine[Channel, 0]) then
        FDelayLine[Channel, 0].BufferSize := CInputDelay;
      if Assigned(FDelayLine[Channel, 1]) then
        FDelayLine[Channel, 1].BufferSize :=
          Max(1, Round(Parameter[2 + 4 * Channel] * 1E-3 * abs(SampleRate)));
      if Assigned(FLowpassFilter[Channel]) then
        FLowpassFilter[Channel].SampleRate := SampleRate;
    end;
  finally
    FCriticalSection.Leave;

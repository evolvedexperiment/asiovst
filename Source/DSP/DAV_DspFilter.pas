...
{$ENDIF}
  ...
  private
    function GetAnaloguePrototypeClass
      : TCustomBiquadAnalogueFilterPrototypeClass;
    procedure SetAnaloguePrototype(const Value
      : TCustomBiquadAnalogueFilterPrototype);
    procedure SetAnaloguePrototypeClass(const Value
      : TCustomBiquadAnalogueFilterPrototypeClass);
  protected
...
var
  cmplx: TComplex64;
begin
  cmplx := Goertzel(PDAVDoubleFixedArray(@FIR[0]), FKernelSize,
...
begin
  ...
  Move(FStateStack[Length(FHistory)], FCircular[0],
    Length(FCircular) * SizeOf(Double));
end;

procedure ConvolveIR_X87(InOutBuffer, IRBuffer: PDAVDoubleFixedArray;
...
begin
  for SampleIndex := 0 to SampleFrames - 1 do
    InOutBuffer[SampleIndex] := InOutBuffer[SampleIndex] + IRBuffer[SampleIndex]
      * Current;
{$ELSE}
asm
  FLD     Current.Double
...
begin
  ...
  ConvolveIR_X87large(@FCircular[FBufferPos], @FIR[0], FKernelSize,
    FHistory[FBufferPos]);
  Inc(FBufferPos);
...
begin
  ...
  Move(FCircular[0], FStateStack[Length(FHistory)],
    Length(FCircular) * SizeOf(Double));
end;
...
begin
  if FPrototypeOwned and Assigned(FAnaloguePrototype) then
...
function TBilinearTransformedBiquadIIRFilter.GetAnaloguePrototypeClass
  : TCustomBiquadAnalogueFilterPrototypeClass;
begin
  if Assigned(FAnaloguePrototype) then
    Result := TCustomBiquadAnalogueFilterPrototypeClass
      (FAnaloguePrototype.ClassType)
  else
    Result := nil;
end;

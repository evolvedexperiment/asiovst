...
type
  ...
  public
    ...
    procedure ProcessSample(const Input: Single;
      out Low, High: Single); overload;
    procedure ProcessSample(const Input: Double;
      out Low, High: Double); overload;
    procedure ProcessBlock(const Input: PDAVSingleFixedArray;
      out Low, High: PDAVSingleFixedArray; SampleFrames: Integer); overload;
    procedure ProcessBlock(const Input: PDAVDoubleFixedArray;
      out Low, High: PDAVDoubleFixedArray; SampleFrames: Integer); overload;
...
procedure TCustomLinearPhaseBandSplitter.ProcessSample(const Input: Single;
  out Low, High: Single);
begin
...
procedure TCustomLinearPhaseBandSplitter.ProcessSample(const Input: Double;
  out Low, High: Double);
begin
  Low := FConvolution.ProcessSample32(Input);
  High := FDelay.ProcessSample32(Input) - Low;
...
procedure TCustomLinearPhaseBandSplitter.ProcessBlock
  (const Input: PDAVSingleFixedArray; out Low, High: PDAVSingleFixedArray;
...
procedure TCustomLinearPhaseBandSplitter.ProcessBlock
  (const Input: PDAVDoubleFixedArray; out Low, High: PDAVDoubleFixedArray;
  SampleFrames: Integer);
var
  Sample: Integer;
begin
...
begin
  ...
  try
    ...
    for i := 0 to FKernelSize - 1 do
      if (2 * i = FKernelSize) then
        FilterKernel^[i] := 2.0 * Cutoff
      else
      begin

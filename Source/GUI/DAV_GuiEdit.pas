...
{$IFNDEF New}
    procedure WMNCCalcSize(var Message:
      {$IFDEF FPC}TLMNCCalcSize{$ELSE}TWMNCCalcSize{$ENDIF});
...
{$ENDIF}
    ...
    property ColorBorder: TColor index 1 read FBorderColor write SetColors
      default clBlack;
    property ColorBackground: TColor index 2 read FBackgroundColor
      write SetColors default $00E1EAEB;
    property ColorDisabled: TColor index 3 read FDisabled write SetColors
      default clBtnShadow;
    property ParentColor: Boolean read FParentColor write SetParentColor
      default false;
    property Native: Boolean read FNative write SetNative default false;
    property RoundBorders: Boolean read FRoundBorders write SetRoundBorders
      default false;
...
begin
  ...
  case FAlignment of
    taCenter:
      Params.Style := (Params.Style and not $3) or (ES_CENTER and $3);
    taLeftJustify:
      Params.Style := (Params.Style and not $3) or (ES_LEFT and $3);
    taRightJustify:
      Params.Style := (Params.Style and not $3) or (ES_RIGHT and $3);
  end;
...
begin
  ...
  begin
    ...
    begin
      if Parent <> nil then
        FBackgroundColor := TForm(Parent).Color;
      Invalidate;
    end;
  end;
end;
...
begin
  ...
  if not((csDesigning in ComponentState) and (csLoading in ComponentState)) and
    not Native then
    NewAdjustHeight;
end;
...
procedure TGuiCustomControlsEdit.WMNCCalcSize(var Message:
  {$IFDEF FPC}TLMNCCalcSize{$ELSE}TWMNCCalcSize{$ENDIF});
begin
  inherited;

  if not FNative then
    InflateRect(Message.CalcSize_Params^.rgrc[0], -FBorderMargin,
      -FBorderMargin);
end;
...
begin
  if Value < 1 then
    Value := 1
  else if Value > (Height div 2) then
    Value := (Height div 2);
  if FRoundBorders and (Value < 2) then
    Value := 2;
      ...
      *)
      ...
      begin
        if (not(csDesigning in ComponentState) and
          (Focused or (FMouseInControl and
          not(Screen.ActiveControl is TGuiCustomControlsEdit)))) then
        begin
          { Focus }
        ...
        begin
          ...
          PolyLine([Point(Left + 1, Bottom - 1), Point(Left, Bottom - 2),
            Point(Left, Top + 2), Point(Left + 2, Top), Point(Right - 2, Top),
            Point(Right, Top + 2)]);
          ...
          PolyLine([Point(Right - 1, Top + 1), Point(Right, Top + 2),
            Point(Right, Bottom - 2), Point(Right - 2, Bottom),
            Point(Left + 2, Bottom), Point(Left, Bottom - 2)]);
        end;
      end
      else
      begin

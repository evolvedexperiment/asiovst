...
{$ENDIF}
    ...
    procedure BufferChanged(TextChanged: Boolean = True;
      BoxChanged: Boolean = True); virtual;
...
begin
  ...
  if FCheckBoxSize > Height - 2 then
    FCheckBoxSize := Height - 2;
  if FCheckBoxSize > Width - 2 then
    FCheckBoxSize := Width - 2;
end;
...
procedure TGuiControlsCheckBox.BufferChanged(TextChanged: Boolean = True;
  BoxChanged: Boolean = True);
begin
  if TextChanged then
    FTextChanged := True;
  if BoxChanged then
...
procedure TGuiControlsCheckBox.CMColorChanged(var Message:
...
procedure TGuiControlsCheckBox.CMMouseEnter(var Message:
  {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF});
begin
  inherited;
  MouseEnter;
...
procedure TGuiControlsCheckBox.CMMouseLeave(var Message:
  {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF});
begin
  inherited;
  MouseLeave;
...
procedure TGuiControlsCheckBox.CMEnabledChanged(var Message:
  {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF});
begin
  inherited;
...
procedure TGuiControlsCheckBox.CMFontChanged(var Message:
  {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF});
begin
  FGuiFont.Font.Assign(Font);
end;

procedure TGuiControlsCheckBox.CMTextChanged(var Message:
...
procedure TGuiControlsCheckBox.WMSetFocus(var Message:
...
procedure TGuiControlsCheckBox.CMParentColorChanged(var Message:
  {$IFDEF FPC}TLMCommand{$ELSE}TWMCommand{$ENDIF});
begin
  inherited;
...
procedure TGuiControlsCheckBox.MouseDown(Button: TMouseButton;
...
procedure TGuiControlsCheckBox.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  if (Button = mbLeft) and Enabled then
  begin
...
procedure TGuiControlsCheckBox.WMMove(var Message:
...
procedure TGuiControlsCheckBox.WMPaint(var Message:
  ...
  begin
    ...
    begin
      ...
      for X := Round((Radius - 1) - XStart)
        to Round((FCheckBoxSize - 1) - (Radius - 1) + XStart) do
      begin
        ...
        if X < (Radius - 1) then
          ...
          if X > (FCheckBoxSize - 1) - (Radius - 1) then
          SqrDist := Sqr(X - (FCheckBoxSize - 1) + (Radius - 1)) + SqrYDist
        else
          SqrDist := SqrYDist;

        if SqrDist < SqrRadMinusBorder then
          CombColor := BackColor
        else if SqrDist <= Sqr(RadMinusBorderOne) then
        begin
          Temp := RadMinusBorderOne - FastSqrtBab2(SqrDist);
          CombColor := CombinePixel(BorderColor, BackColor,
            Round($FF - Temp * $FF));
        end
      ...
      begin
        ...
        then
          ...
          begin
            ...
            else if (X > FCheckBoxSize - 1 - BorderWidth) then
            begin
            ...
            begin
              ...
              if (X < BorderWidth - 1) or
                (X > FCheckBoxSize - 1 - BorderWidth + 1) then
                CombColor := BorderColor
              else if (X < BorderWidth) then
              begin
                Temp := Temp + (BorderWidth - X) * (1 - Temp);
              ...
              begin
                Temp := Temp + (X - FCheckBoxSize + 1 + BorderWidth) *
                  (1 - Temp);
                CombColor := CombinePixel(BorderColor, BackColor,
                  Round(Temp * $FF));
              end
    ...
    if Checked then
    begin
      DotRect := Rect(4, YOffset + 4, FCheckBoxSize - 4,
        YOffset + FCheckBoxSize - 4);
...
{$ELSE}
    case Alignment of
      taLeftJustify:
        TextSize.cx := Width - TextSize.cx - FCheckBoxSize - 3;
      taRightJustify:
        TextSize.cx := FCheckBoxSize + 3;
    end;
{$ENDIF}
...
procedure TGuiControlsCheckBox.SetOversampling(const Value: TFontOversampling);
begin
...
begin
  ...
  begin
    ...
    for Y := 0 to FBuffer.Height - 1 do
    begin
      Move(SrcPtr^, DestPtr^, (FBuffer.Width - (FCheckBoxSize + 1)) *
        SizeOf(TPixel32));
      SrcPtr := @SrcPtr^[FBuffer.Width];
      DestPtr := @DestPtr^[FBuffer.Width];
    end;

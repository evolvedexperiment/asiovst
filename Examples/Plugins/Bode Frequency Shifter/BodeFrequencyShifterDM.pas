...
begin
    ...
    try
      ...
      begin
        ...
        begin
          ...
          if TempFreq > ParameterProperties[0].Min then
            ParameterProperties[0].Max := TempFreq;
        end;
        if ValueExists('Lower Frequency') then
        begin
          TempFreq := StrToFloat(ReadString('Lower Frequency'));
          if TempFreq < ParameterProperties[0].Max then
            ParameterProperties[0].Min := TempFreq;
        end;
      end
    finally
      Free;
  ...
  for Channel := 0 to Length(FFreqShifter) - 1 do
    FFreqShifter[Channel] := TBodeFrequencyShifter32.Create;
...
begin
  for Channel := 0 to Length(FFreqShifter) - 1 do
    FreeAndNil(FFreqShifter[Channel]);
end;
...
begin
  ...
  try
    for Channel := 0 to Length(FFreqShifter) - 1 do
      if Assigned(FFreqShifter[Channel]) then
        FFreqShifter[Channel].Frequency := Value;
  finally
    FCriticalSection.Leave;
  end;

  if EditorForm is TFmBodeFrequencyShifter then
    TFmBodeFrequencyShifter(EditorForm).UpdateFrequency;
end;

procedure TBodeFrequencyShifterDataModule.ParameterMixChange(Sender: TObject;
...
begin
  ...
  if EditorForm is TFmBodeFrequencyShifter then
    TFmBodeFrequencyShifter(EditorForm).UpdateMix;
end;

procedure TBodeFrequencyShifterDataModule.ParameterCoeffsChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  ...
  try
    for Channel := 0 to Length(FFreqShifter) - 1 do
      if Assigned(FFreqShifter[Channel]) then
        FFreqShifter[Channel].CoefficientCount := Round(Value);
  finally
    FCriticalSection.Leave;
  end;
end;
...
begin
  for Channel := 0 to Length(FFreqShifter) - 1 do
    if Assigned(FFreqShifter[Channel]) then
      FFreqShifter[Channel].TransitionBandwidth := Value;
end;
...
begin
  if Parameter[Index] < 1000 then
    ...
    PreDefined := AnsiString(FloatToStrF(0.001 * Parameter[Index],
      ffGeneral, 4, 4));
end;
...
begin
  if Abs(SampleRate) > 0 then
    for Channel := 0 to Length(FFreqShifter) - 1 do
      if Assigned(FFreqShifter[Channel]) then
        FFreqShifter[Channel].SampleRate := Abs(SampleRate);
end;

...
type
  ...
  public
    ...
    property ButtonColor: TColor read FButtonColor write SetButtonColor
      default $303030;
    property ButtonState: TMediaButtonState read FButtonState
      write SetButtonState default mbsPlay;
    ...
    case FButtonState of
      ...
      mbsPause:
        begin
          ...
          if Wdth < FGlyphOffset then
            Wdth := FGlyphOffset;

          FillRect(Offset, Offset, Offset + Wdth, Height - Offset, pxGray32);

          FillRect(Width - Offset - Wdth, Offset, Width - Offset,
            Height - Offset, pxGray32);

          // draw frame
          for Y := 0 to Trunc(FOutlineWidth) - 1 do
          begin
            BlendPixelLine(CBorderColor, @Scanline[Offset - IntOff + Y]^
              [Offset - IntOff], Wdth + Trunc(FOutlineWidth));
            BlendPixelLine(CBorderColor, @Scanline[Height - Offset - IntOff + Y]
              ^[Offset - IntOff], Wdth + Trunc(FOutlineWidth));
          end;
          for Y := Offset - IntOff + Trunc(FOutlineWidth) to Height - Offset -
            IntOff - 1 do
          begin
          ...
          for Y := Offset - IntOff + Trunc(FOutlineWidth) to Height - Offset -
            IntOff - 1 do
          begin
            BlendPixelLine(CBorderColor,
              @Scanline[Y]^[Width - 1 - (Offset + IntOff)],
              Trunc(FOutlineWidth));
            BlendPixelLine(CBorderColor,
              @Scanline[Y]^[Width - 1 - (Offset + Wdth + IntOff)],
              Trunc(FOutlineWidth));
          end;
        end;
      mbsStop:
        FillRect(Offset, Offset, Width - Offset, Height - Offset, pxBlack32);
      ...
      mbsNext:
        begin
          ...
          FillRect(Round(0.5 * (Width + Offset)), Offset, Width - Offset,
            Height - Offset, pxBlue32);
        end;

      mbsPrevious:
        begin
          FillRect(Offset, Offset, (Width - Offset) div 2, Height - Offset,
            pxBlue32);
        ...
        begin
          with TGuiPixelFilledCircle.Create do
            try
              ...
              GeometricShape.Radius :=
                ConvertToFixed24Dot8(0.5 * (Min(Width, Height) - FOutlineWidth)
                - Offset);
              Alpha := $FF;
              Draw(PixelMap);
            finally
          ...
          with TGuiPixelFrameCircle.Create do
            try
              ...
              GeometricShape.CenterY := ConvertToFixed24Dot8(0.5 * Height);
              GeometricShape.Radius :=
                ConvertToFixed24Dot8(0.5 * Min(Width, Height) - Offset);
              Draw(PixelMap);
            finally
              Free;
            end;
...
begin
  ...
  begin
    ...
    XAdv := ConvertToFixed24Dot8(2 * (Rect.Right - Rect.Left) /
    ...
    if Reverse then
      ...
      begin
        ...
        BlendPixelInplace(ApplyAlpha(Clr, XFixed.Frac),
          ScnLn[0]^[Rect.Right - X - 1]);
        if ScnLn[0] <> ScnLn[1] then
        begin
          BlendPixelLine(Clr, @ScnLn[1]^[Rect.Right - X], X);
          BlendPixelInplace(ApplyAlpha(Clr, XFixed.Frac),
            ScnLn[1]^[Rect.Right - X - 1]);
        end
        else
          Break;

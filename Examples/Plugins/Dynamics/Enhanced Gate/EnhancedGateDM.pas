...
var
  i: Integer;
begin
  for i := 0 to CNrChannels - 1 do
    FEnhancedGates[i] := TAdvancedGate.Create;
...
begin
  for Channel := 0 to CNrChannels - 1 do
    if Assigned(FEnhancedGates[Channel]) then
      FEnhancedGates[Channel].Threshold_dB := Value;

  // update GUI if necessary
  if EditorForm is TEditorForm then
    TEditorForm(EditorForm).UpdateThreshold;
end;
...
begin
  for Channel := 0 to CNrChannels - 1 do
    if Assigned(FEnhancedGates[Channel]) then
      FEnhancedGates[Channel].Range_dB := Value;

  // update GUI if necessary
  if EditorForm is TEditorForm then
    TEditorForm(EditorForm).UpdateRange;
end;

procedure TEnhancedGateDataModule.EAGRatioChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to CNrChannels - 1 do
    if Assigned(FEnhancedGates[Channel]) then
      FEnhancedGates[Channel].Ratio := Value;

  // update GUI if necessary
  if EditorForm is TEditorForm then
    TEditorForm(EditorForm).UpdateRatio;
end;
...
begin
  if Assigned(EditorForm) then
    with EditorForm As TEditorForm do
      CBSideChain.ItemIndex := Integer(Round(Value));
end;
...
procedure TEnhancedGateDataModule.EAGLoCutChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to CNrChannels - 1 do
    if Assigned(FEnhancedGates[Channel]) then
      FEnhancedGates[Channel].SideChainLowCut := Value;
...
procedure TEnhancedGateDataModule.EAGHiCutChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to CNrChannels - 1 do
    if Assigned(FEnhancedGates[Channel]) then
      FEnhancedGates[Channel].SideChainHighCut := 1000 * Value;
...
procedure TEnhancedGateDataModule.EAGAttackChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to CNrChannels - 1 do
    if Assigned(FEnhancedGates[Channel]) then
      FEnhancedGates[Channel].Attack := Value;

  // update GUI if necessary
  if EditorForm is TEditorForm then
    TEditorForm(EditorForm).UpdateAttack;
end;

procedure TEnhancedGateDataModule.EAGHoldChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to CNrChannels - 1 do
    if Assigned(FEnhancedGates[Channel]) then
      FEnhancedGates[Channel].Hold := Value;

  // update GUI if necessary
  if EditorForm is TEditorForm then
    TEditorForm(EditorForm).UpdateHold;
end;

procedure TEnhancedGateDataModule.EAGKneeChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to CNrChannels - 1 do
    if Assigned(FEnhancedGates[Channel]) then
      FEnhancedGates[Channel].Knee_dB := Value;
...
procedure TEnhancedGateDataModule.EAGDecayChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to CNrChannels - 1 do
    if Assigned(FEnhancedGates[Channel]) then
      FEnhancedGates[Channel].Release := Value;

  // update GUI if necessary
  if EditorForm is TEditorForm then
    TEditorForm(EditorForm).UpdateDecay;
end;

procedure TEnhancedGateDataModule.EAGDuckChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  // update GUI
  if EditorForm is TEditorForm then
    with TEditorForm(EditorForm) do
      if CBDuck.Checked <> Boolean(Round(Value)) then
        CBDuck.Checked := Boolean(Round(Value));
end;

procedure TEnhancedGateDataModule.EAGStereoLinkChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  // update GUI
  if EditorForm is TEditorForm then
    with TEditorForm(EditorForm) do
      if CBStereoLink.Checked <> Boolean(Round(Value)) then
        CBStereoLink.Checked := Boolean(Round(Value));
end;
...
begin
  for SampleIndex := 0 to CNrChannels - 1 do
    ...
    begin
      FEnhancedGates[SampleIndex].InputSample
...
begin
  FLevels[0] := 0;
  FLevels[1] := 0;
  for ChannelIndex := 0 to CNrChannels - 1 do
    Move(Inputs[ChannelIndex, 0], Outputs[ChannelIndex, 0], SampleFrames * SizeOf(Single));
end;

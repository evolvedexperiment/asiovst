...
begin
  ...
  Parameter[10] := 3;
  // Mode (0 = Stereo, 1 = Mid only, 2 = Light, 1 = Light Mid only)
...
procedure TBassExtenderModule.ParamOrderChange(Sender: TObject;
...
begin
  ...
  begin
    if Assigned(FLowpass[ChannelIndex, 0]) then
      FLowpass[ChannelIndex, 0].Order := Order;
    if Assigned(FLowpass[ChannelIndex, 1]) then
      FLowpass[ChannelIndex, 1].Order := Order;
    if Assigned(FHighpass[ChannelIndex, 0]) then
      FHighpass[ChannelIndex, 0].Order := Order;
    if Assigned(FHighpass[ChannelIndex, 1]) then
      FHighpass[ChannelIndex, 1].Order := Order;
  end;
  ...
  if EditorForm is TFmBassExtender then
    TFmBassExtender(EditorForm).UpdateSplitOrder;
end;

procedure TBassExtenderModule.ParamReleaseChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FCompressor[0]) then
    FCompressor[0].Release := Value;
  if Assigned(FCompressor[1]) then
    FCompressor[1].Release := Value;

  // update GUI
  if EditorForm is TFmBassExtender then
    TFmBassExtender(EditorForm).UpdateRelease;
end;

procedure TBassExtenderModule.ParamReleaseLabel(Sender: TObject;
...
procedure TBassExtenderModule.ParamAttackDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
var
  Attack: Single;
begin
  ...
  if Attack < 1000 then
    PreDefined := AnsiString(FloatToStrF(Attack, ffGeneral, 3, 2))
  else
    PreDefined := AnsiString(FloatToStrF(Attack * 1E-3, ffGeneral, 3, 2));
end;

procedure TBassExtenderModule.ParamReleaseDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
var
  Release: Single;
begin
  Release := Parameter[index];
  if Release < 1000 then
    PreDefined := AnsiString(FloatToStrF(Release, ffGeneral, 3, 2))
  else
    PreDefined := AnsiString(FloatToStrF(Release * 1E-3, ffGeneral, 3, 2));
end;

procedure TBassExtenderModule.ParamRatioDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  if Parameter[Index] = 1000 then
    PreDefined := '1 : oo'
  else
    PreDefined := '1 : ' + AnsiString(FloatToStrF(Parameter[Index],
      ffGeneral, 3, 5));
end;

procedure TBassExtenderModule.ParamThresholdChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FCompressor[0]) then
  ...
  // update GUI
  if EditorForm is TFmBassExtender then
    TFmBassExtender(EditorForm).UpdateThreshold;
end;

procedure TBassExtenderModule.ParamRatioChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FCompressor[0]) then
  begin
    ...
    if Assigned(FCompressor[1]) then
      FCompressor[1].Ratio := FCompressor[0].Ratio;
  end;

  // update GUI
  if EditorForm is TFmBassExtender then
    TFmBassExtender(EditorForm).UpdateRatio;
end;

procedure TBassExtenderModule.ParamShapeChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FOctaveDivider[0]) then
    FOctaveDivider[0].Shape := 0.01 * Value;
  if Assigned(FOctaveDivider[1]) then
    FOctaveDivider[1].Shape := 0.01 * Value;

  // update GUI
  if EditorForm is TFmBassExtender then
    TFmBassExtender(EditorForm).UpdateShape;
end;

procedure TBassExtenderModule.ParamDividerChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  ...
  // update GUI
  if EditorForm is TFmBassExtender then
    TFmBassExtender(EditorForm).UpdateDivider;
end;

procedure TBassExtenderModule.ParamCompressionMixChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  ...
  // update GUI
  if EditorForm is TFmBassExtender then
    TFmBassExtender(EditorForm).UpdateCompressionMix;
end;

procedure TBassExtenderModule.ParamBalanceChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  ...
  // update GUI
  if EditorForm is TFmBassExtender then
    TFmBassExtender(EditorForm).UpdateBalance;
end;

procedure TBassExtenderModule.ParamSplitOrderDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := AnsiString(IntToStr(Round(2 * Parameter[Index])));
end;

procedure TBassExtenderModule.ParamFreqDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
var
  Freq: Single;
begin
  ...
  if Freq < 1000 then
    PreDefined := AnsiString(FloatToStrF(Freq, ffGeneral, 4, 4))
  else
    PreDefined := AnsiString(FloatToStrF(Freq * 1E-3, ffGeneral, 4, 4));
end;

procedure TBassExtenderModule.ParamFreqLabel(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  if Parameter[Index] < 1000 then
    PreDefined := 'Hz'
...
procedure TBassExtenderModule.ParamModeChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
...
procedure TBassExtenderModule.ParamModeDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  case Round(Parameter[Index]) of
    0:
    ...
    2:
      PreDefined := 'Light';
    3:
      PreDefined := 'Light Mid Only';
  end;
end;

procedure TBassExtenderModule.ParamAttackChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FCompressor[0]) then
    FCompressor[0].Attack := 1E-3 * Value;
  if Assigned(FCompressor[1]) then
    FCompressor[1].Attack := 1E-3 * Value;

  // update GUI
  if EditorForm is TFmBassExtender then
    TFmBassExtender(EditorForm).UpdateAttack;
end;

procedure TBassExtenderModule.ParamAttackLabel(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  if Parameter[index] < 1000 then
    PreDefined := '탎'
...
procedure TBassExtenderModule.ParamFrequencyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ChannelIndex: Integer;
begin
  ...
  begin
    if Assigned(FLowpass[ChannelIndex, 0]) then
      FLowpass[ChannelIndex, 0].Frequency := Value;
    if Assigned(FLowpass[ChannelIndex, 1]) then
      FLowpass[ChannelIndex, 1].Frequency := Value;
    if Assigned(FHighpass[ChannelIndex, 0]) then
      FHighpass[ChannelIndex, 0].Frequency := Value;
    if Assigned(FHighpass[ChannelIndex, 1]) then
      FHighpass[ChannelIndex, 1].Frequency := Value;
  end;

  // update GUI
  if EditorForm is TFmBassExtender then
    TFmBassExtender(EditorForm).UpdateSplitFrequency;
end;

procedure TBassExtenderModule.VSTModuleProcess32(const Inputs,
...
begin
  for SampleIndex := 0 to SampleFrames - 1 do
    ...
    begin
      ...
      Outputs[ChannelIndex, SampleIndex] := FBalance[0] *
        (FCompressorMix[0] * FCompressor[ChannelIndex].GainSample(L) +
...
procedure TBassExtenderModule.VSTModuleProcessMS32(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
  SampleIndex: Integer;
  L, H, M, S: Double;
begin
  ...
  begin
    ...
    L := FLowpass[1, 0].ProcessSample32(FLowpass[1, 1].ProcessSample32(L));
...
begin
  ...
  begin
    ...
    H := FHighpass[1, 0].ProcessSample64
      (FHighpass[1, 1].ProcessSample64(FSign * L));
    L := FLowpass[1, 0].ProcessSample64(FLowpass[1, 1].ProcessSample64(L));
    S := FBalance[0] * L + FBalance[1] * H;
...
procedure TBassExtenderModule.VSTModuleProcessLight32(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
  SampleIndex: Integer;
  ChannelIndex: Integer;
...
procedure TBassExtenderModule.VSTModuleProcessLightMS32(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
  SampleIndex: Integer;
  L, H, M, S: Double;

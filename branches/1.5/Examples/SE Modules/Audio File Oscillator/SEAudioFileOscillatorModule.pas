...
{$ENDIF}
  ...
  public
    ...
    procedure SubProcessNoInterpolation(const BufferOffset,
      SampleFrames: Integer);
    procedure SubProcessLinearInterpolation(const BufferOffset,
      SampleFrames: Integer);
    procedure SubProcessHermiteInterpolation(const BufferOffset,
      SampleFrames: Integer);
    procedure SubProcessBSpline4Interpolation(const BufferOffset,
      SampleFrames: Integer);
{$IFDEF UseEmbedding}
    procedure SubProcessNoInterpolationMulti(const BufferOffset,
      SampleFrames: Integer);
    procedure SubProcessLinearInterpolationMulti(const BufferOffset,
      SampleFrames: Integer);
    procedure SubProcessHermiteInterpolationMulti(const BufferOffset,
      SampleFrames: Integer);
    procedure SubProcessBSpline4InterpolationMulti(const BufferOffset,
      SampleFrames: Integer);
{$ENDIF}
  end;
...
{$IFDEF UseEmbedding}
  ...
  EnumResourceNames(HInstance, 'WAVETABLE', @EnumNamesFunc,
    LongWord(FContainedData));
...
{$IFDEF UseEmbedding}
              ...
              LoadFromResource(Integer(FFileName));
{$ENDIF}
          ChooseProcess;
        finally
...
begin
  if {$IFDEF UseEmbedding} (FContainedData.Count = 0) and
  {$ENDIF} (not FileExists(FFileName)) then
...
{$IFDEF UseEmbedding}
    if (FContainedData.Count = 1) and (FAudioData.ChannelCount > 1) then
      case FInterpolation of
        imNone:
          OnProcess := SubProcessNoInterpolationMulti;
        imLinear:
          OnProcess := SubProcessLinearInterpolationMulti;
        imHermite:
          OnProcess := SubProcessHermiteInterpolationMulti;
        imBSpline4:
          OnProcess := SubProcessBSpline4InterpolationMulti;
      end
    else
{$ENDIF}
      case FInterpolation of
        imNone:
          OnProcess := SubProcessNoInterpolation;
        imLinear:
          OnProcess := SubProcessLinearInterpolation;
        imHermite:
          OnProcess := SubProcessHermiteInterpolation;
        imBSpline4:
          OnProcess := SubProcessBSpline4Interpolation;
      end;
...
begin
  ...
  begin
    ...
    else if FMode <> lmOneShot then
      FIsPlaying := False;
  end;
end;

// The most important part, processing the audio
procedure TSEAudioFileOscillatorModule.SubProcessNoInterpolation
      ...
      begin
        ...
        begin
          FOutputBuffer[BufferOffset + Sample] :=
            FAudioData[0].ChannelDataPointer^[Round(FPosition)];

          // advance position
          FPosition := FPosition + FPlaybackSpeed[BufferOffset + Sample];

          // handle one shot
          if FMode = lmOneShot then
            FIsPlaying := (FPosition >= 0) and
              (FPosition < FAudioData[0].SampleCount)
          else
          begin
            // wrap around
            while Round(FPosition) >= FAudioData[0].SampleCount do
              FPosition := FPosition - FAudioData[0].SampleCount;
            while Round(FPosition) < 0 do
              FPosition := FPosition + FAudioData[0].SampleCount;
          end;
        end
        else
...
procedure TSEAudioFileOscillatorModule.SubProcessNoInterpolationMulti
  (const BufferOffset, SampleFrames: Integer);
var
  Sample: Integer;
  Channel: Integer;
    ...
    if FAudioData.SampleFrames > 0 then
      ...
      begin
        ...
        begin
          FOutputBuffer[BufferOffset + Sample] :=
            FAudioData[0].ChannelDataPointer^[Round(FPosition)];
          for Channel := 0 to Length(FExtraOutputBuffers) - 1 do
            FExtraOutputBuffers[Channel, BufferOffset + Sample] :=
        ...
        begin
          FOutputBuffer[BufferOffset + Sample] := 0;
          for Channel := 0 to Length(FExtraOutputBuffers) - 1 do
            FExtraOutputBuffers[Channel, BufferOffset + Sample] := 0;
        end;
      end
    else
      FillChar(FOutputBuffer[BufferOffset], SampleFrames * SizeOf(Single), 0);
...
procedure TSEAudioFileOscillatorModule.SubProcessLinearInterpolation
  (const BufferOffset, SampleFrames: Integer);
var
  Sample: Integer;
  Offset: Integer;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            FOutputBuffer[BufferOffset + Sample] := Ratio * FAudioData[0]
              .ChannelDataPointer^[0] + (1 - Ratio) * FAudioData[0]
              .ChannelDataPointer^[Offset]
          end
          else
          begin
            Ratio := FPosition - Offset;
            FOutputBuffer[BufferOffset + Sample] := Ratio * FAudioData[0]
              .ChannelDataPointer^[Offset + 1] + (1 - Ratio) * FAudioData[0]
              .ChannelDataPointer^[Offset]
          end;
...
procedure TSEAudioFileOscillatorModule.SubProcessHermiteInterpolation
  (const BufferOffset, SampleFrames: Integer);
var
  Sample: Integer;
  Offset: Integer;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            inc(Offset);
            if Offset >= FAudioData[0].SampleCount then
              Offset := 0;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            for Channel := 0 to Length(FExtraOutputBuffers) - 1 do
              FExtraOutputBuffers[Channel, BufferOffset + Sample] :=
                Hermite32_asm(Ratio, @FAudioData[Channel + 1]
                .ChannelDataPointer^[Offset]);
          end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            FOutputBuffer[BufferOffset + Sample] :=
              BSplineInterpolation4Point3rdOrder(Ratio, Data);
          end
          else
          begin
            Ratio := FPosition - Offset;
            FOutputBuffer[BufferOffset + Sample] :=
              BSplineInterpolation4Point3rdOrder(Ratio,
              @FAudioData[0].ChannelDataPointer^[Offset]);
          end;
        ...
        begin
          ...
          begin
            ...
            for Channel := 0 to Length(FExtraOutputBuffers) - 1 do
              FExtraOutputBuffers[Channel, BufferOffset + Sample] :=
                BSplineInterpolation4Point3rdOrder(Ratio,
                @FAudioData[Channel + 1].ChannelDataPointer^[Offset]);
          end;
...
class procedure TSEAudioFileOscillatorModule.GetModuleProperties
...
{$IFDEF UseEmbedding}
      ...
      begin
        ...
        str := 'DAV EAFO';
        for i := 0 to ContainedData.Count - 1 do
          str := str + ' ' + Trim(ContainedData[i]);
        if Length(str) > 31 then
          SetLength(str, 31);
...
function TSEAudioFileOscillatorModule.GetPinProperties(const Index: Integer;
...
{$IFDEF UseEmbedding}
      ...
      begin
        ...
        if Index - Integer(pinOutput) < FAudioData.ChannelCount then
          ...
          begin
            Name := 'Output';
            VariableAddress := @FExtraOutputBuffers
              [Index - Integer(pinOutput) - 1];
            Direction := drOut;
            DataType := dtFSample;
            Result := True;
          end;

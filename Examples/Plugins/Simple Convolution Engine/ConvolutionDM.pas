    ...
    try
      ...
      with ADC do
        try
          ...
          Move(ADC[0].ChannelDataPointer^[0], FFilterKernel^[0],
            ADC.SampleFrames * SizeOf(Single));
          FillChar(FFilterKernel^[ADC.SampleFrames],
            (FFFTSize - ADC.SampleFrames) * SizeOf(Single), 0);
        finally
          FreeAndNil(ADC);
        end;
...
procedure TConvolutionDataModule.PerformConvolution
  (Signal: PDAVSingleFixedArray);
begin
...
begin
  ...
  try
    ...
    repeat
      ...
      if SampleCount > FFFTSizeHalf then
        SampleCount := FFFTSizeHalf;

      for Channel := 0 to numOutputs - 1 do
      begin
        Move(FSignalTime[Channel, SampleCount], FSignalTime[Channel, 0],
          (FFFTSize - SampleCount) * SizeOf(Single));
        Move(Inputs[Channel, SamplesPos],
          FSignalTime[Channel, (FFFTSize - SampleCount)],
          SampleCount * SizeOf(Single));
        Move(FSignalTime[Channel, 0], FBuffer[0], FFFTSize * SizeOf(Single));
        PerformConvolution(@FBuffer[0]);
        Move(FBuffer[(FFFTSize - SampleCount)], Outputs[Channel, SamplesPos],
          SampleCount * SizeOf(Single));
      end;

...
begin
  ...
  begin
    ...
    ASIOTime.Flags := [atSystemTimeValid, atSamplePositionValid,
      atSampleRateValid, atSpeedValid];
...
begin
  if Assigned(FASIOHost) then
    with FASIOHost do
    begin
      FIntBufSize := Max(Integer(FASIOHost.BufferSize), BlockSize) *
        FNrOfBuffers;
...
procedure TASIOVSTModule.AHBufferSwitch32(Sender: TObject;
...
begin
  ...
  if (FIntWritePos > FIntReadPos) and
    (FIntWritePos < FIntReadPos + Integer(FASIOHost.BufferSize)) then
  ...
  FIntReadPos := (FIntReadPos + Integer(FASIOHost.BufferSize))
    mod (FNrOfBuffers * Integer(FASIOHost.BufferSize));
end;

procedure TASIOVSTModule.AHBufferSwitch64(Sender: TObject;
...
procedure TASIOVSTModule.AHShortCircuit(Sender: TObject;
  const InBuffer, OutBuffer: TDAVArrayOfSingleFixedArray);
var
  BufferByteSize: Integer;
begin
...
begin
  ...
  begin
    ...
    Inc(FIntWritePos);
    if FIntWritePos >= FIntBufSize then
      FIntWritePos := 0;
    DelayCnt := 0;
    while (DelayCnt < 500) and (FIntWritePos = FIntReadPos) and
      FASIOHost.Active do
    begin
      Inc(DelayCnt);
      Sleep(1);
...
procedure TASIOVSTModule.ASIODriverDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := FASIOHost.DriverName;
end;

procedure TASIOVSTModule.ParameterAccuracyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Value > 0.5 then
    FASIOHost.ConvertMethod := cm64
  else
    FASIOHost.ConvertMethod := cm32;
end;

procedure TASIOVSTModule.ParameterAccuracyDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
...
procedure TASIOVSTModule.ASIODriverChange(Sender: TObject; const Index: Integer;
  var Value: Single);
begin
  if not Assigned(FASIOHost) then
    Exit;
  if FASIOHost.DriverIndex = Round(Value) then
    Exit;
  FASIOHost.Active := False;
  FASIOHost.DriverIndex := Round(Value);
  InitialDelay := FASIOHost.InputLatency + FASIOHost.OutputLatency +
    Integer(FASIOHost.BufferSize);

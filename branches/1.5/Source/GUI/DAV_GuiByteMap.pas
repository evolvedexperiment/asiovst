...
type
  ...
  public
    ...
    procedure Draw(ByteMap: TGuiCustomByteMap; Alpha: Byte = $FF);
      overload; virtual;
    procedure Draw(ByteMap: TGuiCustomByteMap; X, Y: Integer);
      overload; virtual;
    procedure Draw(ByteMap: TGuiCustomByteMap; X, Y: Integer; Alpha: Byte);
      overload; virtual;
    procedure DrawTransparent(ByteMap: TGuiCustomByteMap); overload; virtual;
    procedure DrawTransparent(ByteMap: TGuiCustomByteMap; X, Y: Integer);
      overload; virtual;
...
begin
  ...
  begin
    ...
    for Index := Top to Bottom - 1 do
      System.Move(ByteMap.ValuePointer[Left - X, Top - Y + Index]^,
...
begin
  ...
  begin
    ...
    // blend scanlines
    for Index := Top to Bottom - 1 do
      System.Move(ByteMap.ValuePointer[Left - X, Index - Y]^,
        ValuePointer[Left, Index]^, Right - Left);
  end;
end;

procedure TGuiCustomByteMap.Draw(ByteMap: TGuiCustomByteMap; Alpha: Byte = $FF);
begin
...
begin
  for Index := 0 to FWidth * FHeight - 1 do
    FDataPointer^[Index] := ((FDataPointer^[Index] * Factor + $80) shr 8);
end;
...
begin
  ...
  if Result then
    Result := CompareMem(FDataPointer, ByteMap.FDataPointer, FDataSize);
end;
...
begin
  ...
  if HasData and (Filer.Ancestor <> nil) then
    HasData := not((Filer.Ancestor is TGuiCustomByteMap) and
...
procedure TGuiCustomByteMap.HorizontalLine(FromX, ToX, Y: Integer; Data: Byte);
var
  X: Integer;
begin
  if FromX > ToX then
...
begin
  if FromY = ToY then
    HorizontalLine(FromX, ToX, FromY, Data)
  else if FromX = ToX then
    VerticalLine(FromX, FromY, ToY, Data)
  else
  begin
    dx := ToX - FromX;
    dy := ToY - FromY;

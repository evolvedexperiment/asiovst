...
procedure TConvoFXLiteDataModule.ParameterMaximumIROrderChange(Sender: TObject;
...
procedure TConvoFXLiteDataModule.ParameterDampingChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FDamping := Value;
  ImpulseResponseChanged;

  if EditorForm is TFmConvoFXLite then
    TFmConvoFXLite(EditorForm).UpdateDamping;
end;

procedure TConvoFXLiteDataModule.ParameterGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  ...
  if EditorForm is TFmConvoFXLite then
    TFmConvoFXLite(EditorForm).UpdateGain;
end;

procedure TConvoFXLiteDataModule.ParameterIRChange(Sender: TObject;
  const Index: Integer; var Value: Single);
const
...
begin
  ...
  begin
    ...
    while not FIRUpdateThread.Suspended do
      Sleep(1);
  ...
  if EditorForm is TFmConvoFXLite then
    TFmConvoFXLite(EditorForm).UpdateIRSelect;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Move(FImpulseResponse[Channel]^[0], FTempIR[Channel]^[0],
        FTempIRSize[Channel] * SizeOf(Single));
    end
    else
    begin
      FTempIRSize[Channel] := Round((FImpulseLength[Channel] - 4) * SampleRate /
        CDefaultSampleRate);
      ...
      begin
        ...
        Assert(Pos - r >= 0);
        Assert(Pos - r <= 1);
        FTempIR[Channel]^[Sample] := Hermite32_asm(Pos - r,
...
begin
  ...
  begin
    ...
    IR[Sample] := s * IR[Sample] + (1 - s) * FDampingFilter.ProcessSample64(IR[Sample]);
  end;
end;

procedure TConvoFXLiteDataModule.ParameterLatencyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
...
{$IFDEF DirectUpdate}
  UpdateImpulseResponse;
{$ELSE}
  if FIRUpdateThread.Suspended then
...
begin
  ...
  try
    for Channel := 0 to Length(FConvolution) - 1 do
      FConvolution[Channel].ProcessBlock(
        @Inputs[Channel, 0],

...
type
  ...
  public
    ...
    function GetInputProperties(const InputIndex: Integer): TVstPinProperties;
    ...
    function GetOutputProperties(const OutputIndex: Integer): TVstPinProperties;
...
begin
  ...
  begin
    GDebugLog.Add(Text);
    GDebugLog.SaveToFile('VstHostDebug.log');
  end;
end;
{$ENDIF}
...
begin
 Result := '';
 if effFlagsHasEditor     in EffOpts then
   Result := Result + 'HasEditor, ';
 if effFlagsHasClip       in EffOpts then
   Result := Result + 'HasClip, ';
 if effFlagsHasVu         in EffOpts then
   Result := Result + 'HasVU, ';
 if effFlagsCanMono       in EffOpts then
   Result := Result + 'CanMono, ';
 if effFlagsCanReplacing  in EffOpts then
   Result := Result + 'CanReplacing, ';
 if effFlagsProgramChunks in EffOpts then
   Result := Result + 'ProgramChunks, ';
 if effFlagsIsSynth       in EffOpts then
   Result := Result + 'IsSynth, ';
 if effFlagsNoSoundInStop in EffOpts then
   Result := Result + 'NoSoundInStop, ';
 if effFlagsExtIsAsync    in EffOpts then
   Result := Result + 'ExtIsAsync, ';
 if effFlagsExtHasBuffer  in EffOpts then
   Result := Result + 'ExtHasBuffer, ';
...
begin
 if vtiTransportChanged in VstTimeInfoFlags then
   Result := 1 else Result := 0;
 if vtiTransportPlaying in VstTimeInfoFlags then
   Result := Result + 1 shl 1;
 if vtiTransportCycleActive in VstTimeInfoFlags then
   Result := Result + 1 shl 2;
 if vtiTransportRecording in VstTimeInfoFlags then
   Result := Result + 1 shl 3;

 if vtiAutomationWriting in VstTimeInfoFlags then
   Result := Result + 1 shl 6;
 if vtiAutomationReading in VstTimeInfoFlags then
   Result := Result + 1 shl 7;

 if vtiNanosValid in VstTimeInfoFlags then
   Result := Result + 1 shl 8;
 if vtiPpqPosValid in VstTimeInfoFlags then
   Result := Result + 1 shl 9;
 if vtiTempoValid in VstTimeInfoFlags then
   Result := Result + 1 shl 10;
 if vtiBarsValid in VstTimeInfoFlags then
   Result := Result + 1 shl 11;
 if vtiCyclePosValid in VstTimeInfoFlags then
   Result := Result + 1 shl 12;
 if vtiTimeSigValid in VstTimeInfoFlags then
   Result := Result + 1 shl 13;
 if vtiSmpteValid in VstTimeInfoFlags then
   Result := Result + 1 shl 14;
 if vtiClockValid in VstTimeInfoFlags then
   Result := Result + 1 shl 15;
end;

function Integer2TimeInfoFlags(Value: Integer): TVstTimeInfoFlags;
begin
  if (Value and 1) <> 0 then
    Result := [vtiTransportChanged]
  else
    Result := [];

  if (Value and (1 shl  1)) <> 0 then
    Result := Result + [vtiTransportPlaying];
  if (Value and (1 shl  2)) <> 0 then
    Result := Result + [vtiTransportCycleActive];
  if (Value and (1 shl  3)) <> 0 then
    Result := Result + [vtiTransportRecording];

  if (Value and (1 shl  6)) <> 0 then
    Result := Result + [vtiAutomationWriting];
  if (Value and (1 shl  7)) <> 0 then
    Result := Result + [vtiAutomationReading];

  if (Value and (1 shl  8)) <> 0 then
    Result := Result + [vtiNanosValid];
  if (Value and (1 shl  9)) <> 0 then
    Result := Result + [vtiPpqPosValid];
  if (Value and (1 shl 10)) <> 0 then
    Result := Result + [vtiTempoValid];
  if (Value and (1 shl 11)) <> 0 then
    Result := Result + [vtiBarsValid];
  if (Value and (1 shl 12)) <> 0 then
    Result := Result + [vtiCyclePosValid];
  if (Value and (1 shl 13)) <> 0 then
    Result := Result + [vtiTimeSigValid];
  if (Value and (1 shl 14)) <> 0 then
    Result := Result + [vtiSmpteValid];
  if (Value and (1 shl 15)) <> 0 then
    Result := Result + [vtiClockValid];
end;
  ...
  if VstDllHandle <> 0 then
    try
      ...
      if Result = False then
        Result := GetProcAddress(VstDllHandle, 'VSTPluginMain') <> nil;
    finally
...
begin
  ...
  {$ENDIF}
    ...
    begin
      ...
      begin
        Plugin := GTempPlugin;
        if Assigned(Plugin.Collection) and
          (Plugin.Collection.Owner is TCustomVstHost) then
    ...
    case TAudiomasterOpcode(Opcode) of
      amAutomate:
        begin
          if Assigned(Plugin) then
            Plugin.AudioMasterAutomate(Index, Opt);
          Result := 0;
        end;
      ...
      amIdle:
        begin
          if Assigned(Plugin) then
            Plugin.AudioMasterIdle;
        end;

      amCurrentId:
        begin
          if Assigned(Plugin) then
            Result := Plugin.AudioMasterCurrentId
          else
            Result := 0;
        end;
      ...
      amPinConnected:
        begin
          if Assigned(Plugin) then
            Result := Integer(Plugin.AudioMasterPinConnected(Index, Value = 0))
          else
            Result := 0;
        end;
      ...
      amWantMidi:
        begin
          if Assigned(Plugin) then
            Plugin.AudioMasterWantMidi;
        end;

      amGetTime:
        begin
          if Assigned(Host) then
            Result := LongInt(Host.AudioMasterGetTime)
          else
            Result := 0;
        end;
      ...
      amProcessEvents:
        begin
          if Assigned(Plugin) then
            Plugin.AudioMasterProcessEvents(ptr);
        end;

      amSetTime:
        begin
          if Assigned(Host) then
            Host.AudioMasterSetTime(ptr, Integer2TimeInfoFlags(Value));
        end;

      amTempoAt:
        begin
          if Assigned(Host) then
            Result := Host.AudioMasterTempoAt(Value)
          else
            Result := 1200000;
        end;
      ...
      amGetNumAutomatableParameters:
        begin
          if Assigned(Host) then
            Result := Host.AudioMasterGetNumAutomatableParameters
          else
            Result := 0;
        end;
      ...
      amGetParameterQuantization:
        begin
          if Assigned(Host) then
            Result := Host.AudioMasterGetParameterQuantization(Value)
          else
            Result := 0;
        end;
      ...
      amIOChanged:
        begin
          if Assigned(Plugin) then
            Plugin.AudioMasterIOChanged;
        end;

      amNeedIdle:
        begin
          if Assigned(Plugin) then
            Plugin.AudioMasterNeedIdle;
        end;

      amSizeWindow:
        begin
          if Assigned(Plugin) then
            Result := Integer(Plugin.AudioMasterSizeWindow(Index, Value));
        end;

      amGetSampleRate:
        begin
          if Assigned(Host) then
            Result := Round(Host.AudioMasterGetSampleRate)
          else
            Result := CDefaultSampleRate;
        end;

      amGetBlockSize:
        begin
          if Assigned(Host) then
            Result := Host.BlockSize
          else
            Result := CDefaultBlockSize;
        end;

      amGetInputLatency:
        begin
          if Assigned(Host) then
            Result := Host.FInputLatency
          else
            Result := 0;
        end;
      ...
      amGetPreviousPlug:
        begin
          if Assigned(Host) then
            Result := Integer(Host.AudioMasterGetPreviousPlug(Value))
          else
            Result := 0;
        end;
      ...
      amGetNextPlug:
        begin
          if Assigned(Host) then
            Result := Integer(Host.AudioMasterGetNextPlug(Value))
          else
            Result := 0;
        end;
      ...
      amWillReplaceOrAccumulate:
        begin
          if Assigned(Plugin) then
            Result := Integer(Plugin.FReplaceOrAccumulate)
          else
            Result := 0;
        end;
      ...
      amGetCurrentProcessLevel:
        begin
          if Assigned(Plugin) then
            Result := Integer(Plugin.FProcessLevel)
          else
            Result := 0;
        end;
      ...
      amGetAutomationState:
        begin
          if Assigned(Plugin) then
            Result := Integer(Plugin.FAutomationState)
          else
            Result := 0;
        end;
      ...
      amOfflineStart:
        begin
          if Assigned(Plugin) and Assigned(Plugin.FOnAMOfflineStart) then
            Plugin.FOnAMOfflineStart(Plugin); // audioMasterOfflineStart
        end;

      amOfflineRead:
        begin
          if Assigned(Plugin) and Assigned(Plugin.FOnAMOfflineRead) then
            Plugin.FOnAMOfflineRead(Plugin,ptr); // audioMasterOfflineRead, ptr points to offline structure, see below. return 0: error, 1 ok
        end;

      amOfflineWrite:
        begin
          if Assigned(Plugin) then
            if Assigned(Plugin.FOnAMOfflineWrite) then
              Plugin.FOnAMOfflineWrite(Plugin,ptr); // audioMasterOfflineWrite, same as read
        end;

      amOfflineGetCurrentPass:
        begin
          if Assigned(Plugin) then
            if Assigned(Plugin.FOnAMOfflineGetCurrentPass) then
              Plugin.FOnAMOfflineGetCurrentPass(Plugin); // audioMasterOfflineGetCurrentPass
        end;

      amOfflineGetCurrentMetaPass:
        begin
          if Assigned(Plugin) then
            if Assigned(Plugin.FOnAMOfflineGetCurrentMetaPass) then
              Plugin.FOnAMOfflineGetCurrentMetaPass(Plugin); // audioMasterOfflineGetCurrentMetaPass
        end;

      amSetOutputSampleRate:
        begin
          if Assigned(Plugin) then
            if Assigned(Plugin.FOnAMSetOutputSampleRate) then
              Plugin.FOnAMSetOutputSampleRate(Plugin,opt);
          if Assigned(Host) then Host.VstTimeInfo.SampleRate := Opt;
          // raise Exception.Create('audioMasterSetOutputSampleRate, for variable i/o, sample rate in <opt>');
        end;

      amGetOutputspeakerArrangement:
        begin
          {$IFDEF Debug}
          raise Exception.Create('TODO: audioMasterGetSpeakerArrangement, (long)input in <value>, output in <ptr>')
          {$ENDIF Debug};
        end;
      ...
      amGetVendorString:
        begin
          try
            ...
            {$IFDEF FPC}
            if Assigned(Host) then
              Move(Host.VendorString[1], ptr^, Length(Host.VendorString))
            else
              Result := 0;
            {$ELSE}
            if Assigned(Host) then
              StrPCopy(PAnsiChar(ptr), Host.VendorString)
            else
              StrPCopy(PAnsiChar(ptr), 'Delphi ASIO & VST Project');
            {$ENDIF}
          except
            Result := 0;
          end;
      ...
      amGetProductString:
        begin
          try
            Result := 1;
            {$IFDEF FPC}
            if Assigned(Host) and Assigned(ptr) then
              Move(Host.ProductString[1], ptr^, Length(Host.ProductString))
            else
              Result := 0;
            {$ELSE}
            if Assigned(Host) then
              StrPCopy(PAnsiChar(ptr), Host.ProductString)
            else
              if Assigned(ptr) then
                StrPCopy(PAnsiChar(ptr), 'Delphi VST Host')
              else
                Result := 0;
            {$ENDIF}
          except
      ...
      amVendorSpecific:
        begin
          if Assigned(Plugin) then
            Plugin.AudioMasterVendorSpecific(index, value, ptr, opt)
          else
            Result := 0;
        end;
      ...
      amSetIcon:
        begin
          {$IFDEF Debug}
          ShowMessage('TODO: audioMasterSetIcon, void* in <ptr>, format not defined yet, Could be a CBitmap .');
          {$ENDIF Debug}
        end;
      ...
      amCanDo:
        begin
          if Assigned(Host) then
            Result := Integer(Host.AudioMasterCanDo(PAnsiChar(ptr)))
          else
            Result := 0;
        end;
      ...
      amGetLanguage:
        begin
          if Assigned(Host) then
            Result := Integer(Host.FLanguage)
          else
            Result := Integer(hlUnknown);
        end;
  ...
  //          PVstWindow(ptr).winHandle := (GHostWindows.Items[i] as TForm).Handle;
     ...
     amGetDirectory:
       begin
         if Assigned(Host) then
           Result := LongInt(PAnsiChar(AnsiString(Host.FPlugInDir)));
       end;

     amUpdateDisplay:
       begin
         if Assigned(Plugin) then
           Plugin.AudioMasterUpdateDisplay;
       end;

     amBeginEdit:
       begin
         if Assigned(Plugin) then
           Plugin.AudioMasterBeginEdit(Index);
       end;

     amEndEdit:
       begin
         if Assigned(Plugin) then
           Plugin.AudioMasterEndEdit(Index);
       end;
  ...
  //                   StrCopy(PVstFileSelect(ptr).returnPath,PAnsiChar(TOpenDialog(GHostDialog).FileName));
                 ...
                 begin
                   ...
                   for i := 0 to PVstFileSelect(ptr).nbFileTypes - 1 do
                     Filter := Filter +
                       ...
                       ' (*.' + string(PVstFileType(PVstFileSelect(ptr).fileTypes).dosType) +
                       ')|*.' + string(PVstFileType(PVstFileSelect(ptr).fileTypes).dosType) + '|';
                   if TOpenDialog(GHostDialog).Execute then
                   begin
                     PVstFileSelect(ptr).returnPath := PAnsiChar(AnsiString(FileName));
                     PVstFileSelect(ptr).sizeReturnPath := Length(FileName);
                   end;
             ...
             kVstDirectorySelect:
               begin
                 {$IFDEF Debug} ShowMessage('TODO: Not implemented!') {$ENDIF Debug};
               end;
           end;

           if PVstFileSelect(ptr).returnPath = nil then
             FreeAndNil(GHostDialog);
        ...
        begin
          ...
          if Assigned(GHostDialog) then
            case PVstFileSelect(ptr).Command of
              kVstFileLoad:
                begin
                  if TOpenDialog(GHostDialog).Title = string(PVstFileSelect(ptr).Title) then
                    FreeAndNil(GHostDialog);
                end;

              kVstFileSave:
                begin
                  if TSaveDialog(GHostDialog).Title = string(PVstFileSelect(ptr).Title) then
                    FreeAndNil(GHostDialog);
                end;
              ...
              kVstDirectorySelect:
                ...
                {$IFDEF Debug}
                raise EVstHostException.Create('TODO: close a fileselector operation with VstFileSelect* in <ptr>: Must be always called after an open !')
                {$ENDIF Debug};
            end;
          {$ENDIF}
          {$ENDIF}
      ...
      amEditFile:
        begin
          {$IFDEF Debug}
          raise EVstHostException.Create('TODO: open an editor for audio (defined by XML text in ptr')
          {$ENDIF Debug}
        end;
      amGetChunkFile:
        begin
          {$IFDEF Debug}
          raise EVstHostException.Create('TODO: get the native path of currently loading bank or project')
          {$ENDIF Debug}
        end
     else
       try
         raise EVstHostException.Create('Check: ' +
           IntToStr(Integer(Opcode)) + ' - ' + IntToStr(index) + ' - ' +
           IntToStr(value) + ' - ' + FloatToStr(opt));
       except
         Result := 0;
       end;
    end;
...
//  if AOwner <> nil then
  ...
  try
    ...
    if ValueExists('VstPluginsPath') then
      FPlugInDir := ReadString('VstPluginsPath')
    {$IFDEF VstHostGUI} {$IFNDEF FMX}
    else
      FPlugInDir := ExtractFileDir(Application.ExeName)
    {$ENDIF}{$ENDIF};
    CloseKey;
  finally
    Free;
...
begin
 ...
 // float precision is maintained in automation. Index = -1 for all parameters
  ...
  if Index = -1 then
    ...
    {$IFDEF Debug} raise EVstHostException.Create('TODO: audioMasterGetParameterQuantization, returns the Integer value for +1.0 representation') {$ELSE} Result := 0 {$ENDIF Debug}
end;
...
begin
  {$IFDEF Debug}
  raise EVstHostException.Create('TODO: audioMasterGetNextPlug, output pin in <value> (-1: first to come), returns cEffect*')
  {$ELSE}
  Result := nil
  {$ENDIF Debug};
end;
...
begin
  Assert(Assigned(VstPlugIns));
  for PlugIndex := 0 to VstPlugIns.Count - 1 do
    if Assigned(VstPlugIns[PlugIndex].FVstEffect) then
      VstPlugIns[PlugIndex].SetBlockSize(FBlockSize);
end;
...
begin
  try
    if Active then
      Close;
    if Assigned(FVstOfflineTasks) then
      FreeAndNil(FVstOfflineTasks);
    ...
    try
      ...
      if EditVisible then
        CloseEdit;
      {$IFNDEF FMX}
      if Assigned(GUIControl) and FGUIControlCreated then
        FreeAndNil(FGUIControl);
     {$ENDIF}
     {$ENDIF}
    finally
      Unload;
...
begin
  ...
  begin
    ...
    begin
      if FileExists(Self.FVstDllFileName) then
        LoadFromFile(Self.FVstDllFileName)
      else
      if Assigned(Self.FVstEffect) then
        LoadFromVSTEffect(Self.FVstEffect);
...
begin
  ...
  {$ENDIF}
    ...
    if VstCanDo('bypass') <> 0 then
      FVSTCanDos := [vcdBypass]
    else
      FVSTCanDos := [];
    ...
    if VstCanDo('receiveVstMidiEvent') <> 0 then
      FVSTCanDos := FVSTCanDos + [vcdReceiveVstMidiEvent];

    if VstCanDo('sendVstMidiEvent') <> 0 then
      FVSTCanDos := FVSTCanDos + [vcdSendVstMidiEvent];

    if VstCanDo('midiProgramNames') <> 0 then
      FVSTCanDos := FVSTCanDos + [vcdMidiProgramNames];
    ...
    // update blocksize
    if Collection.Owner is TCustomVstHost then
      SetBlockSize(TCustomVstHost(Collection.Owner).BlockSize)
    else
      SetBlockSize(CDefaultBlockSize);

   // update samplerate
   if Collection.Owner is TCustomVstHost then
     SetSampleRate(TCustomVstHost(Collection.Owner).VstTimeInfo.SampleRate)
   else
     SetSampleRate(CDefaultSampleRate);

   // eventually disable bypass
   if vcdBypass in FVSTCanDos then
     SetBypass(False);
...
begin
  ...
  if FVstEffect <> nil then
    FVstEffect.Process(FVstEffect, Inputs, Outputs, SampleFrames);
end;
...
begin
  ...
  if FVstEffect <> nil then
    FVstEffect.Process32Replacing(FVstEffect, Inputs, Outputs, SampleFrames);
end;
...
begin
  ...
  if FVstEffect <> nil then
    FVstEffect.Process64Replacing(FVstEffect, Inputs, Outputs, SampleFrames);
end;
...
begin
  ...
  if FVstEffect <> nil then
    FVstEffect.SetParameter(FVstEffect, Index, Parameter);
end;
...
begin
  ...
  if FVstEffect = nil then
    ...
    Result := FVstEffect.GetParameter(FVstEffect, Index);
end;
...
begin
  if FActive then
    Result := VstDispatch(effGetProgram)
  else
    Result := -1;
  FProgramNr := Result;
end;
...
begin
  if FActive then
    VstDispatch(effSetProgramName, 0, 0, PAnsiChar(@NewName[1]));
end;
...
begin
  ...
  try
    ...
    if FActive then
      ...
      begin
        // check is not part of the official specification
        Result := StrPas(Temp);
      end;
...
begin
  ...
  try
    ...
    if FActive then
      ...
      begin
        // check is not part of the specification
        Result := StrPas(Temp);
      end;

     // check whether the result string accords to the specs
     if Assigned(Collection) and Assigned(TVSTPlugins(Collection).VSTHost) then
       if TVSTPlugins(Collection).VSTHost.CheckStringLengths and (Length(Result) > 8) then
         raise EVstHostException.Create('String too short');
  finally
    // dispose memory
    Dispose(Temp);
  end;
...
begin
  ...
  try
   FillChar(Temp^, Lngth, 0);
   if FActive then
     if VstDispatch(effGetParamDisplay, index, 0, Temp) = 0 then
     begin
       // check is not part of the specification
       Result := StrPas(Temp);
     end;
...
begin
  FNeedIdle := True;
  if Assigned(FOnAMNeedIdle) then
    FOnAMNeedIdle(Self);

  if (Collection.Owner <> nil) and TCustomVSTHost(Collection.Owner).FAutoIdle then
    Idle;
end;
...
begin
  if FActive then
    Result := VstDispatch(effGetVu) * Divisor
  else
    Result := -1;
end;
...
begin
  if Assigned(FVstEffect) then
    Result := FVstEffect^.RealQualities
  else
    Result := 0;
end;
...
begin
  ...
  case FGUIStyle of
    gsDefault:
      begin
        if (effFlagsHasEditor in FVstEffect.EffectFlags) then
          theRect := EditGetRect
        else
          theRect := CalculateGuiRectList;
      end;
    ...
    gsParameterSelector:
      begin
        theRect := CalculateGuiRectSelector;
      end;
    ...
    gsCustom :
      begin
        ...
        if Assigned(FGUIControl) then
          theRect := Rect(0, 0, FGUIControl.Width, FGUIControl.Height)
        else
          theRect := Rect(0, 0, 0, 0);
        {$ENDIF}
     end;
...
begin
  if not Active then
    raise EVstHostException.Create('Plugin not active!');

  if Control = 0 then
    raise EVstHostException.Create('Control must exist!');
...
//  else raise EVstHostException.Create('Editor is already open!');
  ...
  if Assigned(FOnShowEdit) then
    FOnShowEdit(Self, GUIControl);
end;
...
begin
  ...
  begin
    ...
    Min := 0;
    Max := 1000;
    OnChange := ControlChangeSelector;
    TickMarks := tmBottomRight;
    TickStyle := tsNone;//Auto;
  end;
  ...
  begin
   ...
   ItemHeight := 13;
   TabOrder := 2;
   OnChange := GuiParameterChangeSelector;
   Text := '';
   ItemIndex := 0;
...
begin
  ...
  try
    ...
    Alignment := taCenter;
    for i := 0 to numParams - 1 do
      if Canvas.TextWidth(string(GetParamName(i)) + ':_') > MaxParamWidth then
        MaxParamWidth := Canvas.TextWidth(string(GetParamName(i)) + ':_');
  finally
    Free;
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      if GetParamLabel(Tag) <> '' then
        Str := GetParamDisplay(Tag) + ' ' + GetParamLabel(Tag)
      else
      ...
      begin
        ...
        if GetParamLabel(Tag) <> '' then
          Lbl.Caption := string(Str) + ' ' + string(GetParamLabel(Tag))
        else
          Lbl.Caption := string(Str);
...
begin
  ...
  if not Assigned(FGUIControl) then
    raise EVstHostException.Create('Editor not instanciated yet');
...
begin
  ...
  try
    ...
    if (GuiControlIndex >= 0) and (GuiControlIndex < numParams) then
      Position := Round(Parameter[GuiControlIndex] * 1000);
...
begin
  if FEditOpen then
    if not Boolean(VstDispatch(effEditClose)) then; // ToDo
  FEditOpen := False;
end;
...
begin
  ...
  try
    ...
    if (effFlagsHasEditor in FVstEffect.EffectFlags) {$IFNDEF FMX} and (FGUIStyle = gsDefault) {$ENDIF} then
      EditClose
    {$IFNDEF FMX}
    else if Assigned(FGUIElements) then
      FreeAndNil(FGUIElements);
    if Assigned(GUIControl) and FGUIControlCreated then
      FreeAndNil(FGUIControl); //and (not FExternalForm) then
  {$ENDIF}
  finally
    FEditOpen := False;
  end;
...
begin
  if FEditOpen then
    Result := VstDispatch(effEditIdle)
  else
    Result := 0;
end;
...
begin
  ...
  try
    ...
    if FActive then
    begin
      if Length(ValueString) > Lngth then
        SetLength(ValueString, Lngth);
...
function TCustomVstPlugIn.ConnectInput(const InputIndex: Integer; const State: Boolean): Integer;
begin
  if FActive then
    Result := VstDispatch(effConnectInput, InputIndex, Integer(State))
  else
    Result := -1;
end;

function TCustomVstPlugIn.ConnectOutput(const OutputIndex: Integer; const State: Boolean): Integer;
begin
  if FActive then
    Result := VstDispatch(effConnectOutput, OutputIndex, Integer(State))
  else
    Result := -1;
end;

function TCustomVstPlugIn.GetInputProperties(const InputIndex: Integer): TVstPinProperties;
begin
  FillChar(Result, SizeOf(TVstPinProperties), 0);
  if FActive then
    VstDispatch(effGetInputProperties, InputIndex, 0, @Result);
end;

function TCustomVstPlugIn.GetIORatio: Single;
begin
  if Assigned(FVstEffect) then
    Result := FVstEffect^.IORatio
  else
    Result := 1;
end;
...
begin
  if Assigned(FVstEffect) then
    Result := FVstEffect^.OffQualities
  else
    Result := 0;
end;

function TCustomVstPlugIn.GetOutputProperties(const OutputIndex: Integer): TVstPinProperties;
begin
  FillChar(Result, SizeOf(TVstPinProperties), 0);
  if FActive then
    VstDispatch(effGetOutputProperties, OutputIndex, 0, @Result);
end;

function TCustomVstPlugIn.GetPlugCategory: TVstPluginCategory;
begin
  if FActive then
    Result := TVstPluginCategory(VstDispatch(effGetPlugCategory))
  else
    Result := vpcUnknown;
end;
...
begin
  if FActive then
    Result := VstDispatch(effGetCurrentPosition)
  else
    Result := -1;
end;
...
begin
  if FActive then
    Result := VstDispatch(effGetDestinationBuffer)
  else
    Result := -1;
end;
...
begin
  ...
  try
   ...
   // check whether the Result string accords to the specs
   if Assigned(Collection) and Assigned(TVSTPlugins(Collection).VSTHost) then
     if TVSTPlugins(Collection).VSTHost.CheckStringLengths then
...
begin
  if FVSTCanDosScannedComplete then
    Result := FVstCanDos
  else
  begin
...
begin
  if FActive then
    Result := VstDispatch(effGetTailSize) else Result := -1;
end;
...
begin
  if Assigned(FVstEffect) then
    with FVstEffect^ do
      Result := uniqueID[3] + uniqueID[2] + uniqueID[1] + uniqueID[0]
  else
    Result := '';
end;
...
begin
  if Assigned(FOnAMIOChanged) then
    FOnAMIOChanged(Self);
end;
...
begin
  if FActive then
    Result := VstDispatch(effGetParameterProperties, Index, 0, @ParameterProperties) <> 0
  else
    Result := False;
end;
...
begin
  if FActive then
    Result := VstDispatch(effKeysRequired)
  else
    Result := -1;
end;
...
begin
  if FActive then
    Result := VstDispatch(effGetVstVersion)
  else
    Result := -1;
end;
...
begin
  if FActive then
    VstDispatch(effSetEditKnobMode, 0, Integer(Mode));
end;
{$ENDIF}
...
begin
  ...
  if FActive then
    Result := VstDispatch(effGetMidiProgramName, 0, 0, @MidiProgramName, 0)
  else
    Result := 0;
end;
...
begin
  ...
  if FActive then
    Result := VstDispatch(effGetCurrentMidiProgram, Channel, 0, @MidiProgramName, 0)
  else
    Result := 0;
end;
...
begin
  ...
  if FActive then
    Result := VstDispatch(effGetMidiProgramCategory, Channel, 0, @MidiProgramCategory, 0)
  else
    Result := 0;
end;
...
begin
  ...
  if FActive then
    Result := VstDispatch(effHasMidiProgramsChanged, Channel, 0, nil, 0)
  else
    Result := 0;
end;
...
begin
  ...
  if FActive then
    Result := VstDispatch(effGetMidiKeyName, Channel, 0, @MidiKeyName, 0)
  else
    Result := 0;
end;
...
begin
  ...
  if FActive then
    VstDispatch(effSetTotalSampleToProcess);
end;
...
begin
  ...
  i := Integer(PanLaw);
  if FActive then
    VstDispatch(effSetPanLaw, 0, i, nil, Gain);
end;

procedure TCustomVstPlugIn.AudioMasterAutomate(Index: Integer; Value: Single);
begin
  if Assigned(FOnAMAutomate) then
    FOnAMAutomate(Self, Index, Value);

  {$IFDEF VstHostGUI}
  {$IFNDEF FMX}
  // eventually update GUI
  if Assigned(FGUIElements) then
    UpdateParameterOnGui(Index);
  {$ENDIF}
  {$ENDIF}
end;
...
begin
  if Assigned(FOnAMBeginEdit) then
    FOnAMBeginEdit(Self, ParameterIndex);
end;

function TCustomVstPlugIn.AudioMasterPinConnected(Index: Integer;
  PinConnected: Boolean): Boolean;
begin
  if Assigned(FOnAMPinConnected) then
    Result := not FOnAMPinConnected(Self, Index, PinConnected)
  else
    Result := False;
end;
...
begin
  if Assigned(FOnProcessEvents) then
    FOnProcessEvents(Self, VstEvents);
end;
...
begin
  Result := False;
  {$IFDEF VstHostGUI}
  if Pos('DASH', UpperCase(string(VendorString))) > 0 then
    Result := False else
  if Pos('WUSIK', UpperCase(string(VendorString))) > 0 then
    Result := False else
  {$IFNDEF FMX}
  if Assigned(GUIControl) then
  begin
...
begin
  if Assigned(FOnAMUpdateDisplay) then
    FOnAMUpdateDisplay(Self);

  {$IFDEF VstHostGUI}
  {$IFNDEF FMX}
  // eventually update all GUI elements
  if Assigned(FGUIElements) then
    UpdateParameterOnGui(-1);
  {$ENDIF}
  {$ENDIF}
end;
...
begin
  if Assigned(FOnVendorSpecific) then
    Result := FOnVendorSpecific(index, value, ptr, opt)
  else
    Result := 0;
end;
...
begin
  if Assigned(FOnAMWantMidi) then
    FOnAMWantMidi(Self);
end;
...
begin
  if Assigned(FOnAMEndEdit) then
    FOnAMEndEdit(Self, ParameterIndex);
end;
  ...
  if Collection.Owner <> nil then
    ...
    begin
      ...
      for PluginIndex := 0 to VstPlugIns.Count - 1 do
        if VstPlugIns[PluginIndex].EditVisible then
          VstPlugIns[PluginIndex].EditIdle;
    end;
 {$ENDIF}
end;
    ...
    gsDefault, gsParameterList :
      begin
        ...
        begin
          ...
          if Assigned(SB) then
            GuiParameterChangeList(SB);
        end
        else
        for ParamIndex := 0 to numParams - 1 do
          UpdateParameterOnGui(ParamIndex);
     end;
...
begin
  ...
  if FActive then
    Result := VstDispatch(effBeginLoadBank, 0, 0, PatchChunkInfo)
  else
    Result := 0;
end;
...
begin
  ...
  if FActive then
    Result := VstDispatch(effBeginLoadProgram, 0, 0, PatchChunkInfo)
  else
    Result := 0;
end;
...
begin
  ...
  if Assigned(FMainFunction) then
    FVstEffect := FMainFunction(@GAudioMaster)
  {$IFDEF jBridge}
  else
  if Assigned(FJBridgeMainFunction) then
    FVstEffect := FJBridgeMainFunction(@GAudioMaster, @FJBridgeDLL[1])
  {$ENDIF};
  ...
  if FVstEffect = nil then
    raise EVstHostException.Create(RStrLoadingFailed);

  // check if the VstEffect pointer is valid
  if FVstEffect.Magic <> 'PtsV' then
  begin
    // reversed (due to reversed byte order)
    raise EVstHostException.Create(RStrVSTPluginNotValid);
  end;

  // set host related variables
  FVstEffect.ReservedForHost := Self;
  if Assigned(Collection) then
    FVstEffect.Resvd2 := Collection.Owner
  else
    FVstEffect.Resvd2 := nil;
...
begin
  if not FileExists(FileName) then
    raise EVstHostException.CreateFmt(RStrFileDoesNotExist, [FileName]);
...
//    FVstDllHandle := LoadLibraryEx(PAnsiChar(DLLFileName), 0, DONT_RESOLVE_DLL_REFERENCES);
    ...
    begin
      ...
      begin
        ...
        if CheckIsBootStrapDll(Filename) then
          raise EVstHostException.Create('Error: This is a jBridge bootstrap dll!');

        with TRegistry.Create do
        try
          RootKey := HKEY_LOCAL_MACHINE;
          if OpenKeyReadOnly(PROXY_REGKEY) then
            {$IFDEF DELPHI2010_UP}
            str := GetDataAsString(PROXY_REGVAL)
            {$ELSE}
            str := ReadString(PROXY_REGVAL)
            {$ENDIF}
          else
            raise EVstHostException.Create('Error: jBridge not found!');
        finally
          Free;
        end;

        if str = '' then
          raise EVstHostException.Create('Error: Unable to locate proxy DLL');

        FVstDllHandle := SafeLoadLibrary(str, 7);
        if (FVstDllHandle = 0) then
          raise EVstHostException.Create('Failed to load proxy');
...
(*
       ...
       begin
         ...
         raise EVstHostEVstHostException.Create(string(StrPas(Buf)));
*)
      end;
      {$ENDIF}
    end
    ...
    begin
      if Lowercase(ExtractFileExt(DLLFileName)) = '.vst3' then
        @FMainFunction := GetProcAddress(FVstDllHandle, 'GetPluginFactory')
      else
        @FMainFunction := GetProcAddress(FVstDllHandle, 'main');
      if not Assigned(FMainFunction) then
        @FMainFunction := GetProcAddress(FVstDllHandle, 'VSTPluginMain');
      if not Assigned(FMainFunction) then
      begin
...
begin
  if not FileExists(FileName) then
    raise EVstHostException.Create(RStrBankFileDoesNotExist);
...
begin
  if not FileExists(FileName) then
    raise EVstHostException.Create(StrPresetFileDoesNotExist);
  FileStream := TFileStream.Create(FileName, fmOpenRead);
  try
...
begin
  ...
  begin
    ...
    if FXChunkBank.fxId <> FVstEffect^.UniqueID then
      raise EVstHostException.Create(RStrBankFileNotForThisPlugin);

    // allocate chunk data memory
    ChunkDataSize := Stream.Size - Stream.Position;
    GetMem(ChunkData, ChunkDataSize);
    try
      if Stream.Read(ChunkData^, ChunkDataSize) <> ChunkDataSize then
        raise EVstHostException.Create('chunk error, actual stream smaller than chunk');
  ...
  begin
    ...
    // swap
    Flip32(FXSet.fxId);
    if FXSet.fxId <> FVstEffect^.UniqueID then
      raise EVstHostException.Create(RStrBankFileNotForThisPlugin);
...
begin
  ...
  begin
    ...
    if FVstEffect^.UniqueID <> FXChunkset.fxId then
      raise EVstHostException.Create(RStrPresetFileNotForThisPlugin);
  ...
  begin
    ...
    if FVstEffect^.UniqueID <> FXPreset.fxId then
      raise EVstHostException.Create(RStrPresetFileNotForThisPlugin);
...
begin
  if FVstEffect <> nil then
    with FVstEffect^ do
      if effFlagsCanReplacing in EffectFlags then
        Process32Replacing(FVstEffect, Inputs, Outputs, SampleFrames)
      else
        Process(FVstEffect, Inputs, Outputs, SampleFrames);
end;

function TCustomVstPlugIn.GetInitialDelay: Integer;
begin
  if Assigned(FVstEffect) then
    Result := FVstEffect.initialDelay
  else
    Result := 0;
end;
...
begin
  if FEditOpen then
    raise EVstHostException.Create(RStrCloseEditorFirst)
  else
    FGUIStyle := Value;
end;
{$ENDIF}
...
initialization
  GAudioMaster := AudioMasterCallback;

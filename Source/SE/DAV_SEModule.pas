...
type
  TUgFlag = (ugfVoiceMonIgnore = 2, ugfPolyphonicAgregator = 6, ugfSuspend = 7,
    ugfOpen = 8, ugfNeverSuspend = 9, ugfClone = 11, ugfSendTimeInfoToHost = 17,
    ugfDoNeitherUseNorRemove = 31);
  TUgFlags = set of TUgFlag;

  TGuiFlag = (gfControlView = 7, gfStructureView = 8,
    gfDoNeitherUseNorRemove = 31);
  TGuiFlags = set of TGuiFlag;

  TSEIOFlag = (iofPolyphonicActive, iofIgnorePatchChange, iofRename,
    iofAutoDuplicate, iofFilename, iofSetableOutput, iofCustomisable, iofAdder,
    iofHidePin, iofLinearInput, iofUICommunication, iofAutoEnum,
    iofHideWhenLocked, iofParameterScreenOnly, iofDoNotCheckEnum, iofUIDualFlag,
    iofPatchStore, iofParamPrivate, iofMinimized, iofDisableIfPos = iofHidePin,
    iofPrivate = iofHidePin);
  TSEIOFlags = set of TSEIOFlag;

  // state type
  TSEStateType = (stStop, stOneOff, stStatic = 1, stRun);

  // Order is important for comparisons
  TUGEventType = (uetStatChange, uetSuspend, uetMIDI, uetRunFunction, uetIOFunc,
    uetUINotify, uetProgChange, uetNoteOn, uetNoteOff, uetNoteMute,
    uetPitchBend, uetAftertouch, uetStartPortamento, uetWSTableChange,
    uetDelayedGate, uetParamAutomation, uetNoteOffHeld, uetHeldNotesOff,
    uetNull, uetGeneric1, uetSetOutputPin, uetRunFunction2);
  ...
  TSEPluginModuleOpcodes = (seffOpen = 0, seffClose = 1, seffSetSampleRate,
    seffSetBlockSize, seffGetUniqueId, seffGetEffectName, seffGetPinProperties,
    seffAddEvent, seffResume, seffGetModuleProperties, seffIsEventListEmpty,
    seffGetSdkVersion, seffGuiNotify, seffQueryDebugInfo);

  // Host Opcodes

  TSEHostOpcodes = (SEAudioMasterSetPinStatus = 0, SEAudioMasterIsPinConnected,
    SEAudioMasterGetPinInputText, SEAudioMasterGetSampleClock,
    SEAudioMasterSendMIDI, SEAudioMasterGetInputPinCount,
    SEAudioMasterGetOutputPinCount, SEAudioMasterGetPinVarAddress,
    SEAudioMasterGetBlockStartClock, SEAudioMasterGetTime,
    SEAudioMasterSleepMode, SEAudioMasterGetRegisteredName,
    SEAudioMasterGetFirstClone, SEAudioMasterGetNextClone,
    SEAudioMasterGetTotalPinCount, SEAudioMasterCallVstHost,
    SEAudioMasterResolveFilename, SEAudioMasterSendStringToGui,
    SEAudioMasterGetModuleHandle, SEAudioMasterAddEvent,
    SEAudioMasterCreateSharedLookup, SEAudioMasterSetPinOutputText,
    SEAudioMasterSetProcessFunction, SEAudioMasterResolveFilename2,
    SEAudioMasterGetSeVersion, SEAudioMasterIsInteger = $7FFFFFFF);
  ...
  TSE1Dispatcher = function(Effect: PSE1ModStructBase;
    Opcode: TSEPluginModuleOpcodes; Index, Value: Integer; Ptr: Pointer;
    Opt: Single): Integer; cdecl;
  TSE2Dispatcher = function(Effect: PSE2ModStructBase;
    Opcode: TSEPluginModuleOpcodes; Index, Value: Integer; Ptr: Pointer;
    Opt: Single): Integer; cdecl;
  TSE1Process = procedure(Effect: PSE1ModStructBase;
    inputs, outputs: PDAVArrayOfSingleFixedArray; SampleFrames: Integer); cdecl;
  TSE2Process = procedure(ModuleBase: TSEModuleBase; BufferOffset: Integer;
    SampleFrames: Integer); cdecl;
  TSE2Event = procedure(ModuleBase: TSEModuleBase; Event: PSEEvent); cdecl;

  TSESetParameter = procedure(Effect: PSE1ModStructBase; Index: Integer;
    Parameter: Single);
  ...
  TSE2DispatcherEvent = function(Opcode: Integer; Index, Value: Integer;
    Ptr: Pointer; Opt: Single): Integer of object;
  TSE2ProcessEvent = procedure(const BufferOffset, SampleFrames: Integer)
    of object;
  TSEPlugStateChangeEvent = procedure(Sender: TObject; Pin: TSEPin) of object;
  TSEPinStatusUpdateEvent = procedure(Sender: TObject; AStatus: TSEStateType)
    of object;
  TSEMidiDataEvent = procedure(Sender: TObject; AClock, AMidiMsg: Cardinal;
    PinID: Integer) of object;
  TSEInputStateChangedEvent = procedure(Sender: TObject; PlugIndex: Integer;
    NewState: TSEStateType) of object;
  TSEGuiNotifyEvent = procedure(Sender: TObject; AUserMsgID, ASize: Integer;
    AData: Pointer) of object;
  TSEVoiceResetEvent = procedure(Sender: TObject; Future: Integer);

  TSE1AudioMasterCallback = function(Effect: PSE1ModStructBase;
    Opcode: TSEHostOpcodes; Index, Value: Integer; Ptr: Pointer; Opt: Single)
    : Integer; cdecl;
  TSE2AudioMasterCallback = function(Effect: PSE2ModStructBase;
    Opcode: TSEHostOpcodes; Index, Value: Integer; Ptr: Pointer; Opt: Single)
    : Integer; cdecl;
...
{$A4} // align all records to 4 byte
  ...
  public
    ...
    procedure Init(Module: TSEModuleBase; PinIndex: Integer;
      Datatype: TSEPlugDataType; VariablePtr: Pointer);
    procedure TransmitStatusChange(SampleClock: Cardinal;
    ...
    property OnStatusupdate: TSEPinStatusUpdateEvent read FOnStatusUpdate
  ...
  private
    ...
    function GetPinPropertiesClean(const Index: Integer;
  ...
  protected
    ...
    function GetPinProperties(const Index: Integer;
      Properties: PSEPinProperties): Boolean; virtual;
    ...
    procedure InputStatusChange(PlugIndex: Integer;
      NewState: TSEStateType); virtual;
    ...
    function Dispatcher(Opcode: TSEPluginModuleOpcodes; Index, Value: Integer;
      Ptr: Pointer; Opt: Single): Integer; virtual;
  public
    constructor Create(AudioMaster: TSE2AudioMasterCallback;
      Reserved: Pointer); virtual;
    destructor Destroy; override;
    ...
    property OnProgramChange: TNotifyEvent read FOnProgramChange
      write FOnProgramChange;
    property OnSampleRateChange: TNotifyEvent read FOnSampleRateChangeEvent
      write FOnSampleRateChangeEvent;
    property OnBlockSizeChange: TNotifyEvent read FOnBlockSizeChangeEvent
      write FOnBlockSizeChangeEvent;
    property OnPlugStateChange: TSEPlugStateChangeEvent
      read FOnPlugStateChangeEvent write FOnPlugStateChangeEvent;
    property OnInputStateChanged: TSEInputStateChangedEvent
      read FOnInputStatusChange write FOnInputStatusChange;
  end;
...
function SE1Dispatcher(Effect: PSE1ModStructBase;
  Opcode: TSEPluginModuleOpcodes; Index, Value: Integer; Ptr: Pointer;
  Opt: Single): Integer; cdecl;
function SE2Dispatcher(Effect: PSE2ModStructBase;
  Opcode: TSEPluginModuleOpcodes; Index, Value: Integer; Ptr: Pointer;
  Opt: Single): Integer; cdecl;
procedure SE1Process(Effect: PSE1ModStructBase;
  inputs, outputs: PDAVArrayOfSingleFixedArray; SampleFrames: Integer); cdecl;
procedure SE2Process(ModuleBase: TSEModuleBase; BufferOffset: Integer;
  SampleFrames: Integer); cdecl;
...
procedure TSEPin.Init(Module: TSEModuleBase; PinIndex: Integer;
  Datatype: TSEPlugDataType; VariablePtr: Pointer);
begin
  FModule := Module;
  FPinIndex := PinIndex;
  ...
  begin
    case FDataType of
      dtFSample:
        begin
          FAutoDuplicatePlugVar.FloatPtr :=
            PSingle(FModule.CallHost(SEAudioMasterGetPinVarAddress, FPinIndex));
    ...
    FAutoDuplicatePlugVar.FloatPtr := nil;
end;

function TSEPin.GetIsConnected: Boolean;
begin
  Result := FModule.CallHost(SEAudioMasterIsPinConnected, FPinIndex, 0,
    nil) <> 0;
end;
...
begin
  ...
  BlockPos := FModule.CallHost(SEAudioMasterGetBlockStartClock,
    FPinIndex, 0, nil);
  SampleClock := FModule.SampleClock;
  Result := PDAVSingleFixedArray(FVariablePtr^)[(SampleClock - BlockPos)];
end;

procedure TSEPin.TransmitStatusChange(SampleClock: Cardinal;
  NewState: TSEStateType);
begin
  if Assigned(FModule) then
    FModule.CallHost(SEAudioMasterSetPinStatus, FPinIndex, Integer(NewState),
      Pointer(SampleClock));
end;
...
begin
  ...
  begin
    ...
    FModule.CallHost(SEAudioMasterSendMIDI, FPinIndex, Msg,
      Pointer(SampleClock));
  end;
end;
...
constructor TSEModuleBase.Create(AudioMaster: TSE2AudioMasterCallback;
  Reserved: Pointer);
begin
  FPins := nil;
...
begin
  ...
  begin
    ...
    for i := 0 to ActualPlugCount - 1 do
      FPins[i] := TSEPin.Create;
  end;
  ...
  while (GetPinPropertiesClean(PlugDescriptionIndex, @Properties)) and
    (i < ActualPlugCount) do
  begin
    if (not(iofUICommunication in Properties.Flags)) or
      (iofUIDualFlag in Properties.Flags) then // skip GUI plugs
    begin
      FPins[i].Init(Self, i, TSEPlugDataType(Properties.Datatype),
        Properties.VariableAddress);
      Inc(i);
    end;
...
function TSEModuleBase.Dispatcher(Opcode: TSEPluginModuleOpcodes;
...
begin
  ...
  case Opcode of
    ...
    seffIsEventListEmpty:
      Assert(False);
    seffGetSdkVersion:
      Result := CSeSdkVersion;
    seffGuiNotify:
      GuiNotify(Value, Index, Ptr);
    seffQueryDebugInfo: ;
  end;
end;
...
begin
  if (Length(FPins) > Index) and (Index >= 0) then
    Result := FPins[Index]
  else
    Result := nil;
end;

function TSEModuleBase.CallHost(Opcode: TSEHostOpcodes; Index: Integer = 0;
  Value: Integer = 0; Ptr: Pointer = nil; Opt: Single = 0): Integer;
begin
  if Assigned(FSEAudioMaster) then
    Result := FSEAudioMaster(@FEffect, Opcode, Index, Value, Ptr, Opt)
  else
    Result := 0;
end;
...
begin
  if Assigned(FOnGuiNotify) then
    FOnGuiNotify(Self, AUserMsgID, ASize, AData);
end;

procedure TSEModuleBase.InputStatusChange(PlugIndex: Integer;
  NewState: TSEStateType);
begin
  if Assigned(FOnInputStatusChange) then
    FOnInputStatusChange(Self, PlugIndex, NewState);
end;

procedure TSEModuleBase.MidiData(AClock, AMidiMsg: Cardinal; PinID: Integer);
begin
  if Assigned(FOnMidiData) then
    FOnMidiData(Self, AClock, AMidiMsg, PinID);
end;

procedure TSEModuleBase.PlugStateChange(const CurrentPin: TSEPin);
begin
  if Assigned(FOnPlugStateChangeEvent) then
    FOnPlugStateChangeEvent(Self, CurrentPin);
end;
...
begin
  if Assigned(FOnBlockSizeChangeEvent) then
    FOnBlockSizeChangeEvent(Self);
end;

procedure TSEModuleBase.SampleRateChanged;
begin
  if Assigned(FOnSampleRateChangeEvent) then
    FOnSampleRateChangeEvent(Self);
end;
...
begin
  if Assigned(FOnVoiceReset) then
    FOnVoiceReset(Self, Future);
end;
...
class procedure TSEModuleBase.GetModuleProperties
...
function TSEModuleBase.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
...
function TSEModuleBase.GetPinPropertiesClean(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
...
begin
  ...
  case Event.EventType of
    uetStatChange:
      GetPin(Integer(Event.PtrParam))
        .StatusUpdate(TSEStateType(Event.IntParamA));
    uetRunFunction2: ;
    uetProgChange:
      if Assigned(FOnProgramChange) then
        FOnProgramChange(Self);
    uetMIDI:
      MidiData(Event.TimeStamp, Event.IntParamA, Event.IntParamB);
  else
    ;
  end;
...
begin
  with ModuleBase do
    if Assigned(OnProcess) then
      OnProcess(BufferOffset, SampleFrames);
end;
...
begin
  case IOFlag of
    iofPolyphonicActive:
      Result := 'Polyphonic Active';
    iofIgnorePatchChange:
      Result := 'Ignore Patch Change';
    iofRename:
      Result := 'Rename';
    iofAutoDuplicate:
      Result := 'Auto Duplicate';
    iofFilename:
      Result := 'Filename';
    iofSetableOutput:
      Result := 'Setable Output';
    iofCustomisable:
      Result := 'Customisable';
    iofAdder:
      Result := 'Adder';
    ...
    iofLinearInput:
      Result := 'Linear Input';
    iofUICommunication:
      Result := 'UI Communication';
    iofAutoEnum:
      Result := 'Auto Enum';
    iofHideWhenLocked:
      Result := 'Hide When Locked';
    iofParameterScreenOnly:
      Result := 'Parameter Screen Only';
    iofDoNotCheckEnum:
      Result := 'Do Not Check Enum';
    iofUIDualFlag:
      Result := 'UI Dual Flag';
    iofPatchStore:
      Result := 'Patch Store';
    iofParamPrivate:
      Result := 'Parameter Private';
    iofMinimized:
      Result := 'Minimized';
  else
    Result := '';
...
begin
  Result := '';
  if iofPolyphonicActive in IOFlags then
    Result := 'Polyphonic Active';
  if iofIgnorePatchChange in IOFlags then
    Result := Result + 'Ignore Patch Change,';
  if iofRename in IOFlags then
    Result := Result + 'Rename,';
  if iofAutoDuplicate in IOFlags then
    Result := Result + 'Auto Duplicate,';
  if iofFilename in IOFlags then
    Result := Result + 'Filename,';
  if iofSetableOutput in IOFlags then
    Result := Result + 'Setable Output,';
  if iofCustomisable in IOFlags then
    Result := Result + 'Customisable,';
  if iofAdder in IOFlags then
    Result := Result + 'Adder,';
  if iofHidePin in IOFlags then
    Result := Result + 'Hide Pin,';
  if iofLinearInput in IOFlags then
    Result := Result + 'Linear Input,';
  if iofUICommunication in IOFlags then
    Result := Result + 'UI Communication,';
  if iofAutoEnum in IOFlags then
    Result := Result + 'Auto Enum,';
  if iofHideWhenLocked in IOFlags then
    Result := Result + 'Hide When Locked,';
  if iofParameterScreenOnly in IOFlags then
    Result := Result + 'Parameter Screen Only,';
  if iofDoNotCheckEnum in IOFlags then
    Result := Result + 'Do Not Check Enum,';
  if iofUIDualFlag in IOFlags then
    Result := Result + 'UI Dual Flag,';
  if iofPatchStore in IOFlags then
    Result := Result + 'Patch Store,';
  if iofParamPrivate in IOFlags then
    Result := Result + 'Parameter Private,';
  if iofMinimized in IOFlags then
...
begin
  Result := '';
  if ugfVoiceMonIgnore in Flags then
    Result := Result + 'Voice Monitor Ignore, ';
  if ugfPolyphonicAgregator in Flags then
    Result := Result + 'Polyphonic Agregator, ';
  if ugfSuspend in Flags then
    Result := Result + 'Suspended, ';
  if ugfOpen in Flags then
    Result := Result + 'Open, ';
  if ugfNeverSuspend in Flags then
    Result := Result + 'Never Suspend, ';
  if ugfClone in Flags then
    Result := Result + 'Clone, ';
  if ugfSendTimeInfoToHost in Flags then
    Result := Result + 'Send TimeInfo to Host';
  if Result = '' then
    Result := '-'
end;
...
begin
  Result := '';
  if gfControlView in Flags then
    Result := Result + 'Control View, ';
  if gfStructureView in Flags then
    Result := Result + 'Structure View';
  if Result = '' then
    Result := '-'
end;

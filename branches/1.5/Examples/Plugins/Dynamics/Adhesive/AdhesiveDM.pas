...
begin
  ...
  for Preset := 1 to numPrograms - 1 do
    Programs[Preset].SetParameters(CPresets[Preset]);
...
procedure TAdhesiveDataModule.ParameterMixChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  MixChanged;
  if EditorForm is TFmAdhesive then
    TFmAdhesive(EditorForm).UpdateMix;
end;
...
procedure TAdhesiveDataModule.ParameterTimeLabel(Sender: TObject;
...
procedure TAdhesiveDataModule.ParameterTimeDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
var
  Val: Single;
begin
  Val := Parameter[Index];
  if Val < 1 then
    PreDefined := AnsiString(FloatToStrF(RoundTo(1E3 * Val, -2), ffGeneral, 3, 3))
  else if Val < 1000 then
...
begin
  MixChanged;
  if EditorForm is TFmAdhesive then
    TFmAdhesive(EditorForm).UpdateMakeUp;
end;

procedure TAdhesiveDataModule.ParameterThresholdDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := AnsiString(FloatToStrF(RoundTo(Parameter[Index], -2), ffGeneral, 3, 3));
end;
...
begin
  ChooseProcess;
  if EditorForm is TFmAdhesive then
    TFmAdhesive(EditorForm).UpdateOnOff;
end;

procedure TAdhesiveDataModule.ParameterPeakClipChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  ChooseProcess;
  if EditorForm is TFmAdhesive then
    TFmAdhesive(EditorForm).UpdatePeakClip;
end;

procedure TAdhesiveDataModule.ParameterSideChainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FFilter) then
    FFilter.Frequency := Value;
  if EditorForm is TFmAdhesive then
    TFmAdhesive(EditorForm).UpdateSideChainFilter;
end;
  ...
  case Round(Parameter[7]) of
    ...
    1:
      case Round(Parameter[8]) of
        0:
          case Round(Parameter[10]) of
            0:
              OnProcess := VSTModuleProcess;
            1:
              OnProcess := VSTModuleProcessSC;
          end;
        1:
          case Round(Parameter[10]) of
            0:
              OnProcess := VSTModuleProcessPeakClip;
            1:
              OnProcess := VSTModuleProcessSCPeakClip;
          end;
      end;
  end;
...
procedure TAdhesiveDataModule.ParameterExtSideChsinChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if EditorForm is TFmAdhesive then
    TFmAdhesive(EditorForm).UpdateExtSideChain;
end;

procedure TAdhesiveDataModule.ParameterAttackChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  ...
  if EditorForm is TFmAdhesive then
    TFmAdhesive(EditorForm).UpdateAttack;
end;

procedure TAdhesiveDataModule.ParameterReleaseChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FCompressor) then
  ...
  // update GUI if necessary
  if EditorForm is TFmAdhesive then
    TFmAdhesive(EditorForm).UpdateRelease;
end;

procedure TAdhesiveDataModule.ParameterThresholdChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FCompressor) then
  ...
  // update GUI if necessary
  if EditorForm is TFmAdhesive then
    TFmAdhesive(EditorForm).UpdateThreshold;
end;

procedure TAdhesiveDataModule.ParameterRatioChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FCompressor) then
  ...
  // update GUI if necessary
  if EditorForm is TFmAdhesive then
    TFmAdhesive(EditorForm).UpdateRatio;
end;

procedure TAdhesiveDataModule.ParameterKneeChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FCompressor) then
  ...
  // update GUI if necessary
  if EditorForm is TFmAdhesive then
    TFmAdhesive(EditorForm).UpdateKnee;
end;

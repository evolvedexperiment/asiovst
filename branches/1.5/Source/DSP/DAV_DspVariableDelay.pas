...
begin
  if InterpolatorLength < FBufferSize then
    FBufferPos := InterpolatorLength
  else if FBufferSize > 0 then
    FBufferPos := FBufferSize - 1
  else
    FBufferPos := 0;
end;
...
procedure TCustomVariableDelay32.ProcessBlock32
...
begin
  ...
  begin
    ...
    if ClearBufferOnChange then
      FillChar(FBuffer^[FBufferSize], (NewSize - FBufferSize) *
        SizeOf(Single), 0)
    else
    begin
      Move(FBuffer^[FBufferPos], FBuffer^[(NewSize - FBufferPos)],
        (FBufferSize - FBufferPos) * SizeOf(Single));
      if (NewSize - 2 * FBufferPos) > 0 then
        FillChar(FBuffer^[FBufferPos], (NewSize - 2 * FBufferPos) *
          SizeOf(Single), 0);
    end;
    FBufferSize := NewSize;
  end
  else if NewSize < FBufferSize then
  begin
    FBufferSize := NewSize;
    ReallocMem(FBuffer, NewSize * SizeOf(Single));
    if not ClearBufferOnChange and (FBufferPos < NewSize) then
      Move(FBuffer^[FBufferSize + FBufferPos - NewSize], FBuffer^[FBufferPos],
        (NewSize - FBufferPos) * SizeOf(Single));
...
begin
  ...
  begin
    ...
    Result := LinearInterpolation(FFractional, @FBuffer[FBufferPos]);
end;
...
function ModifiedHermite32(const Fractional: Single;
...
begin
  ...
  begin
    if FBufferPos + 2 = FBufferSize then
      Move(FBuffer[FBufferPos], FBuffer[0], 2 * SizeOf(Single));
    Result := ModifiedHermite32(FFractional,
      @FBuffer[FBufferPos - FBufferSize + 2]);
  end
  else
    Result := ModifiedHermite32(FFractional, @FBuffer[FBufferPos]);
end;
...
begin
  inherited;
  if Assigned(FAllpassFilter) then
    FAllpassFilter.FractionalDelay := FFractional;
end;

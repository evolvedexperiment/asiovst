...
procedure TSEConvolutionModule.SubProcessBypass(const BufferOffset,
...
procedure TSEConvolutionModule.SubProcessStatic(const BufferOffset,
  SampleFrames: Integer);
begin
...
class procedure TSEConvolutionModule.GetModuleProperties
...
function TSEConvolutionModule.GetPinProperties(const Index: Integer;
  ...
  case TSEConvolutionPins(CurrentPin.PinID) of
    ...
    pinFileName:
      begin
        if FContainedIRs.Count <= 0 then
        begin
          CallHost(seaudioMasterResolveFilename, Integer(pinFileName), 300,
            FFileName);
    ...
    pinMaxIRSize:
      begin
        ...
        Inc(FSemaphore);
        try
          if FConvolver.IRSize > FMaxIRSize then
            FConvolver.IRSize := FMaxIRSize;
        finally
          Dec(FSemaphore);
        end;
      end;
    pinDesiredLatency:
      FConvolver.FFTOrder := 6 + FDesiredLatencyIndex;
  end;
  inherited;
end;
...
procedure TSEConvolutionModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
begin
  // lock processing
  while FSemaphore > 0 do;

...
type
  ...
  protected
    ...
    procedure BlurPixelRow(CurrentRow, TempRow: PPixel32Array;
      PixelCount: Integer);
...
begin
  ...
  for I := 0 to High(DivisionLUT) do
    DivisionLUT[I] := (I div DivSum);
  ...
  begin
    ...
    begin
      if I < 0 then
        CurrentData := CurrentLine^[0]
      else if I > WidthMinus1 then
        CurrentData := CurrentLine^[WidthMinus1]
      else
    ...
    begin
      ...
      if Y = 0 then
        VMin[X] := Min(X + KernelSizePlus1, WidthMinus1);
  ...
  begin
    ...
    begin
      ...
      if I > 0 then
        Inc(ASumIn, CurrentData)
      else if I < HeightMinus1 then
        Inc(ASumOut, CurrentData);

      if I < HeightMinus1 then
    ...
    begin
      ...
      if X = 0 then
        VMin[Y] := Min(Y + KernelSizePlus1, HeightMinus1) * Width;
...
begin
  ...
  Temp := 0;
  for j := -FKernelSize to FKernelSize do
    Temp := Temp + FKernel[FKernelSize + j];
  Temp := 1 / Temp;
  for j := -FKernelSize to FKernelSize do
    FKernel[FKernelSize + j] := FKernel[FKernelSize + j] * Temp;
  ...
  Temp := 0;
  for j := -FActualKernelSize to FActualKernelSize do
    Temp := Temp + FKernel[FKernelSize + j];
  Temp := 1 / Temp;
  for j := -FActualKernelSize to FActualKernelSize do
    FKernel[FKernelSize + j] := FKernel[FKernelSize + j] * Temp;
...
begin
  ...
  for Y := 0 to ByteMap.Height - 1 do
    BlurByteRow(ByteMap.ScanLine[Y], TempRow, ByteMap.Width);
...
begin
  ...
  // allocate temporary memory
  GetAlignedMemory(TempRow, Max(PixelMap.Width, PixelMap.Height) *
    SizeOf(TPixel32));
  GetAlignedMemory(TempCol, PixelMap.Height * SizeOf(TPixel32));

  // blur each y value
  for Y := 0 to PixelMap.Height - 1 do
    BlurPixelRow(PixelMap.ScanLine[Y], TempRow, PixelMap.Width);
  ...
  begin
    // first read the column into a temporary row
    for Y := 0 to PixelMap.Height - 1 do
      TempCol[Y] := PixelMap.ScanLine[Y]^[X];
    BlurPixelRow(TempCol, TempRow, PixelMap.Height);

    // now translate the temporary row back to the columns
    for Y := 0 to PixelMap.Height - 1 do
      PixelMap.ScanLine[Y]^[X] := TempCol[Y];
  end;

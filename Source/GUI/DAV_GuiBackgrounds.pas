...
begin
  if not(AOwner is TWinControl) then
    raise Exception.Create(RCStrParentInvalid);
  ...
  if AOwner is TCustomForm then
    FColor := ColorToRGB(TCustomForm(AOwner).Color)
  else
    FColor := $C8D0D4;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ScnLn^[X].R := EnsureRange(Round(((FColor shr 16) and $FF) + FAmount *
          s[1]), 0, 255);
        ScnLn^[X].G := EnsureRange(Round(((FColor shr 8) and $FF) + FAmount *
          s[1]), 0, 255);
        ScnLn^[X].B := EnsureRange(Round((FColor and $FF) + FAmount * s[1]
          ), 0, 255);
        s[0] := s[1];
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ScnLne[X].B := EnsureRange(Round(Color.B * (1 - Amount * (s[1] - h))
          ), 0, 255);
        ScnLne[X].G := EnsureRange(Round(Color.G * (1 - Amount * (s[1] - h))
          ), 0, 255);
        ScnLne[X].R := EnsureRange(Round(Color.R * (1 - Amount * (s[1] - h))
          ), 0, 255);
      end;
    end;
  end;

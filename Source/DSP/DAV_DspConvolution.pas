...
type
  ...
  public
    ...
    procedure LoadImpulseResponse(const Data: PDAVSingleFixedArray;
      const SampleFrames: Integer); overload; virtual;
    procedure LoadImpulseResponse(const Data: TDAVSingleDynArray);
      overload; virtual;
  published
    property FFTOrder;
    property FFTSize;
  ...
  protected
    ...
    procedure FFTOrderChanged; override;
    procedure PerformConvolution(SignalIn,
      SignalOut: PDAVDoubleFixedArray); virtual;
  ...
  public
    ...
    procedure LoadImpulseResponse(const Data: PDAVDoubleFixedArray;
      const SampleFrames: Integer); overload; virtual;
    procedure LoadImpulseResponse(const Data: TDAVDoubleDynArray);
      overload; virtual;
  published
    property FFTOrder;
    property FFTSize;
  ...
  public
    ...
    procedure PerformConvolution(const SignalIn,
      SignalOut: PDAVSingleFixedArray); virtual; abstract;
  published
  ...
  public
    ...
    procedure ProcessBlock(const Input, Output: PDAVSingleFixedArray;
      const SampleFrames: Integer); overload; virtual;
    procedure ProcessBlock(const Inplace: PDAVSingleFixedArray;
      const SampleFrames: Integer); overload; virtual;
    function ProcessSample32(Input: Single): Single; virtual;
    procedure LoadImpulseResponse(const Data: PDAVSingleFixedArray;
      const SampleFrames: Integer); overload; virtual;
  ...
  public
    ...
    procedure ProcessBlock(const Left, Right: PDAVSingleFixedArray;
  ...
  public
    ...
    procedure PerformConvolution(const SignalIn,
      SignalOut: PDAVDoubleFixedArray); virtual; abstract;
  published
    property Count: Integer read GetCount;
    property Latency: Integer read FLatency;
  end;
  ...
  public
    ...
    procedure ProcessBlock(const Input, Output: PDAVDoubleFixedArray;
      const SampleFrames: Integer); overload; virtual;
    procedure ProcessBlock(const Inplace: PDAVDoubleFixedArray;
      const SampleFrames: Integer); overload; virtual;
    procedure LoadImpulseResponse(const Data: PDAVDoubleFixedArray;
      const SampleFrames: Integer); overload; virtual;
    procedure LoadImpulseResponse(const Data: TDAVDoubleDynArray);
  ...
  public
    constructor Create; override;
    destructor Destroy; override;
    procedure ProcessBlock(const Left, Right: PDAVDoubleFixedArray;
      const SampleFrames: Integer); reintroduce; virtual;
  end;
...
begin
  ...
  for i := 0 to Length(FFilterFreqs) - 1 do
    Dispose(FFilterFreqs[i]);
  FreeAndNil(FFFT);
  inherited;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Move(FImpulseResponse^[Blocks * FFFTSizeHalf], TempIR^[FFFTSizeHalf],
        sz * SizeOf(Single));
      FillChar(TempIR^[FFFTSizeHalf + sz], (FFFTSizeHalf - sz) *
        SizeOf(Single), 0);
    end
    else
      Move(FImpulseResponse^[Blocks * FFFTSizeHalf], TempIR^[FFFTSizeHalf],
...
procedure TConvolution32.LoadImpulseResponse(const Data: PDAVSingleFixedArray;
...
begin
  ...
  begin
    ...
    FFFT.PerformIFFT(PDAVComplexSingleFixedArray(@FConvolved^[0]),
      FConvolvedTime);

    // copy and combine
    BlockAdditionInplace32(@SignalOut^[Block * Half],
      @FConvolvedTime^[0], Half);
  end;
end;

procedure TConvolution32.ProcessBlock(const Input, Output: PDAVSingleFixedArray;
  const SampleFrames: Integer);
var
...
begin
  ...
  repeat
    ...
    begin
      ...
      Move(FOutputBuffer^[FFFTSizeHalf], FOutputBuffer^[0],
        (FIRSizePadded - FFFTSizeHalf) * SizeOf(Single));
      FillChar(FOutputBuffer^[(FIRSizePadded - FFFTSizeHalf)],
        FFFTSizeHalf * SizeOf(Single), 0);
...
begin
  ...
  begin
    ...
    // build temporary IR part
    Move(FImpulseResponse^[Blocks * FFFTSizeHalf], TempIR^[0],
      sz * SizeOf(Double));
...
procedure TConvolution64.LoadImpulseResponse(const Data: PDAVDoubleFixedArray;
  const SampleFrames: Integer);
begin
  if FIRSize = SampleFrames then
  begin
...
begin
  ...
  begin
    ...
    // copy and combine
    BlockAdditionInplace64(@SignalOut^[Block * Half],
      @FConvolvedTime^[Half], Half);
  end;
end;

procedure TConvolution64.ProcessBlock(const Input, Output: PDAVDoubleFixedArray;
  const SampleFrames: Integer);
var
  CurrentPosition: Integer;
begin
  ...
  repeat
    ...
    begin
      Move(Input^[CurrentPosition],
        FInputBuffer^[FFFTSizeHalf + FBlockPosition],
        (FFFTSizeHalf - FBlockPosition) * SizeOf(Double));
      Move(FOutputBuffer^[FBlockPosition], Output^[CurrentPosition],
        (FFFTSizeHalf - FBlockPosition) * SizeOf(Double));

      // discard already used output buffer part and make space for new data
      Move(FOutputBuffer^[FFFTSizeHalf], FOutputBuffer^[0],
        (FIRSizePadded - FFFTSizeHalf) * SizeOf(Double));
      FillChar(FOutputBuffer^[(FIRSizePadded - FFFTSizeHalf)],
        FFFTSizeHalf * SizeOf(Double), 0);

      PerformConvolution(FInputBuffer, FOutputBuffer);

      // discard already used input buffer part to make space for new data
      Move(FInputBuffer[FFFTSizeHalf], FInputBuffer[0],
        FFFTSizeHalf * SizeOf(Double));
...
begin
  ...
  else if Dest is TCustomLowLatencyConvolutionStage64 then
...
begin
  ...
  begin
    ...
    begin
      ...
      ComplexMultiplyBlock32(@FSignalFreq^[0], @FIRSpectrums[Block]^[0],
        Half, Dest);
...
begin
  ...
  begin
    ...
    FConvStages[c - FMinimumIRBlockOrder] :=
      TLowLatencyConvolutionStage32.Create(c, StartPos, FLatency, cnt);
  ...
  FConvStages[Length(FConvStages) - 1] := TLowLatencyConvolutionStage32.Create
...
procedure TLowLatencyConvolution32.ProcessBlock(const Inplace
  : PDAVSingleFixedArray; const SampleFrames: Integer);
begin
...
begin
  ...
  else if Dest is TCustomLowLatencyConvolutionStage32 then
    with TCustomLowLatencyConvolutionStage32(Dest) do
    begin
      inherited;
      FOutputPos := Self.FOutputPos;
...
constructor TLowLatencyConvolutionStage64.Create(const IROrder: Byte;
  const StartPos, Latency, Count: Integer);
begin
  inherited Create(IROrder, StartPos, Latency, Count);
...
procedure TLowLatencyConvolutionStage64.CalculateIRSpectrums
  (const IR: PDAVDoubleFixedArray);
var
  TempIR: PDAVDoubleFixedArray;
  Blocks: Integer;
begin
...
begin
  ...
  begin
    ...
    begin
      // complex multiply with frequency response
      ComplexMultiplyBlock64(@FSignalFreq^[0], @FIRSpectrums[Block]^[0],
        Half, Dest);
      ...
      // copy and combine
      BlockAdditionInplace64(@SignalOut^[FOutputPos + FLatency - FFFTSizeHalf +
        Block * Half], @FConvolvedTime^[0], Half);
    end;
  end;
...
begin
  ...
  begin
    cnt := 1 + (ResIRSize and (1 shl c)) shr c;
    FConvStages[c - FMinimumIRBlockOrder] :=
      TLowLatencyConvolutionStage64.Create(c, StartPos, FLatency, cnt);
    StartPos := StartPos + cnt * (1 shl c);
    ResIRSize := ResIRSize - (cnt - 1) * (1 shl c);
  end;
  ...
  // last stage
  cnt := 1 + ResIRSize div (1 shl MaxIROrd);
  FConvStages[Length(FConvStages) - 1] := TLowLatencyConvolutionStage64.Create
    (MaxIROrd, StartPos, FLatency, cnt);
...
procedure TLowLatencyConvolution64.ProcessBlock(const Inplace
  : PDAVDoubleFixedArray; const SampleFrames: Integer);
begin
  ProcessBlock(Inplace, Inplace, SampleFrames);
end;
...
begin
  ...
  repeat
    ...
    begin
      Move(Input^[CurrentPosition],
        FInputBuffer^[FInputHistorySize + FBlockPosition],
        (FLatency - FBlockPosition) * SizeOf(Double));
      Move(FOutputBuffer^[FBlockPosition], Output^[CurrentPosition],
        (FLatency - FBlockPosition) * SizeOf(Double));

      // discard already used output buffer part and make space for new data
      Move(FOutputBuffer^[FLatency], FOutputBuffer^[0],
        FOutputHistorySize * SizeOf(Double));
      FillChar(FOutputBuffer^[FOutputHistorySize],
        FLatency * SizeOf(Double), 0);
      ...
      // discard already used input buffer part to make space for new data
      Move(FInputBuffer[FLatency], FInputBuffer[0],
        FInputHistorySize * SizeOf(Double));

...
begin
  with TBaxxpanderModule(Owner) do
    if Parameter[0] <> DlDryWet.Value then
      Parameter[0] := DlDryWet.Value;
end;

procedure TFmBaxxpanderGui.DlLimitChange(Sender: TObject);
begin
  with TBaxxpanderModule(Owner) do
    if Parameter[1] <> DlLimit.Value then
      Parameter[1] := DlLimit.Value;
end;

procedure TFmBaxxpanderGui.DlMixerChange(Sender: TObject);
begin
  with TBaxxpanderModule(Owner) do
    if Parameter[2] <> DlMixer.Value then
      Parameter[2] := DlMixer.Value;
end;

procedure TFmBaxxpanderGui.DlShapeChange(Sender: TObject);
begin
  with TBaxxpanderModule(Owner) do
    if Parameter[4] <> DlShape.Value then
      Parameter[4] := DlShape.Value;
end;
...
begin
  ...
  with TBaxxpanderModule(Owner) do
    if Parameter[3] <> 0.01 * LEDSaturation.Brightness_Percent then
      Parameter[3] := 0.01 * LEDSaturation.Brightness_Percent;
end;

procedure TFmBaxxpanderGui.UpdateDryWet;
begin
  with TBaxxpanderModule(Owner) do
    if DlDryWet.Value <> Parameter[0] then
      DlDryWet.Value := Parameter[0];
end;

procedure TFmBaxxpanderGui.UpdateLimit;
begin
  with TBaxxpanderModule(Owner) do
    if DlLimit.Value <> Parameter[1] then
      DlLimit.Value := Parameter[1];
end;

procedure TFmBaxxpanderGui.UpdateMixer;
begin
  with TBaxxpanderModule(Owner) do
    if DlMixer.Value <> Parameter[2] then
      DlMixer.Value := Parameter[2];
end;

procedure TFmBaxxpanderGui.UpdateSaturation;
begin
  with TBaxxpanderModule(Owner) do
    if LEDSaturation.Brightness_Percent <> 100 * Limit(Parameter[3], 0, 1) then
      LEDSaturation.Brightness_Percent := 100 * Limit(Parameter[3], 0, 1);
end;

procedure TFmBaxxpanderGui.UpdateShape;
begin
  with TBaxxpanderModule(Owner) do
    if DlShape.Value <> Parameter[4] then
      DlShape.Value := Parameter[4];
end;

...
begin
  inherited Create;
  FComb[0] := TFreeverbCombFilter.Create
    (Round(CCombTuningL1 * SampleRate * CReci441));
  FComb[1] := TFreeverbCombFilter.Create
    (Round(CCombTuningL2 * SampleRate * CReci441));
  FComb[2] := TFreeverbCombFilter.Create
    (Round(CCombTuningL3 * SampleRate * CReci441));
  FComb[3] := TFreeverbCombFilter.Create
    (Round(CCombTuningL4 * SampleRate * CReci441));
  FComb[4] := TFreeverbCombFilter.Create
    (Round(CCombTuningL5 * SampleRate * CReci441));
  FComb[5] := TFreeverbCombFilter.Create
    (Round(CCombTuningL6 * SampleRate * CReci441));
  FComb[6] := TFreeverbCombFilter.Create
    (Round(CCombTuningL7 * SampleRate * CReci441));
  FComb[7] := TFreeverbCombFilter.Create
    (Round(CCombTuningL8 * SampleRate * CReci441));
  FAllpass[0] := TFreeverbAllpass.Create
    (Round(CAllpassTuningL1 * SampleRate * CReci441));
  FAllpass[1] := TFreeverbAllpass.Create
    (Round(CAllpassTuningL2 * SampleRate * CReci441));
  FAllpass[2] := TFreeverbAllpass.Create
    (Round(CAllpassTuningL3 * SampleRate * CReci441));
  FAllpass[3] := TFreeverbAllpass.Create
    (Round(CAllpassTuningL4 * SampleRate * CReci441));
...
var
  Index: Integer;
begin
  for Index := 0 to CAllpassCount - 1 do
    FreeAndNil(FAllpass[Index]);
  for Index := 0 to CCombFilterCount - 1 do
    FreeAndNil(FComb[Index]);
  inherited;
end;
...
begin
  ...
  for Band := 0 to CCombFilterCount - 1 do
    Result := Result + FComb[Band].ProcessSample32(Input);

  // Feed through allpasses in series
  for Band := 0 to CAllpassCount - 1 do
    Result := FAllpass[Band].ProcessSample32(Result);
...
begin
  for Index := 0 to CCombFilterCount - 1 do
    FComb[Index].Feedback := FRoomSize;
  Changed;
end;

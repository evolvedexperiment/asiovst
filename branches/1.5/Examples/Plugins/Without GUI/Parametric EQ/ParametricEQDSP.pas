...
begin
  for Channel := 0 to Length(FFilters) - 1 do
    for Band := 0 to Length(FFilters[Channel]) - 1 do
      FreeAndNil(FFilters[Channel, Band]);
end;

procedure TParametricEQDataModule.ParameterTypeDisplay(Sender: TObject;
...
function TParametricEQDataModule.GetUpSampler(Index: Integer)
...
procedure TParametricEQDataModule.ParameterOutputChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
...
procedure TParametricEQDataModule.ParameterTypeChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  ...
  try
    if Assigned(FFilters[0, Band]) then
      case Round(Value) of
        ...
        4:
          FilterClass[Band] := TBasicLowpassFilter;
        5:
        ...
        8:
          FilterClass[Band] := TBasicAllpassFilter;
        9:
          FilterClass[Band] := TBasicLowShelfAFilter;
        10:
          FilterClass[Band] := TBasicLowShelfBFilter;
        11:
          FilterClass[Band] := TBasicHighShelfAFilter;
        12:
          FilterClass[Band] := TBasicHighShelfBFilter;
      end;
  finally
    FCriticalSection.Leave;
  end;
...
procedure TParametricEQDataModule.ParameterGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  ...
  try
    ...
    if Assigned(FFilters[1, Band]) then
...
procedure TParametricEQDataModule.ParameterFrequencyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
...
procedure TParametricEQDataModule.ParameterBandwidthChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  ...
  try
    if Abs(SampleRate) > 0 then
      ...
      begin
        ...
        if 20000 / Abs(SampleRate) < 0.5 then
          Transition := 20000 / Abs(SampleRate)
        else
          Transition := 0.499;

        if Assigned(FUpSampler[Channel]) then
          FUpSampler[Channel].Transition := Transition;
        if Assigned(FDownSampler[Channel]) then
          FDownSampler[Channel].Transition := Transition;

        for Band := 0 to Length(FFilters[Channel]) - 1 do
          if Assigned(FFilters[Channel, Band]) then
            FFilters[Channel, Band].SampleRate := Abs(SampleRate);
      end;
  finally
    FCriticalSection.Leave;
  end;

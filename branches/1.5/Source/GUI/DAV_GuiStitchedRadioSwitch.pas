...
procedure TCustomGuiRadioStitchedSwitch.MouseUp(Button: TMouseButton;
  ...
  begin
    if Parent <> nil then
      with Parent do
        ...
        begin
          Sibling := Controls[I];
          if (Sibling <> Self) and (Sibling is TCustomGuiRadioStitchedSwitch)
          then
            with TCustomGuiRadioStitchedSwitch(Sibling) do
              if (GroupIndex = Self.GroupIndex) and (GlyphCount >= 1) then
                GlyphIndex := 0;
        end;
  end;
...
begin
  if not Enabled then
    ...
    begin
      if (GlyphIndex = 1) and (GlyphCount >= 4) then
        GlyphIndex := 3
      else if (GlyphCount >= 3) then
        GlyphIndex := 2;
    end
    else
  else if (GlyphIndex in [2 .. 3]) then
    GlyphIndex := GlyphIndex - 2;
end;
...
begin
  ...
  if not(Assigned(FImageList) and (ImageIndex >= 0)) then
    if Assigned(FBuffer) then
      if (FGlyphIndex = 0) then
        FBuffer.FillRect(ClientRect, pxLime32)
      else
        FBuffer.FillRect(ClientRect, pxRed32);
end;

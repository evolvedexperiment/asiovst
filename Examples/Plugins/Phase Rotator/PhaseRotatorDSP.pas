      ...
      if OpenKey(CRegKeyRoot, False) then
        try
          ...
          begin
            ...
            if MaxStages > ParameterProperties[1].Min then
              ParameterProperties[1].Max := MaxStages;
          end;
        except
        end;
    finally
...
begin
  for ChannelIndex := 0 to Length(FAllpass) - 1 do
    for BandIndex := 0 to Length(FAllpass[ChannelIndex]) - 1 do
      FreeAndNil(FAllpass[ChannelIndex, BandIndex]);
end;
...
begin
  ...
  try
    for ChannelIndex := 0 to Length(FAllpass) - 1 do
      for BandIndex := 0 to Length(FAllpass[ChannelIndex]) - 1 do
        if Assigned(FAllpass[ChannelIndex, BandIndex]) then
          FAllpass[ChannelIndex, BandIndex].ResetStates;
  finally
...
procedure TPhaseRotatorModule.ParameterBandwidthDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := AnsiString(FloatToStrF(Parameter[Index], ffGeneral, 2, 2));
end;

procedure TPhaseRotatorModule.ParameterBandwidthChange(Sender: TObject;
...
begin
  ...
  try
    for ChannelIndex := 0 to Length(FAllpass) - 1 do
      for BandIndex := 0 to Length(FAllpass[ChannelIndex]) - 1 do
        if Assigned(FAllpass[ChannelIndex, BandIndex]) then
          FAllpass[ChannelIndex, BandIndex].Bandwidth := Value;
  finally
    FCriticalSection.Leave;
  end;

  // update GUI
  if EditorForm is TFmPhaseRotator then
    TFmPhaseRotator(EditorForm).UpdateBandwidth;
end;
...
begin
  ...
  try
    for ChannelIndex := 0 to Length(FAllpass) - 1 do
      for BandIndex := 0 to Length(FAllpass[ChannelIndex]) - 1 do
        if Assigned(FAllpass[ChannelIndex, BandIndex]) then
          FAllpass[ChannelIndex, BandIndex].Frequency := Value;
  finally
    FCriticalSection.Leave;
  end;
  ...
  // update GUI
  if EditorForm is TFmPhaseRotator then
    TFmPhaseRotator(EditorForm).UpdateFrequency;
end;
...
begin
  ...
  // update GUI
  if EditorForm is TFmPhaseRotator then
    TFmPhaseRotator(EditorForm).UpdateStages;
end;
  ...
  try
    if Abs(SampleRate) > 0 then
      for ChannelIndex := 0 to Length(FAllpass) - 1 do
        for BandIndex := 0 to Length(FAllpass[ChannelIndex]) - 1 do
          if Assigned(FAllpass[ChannelIndex, BandIndex]) then
            FAllpass[ChannelIndex, BandIndex].SampleRate := Abs(SampleRate);
  finally
    FCriticalSection.Leave;
  end;
end;
  ...
  try
    for ChannelIndex := 0 to Length(FAllpass) - 1 do
      ...
      begin
        ...
        for BandIndex := 0 to FOrder - 1 do
          Data := FAllpass[ChannelIndex, BandIndex].ProcessSample64(Data);
...
begin
  ...
  begin
    ...
    case MidiEvent.MidiData[1] of // midi CC#
      70:
        Parameter[0] := FreqLinearToLog(CCData);
      71:
        Parameter[1] := 4 * CCData;
      72:
        Parameter[2] := 0.1 * Power(10, 2 * CCData);
    end;
  end;
end;

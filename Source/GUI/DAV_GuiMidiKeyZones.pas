...
type
  ...
  public
    ...
    procedure SetMouseOver(MouseOverState: Boolean = True;
  ...
  published
    ...
    property LowestZoneKey: Byte read FLowestZoneKey write SetLowestZoneKey
      default 48;
    property HighestZoneKey: Byte read FHighestZoneKey write SetHighestZoneKey
      default 59;

    property Visible: Boolean read FVisible write SetVisible default True;

    property DefaultBrushColor: TColor read FDefaultBrushColor
      write SetDefaultBrushColor default clSkyBlue;
    property DefaultBrushStyle: TBrushStyle read FDefaultBrushStyle
      write SetDefaultBrushStyle default bsSolid;
    property DefaultBorderColor: TColor read FDefaultBorderColor
      write SetDefaultBorderColor default clBlack;
    property DefaultBorderWidth: Integer read FDefaultBorderWidth
      write SetDefaultBorderWidth default 1;
    property DefaultBorderStyle: TPenStyle read FDefaultBorderStyle
      write SetDefaultBorderStyle default psSolid;

    property HoverBrushColor: TColor read FHoverBrushColor
      write SetHoverBrushColor default $FFDAB6;
    property HoverBrushStyle: TBrushStyle read FHoverBrushStyle
      write SetHoverBrushStyle default bsSolid;
    property HoverBorderColor: TColor read FHoverBorderColor
      write SetHoverBorderColor default clBlack;
    property HoverBorderWidth: Integer read FHoverBorderWidth
      write SetHoverBorderWidth default 1;
    property HoverBorderStyle: TPenStyle read FHoverBorderStyle
      write SetHoverBorderStyle default psSolid;

    property SelectedBrushColor: TColor read FSelectedBrushColor
      write SetSelectedBrushColor default $000099;
    property SelectedBrushStyle: TBrushStyle read FSelectedBrushStyle
      write SetSelectedBrushStyle default bsSolid;
    property SelectedBorderColor: TColor read FSelectedBorderColor
      write SetSelectedBorderColor default clBlack;
    property SelectedBorderWidth: Integer read FSelectedBorderWidth
      write SetSelectedBorderWidth default 1;
    property SelectedBorderStyle: TPenStyle read FSelectedBorderStyle
      write SetSelectedBorderStyle default psSolid;
  ...
  public
    ...
    property Items[Index: Integer]: TGuiKeyZoneItem read GetItem
...
begin
  ...
  FDefaultBrushColor := $F0CAA6;
  FHoverBrushColor := $FFDAB6;
  FSelectedBrushColor := $000099;
  FDefaultBrushStyle := bsSolid;
  FHoverBrushStyle := FDefaultBrushStyle;
  FSelectedBrushStyle := FDefaultBrushStyle;
  FDefaultBorderWidth := 1;
  FHoverBorderWidth := FDefaultBorderWidth;
  FSelectedBorderWidth := FDefaultBorderWidth;
  FDefaultBorderColor := clBlack;
  FHoverBorderColor := FDefaultBorderColor;
  FSelectedBorderColor := FDefaultBorderColor;
  FDefaultBorderStyle := psSolid;
  FHoverBorderStyle := FDefaultBorderStyle;
  FSelectedBorderStyle := FDefaultBorderStyle;
end;
...
begin
  ...
  begin
    ...
    if FHighestZoneKey < FLowestZoneKey then
      FHighestZoneKey := FLowestZoneKey;
    if Visible then
      (Collection as TGuiKeyZoneCollection).UpdateOwner;
  end;
end;
...
begin
  ...
  begin
    ...
    if FHighestZoneKey < FLowestZoneKey then
      FLowestZoneKey := FHighestZoneKey;
    if Visible then
      (Collection as TGuiKeyZoneCollection).UpdateOwner;
  end;
end;
    ...
    if MouseOverState then
      with (Collection as TGuiKeyZoneCollection) do
        for i := Count - 1 downto 0 do
          Items[i].SetMouseOver(False, False);
...
begin
  if FAllowUpdate then
    (Owner as TGuiMidiKeys).Invalidate;
end;

procedure TGuiKeyZoneCollection.Notify(Item: TCollectionItem;
...
procedure TGuiKeyZoneCollection.ClipZones;
  ...
  var
    mink, maxk: Byte;
  begin
    if not Items[OverlayIndex].Visible then
      Exit;
    ...
    begin
      ...
      if (HighestZoneKey < mink) and (LowestZoneKey < mink) then
        Exit;
      if (LowestZoneKey > maxk) and (HighestZoneKey > maxk) then
        Exit;
      ...
      begin
        if LowestZoneKey > maxk then
          HighestZoneKey := maxk + 1
        else
          HighestZoneKey := mink - 1;
      end
      else if (LowestZoneKey >= mink) and (LowestZoneKey <= maxk) then
      begin
        if HighestZoneKey > maxk then
          LowestZoneKey := maxk + 1
        else
          LowestZoneKey := mink - 1;
      end
      else
      begin

...
type
  ...
  public
    constructor Create(const Buffersize: Integer = 0;
      Amount: Single = 0.5); virtual;
  ...
  protected
    procedure BuffersizeChanged; virtual;
  public
    constructor Create(const Buffersize: Integer = 0; Amount: Single = 0.5;
      Excursion: Integer = 16); virtual;
    destructor Destroy; override;
  ...
  public
    ...
    property DecayDiffusion: Single read FDecayDiffusion
      write SetDecayDiffusion;
    property InputDiffusion: Single read FInputDiffusion
      write SetInputDiffusion;
...
constructor TDiffusor.Create(const Buffersize: Integer = 0;
  Amount: Single = 0.5);
begin
  inherited Create;
  FAmount := Amount;
...
begin
  if Value < 0 then
    raise Exception.Create(RCStrBuffersizePositive);
...
begin
  ...
  Inc(FBufferPos);
  if FBufferPos >= FInternalBufferSize then
    FBufferPos := 0;
...
constructor TModulatedDiffusor.Create(const Buffersize: Integer = 0;
  Amount: Single = 0.5; Excursion: Integer = 16);
begin
  inherited Create;
  FAmount := Amount;
...
begin
  ...
  begin
    ...
    if FBufferOutPos >= FInternalBufferSize then
      FBufferOutPos := FBufferOutPos - FInternalBufferSize;
    BuffersizeChanged;
  end;
end;
...
begin
  ...
  begin
    ...
    if Value = 0 then
      ...
      FLFO.Frequency := Power(10, 2 * Value - 1);
  end;
end;
...
begin
  ...
  if Pos >= FInternalBufferSize then
    Pos := Pos - FInternalBufferSize
  else if Pos < 0 then
    Pos := Pos + FInternalBufferSize;
...
begin
  if Value < 0 then
    raise Exception.Create(RCStrDampingPositive);
...
begin
  if Value < 0 then
    raise Exception.Create(RCStrDecayPositive);
...
begin
  if Value < 0 then
    raise Exception.Create(RCStrDecayDiffusionPositive);
...
begin
  if Value < 0 then
    raise Exception.Create(RCStrInputDiffusionPositive);
...
begin
  if Value < 0 then
    raise Exception.Create(RCStrPreDelayPositive);
...
begin
  ...
  if FPreDelayBufferPos = 3 then
    Move(FPreDelayBuffer[0], FPreDelayBuffer[FPreDelayBufferSize],
      3 * SizeOf(Single))
  else if FPreDelayBufferPos >= FPreDelayBufferSize then
    FPreDelayBufferPos := 0;

  while FResamplePos >= 1 do
  begin
    FResamplePos := FResamplePos - 1;
    FCurrentInput := Hermite32_asm(1 - FResamplePos,
      @FPreDelayBuffer[FPreDelayBufferPos]);

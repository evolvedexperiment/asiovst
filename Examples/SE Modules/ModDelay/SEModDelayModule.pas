...
constructor TCustomSEModDelayModule.Create(SEAudioMaster
...
procedure TCustomSEModDelayModule.SubProcessStatic(const BufferOffset,
...
class procedure TCustomSEModDelayModule.GetModuleProperties
...
function TCustomSEModDelayModule.GetPinProperties(const Index: Integer;
...
procedure TSEModDelayStaticModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
var
...
begin
  ...
  for Sample := 0 to SampleFrames - 1 do
    Outp^[Sample] := FModDelay.ProcessSample32(Inp^[Sample]);
end;

// describe the pins (plugs)
function TSEModDelayStaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
...
begin
  ...
  case TSEModDelayPins(CurrentPin.PinID) of
    pinMix:
      FModDelay.Mix := FMix;
    pinDepth:
      FModDelay.Depth := FDepth;
    pinDelay:
      FModDelay.Delay := FDelay;
    pinFeedback:
      FModDelay.Feedback := FFeedback;
    pinLowpassFrequency:
      FModDelay.LowpassFrequency := FLowpass;
    pinRate:
      FModDelay.Rate := FRate;
  end;
end;
...
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSEModDelayPins(index) in [pinDepth .. pinRate] then
    with Properties^ do
      Direction := drIn;
end;

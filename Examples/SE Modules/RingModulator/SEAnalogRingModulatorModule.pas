...
type
  ...
  TSEAnalogRingModulatorControllableModule = class
...
constructor TCustomSEAnalogRingModulatorModule.Create(SEAudioMaster
...
procedure TCustomSEAnalogRingModulatorModule.SubProcessStatic
...
class procedure TCustomSEAnalogRingModulatorModule.GetModuleProperties
...
function TCustomSEAnalogRingModulatorModule.GetPinProperties
...
procedure TCustomSEAnalogRingModulatorModule.PlugStateChange
...
procedure TSEAnalogRingModulatorStaticModule.SubProcess(const BufferOffset,
...
// describe the pins (plugs)
function TSEAnalogRingModulatorStaticModule.GetPinProperties
  (const Index: Integer; Properties: PSEPinProperties): Boolean;
begin
...
// An input plug has changed value
procedure TSEAnalogRingModulatorStaticModule.PlugStateChange
  (const CurrentPin: TSEPin);
begin
  inherited;
  case TSEAnalogRingModulatorPins(CurrentPin.PinID) of
...
function TSEAnalogRingModulatorControllableModule.GetPinProperties
  (const Index: Integer; Properties: PSEPinProperties): Boolean;
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSEAnalogRingModulatorPins(index) in [pinFrequency] then
    with Properties^ do
      Direction := drIn;
end;

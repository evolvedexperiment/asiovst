...
procedure TVSTDecimator.ParameterFilterTypeDisplay(Sender: TObject;
...
procedure TVSTDecimator.ParameterCutoffChange(Sender: TObject;
...
procedure TVSTDecimator.ParameterResonanceChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FResonance := Value * 0.1;
  if FResonance > 0.8 then
    FResonance := 0.8;
  FCutoff := FResonance * (1 + 1 / (1 - FCutoffFreqNorm));
...
procedure TVSTDecimator.ParameterWetDryMixChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FMixWet := Value * 0.01;
...
procedure TVSTDecimator.ParameterOutputVolumeChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
...
procedure TVSTDecimator.ParameterSampleRateChange(Sender: TObject;
  const Index: Integer; var Value: Single);
const
  CMul = 200 / 441;
begin
...
procedure TVSTDecimator.ParameterBitsChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FBitDepth := Value;
...
procedure TVSTDecimator.ParameterFilterTypeChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
...
begin
  ...
  begin
    ...
    case MidiEvent.midiData[1] of // midi CC#
      70:
        Parameter[0] := FreqLinearToLog(CCData) * 2.205;
      71:
        Parameter[1] := 24 * CCData;
      72:
        Parameter[2] := FreqLinearToLog(CCData);
      73:
        Parameter[3] := 10 * CCData;
      74:
        Parameter[5] := 100 * CCData;
      75:
        Parameter[6] := 6 - 30 * CCData;
    end;
  end;
end;

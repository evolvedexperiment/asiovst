...
type
  ...
  public
    ...
    procedure CalculateCoefficients; override;
    procedure ProcessSample32(Input: Single; out Lowpass, Highpass: Single);
      reintroduce; overload; virtual;
    procedure ProcessSample64(Input: Double; out Lowpass, Highpass: Double);
      reintroduce; overload; virtual;
    function ProcessSample64(Input: Double): Double; overload; override;
    function MagnitudeSquared(const Frequency: Double): Double; override;
    procedure Complex(const Frequency: Double;
      out Real, Imaginary: Double); override;
    procedure ResetStates; override;
  end;
  ...
  TButterworthHighPassFilterAutomatable = class
    (TCustomButterworthHighPassFilter)
  public
    procedure CalculateW0; override;
    function MagnitudeSquared(const Frequency: Double): Double; override;
    function MagnitudeLog10(const Frequency: Double): Double; override;
...
procedure TCustomButterworthFilter.SetFilterValues(const AFrequency,
  AGain: Single);
const
...
function TCustomButterworthFilter.MagnitudeLog10(const Frequency
  : Double): Double;
begin
...
function TCustomButterworthLowPassFilter.MagnitudeSquared(const Frequency
  : Double): Double;
var
  i: Integer;
  a, cw: Double;
begin
  ...
  for i := 0 to (FOrder div 2) - 1 do
    Result := Result * a / (1 + Sqr(FCoeffs[2 * i]) + Sqr(FCoeffs[2 * i + 1]) +
      2 * FCoeffs[2 * i + 1] + cw * ((FCoeffs[2 * i] - cw) * FCoeffs[2 * i + 1]
      - FCoeffs[2 * i]));
  if (FOrder mod 2) = 1 then
  begin
    i := ((FOrder + 1) div 2) - 1;
    Result := Result * (cw + 2) /
      (1 + Sqr(FCoeffs[2 * i]) - cw * FCoeffs[2 * i]);
  end;
...
begin
  ...
  *)
  GetSinCos(2 * Frequency * Pi * SampleRateReciprocal, Cmplx[0].Im,
    Cmplx[0].Re);
  Cmplx[1].Im := 0;
  Cmplx[1].Re := 1;
  for i := 0 to (FOrder div 2) - 1 do
  begin
    ComplexMultiplyInplace64(Cmplx[1].Re, Cmplx[1].Im,
      (Cmplx[0].Re * (1 - FCoeffs[2 * i + 1] - FCoeffs[2 * i] + Cmplx[0].Re *
      (1 - FCoeffs[2 * i + 1])) - FCoeffs[2 * i]),
      (Cmplx[0].Im * (1 + FCoeffs[2 * i + 1]) * (Cmplx[0].Re + 1)));
  end;
  Result := ArcTan2(Cmplx[1].Im, Cmplx[1].Re);
end;

procedure TCustomButterworthLowPassFilter.Complex(const Frequency: Double;
...
begin
  ...
  begin
    ...
    B.Re := 1 - FCoeffs[2 * i] * Cmplx.Re - FCoeffs[2 * i + 1] *
      (2 * Sqr(Cmplx.Re) - 1);
...
function TCustomButterworthHighPassFilter.MagnitudeSquared(const Frequency
  : Double): Double;
var
  i: Integer;
  a, cw: Double;
...
function TCustomButterworthHighPassFilter.Phase(const Frequency
  : Double): Double;
var
  cw, sw: Double;
  Nom, Den: Double;
  ...
  *)
  ...
  begin
    ComplexMultiplyInplace64(Den, Nom,
      (cw * (FCoeffs[2 * i + 1] - FCoeffs[2 * i] - 1 + cw *
      (1 - FCoeffs[2 * i + 1])) + FCoeffs[2 * i]),
...
procedure TCustomButterworthHighPassFilter.Complex(const Frequency: Double;
  out Real, Imaginary: Double);
var
  i: Cardinal;
  Cmplx: TComplex64;
...
begin
  ...
  begin
    ...
    a.Re := 1 - FCoeffs[2 * i] * Cmplx.Re - FCoeffs[2 * i + 1] *
      (2 * Sqr(Cmplx.Re) - 1);
...
function TCustomButterworthHighPassFilter.ProcessSample32
  (Input: Single): Single;
{$IFDEF PUREPASCAL}
var
  i: Integer;
...
function TCustomButterworthHighPassFilter.ProcessSample64
  (Input: Double): Double;
{$IFDEF PUREPASCAL}
var
  x: Double;
...
function TCustomButterworthSplitBandFilter.MagnitudeSquared(const Frequency
  : Double): Double;
var
  Cmplx: TComplex64;
begin
...
procedure TCustomButterworthSplitBandFilter.ProcessSample32(Input: Single;
  out Lowpass, Highpass: Single);
{$IFDEF PUREPASCAL}
var
  x: Double;
  i: Integer;
...
procedure TCustomButterworthSplitBandFilter.ProcessSample64(Input: Double;
  out Lowpass, Highpass: Double);
{$IFDEF PUREPASCAL}
var
  x: Double;
  i: Integer;
begin
  ...
  Lowpass := {$IFDEF HandleDenormals}CDenorm32 +
  {$ENDIF} FFilterGain * Input * FKs;
  for i := 0 to (FOrder div 2) - 1 do
  begin
    x := Lowpass;
    Lowpass := x + FState[2 * i];
...
function TButterworthLowPassFilterAutomatable.MagnitudeLog10(const Frequency
  : Double): Double;
const
...
function TButterworthLowPassFilterAutomatable.MagnitudeSquared(const Frequency
  : Double): Double;
var
  i: Integer;
  a, cw: Double;
...
function TButterworthHighPassFilterAutomatable.MagnitudeLog10(const Frequency
  : Double): Double;
const
  CLogScale: Double = 3.0102999566398119521373889472449;
begin
...
function TButterworthHighPassFilterAutomatable.MagnitudeSquared(const Frequency
  : Double): Double;
var
  i: Integer;
  a, cw: Double;
begin
  ...
  Result := {$IFDEF HandleDenormals}CDenorm32 +
  {$ENDIF} Abs(Sqr(FFilterGain) * Result);
end;

initialization

RegisterDspProcessors32([TButterworthHighPassFilter, TButterworthLowPassFilter,
  TButterworthLowPassFilterAutomatable, TButterworthHighPassFilterAutomatable]);

RegisterDspProcessors64([TButterworthHighPassFilter, TButterworthLowPassFilter,
  TButterworthLowPassFilterAutomatable, TButterworthHighPassFilterAutomatable]);

...
begin
  ...
  for ChannelIndex := 0 to Length(FFreqShifter) - 1 do
    FFreqShifter[ChannelIndex] := TBodeFrequencyShifter32.Create;
...
begin
  for ChannelIndex := 0 to Length(FFreqShifter) - 1 do
    FreeAndNil(FFreqShifter[ChannelIndex]);
end;

procedure TBarberpoleShifterDataModule.ParameterFrequencyChange(Sender: TObject;
...
begin
  ...
  try
    for ChannelIndex := 0 to Length(FFreqShifter) - 1 do
      if Assigned(FFreqShifter[ChannelIndex]) then
        FFreqShifter[ChannelIndex].Frequency := Value;
  finally
    FCriticalSection.Leave;
  end;

  // update GUI editor
  if EditorForm is TFmBarberpoleShifter then
    TFmBarberpoleShifter(EditorForm).UpdateFrequency;
end;

procedure TBarberpoleShifterDataModule.ParameterMixChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  ...
  // update GUI editor
  if EditorForm is TFmBarberpoleShifter then
    TFmBarberpoleShifter(EditorForm).UpdateMix;
end;

procedure TBarberpoleShifterDataModule.ParameterCoeffsChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ChannelIndex: Integer;
begin
  ...
  try
    for ChannelIndex := 0 to Length(FFreqShifter) - 1 do
      if Assigned(FFreqShifter[ChannelIndex]) then
        FFreqShifter[ChannelIndex].CoefficientCount := Round(Value);
  finally
    FCriticalSection.Leave;
  end;
end;
...
begin
  ...
  try
    for ChannelIndex := 0 to Length(FFreqShifter) - 1 do
      if Assigned(FFreqShifter[ChannelIndex]) then
        FFreqShifter[ChannelIndex].TransitionBandwidth := Value;
  finally
    FCriticalSection.Leave;
  end;
end;
...
begin
  case numInputs of
    ...
    OnProcess := VSTModuleProcessMultiChannel;
  end;
...
begin
  ...
  try
    if Abs(SampleRate) > 0 then
      for ChannelIndex := 0 to Length(FFreqShifter) - 1 do
        if Assigned(FFreqShifter[ChannelIndex]) then
          FFreqShifter[ChannelIndex].Clear;
  finally
    FCriticalSection.Leave;
  end;
end;
...
begin
  ...
  try
    if Abs(SampleRate) > 0 then
      for ChannelIndex := 0 to Length(FFreqShifter) - 1 do
        if Assigned(FFreqShifter[ChannelIndex]) then
          FFreqShifter[ChannelIndex].SampleRate := Abs(SampleRate);
  finally
    FCriticalSection.Leave;
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      Outputs[0, Sample] := FMix[1] * Inputs[0, Sample] + FMix[0] * Down +
        FMix[2] * Up;
    end;
  finally
    FCriticalSection.Leave;
...
begin
  ...
  try
    ...
    begin
      ...
      Outputs[1, Sample] := FMix[1] * Inputs[1, Sample] + FMix[0] * Down +
        FMix[2] * Up;
    end;
  finally
    FCriticalSection.Leave;
...
begin
  ...
  try
    for ChannelIndex := 0 to Length(FFreqShifter) - 1 do
      for Sample := 0 to Min(BlockSize, SampleFrames) - 1 do
      begin
        FFreqShifter[ChannelIndex].ProcessSample(Inputs[ChannelIndex, Sample],
          Up, Down);

...
type
  ...
  public
    ...
    procedure Reset; override;
  published
    property BufferPrecision: TPrecision read FBufferPrecision
      write SetBufferPrecision;
...
begin
  ...
  for i := FStages to OldStages - 1 do
    FreeAndNil(FLFOs[i]);
...
begin
  ...
  if FBufferOutPos >= FRealBufSize then
    FBufferOutPos := FBufferOutPos - FBufferSize;
end;
...
begin
  ...
  if FRealBufSize > OldBufferSize then
    FillChar(FBuffer32^[OldBufferSize], (FRealBufSize - OldBufferSize) *
      SizeOf(Single), 0);
end;
...
begin
  ...
  if FRealBufSize > OldBufferSize then
    FillChar(FBuffer64^[OldBufferSize], (FRealBufSize - OldBufferSize) *
      SizeOf(Double), 0);
end;
...
begin
  ...
  case FBufferPrecision of
    pcHalf:
      Result := (1 - FMix) * PDAVHalfFloatFixedArray(FBuffer)^[FBufferOutPos];
    pcSingle:
      Result := (1 - FMix) * PDAVSingleFixedArray(FBuffer)^[FBufferOutPos];
    pcDouble:
      Result := (1 - FMix) * PDAVDoubleFixedArray(FBuffer)^[FBufferOutPos];
  else
    Result := 0;
  end;
  ...
  begin
    ...
    case FBufferPrecision of
      pcHalf:
        raise Exception.Create('yet to implement!');
      pcSingle:
        Result := Result + m * Hermite32_asm(d,
          @PDAVSingleFixedArray(FBuffer)^[p - 4]);
      pcDouble:
        Result := Result + m * Hermite64_asm(d,
          @PDAVDoubleFixedArray(FBuffer)^[p - 4]);
    end;
  ...
  case FBufferPrecision of
    pcHalf:
      PDAVHalfFloatFixedArray(FBuffer)^[FBufferInPos] :=
        FastSingleToHalfFloat(Input);
    pcSingle:
      PDAVSingleFixedArray(FBuffer)^[FBufferInPos] := Input;
    pcDouble:
      PDAVDoubleFixedArray(FBuffer)^[FBufferInPos] := Input;
  end;
  ...
  begin
    case FBufferPrecision of
      pcHalf:
        Move(PDAVHalfFloatFixedArray(FBuffer)[FRealBufSize - 4], FBuffer^,
          4 * SizeOf(THalfFloat));
      pcSingle:
        Move(PDAVSingleFixedArray(FBuffer)[FRealBufSize - 4], FBuffer^,
          4 * SizeOf(Single));
      pcDouble:
        Move(PDAVDoubleFixedArray(FBuffer)[FRealBufSize - 4], FBuffer^,
          4 * SizeOf(Double));
    end;
...
begin
  case FBufferPrecision of
    pcHalf:
      FillChar(FBuffer^, FRealBufSize * 2, 0);
    pcSingle:
      FillChar(FBuffer^, FRealBufSize * SizeOf(Single), 0);
    pcDouble:
      FillChar(FBuffer^, FRealBufSize * SizeOf(Double), 0);
  end;
end;
  ...
  case FBufferPrecision of
    pcHalf:
      begin
        ...
        if FRealBufSize > OldBufferSize then
          FillChar(PDAVHalfFloatFixedArray(FBuffer)^[OldBufferSize],
    ...
    pcSingle:
      begin
        ...
        if FRealBufSize > OldBufferSize then
          FillChar(PDAVSingleFixedArray(FBuffer)^[OldBufferSize],
    ...
    pcDouble:
      begin
        ...
        if FRealBufSize > OldBufferSize then
          FillChar(PDAVDoubleFixedArray(FBuffer)^[OldBufferSize],

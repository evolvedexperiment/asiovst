...
begin
  for Channel := 0 to Length(FConvolution) - 1 do
    FreeAndNil(FConvolution[Channel]);
end;

procedure TConvoFxDataModule.ParameterMaximumIROrderChange(Sender: TObject;
  ...
  try
    for Channel := 0 to Length(FConvolution) - 1 do
      if Assigned(FConvolution[Channel]) then
        if Value >= FConvolution[Channel].MinimumIRBlockOrder then
          FConvolution[Channel].MaximumIRBlockOrder :=
            Round(Limit(Value, 7, 20))
        else
          Value := FConvolution[Channel].MinimumIRBlockOrder;
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TConvoFxDataModule.ParameterDampingChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FDampingFilter) then
  ...
  // update GUI
  if EditorForm is TFmConvoFx then
    TFmConvoFx(EditorForm).UpdateDamping;
end;

procedure TConvoFxDataModule.ParameterGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  ...
  // update GUI
  if EditorForm is TFmConvoFx then
    TFmConvoFx(EditorForm).UpdateGain;
end;

procedure TConvoFxDataModule.ParameterIRChange(Sender: TObject;
  const Index: Integer; var Value: Single);
const
  CResNames: array [0 .. 39] of string[3] = ('BR2', 'LR1', 'BR1', 'BT1', 'PB1',
    'WR1', 'PT1', 'PT2', 'PT3', 'PT4', 'CP1', 'GV1', 'GV2', 'GV3', 'GV4', 'GV5',
    'HL1', 'MV1', 'NT1', 'OB1', 'RM1', 'SA1', 'SA2', 'SA3', 'SO1', 'SV1', 'SV2',
    'SV3', 'SV4', 'SV5', 'SV6', 'SV7', 'CH1', 'CT1', 'CT2', 'CV1', 'OS1', 'YS1',
    'VS1', 'OC1');
var
  i, c, r: Integer;
  ResName: AnsiString;
...
begin
        ...
        try
          ...
          begin
            ...
            for i := 0 to Length(HFData) - 1 do
              FImpulseResponse[0]^[c + i] := HalfFloatToSingle(HFData[i]);
            Inc(c, Length(HFData));
          end;
          ...
          begin
            ...
            Scale := 1 / r;
            for i := 0 to r - 1 do
              FImpulseResponse[0]^[c + i] := (r - i) * Scale *
      ...
      with TResourceStream.Create(HInstance, string(ResName + 'R'), 'F16') do
        try
          ...
          begin
            Read(HFData[0], Length(HFData) * 2);
            for i := 0 to Length(HFData) - 1 do
              FImpulseResponse[1]^[c + i] := HalfFloatToSingle(HFData[i]);
            Inc(c, Length(HFData));
          end;
          ...
          begin
            ...
            Scale := 1 / r;
            for i := 0 to r - 1 do
              FImpulseResponse[1]^[c + i] := (r - i) * Scale *
                HalfFloatToSingle(HFData[i]);
          end;
        finally
          Free;
  ...
  // update GUI
  if EditorForm is TFmConvoFx then
    TFmConvoFx(EditorForm).UpdateIRSelect;
end;
...
begin
  ...
  begin
    ...
    IR[Sample] := s * IR[Sample] + (1 - s) * FDampingFilter.ProcessSample64
      (IR[Sample]);
  end;
end;
...
begin
  for Channel := 0 to Length(FConvolution) - 1 do
  begin
    if Abs(SampleRate - CDefaultSampleRate) < CDenorm32 then
    begin
      ...
      try
        ...
        begin
          Move(FImpulseResponse[Channel]^[0], TempIR^[0],
            TempSize * SizeOf(Single));
    ...
    begin
      TempSize := Round((FImpulseLength[Channel] - 4) * SampleRate /
        CDefaultSampleRate);
      if TempSize < 0 then
        Exit;
      ...
      try
        ...
        begin
          ...
          Assert(Pos - r >= 0);
          Assert(Pos - r <= 1);
          TempIR^[Sample] := Hermite32_asm(Pos - r,
            @FImpulseResponse[Channel]^[r]);
          Pos := Pos + Offset;
        end;
        RenderEnvelopes(TempIR, TempSize);
        FConvolution[Channel].LoadImpulseResponse(TempIR, TempSize);
...
procedure TConvoFxDataModule.ParameterLatencyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  ...
  try
    for Channel := 0 to Length(FConvolution) - 1 do
      if Assigned(FConvolution[Channel]) then
      begin
        if Value > FConvolution[Channel].MaximumIRBlockOrder then
          Value := FConvolution[Channel].MaximumIRBlockOrder;
...
begin
  ...
  try
    for Channel := 0 to Length(FConvolution) - 1 do
      FConvolution[Channel].ProcessBlock(@Inputs[Channel, 0],

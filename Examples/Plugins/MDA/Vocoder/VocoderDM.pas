...
type
  TVocoderDataModule = class(TVSTModule)
    ...
    procedure ParameterModInDisplay(Sender: TObject; const Index: Integer; var PreDefined: AnsiString);
    ...
    procedure ParamOutputDisplay(Sender: TObject; const Index: Integer; var PreDefined: AnsiString);
    procedure ParamEnvelopeDisplay(Sender: TObject; const Index: Integer; var PreDefined: AnsiString);
    procedure ParamMidFreqDisplay(Sender: TObject; const Index: Integer; var PreDefined: AnsiString);
    procedure ParamQualityDisplay(Sender: TObject; const Index: Integer; var PreDefined: AnsiString);
  private
    FSwap: Integer;
    FGain: Single;
    FThru: Single;
...
procedure TVocoderDataModule.ParameterModInDisplay(Sender: TObject;
...
procedure TVocoderDataModule.ParameterModInChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FSwap := Integer(Parameter[0] > 0.5);
end;

procedure TVocoderDataModule.ParameterHiBandChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FHigh := sqr(Value) * Value * Power(10, 0.5 + 2 * Parameter[1]);
end;

procedure TVocoderDataModule.ParamOutputDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := FloatToStrF(40.0 * Parameter[index] - 20.0, ffGeneral, 3, 3);
end;

procedure TVocoderDataModule.ParamEnvelopeDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  if (Parameter[index] < 0.05) then
    PreDefined := 'FREEZE'
  else
    PreDefined := FloatToStr(Power(10.0, 1 + 3 * Parameter[index]));
end;

procedure TVocoderDataModule.ParamMidFreqDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := FloatToStr(800.0 * Power(2.0, 3.0 * Parameter[index] - 2.0));
end;

procedure TVocoderDataModule.ParamQualityDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  if (FNumBands = 8) then
    PreDefined := '8 BAND'
...
begin
  ...
  begin
    ...
    begin
      ...
      if (FFreq[0][12] < FFreq[i][12]) then
        FFreq[i][12] := FFreq[0][12];
    end;

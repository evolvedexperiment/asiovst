...
type
  ...
  public
    ...
    procedure ProcessBlock32(const Input, Output: PDAVSingleFixedArray;
      SampleFrames: Integer); overload;
  end;
...
begin
  ...
  repeat
    ...
    begin
      ...
      Move(FBlock32^[FBlockPosition], Output^[CurrentPosition],
        (SampleFrames - CurrentPosition) * SizeOf(Single));
...
begin
  ...
  for Sample := 0 to FBlockSize - 1 do
    FBlock32^[Sample] := FFilter[FFilterIndex].ProcessSample64
      (FFactor * FBlock32^[Sample]);
  ...
  begin
    ...
    FAddBuffer32^[(FBlockSize div 2) + Sample] :=
      -FBlock32^[(FBlockSize div 2) + Sample];
  end;

  Move(FAddBuffer32^[0], FBlock32^[FBlockSize div 2],
    (FBlockSize div 2) * SizeOf(Single));
end;

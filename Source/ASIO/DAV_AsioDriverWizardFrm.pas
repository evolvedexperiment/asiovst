...
resourcestring
  ...
  CMissingDriverFormName =
    'You must enter a name for the ASIOModule descendant class.';
  CDriverFormNameSameAsBase =
  ...
  CMissingControlPanelFormName =
    'You must enter a name for the control panel form.';
  CMissingControlPanelUnitName =
    'You must enter a unit name for the control panel form.';
...
begin
  ...
  if SelectDirectory(CBrowsePathDialogCaption, '', sDirectory) then
...
procedure TAsioDriverWizardForm.InvalidFieldError(Control: TWinControl;
  const MessageText: string);
begin
  Control.SetFocus;
  ...
  case StepIndex of
    STEP_DESTINATION:
      begin
        ...
        begin
          if MessageDlg(CProjectPathDoesNotExist, mtConfirmation, [mbYes, mbNo],
            0) = mrYes then
          begin
    ...
    STEP_EDITOR:
      begin
        if chkUseEditor.Checked and (edtControlPanelFormName.Text = '') then
          InvalidFieldError(edtControlPanelFormName,
            CMissingControlPanelFormName)
        else if chkUseEditor.Checked and (edtControlPanelUnitName.Text = '')
        then
          InvalidFieldError(edtControlPanelUnitName,
            CMissingControlPanelUnitName)
        else
          Result := True;
      end;

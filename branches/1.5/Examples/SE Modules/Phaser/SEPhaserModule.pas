...
type
  ...
  TSEPhaserPins = (pinInput, pinOutput, pinStages, pinDepth, pinFeedback,
    pinMinimum, pinMaximum, pinRate);
...
constructor TCustomSEPhaserModule.Create(SEAudioMaster: TSE2audioMasterCallback;
  Reserved: Pointer);
begin
...
procedure TCustomSEPhaserModule.SubProcessStatic(const BufferOffset,
...
class procedure TCustomSEPhaserModule.GetModuleProperties
...
function TCustomSEPhaserModule.GetPinProperties(const Index: Integer;
...
procedure TSEPhaserStaticModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
var
...
begin
  ...
  for Sample := 0 to SampleFrames - 1 do
    Outp^[Sample] := FPhaser.ProcessSample32(Inp^[Sample]);
end;

// describe the pins (plugs)
function TSEPhaserStaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
...
begin
  ...
  case TSEPhaserPins(CurrentPin.PinID) of
    pinDepth:
      FPhaser.Depth := 0.01 * FDepth;
    pinFeedback:
      FPhaser.Feedback := 0.01 * FFeedback;
    pinMinimum:
      FPhaser.Minimum := FMinimum;
    pinMaximum:
      FPhaser.Maximum := FMaximum;
    pinRate:
      FPhaser.Rate := FRate;
  end;
end;
...
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSEPhaserPins(index) in [pinDepth .. pinRate] then
    with Properties^ do
      Direction := drIn;
end;

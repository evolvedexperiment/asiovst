...
type
  ...
  published
    property BufferSize;
    property BufferFloatType: TBufferFloatType read FBufferFloatType
      write SetBufferFloatType;
  ...
  public
    constructor Create(const FractionalBufferSize: Double = 0);
      reintroduce; virtual;
    property FractionalBufferSize: Double read GetFractional
      write SetFractional;
  end;
...
begin
  ...
  for Pos := 1 to FBufferSize - 1 do
    if Abs(FBuffer^[Pos]) > Result then
      Result := Abs(FBuffer^[Pos]);
end;
...
procedure TCustomDelayLineSamples32.ProcessBlock32
...
begin
  ...
  else if Dest is TCustomDelayLineSamples64 then
    with TCustomDelayLineSamples64(Dest) do
    begin
      inherited;
      Assert(FBufferSize = Self.FBufferSize);
      for SampleIndex := 0 to FBufferSize - 1 do
        Self.FBuffer^[SampleIndex] := FBuffer^[SampleIndex];
    end
  else
    inherited;
end;
...
procedure TCustomDelayLineSamples64.ProcessBlock64
...
begin
  ...
  case FBufferFloatType of
    ft16:
      Result := HalfFloatToSingle(PDAVHalfFloatFixedArray(FBuffer)^[Pos]);
    ft32:
      Result := PDAVSingleFixedArray(FBuffer)^[Pos];
    ft64:
      Result := PDAVDoubleFixedArray(FBuffer)^[Pos];
  else
    raise Exception.Create(RCStrUnknownFloatingPointFormat);
  end;
end;
  ...
  if Dest is TDelayLineSamples32 then
    ...
    begin
      ...
      case FBufferFloatType of
        ft16:
          for SampleIndex := 0 to FBufferSize - 1 do
            FBuffer^[SampleIndex] :=
              HalfFloatToSingle(PDAVHalfFloatFixedArray(Self.FBuffer)
              ^[SampleIndex]);
        ft32:
          Move(FBuffer^, Self.FBuffer^, Self.FBufferSize * FBytesPerSample);
        ft64:
          for SampleIndex := 0 to FBufferSize - 1 do
            FBuffer^[SampleIndex] := PDAVDoubleFixedArray(Self.FBuffer)
              ^[SampleIndex];
      end;
    end
  else if Dest is TDelayLineSamples64 then
    ...
    begin
      ...
      case FBufferFloatType of
        ...
        ft32:
          for SampleIndex := 0 to FBufferSize - 1 do
            FBuffer^[SampleIndex] := PDAVSingleFixedArray(Self.FBuffer)
              ^[SampleIndex];
        ft64:
          Move(FBuffer^, Self.FBuffer^, Self.FBufferSize * FBytesPerSample);
      end;
...
begin
  case FBufferFloatType of
    ft16:
      FBytesPerSample := SizeOf(THalfFloat);
    ft32:
      FBytesPerSample := SizeOf(Single);
    ft64:
      FBytesPerSample := SizeOf(Double);
  end;
...
begin
  case FBufferFloatType of
    ft16:
      begin
        Result := HalfFloatToSingle(PDAVHalfFloatFixedArray(FBuffer)
          ^[FBufferPos]);
        PDAVHalfFloatFixedArray(FBuffer)^[FBufferPos] :=
          SingleToHalfFloat(Input);
      end;
    ft32:
      begin
    ...
    ft16:
      begin
        ...
        for Pos := 1 to FBufferSize - 1 do
          ...
          then
            Result := Abs
              (HalfFloatToSingle(PDAVHalfFloatFixedArray(FBuffer)^[Pos]));
      end;
    ft32:
      begin
        Result := Abs(PDAVSingleFixedArray(FBuffer)^[0]);
        for Pos := 1 to FBufferSize - 1 do
          if Abs(PDAVSingleFixedArray(FBuffer)^[Pos]) > Result then
            Result := Abs(PDAVSingleFixedArray(FBuffer)^[Pos]);
      end;
    ft64:
      begin
        Result := Abs(PDAVDoubleFixedArray(FBuffer)^[0]);
        for Pos := 1 to FBufferSize - 1 do
          if Abs(PDAVDoubleFixedArray(FBuffer)^[Pos]) > Result then
            Result := Abs(PDAVDoubleFixedArray(FBuffer)^[Pos]);
      end;
  else
    raise Exception.Create(RCStrUnknownFloatingPointFormat);
  end;
  ...
  case FBufferFloatType of
    ...
    ft32:
      begin
        ...
        for Pos := 1 to FBufferSize - 1 do
          if PDAVSingleFixedArray(FBuffer)^[Pos] > Result then
            Result := PDAVSingleFixedArray(FBuffer)^[Pos];
      end;
    ft64:
      begin
        ...
        for Pos := 1 to FBufferSize - 1 do
          if PDAVDoubleFixedArray(FBuffer)^[Pos] > Result then
            Result := PDAVDoubleFixedArray(FBuffer)^[Pos];
      end;
  else
    raise Exception.Create(RCStrUnknownFloatingPointFormat);
...
constructor TCustomDelayLineFractional.Create(const FractionalBufferSize
  : Double = 0);
begin
...
constructor TDelayLineFractional32.Create(const FractionalBufferSize
  : Double = 0);
begin
  FBuffer := nil;
  inherited Create(Trunc(FractionalBufferSize));
...
begin
  ...
  if FBufferPos > RealBufferPos then
    FBufferPos := 0;
...
procedure TDelayLineFractional32.ProcessBlock32
  (const Data: PDAVSingleFixedArray; SampleCount: Integer);
var
  Sample: Integer;
begin
...
begin
  ...
  Inc(FBufferPos);
  if FBufferPos >= BufferSize - 1 then
    FBufferPos := 0;
...
constructor TDelayLineFractional64.Create(const FractionalBufferSize
  : Double = 0);
begin
  FBuffer := nil;
  inherited Create(Trunc(FractionalBufferSize));
...
procedure TDelayLineFractional64.ProcessBlock64
  (const Data: PDAVDoubleFixedArray; SampleCount: Integer);
var
  Sample: Integer;
begin

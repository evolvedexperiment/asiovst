...
begin
  ...
  begin
    if Round(Parameter[0]) <> SwEncoding.GlyphNr then
      Parameter[0] := SwEncoding.GlyphNr;
  end;
end;
...
begin
  ...
  begin
    case Channel of
      ...
      raise Exception.CreateFmt(RCStrChannelInvalid, [Channel]);
    end;
    if Dial.Value <> Parameter[2 + 4 * Channel] then
      Dial.Value := Parameter[2 + 4 * Channel];
    Labl.Caption := FloatToStrF(RoundTo(Parameter[2 + 4 * Channel], -1),
      ffGeneral, 4, 4) + ' ms';
  end;
end;
...
begin
  with TSplitHarmonizerModule(Owner) do
    SwEncoding.GlyphNr := Round(Parameter[0]);
end;
...
begin
  ...
  begin
    ...
    if Dial.Value <> Parameter[1 + 4 * Channel] then
      Dial.Value := Parameter[1 + 4 * Channel];
  ...
  begin
    case Channel of
      0:
        begin
          Dial := DialLowpassA;
          Labl := LbLowpassAValue;
        end;
      1:
        begin
          Dial := DialLowpassB;
          Labl := LbLowpassBValue;
        end;
    else
      raise Exception.CreateFmt(RCStrChannelInvalid, [Channel]);
    end;
    if Dial.Value <> Parameter[3 + 4 * Channel] then
      Dial.Value := Parameter[3 + 4 * Channel];
    if Parameter[3 + 4 * Channel] < 1000 then
      Labl.Caption := FloatToStrF(Parameter[3 + 4 * Channel], ffGeneral, 4,
        4) + ' Hz'
    else
      Labl.Caption := FloatToStrF(Parameter[3 + 4 * Channel] * 1E-3, ffGeneral,
        3, 3) + ' kHz';
  end;
end;
...
begin
  ...
  begin
    ...
    if Dial.Value <> Parameter[4 + 4 * Channel] then
      Dial.Value := Parameter[4 + 4 * Channel];
    Labl.Caption := FloatToStrF(RoundTo(50 + 0.5 * Parameter[4 + 4 * Channel],
      -1), ffGeneral, 4, 4) + ' %';
  end;
end;

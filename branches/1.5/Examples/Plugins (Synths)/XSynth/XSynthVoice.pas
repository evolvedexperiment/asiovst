...
type
  ...
  TXSynthVoice = class(TSynthVoice)
  private
    FFrequency: Single;
    FAmplitude: Single;
    FReleased: Boolean;
...
begin
  ...
  FReleased := False;
  if theModule.SampleRate = 0 then
    SampleRate := 44100
  else
    SampleRate := theModule.SampleRate;
  ...
  for i := 0 to 1 do
    ...
    begin
      case Oscilators[i].OType of
        otNone:
          FOscilators[i] := TOscilator.Create;
        otSine:
          FOscilators[i] := TSineOscilator.Create;
        otHalfSine:
          FOscilators[i] := THalfSineOscilator.Create;
        otSquare:
          FOscilators[i] := TSquareOscilator.Create;
        otNoise:
          FOscilators[i] := TNoiseOscilator.Create;
      end;
...
begin
  FOscilators[0].SampleRate := SampleRate;
  FOscilators[1].SampleRate := SampleRate;
end;

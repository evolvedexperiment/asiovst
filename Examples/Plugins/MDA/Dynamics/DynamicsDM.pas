...
type
  TDynamicsDataModule = class(TVSTModule)
    ...
    procedure ParameterLimiterDisplay(Sender: TObject; const Index: Integer; var PreDefined: AnsiString);
    procedure ParameterGateThresholdDisplay(Sender: TObject; const Index: Integer; var PreDefined: AnsiString);
    procedure ParameterGateAttackDisplay(Sender: TObject; const Index: Integer; var PreDefined: AnsiString);
    procedure ParameterGateReleaseDisplay(Sender: TObject; const Index: Integer; var PreDefined: AnsiString);
    procedure ParameterMixDisplay(Sender: TObject; const Index: Integer; var PreDefined: AnsiString);
    procedure ParameterLimiterChange(Sender: TObject; const Index: Integer; var Value: Single);
    procedure ParameterLimiterLabel(Sender: TObject; const Index: Integer; var PreDefined: AnsiString);
    procedure ParameterGateThresholdLabel(Sender: TObject; const Index: Integer; var PreDefined: AnsiString);
...
begin
  if Parameter[Index] > 0.58 then
    if Parameter[Index] < 0.62 then
      PreDefined := 'Limit'
    else
      PreDefined := FloatToStrF(-FRatio, ffGeneral, 2, 2)
  else if (Parameter[Index] < 0.2) then
    PreDefined := FloatToStrF(0.5 + 2.5 * Parameter[Index], ffGeneral, 2, 2)
  else
    PreDefined := FloatToStrF(1 / (1 - FRatio), ffGeneral, 2, 2);
end;
...
begin
  if (Value < -60) then
    FExpanderThreshold := 0 // Expander
  else
    FExpanderThreshold := dB_to_Amp(Value);
end;

procedure TDynamicsDataModule.ParameterGateThresholdDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  if FExpanderThreshold = 0 then
    PreDefined := 'OFF';
end;
...
begin
  PreDefined := IntToStr
    (Round(-301030.1 / (SampleRate * log10(1 - FGateAttack))));
end;
...
begin
  if Value >= 0 then
    FLimiterThreshold := 0 // Limiter
  else
    FLimiterThreshold := dB_to_Amp(Value);
  CheckProcessChanged;
end;
...
begin
  if (FRatio > 1) or (Parameter[5] < 0) or (Parameter[6] > -60) then
    OnProcess := VSTModuleProcess
  else
    OnProcess := VSTModuleProcessCompressor;
...
begin
  ...
  if (FRatio < 0) and (FThreshold < 0.1) then
    FRatio := FRatio * FThreshold * 15;
...
begin
  ...
  begin
    ...
    if env[0] > xth then
      env[2] := env[2] + ga - ga * env[2]
    else
      env[2] := gr + env[2] * gr; // Gate

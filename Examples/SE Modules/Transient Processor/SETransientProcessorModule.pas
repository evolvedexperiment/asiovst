...
type
  ...
  TSETransientProcessorPins = (pinInput, pinOutput, pinAttack, pinRelease,
    pinGain, pinFilter, pinAttackHold, pinReleaseHold);
...
procedure TCustomSETransientProcessorModule.SubProcessStatic(const BufferOffset,
...
class procedure TCustomSETransientProcessorModule.GetModuleProperties
...
function TCustomSETransientProcessorModule.GetPinProperties
...
procedure TCustomSETransientProcessorModule.PlugStateChange
...
procedure TSETransientProcessorStaticModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
var
...
begin
  ...
  for Sample := 0 to SampleFrames - 1 do
    Outp^[Sample] := FTransientProcessor.ProcessSample32(Inp^[Sample]);
end;

// describe the pins (plugs)
function TSETransientProcessorStaticModule.GetPinProperties
  (const Index: Integer; Properties: PSEPinProperties): Boolean;
begin
...
// An input plug has changed value
procedure TSETransientProcessorStaticModule.PlugStateChange
  (const CurrentPin: TSEPin);
begin
  inherited;
  case TSETransientProcessorPins(CurrentPin.PinID) of
    pinAttack:
      FTransientProcessor.Attack := FAttack;
    pinRelease:
      FTransientProcessor.Release := FRelease;
    pinGain:
      FTransientProcessor.Output := FOutputGain;
    pinFilter:
      FTransientProcessor.Filter := FFilter;
    pinAttackHold:
      FTransientProcessor.AttackHold := FAttackHold;
    pinReleaseHold:
      FTransientProcessor.ReleaseHold := FReleaseHold;
  end;
end;
...
function TSETransientProcessorControllableModule.GetPinProperties
  (const Index: Integer; Properties: PSEPinProperties): Boolean;
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSETransientProcessorPins(index) in [pinAttack .. pinReleaseHold] then
    with Properties^ do
      Direction := drIn;
end;

...
{$ENDIF}{$ENDIF}
  protected
    ...
    procedure PerformAnalysis(SignalIn: PDAVSingleFixedArray);
      overload; virtual;
    procedure PerformAnalysis(Spectum: PDAVComplexSingleFixedArray); overload;
      virtual; abstract;
...
{$ENDIF}{$ENDIF}
  public
    ...
    procedure ProcessBlock(const Input: PDAVSingleFixedArray;
...
{$ENDIF}
  protected
    ...
    procedure AssignTo(Dest: TPersistent); override;
    procedure FFTOrderChanged; override;
    procedure PerformAnalysis(SignalIn: PDAVDoubleFixedArray);
      overload; virtual;
    procedure PerformAnalysis(Spectum: PDAVComplexDoubleFixedArray); overload;
      virtual; abstract;
...
{$ENDIF}
  public
    constructor Create; override;
    destructor Destroy; override;
    procedure ProcessBlock(const Input: PDAVDoubleFixedArray;
      const SampleFrames: Integer); virtual;
...
procedure TCustomSpectralAnalysis32.PerformAnalysis
  (SignalIn: PDAVSingleFixedArray);
begin
  FFft.PerformFFT(FSignalFreq, SignalIn);
  PerformAnalysis(FSignalFreq);
end;

procedure TCustomSpectralAnalysis32.ProcessBlock(const Input
...
procedure TCustomSpectralAnalysis64.PerformAnalysis
  (SignalIn: PDAVDoubleFixedArray);
begin
  FFft.PerformFFT(FSignalFreq, SignalIn);
  PerformAnalysis(FSignalFreq);
end;

procedure TCustomSpectralAnalysis64.ProcessBlock(const Input

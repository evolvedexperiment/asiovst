...
begin
  ...
  for SampleIndex := 0 to SampleFrames - 1 do
    for ChannelIndex := 0 to Voices.Count - 1 do
      Outputs[0, SampleIndex] := Outputs[0, SampleIndex] + Voices[ChannelIndex].Process;

  for ChannelIndex := 1 to numOutputs - 1 do
    Move(Outputs[0, 0], Outputs[ChannelIndex, 0], SampleFrames * SizeOf(Single));
end;
...
begin
  ...
  for ChannelIndex := 1 to numOutputs - 1 do
    Move(Outputs[0, 0], Outputs[ChannelIndex, 0], SampleFrames * SizeOf(Double));
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      NoteOn(Midi2Pitch[MidiKey], Velocity * CVeloDiv);
    end;
    Voices.Add(newNote);
  end
  ...
  begin
    ...
    begin
      if (Voices.Items[Index].MidiKey = MidiEvent.MidiData[1]) then
      begin
        Voices.Delete(Index);
        Break;
      end;

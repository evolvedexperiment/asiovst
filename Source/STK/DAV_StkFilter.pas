...
begin
  ...
  // check the arguments
  if (ACoefficientCount < 1) then
    raise Exception.Create(RCStrNoACoeffs);

  if (BCoefficientCount < 1) then
    raise Exception.Create(RCStrNoBCoeffs);

  if (ACoefficients^[0] = 0) then
    raise Exception.Create(RCStrFirstACoeffZero);

...
{$ELSE}
    ...
    procedure CMParentColorChanged(var Message: TLMessage);
      message CM_PARENTCOLORCHANGED;
{$ENDIF}
...
procedure TGuiCustomControl.CMColorChanged(var Message:
...
procedure TGuiCustomControl.CMParentColorChanged(var Message:
  {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF});
begin
  inherited;
  if not FTransparent then
...
{$ENDIF}
  begin
    ...
    with FBackBuffer do
      FillRect(0, 0, Width, Height, PixelColor32);
  end;
...
procedure TGuiCustomControl.WMMove(var Message:

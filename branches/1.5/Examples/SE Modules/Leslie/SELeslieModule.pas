...
type
  ...
  TSELesliePins = (pinInput, pinOutputLeft, pinOutputRight, pinLeslieSpeed,
    pinLowThrob, pinLowWidth, pinHighThrob, pinHighDepth, pinHighWidth,
    pinCrossover, pinOutputGain, pinSpeed);
...
constructor TCustomSELeslieModule.Create(SEAudioMaster: TSE2audioMasterCallback;
  Reserved: Pointer);
begin
...
procedure TCustomSELeslieModule.SubProcessStatic(const BufferOffset,
...
class procedure TCustomSELeslieModule.GetModuleProperties
...
function TCustomSELeslieModule.GetPinProperties(const Index: Integer;
...
procedure TSELeslieStaticModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
var
...
// describe the pins (plugs)
function TSELeslieStaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
...
begin
  ...
  case TSELesliePins(CurrentPin.PinID) of
    ...
    pinLowThrob:
      FLeslie.LowThrob := FLowThrob;
    pinLowWidth:
      FLeslie.LowWidth := FLowWidth;
    pinHighThrob:
      FLeslie.HighThrob := FHighThrob;
    pinHighDepth:
      FLeslie.HighDepth := FHighDepth;
    pinHighWidth:
      FLeslie.HighWidth := FHighWidth;
    pinCrossover:
      FLeslie.Crossover := 0.01 * FCrossover;
    pinOutputGain:
      FLeslie.OutputGain := FOutputGain;
    pinSpeed:
      FLeslie.Speed := FSpeed;
  end;
end;
...
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSELesliePins(index) in [pinLeslieSpeed .. pinSpeed] then
    with Properties^ do
      Direction := drIn;
end;

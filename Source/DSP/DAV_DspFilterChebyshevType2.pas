...
type
  ...
  protected
    ...
    function CorrectFrequency(CurrentFrequency: Double): Double;
      virtual; abstract;
  ...
  public
    ...
    property FixFrequency: Boolean read FFixFrequency write SetFixFrequency
      default False;
...
function TCustomChebyshev2LowpassFilter.CorrectFrequency(CurrentFrequency
  : Double): Double;
var
  t: Double;
begin
...
begin
  ...
  begin
    ...
    B.Re := 1 - FCoeffs[3 * i + 1] * Cmplx.Re - FCoeffs[3 * i + 2] *
      (2 * sqr(Cmplx.Re) - 1);
...
function TChebyshev2LowpassFilter.MagnitudeSquared(const Frequency
  : Double): Double;
var
  i: Integer;
  cw: Double;
...
function TCustomChebyshev2HighpassFilter.CorrectFrequency(CurrentFrequency
  : Double): Double;
var
  t: Double;
begin
...
procedure TChebyshev2HighpassFilter.Complex(const Frequency: Double;
  out Real, Imaginary: Double);
var
  i: Cardinal;
  Cmplx: TComplex64;
...
function TChebyshev2HighpassFilter.MagnitudeSquared(const Frequency
  : Double): Double;
var
  i: Integer;
  cw: Double;
...
function TChebyshev2LowpassFilterAutomatable.MagnitudeSquared(const Frequency
  : Double): Double;
var
  i: Cardinal;
  cw: Double;
...
begin
  ...
  begin
    ...
    Den := Den * (1 + sqr(FCoeffs[3 * i + 1]) + sqr(FCoeffs[3 * i + 2]) + 2 *
      FCoeffs[3 * i + 2] + cw * (FCoeffs[3 * i + 1] * (FCoeffs[3 * i + 2] - 1) -
      cw * FCoeffs[3 * i + 2]));
    inc(i);
  end;
...
function TChebyshev2HighpassFilterAutomatable.MagnitudeSquared(const Frequency
  : Double): Double;
var
  i: Cardinal;
  cw: Single;
...
RegisterDspProcessors32([TChebyshev2LowpassFilter, TChebyshev2HighpassFilter,
  TChebyshev2LowpassFilterAutomatable, TChebyshev2HighpassFilterAutomatable]);
RegisterDspProcessors64([TChebyshev2LowpassFilter, TChebyshev2HighpassFilter,
  TChebyshev2LowpassFilterAutomatable, TChebyshev2HighpassFilterAutomatable]);

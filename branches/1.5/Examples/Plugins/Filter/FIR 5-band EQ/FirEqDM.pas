...
uses
  Math, DAV_Common, DAV_Convert, DAV_Math, DAV_VSTModuleWithPrograms;
...
begin
  ...
  for Band := 0 to Length(FFilterProtos) - 1 do
    FFilterProtos[Band] := TAnaloguePeakFilterPrototype.Create;;
...
procedure TFirEQDataModule.ParameterWindowFunctionsDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := GWindowFunctions[Round(Parameter[Index])].GetWindowFunctionName;
end;

procedure TFirEQDataModule.StringToWindowParameter(Sender: TObject;
...
procedure TFirEQDataModule.ParameterWindowFunctionsChange(Sender: TObject;
...
procedure TFirEQDataModule.ParamFrequencyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
...
procedure TFirEQDataModule.ParameterGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
...
procedure TFirEQDataModule.ParameterTypeDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
const
  CFilterClasses: array [0 .. 11] of string = ('Lowcut', 'Lowshelf',
    'Lowshelf A', 'Lowshelf B', 'Peak', 'Notch', 'Allpass', 'Bandpass',
    'Highshelf', 'Highshelf A', 'Highshelf B', 'Highcut');
begin
  PreDefined := CFilterClasses[Round(Parameter[Index])];
end;

procedure TFirEQDataModule.ParameterInputGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FInputGain := dB_to_Amp(Value);
end;

procedure TFirEQDataModule.ParameterFrequencyDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  if Parameter[Index] >= 1000 then
    PreDefined := FloatToStrF(1E-3 * Parameter[Index], ffGeneral, 4, 4);
end;

procedure TFirEQDataModule.ParameterFrequencyLabel(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  if Parameter[Index] >= 1000 then
    PreDefined := 'kHz';
...
procedure TFirEQDataModule.ParameterBandwidthChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
...
procedure TFirEQDataModule.ParameterTypeChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
...
begin
  ...
  for Band := 0 to Length(FFilterProtos) - 1 do
    if not Assigned(FFilterProtos[Band]) then
      Exit;

  if (not Assigned(FFilterKernel)) or (not Assigned(FFilterFreq)) or
    (not Assigned(FFft)) then
    Exit;
  ...
  try
    ...
    for Band := 1 to Length(FFilterProtos) - 1 do
      FFilterFreq[0].Re := FFilterFreq[0].Re * FFilterProtos[Band].Complex32(0).Re;

    Cmplx[2] := FFilterProtos[0].Complex64(0.5 * FSampleRate);
    for Band := 1 to Length(FFilterProtos) - 1 do
      ComplexMultiplyInplace64(Cmplx[2], FFilterProtos[Band].Complex64(0.5 * FSampleRate));
    ...
    begin
      ...
      for Band := 1 to Length(FFilterProtos) - 1 do
        ComplexMultiplyInplace64(Cmplx[2], FFilterProtos[Band].Complex64(Freq));

      // compensate nyquist phase offset
      ComplexMultiplyInplace64(Cmplx[2], Cmplx[1]);
      ComplexMultiplyInplace64(Cmplx[1], Cmplx[0]);

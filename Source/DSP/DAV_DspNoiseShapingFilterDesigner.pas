...
type
  TCoefficientUpdate = procedure(Sender: TObject;
    Coefficients: PDAVDoubleFixedArray; Best: Double) of object;
...
{$ENDIF}{$ENDIF}
  ...
  public
    ...
    property SampleFrames: Integer read FSampleFrames write SetSampleFrames
      default 20;
    property OnCoefficientUpdate: TCoefficientUpdate read FOnCoefficientUpdate
      write FOnCoefficientUpdate;
...
begin
  if Value <= 2 then
    raise Exception.Create(RCStrInvalidLoopcount);
...
procedure TCustomNoiseShapingFilterDesigner.SetSampleFrames
  (const Value: Integer);
begin
  if Value <= 2 then
    raise Exception.Create(RCStrInvalidSampleframes);
...
begin
  Frequency := Max(0.01, Frequency * 0.001);
  // convert to khz and define a minimum
...
begin
  ...
  begin
    ...
    ath[i] := Power(10, ATHformula(Freq) * 0.1) /
      FrequencyToCriticalBandwidth(Freq);
    iath[i] := 1 / ath[i];
  ...
  LogathAvg := logath[0];
  for i := 1 to FFTSizeHalf - 1 do
    LogathAvg := LogathAvg + logath[i];
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          xtry^[j] := Best^[j];
          if random(2) = 0 then
            xtry^[j] := xtry^[j] + ((random - 0.5) * (FLoopCount - i) *
              FLoopCountReciprocal);
        end;
    ...
    begin
      ...
      if Assigned(FOnCoefficientUpdate) then
        FOnCoefficientUpdate(Self, Best2, log10(BestMax2));

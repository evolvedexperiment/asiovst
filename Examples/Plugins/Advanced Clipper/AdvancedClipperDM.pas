...
//                                                                            //
//  Portions created by Christian-W. Budde are Copyright (C) 2008-2013        //
...
type
  TAdvancedClipperDataModule = class(TVSTModule)
    ...
    procedure ParaOSFactor1Change(Sender: TObject; const Index: Integer;
      var Value: Single);
    procedure ParamOSFactor2Change(Sender: TObject; const Index: Integer;
      var Value: Single);
    procedure ParamFilterOrder1Change(Sender: TObject; const Index: Integer;
      var Value: Single);
    procedure ParamFilterOrder2Change(Sender: TObject; const Index: Integer;
      var Value: Single);
    procedure ParamInputGainChange(Sender: TObject; const Index: Integer;
      var Value: Single);
    procedure ParamOutputGainChange(Sender: TObject; const Index: Integer;
      var Value: Single);
    procedure ParamBW2Change(Sender: TObject; const Index: Integer;
      var Value: Single);
    procedure ParamBW1Change(Sender: TObject; const Index: Integer;
      var Value: Single);
    procedure ParamRoundDisplay(Sender: TObject; const Index: Integer;
      var PreDefined: AnsiString);
    procedure ParamHardClipDisplay(Sender: TObject; const Index: Integer;
      var PreDefined: AnsiString);
    procedure ParamHardClipChange(Sender: TObject; const Index: Integer;
      var Value: Single);
  private
...
begin
  ...
  for StageIndex := 0 to Length(FUpDownSampling) - 1 do
    for ChannelIndex := 0 to Length(FUpDownSampling[StageIndex]) - 1 do
      FUpDownSampling[StageIndex, ChannelIndex] := TDAVUpDownsampling.Create;
...
begin
  for StageIndex := 0 to Length(FUpDownSampling) - 1 do
    for ChannelIndex := 0 to Length(FUpDownSampling[StageIndex]) - 1 do
      FreeAndNil(FUpDownSampling[StageIndex, ChannelIndex]);
end;

procedure TAdvancedClipperDataModule.ParaOSFactor1Change(Sender: TObject;
...
begin
  ...
  try
    for ChannelIndex := 0 to Length(FUpDownSampling[0]) - 1 do
      if Assigned(FUpDownSampling[0, ChannelIndex]) then
        FUpDownSampling[0, ChannelIndex].Factor := Round(Value);
  finally
  ...
  if EditorForm is TFmAdvancedClipper then
    TFmAdvancedClipper(EditorForm).UpdateOSFactor1;
end;

procedure TAdvancedClipperDataModule.ParamOSFactor2Change(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ChannelIndex: Integer;
begin
  ...
  try
    for ChannelIndex := 0 to Length(FUpDownSampling[1]) - 1 do
      if Assigned(FUpDownSampling[1, ChannelIndex]) then
        FUpDownSampling[1, ChannelIndex].Factor := Round(Value);
  finally
    FCriticalSection.Leave;
  end;
  ...
  // update GUI
  if EditorForm is TFmAdvancedClipper then
    TFmAdvancedClipper(EditorForm).UpdateOSFactor2;
end;

procedure TAdvancedClipperDataModule.ParamInputGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  ...
  // update GUI
  if EditorForm is TFmAdvancedClipper then
    TFmAdvancedClipper(EditorForm).UpdateInputGain;
end;

procedure TAdvancedClipperDataModule.ParamRoundDisplay(Sender: TObject;
...
begin
  ...
  // update GUI
  if EditorForm is TFmAdvancedClipper then
    TFmAdvancedClipper(EditorForm).UpdateHardClip;
end;
...
begin
  ...
  try
    for ChannelIndex := 0 to Length(FUpDownSampling[0]) - 1 do
      if Assigned(FUpDownSampling[0, ChannelIndex]) then
        FUpDownSampling[0, ChannelIndex].TransitionBandwidth := 0.01 * Value;
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TAdvancedClipperDataModule.ParamBW2Change(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ChannelIndex: Integer;
begin
  ...
  try
    for ChannelIndex := 0 to Length(FUpDownSampling[1]) - 1 do
      if Assigned(FUpDownSampling[1, ChannelIndex]) then
        FUpDownSampling[1, ChannelIndex].TransitionBandwidth := 0.01 * Value;
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TAdvancedClipperDataModule.ParamOutputGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  ...
  // update GUI
  if EditorForm is TFmAdvancedClipper then
    TFmAdvancedClipper(EditorForm).UpdateOutputGain;
end;

procedure TAdvancedClipperDataModule.ParamFilterOrder1Change(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ChannelIndex: Integer;
begin
  ...
  try
    for ChannelIndex := 0 to Length(FUpDownSampling[0]) - 1 do
      if Assigned(FUpDownSampling[0, ChannelIndex]) then
        FUpDownSampling[0, ChannelIndex].Order := Round(Value);
  finally
    FCriticalSection.Leave;
  end;
  ...
  // update GUI
  if EditorForm is TFmAdvancedClipper then
    TFmAdvancedClipper(EditorForm).UpdateOrder1;
end;
...
begin
  ...
  try
    for ChannelIndex := 0 to Length(FUpDownSampling[1]) - 1 do
      if Assigned(FUpDownSampling[1, ChannelIndex]) then
        FUpDownSampling[1, ChannelIndex].Order := Round(Value);
  finally
    FCriticalSection.Leave;
  end;
  ...
  // update GUI
  if EditorForm is TFmAdvancedClipper then
    TFmAdvancedClipper(EditorForm).UpdateOrder2;
end;
  ...
  try
    for SampleIndex := 0 to Min(BlockSize, SampleFrames) - 1 do
      ...
      begin
        ...
        if IsNan(Inputs[ChannelIndex, SampleIndex]) then
          InterStage := 0
        else
        ...
        if Abs(InterStage) > FPeakInput then
          FPeakInput := Abs(InterStage);

        // first stage
        FUpDownSampling[0, ChannelIndex].Upsample64(InterStage, @SubSamples);
        for SubSampleIndex := 0 to FUpDownSampling[0, ChannelIndex].Factor - 1 do
          SubSamples[SubSampleIndex] :=
            (Abs(SubSamples[SubSampleIndex] + 1) -
            Abs(SubSamples[SubSampleIndex] - 1)) * 0.5;
        InterStage := FUpDownSampling[0, ChannelIndex].Downsample64
          (@SubSamples);

        // detect stage 1 peak level
        FPeakStage1 := FPeakStage1 * FReleaseFactor;
        if Abs(InterStage) > FPeakStage1 then
          FPeakStage1 := Abs(InterStage);

        // second stage
        FUpDownSampling[1, ChannelIndex].Upsample64(InterStage, @SubSamples);
        for SubSampleIndex := 0 to FUpDownSampling[1, ChannelIndex].Factor - 1 do
          SubSamples[SubSampleIndex] :=
            (Abs(SubSamples[SubSampleIndex] + 1) -
            Abs(SubSamples[SubSampleIndex] - 1)) * 0.5;
        SubSamples[0] := FUpDownSampling[1, ChannelIndex].Downsample64
          (@SubSamples);

        // detect stage 2 peak level
        FPeakStage2 := FPeakStage2 * FReleaseFactor;
        if Abs(SubSamples[0]) > FPeakStage2 then
          FPeakStage2 := Abs(SubSamples[0]);

        // process hard clip at 0 dBFS
        if FHardClip then
          SubSamples[0] :=
            (Abs(SubSamples[0] + 1) - Abs(SubSamples[0] - 1)) * 0.5;
...
begin
  ...
  try
    for StageIndex := 0 to Length(FUpDownSampling) - 1 do
      for ChannelIndex := 0 to Length(FUpDownSampling[StageIndex]) - 1 do
        if Assigned(FUpDownSampling[StageIndex, ChannelIndex]) then
          FUpDownSampling[StageIndex, ChannelIndex].ResetStates;
  finally
    FCriticalSection.Leave;
  end;
end;
  ...
  try
    ...
    if Abs(SampleRate) > 0 then
      for StageIndex := 0 to Length(FUpDownSampling) - 1 do
        for ChannelIndex := 0 to Length(FUpDownSampling[StageIndex]) - 1 do
          if Assigned(FUpDownSampling[StageIndex, ChannelIndex]) then
            FUpDownSampling[StageIndex, ChannelIndex].SampleRate :=
              Abs(SampleRate);
  finally
    FCriticalSection.Leave;
  end;

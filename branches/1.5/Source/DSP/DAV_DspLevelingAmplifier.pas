...
type
  ...
  published
    ...
    property Attack_ms;
    property Release_ms;
    property ProgramDependency: Double read FProgramDependency
      write SetProgramDependency;
    property Ratio;
    property SampleRate;
    property Knee;
  end;
...
begin
  if FAttack_ms = 0 then
    FAttackFactor := 0
  else
    FAttackFactor := exp(-ln2 / (FAttack_ms * 0.001 * SampleRate));
end;

procedure TCustomLevelingAmplifier.CalculateReleaseFactor;
begin
  if FRelease_ms = 0 then
    FReleaseFactor := 0
  else
    FReleaseFactor := exp(-ln2 / (FRelease_ms * 0.001 * SampleRate));
end;

function TCustomLevelingAmplifier.CharacteristicCurve(const InputLevel
  : Double): Double;
begin
...
function TCustomLevelingAmplifier.TranslatePeakToGain(const PeakLevel
  : Double): Double;
var
  d: Single;
begin
...
procedure TCustomLevelingAmplifier.ProcessBlock64
...
begin
  Result := FInputLevel *
    (Harms[0] + Input * (1 + Input * (Harms[1] + Sqr(Input) *
...
RegisterDspProcessors64([TLevelingAmplifier,
  TLevelingAmplifierProgramDependentRelease]);

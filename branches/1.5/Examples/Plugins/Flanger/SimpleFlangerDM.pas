...
procedure TSimpleFlangerModule.ParamSpeedChange(Sender: TObject;
...
begin
  ...
  if EditorForm is TFmSimpleFlanger then
    with TFmSimpleFlanger(EditorForm) do
      UpdateSpeed;
end;

procedure TSimpleFlangerModule.ParamMixChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
...
procedure TSimpleFlangerModule.ParamDepthChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
    ...
    for ChannelIndex := 0 to 1 do
      ...
      begin
        if IsNaN(Inputs[ChannelIndex, SampleIndex]) then
          ...
          Input := Inputs[ChannelIndex, SampleIndex];
        Outputs[ChannelIndex, SampleIndex] :=
          FastTanhContinousError4(FMix * Input + FMixInv *
          FVibrato[ChannelIndex].ProcessSample32(Input));
      end;
  finally
    FCriticalSection.Leave;
  end;

...
type
  ...
  public
    ...
    procedure GetSamples(Left, Right: PDAVSingleFixedArray;
  ...
  begin
    ...
    begin
      ...
      if Assigned(FMpegAudio) then
        FMpegAudio.ReadBuffer(FStreamBuffer[0], FStreamBuffer[1],
          FStreamBufSize)
      else
      begin
...
begin
  ...
  if SampleFrames < SampleInBuffer then
    FBuffer.ReadBuffer(Left, Right, SampleFrames)
  else
  begin
    if SampleInBuffer > 0 then
      FBuffer.ReadBuffer(Left, Right, SampleInBuffer);
    ...
    case FInterpolation of
      biNone:
        ...
        begin
          ...
          begin
            FBufferThread.GetSamples
              (PDAVSingleFixedArray(FInterpolationBuffer[0]),
      ...
      biLinear:
        ...
        begin
          ...
          Left^[Sample] := LinearInterpolation(1 - FFractalPos,
            PDAV2SingleArray(FInterpolationBuffer[0]));
          Right^[Sample] := LinearInterpolation(1 - FFractalPos,
            PDAV2SingleArray(FInterpolationBuffer[1]));
        end;
      biHermite:
        for Sample := 0 to SampleFrames - 1 do
        begin
          ...
          while FFractalPos > 1 do
          begin
            Move(FInterpolationBuffer[0]^[0], FInterpolationBuffer[0]^[1],
              3 * SizeOf(Single));
            Move(FInterpolationBuffer[1]^[0], FInterpolationBuffer[1]^[1],
              3 * SizeOf(Single));
            FBufferThread.GetSamples
              (PDAVSingleFixedArray(FInterpolationBuffer[0]),
              PDAVSingleFixedArray(FInterpolationBuffer[1]), 1);
          ...
          Left^[Sample] := Hermite32_asm(1 - FFractalPos,
            PDAV4SingleArray(FInterpolationBuffer[0]));
          Right^[Sample] := Hermite32_asm(1 - FFractalPos,
            PDAV4SingleArray(FInterpolationBuffer[1]));
        end;
      biBSpline6Point5thOrder:
        for Sample := 0 to SampleFrames - 1 do
        begin
          ...
          Left^[Sample] := BSplineInterpolation6Point5thOrder(1 - FFractalPos,
            PDAV6SingleArray(FInterpolationBuffer[0])^);
          Right^[Sample] := BSplineInterpolation6Point5thOrder(1 - FFractalPos,
            PDAV6SingleArray(FInterpolationBuffer[1])^);
        end;
    end;
end;

...
uses
  ...
  Messages, {$ENDIF} SysUtils, Classes, Controls, Graphics,
...
{$ENDIF}
    ...
    property Direction: TCorrelationMeterDirection read FDirection
      write SetDirection default cmdHorizontal;
  end;
    ...
    begin
      ...
      begin
        ...
        FillRect(rect(FCenter.X, FStart.Top + 22,
          Round(FCenter.X + FCorrelation * (Width - FCenter.X)),
          FStart.Bottom - 24));
      end
      else if FCorrelation < 0 then
      begin
        Brush.Color := clRed;
        FillRect(rect(Round(FCenter.X + FCorrelation * FCenter.X),
          FStart.Top + 22, FCenter.X, FStart.Bottom - 24));
      end;
    end
  else
    with Canvas do
...
procedure TGuiCorrelationMeter.SetDirection(const Value
  : TCorrelationMeterDirection);
begin

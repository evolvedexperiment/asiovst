...
{$IFDEF DELPHI10_UP} {$REGION 'Basic types'} {$ENDIF}
  ...
  TAsioMessage = (amBufferSwitch, // new buffer index in lParam
    amBufferSwitchTimeInfo, // new buffer index in lParam
    amResetRequest, amBufferSizeChange, amLatencyChanged, amResyncRequest);
...
{$ENDIF}
    ...
    function MessageHandler(Selector, Value: LongInt; MessagePointer: Pointer;
      Optional: PDouble): LongInt; virtual;
  ...
  public
    ...
    property InputChannel[Index: Integer]: TAsioChannelInput
      read GetInputChannel;
    property InputChannelCount: Integer read GetInputChannelCount;
    property InputLatency: Integer read FInputLatency;
    property OutputChannel[Index: Integer]: TAsioChannelOutput
      read GetOutputChannel;
    ...
    property Supports: TAsioSupports read FAsioSupports write FAsioSupports
      default [asTimeInfo, asTimeCode];

    // events
    property OnBufferSwitch: TBufferSwitchEvent read FOnBufferSwitch
...
procedure DefaultBufferSwitch(DoubleBufferIndex: Integer;
  DirectProcess: TAsioBool); cdecl;
begin
{$IFDEF AllowMultipleAsioHosts}
  if Assigned(GAsioDriver[0]) then
    GAsioDriver[0].InternalBufferSwitch(DoubleBufferIndex, DirectProcess);
{$ELSE}
  if Assigned(GAsioDriver) then
    GAsioDriver.InternalBufferSwitch(DoubleBufferIndex, DirectProcess);
{$ENDIF}
end;
...
{$IFDEF AllowMultipleAsioHosts}
  if Assigned(GAsioDriver[0]) then
    Result := GAsioDriver[0].InternalBufferSwitchTimeInfo(Params,
      DoubleBufferIndex, DirectProcess)
  else
    Result := nil;
{$ELSE}
  if Assigned(GAsioDriver) then
    Result := GAsioDriver.InternalBufferSwitchTimeInfo(Params,
      DoubleBufferIndex, DirectProcess)
  else
    Result := nil;
{$ENDIF}
...
{$IFDEF AllowMultipleAsioHosts}
  if Assigned(GAsioDriver[0]) then
    GAsioDriver[0].InternalSampleRateDidChange(SampleRate);
{$ELSE}
  if Assigned(GAsioDriver) then
    GAsioDriver.InternalSampleRateDidChange(SampleRate);
{$ENDIF}
end;

function DefaultMessageHandler(Selector, Value: Integer;
  MessagePointer: Pointer; Opt: PDouble): Integer; cdecl;
begin
{$IFDEF AllowMultipleAsioHosts}
  if Assigned(GAsioDriver[0]) then
    Result := GAsioDriver[0].MessageHandler(Selector, Value,
      MessagePointer, Opt)
  else
    Result := 0;
{$ELSE}
  if Assigned(GAsioDriver) then
    Result := GAsioDriver.MessageHandler(Selector, Value, MessagePointer, Opt)
  else
    Result := 0;
{$ENDIF}
end;
...
procedure BufferSwitch1(DoubleBufferIndex: Integer;
  DirectProcess: TAsioBool); cdecl;
begin
  if Assigned(GAsioDriver[1]) then
    GAsioDriver[1].InternalBufferSwitch(DoubleBufferIndex, DirectProcess);
end;
...
begin
  if Assigned(GAsioDriver[1]) then
    Result := GAsioDriver[1].InternalBufferSwitchTimeInfo(Params,
      DoubleBufferIndex, DirectProcess)
  else
    Result := nil;
end;
...
begin
  if Assigned(GAsioDriver[1]) then
    GAsioDriver[1].InternalSampleRateDidChange(SampleRate);
end;
...
begin
  if Assigned(GAsioDriver[1]) then
    Result := GAsioDriver[1].MessageHandler(Selector, Value,
      MessagePointer, Opt)
  else
    Result := 0;
end;

procedure BufferSwitch2(DoubleBufferIndex: Integer;
  DirectProcess: TAsioBool); cdecl;
begin
  if Assigned(GAsioDriver[2]) then
    GAsioDriver[2].InternalBufferSwitch(DoubleBufferIndex, DirectProcess);
end;
...
begin
  if Assigned(GAsioDriver[2]) then
    Result := GAsioDriver[2].InternalBufferSwitchTimeInfo(Params,
      DoubleBufferIndex, DirectProcess)
  else
    Result := nil;
end;
...
begin
  if Assigned(GAsioDriver[2]) then
    GAsioDriver[2].InternalSampleRateDidChange(SampleRate);
end;
...
begin
  if Assigned(GAsioDriver[2]) then
    Result := GAsioDriver[2].MessageHandler(Selector, Value,
      MessagePointer, Opt)
  else
    Result := 0;
end;

procedure BufferSwitch3(DoubleBufferIndex: Integer;
  DirectProcess: TAsioBool); cdecl;
begin
  if Assigned(GAsioDriver[3]) then
    GAsioDriver[3].InternalBufferSwitch(DoubleBufferIndex, DirectProcess);
end;
...
begin
  if Assigned(GAsioDriver[3]) then
    Result := GAsioDriver[3].InternalBufferSwitchTimeInfo(Params,
      DoubleBufferIndex, DirectProcess)
  else
    Result := nil;
end;
...
begin
  if Assigned(GAsioDriver[3]) then
    GAsioDriver[3].InternalSampleRateDidChange(SampleRate);
end;
...
begin
  if Assigned(GAsioDriver[3]) then
    Result := GAsioDriver[3].MessageHandler(Selector, Value,
      MessagePointer, Opt)
  else
    Result := 0;
end;
...
begin
  ...
  if (FBufferTime.timeInfo.Flags and kSystemTimeValid) <> 0 then
    Include(Result, atSystemTimeValid)
  else
    Exclude(Result, atSystemTimeValid);
  if (FBufferTime.timeInfo.Flags and kSamplePositionValid) <> 0 then
    Include(Result, atSamplePositionValid)
  else
    Exclude(Result, atSamplePositionValid);
  if (FBufferTime.timeInfo.Flags and kSampleRateValid) <> 0 then
    Include(Result, atSampleRateValid)
  else
    Exclude(Result, atSampleRateValid);
  if (FBufferTime.timeInfo.Flags and kSpeedValid) <> 0 then
    Include(Result, atSpeedValid)
  else
    Exclude(Result, atSpeedValid);
  if (FBufferTime.timeInfo.Flags and kSampleRateChanged) <> 0 then
    Include(Result, atSampleRateChanged)
  else
    Exclude(Result, atSampleRateChanged);
  if (FBufferTime.timeInfo.Flags and kClockSourceChanged) <> 0 then
    Include(Result, atClockSourceChanged)
  else
    Exclude(Result, atClockSourceChanged);
end;
...
begin
  ...
  else if SampleType in [CAsioSTDSDInt8LSB1, CAsioSTDSDInt8LSB1,
    CAsioSTDSDInt8NER8] then
    Result := SizeOf(Byte)
  else
...
begin
  ...
  begin
    ...
    if FDriver = nil then
      raise EAsioHost.Create(RStrAsioDriverFailed);
...
{$IFDEF AllowMultipleAsioHosts}
  ...
  else if GAsioDriver[1] = nil then
  begin
    FAsioDriverIndex := 1;
    GAsioDriver[FAsioDriverIndex] := Self;
  ...
  else if GAsioDriver[2] = nil then
  begin
    FAsioDriverIndex := 2;
    GAsioDriver[FAsioDriverIndex] := Self;
  ...
  else if GAsioDriver[3] = nil then
  begin
    FAsioDriverIndex := 3;
    GAsioDriver[FAsioDriverIndex] := Self;
    ...
    raise EAsioHost.Create(RCStrOnlyOneAsioHost);
{$ELSE}
...
begin
  case FDriver.Init(FHandle) of
    ...
    ASE_NotPresent:
      raise EAsioHost.Create(RCStrDriverNotPresent);
    ASE_HWMalfunction:
      raise EAsioHost.Create(RCStrHardwareMalfunction);
    ASE_InvalidParameter:
      raise EAsioHost.Create(RCStrInputParameterInvalid);
    ASE_InvalidMode:
      raise EAsioHost.Create(RCStrInvalidMode);
    ASE_SPNotAdvancing:
      raise EAsioHost.Create(RCStrSPNotAdvancing);
    ASE_NoClock:
      raise EAsioHost.Create(RCStrNoClock);
    ASE_NoMemory:
      raise EAsioHost.Create(RCStrNoMemory);
  end;
...
begin
  ...
  for Channel := ChannelCount[0] to Length(FInputChannels) - 1 do
    if Assigned(FInputChannels) then
      FreeAndNil(FInputChannels);
  ...
  for Channel := 0 to ChannelCount[0] - 1 do
    if not Assigned(FInputChannels[Channel]) then
      FInputChannels[Channel] := TAsioChannelInput.Create(FDriver, Channel);
  ...
  for Channel := ChannelCount[1] to Length(FOutputChannels) - 1 do
    if Assigned(FOutputChannels) then
      FreeAndNil(FOutputChannels);
  ...
  for Channel := 0 to ChannelCount[1] - 1 do
    if not Assigned(FOutputChannels[Channel]) then
      FOutputChannels[Channel] := TAsioChannelOutput.Create(FDriver, Channel);
  ...
  if not FDriver.CreateBuffers(FInputBuffers, (ChannelCount[0] + ChannelCount[1]
    ), FBufferSize, FCallbacks) = ASE_OK then
    raise EAsioHost.Create(RCStrBufferAllocationError);
...
begin
  ...
  begin
    ...
    for Channel := 0 to Length(FInputChannels) - 1 do
      FreeAndNil(FInputChannels[Channel]);
    for Channel := 0 to Length(FOutputChannels) - 1 do
      FreeAndNil(FOutputChannels[Channel]);
...
begin
  if (Index >= 0) and (Index < Length(FInputChannels)) then
    Result := FInputChannels[Index]
  else
    raise EAsioHost.CreateFmt(RCStrIndexOutOfBounds, [Index]);
end;
...
begin
  if (Index >= 0) and (Index < Length(FOutputChannels)) then
    Result := FOutputChannels[Index]
  else
    raise EAsioHost.CreateFmt(RCStrIndexOutOfBounds, [Index]);
end;
...
begin
  ...
  if FPrefered <= 0 then
    raise EAsioHost.Create(RCStrPreferedBufferSize);
...
begin
  ...
  if FDriver.Future(kAsioSetIoFormat, nil) = ASE_SUCCESS then
    FAsioCanDos := FAsioCanDos + [acdSetIoFormat]
  else
    FAsioCanDos := FAsioCanDos - [acdSetIoFormat];

  // test can do 'get I/O format'
  if FDriver.Future(kAsioSetIoFormat, nil) = ASE_SUCCESS then
    FAsioCanDos := FAsioCanDos + [acdGetIoFormat]
  else
    FAsioCanDos := FAsioCanDos - [acdGetIoFormat];

  // test can do 'can do I/O format'
  if FDriver.Future(kAsioSetIoFormat, nil) = ASE_SUCCESS then
    FAsioCanDos := FAsioCanDos + [acdCanDoIoFormat]
  else
    FAsioCanDos := FAsioCanDos - [acdCanDoIoFormat];
end;
...
{$ELSE}
function DefWindowProc(hWnd: THandle; Msg: UINT; wParam: wParam; lParam: lParam)
  : LResult; external 'user32' name 'DefWindowProcA';
...
begin
  case DirectProcess of
    CAsioFalse:
      PostMessage(FHandle, PM_BufferSwitch, Integer(amBufferSwitch),
        DoubleBufferIndex);
    CAsioTrue:
...
begin
  case DirectProcess of
    ...
    CAsioTrue:
      BufferSwitchTimeInfo(DoubleBufferIndex, Params);
  end;
...
begin
  ...
  if Assigned(FOnBufferSwitch) then
    FOnBufferSwitch(Self, @(FInputBuffers^), Index);
...
begin
  ...
  case Selector of
    kAsioSelectorSupported:
      Result := CAsioTrue; // return 1 if a selector is supported
    kAsioEngineVersion:
      Result := 2; // return 2 if Asio 2 is supported
    kAsioResetRequest:
      PostAsioMessage(amResetRequest);
    kAsioBufferSizeChange:
      PostAsioMessage(amBufferSizeChange);
    kAsioResyncRequest:
      PostAsioMessage(amResyncRequest);
    kAsioLatenciesChanged:
      PostAsioMessage(amLatencyChanged);
    kAsioSupportsTimeInfo:
      Result := Integer(asTimeInfo in Supports);
    kAsioSupportsTimeCode:
      Result := Integer(asTimeCode in Supports);
    kAsioSupportsInputMonitor:
      Result := Integer(asInputMonitor in Supports);
  end;
end;
...
begin
  ...
  Value := Abs(Value);
  if (Value = 0) then
    raise EAsioHost.Create(RCStrWrongSamplerate);

...
type
  ...
  public
    ...
    function Complex64(Frequency: Double): TComplex64; overload;
      virtual; abstract;
  ...
  TCustomVariableOrderAnalogueFilterPrototype = class
  ...
  TCustomIntegerOrderAnalogueFilterPrototype = class
    (TCustomAnalogueFilterPrototype)
  protected
  ...
  TCustomAnalogueArbitraryIntegerOrderFilterPrototype = class
    (TCustomIntegerOrderAnalogueFilterPrototype)
  private
    FOrder: Integer;
  protected
  ...
  TCustomAnalogueButterworthFilterPrototype = class
    (TCustomAnalogueFilterPrototype)
  private
    FFrequency: Double;
    FOrder: Double;
  ...
  TCustomBiquadAnalogueFilterPrototype = class
    (TCustomIntegerOrderAnalogueFilterPrototype)
  private
  ...
  TCustomBiquadAnalogueFilterPrototypeClass = class of
  ...
  TAnalogueHighshelfFilterPrototype = class
    (TCustomBiquadAnalogueFilterPrototype)
  protected
    procedure CalculateCoefficients; override;
  end;

  TAnalogueLowshelfAFilterPrototype = class
    (TCustomBiquadAnalogueFilterPrototype)
  protected
    procedure CalculateCoefficients; override;
  end;

  TAnalogueHighshelfAFilterPrototype = class
    (TCustomBiquadAnalogueFilterPrototype)
  protected
    procedure CalculateCoefficients; override;
  end;

  TAnalogueLowshelfBFilterPrototype = class
    (TCustomBiquadAnalogueFilterPrototype)
  protected
    procedure CalculateCoefficients; override;
  end;

  TAnalogueHighshelfBFilterPrototype = class
    (TCustomBiquadAnalogueFilterPrototype)
  protected
    procedure CalculateCoefficients; override;
  end;

  TCustomAnalogueShapeFilterPrototype = class
    (TCustomBiquadAnalogueFilterPrototype)
  private
    FShape: Double;
  ...
  TAnalogueSimpleShapeFilterPrototype = class
  ...
  TCustomAnalogueWeightingFilterPrototype = class
    (TCustomIntegerOrderAnalogueFilterPrototype)
  protected
    procedure SetOrder(Value: Integer); override;
  end;

  TCustomAnalogueWeightingFilterPrototypeClass = class of
    TCustomAnalogueWeightingFilterPrototype;

  TAnalogueAWeightingFilterPrototype = class
    (TCustomAnalogueWeightingFilterPrototype)
  private
  ...
  TAnalogueBWeightingFilterPrototype = class
    (TCustomAnalogueWeightingFilterPrototype)
  private
    FDenominator: Array [0 .. 2] of Double;
    FGainFactor: Double;
  ...
  TAnalogueCWeightingFilterPrototype = class
    (TCustomAnalogueWeightingFilterPrototype)
  private
  ...
  TAnalogueDWeightingFilterPrototype = class
    (TCustomAnalogueWeightingFilterPrototype)
  private
...
function TCustomAnalogueFilterPrototype.Complex32(Frequency: Single)
  : TComplex32;
var
  Cmplx: TComplex64;
begin
...
procedure TCustomAnalogueArbitraryIntegerOrderFilterPrototype.SetOrder
  (Value: Integer);
begin
...
function TCustomBiquadAnalogueFilterPrototype.Complex64(Frequency: Double)
  : TComplex64;
var
  Omega: Double;
  Divisor: Double;
...
begin
  if Dest is TCustomBiquadAnalogueFilterPrototype then
    ...
    begin
      Move(Self.FNominator[0], FNominator[0], Length(FNominator) *
        SizeOf(Double));
      Move(Self.FDenominator[0], FDenominator[0], Length(FDenominator) *
        SizeOf(Double));
      FGain := Self.FGain;
...
begin
  FNominator[0] := Power(FGainFactor, 0.5 * (Abs(FShape) - FShape)) *
    Power(FGainFactor, Abs(FShape));
  FNominator[1] := FGainFactor * Power(FGainFactor, 0.5 * Abs(FShape)) /
    FBandwidth;
  FNominator[2] := Power(FGainFactor, 0.5 * (Abs(FShape) + FShape)) *
    Power(FGainFactor, Abs(FShape));
  FDenominator[0] := Power(FGainFactor, 0.5 * (Abs(FShape) + FShape));
  FDenominator[1] := Power(FGainFactor, 0.5 * Abs(FShape)) /
    (Power(FGainFactor, 1 - Abs(FShape)) * FBandwidth);
  ...
  *)
  ...
  begin
    ...
    begin
      InterShape := 3 + FShape;
      FNominator[0] := Sqr(FGainFactor);
      FNominator[1] := FGainFactor * Power(Sqrt(FGainFactor),
        InterShape) * FAlpha;
      FNominator[2] := FGainFactor;
      FDenominator[0] := 1;
      FDenominator[1] := FGainFactor * Power(Sqrt(FGainFactor),
        -InterShape) * FAlpha;
      FDenominator[2] := FGainFactor;
    end
    ...
    begin
      InterShape := 2 + FShape;
      FNominator[0] := Sqr(FGainFactor);
      FNominator[1] := Power(Sqrt(FGainFactor), InterShape) * FGainFactor *
        Sqrt(FGainFactor) * FAlpha;
      FNominator[2] := FGainFactor;
      FDenominator[0] := 1;
      FDenominator[1] := Power(Sqrt(FGainFactor), 1 - InterShape) * FAlpha;
      FDenominator[2] := FGainFactor;
    end
    else if FShape > 2 then
    begin
      InterShape := FShape - 2;
      FNominator[0] := FGainFactor;
      FNominator[1] := FGainFactor * Power(Sqrt(FGainFactor),
      ...
      FNominator[2] := Sqr(FGainFactor);
      FDenominator[0] := FGainFactor;
      FDenominator[1] := Sqrt(FGainFactor) * Power(Sqrt(FGainFactor),
        InterShape) * FAlpha;
      FDenominator[2] := 1;
    end
    else if FShape > 1 then
    begin
      InterShape := FShape - 1;
      FNominator[0] := FGainFactor;
      FNominator[1] := Sqr(FGainFactor) * Power(Sqrt(FGainFactor),
        -InterShape) * FAlpha;
      FNominator[2] := Sqr(FGainFactor);
      FDenominator[0] := FGainFactor;
...
function TAnalogueAWeightingFilterPrototype.Complex64(Frequency: Double)
  : TComplex64;
var
  Omega: Double;
  Divisor: Double;
...
begin
  ...
  Result.Re := Temp[0] * Temp[1] - 4 * Sqr(Omega) * FDenominator[0] *
    FDenominator[3];
  Result.Im := 2 * Omega * (FDenominator[0] * Temp[1] + FDenominator[3]
    * Temp[0]);
  ...
  Divisor := FGainFactor * Sqr(Omega) * Sqr(Omega) /
    ...
    * (Sqr(Sqr(FDenominator[0]) - Sqr(Omega)) + Sqr(2 * FDenominator[0] * Omega)
    ) * (Sqr(FDenominator[1] * FDenominator[2] - Sqr(Omega)) +
...
function TAnalogueBWeightingFilterPrototype.Complex64(Frequency: Double)
  : TComplex64;
var
  Omega: Double;
  Divisor: Double;
...
function TAnalogueCWeightingFilterPrototype.Complex64(Frequency: Double)
  : TComplex64;
var
  Omega: Double;
  Divisor: Double;
...
function TAnalogueDWeightingFilterPrototype.Complex64(Frequency: Double)
  : TComplex64;
var
  Omega: Double;
  Divisor: Double;
...
begin
  ...
  Divisor := 1 / (Sqr(FDenominator[0] - Sqr(Omega)) +
    Sqr(FDenominator[1] * Omega));
  ...
  Cmplex.Im := Omega * (FDenominator[2] * FDenominator[3] - Sqr(Omega))
    * Divisor;

...
type
  ...
  TSETunerPins = (pinInput, pinMinimum, pinMaximum, pinSmooth,
    ...
    pinThreshold, pinFrequency, pinNote, pinDetune);
...
procedure TCustomSEAdvancedTunerModule.SubProcessStatic(const BufferOffset,
  SampleFrames: Integer);
begin
...
class procedure TCustomSEAdvancedTunerModule.GetModuleProperties
...
function TCustomSEAdvancedTunerModule.GetPinProperties(const Index: Integer;
...
begin
  ...
  case TSETunerPins(CurrentPin.PinID) of
    pinInput:
      begin
        Pin[Integer(pinFrequency)].TransmitStatusChange(SampleClock,
          Pin[0].Status);
        ChooseProcess;
      end;
  end;
...
// describe the pins (plugs)
function TSEAdvancedTunerStaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
...
begin
  ...
  case TSETunerPins(CurrentPin.PinID) of
    pinMinimum:
      FTuner.MinimumFrequency := FMinimum;
    pinMaximum:
      FTuner.MaximumFrequency := FMaximum;
    pinSmooth:
      FTuner.SmoothFactor := FSmoothFactor;
    pinOneCrossingOnly:
      FTuner.OneCrossingOnly := FOneCrossingOnly;
    pinDSFilterOrder:
      FTuner.DownSampleFilterOrder := FDSFilterOrder;
    pinDSBandwidth:
      FTuner.DownSampleBandwidth := FDSBandwidth;
    pinAttack:
      FTuner.Attack := FAttack;
    pinRelease:
      FTuner.Release := FRelease;
  end;
end;
...
function TSEAdvancedTunerControllableModule.GetPinProperties
  (const Index: Integer; Properties: PSEPinProperties): Boolean;
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSETunerPins(index) in [pinMinimum .. pinThreshold] then
    with Properties^ do
      Direction := drIn;
end;

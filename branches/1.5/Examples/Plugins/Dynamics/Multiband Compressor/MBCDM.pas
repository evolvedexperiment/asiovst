...
procedure TMBCDataModule.MBCDMLowFrequencyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  i: Integer;
begin
  for i := 0 to Length(FLowSplit) - 1 do
    if Assigned(FLowSplit[i]) then
      FLowSplit[i].Frequency := Value;
  ...
  if EditorForm is TFmMBC then
    ...
    begin
      if Value < 1000 then
        LbLowFreqHz.Caption := FloatToStrF(Value, ffGeneral, 3, 2) + ' Hz'
      else
        LbLowFreqHz.Caption := FloatToStrF(0.001 * Value, ffGeneral, 3, 2) + 'kHz';
      if SbLowFreq.Value <> 10000 * FreqLogToLinear(Value) then
        SbLowFreq.Value := 10000 * FreqLogToLinear(Value);
    end;
end;

procedure TMBCDataModule.MBCDCLowOrderChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  i: Integer;
begin
  for i := 0 to Length(FLowSplit) - 1 do
    if Assigned(FLowSplit[i]) then
      FLowSplit[i].Order := Round(Value);
end;

procedure TMBCDataModule.MBCDMLowGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FLowComp) then
  ...
  if EditorForm is TFmMBC then
    ...
    begin
      ...
      if DlLowGain.Value <> Value then
        DlLowGain.Value := Value;
    end;
end;

procedure TMBCDataModule.MBCDMMidGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FMidComp) then
  ...
  if EditorForm is TFmMBC then
    ...
    begin
      ...
      if DlMidGain.Value <> Value then
        DlMidGain.Value := Value;
    end;
end;

procedure TMBCDataModule.MBCDMHighGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FHighComp) then
  ...
  if EditorForm is TFmMBC then
    ...
    begin
      ...
      if DlHighGain.Value <> Value then
        DlHighGain.Value := Value;
    end;
end;

procedure TMBCDataModule.MBCDMLowThresholdChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FLowComp) then
  ...
  if EditorForm is TFmMBC then
    ...
    begin
      ...
      if DlLowThreshold.Value <> Value then
        DlLowThreshold.Value := Value;
    end;
end;

procedure TMBCDataModule.MBCDMMidThresholdChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FMidComp) then
  ...
  if EditorForm is TFmMBC then
    ...
    begin
      ...
      if DlMidThreshold.Value <> Value then
        DlMidThreshold.Value := Value;
    end;
end;

procedure TMBCDataModule.MBCDMHighThresholdChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FHighComp) then
  ...
  if EditorForm is TFmMBC then
    ...
    begin
      ...
      if DlHighThreshold.Value <> Value then
        DlHighThreshold.Value := Value;
    end;
end;

procedure TMBCDataModule.MBCDMLowRatioChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FLowComp) then
  ...
  if EditorForm is TFmMBC then
    ...
    begin
      ...
      if DlLowRatio.Value <> Log10(Value) then
        DlLowRatio.Value := Log10(Value);
    end;
end;

procedure TMBCDataModule.MBCDMMidRatioChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FMidComp) then
  ...
  if EditorForm is TFmMBC then
    ...
    begin
      ...
      if DlMidRatio.Value <> Log10(Value) then
        DlMidRatio.Value := Log10(Value);
    end;
end;

procedure TMBCDataModule.MBCDMHighRatioChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FHighComp) then
  ...
  if EditorForm is TFmMBC then
    ...
    begin
      ...
      if DlHighRatio.Value <> Log10(Value) then
        DlHighRatio.Value := Log10(Value);
    end;
end;

procedure TMBCDataModule.MBCDMLowAttackChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FLowComp) then
  ...
  if EditorForm is TFmMBC then
    ...
    begin
      ...
      if DlLowAttack.Value <> Log10(Value) then
        DlLowAttack.Value := Log10(Value);
    end;
end;

procedure TMBCDataModule.MBCDMMidAttackChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FMidComp) then
  ...
  if EditorForm is TFmMBC then
    ...
    begin
      ...
      if DlMidAttack.Value <> Log10(Value) then
        DlMidAttack.Value := Log10(Value);
    end;
end;

procedure TMBCDataModule.MBCDMHighAttackChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FHighComp) then
  ...
  if EditorForm is TFmMBC then
    ...
    begin
      ...
      if DlHighAttack.Value <> Log10(Value) then
        DlHighAttack.Value := Log10(Value);
    end;
end;

procedure TMBCDataModule.MBCDMLowReleaseChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FLowComp) then
...
procedure TMBCDataModule.MBCDMMidReleaseChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FMidComp) then
...
procedure TMBCDataModule.MBCDMHighReleaseChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FHighComp) then
...
procedure TMBCDataModule.MBCDCHighOrderChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  i: Integer;
begin
  for i := 0 to Length(FHighSplit) - 1 do
    if Assigned(FHighSplit[i]) then
      FHighSplit[i].Order := Round(Value);
end;

procedure TMBCDataModule.MBCDMHighFrequencyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  i: Integer;
begin
  for i := 0 to Length(FHighSplit) - 1 do
    if Assigned(FHighSplit[i]) then
      FHighSplit[i].Frequency := Value;
  if Assigned(EditorForm) then
    with TFmMBC(EditorForm) do
    begin
      if Value < 1000 then
        LbHighFreqHz.Caption := FloatToStrF(Value, ffGeneral, 3, 2) + 'Hz'
      else
        LbHighFreqHz.Caption := FloatToStrF(0.001 * Value, ffGeneral, 3, 2) + 'kHz';
      if SbHighFreq.Value <> 10000 * FreqLogToLinear(Value) then
        SbHighFreq.Value := 10000 * FreqLogToLinear(Value);
    end;
end;
...
begin
  ...
  begin
    if Abs(Inputs[0, i]) > FInputPeak[0] then
      FInputPeak[0] := Abs(Inputs[0, i]);
    FInputPeak[0] := FMeterRelease * FInputPeak[0];
    if Abs(Inputs[1, i]) > FInputPeak[1] then
      FInputPeak[1] := Abs(Inputs[1, i]);
    ...
    if Abs(L[0]) > FInputPeak[2] then
      FInputPeak[2] := Abs(L[0]);
    FInputPeak[2] := FMeterRelease * FInputPeak[2];
    if Abs(L[1]) > FInputPeak[3] then
      FInputPeak[3] := Abs(L[1]);
    FInputPeak[3] := FMeterRelease * FInputPeak[3];

    if Abs(M[0]) > FInputPeak[4] then
      FInputPeak[4] := Abs(M[0]);
    FInputPeak[4] := FMeterRelease * FInputPeak[4];
    if Abs(M[1]) > FInputPeak[5] then
      FInputPeak[5] := Abs(M[1]);
    FInputPeak[5] := FMeterRelease * FInputPeak[5];

    if Abs(H[0]) > FInputPeak[6] then
      FInputPeak[6] := Abs(H[0]);
    FInputPeak[6] := FMeterRelease * FInputPeak[6];
    if Abs(H[1]) > FInputPeak[7] then
      FInputPeak[7] := Abs(H[1]);
    ...
    if Abs(L[0]) > FOutputPeak[2] then
      FOutputPeak[2] := Abs(L[0]);
    FOutputPeak[2] := FMeterRelease * FOutputPeak[2];
    if Abs(L[1]) > FOutputPeak[3] then
      FOutputPeak[3] := Abs(L[1]);
    FOutputPeak[3] := FMeterRelease * FOutputPeak[3];

    if Abs(M[0]) > FOutputPeak[4] then
      FOutputPeak[4] := Abs(M[0]);
    FOutputPeak[4] := FMeterRelease * FOutputPeak[4];
    if Abs(M[1]) > FOutputPeak[5] then
      FOutputPeak[5] := Abs(M[1]);
    FOutputPeak[5] := FMeterRelease * FOutputPeak[5];

    if Abs(H[0]) > FOutputPeak[6] then
      FOutputPeak[6] := Abs(H[0]);
    FOutputPeak[6] := FMeterRelease * FOutputPeak[6];
    if Abs(H[1]) > FOutputPeak[7] then
      FOutputPeak[7] := Abs(H[1]);
    FOutputPeak[7] := FMeterRelease * FOutputPeak[7];

    Outputs[0, i] := L[0] + M[0] + H[0];
    Outputs[1, i] := L[1] + M[1] + H[1];

    if Abs(Outputs[0, i]) > FOutputPeak[0] then
      FOutputPeak[0] := Abs(Outputs[0, i]);
    FOutputPeak[0] := FMeterRelease * FOutputPeak[0];
    if Abs(Outputs[1, i]) > FOutputPeak[1] then
      FOutputPeak[1] := Abs(Outputs[1, i]);
...
begin
  ...
  begin
    for Channel := 0 to Length(FLowSplit) - 1 do
      if Assigned(FLowSplit[Channel]) then
        FLowSplit[Channel].SampleRate := Abs(SampleRate);
    for Channel := 0 to Length(FHighSplit) - 1 do
      if Assigned(FHighSplit[Channel]) then
        FHighSplit[Channel].SampleRate := Abs(SampleRate);
  end;
end;

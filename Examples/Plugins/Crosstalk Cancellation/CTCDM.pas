...
procedure TCTCDataModule.ParamRecursionStepsChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
    if Assigned(FCrosstalkCancellation) then
      FCrosstalkCancellation.StageCount := Round(Value);
  finally
    FCriticalSection.Leave;
  end;

  // update GUI
  if EditorForm is TFmCTC then
    TFmCTC(EditorForm).UpdateRecursionSteps;
end;

procedure TCTCDataModule.ParamDistanceDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  if Parameter[Index] >= 100 then
    PreDefined := AnsiString(FloatToStrF(RoundTo(0.01 * Parameter[Index], -2),
      ffGeneral, 3, 3));
end;
...
procedure TCTCDataModule.ParameterFilterTypeDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  case Round(Parameter[Index]) of
    1:
      PreDefined := 'Simple Highpass';
  end;
end;

procedure TCTCDataModule.ParameterOutputChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FOutputGain := db_to_Amp(Value);
  if EditorForm is TFmCTC then
    TFmCTC(EditorForm).UpdateOutputGain;
end;

procedure TCTCDataModule.ParameterFilterGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FCrosstalkCancellation) then
    FCrosstalkCancellation.CrosstalkFilterGain := Value;
  if EditorForm is TFmCTC then
    TFmCTC(EditorForm).UpdateFilterGain;
end;

procedure TCTCDataModule.ParameterFilterFrequencyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FCrosstalkCancellation) then
    FCrosstalkCancellation.CrosstalkFilterFrequency := Value;
  if EditorForm is TFmCTC then
    TFmCTC(EditorForm).UpdateFilterFrequency;
end;

procedure TCTCDataModule.ParameterFilterTypeChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FCrosstalkCancellation) then
    FCrosstalkCancellation.CrosstalkFilterGain := Value;
  if EditorForm is TFmCTC then
    TFmCTC(EditorForm).UpdateFilterType;
end;

procedure TCTCDataModule.ParameterFilterFrequencyDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  if Parameter[Index] < 1000 then
    PreDefined := AnsiString(FloatToStrF(Parameter[Index], ffGeneral, 3, 3))
  else
    PreDefined := AnsiString(FloatToStrF(1E-3 * Parameter[Index],
      ffGeneral, 3, 3));
end;

procedure TCTCDataModule.ParameterFilterFrequencyLabel(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  if Parameter[Index] < 1000 then
    PreDefined := 'Hz'
...
procedure TCTCDataModule.ParameterBypassChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
...
procedure TCTCDataModule.ParamAttenuationChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FCrosstalkCancellation) then
    FCrosstalkCancellation.Attenuation := db_to_Amp(Value);

  // update GUI
  if EditorForm is TFmCTC then
    TFmCTC(EditorForm).UpdateAttenuation;
end;

procedure TCTCDataModule.ParamListenerChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
    if Assigned(FCrosstalkCancellation) then
      FCrosstalkCancellation.ListenerDistance := Value;
  finally
    FCriticalSection.Leave;
  end;
  ...
  // update GUI
  if EditorForm is TFmCTC then
    TFmCTC(EditorForm).UpdateListenerDistance;
end;

procedure TCTCDataModule.ParamSpeakerChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
    if Assigned(FCrosstalkCancellation) then
      FCrosstalkCancellation.SpeakerDistance := Value;
  finally
    FCriticalSection.Leave;
  end;
  ...
  // update GUI
  if EditorForm is TFmCTC then
    TFmCTC(EditorForm).UpdateSpeakerDistance;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      FCrosstalkCancellation.ProcessStereo(Outputs[0, Sample],
        Outputs[1, Sample]);
    end;
  finally
    FCriticalSection.Leave;
...
begin
  if Abs(SampleRate) > 0 then
    if Assigned(FCrosstalkCancellation) then
      FCrosstalkCancellation.SampleRate := Abs(SampleRate);
end;

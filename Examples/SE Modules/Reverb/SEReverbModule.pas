...
type
  ...
  TSEFreeverbPins = (pinFvInput, pinFvOutput, pinRoomsize, pinDamp,
    pinWet, pinDry);
...
constructor TCustomSEFreeverbModule.Create(SEAudioMaster
...
procedure TCustomSEFreeverbModule.SubProcessStatic(const BufferOffset,
...
class procedure TCustomSEFreeverbModule.GetModuleProperties
...
function TCustomSEFreeverbModule.GetPinProperties(const Index: Integer;
...
procedure TSEFreeverbStaticModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
var
...
begin
  ...
  for Sample := 0 to SampleFrames - 1 do
    Outp^[Sample] := FFreeverb.ProcessSample64(Inp^[Sample]);
end;

// describe the pins (plugs)
function TSEFreeverbStaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
...
begin
  ...
  case TSEFreeverbPins(CurrentPin.PinID) of
    pinRoomsize:
      FFreeverb.RoomSize := FRoomsize;
    pinDamp:
      FFreeverb.Damp := FDamp;
    pinWet:
      FFreeverb.Wet := 0.01 * FWet;
    pinDry:
      FFreeverb.Dry := 0.01 * FDry;
  end;
end;
...
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSEStkReverbPins(index) in [pinT60 .. pinEffectMix] then
    with Properties^ do
      Direction := drIn;
end;
...
constructor TCustomSEStkNReverbModule.Create(SEAudioMaster
  : TSE2audioMasterCallback; Reserved: Pointer);
begin
  inherited Create(SEAudioMaster, Reserved);
...
procedure TCustomSEStkNReverbModule.SubProcessStatic(const BufferOffset,
  SampleFrames: Integer);
begin
  SubProcess(BufferOffset, SampleFrames);
  FStaticCount := FStaticCount - SampleFrames;
...
// describe your module
class procedure TCustomSEStkNReverbModule.GetModuleProperties
  (Properties: PSEModuleProperties);
begin
  with Properties^ do
  begin
...
// describe the pins (plugs)
function TCustomSEStkNReverbModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
  Result := True;
...
procedure TSEStkNReverbStaticModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
var
  Inp: PDAVSingleFixedArray;
  Outp: PDAVSingleFixedArray;
...
begin
  ...
  for Sample := 0 to SampleFrames - 1 do
    Outp^[Sample] := FNReverb.Tick(Inp^[Sample]);
end;

// describe the pins (plugs)
function TSEStkNReverbStaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
  Result := inherited GetPinProperties(Index, Properties);
  case TSEStkReverbPins(index) of
...
begin
  ...
  case TSEStkReverbPins(CurrentPin.PinID) of
    pinT60:
      FNReverb.T60 := FT60;
    pinEffectMix:
      FNReverb.EffectMix := 0.01 * FEffectMix;
  end;
end;
...
constructor TCustomSEStkJCReverbModule.Create(SEAudioMaster
  : TSE2audioMasterCallback; Reserved: Pointer);
begin
  inherited Create(SEAudioMaster, Reserved);
...
procedure TCustomSEStkJCReverbModule.SubProcessStatic(const BufferOffset,
  SampleFrames: Integer);
begin
  SubProcess(BufferOffset, SampleFrames);
  FStaticCount := FStaticCount - SampleFrames;
...
// describe your module
class procedure TCustomSEStkJCReverbModule.GetModuleProperties
  (Properties: PSEModuleProperties);
begin
  with Properties^ do
  begin
...
// describe the pins (plugs)
function TCustomSEStkJCReverbModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
  Result := True;
  case TSEStkReverbPins(index) of
...
procedure TSEStkJCReverbStaticModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
var
  Inp: PDAVSingleFixedArray;
  Outp: PDAVSingleFixedArray;
...
begin
  ...
  for Sample := 0 to SampleFrames - 1 do
    Outp^[Sample] := FJCReverb.Tick(Inp^[Sample]);
end;

// describe the pins (plugs)
function TSEStkJCReverbStaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
  Result := inherited GetPinProperties(Index, Properties);
  case TSEStkReverbPins(index) of
...
begin
  ...
  case TSEStkReverbPins(CurrentPin.PinID) of
    pinT60:
      FJCReverb.T60 := FT60;
    pinEffectMix:
      FJCReverb.EffectMix := 0.01 * FEffectMix;
  end;
end;
...
constructor TCustomSEStkNReverb2Module.Create(SEAudioMaster
  : TSE2audioMasterCallback; Reserved: Pointer);
begin
  inherited Create(SEAudioMaster, Reserved);
  FNReverb := TStkNReverb.Create
...
procedure TCustomSEStkNReverb2Module.SubProcessStatic(const BufferOffset,
  SampleFrames: Integer);
begin
  SubProcess(BufferOffset, SampleFrames);
  FStaticCount := FStaticCount - SampleFrames;
...
// describe your module
class procedure TCustomSEStkNReverb2Module.GetModuleProperties
  (Properties: PSEModuleProperties);
begin
  with Properties^ do
  begin
...
// describe the pins (plugs)
function TCustomSEStkNReverb2Module.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
  Result := True;
...
procedure TSEStkNReverb2StaticModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
var
  Inp: PDAVSingleFixedArray;
...
// describe the pins (plugs)
function TSEStkNReverb2StaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
  Result := inherited GetPinProperties(Index, Properties);
  case TSEStkReverb2Pins(index) of
...
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSEStkReverb2Pins(index) in [pin2T60 .. pin2EffectMix] then
    with Properties^ do
      Direction := drIn;
end;
...
constructor TCustomSEStkJCReverb2Module.Create(SEAudioMaster
  : TSE2audioMasterCallback; Reserved: Pointer);
begin
  inherited Create(SEAudioMaster, Reserved);
  FJCReverb := TStkJCReverb.Create
...
procedure TCustomSEStkJCReverb2Module.SubProcessStatic(const BufferOffset,
  SampleFrames: Integer);
begin
  SubProcess(BufferOffset, SampleFrames);
  FStaticCount := FStaticCount - SampleFrames;
...
// describe your module
class procedure TCustomSEStkJCReverb2Module.GetModuleProperties
  (Properties: PSEModuleProperties);
begin
  with Properties^ do
  begin
...
// describe the pins (plugs)
function TCustomSEStkJCReverb2Module.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
  Result := True;
  case TSEStkReverb2Pins(index) of
...
procedure TSEStkJCReverb2StaticModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
var
  Inp: PDAVSingleFixedArray;
  Outp1: PDAVSingleFixedArray;
...
// describe the pins (plugs)
function TSEStkJCReverb2StaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
  Result := inherited GetPinProperties(Index, Properties);
  case TSEStkReverb2Pins(index) of
...
function TSEStkJCReverb2ControllableModule.GetPinProperties

...
type
  TBasicGainFilter = class(TBiquadIIRFilter, IDspProcessor32, IDspProcessor64)
  protected
...
{$ENDIF}
  ...
  TBasicPeakFilter = class(TBiquadIIRFilter, IDspProcessor32, IDspProcessor64)
  protected
    procedure CalculateCoefficients; override;
  end;

  TBasicPeakAFilter = class(TBiquadIIRFilter, IDspProcessor32, IDspProcessor64)
  protected
    procedure CalculateCoefficients; override;
  end;
  ...
  TBasicShapeFilter = class(TBiquadIIRFilter, IDspProcessor32, IDspProcessor64)
  private
    FShape: Double;
  ...
  TBasicLowcutFilter = class(TBiquadIIRFilter, IDspProcessor32, IDspProcessor64)
  protected
    procedure CalculateCoefficients; override;
  public
    procedure Complex(const Frequency: Double; out Real: Double;
  ...
  TBasicNotchFilter = class(TBiquadIIRFilter, IDspProcessor32, IDspProcessor64)
  protected
    procedure CalculateCoefficients; override;
  end;
...
begin
  ...
  if FGainFactorSquared = Sqr(G0) then
    GainRelation := 1
  else
  ...
  C := Sqr(DW) * Abs(Sqr(Gb) - Sqr(G1)) - 2 * Wq *
    (Abs(Sqr(Gb) - G0 * G1) - Sqrt((Sqr(Gb) - Sqr(G0)) * (Sqr(Gb) - Sqr(G1))));
  D := 2 * Wq * (Abs(FGainFactorSquared - G0 * G1) -
    Sqrt((FGainFactorSquared - Sqr(G0)) * (FGainFactorSquared - Sqr(G1))));

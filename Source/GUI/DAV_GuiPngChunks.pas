...
type
  ...
  public
    ...
    property GreenSampleValue: Word read FGreenSampleValue
      write FGreenSampleValue;
  end;
  ...
  public
    ...
    property PixelsPerUnitX: Cardinal read FPixelsPerUnitX
      write FPixelsPerUnitX;
    property PixelsPerUnitY: Cardinal read FPixelsPerUnitY
      write FPixelsPerUnitY;
  ...
  public
    ...
    property CalibratorName: AnsiString read FCalibratorName
      write FCalibratorName;
    property OriginalZeroMin: Integer read FOriginalZeroes[0]
      write FOriginalZeroes[0];
    property OriginalZeroMax: Integer read FOriginalZeroes[1]
      write FOriginalZeroes[1];
  ...
  public
    ...
    property LanguageString: AnsiString read FLanguageString
      write FLanguageString;
    property TranslatedKeyword: string read FTranslatedKeyword
      write FTranslatedKeyword;
  end;
...
procedure RegisterPngChunks(ChunkClasses
  : array of TCustomChunkPngWithHeaderClass);
function FindPngChunkByChunkName(ChunkName: TChunkName)
  : TCustomChunkPngWithHeaderClass;
...
function IsPngChunkRegistered(ChunkClass
  : TCustomChunkPngWithHeaderClass): Boolean;
var
...
begin
  if Dest is TChunkPngPalette then
    ...
    begin
      ...
      Move(Self.FPaletteEntries[0], FPaletteEntries[0],
        Length(Self.FPaletteEntries) * SizeOf(TRGB24));
    end
  else
    inherited;
end;
...
begin
  ...
  with Stream do
  begin
    if ((Size - Position) mod SizeOf(TRGB24)) <> 0 then
...
begin
  if Assigned(FTransparency) then
    FChunkSize := FTransparency.ChunkSize
  else
    FChunkSize := 0;
...
begin
  // determine chunk size
  if Assigned(FSignificantBits) then
    FChunkSize := FSignificantBits.GetChunkSize
  else
    FChunkSize := 0;
...
begin
  // determine chunk size
  if Assigned(FBackground) then
    FChunkSize := FBackground.GetChunkSize
  else
    FChunkSize := 0;
  ...
  if Dest is TPngChunkList then
    ...
    begin
      ...
      for Index := 0 to Self.Count - 1 do
        ...
        begin
          ChunkClass := TCustomChunkPngWithHeaderClass
            (Self.FChunks[Index].ClassType);
          FChunks[Index] := ChunkClass.Create
            (TCustomChunkPngWithHeader(Self.FChunks[Index]).FHeader);
...
begin
  ...
  FreeAndNil(FChunks[Index]);
  if Index < Count then
    System.Move(FChunks[Index + 1], FChunks[Index],
...
RegisterPngChunks([TChunkPngImageData, TChunkPngPalette, TChunkPngGamma,
  TChunkPngStandardColorSpaceRGB, TChunkPngPrimaryChromaticities, TChunkPngTime,
  TChunkPngTransparency, TChunkPngEmbeddedIccProfile,
  TChunkPngPhysicalPixelDimensions, TChunkPngTextChunk,
  TChunkPngCompressedTextChunk, TChunkPngInternationalTextChunk,
  TChunkPngImageHistogram, TChunkPngBackgroundColor, TChunkPngSignificantBits,
  TChunkPngImageOffset, TChunkPngPixelCalibrator]);

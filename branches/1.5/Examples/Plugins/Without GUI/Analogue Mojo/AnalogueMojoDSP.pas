...
//                                                                            //
//  Portions created by Christian-W. Budde are Copyright (C) 2005-2019        //
...
begin
  ...
  for ChannelIndex := 0 to Length(FTransformator) - 1 do
    FTransformator[ChannelIndex] := TTransformatorSimulation.Create;
...
begin
  for ChannelIndex := 0 to Length(FTransformator) - 1 do
    FreeAndNil(FTransformator[ChannelIndex]);
end;
...
procedure TAnalogueMojoDM.ParameterDampingDisplay(Sender: TObject;
...
begin
  case Length(FTransformator) of
    ...
    OnProcess := VSTModuleProcessMulti;
  end;
  OnProcess32Replacing := OnProcess;
end;

procedure TAnalogueMojoDM.ParameterFrequencyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ChannelIndex: Integer;
begin
  ...
  try
    for ChannelIndex := 0 to Length(FTransformator) - 1 do
      if Assigned(FTransformator[ChannelIndex]) then
        FTransformator[ChannelIndex].HighpassFrequency :=
          (1 + 0.02 * ChannelIndex) * Value;
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TAnalogueMojoDM.ParameterDampingChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ChannelIndex: Integer;
begin
  ...
  try
    for ChannelIndex := 0 to Length(FTransformator) - 1 do
      if Assigned(FTransformator[ChannelIndex]) then
        FTransformator[ChannelIndex].DampingFrequency :=
          (1 - 0.02 * ChannelIndex) * Value;
  finally
    FCriticalSection.Leave;
  end;
...
procedure TAnalogueMojoDM.VSTModuleSampleRateChange(Sender: TObject;
...
begin
  ...
  try
    if Abs(SampleRate) > 0 then
      for ChannelIndex := 0 to Length(FTransformator) - 1 do
        if Assigned(FTransformator[ChannelIndex]) then
          FTransformator[ChannelIndex].SampleRate := SampleRate;
  finally
    FCriticalSection.Leave;
  end;
end;
...
begin
  ...
  try
    for SampleIndex := 0 to SampleFrames - 1 do
      Outputs[0, SampleIndex] := FTransformator[0].ProcessSample64(Inputs[0, SampleIndex])
  finally
    FCriticalSection.Leave;
  end;
end;
...
var
  SampleIndex: Integer;
const
  CCrossTalkCoeff: array [0 .. 1, 0 .. 1] of Single = ((0.033, 0.967),
    (0.034, 0.967));
begin
  FCriticalSection.Enter;
  try
...
begin
  ...
  try
    for ChannelIndex := 0 to Length(FTransformator) - 1 do
      for SampleIndex := 0 to SampleFrames - 1 do
        Outputs[ChannelIndex, SampleIndex] := FTransformator[ChannelIndex].ProcessSample64(Inputs[ChannelIndex, SampleIndex])
  finally
    FCriticalSection.Leave;
  end;
end;

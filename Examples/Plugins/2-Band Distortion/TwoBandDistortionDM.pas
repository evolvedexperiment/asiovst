...
{$IFDEF FPC}LCLIntf, {$ELSE} Windows, {$ENDIF} Classes, SysUtils, Forms,
  SyncObjs, DAV_Types, DAV_VSTModule, DAV_DspFilterLinkwitzRiley,
  DAV_VSTCustomModule;
...
type
  TTwoBandDistortionDataModule = class(TVSTModule)
    ...
    procedure VSTModuleProcess32(const Inputs,
      Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
    procedure VSTModuleProcess64(const Inputs,
...
begin
  ...
  if EditorForm is TFmTwoBandDistortion then
    with TFmTwoBandDistortion(EditorForm) do
      UpdateFrequency;
end;
  ...
  try
    for SampleIndex := 0 to SampleFrames - 1 do
      ...
      begin
        ...
        FLinkwitzRiley[ChannelIndex].ProcessSample32
          (Inputs[ChannelIndex, SampleIndex], Low, High);
...
begin
  ...
  try
    for SampleIndex := 0 to SampleFrames - 1 do
      ...
      begin
        // using Linkwitz-Riley filters
        FLinkwitzRiley[ChannelIndex].ProcessSample64
          (Inputs[ChannelIndex, SampleIndex], Low, High);
...
begin
  FCriticalSection.Enter;
  try
    if Assigned(FLinkwitzRiley[0]) then
      FLinkwitzRiley[0].ResetStates;
    if Assigned(FLinkwitzRiley[1]) then
      FLinkwitzRiley[1].ResetStates;
  finally
    FCriticalSection.Leave;
  end;
end;

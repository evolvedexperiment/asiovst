...
type
  ...
  protected
    procedure AssignTo(Dest: TPersistent); override;
  public
    property OnGetFillColor: TGuiFillEvent read FOnGetFillColor
      write FOnGetFillColor;
  end;
...
procedure TCustomGuiPixelFramePrimitive.SetLineWidth(const Value: TFixed24Dot8);
begin
...
procedure TGuiPixelFilledEllipse.DrawFloatingPoint
...
begin
  ...
  begin
    ...
    if YRange[1] < 0 then
      Exit;
    if Center.Re - Rad.Re >= Width then
      Exit;
    if Center.Re + Rad.Re < 0 then
      Exit;

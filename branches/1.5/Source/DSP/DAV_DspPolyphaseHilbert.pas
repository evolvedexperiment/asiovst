...
procedure TPhaseHalfPi32.ProcessBlock(const Input, OutputA,
  OutputB: PDAVSingleFixedArray; SampleFrames: Integer);
var
  Pos: Integer;
begin
...
begin
  ...
  MemY[0] := (Input + {$IFDEF HandleDenormals}CDenorm32 + {$ENDIF}
    MemY[0]) * PDAV4DoubleArray(FCoefficients)[0] - MemX[0];
  MemX[0] := Input;
  MemY[1] := (FPrev + {$IFDEF HandleDenormals}CDenorm32 + {$ENDIF}
    MemY[1]) * PDAV4DoubleArray(FCoefficients)[1] - MemX[1];
  MemX[1] := FPrev;
  MemY[2] := (MemY[0] + MemY[2]) * PDAV4DoubleArray(FCoefficients)[2] - MemX[2];
  MemX[2] := MemY[0];
  OutputA := MemY[1];
  OutputB := MemY[2];
  FPrev := Input;
...
begin
  ...
  MemY[1] := (FPrev + {$IFDEF HandleDenormals}CDenorm32 + {$ENDIF}
    MemY[1]) * PDAV4DoubleArray(FCoefficients)^[1] - MemX[1];
  MemX[1] := FPrev;
  MemY[2] := (MemY[0] + MemY[2]) * PDAV4DoubleArray(FCoefficients)^[2]
    - MemX[2];
  MemX[2] := MemY[0];
  MemY[3] := (MemY[1] + MemY[3]) * PDAV4DoubleArray(FCoefficients)^[3]
    - MemX[3];
  MemX[3] := MemY[1];
  OutputA := MemY[2];
  OutputB := MemY[3];
...
begin
  ...
  FMem[FPhase].Y[1] := (FPrev + {$IFDEF HandleDenormals}CDenorm32 + {$ENDIF}
    FMem[FPhase].Y[1]) * FCoefficients[1] - FMem[FPhase].X[1];
  FMem[FPhase].X[1] := FPrev;
  FMem[FPhase].Y[2] := (FMem[FPhase].Y[0] + FMem[FPhase].Y[2]) * FCoefficients
    [2] - FMem[FPhase].X[2];
...
begin
  ...
  FMem[FPhase].Y[2] := (FMem[FPhase].Y[0] + FMem[FPhase].Y[2]) * FCoefficients
    [2] - FMem[FPhase].X[2];
  FMem[FPhase].X[2] := FMem[FPhase].Y[0];
  FMem[FPhase].Y[3] := (FMem[FPhase].Y[1] + FMem[FPhase].Y[3]) * FCoefficients
    [3] - FMem[FPhase].X[3];
...
begin
  ...
  Result := Sqrt(Sqr(FMem[FPhase].Y[FNumberOfCoeffs - 2]) +
    Sqr(FMem[FPhase].Y[FNumberOfCoeffs - 1]));
  FPrev := Input;
  FPhase := 1 - FPhase;
end;
{$ENDIF}
...
procedure TPhaseHalfPi64.ProcessBlock(const Input, OutputA,
  OutputB: PDAVDoubleFixedArray; SampleFrames: Integer);
var
  Pos: Integer;
begin
  Assert(SampleFrames > 0);

...
type
  ...
  public
    ...
    property HighpassSelect: THighpassSelect read FHighpassSelect
      write SetHighpassSelect;
  end;
  ...
  public
    ...
    property AddOriginalBass: Boolean read FAddOriginalBass
      write SetAddOriginalBass;
  end;
...
procedure TCustomHarmonicBass.SetHighpassSelect(const Value: THighpassSelect);
begin
...
begin
  if FHighpassSelect in [hp1stOrder, hp2ndOrder] then
    FHighpass.Frequency := 0.5 * FFrequency;
...
begin
  ...
  *)
  ...
  Harmonic := Limit(0.5 * FLimiter.ProcessSample64(4 * FHighpass.ProcessSample64
...
begin
  ...
  begin
    ...
    Harmonic := // 0.5 * FUpwardComp.ProcessSample64(
      Limit(0.5 * FDrive * FLimiter.ProcessSample64
      (4 * FHighpass.ProcessSample64(FDecay + Harmonic * (1 + Harmonic * -2 *
      FDecay))));
...
procedure TCustomLinkwitzBass.SetHighpassSelect(const Value: THighpassSelect);
begin
  if FHighpassSelect <> Value then
  begin
    FHighpassSelect := Value;
...
begin
  ...
  Harmonic := Limit(0.5 * FDrive * FLimiter.ProcessSample64
    (4 * FHighpass.ProcessSample64(FDecay + Low * (1 + Low * -2 * FDecay))));

...
procedure TSimpleSampleDelayVST.SDDelayLengthChange(Sender: TObject;
...
begin
  ...
  // update GUI
  if EditorForm is TFmSimpleSampleDelay then
    TFmSimpleSampleDelay(EditorForm).UpdateDelayLength;
end;

procedure TSimpleSampleDelayVST.ParameterFeedbackChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  ...
  // update GUI
  if EditorForm is TFmSimpleSampleDelay then
    TFmSimpleSampleDelay(EditorForm).UpdateFeedback;
end;

procedure TSimpleSampleDelayVST.ParameterInvFBChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  ...
  // update GUI
  if EditorForm is TFmSimpleSampleDelay then
    TFmSimpleSampleDelay(EditorForm).UpdateFeedbackInvert;
end;
...
begin
  ...
  // update GUI
  if EditorForm is TFmSimpleSampleDelay then
    TFmSimpleSampleDelay(EditorForm).UpdateWetMix;
end;

procedure TSimpleSampleDelayVST.ParamDryMixChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FMix[0] := 0.01 * Value;
  ...
  // update GUI
  if EditorForm is TFmSimpleSampleDelay then
    TFmSimpleSampleDelay(EditorForm).UpdateDryMix;
end;

procedure TSimpleSampleDelayVST.VSTModuleProcess(const Inputs,
...
begin
  ...
  try
    ...
    begin
      FBuffer[0, FBufferPos] := Inputs[0, SampleIndex] + FFeedFactor *
        FBuffer[0, FBufferPos];
      FBuffer[1, FBufferPos] := Inputs[1, SampleIndex] + FFeedFactor *
        FBuffer[1, FBufferPos];
      Inc(FBufferPos);
      if FBufferPos >= FBufferSize then
        FBufferPos := 0;

...
type
  ...
  public
    ...
    procedure ProcessStereo(var Left, Right: Single); overload;
      virtual; abstract;

    property Attenuation: Single read FAttenuation write FAttenuation;
    property CrosstalkFilterType: TCrosstalkFilterType read FCrosstalkFilterType
      write SetCrosstalkFilterType;
    property CrosstalkFilterFrequency: Single read FCrosstalkFilterFrequency
      write SetCrosstalkFilterFrequency;
    property CrosstalkFilterGain: Single read FCrosstalkFilterGain
      write SetCrosstalkFilterGain;
    property HeadRadius: Single read FHeadRadius;
    property ListenerDistance: Single read FListenerDistance
      write SetListenerDistance;
    property SpeakerDistance: Single read FSpeakerDistance
      write SetSpeakerDistance;
...
begin
  if FSpeakerDistance > 2 * FListenerDistance then
    SpeakerDistance := 2 * FListenerDistance;
  Changed;
end;
...
begin
  if 2 * FListenerDistance < FSpeakerDistance then
    FListenerDistance := 0.5 * SpeakerDistance;
  Changed;
end;
...
begin
  if Dest is TCrosstalkCancellation32 then
    ...
    begin
      inherited;
      // assign delay lines
      for Stage := 0 to Length(FDelayLine[0]) - 1 do
        FDelayLine[0, Stage].Assign(Self.FDelayLine[0, Stage]);
      for Stage := 0 to Length(FDelayLine[1]) - 1 do
        FDelayLine[1, Stage].Assign(Self.FDelayLine[1, Stage]);

      // assign filters
      for Stage := 0 to Length(FCrosstalkFilter[0]) - 1 do
        FCrosstalkFilter[0, Stage].Assign(Self.FCrosstalkFilter[0, Stage]);
      for Stage := 0 to Length(FCrosstalkFilter[1]) - 1 do
        FCrosstalkFilter[1, Stage].Assign(Self.FCrosstalkFilter[1, Stage]);
    end
  else
    inherited;
end;
...
begin
  for Channel := 0 to Length(FDelayLine) - 1 do
    if Assigned(FDelayLine[Channel]) then
      for Stage := 0 to Length(FDelayLine[Channel]) - 1 do
        FDelayLine[Channel, Stage].Samplerate := Samplerate;
  for Channel := 0 to Length(FDelayLine) - 1 do
    if Assigned(FCrosstalkFilter[Channel]) then
      for Stage := 0 to Length(FCrosstalkFilter[Channel]) - 1 do
        FCrosstalkFilter[Channel, Stage].Samplerate := Samplerate;
end;
...
begin
  ...
  *)
  ...
  for Stage := 0 to FStageCount - 1 do
  begin
    Temp := FDelayLine[0, Stage].ProcessSample32(FAttenuation * FCrosstalkFilter
      [0, Stage].ProcessSample32(-TempIn[1]));
    TempIn[1] := FDelayLine[1, Stage].ProcessSample32
      (FAttenuation * FCrosstalkFilter[1, Stage].ProcessSample32(-TempIn[0]));
    TempIn[0] := Temp;

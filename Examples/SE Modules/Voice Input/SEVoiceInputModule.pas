...
constructor TCustomSEVoiceInputModule.Create(SEAudioMaster
...
procedure TCustomSEVoiceInputModule.SubProcessStatic(const BufferOffset,
...
class procedure TCustomSEVoiceInputModule.GetModuleProperties
...
function TCustomSEVoiceInputModule.GetPinProperties(const Index: Integer;
...
procedure TSEVoiceInputStaticModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
var
...
begin
  ...
  for Sample := 0 to SampleFrames - 1 do
    Outp^[Sample] := FVoiceInput.ProcessSample(Inp^[Sample]);
end;

// describe the pins (plugs)
function TSEVoiceInputStaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
...
begin
  ...
  case TSEVoiceInputPins(CurrentPin.PinID) of
    pinTracking:
      FVoiceInput.Tracking := FTracking;
    pinPitch:
      FVoiceInput.Pitch := FPitch;
    pinBreath:
      FVoiceInput.Breath := FBreath;
    pinVoicedUnvoicedRatio:
      FVoiceInput.VoicedUnvoicedRatio := FVoicedUnvoicedRatio;
    pinMaximumFrequency:
      FVoiceInput.MaximumFrequency := FMaximumFrequency;
  end;
end;
...
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSEVoiceInputPins(index) in [pinTracking .. pinMaximumFrequency] then
    with Properties^ do
      Direction := drIn;
end;

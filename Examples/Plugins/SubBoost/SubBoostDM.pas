...
procedure TSubBoostDataModule.ParameterLevelChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FWet := 0.01 * Value;
end;

procedure TSubBoostDataModule.ParameterThresholdChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FThreshold := dB_to_Amp(Parameter[4]);
end;

procedure TSubBoostDataModule.ParameterTuneDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := AnsiString(FloatToStrF(FInputFilter.Frequency,
    ffGeneral, 3, 3));
end;

procedure TSubBoostDataModule.ParamOrderChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
...
procedure TSubBoostDataModule.ParameterTuneChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FInputFilter) then
    FInputFilter.Frequency := Value;
  if Assigned(FOutputFilter) then
...
procedure TSubBoostDataModule.ParameterReleaseDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := AnsiString
    (IntToStr(Round(-301.03 / (SampleRate * Log10(FRelease)))));
end;

procedure TSubBoostDataModule.ParameterReleaseChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FRelease := 1 - Power(10, -2 - (3 * Parameter[5]));
end;

procedure TSubBoostDataModule.ParameterModeDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
...
procedure TSubBoostDataModule.ParameterDryChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FDry := 0.01 * Value;
end;
...
begin
  ...
  begin
    // Input Filter
    FilteredIn := FInputFilter.ProcessSample64(Inputs[0, Sample] +
      Inputs[1, Sample]);
    ...
    case FType of
      ptDistort:
        if FilteredIn > th then
          SubBass := 1
        else if FilteredIn < -th then
          SubBass := -1
        else
          SubBass := 0;
      ptDivide:
        if FilteredIn > th then
          ...
          SubBass := 0;
      ptInvert:
        SubBass := ph * FilteredIn * 2; // Invert
      ptKeyOsc:
        begin // Osc
          if (FilteredIn > th) then
            en := 1
...
begin
  ...
  begin
    if Assigned(FInputFilter) then
      FInputFilter.SampleRate := Abs(SampleRate);
    if Assigned(FOutputFilter) then
      FOutputFilter.SampleRate := Abs(SampleRate);
  end;
end;

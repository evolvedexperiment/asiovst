...
type
  ...
  protected
    ...
    procedure DoProcessCopy(const Inputs, Outputs: TDAVArrayOfSingleFixedArray;
      const SampleFrames: Cardinal); overload;
    procedure DoProcessCopy(const Inputs, Outputs: TDAVArrayOfDoubleFixedArray;
      const SampleFrames: Cardinal); overload;
    procedure DoProcessMute(const Inputs, Outputs: TDAVArrayOfSingleFixedArray;
      const SampleFrames: Cardinal); overload;
    procedure DoProcessMute(const Inputs, Outputs: TDAVArrayOfDoubleFixedArray;
      const SampleFrames: Cardinal); overload;
    procedure DoBlockSaveProcess(const Inputs,
      Outputs: TDAVArrayOfSingleFixedArray;
      const SampleFrames: Cardinal); overload;
    procedure DoBlockSaveProcess(const Inputs,
      Outputs: TDAVArrayOfDoubleFixedArray;
      const SampleFrames: Cardinal); overload;
    procedure DoBlockSaveProcess32Replacing(const Inputs,
      Outputs: TDAVArrayOfSingleFixedArray;
      const SampleFrames: Cardinal); overload;
    procedure DoBlockSaveProcess32Replacing(const Inputs,
      Outputs: TDAVArrayOfDoubleFixedArray;
      const SampleFrames: Cardinal); overload;
    procedure DoProcessDspQueue(const Inputs,
      Outputs: TDAVArrayOfSingleFixedArray;
      const SampleFrames: Cardinal); overload;
    procedure DoProcessDspQueue(const Inputs,
      Outputs: TDAVArrayOfDoubleFixedArray;
  ...
  public
    ...
    property BlockModeSize: Cardinal read FBlockModeSize
      write SetBlockForcedSize default 1024;
    property BlockModeOverlap: Cardinal read FBlockModeOverlap
      write SetBlockOverlapSize default 0;
    property ProcessingMode: TProcessingMode read FProcessingMode
      write SetProcessingMode default pmNormal;

    property OnProcess: TProcessAudio32Event read FOnProcess write SetOnProcess;
    property OnProcess32Replacing: TProcessAudio32Event
      read FOnProcess32Replacing write SetOnProcess32Replacing;
    property OnProcess64Replacing: TProcessAudio64Event
      read FOnProcess64Replacing write SetOnProcess64Replacing;
  published
    property DspDirectProcessItem: TDAVProcessingComponent
      read FDspDirectProcessItem write SetDspDirectProcessItem default nil;
  end;
...
begin
  if Assigned(FDspQueueList) then
    FreeAndNil(FDspQueueList);
  inherited;
end;
...
begin
  ...
  if FDspDirectProcessItem = item then
    if FDspQueueList.Count > 0 then
      FDspDirectProcessItem := FDspQueueList.Items[0]
    else
      FDspDirectProcessItem := nil;
end;
...
begin
  if (FProcessingMode = pmBlockSave) and
    (FInitialDelay < Integer(FBlockModeSize - FBlockModeOverlap)) then
    FEffect.initialDelay := FBlockModeSize - FBlockModeOverlap
  else
    FEffect.initialDelay := FInitialDelay;

  if HostProduct <> 'energyXT' then
    IOChanged;
end;
...
begin
  if Value < FBlockModeSize then
    FBlockModeOverlap := Value;

  if (FProcessingMode = pmBlockSave) and
    (FEffect.initialDelay < Integer(FBlockModeSize - FBlockModeOverlap)) then
    InitialDelayChanged;
end;
...
procedure TDspVSTModule.DoProcessCopy(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
  Channel: Integer;
begin
{$IFDEF DebugLog} AddLogMessage('TDspVSTModule.DoProcessCopy'); {$ENDIF}
  for Channel := 0 to min(FEffect.numInputs, FEffect.numOutputs) - 1 do
    Move(Inputs[Channel, 0], PSingle(@Outputs[Channel, 0])^,
...
{$IFDEF DebugLog} AddLogMessage('TDspVSTModule.DoProcessCopy'); {$ENDIF}
  for Channel := 0 to min(FEffect.numInputs, FEffect.numOutputs) - 1 do
    Move(Inputs[Channel, 0], PDouble(@Outputs[Channel, 0])^,
      SampleFrames * SizeOf(Double));
end;

procedure TDspVSTModule.DoProcessMute(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
  Channel: Integer;
begin
{$IFDEF DebugLog} AddLogMessage('TDspVSTModule.DoProcessMute'); {$ENDIF}
  for Channel := 0 to FEffect.numOutputs - 1 do
    FillChar(PSingle(@Outputs[Channel, 0])^, SampleFrames * SizeOf(Single), 0);
end;
...
{$IFDEF DebugLog} AddLogMessage('TDspVSTModule.DoProcessMute'); {$ENDIF}
  for Channel := 0 to FEffect.numOutputs - 1 do
    FillChar(PDouble(@Outputs[Channel, 0])^, SampleFrames * SizeOf(Single), 0);
end;
  ...
  repeat
    ...
    begin
      for Channel := 0 to numInputs - 1 do
        Move(Inputs[Channel, CurrentPosition], FBlockInBuffer32[Channel,
          FBlockPosition], (SampleFrames - CurrentPosition) * SizeOf(Single));
      for Channel := 0 to numOutputs - 1 do
        Move(FBlockOutBuffer32[Channel, FBlockPosition],
          PSingle(@Outputs[Channel, CurrentPosition])^,
    ...
    begin
      for Channel := 0 to numInputs - 1 do
        Move(Inputs[Channel, CurrentPosition], FBlockInBuffer32[Channel,
          FBlockPosition], (FBlockModeSize - FBlockPosition) * SizeOf(Single));
      for Channel := 0 to numOutputs - 1 do
        Move(FBlockOutBuffer32[Channel, FBlockPosition],
          PSingle(@Outputs[Channel, CurrentPosition])^,
          (FBlockModeSize - FBlockPosition) * SizeOf(Single));

      FOnProcess(FBlockInBuffer32, FBlockOutBuffer32, FBlockModeSize);

      for Channel := 0 to numInputs - 1 do
        Move(FBlockInBuffer32[Channel, (FBlockModeSize - FBlockModeOverlap)],
          FBlockInBuffer32[Channel, 0], FBlockModeOverlap * SizeOf(Single));
...
{$IFDEF DebugLog} AddLogMessage('TDspVSTModule.DoBlockSaveProcess'); {$ENDIF}
  ...
  repeat
    ...
    begin
      for Channel := 0 to numInputs - 1 do
        Move(Inputs[Channel, CurrentPosition], FBlockInBuffer64[Channel,
          FBlockPosition], (SampleFrames - CurrentPosition) * SizeOf(Double));
      for Channel := 0 to numOutputs - 1 do
        Move(FBlockOutBuffer64[Channel, FBlockPosition],
          PDouble(@Outputs[Channel, CurrentPosition])^,
          (SampleFrames - CurrentPosition) * SizeOf(Double));
    ...
    begin
      for Channel := 0 to numInputs - 1 do
        Move(Inputs[Channel, CurrentPosition], FBlockInBuffer64[Channel,
          FBlockPosition], (FBlockModeSize - FBlockPosition) * SizeOf(Double));
      for Channel := 0 to numOutputs - 1 do
        Move(FBlockOutBuffer64[Channel, FBlockPosition],
          PDouble(@Outputs[Channel, CurrentPosition])^,
          (FBlockModeSize - FBlockPosition) * SizeOf(Double));

      FOnProcess64Replacing(FBlockInBuffer64, FBlockOutBuffer64,
        FBlockModeSize);

      for Channel := 0 to numInputs - 1 do
        Move(FBlockInBuffer64[Channel, (FBlockModeSize - FBlockModeOverlap)],
          FBlockInBuffer64[Channel, 0], FBlockModeOverlap * SizeOf(Double));
...
procedure TDspVSTModule.DoBlockSaveProcess32Replacing(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
  CurrentPosition: Cardinal;
  Channel: Integer;
begin
{$IFDEF DebugLog} AddLogMessage('TDspVSTModule.DoBlockSaveProcess32Replacing');
  {$ENDIF}
  CurrentPosition := 0;
  ...
  repeat
    ...
    begin
      ...
      FOnProcess32Replacing(FBlockInBuffer32, FBlockOutBuffer32,
        FBlockModeSize);
...
procedure TDspVSTModule.DoProcessDspQueue(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
...
{$IFDEF DebugLog} AddLogMessage('TDspVSTModule.DoProcessDspQueue'); {$ENDIF}
  SetLength(ProcessBuffer, max(numOutputs, numInputs) *
    SizeOf(PDAVSingleFixedArray));
  ...
  try
    for Channel := 0 to numInputs - 1 do
      Move(Inputs[Channel, 0], ProcessBuffer[Channel, 0],
        SampleFrames * SizeOf(Single));
    ...
    *)
    ...
    for Channel := 0 to numOutputs - 1 do
      Move(ProcessBuffer[Channel, 0], Outputs[Channel, 0],
        SampleFrames * SizeOf(Single));
  finally
    for Channel := 0 to max(numOutputs, numInputs) - 1 do
      FreeMem(ProcessBuffer[Channel]);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FillChar(FBlockInBuffer32[ChannelIndex]^,
        FBlockModeSize * SizeOf(Single), 0);
      FillChar(FBlockInBuffer64[ChannelIndex]^,
        FBlockModeSize * SizeOf(Double), 0);
    end;
    ...
    begin
      ...
      FillChar(FBlockOutBuffer32[ChannelIndex]^,
        FBlockModeSize * SizeOf(Single), 0);
      FillChar(FBlockOutBuffer64[ChannelIndex]^,
        FBlockModeSize * SizeOf(Double), 0);
    end;
...
begin
  case FProcessingMode of
    ...
    pmBlockSave:
      if Assigned(FOnProcess32Replacing) then
        FOnProcess32ReplacingEx := DoBlockSaveProcess32Replacing
      else
        FOnProcess32ReplacingEx := FOnProcess32Replacing;
    pmCopy:
...
begin
  case FProcessingMode of
    ...
    pmBlockSave:
      if Assigned(FOnProcess64Replacing) then
        FOnProcess64ReplacingEx := DoBlockSaveProcess32Replacing
      else
        FOnProcess64ReplacingEx := FOnProcess64Replacing;
    pmCopy:
  ...
  begin
    ...
    begin
      ...
      raise Exception.Create
        ('DspDirectProcessItem has to be the first item of a queue');
  end;
end;

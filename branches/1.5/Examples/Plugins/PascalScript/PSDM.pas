...
type
  ...
  TPascalScriptDataModule = class(TVSTModule)
    ...
    procedure VPSStoreChunk(Sender: TObject; const Index: Integer;
      const isPreset: Boolean);
    procedure VPSLoadChunk(Sender: TObject; const Index: Integer;
      const isPreset: Boolean);
...
procedure TPascalScriptDataModule.VSTModuleEditOpen(Sender: TObject;
  var GUI: TForm; ParentWindow: NativeUInt);
begin
  GUI := TFmPascalScript.Create(Self);
  if Length(FScriptCode) > 0 then
    with TFmPascalScript(GUI) do
      SynEdit.LineText := string(FScriptCode);
end;

procedure TPascalScriptDataModule.VPSLoadChunk(Sender: TObject;
  ...
  begin
    ...
    begin
      ...
      if Assigned(EditorForm) then
        with TFmPascalScript(EditorForm) do
          SynEdit.Lines.Text := string(ScriptCode);
    end;
  end;
end;

procedure TPascalScriptDataModule.VPSStoreChunk(Sender: TObject;
  const Index: Integer; const isPreset: Boolean);
var
  AtomName: array [0 .. 3] of Char;
  AtomSize: Integer;
...
begin
  ...
  begin
    FByteCode := Value;
    try
      if FPascalScriptExecuter.LoadData(FByteCode) then
        FVSTProcessSample := TVSTProcessSample
          (FPascalScriptExecuter.GetProcAsMethodN('VSTProcessSample'));
    except
...
begin
  if @FVSTProcessSample <> nil then
    ...
    begin
      d := Inputs[0, i];
      FVSTProcessSample(0, d);
      Outputs[0, i] := d;
      d := Inputs[1, i];
      FVSTProcessSample(1, d);
      Outputs[1, i] := d;
    end
  else
  begin

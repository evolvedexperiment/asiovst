...
begin
  ...
  begin
    ...
    if Abs(Outputs[0, i]) > FPeak[0] then
      FPeak[0] := Abs(Outputs[0, i]);
    if Abs(Outputs[1, i]) > FPeak[1] then
      FPeak[1] := Abs(Outputs[1, i]);
  end;
end;
...
begin
  ...
  if Amp_to_dB(FVolume[Index]) <> Value then
    with (EditorForm As TVSTVUMeterGUI) do
      case Index of
        0:
          SBLeft.Position := Round(Amp_to_dB(FVolume[Index]));
        1:
          SBRight.Position := Round(Amp_to_dB(FVolume[Index]));
      end;
end;

...
type
  ...
  public
    property FilterClass[Index: Integer]: TBandwidthIIRFilterClass
      read GetFilterClass write SetFilterClass;
...
begin
  // free DC filters
  for Channel := 0 to Length(FDCFilter) - 1 do
    FreeAndNil(FDCFilter[Channel]);

  // free EQ filters
  for Channel := 0 to Length(FEqFilter) - 1 do
    for Band := 0 to Length(FEqFilter[Channel]) - 1 do
      FreeAndNil(FEqFilter[Channel, Band]);

  // free gate
  for Channel := 0 to Length(FGate) - 1 do
    FreeAndNil(FGate[Channel]);

  // free compressor
  for Channel := 0 to Length(FCompressor) - 1 do
    for Band := 0 to Length(FCompressor[Channel]) - 1 do
      FreeAndNil(FCompressor[Channel, Band]);
end;

function TTrackPlugModule.GetFilter(Index: Integer): TCustomIIRFilter;
begin
  if (Index >= 0) and (Index < Length(FEqFilter[0])) then
    Result := FEqFilter[0, Index]
  else
    raise Exception.CreateFmt(RCStrIndexOutOfBounds, [Index]);
end;

function TTrackPlugModule.GetFilterClass(Index: Integer): TBandwidthIIRFilterClass;
begin
  if (Index >= 0) and (Index < Length(FEqFilter[0])) then
    Result := TBandwidthIIRFilterClass(FEqFilter[0, Index].ClassType)
  else
    raise Exception.CreateFmt(RCStrIndexOutOfBounds, [Index]);
end;

procedure TTrackPlugModule.ParameterDcFilterChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to Length(FDCFilter) - 1 do
    if Assigned(FDCFilter[Channel]) then
      FDCFilter[Channel].Frequency := Value;
end;

procedure TTrackPlugModule.ParameterDcFilterChangeOrder(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to Length(FDCFilter) - 1 do
    if Assigned(FDCFilter[Channel]) then
      FDCFilter[Channel].Order := Round(Value);
end;

procedure TTrackPlugModule.ParameterEqTypeDisplay(Sender: TObject;
...
begin
  case Round(Parameter[Index]) of
    ...
    5:
      PreDefined := 'Notch';
    6:
      PreDefined := 'Highshelf';
    7:
      PreDefined := 'Highcut';
  end;
end;

procedure TTrackPlugModule.ParameterEqFilterFrequencyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel, Band: Integer;
begin
  ...
  for Channel := 0 to Length(FEqFilter) - 1 do
    if Assigned(FEqFilter[Channel, Band]) then
      FEqFilter[Channel, Band].Frequency := Value;
end;

procedure TTrackPlugModule.ParameterEqFilterGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel, Band: Integer;
begin
  ...
  for Channel := 0 to Length(FEqFilter) - 1 do
    if Assigned(FEqFilter[Channel, Band]) then
      FEqFilter[Channel, Band].Gain := Value;
end;

procedure TTrackPlugModule.ParameterFilterBandwidthChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel, Band: Integer;
begin
  ...
  for Channel := 0 to Length(FEqFilter) - 1 do
    if Assigned(FEqFilter[Channel, Band]) then
      FEqFilter[Channel, Band].Bandwidth := Value;
end;
  ...
  if (Index >= 0) and (Index < Length(FEqFilter[0])) then
    for Channel := 0 to Length(FEqFilter) - 1 do
      if Assigned(FEqFilter[Channel, Index]) then
        if TBandwidthIIRFilterClass(FEqFilter[Channel, Index].ClassType) <> Value
        then
        begin
...
procedure TTrackPlugModule.ParameterEqTypeChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  ...
  if Assigned(FEqFilter[0, Band]) then
    case Round(Value) of
      0:
        FilterClass[Band] := TBasicGainFilter;
      1:
        FilterClass[Band] := TBasicLowcutFilter;
      2:
        FilterClass[Band] := TBasicLowShelfFilter;
      3:
        FilterClass[Band] := TBasicPeakFilter;
      4:
        FilterClass[Band] := TBasicBandpassFilter;
      5:
        FilterClass[Band] := TBasicNotchFilter;
      6:
        FilterClass[Band] := TBasicHighShelfFilter;
      7:
        FilterClass[Band] := TBasicHighpassFilter;
    end;
end;

procedure TTrackPlugModule.ParameterCompressorAttackChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  ...
  begin
    ...
    if Assigned(FCompressor[1, Band]) then
      FCompressor[1, Band].Attack := FCompressor[0, Band].Attack;
  end;
end;

procedure TTrackPlugModule.ParameterCompressorReleaseChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  ...
  begin
    ...
    if Assigned(FCompressor[1, Band]) then
      FCompressor[1, Band].Release := FCompressor[0, Band].Release;
  end;
end;

procedure TTrackPlugModule.ParameterCompressorThresholdChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  ...
  begin
    ...
    if Assigned(FCompressor[1, Band]) then
      FCompressor[1, Band].Threshold_dB := Value;
  end;
end;

procedure TTrackPlugModule.ParameterCompressorRatioChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  ...
  begin
    ...
    if Assigned(FCompressor[1, Band]) then
      FCompressor[1, Band].Ratio := Value;
  end;
end;

procedure TTrackPlugModule.ParameterCompressorKneeChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  ...
  begin
    ...
    if Assigned(FCompressor[1, Band]) then
      FCompressor[1, Band].Knee_dB := Value;
  end;
end;
...
procedure TTrackPlugModule.ParameterCompressorMixChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  Value := 100;
end;

procedure TTrackPlugModule.ParameterGateAttackChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FGate[0]) then
  begin
    FGate[0].Attack := Value;
    if Assigned(FGate[1]) then
      FGate[1].Attack := FGate[0].Attack;
  end;
end;

procedure TTrackPlugModule.ParameterGateReleaseChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FGate[0]) then
  begin
    ...
    if Assigned(FGate[1]) then
      FGate[1].Release := FGate[0].Release;
  end;
end;

procedure TTrackPlugModule.ParameterGateThresholdChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FGate[0]) then
  begin
    ...
    if Assigned(FGate[1]) then
      FGate[1].Threshold_dB := Value;
  end;
end;

procedure TTrackPlugModule.ParameterGateRatioChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FGate[0]) then
  begin
...
procedure TTrackPlugModule.ParameterGateKneeChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FGate[0]) then
  begin
    ...
    if Assigned(FGate[1]) then
      FGate[1].Knee_dB := Value;
  end;
end;

procedure TTrackPlugModule.ParameterTimeLabel(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
var
  Val: Single;
begin
...
procedure TTrackPlugModule.ParameterTimeDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
var
  Val: Single;
begin
  Val := Parameter[Index];
  if Val < 1 then
    PreDefined := AnsiString(FloatToStrF(RoundTo(1E3 * Val, -2),
      ffGeneral, 3, 3))
  else if Val < 1000 then
    PreDefined := AnsiString(FloatToStrF(RoundTo(Val, -2), ffGeneral, 3, 3))
  else
    PreDefined := AnsiString(FloatToStrF(RoundTo(1E-3 * Val, -2),
      ffGeneral, 3, 3));
end;

procedure TTrackPlugModule.ParameterMakeUpGainDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := AnsiString(FloatToStrF(RoundTo(Parameter[Index], -2),
    ffGeneral, 3, 3));
end;

procedure TTrackPlugModule.ParameterCompressorMakeUpGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
...
procedure TTrackPlugModule.ParameterThresholdDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := AnsiString(FloatToStrF(RoundTo(Parameter[Index], -2),
    ffGeneral, 3, 3));
...
procedure TTrackPlugModule.ParameterRatioDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := AnsiString(FloatToStrF(RoundTo(Parameter[Index], -2),
    ffGeneral, 3, 3));
...
procedure TTrackPlugModule.ParameterKneeDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := AnsiString(FloatToStrF(RoundTo(Parameter[Index], -2),
    ffGeneral, 3, 3));
...
begin
  ...
  for Channel := 0 to Length(FCompressor) - 1 do
    for Band := 0 to Length(FCompressor[Channel]) - 1 do
      if Assigned(FCompressor[Channel, Band]) then
        FCompressor[Channel, Band].SampleRate := SampleRate;
end;
...
begin
  for Channel := 0 to Length(FDCFilter) - 1 do
    ...
    begin
      // DC filter first
      CurrentSample := FDCFilter[Channel].ProcessSample64
        (Inputs[Channel, Sample]);

      // process EQ filters
      for Band := 0 to Length(FEqFilter) - 1 do
        CurrentSample := FEqFilter[Channel, Band].ProcessSample64
          (CurrentSample);

...
type
  ...
  TProcessBlock32 = procedure(Sender: TObject;
    const Input: PDAVSingleFixedArray) of object;
  TProcessBlock64 = procedure(Sender: TObject;
    const Input: PDAVDoubleFixedArray) of object;
...
procedure TBuildingBlocks32.ProcessBlock32(const Input: PDAVSingleFixedArray;
...
begin
  ...
  repeat
    ...
    begin
      Move(Input^[CurrentPosition], FBuffer32^[FBlockPosition],
    ...
    begin
      Move(Input^[CurrentPosition], FBuffer32^[FBlockPosition],
        (FBlockSize - FBlockPosition) * SizeOf(Single));

      BlockComplete;

      Move(FBuffer32^[(FBlockSize - FOverlapSize)], FBuffer32^[0],
        FOverlapSize * SizeOf(Single));
...
begin
  Move(FBuffer32^[FBlockPosition], FBlock32^[0], (FBlockSize - FBlockPosition) *
    SizeOf(Single));
  Move(FBuffer32^[0], FBlock32^[(FBlockSize - FBlockPosition)],
    FBlockPosition * SizeOf(Single));
  inherited;
  FSamplesInBlock := 0;
end;
...
begin
  ...
  repeat
    ...
    begin
      ...
      begin
        Move(Input[SamplesWritten], FBuffer32[FBlockPosition],
      ...
      begin
        ...
        Move(Input[SamplesWritten], FBuffer32[0],
          SampleFrames * SizeOf(Single));
      end;
    ...
    begin
      ...
      begin
        Move(Input[SamplesWritten], FBuffer32[FBlockPosition],
          (FSampleAdvance - FSamplesInBlock) * SizeOf(Single));
...
begin
  ...
  if FSamplesInBlock = FSampleAdvance then
    BlockComplete;
end;

{ TBuildingBlocks64 }

procedure TBuildingBlocks64.ProcessBlock64(const Input: PDAVDoubleFixedArray;
  SampleFrames: Integer);
var
  CurrentPosition: Integer;
begin
  ...
  repeat
    if FBlockPosition + (SampleFrames - CurrentPosition) < FBlockSize then
    begin
      Move(Input^[CurrentPosition], FBuffer64^[FBlockPosition],
    ...
    begin
      Move(Input^[CurrentPosition], FBuffer64^[FBlockPosition],
        (FBlockSize - FBlockPosition) * SizeOf(Double));

      if Assigned(FOnProcess) then
        FOnProcess(Self, FBuffer64);

      Move(FBuffer64^[(FBlockSize - FOverlapSize)], FBuffer64^[0],
        FOverlapSize * SizeOf(Double));
...
begin
  Move(FBuffer64^[FBlockPosition], FBlock64^[0], (FBlockSize - FBlockPosition) *
    SizeOf(Double));
  Move(FBuffer64^[0], FBlock64^[(FBlockSize - FBlockPosition)],
    FBlockPosition * SizeOf(Double));
...
begin
  ...
  repeat
    ...
    begin
      ...
      if FBlockPosition + SampleFrames < FBlockSize then
      begin
        Move(Input[SamplesWritten], FBuffer64[FBlockPosition],
      ...
      begin
        ...
        // move missing samples
        Move(Input[SamplesWritten], FBuffer64[0],
          SampleFrames * SizeOf(Double));
      end;
    ...
    begin
      ...
      begin
        Move(Input[SamplesWritten], FBuffer64[FBlockPosition],
          (FSampleAdvance - FSamplesInBlock) * SizeOf(Double));

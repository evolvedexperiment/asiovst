...
procedure TSimpleFeedbackCompressorDataModule.SLThresholdChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FSimpleFeedbackCompressors[0]) then
    FSimpleFeedbackCompressors[0].Threshold_dB := Value;
  if Assigned(FSimpleFeedbackCompressors[1]) then
...
procedure TSimpleFeedbackCompressorDataModule.SLRatioChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FSimpleFeedbackCompressors[0]) then
  begin
...
procedure TSimpleFeedbackCompressorDataModule.SLReleaseChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  if Assigned(FSimpleFeedbackCompressors[0]) then
...
begin
  ...
  if Value < 3 * SampleDuration_ms then
    Value := 3 * SampleDuration_ms;
...
begin
  ...
  begin
    ...
    SampleDuration_ms := 1000 / SampleRate;
    if Parameter[3] < 3 * SampleDuration_ms then
      Parameter[3] := 3 * SampleDuration_ms;
  end;
end;

...
type
  ...
  public
    ...
    procedure FilterHorizontal(Data: PByteArray; Count: Integer);
      overload; virtual;
    procedure FilterHorizontal(Data: PPixel32Array; Count: Integer);
      overload; virtual;
...
begin
  if FRadius <= 0 then
    raise Exception.Create('Radius must be larger than zero!');
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Sum := Sum - Current;
        if Sum > 0 then
          Data^[X] := Round($FF * Sum * FRadiusReciprocal)
        else
          Data^[X] := 0;
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        Sum := Sum - Current;
        if Sum > 0 then
          Data^[Y * Width + X] := Round($FF * Sum * FRadiusReciprocal)
        else
          Data^[Y * Width + X] := 0;
      end;
    end;
...
var
  ...
  Sum: Integer;
begin
  if FKernelSize <= 1 then
    Exit;

  with ByteMap do
  begin
    for Y := 0 to Height - 1 do
      FilterHorizontal(ScanLine[Y], Width);
...
procedure TGuiBlurFIRFilter.FilterHorizontal(Data: PPixel32Array;
  Count: Integer);
begin
...
begin
  ...
  for Index := 0 to (ByteMap.Width * ByteMap.Height) - 1 do
    Data^[Index] := FLookUpTable[Data^[Index]];
end;
...
begin
  ...
  for Index := 0 to Length(FLookUpTable) - 1 do
    FLookUpTable[Index] :=
      Round($FF * Index / (Index - ExpPos * (Index xor $FF)));
end;
...
begin
  ...
  begin
    if ptr[X].R > RMx then
      RMx := ptr[X].R
    else if ptr[X].R < RMn then
      RMn := ptr[X].R;
    if ptr[X].G > GMx then
      GMx := ptr[X].G
    else if ptr[X].G < GMn then
      GMn := ptr[X].G;
    if ptr[X].B > BMx then
      BMx := ptr[X].B
    else if ptr[X].B < BMn then
      BMn := ptr[X].B;
    if ptr[X].A > AMx then
      AMx := ptr[X].A
    else if ptr[X].A < AMn then
      AMn := ptr[X].A;
  end;
  ...
  begin
    ...
    begin
      ptr := PixelMap.ScanLine[Y];
      for X := 0 to PixelMap.Width - 1 do
        ptr[X].A := Round((ptr[X].A - AMn) * Scale);
    end;
  end;
  ...
  begin
    ...
    begin
      ptr := PixelMap.ScanLine[Y];
      for X := 0 to PixelMap.Width - 1 do
        ptr[X].R := Round((ptr[X].R - RMn) * Scale);
    end;
  end;
  ...
  begin
    ...
    begin
      ptr := PixelMap.ScanLine[Y];
      for X := 0 to PixelMap.Width - 1 do
        ptr[X].G := Round((ptr[X].G - GMn) * Scale);
    end;
  end;
  ...
  begin
    ...
    begin
      ptr := PixelMap.ScanLine[Y];
      for X := 0 to PixelMap.Width - 1 do
        ptr[X].B := Round((ptr[X].B - BMn) * Scale);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    for X := 0 to ByteMap.Width - 2 do
      p1[X] := (p1[X] + (p2[(X + 1)] xor $FF)) shr 1;
  end;
end;
...
begin
  Data := PixelMap.DataPointer;
  for Index := 0 to PixelMap.Width * PixelMap.Height - 1 do
    Data^[Index].ARGB := not Data^[Index].ARGB;
end;
  ...
  with ByteMap do
    ...
    begin
      ...
      begin
        ...
        begin
          if (X > 0) then
            ByteArray[0] := Value[X - 1, Y - 1];
          ByteArray[1] := Value[X, Y - 1];
          if (X < Width - 1) then
            ByteArray[2] := Value[X + 1, Y - 1];
        end
        else
        begin
          ByteArray[0] := 0;
        ...
        if (X > 0) then
          ByteArray[3] := Value[X - 1, Y];
        if (X < Width - 1) then
          ByteArray[4] := Value[X + 1, Y];
        if (Y < Height - 1) then
        begin
          if (X > 0) then
            ByteArray[5] := Value[X - 1, Y + 1];
          ByteArray[6] := Value[X, Y + 1];
          if (X < Height - 1) then
            ByteArray[7] := Value[X + 1, Y + 1];
        end
        else
        begin
          ByteArray[5] := 0;
        ...
        if (Sum = 0) then
          ...
          Reference := (Sum + nx shr 1) div nx;

        Temp := Value[X, Y];
        if Reference <> 0 then
          Temp := Limit(2 * Temp - Reference, 0, 255);
        Dest^[X] := Temp;
      end;
    end;
end;
  ...
  with PixelMap do
    ...
    begin
      ...
      begin
        ...
        begin
          if (X > 0) then
            PixelArray[0] := Pixel[X - 1, Y - 1];
          PixelArray[1] := Pixel[X, Y - 1];
          if (X < Width - 1) then
            PixelArray[2] := Pixel[X + 1, Y - 1];
        end
        else
        begin
          PixelArray[0].ARGB := 0;
        ...
        if (X > 0) then
          PixelArray[3] := Pixel[X - 1, Y];
        if (X < Width - 1) then
          PixelArray[4] := Pixel[X + 1, Y];
        if (Y < Height - 1) then
        begin
          if (X > 0) then
            PixelArray[5] := Pixel[X - 1, Y + 1];
          PixelArray[6] := Pixel[X, Y + 1];
          if (X < Height - 1) then
            PixelArray[7] := Pixel[X + 1, Y + 1];
        end
        else
        begin
          PixelArray[5].ARGB := 0;

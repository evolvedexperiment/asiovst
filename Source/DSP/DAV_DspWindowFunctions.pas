...
type
  ...
  TCosForwardLoop32 = procedure(Data: PDAVSingleFixedArray;
    var ParamRec: TParameterRecord; SampleCount: Integer);
  TCosForwardLoop64 = procedure(Data: PDAVDoubleFixedArray;
    var ParamRec: TParameterRecord; SampleCount: Integer);
  TCosSymmetricLoop32 = procedure(Data: PDAVSingleFixedArray;
    var ParamRec: TParameterRecord; SampleCount: Integer;
    EndAdr: PDAVSingleFixedArray);
  TCosSymmetricLoop64 = procedure(Data: PDAVDoubleFixedArray;
    var ParamRec: TParameterRecord; SampleCount: Integer;
    EndAdr: PDAVDoubleFixedArray);
  ...
  published
    property Start: Integer read FStart write SetWinStart default 0;
    // Window Start
    property Length: Integer read FLength write SetWinLength; // Window Length
    property ZeroDC: Boolean read FZeroDC write FZeroDC default False;
    // Colva DC removal after windowing
    property Tukey: Single read FTukey write SetTukey; // Tukey percentage
    property Bartlett: Boolean read FBartlett write SetBartlett default False;
    // Bartlett
    property Slope: TWindowSlope read FWinSlope write SetWinSlope
      default wsSymmetric; // Slope left, sym, right
    property Invert: Boolean read FInvert write FInvert default False;
    // Inverse window
    property SpectrumCorrectionFaktor: Double read GetSpkCorFak;
    // Calculates the Correction Factor or delivers last
    property CoherentGain: Double read GetSpkCorFakSq;
    // Calculates the squared Correction Factor or delivers last
  ...
  public
    procedure ProcessBlock32(Data: PDAVSingleFixedArray;
      SampleCount: Integer); override;
    procedure ProcessBlock64(Data: PDAVDoubleFixedArray;
      SampleCount: Integer); override;
  end;
...
uses
  Math, SysUtils, DAV_Common, DAV_Math, DAV_BlockProcessing, DAV_Consts;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Round(2 * FEffLength - 0.5) mod 2) > 0 then
      // remove sample in the middle if necessary
      begin
        FSpkCorFak := FSpkCorFak - CurWinVal;
        FSpkCorFakSq := FSpkCorFakSq - CurWinVal * CurWinVal;
      end;
...
procedure TWindowFunctionCustomFactor.ProcessBlock32(Data: PDAVSingleFixedArray;
  SampleCount: Integer);
var
  i, p1, p2: Integer;
  Counter: Integer;
...
begin
  ...
  if FInvert then
    StartPos := FStart + Length - FEffLengthRnd
  else
    StartPos := FStart;

  if FInvert then
    EndPos := FStart + FEffLengthRnd - 1
  else
  ...
  case FWinSlope of
    ...
    wsSymmetric:
      begin
        if (StartPos + FEffLengthRnd < SampleCount) and
          (EndPos - FEffLengthRnd >= 0) then
        ...
        begin
          ...
          begin // Only right wrap around!
            ...
            begin
              ...
              FSpkCorFakSq := FSpkCorFakSq + CurWinVal * CurWinVal;
              Data[StartPos + i] := Data[StartPos + i] * CurWinVal;
              Data[SampleCount - i + p1 - 1] := Data[SampleCount - i + p1 - 1] *
                CurWinVal;
            end;
          end
          else if (EndPos - FEffLengthRnd >= 0) then
...
procedure TWindowFunctionCustomFactor.ProcessBlock64(Data: PDAVDoubleFixedArray;
  SampleCount: Integer);
var
  CurWinVal: Double;
  i, p1, p2: Integer;
    ...
    wsSymmetric:
      begin
        ...
        begin
          ...
          else if (StartPos + FEffLengthRnd <= SampleCount) then
          begin // Only right wrap around!
            p1 := EndPos + 1;
            for i := 0 to p1 - 1 do
            begin
...
function TWindowFunctionRectangle.GetWindowFactor(Pos: Double): Double;
// inline;
begin
  Result := 1;
end;
...
procedure TWindowFunctionRectangle.ProcessBlock32(Data: PDAVSingleFixedArray;
  SampleCount: Integer);
var
  StartPos: Integer;
  EndPos: Integer;
...
procedure TWindowFunctionRectangle.ProcessBlock64(Data: PDAVDoubleFixedArray;
  SampleCount: Integer);
var
  StartPos: Integer;
  EndPos: Integer;
...
function TWindowFunctionTriangle.GetWindowFactor(Pos: Double): Double;
// inline;
begin
  Result := Pos;
end;
...
procedure TWindowFunctionTriangle.ProcessBlock32(Data: PDAVSingleFixedArray;
  SampleCount: Integer);
var
...
begin
  ...
  begin
    if FBartlett then
      ParamRec.ComplexPosition.Re := 0.5 * FEffLengthReci
    else
      ParamRec.ComplexPosition.Re := 0;
    ParamRec.ComplexAngle.Re := FEffLengthReci;
    if (FWinSlope = wsSymmetric) and FInvert then
      if (FLength mod 2 = 0) then
        ParamRec.ComplexPosition.Re := 0.5 * ParamRec.ComplexAngle.Re
      else
      begin
        EndPos := EndPos + 1;
        StartPos := StartPos - 1;
  ...
  case FWinSlope of
    wsLeft:
      begin
        if StartPos + FEffLengthRnd <= SampleCount then
          DoWinLoopTriangle32Forward(@Data[StartPos], ParamRec, FEffLengthRnd)
        else // Wrap arround
        begin
          DoWinLoopTriangle32Forward(@Data[StartPos], ParamRec,
            SampleCount - StartPos);
          DoWinLoopTriangle32Forward(@Data[0], ParamRec,
            FEffLengthRnd - (SampleCount - StartPos));
        end;
        ParamRec.SpectrumCorrectionFactor := ParamRec.SpectrumCorrectionFactor +
          FLength - FEffLengthRnd;
        ParamRec.SpuaredCorrectionFactor := ParamRec.SpuaredCorrectionFactor +
          FLength - FEffLengthRnd;
      end;
    wsRight:
      begin
        if EndPos + 1 - FEffLengthRnd >= 0 then
          DoWinLoopTriangle32Forward(@Data[EndPos + 1 - FEffLengthRnd],
            ParamRec, FEffLengthRnd)
        else // Wrap arround
        begin
          DoWinLoopTriangle32Forward
            (@Data[(EndPos + 1 - FEffLengthRnd + SampleCount)], ParamRec,
            FEffLengthRnd - (EndPos + 1));
    ...
    wsSymmetric:
      begin
        if (StartPos + FEffLengthRnd < SampleCount) and
          (EndPos - FEffLengthRnd >= 0) then
          DoWinLoopTriangle32Symmetric(@Data[StartPos], ParamRec, FEffLengthRnd,
            @Data[EndPos]) // No wrap arround!
        else // Wrap around
        begin
          if (EndPos - FEffLengthRnd >= 0) then
          begin // Only left wrap around!
            i := SampleCount - StartPos;
            DoWinLoopTriangle32Symmetric(@Data[StartPos], ParamRec, i,
              @Data[EndPos]);
            DoWinLoopTriangle32Symmetric(@Data[0], ParamRec, FEffLengthRnd - i,
              @Data[EndPos - i]);
          end
          else if (StartPos + FEffLengthRnd <= SampleCount) then
          begin // Only right wrap around!
            DoWinLoopTriangle32Symmetric(@Data[StartPos], ParamRec, EndPos + 1,
              @Data[EndPos]);
            DoWinLoopTriangle32Symmetric(@Data[StartPos + EndPos + 1], ParamRec,
              FEffLengthRnd - EndPos - 1, @Data[SampleCount - 1]);
          end
          else
          begin
            Exit;
            ...
            if EndPos < i then
            begin
              DoWinLoopTriangle32Symmetric(@Data[StartPos], ParamRec, EndPos,
                @Data[EndPos]);
              DoWinLoopTriangle32Symmetric(@Data[StartPos + EndPos], ParamRec,
                i - EndPos, @Data[SampleCount - 1]);
              DoWinLoopTriangle32Symmetric(@Data[0], ParamRec,
                FEffLengthRnd - i, @Data[SampleCount - (i - EndPos)]);
            end
            else
            begin
              DoWinLoopTriangle32Symmetric(@Data[StartPos], ParamRec, i,
                @Data[EndPos]);
              DoWinLoopTriangle32Symmetric(@Data[0], ParamRec, EndPos - i,
                @Data[EndPos - i]);
              DoWinLoopTriangle32Symmetric(@Data[EndPos - i], ParamRec,
                FEffLengthRnd - EndPos, @Data[0]);
            end;
          end;
        end;
        ParamRec.SpectrumCorrectionFactor := ParamRec.SpectrumCorrectionFactor +
          FLength - 2 * FEffLengthRnd;
        ParamRec.SpuaredCorrectionFactor := ParamRec.SpuaredCorrectionFactor +
          FLength - 2 * FEffLengthRnd;
      end;
  end;

  ParamRec.SpuaredCorrectionFactor :=
    Sqrt(FEffLength / (ParamRec.SpuaredCorrectionFactor));
  FillWithZeroes(Data, StartPos, EndPos, SampleCount);
...
procedure TWindowFunctionTriangle.ProcessBlock64(Data: PDAVDoubleFixedArray;
  SampleCount: Integer);
var
  StartPos, EndPos, i: Integer;
  ParamRec: TParameterRecord;
begin
  ...
  case FWinSlope of
    wsLeft:
      begin
        if StartPos + FEffLengthRnd <= SampleCount then
          DoWinLoopTriangle64Forward(@Data[StartPos], ParamRec, FEffLengthRnd)
        else // Wrap arround
        begin
          DoWinLoopTriangle64Forward(@Data[StartPos], ParamRec,
            SampleCount - StartPos);
          DoWinLoopTriangle64Forward(@Data[0], ParamRec,
            FEffLengthRnd - (SampleCount - StartPos));
        end;
        ParamRec.SpectrumCorrectionFactor := ParamRec.SpectrumCorrectionFactor +
          FLength - FEffLengthRnd;
    ...
    wsRight:
      begin
        if EndPos + 1 - FEffLengthRnd >= 0 then
          DoWinLoopTriangle64Forward(@Data[EndPos + 1 - FEffLengthRnd],
            ParamRec, FEffLengthRnd)
        else // Wrap arround
        begin
          DoWinLoopTriangle64Forward
            (@Data[(EndPos + 1 - FEffLengthRnd + SampleCount)], ParamRec,
            FEffLengthRnd - (EndPos + 1));
    ...
    wsSymmetric:
      begin
        if (StartPos + FEffLengthRnd < SampleCount) and
          (EndPos - FEffLengthRnd >= 0) then
          DoWinLoopTriangle64Symmetric(@Data[StartPos], ParamRec, FEffLengthRnd,
            @Data[EndPos]) // No wrap arround!
        else // Wrap around
        begin
          if (EndPos - FEffLengthRnd >= 0) then
          begin // Only left wrap around!
            i := SampleCount - StartPos;
            DoWinLoopTriangle64Symmetric(@Data[StartPos], ParamRec, i,
              @Data[EndPos]);
            DoWinLoopTriangle64Symmetric(@Data[0], ParamRec, FEffLengthRnd - i,
              @Data[EndPos - i]);
          end
          else if (StartPos + FEffLengthRnd <= SampleCount) then
          begin // Only right wrap around!
            DoWinLoopTriangle64Symmetric(@Data[StartPos], ParamRec, EndPos + 1,
              @Data[EndPos]);
            DoWinLoopTriangle64Symmetric(@Data[StartPos + EndPos + 1], ParamRec,
              FEffLengthRnd - EndPos - 1, @Data[SampleCount - 1]);
          end
          else
          begin
            ...
            if EndPos < i then
            begin
              DoWinLoopTriangle64Symmetric(@Data[StartPos], ParamRec, EndPos,
                @Data[EndPos]);
              DoWinLoopTriangle64Symmetric(@Data[StartPos + EndPos], ParamRec,
                i - EndPos, @Data[SampleCount - 1]);
              DoWinLoopTriangle64Symmetric(@Data[0], ParamRec,
                FEffLengthRnd - i, @Data[SampleCount - (i - EndPos)]);
            end
            else
            begin
              DoWinLoopTriangle64Symmetric(@Data[StartPos], ParamRec, i,
                @Data[EndPos]);
              DoWinLoopTriangle64Symmetric(@Data[0], ParamRec, EndPos - i,
                @Data[EndPos - i]);
              DoWinLoopTriangle64Symmetric(@Data[EndPos - i], ParamRec,
                FEffLengthRnd - EndPos, @Data[0]);
            end;
          end;
        end;
...
procedure TWindowFunctionCosine.ProcessBlock32(Data: PDAVSingleFixedArray;
  SampleCount: Integer);
var
  StartPos: Integer;
  EndPos, i: Integer;
...
begin
  ...
  if FInvert then
    StartPos := (FStart + Length - 1) - FEffLengthRnd
  else
    StartPos := FStart;
  ...
  begin
    ...
    if FBartlett then
      case FWinSlope of
        wsLeft:
          begin
            ComplexPosition.Im := -ComplexAngle.Re;
            ComplexPosition.Re := ComplexAngle.Im;
          end;
        wsRight:
          begin
            ComplexPosition.Im := 0;
            ComplexPosition.Re := 1;
          end;
        wsSymmetric:
          begin
    ...
    else if FWinSlope = wsRight then
    begin
      ComplexPosition.Im := 0;
      ComplexPosition.Re := 1;
    end
    ...
    begin
      ComplexPosition.Im := -1;
      ComplexPosition.Re := 0;
    end;
  end;
  ...
  case FWinSlope of
    wsLeft:
      begin
        if StartPos + FEffLengthRnd <= SampleCount then
          DoWinLoopCosine32Forward(@Data[StartPos], ParamRec, FEffLengthRnd)
        else // Wrap arround
        begin
          DoWinLoopCosine32Forward(@Data[StartPos], ParamRec,
            SampleCount - StartPos);
          DoWinLoopCosine32Forward(@Data[0], ParamRec,
            FEffLengthRnd - (SampleCount - StartPos));
        end;
        ParamRec.SpectrumCorrectionFactor := ParamRec.SpectrumCorrectionFactor +
          FLength - FEffLengthRnd;
    ...
    wsRight:
      begin
        if EndPos + 1 - FEffLengthRnd >= 0 then
          DoWinLoopCosine32Forward(@Data[EndPos + 1 - FEffLengthRnd], ParamRec,
            FEffLengthRnd)
        else // Wrap arround
        begin
          DoWinLoopCosine32Forward
            (@Data[(EndPos + 1 - FEffLengthRnd + SampleCount)], ParamRec,
            FEffLengthRnd - (EndPos + 1));
    ...
    wsSymmetric:
      begin
        if (StartPos + FEffLengthRnd < SampleCount) and
          (EndPos - FEffLengthRnd >= 0) then
          DoWinLoopCosine32Symmetric(@Data[StartPos], ParamRec, FEffLengthRnd,
            @Data[EndPos]) // No wrap arround!
        else // Wrap around
        begin
          if (EndPos - FEffLengthRnd >= 0) then
          begin // Only left wrap around!
            i := SampleCount - StartPos;
            DoWinLoopCosine32Symmetric(@Data[StartPos], ParamRec, i,
              @Data[EndPos]);
            DoWinLoopCosine32Symmetric(@Data[0], ParamRec, FEffLengthRnd - i,
              @Data[EndPos - i]);
          end
          else if (StartPos + FEffLengthRnd <= SampleCount) then
          begin // Only right wrap around!
            DoWinLoopCosine32Symmetric(@Data[StartPos], ParamRec, EndPos + 1,
              @Data[EndPos]);
            DoWinLoopCosine32Symmetric(@Data[StartPos + EndPos + 1], ParamRec,
              FEffLengthRnd - EndPos - 1, @Data[SampleCount - 1]);
          end
          else
          begin
            ...
            if EndPos < i then
            begin
              DoWinLoopCosine32Symmetric(@Data[StartPos], ParamRec, EndPos,
                @Data[EndPos]);
              DoWinLoopCosine32Symmetric(@Data[StartPos + EndPos], ParamRec,
                i - EndPos, @Data[SampleCount - 1]);
              DoWinLoopCosine32Symmetric(@Data[0], ParamRec, FEffLengthRnd - i,
                @Data[SampleCount - (i - EndPos)]);
            end
            else
            begin
              DoWinLoopCosine32Symmetric(@Data[StartPos], ParamRec, i,
                @Data[EndPos]);
              DoWinLoopCosine32Symmetric(@Data[0], ParamRec, EndPos - i,
                @Data[EndPos - i]);
              DoWinLoopCosine32Symmetric(@Data[EndPos - i], ParamRec,
                FEffLengthRnd - EndPos, @Data[0]);
            end;
          end;
        end;
...
procedure TWindowFunctionCosine.ProcessBlock64(Data: PDAVDoubleFixedArray;
  SampleCount: Integer);
var
  StartPos: Integer;
  EndPos, i: Integer;
...
begin
  ...
  case FWinSlope of
    wsLeft:
      begin
        if StartPos + FEffLengthRnd <= SampleCount then
          DoWinLoopCosine64Forward(@Data[StartPos], ParamRec, FEffLengthRnd)
        else // Wrap arround
        begin
          DoWinLoopCosine64Forward(@Data[StartPos], ParamRec,
            SampleCount - StartPos);
          DoWinLoopCosine64Forward(@Data[0], ParamRec,
            FEffLengthRnd - (SampleCount - StartPos));
        end;
        ParamRec.SpectrumCorrectionFactor := ParamRec.SpectrumCorrectionFactor +
          FLength - FEffLengthRnd;
    ...
    wsRight:
      begin
        if EndPos + 1 - FEffLengthRnd >= 0 then
          DoWinLoopCosine64Forward(@Data[EndPos + 1 - FEffLengthRnd], ParamRec,
            FEffLengthRnd)
        else // Wrap arround
        begin
          DoWinLoopCosine64Forward
            (@Data[(EndPos + 1 - FEffLengthRnd + SampleCount)], ParamRec,
            FEffLengthRnd - (EndPos + 1));
    ...
    wsSymmetric:
      begin
        if (StartPos + FEffLengthRnd < SampleCount) and
          (EndPos - FEffLengthRnd >= 0) then
          DoWinLoopCosine64Symmetric(@Data[StartPos], ParamRec, FEffLengthRnd,
            @Data[EndPos]) // No wrap arround!
        else // Wrap around
        begin
          if (EndPos - FEffLengthRnd >= 0) then
          begin // Only left wrap around!
            i := SampleCount - StartPos;
            DoWinLoopCosine64Symmetric(@Data[StartPos], ParamRec, i,
              @Data[EndPos]);
            DoWinLoopCosine64Symmetric(@Data[0], ParamRec, FEffLengthRnd - i,
              @Data[EndPos - i]);
          end
          else if (StartPos + FEffLengthRnd <= SampleCount) then
          begin // Only right wrap around!
            DoWinLoopCosine64Symmetric(@Data[StartPos], ParamRec, EndPos + 1,
              @Data[EndPos]);
            DoWinLoopCosine64Symmetric(@Data[StartPos + EndPos + 1], ParamRec,
              FEffLengthRnd - EndPos - 1, @Data[SampleCount - 1]);
          end
          else
          begin
            ...
            if EndPos < i then
            begin
              DoWinLoopCosine64Symmetric(@Data[StartPos], ParamRec, EndPos,
                @Data[EndPos]);
              DoWinLoopCosine64Symmetric(@Data[StartPos + EndPos], ParamRec,
                i - EndPos, @Data[SampleCount - 1]);
              DoWinLoopCosine64Symmetric(@Data[0], ParamRec, FEffLengthRnd - i,
                @Data[SampleCount - (i - EndPos)]);
            end
            ...
            begin
              DoWinLoopCosine64Symmetric(@Data[StartPos], ParamRec, i,
                @Data[EndPos]);
              DoWinLoopCosine64Symmetric(@Data[0], ParamRec, EndPos - i,
                @Data[EndPos - i]);
              DoWinLoopCosine64Symmetric(@Data[EndPos - i], ParamRec,
                FEffLengthRnd - EndPos, @Data[0]);
            end;
          end;
        end;
...
begin
  if (1 - Pos) = 0 then
    ...
    Result := Power(Sin(PI * Pos) / (PI * (1 - Pos)), FAlpha);
end;
...
procedure TWindowFunctionCosineTerm.ProcessBlock32(Data: PDAVSingleFixedArray;
  SampleCount: Integer);
var
  StartPos: Integer;
  EndPos, i: Integer;
...
begin
  ...
  case FWinSlope of
    wsLeft:
      begin
        if StartPos + FEffLengthRnd <= SampleCount then
          FCosFwdLoop32(@Data[StartPos], ParamRec, FEffLengthRnd)
        else // Wrap arround
        begin
          FCosFwdLoop32(@Data[StartPos], ParamRec, SampleCount - StartPos);
          FCosFwdLoop32(@Data[0], ParamRec,
            FEffLengthRnd - (SampleCount - StartPos));
        end;
        ParamRec.SpectrumCorrectionFactor := ParamRec.SpectrumCorrectionFactor +
          FLength - FEffLengthRnd;
    ...
    wsRight:
      begin
        if EndPos + 1 - FEffLengthRnd >= 0 then
          FCosFwdLoop32(@Data[EndPos + 1 - FEffLengthRnd], ParamRec,
            FEffLengthRnd)
        else // Wrap arround
        begin
          FCosFwdLoop32(@Data[(EndPos + 1 - FEffLengthRnd + SampleCount)],
            ParamRec, FEffLengthRnd - (EndPos + 1));
    ...
    wsSymmetric:
      begin
        if (StartPos + FEffLengthRnd < SampleCount) and
          (EndPos - FEffLengthRnd >= 0) then
          FCosSymLoop32(@Data[StartPos], ParamRec, FEffLengthRnd, @Data[EndPos])
          // No wrap arround!
        else // Wrap around
        begin
          if (EndPos - FEffLengthRnd >= 0) then
          begin // Only left wrap around!
            ...
            FCosSymLoop32(@Data[0], ParamRec, FEffLengthRnd - i,
              @Data[EndPos - i]);
          end
          else if (StartPos + FEffLengthRnd <= SampleCount) then
          begin // Only right wrap around!
            FCosSymLoop32(@Data[StartPos], ParamRec, EndPos + 1, @Data[EndPos]);
            FCosSymLoop32(@Data[StartPos + EndPos + 1], ParamRec,
              FEffLengthRnd - EndPos - 1, @Data[SampleCount - 1]);
          end
          else
          begin
            ...
            begin
              ...
              FCosSymLoop32(@Data[StartPos + EndPos], ParamRec, i - EndPos,
                @Data[SampleCount - 1]);
              FCosSymLoop32(@Data[0], ParamRec, FEffLengthRnd - i,
                @Data[SampleCount - (i - EndPos)]);
            end
            ...
            begin
              ...
              FCosSymLoop32(@Data[EndPos - i], ParamRec, FEffLengthRnd - EndPos,
                @Data[0]);
            end;
          end;
        end;
...
procedure TWindowFunctionCosineTerm.ProcessBlock64(Data: PDAVDoubleFixedArray;
  SampleCount: Integer);
var
  StartPos: Integer;
  EndPos, i: Integer;
...
begin
  ...
  case FWinSlope of
    wsLeft:
      begin
        if StartPos + FEffLengthRnd <= SampleCount then
          FCosFwdLoop64(@Data[StartPos], ParamRec, FEffLengthRnd)
        else // Wrap arround
        begin
          FCosFwdLoop64(@Data[StartPos], ParamRec, SampleCount - StartPos);
          FCosFwdLoop64(@Data[0], ParamRec,
            FEffLengthRnd - (SampleCount - StartPos));
        end;
        ParamRec.SpectrumCorrectionFactor := ParamRec.SpectrumCorrectionFactor +
          FLength - FEffLengthRnd;
    ...
    wsRight:
      begin
        if EndPos + 1 - FEffLengthRnd >= 0 then
          FCosFwdLoop64(@Data[EndPos + 1 - FEffLengthRnd], ParamRec,
            FEffLengthRnd)
        else // Wrap arround
        begin
          FCosFwdLoop64(@Data[(EndPos + 1 - FEffLengthRnd + SampleCount)],
            ParamRec, FEffLengthRnd - (EndPos + 1));
    ...
    wsSymmetric:
      begin
        if (StartPos + FEffLengthRnd < SampleCount) and
          (EndPos - FEffLengthRnd >= 0) then
          FCosSymLoop64(@Data[StartPos], ParamRec, FEffLengthRnd, @Data[EndPos])
          // No wrap arround!
        else // Wrap around
        begin
          if (EndPos - FEffLengthRnd >= 0) then
          begin // Only left wrap around!
            ...
            FCosSymLoop64(@Data[0], ParamRec, FEffLengthRnd - i,
              @Data[EndPos - i]);
          end
          else if (StartPos + FEffLengthRnd <= SampleCount) then
          begin // Only right wrap around!
            FCosSymLoop64(@Data[StartPos], ParamRec, EndPos + 1, @Data[EndPos]);
            FCosSymLoop64(@Data[StartPos + EndPos + 1], ParamRec,
              FEffLengthRnd - EndPos - 1, @Data[SampleCount - 1]);
          end
          else
          begin
            ...
            begin
              ...
              FCosSymLoop64(@Data[StartPos + EndPos], ParamRec, i - EndPos,
                @Data[SampleCount - 1]);
              FCosSymLoop64(@Data[0], ParamRec, FEffLengthRnd - i,
                @Data[SampleCount - (i - EndPos)]);
            end
            ...
            begin
              ...
              FCosSymLoop64(@Data[EndPos - i], ParamRec, FEffLengthRnd - EndPos,
                @Data[0]);
            end;
          end;
        end;
  ...
  if ParamRec.SpuaredCorrectionFactor <> 0 then
    ParamRec.SpuaredCorrectionFactor :=
      Sqrt(FEffLength / (ParamRec.SpuaredCorrectionFactor));
  FillWithZeroes(Data, StartPos, EndPos, SampleCount);
...
procedure TWindowFunctionHanning.ProcessBlock32(Data: PDAVSingleFixedArray;
  SampleCount: Integer);
var
  StartPos: Integer;
  EndPos, i: Integer;
...
begin
  ...
  case FWinSlope of
    wsLeft:
      begin
        if StartPos + FEffLengthRnd <= SampleCount then
          DoWinLoopHanning32Forward(@Data[StartPos], ParamRec, FEffLengthRnd)
        else // Wrap arround
        begin
          DoWinLoopHanning32Forward(@Data[StartPos], ParamRec,
            SampleCount - StartPos);
          DoWinLoopHanning32Forward(@Data[0], ParamRec,
            FEffLengthRnd - SampleCount + StartPos);
        end;
        ParamRec.SpectrumCorrectionFactor := ParamRec.SpectrumCorrectionFactor +
          FLength - FEffLengthRnd;
    ...
    wsRight:
      begin
        if EndPos + 1 - FEffLengthRnd >= 0 then
          DoWinLoopHanning32Forward(@Data[EndPos + 1 - FEffLengthRnd], ParamRec,
            FEffLengthRnd)
        else // Wrap arround
        begin
          DoWinLoopHanning32Forward
            (@Data[(EndPos + 1 - FEffLengthRnd + SampleCount)], ParamRec,
            FEffLengthRnd - (EndPos + 1));
    ...
    wsSymmetric:
      begin
        if (StartPos + FEffLengthRnd < SampleCount) and
          (EndPos - FEffLengthRnd >= 0) then
          DoWinLoopHanning32Symmetric(@Data[StartPos], ParamRec, FEffLengthRnd,
            @Data[EndPos]) // No wrap arround!
        else // Wrap around
        begin
          if (EndPos - FEffLengthRnd >= 0) then
          begin // Only left wrap around!
            i := SampleCount - StartPos;
            DoWinLoopHanning32Symmetric(@Data[StartPos], ParamRec, i,
              @Data[EndPos]);
            DoWinLoopHanning32Symmetric(@Data[0], ParamRec, FEffLengthRnd - i,
              @Data[EndPos - i]);
          end
          else if (StartPos + FEffLengthRnd <= SampleCount) then
          begin // Only right wrap around!
            DoWinLoopHanning32Symmetric(@Data[StartPos], ParamRec, EndPos + 1,
              @Data[EndPos]);
            DoWinLoopHanning32Symmetric(@Data[StartPos + EndPos + 1], ParamRec,
              FEffLengthRnd - EndPos - 1, @Data[SampleCount - 1]);
          end
          else
          begin
            ...
            if EndPos < i then
            begin
              DoWinLoopHanning32Symmetric(@Data[StartPos], ParamRec, EndPos,
                @Data[EndPos]);
              DoWinLoopHanning32Symmetric(@Data[StartPos + EndPos], ParamRec,
                i - EndPos, @Data[SampleCount - 1]);
              DoWinLoopHanning32Symmetric(@Data[0], ParamRec, FEffLengthRnd - i,
                @Data[SampleCount - (i - EndPos)]);
            end
            ...
            begin
              DoWinLoopHanning32Symmetric(@Data[StartPos], ParamRec, i,
                @Data[EndPos]);
              DoWinLoopHanning32Symmetric(@Data[0], ParamRec, EndPos - i,
                @Data[EndPos - i]);
              DoWinLoopHanning32Symmetric(@Data[EndPos - i], ParamRec,
                FEffLengthRnd - EndPos, @Data[0]);
            end;
          end;
        end;
...
procedure TWindowFunctionHanning.ProcessBlock64(Data: PDAVDoubleFixedArray;
  SampleCount: Integer);
var
  StartPos: Integer;
  EndPos, i: Integer;
...
begin
  ...
  case FWinSlope of
    wsLeft:
      begin
        if StartPos + FEffLengthRnd <= SampleCount then
          DoWinLoopHanning64Forward(@Data[StartPos], ParamRec, FEffLengthRnd)
        else // Wrap arround
        begin
          DoWinLoopHanning64Forward(@Data[StartPos], ParamRec,
            SampleCount - StartPos);
          DoWinLoopHanning64Forward(@Data[0], ParamRec,
            FEffLengthRnd - (SampleCount - StartPos));
        end;
        ParamRec.SpectrumCorrectionFactor := ParamRec.SpectrumCorrectionFactor +
          FLength - FEffLengthRnd;
    ...
    wsRight:
      begin
        if EndPos + 1 - FEffLengthRnd >= 0 then
          DoWinLoopHanning64Forward(@Data[EndPos + 1 - FEffLengthRnd], ParamRec,
            FEffLengthRnd)
        else // Wrap arround
        begin
          DoWinLoopHanning64Forward
            (@Data[EndPos + 1 - FEffLengthRnd + SampleCount], ParamRec,
            FEffLengthRnd - EndPos - 1);
    ...
    wsSymmetric:
      begin
        if (StartPos + FEffLengthRnd < SampleCount) and
          (EndPos - FEffLengthRnd >= 0) then
          DoWinLoopHanning64Symmetric(@Data[StartPos], ParamRec, FEffLengthRnd,
            @Data[EndPos]) // No wrap arround!
        else // Wrap around
        begin
          if (EndPos - FEffLengthRnd >= 0) then
          begin // Only left wrap around!
            i := SampleCount - StartPos;
            DoWinLoopHanning64Symmetric(@Data[StartPos], ParamRec, i,
              @Data[EndPos]);
            DoWinLoopHanning64Symmetric(@Data[0], ParamRec, FEffLengthRnd - i,
              @Data[EndPos - i]);
          end
          else if (StartPos + FEffLengthRnd <= SampleCount) then
          begin // Only right wrap around!
            DoWinLoopHanning64Symmetric(@Data[StartPos], ParamRec, EndPos + 1,
              @Data[EndPos]);
            DoWinLoopHanning64Symmetric(@Data[StartPos + EndPos + 1], ParamRec,
              FEffLengthRnd - EndPos - 1, @Data[SampleCount - 1]);
          end
          else
          begin
            ...
            if EndPos < i then
            begin
              DoWinLoopHanning64Symmetric(@Data[StartPos], ParamRec, EndPos,
                @Data[EndPos]);
              DoWinLoopHanning64Symmetric(@Data[StartPos + EndPos], ParamRec,
                i - EndPos, @Data[SampleCount - 1]);
              DoWinLoopHanning64Symmetric(@Data[0], ParamRec, FEffLengthRnd - i,
                @Data[SampleCount - (i - EndPos)]);
            end
            ...
            begin
              DoWinLoopHanning64Symmetric(@Data[StartPos], ParamRec, i,
                @Data[EndPos]);
              DoWinLoopHanning64Symmetric(@Data[0], ParamRec, EndPos - i,
                @Data[EndPos - i]);
              DoWinLoopHanning64Symmetric(@Data[EndPos - i], ParamRec,
                FEffLengthRnd - EndPos, @Data[0]);
            end;
          end;
        end;
...
procedure TWindowFunctionWelch.ProcessBlock32(Data: PDAVSingleFixedArray;
  SampleCount: Integer);
var
  i, p1, p2: Integer;
...
begin
  ...
  StartPos := FStart;
  while StartPos >= SampleCount do
    StartPos := StartPos - SampleCount;
  while StartPos < 0 do
    StartPos := StartPos + SampleCount;

  EndPos := (FStart + Length - 1);
  while EndPos >= SampleCount do
    EndPos := EndPos - SampleCount;
  while EndPos < 0 do
    EndPos := EndPos + SampleCount;
  ...
  case FWinSlope of
    wsLeft:
      begin
        if StartPos + FEffLengthRnd <= SampleCount then
          DoWinLoopForward(StartPos, StartPos + FEffLengthRnd)
        else // Wrap arround
        begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            for i := 0 to FEffLengthRnd - 1 do
            begin
              CurWinVal := Counter * FEffLengthReci *
                (2 - Counter * FEffLengthReci);
              Inc(Counter);
    ...
    wsRight:
      begin
        if EndPos - FEffLengthRnd >= 0 then
          DoWinLoopBackward(EndPos, EndPos - FEffLengthRnd)
        else // Wrap arround
        begin
          DoWinLoopBackward(EndPos, 0);
          DoWinLoopBackward(SampleCount,
            SampleCount - (FEffLengthRnd - EndPos));
        end;
        FSpkCorFak := FSpkCorFak + FLength - FEffLengthRnd;
        FSpkCorFakSq := FSpkCorFakSq + FLength - FEffLengthRnd;
      end;
...
procedure TWindowFunctionWelch.ProcessBlock64(Data: PDAVDoubleFixedArray;
  SampleCount: Integer);
var
  i, p1, p2: Integer;
  StartPos: Integer;
...
begin
  ...
  for i := 0 to Length(GWindowFunctions) - 1 do
    if GWindowFunctions[i] = AClass then
      raise Exception.Create(RCStrWindowDuplicate);
...
begin
  for i := 0 to Length(AClasses) - 1 do
    RegisterWindowFunction(AClasses[i]);
end;

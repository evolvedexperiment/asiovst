...
type
  ...
  public
    ...
    function ReadBuffer(const Data: PDAVSingleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
    function WriteBuffer(const Data: PDAVSingleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
  end;
  ...
  public
    ...
    destructor Destroy; override;
    procedure Reset; override;
    function ReadBuffer(const Data: PDAVDoubleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
    function WriteBuffer(const Data: PDAVDoubleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
  end;
  ...
  public
    ...
    destructor Destroy; override;
    procedure Reset; override;
    function ReadBuffer(const Left, Right: PDAVSingleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
    function WriteBuffer(const Left, Right: PDAVSingleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
  end;
  ...
  public
    ...
    destructor Destroy; override;
    procedure Reset; override;
    function ReadBuffer(const Left, Right: PDAVDoubleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
    function WriteBuffer(const Left, Right: PDAVDoubleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
  end;
  ...
  public
    ...
    property ChannelCount: Integer read FChannelCount write SetChannelCount
      default 1;
    property OnChannelCountChanged: TNotifyEvent read FOnChannelCountChanged
      write FOnChannelCountChanged;
  end;
  ...
  public
    ...
    function ReadBuffer(const Data: TDAVArrayOfSingleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
    function WriteBuffer(const Data: TDAVArrayOfSingleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
  end;
  ...
  public
    ...
    function ReadBuffer(const Data: TDAVArrayOfDoubleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
    function WriteBuffer(const Data: TDAVArrayOfDoubleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
  end;
  ...
  public
    ...
    function WriteBuffer(const Data: TDAVArrayOfSingleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
    function ReadReserveBuffer(const Data: TDAVArrayOfSingleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
    function WriteReserveBuffer(const Data: TDAVArrayOfSingleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
  end;
  ...
  public
    ...
    function WriteBuffer(const Data: TDAVArrayOfDoubleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
    function ReadReserveBuffer(const Data: TDAVArrayOfDoubleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
    function WriteReserveBuffer(const Data: TDAVArrayOfDoubleFixedArray;
      const SampleFrames: Cardinal): Cardinal;
  end;
...
begin
  ...
  else if Dest is TCustomCircularBuffer64 then
    with TCustomCircularBuffer64(Dest) do
    begin
      inherited;
      Assert(FBufferSize = Self.FBufferSize);
      for Sample := 0 to FBufferSize - 1 do
        Self.FBuffer^[Sample] := FBuffer^[Sample];
    end
  else
    inherited;
end;
...
function TCustomCircularBuffer32.ReadBuffer(const Data: PDAVSingleFixedArray;
  const SampleFrames: Cardinal): Cardinal;
var
  PartialSamples: Cardinal;
begin
  if SampleFrames < SamplesInBuffer then
    Result := SampleFrames
  else
  ...
  begin
    ...
    Move(FBuffer^[0], Data^[PartialSamples], (Result - PartialSamples) *
      SizeOf(Single));
    FReadBufferPos := (Result - PartialSamples);

    if FReadBufferPos >= FBufferSize then
      FReadBufferPos := FReadBufferPos - FBufferSize;
  end
  else
  begin
...
function TCustomCircularBuffer32.WriteBuffer(const Data: PDAVSingleFixedArray;
  const SampleFrames: Cardinal): Cardinal;
var
  PartialSamples: Cardinal;
begin
  if SampleFrames < FreeSampleCount then
    Result := SampleFrames
  else
    Result := FreeSampleCount;
  ...
  begin
    ...
    Move(Data^[PartialSamples], FBuffer^[PartialSamples],
      (Result - PartialSamples) * SizeOf(Single));
    FWriteBufferPos := (Result - PartialSamples);

    if FWriteBufferPos >= FBufferSize then
      FWriteBufferPos := FWriteBufferPos - FBufferSize;
  end
  else
  begin
...
function TCustomCircularBuffer64.ReadBuffer(const Data: PDAVDoubleFixedArray;
  const SampleFrames: Cardinal): Cardinal;
var
  PartialSamples: Cardinal;
begin
...
function TCustomCircularBuffer64.WriteBuffer(const Data: PDAVDoubleFixedArray;
  const SampleFrames: Cardinal): Cardinal;
var
  PartialSamples: Cardinal;
begin
  ...
  begin
    ...
    Move(Data^[PartialSamples], FBuffer^[PartialSamples],
      (Result - PartialSamples) * SizeOf(Double));
...
begin
  ...
  else if Dest is TCustomCircularStereoBuffer32 then
  ...
  else if Dest is TCustomCircularStereoBuffer64 then
...
begin
  ...
  begin
    PartialSamples := FBufferSize - FReadBufferPos;
    Move(FBuffer[0]^[FReadBufferPos], Left^[0],
      PartialSamples * SizeOf(Single));
    Move(FBuffer[1]^[FReadBufferPos], Right^[0],
      PartialSamples * SizeOf(Single));
    Move(FBuffer[0]^[0], Left^[PartialSamples], (Result - PartialSamples) *
      SizeOf(Single));
    Move(FBuffer[1]^[0], Right^[PartialSamples], (Result - PartialSamples) *
      SizeOf(Single));
    FReadBufferPos := (Result - PartialSamples);
...
begin
  ...
  begin
    PartialSamples := FBufferSize - FWriteBufferPos;
    Move(Left^[0], FBuffer[0]^[FWriteBufferPos],
      PartialSamples * SizeOf(Single));
    Move(Right^[0], FBuffer[1]^[FWriteBufferPos],
      PartialSamples * SizeOf(Single));
    Move(Left^[PartialSamples], FBuffer[0]^[PartialSamples],
      (Result - PartialSamples) * SizeOf(Single));
    Move(Right^[PartialSamples], FBuffer[1]^[PartialSamples],
      (Result - PartialSamples) * SizeOf(Single));
    FWriteBufferPos := (Result - PartialSamples);
...
begin
  if Assigned(FOnChannelCountChanged) then
    FOnChannelCountChanged(Self);
end;
...
begin
  ...
  begin
    for Channel := Length(FBuffer) - 1 downto ChannelCount do
      Dispose(FBuffer[Channel]);
    SetLength(FBuffer, ChannelCount);
  end
  else if ChannelCount > Length(FBuffer) then
  begin
    OldChannelCount := Length(FBuffer);
    SetLength(FBuffer, ChannelCount);
    for Channel := OldChannelCount to ChannelCount - 1 do
      AllocateChannelData(Channel);
  end;
  inherited;
end;
...
begin
  if Dest is TCustomCircularBuffer32 then
    ...
    begin
      ...
      Assert(FBufferSize = Self.FBufferSize);
      for Channel := 0 to FChannelCount - 1 do
        Move(FBuffer[Channel mod 2]^, Self.FBuffer[Channel]^,
          Self.FBufferSize * SizeOf(Single));
    end
  else if Dest is TCustomCircularMultiBuffer32 then
    with TCustomCircularMultiBuffer32(Dest) do
    begin
      inherited;
      Assert(FBufferSize = Self.FBufferSize);
      for Channel := 0 to FChannelCount - 1 do
        Move(FBuffer[Channel]^, Self.FBuffer[Channel]^,
          Self.FBufferSize * SizeOf(Single));
    end
  else if Dest is TCustomCircularBuffer64 then
    with TCustomCircularBuffer64(Dest) do
    begin
      ...
      for Channel := 0 to FChannelCount - 1 do
        for Sample := 0 to FBufferSize - 1 do
          Self.FBuffer[Channel]^[Sample] := FBuffer^[Sample];
    end
  else if Dest is TCustomCircularStereoBuffer64 then
    with TCustomCircularStereoBuffer64(Dest) do
    begin
      ...
      for Channel := 0 to FChannelCount - 1 do
        for Sample := 0 to FBufferSize - 1 do
          Self.FBuffer[Channel]^[Sample] := FBuffer[Channel mod 2]^[Sample];
    end
  else if Dest is TCustomCircularMultiBuffer64 then
    ...
    begin
      ...
      for Channel := 0 to FChannelCount - 1 do
        for Sample := 0 to FBufferSize - 1 do
          Self.FBuffer[Channel]^[Sample] := FBuffer[Channel]^[Sample];
    end
  else
    inherited;
end;
...
begin
  ...
  begin
    ...
    begin
      Move(FBuffer[Channel]^[FReadBufferPos], Data[Channel]^[0],
        PartialSamples * SizeOf(Single));
      Move(FBuffer[Channel]^[0], Data[Channel]^[PartialSamples],
        (Result - PartialSamples) * SizeOf(Single));
    end;
...
begin
  ...
  begin
    ...
    for Channel := 0 to Min(Length(Data), ChannelCount) - 1 do
    begin
      Move(Data[Channel]^[0], FBuffer[Channel]^[FWriteBufferPos],
        PartialSamples * SizeOf(Single));
      Move(Data[Channel]^[PartialSamples], FBuffer[Channel]^[PartialSamples],
        (Result - PartialSamples) * SizeOf(Single));
    end;
    FWriteBufferPos := (Result - PartialSamples);
...
begin
  if Dest is TCustomCircularBuffer32 then
    ...
    begin
      ...
      for Channel := 0 to FChannelCount - 1 do
        Move(FBuffer[Channel mod 2]^, Self.FBuffer[Channel]^,
          Self.FBufferSize * SizeOf(Double));
    end
  else if Dest is TCustomCircularMultiBuffer64 then
    with TCustomCircularMultiBuffer64(Dest) do
    begin
...
function TCustomCircularMultiBuffer64.ReadBuffer
  (const Data: TDAVArrayOfDoubleFixedArray; const SampleFrames: Cardinal)
  : Cardinal;
var
  Channel: Integer;
  PartialSamples: Cardinal;
...
begin
  if Value >= Cardinal(FBufferSize) then
    raise Exception.Create(RcStrLatencyTooHigh);
...
begin
  ...
  begin
    if FReadBufferPos < FLatency then
      FWriteBufferPos := FWriteBufferPos + FBufferSize - FLatency
    else
...
constructor TCustomCircularReserveMultiBuffer32.Create(const BufferSize
  : Integer);
begin
  inherited Create(BufferSize);
end;
...
constructor TCustomCircularReserveMultiBuffer.Create(const BufferSize: Integer);
begin
  FLatency := 0;
  inherited;
end;
...
begin
  ...
  begin
    ...
    for Channel := 0 to ChannelCount - 1 do
    begin
      Move(Data[Channel]^[0], FBuffer[0]^[FWriteBufferPos],
        PartialSamples * SizeOf(Single));
      Move(Data[Channel]^[PartialSamples], FBuffer[1]^[PartialSamples],
        (Result - PartialSamples) * SizeOf(Single));
    end;
    FWriteBufferPos := (Result - PartialSamples);
...
constructor TCustomCircularReserveMultiBuffer64.Create(const BufferSize
  : Integer);
begin
  inherited Create(BufferSize);
end;
...
begin
  ...
  else if Dest is TCustomCircularReserveMultiBuffer32 then
  ...
  else if Dest is TCustomCircularReserveMultiBuffer64 then

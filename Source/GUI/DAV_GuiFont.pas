...
{$ENDIF}
  ...
  public
    ...
    property FontOversampling: TFontOversampling read FFontOversampling
      write SetFontOversampling;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ScnLn[0, X] := (ScnLn[1, 2 * X] + ScnLn[2, 2 * X] + ScnLn[1, 2 * X + 1]
          + ScnLn[2, 2 * X + 1]) div 4;
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        x3 := 3 * X;
        ScnLn[0, X] := (ScnLn[1, x3] + ScnLn[2, x3] + ScnLn[3, x3] + ScnLn[1,
          x3 + 1] + ScnLn[2, x3 + 1] + ScnLn[3, x3 + 1] + ScnLn[1, x3 + 2] +
          ScnLn[2, x3 + 2] + ScnLn[3, x3 + 2]) div 9;
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        x4 := 4 * X;
        ScnLn[0, X] := (ScnLn[1, x4] + ScnLn[1, x4 + 1] + ScnLn[1, x4 + 2] +
          ScnLn[1, x4 + 3] + ScnLn[2, x4] + ScnLn[2, x4 + 1] + ScnLn[2, x4 + 2]
          + ScnLn[2, x4 + 3] + ScnLn[3, x4] + ScnLn[3, x4 + 1] + ScnLn[3,
          x4 + 2] + ScnLn[3, x4 + 3] + ScnLn[4, x4] + ScnLn[4, x4 + 1] +
          ScnLn[4, x4 + 2] + ScnLn[4, x4 + 3]) div 16;
      end;
    end;
  end;
end;
...
{$IFDEF UseShadowBuffer}
  if Assigned(FShadowBuffer) then
    FreeAndNil(FShadowBuffer);
{$ENDIF}
  inherited;
end;
...
begin
  ...
  if FAntiAliasing then
    SetFontAntialiasing(FFont, ANTIALIASED_QUALITY)
  else
    SetFontAntialiasing(FFont, NONANTIALIASED_QUALITY);
end;
...
begin
  ...
  if FBuffer.Handle <> 0 then
    GetTextExtentPoint32(FBuffer.Handle, PChar(Text), Length(Text), Result);
end;

procedure TGuiCustomSimpleGDIFont.TextOut(Text: string;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
{$IFDEF UseShadowBuffer}
        if not Assigned(FShadowBuffer) then
          FShadowBuffer := TGuiByteMapMemory.Create;
{$ENDIF}
...
{$IFDEF UseShadowBuffer}
        ...
        if PixelMap <> nil then
          PixelMap.DrawByteMap(FShadowBuffer, FShadowColor,
            X + FShadow.Offset.X - BlurOffset, Y + FShadow.Offset.Y -
            BlurOffset);
{$ELSE}
        if FShadow.Blur > 0 then
        begin
        ...
        if PixelMap <> nil then
          PixelMap.DrawByteMap(FBuffer, FShadowColor, X + FShadow.Offset.X -
            BlurOffset, Y + FShadow.Offset.Y - BlurOffset);

        FBuffer.Clear;

        // eventually turn bytemap back
        case FFontTurn of
          ftClockwise, ftCounterClockwise:
            FBuffer.SetSize(Height, Width);
        end;
...
{$ENDIF}
        if PixelMap <> nil then
          PixelMap.DrawByteMap(FBuffer, ConvertColor(Font.Color),
...
{$ENDIF}
        ...
        if PixelMap <> nil then
          PixelMap.DrawByteMap(FBuffer, ConvertColor(Font.Color), X, Y);
      end;
    end;
  end;
end;
...
procedure TGuiCustomOversampledGDIFont.DownsampleByteMap
  (Buffer: TGuiCustomByteMap);
begin
...
{$IFDEF UseShadowBuffer}
        ...
        begin
          ...
          if FShadow.Saturation <> 0 then
            FSaturation.Filter(FShadowBuffer);
        end;
        ...
        case FFontTurn of
          ftClockwise:
            FShadowBuffer.Turn;
          ftCounterClockwise:
            FShadowBuffer.Turn(True);
        end;

        if PixelMap <> nil then
          PixelMap.DrawByteMap(FShadowBuffer, FShadowColor,
            Rect(X + FShadow.Offset.X - BlurOffset div FOSFactor,
            Y + FShadow.Offset.Y - BlurOffset div FOSFactor,
            X + FShadow.Offset.X + (FShadowBuffer.Width - BlurOffset)
            div FOSFactor, Y + FShadow.Offset.Y + (FShadowBuffer.Height -
            BlurOffset) div FOSFactor));
{$ELSE}
        if FShadow.Blur > 0 then
        begin
          FBlurFilter.Radius := FOSFactor * FShadow.Blur;
        ...
        if PixelMap <> nil then
          PixelMap.DrawByteMap(FBuffer, FShadowColor,
            Rect(X + FShadow.Offset.X - BlurOffset div FOSFactor,
            Y + FShadow.Offset.Y - BlurOffset div FOSFactor,
            X + FShadow.Offset.X + (FBuffer.Width - BlurOffset) div FOSFactor,
            Y + FShadow.Offset.Y + (FBuffer.Height - BlurOffset)
            div FOSFactor));
...
begin
  if not Assigned(FontClassList) then
    FontClassList := TClassList.Create;

...
begin
  for HysteresisIndex := 0 to numInputs - 1 do
    FreeAndNil(FHysteresis[HysteresisIndex]);
end;

procedure THysteresisModule.ParameterInputChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FScaleFactor[0] := dB_to_Amp(Value);
end;

procedure THysteresisModule.ParameterOutputChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin

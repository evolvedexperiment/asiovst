...
procedure TCustomSEPlateReverbModule.SubProcessStatic(const BufferOffset,
...
class procedure TCustomSEPlateReverbModule.GetModuleProperties
...
function TCustomSEPlateReverbModule.GetPinProperties(const Index: Integer;
...
procedure TSEPlateReverbStaticModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
var
...
// describe the pins (plugs)
function TSEPlateReverbStaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
...
begin
  ...
  case TSEPlateVerbPins(CurrentPin.PinID) of
    pinPreDelay:
      FPlateReverb.PreDelay := 1E-3 * FPreDelay;
    pinDecay:
      FPlateReverb.Decay := 4E-3 * FDecay;
    pinDamping:
      FPlateReverb.DampingFrequency := 1E3 * FDamping;
    pinInputDiffusion:
      FPlateReverb.InputDiffusion := 1E-2 * FInputDiff;
    pinDecayDiffusion:
      FPlateReverb.DecayDiffusion := 1E-2 * FDecayDiff;
    pinModulation:
      FPlateReverb.Modulation := 1E-2 * FModulation;
  end;
end;
...
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSEPlateVerbPins(index) in [pinPreDelay .. pinEffectMix] then
    with Properties^ do
      Direction := drIn;
end;

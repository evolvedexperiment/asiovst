...
constructor TSESonogramGui.Create(SEGuiCallback: TSEGuiCallback;
  AHostPtr: Pointer);
begin
  inherited;
  ...
  if Assigned(FSonogram) and Assigned(CriticalSection) then
    ...
    begin
      ...
      try
        if Assigned(wi) then
        begin
          if Bitmap.Width <> wi.Width then
            Bitmap.Width := wi.Width;
          if Bitmap.Height <> wi.Height then
            Bitmap.Height := wi.Height;
        end;
  ...
  case CurrentPin.PinIndex of
    1:
      begin
        ...
        if not(FSonogram is TBitmapSonogram32) then
          FSonogram := nil
        else
          GUIDraw(nil);
      end;
...
procedure TSESonogramGui.GuiModuleMsg(AUserMsgID, ALength: Integer;
  AData: Pointer);
begin
  CallHost(seGuiHostRequestRepaint);
end;

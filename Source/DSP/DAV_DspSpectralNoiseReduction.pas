...
type
  ...
  public
    ...
    property WindowFunctionClass: TWindowFunctionClass read FWindowClass
  ...
  protected
    ...
    procedure PerformSpectralEffect(SignalIn, SignalOut: PDAVSingleFixedArray);
      overload; override;
    procedure PerformSpectralEffect(Spectrum: PDAVComplexSingleFixedArray);
      overload; override;
  ...
  protected
    ...
    FWindowFunction: TCustomWindowFunction;
    FWindowClass: TWindowFunctionClass;
    procedure BuildFilter(Spectrum: PDAVComplexDoubleFixedArray);
      virtual; abstract;
    procedure FFTOrderChanged; override;
    procedure WindowFunctionClassChanged; virtual;
  public
  ...
  protected
    ...
    procedure PerformSpectralEffect(SignalIn, SignalOut: PDAVDoubleFixedArray);
      overload; override;
    procedure PerformSpectralEffect(Spectrum: PDAVComplexDoubleFixedArray);
      overload; override;
...
{$ENDIF}
  ...
  if Assigned(FWindowFunction) then
    FWindowFunction.Length := FFTSize;
end;
...
procedure TCustomNoiseReduction32.SetWindowClass(const Value
  : TWindowFunctionClass);
begin
  Assert(Value <> nil);
...
procedure TSpectralNoiseCut32.BuildFilter
...
begin
  ...
  // DC bin
  if Abs(Spectrum^[0].Re) > FThreshold then
    FFilter^[0].Re := CHalf32 * (1 + FFilter^[0].Re)
  else
    FFilter^[0].Re := CHalf32 * FFilter^[0].Re;
{$IFDEF ComplexDataOrder}
  FFilter^[Half].Re := 0;
{$ENDIF}
  // other bins
  ...
  // Nyquist bin
{$IFDEF ComplexDataOrder}
  if Abs(Spectrum^[Half].Re) > FThreshold then
    FFilter^[Half].Re := CHalf32 * (1 + FFilter^[Half].Re)
  else
    FFilter^[Half].Re := CHalf32 * FFilter^[Half].Re;
  FFilter^[Half].Im := 0;
{$ELSE}
  if Abs(Spectrum^[0].Im) > FThreshold then
    FFilter^[0].Im := CHalf32 * (1 + FFilter^[0].Im)
  else
    FFilter^[0].Im := CHalf32 * FFilter^[0].Im;
{$ENDIF}
  // transform filter to time domain
  FFft.PerformIFFT(FFilter, FFilterIR);

  // shift filter impulse response and apply window function
  Move(FFilterIR^[0], FFilterIR^[FFFTSizeHalf div 2],
    FFFTSizeHalf div 2 * SizeOf(Single));
  Move(FFilterIR^[FFFTSize - FFFTSizeHalf div 2], FFilterIR^[0],
    FFFTSizeHalf div 2 * SizeOf(Single));
...
procedure TSpectralNoiseCut32.PerformSpectralEffect
  (Spectrum: PDAVComplexSingleFixedArray);
begin
{$IFDEF ComplexDataOrder}
...
begin
  ...
  for Bin := Fft.BinCount to Length(FGates) - 1 do
    if Assigned(FGates[Bin]) then
      FreeAndNil(FGates[Bin]);
...
{$ENDIF}
  ...
  for Bin := 1 to Half - 1 do
  begin
    FGates[Bin].InputSample(COffset + Sqr(Spectrum^[Bin].Re) +
      Sqr(Spectrum^[Bin].Im));
    FFilter^[Bin].Re := FGates[Bin].GainSample(1);
    if Abs(FFilter^[Bin].Re) > 1 then
      FFilter^[Bin].Re := 0;
    FFilter^[Bin].Im := 0;
  end;
...
procedure TSpectralNoiseGate32.PerformSpectralEffect
  (Spectrum: PDAVComplexSingleFixedArray);
begin
{$IFDEF ComplexDataOrder}
  ComplexMultiplyBlock32(Spectrum, FFilter, FFFTSizeHalf + 1);
...
begin
  for Bin := 0 to Length(FGates) - 1 do
    FGates[Bin].Threshold_dB := 2 * FThreshold;
end;
...
procedure TNoiseReduction32.MatchThreshold
  (Spectrum: PDAVComplexSingleFixedArray);
var
  Bin: Integer;
  Half: Integer;
...
begin
  ...
  // DC bin
  FThresholds[0] := Weight[0] * FThresholds[0] + Weight[1] *
    Sqr(FFilter^[0].Re);

  // other bins
  for Bin := 1 to Half - 1 do
    FThresholds[Bin] := Weight[0] * FThresholds[Bin] + Weight[1] *
      (Sqr(Spectrum^[Bin].Re) + Sqr(Spectrum^[Bin].Im));

  // Nyquist bin
  FThresholds[Half] := Weight[0] * FThresholds[Half] + Weight[1] *
    Sqr(Spectrum^[Half].Re);
...
begin
  for Bin := 0 to Length(FGates) - 1 do
    FGates[Bin].Threshold_dB := Amp_to_dB(CDenorm32 + FThresholds[Bin])
      + FOffset;
end;

procedure TNoiseReduction32.PerformSpectralEffect
  (Spectrum: PDAVComplexSingleFixedArray);
begin
{$IFDEF ComplexDataOrder}
  ComplexMultiplyBlock32(Spectrum, FFilter, FFFTSizeHalf + 1);
...
procedure TCustomNoiseReduction64.SetWindowClass(const Value
  : TWindowFunctionClass);
begin
  if FWindowClass <> Value then
  begin
...
procedure TSpectralNoiseCut64.BuildFilter
...
begin
  ...
  // DC bin
  if Abs(Spectrum^[0].Re) > FThreshold then
    FFilter^[0].Re := CHalf64 * (1 + FFilter^[0].Re)
  else
    FFilter^[0].Re := CHalf64 * FFilter^[0].Re;
{$IFDEF ComplexDataOrder}
  FFilter^[0].Im := 0;
{$ENDIF}
  // other bins
  ...
  begin
    if FastSqrtBab2(Sqr(Spectrum^[Bin].Re) + Sqr(Spectrum^[Bin].Im)) > FThreshold
    then
      FFilter^[Bin].Re := CHalf64 * (1 + FFilter^[Bin].Re)
    else
      FFilter^[Bin].Re := CHalf64 * FFilter^[Bin].Re;
    FFilter^[Bin].Im := 0;
  end;
...
{$IFDEF ComplexDataOrder}
  if Abs(Spectrum^[Half].Re) > FThreshold then
    FFilter^[Half].Re := CHalf64 * (1 + FFilter^[Half].Re)
  else
    FFilter^[Half].Re := CHalf64 * FFilter^[Half].Re;
  FFilter^[Half].Im := 0;
{$ELSE}
  if Abs(Spectrum^[0].Im) > FThreshold then
    FFilter^[0].Im := CHalf64 * (1 + FFilter^[0].Im)
  else
    FFilter^[0].Im := CHalf64 * FFilter^[0].Im;
{$ENDIF}
  // transform filter to time domain
  FFft.PerformIFFT(FFilter, FFilterIR);
  ...
  // shift filter impulse response and apply window function
  Move(FFilterIR^[0], FFilterIR^[FFFTSizeHalf div 2],
    FFFTSizeHalf div 2 * SizeOf(Double));
  Move(FFilterIR^[FFFTSize - FFFTSizeHalf div 2], FFilterIR^[0],
    FFFTSizeHalf div 2 * SizeOf(Double));
...
procedure TSpectralNoiseCut64.PerformSpectralEffect
  (Spectrum: PDAVComplexDoubleFixedArray);
begin
{$IFDEF ComplexDataOrder}
...
procedure TSpectralNoiseGate64.PerformSpectralEffect
  (Spectrum: PDAVComplexDoubleFixedArray);
begin
{$IFDEF ComplexDataOrder}
  ComplexMultiplyBlock64(Spectrum, FFilter, FFFTSizeHalf + 1);

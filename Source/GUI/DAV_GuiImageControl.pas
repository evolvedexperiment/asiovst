...
type
  ...
  protected
    ...
    procedure SetItem(Index: Integer;
      const Value: TGuiCustomImageCollectionItem); virtual;
    procedure Notify(Item: TCollectionItem;
      Action: TCollectionNotification); override;
    property Items[Index: Integer]: TGuiCustomImageCollectionItem read GetItem
  ...
  protected
    ...
    function GetItems(Index: Integer): TGuiCustomImageCollectionItem;
      virtual; abstract;
    ...
    property Items[Index: Integer]: TGuiCustomImageCollectionItem
  ...
  protected
    ...
    property ImageItem: TGuiCustomImageCollectionItem read FImageItem
      write SetImageItem;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
...
function TGuiImageCollection.GetItem(Index: Integer)
  : TGuiCustomImageCollectionItem;
begin
  Result := TGuiCustomImageCollectionItem(inherited GetItem(Index));
end;

function TGuiImageCollection.Insert(Index: Integer)
  : TGuiCustomImageCollectionItem;
begin
...
procedure TGuiCustomImageCollectionItem.LinkImageControl
  (Image: TGuiCustomImageControl);
begin
  Assert(Assigned(FLinkedControls));
  if FLinkedControls.IndexOf(Image) < 0 then
    FLinkedControls.Add(Image);
end;

procedure TGuiCustomImageCollectionItem.UnLinkImageControl
  (Image: TGuiCustomImageControl);
begin
  Assert(Assigned(FLinkedControls));
...
begin
  if Assigned(FLinkedControls) then
    while FLinkedControls.Count > 0 do
      UnLinkImageControl(TGuiCustomImageControl(FLinkedControls[0]));
end;
...
procedure TGuiCustomImageList.LinkImageControl(ImageControl
  : TGuiCustomImageControl);
begin
  Assert(Assigned(FLinkedControls));
  if FLinkedControls.IndexOf(ImageControl) < 0 then
    FLinkedControls.Add(ImageControl);
end;

procedure TGuiCustomImageList.UnLinkImageControl(ImageControl
  : TGuiCustomImageControl);
begin
  Assert(Assigned(FLinkedControls));
  if Assigned(ImageControl) then
    FLinkedControls.Remove(ImageControl);
end;
...
begin
  Assert(Assigned(FLinkedControls));
  while FLinkedControls.Count > 0 do
    with TGuiCustomImageControl(FLinkedControls[0]) do
      SetImageList(nil);
end;
...
begin
  ...
  begin
    ...
    if Value < 0 then
      Value := -1
    else if Value >= FImageList.Count then
      Value := FImageList.Count - 1;
  ...
  begin
    ...
    begin
      ...
      if Assigned(FImageItem) then
        FImageItem.UnLinkImageControl(Self);
      FImageItem := Value;
...
begin
  if FLockCount <= 0 then
    raise Exception.Create('Control is not locked');
  Dec(FLockCount);

...
begin
  with TEnhancedGateDataModule(Owner) do
    if Boolean(Round(Parameter[0])) then
...
begin
  with TEnhancedGateDataModule(Owner) do
    Parameter[0] := Integer(CBOnOff.Brightness_Percent > 90);
end;

procedure TEditorForm.DialThresholdChange(Sender: TObject);
begin
  with TEnhancedGateDataModule(Owner) do
    Parameter[1] := DialThreshold.Value;
  UpdateThreshold;
end;
...
begin
  with TEnhancedGateDataModule(Owner) do
    Parameter[3] := Power(10, DialHold.Value);
  UpdateHold;
end;
...
begin
  with TEnhancedGateDataModule(Owner) do
    Parameter[4] := Power(10, DialDecay.Value);
  UpdateDecay;
end;
...
begin
  with TEnhancedGateDataModule(Owner) do
    Parameter[9] := 0.001 * Power(10, DialHiCut.Value);
  UpdateHiCut;
end;
...
begin
  with TEnhancedGateDataModule(Owner) do
  begin
    if DialThreshold.Value <> Parameter[1] then
      DialThreshold.Value := Parameter[1];
...
begin
  with TEnhancedGateDataModule(Owner) do
  begin
    if DialAttack.Value <> Log10(Parameter[2]) then
      DialAttack.Value := Log10(Parameter[2]);
...
begin
  with TEnhancedGateDataModule(Owner) do
  begin
    if DialHold.Value <> Log10(Parameter[3]) then
      DialHold.Value := Log10(Parameter[3]);
...
begin
  with TEnhancedGateDataModule(Owner) do
  begin
    if DialDecay.Value <> Log10(Parameter[4]) then
      DialDecay.Value := Log10(Parameter[4]);
...
begin
  with TEnhancedGateDataModule(Owner) do
  begin
    if DialLoCut.Value <> Log10(Parameter[8]) then
      DialLoCut.Value := Log10(Parameter[8]);
    if Parameter[8] < 1000 then
      EdLoCut.Text := FloatToStrF(Parameter[8], ffFixed, 5,
        Round(2.49999 - Log10(Parameter[8]))) + ' Hz'
    else
      EdLoCut.Text := FloatToStrF(0.001 * Parameter[8], ffFixed, 5, 1) + ' kHz';
    if Parameter[8] > Parameter[9] * 1100 then
      GBSideChain.Font.Color := clRed
    else
      GBSideChain.Font.Color := clWhite;
  end;
end;
...
begin
  with TEnhancedGateDataModule(Owner) do
  begin
    if DialHiCut.Value <> Log10(1000 * Parameter[9]) then
      DialHiCut.Value := Log10(1000 * Parameter[9]);
    if Parameter[9] < 1000 then
      EdHiCut.Text := FloatToStrF(1000 * Parameter[9], ffFixed, 5, 0) + ' Hz'
    else
      EdHiCut.Text := FloatToStrF(Parameter[9], ffFixed, 5, Round(4.49999 - Log10(Parameter[9]))) + ' kHz';
    if Parameter[8] > Parameter[9] * 1100 then
      GBSideChain.Font.Color := clRed
    else
      GBSideChain.Font.Color := clWhite;
...
begin
  with TEnhancedGateDataModule(Owner) do
  begin
    if DialRatio.Value <> Parameter[10] then
      DialRatio.Value := Parameter[10];
...
begin
  with TEnhancedGateDataModule(Owner) do
  begin
    if DialKnee.Value <> Parameter[11] then
      DialKnee.Value := Parameter[11];
...
begin
  with TEnhancedGateDataModule(Owner) do
  begin
    if DialRange.Value <> Parameter[12] then
      DialRange.Value := Parameter[12];

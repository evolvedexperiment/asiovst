...
begin
  ...
  Filer.DefineProperty('Value', ReadValueProperty, WriteValueProperty,
    Value = 0);
end;
...
begin
  ...
  begin
    FNormalizedPosition := MapNormalizedValueToNormalizedPosition
      (NormalizedValue);
    if StitchedImageItem.GlyphCount = 0 then
      FGlyphIndex := -1
    else
      FGlyphIndex := Round(FNormalizedPosition *
        (StitchedImageItem.GlyphCount - 1));
  end;
end;
...
begin
  ...
  begin
    ...
    while FNormalizedPosition < 0 do
      FNormalizedPosition := FNormalizedPosition + 1;
    while FNormalizedPosition > 1 do
      FNormalizedPosition := FNormalizedPosition - 1;
  end
  else
    FNormalizedPosition := Limit(FNormalizedPosition + Amount, 0, 1);
...
begin
  Result := FMinimum + MapNormalizedPositionToNormalizedValue
    (FNormalizedPosition) * FRange;
end;
...
begin
  ...
  if Assigned(FImageItem) then
  begin
    NewGlyphIndex := Round(FNormalizedPosition *
      (StitchedImageItem.GlyphCount - 1));
    if NewGlyphIndex <> GlyphIndex then
...
begin
  if ssShift in Shift then
    ChangeDialPosition(0.1 * FWheelStep * WheelDelta / (120 * FScrollRange))
  else
    ChangeDialPosition(FWheelStep * WheelDelta / (120 * FScrollRange));
  Result := inherited DoMouseWheel(Shift, WheelDelta, MousePos);
end;

procedure TCustomGuiStitchedDial.MouseDown(Button: TMouseButton;
...
begin
  ...
  else if (Button = mbRight) and (ssCtrl in Shift) then
    Value := DefaultValue;
...
procedure TCustomGuiStitchedDial.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  Pt: TPoint;
begin
...
begin
  ...
  if (ssLeft in Shift) then
    if ssShift in Shift then
      ChangeDialPosition(0.1 * (FOldMousePos.Y - Y) / FScrollRange)
    else
      ChangeDialPosition((FOldMousePos.Y - Y) / FScrollRange)
  else if (ssRight in Shift) then; // Position := CircularMouseToPosition(x, y);
...
begin
  ...
  begin
    ...
    DefaultGlyphIndex :=
      Round(MappedNormalizedValue * StitchedImageItem.GlyphCount);
  end;
end;
...
begin
  ...
  begin
    if Value < FMinimum then
      if not(csLoading in ComponentState) then
        raise EInvalidOperation.CreateFmt(RCStrOutOfRange,
          [FMinimum + 1, MaxInt]);
    FMaximum := Value;
    MaximumChanged;
  end;

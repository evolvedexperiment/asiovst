...
constructor TCustomSENoiseReductionModule.Create(SEAudioMaster
...
procedure TCustomSENoiseReductionModule.SubProcessStatic(const BufferOffset,
...
class procedure TCustomSENoiseReductionModule.GetModuleProperties
...
function TCustomSENoiseReductionModule.GetPinProperties(const Index: Integer;
  ...
  case TSENoiseReductionPins(Index) of
    ...
    pinWindowType:
      ...
      begin
        ...
        Datatype := dtEnum;
        DatatypeExtra :=
          'Rectangle, Triangle, Hanning, Hamming, Blackman, Lanczos, Welch';
...
procedure TCustomSENoiseReductionModule.PlugStateChange
...
begin
  ...
  case TSENoiseReductionPins(CurrentPin.PinID) of
    ...
    pinFftOrder:
      FNoiseReduction.FFTOrder := FFftOrder;
    pinWindowType:
      FNoiseReduction.WindowFunctionClass := GWindowFunctions[FWinFuncIndex];
  end;
end;
...
procedure TSENoiseReductionStaticModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
var
...
begin
  ...
  for Sample := 0 to SampleFrames - 1 do
    Outp^[Sample] := FNoiseReduction.ProcessSample32(Inp^[Sample]);
end;

// describe the pins (plugs)
function TSENoiseReductionStaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
...
// An input plug has changed value
procedure TSENoiseReductionStaticModule.PlugStateChange
  (const CurrentPin: TSEPin);
begin
  inherited;
  case TSENoiseReductionPins(CurrentPin.PinID) of
    pinThreshold:
      FNoiseReduction.ThresholdOffset := FThreshold;
    pinRatio:
      FNoiseReduction.Ratio := FRatio;
    pinKnee:
      FNoiseReduction.Knee := FKnee;
    pinAttack:
      FNoiseReduction.Attack := FAttack;
    pinRelease:
      FNoiseReduction.Release := FRelease;
    pinCaptureNoise:
      FNoiseReduction.Match := FCapture;
  end;
end;
...
function TSENoiseReductionControllableModule.GetPinProperties
  (const Index: Integer; Properties: PSEPinProperties): Boolean;
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSENoiseReductionPins(Index) in [pinThreshold .. pinCaptureNoise] then
    with Properties^ do
      Direction := drIn;
end;

...
type
  ...
  TFileFormat = (single_synch, multi_synch, multi_asynch);
  PByte = ^byte;
...
var
  i: integer;
begin
  for i := 0 to tracks.count - 1 do TMidiTrack(tracks[i]).Rewind(0);
  playStartTime := getTickCount;
  playing := true;
  if not FManual then MTimer.enabled := true;
  currentPos := 0.0;
  currentTime := 0;
end;

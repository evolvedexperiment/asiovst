...
type
  ...
  TAudioDataCollection32 = class(TCustomAudioDataCollection)
  private
//    fChannelDataPointerList : array of PAVDSingleFixedArray;
  ...
  TAudioDataCollection64 = class(TCustomAudioDataCollection)
  private
...
begin
 if Dest is TCustomAudioChannel then
  begin
   ...
   TCustomAudioChannel(Dest).fChannelsList := fChannelsList;
  end
 else inherited;
end;
...
constructor TAudioDataCollection32.Create(AOwner: TComponent);
begin
 inherited;
 fChannels := TCustomAudioChannels.Create(Self, TAudioChannel32);
end;

function TAudioDataCollection32.GetAudioChannel(index: Integer): TAudioChannel32;
begin
...
function TAudioDataCollection32.GetChannelDataPointerList(
...
constructor TAudioDataCollection64.Create(AOwner: TComponent);
begin
 inherited;
 fChannels := TCustomAudioChannels.Create(Self, TAudioChannel64);
end;

function TAudioDataCollection64.GetAudioChannel(index: Integer): TAudioChannel64;
begin
 if (Index < 0) or (Index >= fChannels.Count)
  then raise Exception.Create('Index out of bounds')
...
function TAudioDataCollection64.GetChannelDataPointerList(

...
begin
  ...
  with TPhaseRotatorModule(Owner) do
    DialStages.Max := ParameterProperties[1].Max;
  UpdateFrequency;
  UpdateBandwidth;
  UpdateStages;
end;
...
begin
  ...
  with TPhaseRotatorModule(Owner) do
  begin
    if Parameter[1] <> Round(DialStages.Value) then
      Parameter[1] := Round(DialStages.Value);
  end;
end;
...
begin
  ...
  with TPhaseRotatorModule(Owner) do
  begin
    if Parameter[2] <> DialBandwidth.Value then
      Parameter[2] := DialBandwidth.Value;
  end;
end;
...
begin
  ...
  with TPhaseRotatorModule(Owner) do
  begin
    if DialBandwidth.Value <> Parameter[2] then
      DialBandwidth.Value := Parameter[2];

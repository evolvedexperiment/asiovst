...
function TVSTModuleWithMidi.HostCallProcessEvents(const Index: Integer;
...
procedure TVSTModuleWithMidi.ProcessMidiSysExEvent(
  const MidiSysExEvent: TVstMidiSysexEvent);
begin
  if Assigned(FOnProcessMidiSysEx) then
    FOnProcessMidiSysEx(Self, MidiSysExEvent);
end;

function TVSTModuleWithMidi.HostCallGetCurrentMidiProgram(const Index: Integer;
  const Value: TVstIntPtr; const ptr: pointer; const opt: Single): TVstIntPtr;
begin
  Result := -1;
end;
...
procedure TVSTModuleWithMidi.HostCallProcess(const Inputs,
...
procedure TVSTModuleWithMidi.HostCallProcess32Replacing(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
begin
  inherited;
...
procedure TVSTModuleWithMidi.HostCallProcess64Replacing(const Inputs,
...
procedure TVSTModuleWithMidi.MidiOut(const b1, b2, b3: Byte; b4: Byte = 0;
...
begin
  ...
  begin
    ...
    MidiData[3] := b4;
    DeltaFrames := offset;
    if FVstEvents.numEvents < CMaxMidiEvents - 1 then
      Inc(FVstEvents.numEvents);
  end;
end;

procedure TVSTModuleWithMidi.MidiCC(const Channel, Number, Value: Integer;
  const Offset: Integer = 0);
begin
...
procedure TVSTModuleWithMidi.MidiChannelAftertouch(const Channel, Value: Integer;
  const Offset: Integer = 0);
begin
...
procedure TVSTModuleWithMidi.MidiNoteOff(const Channel, Note: Integer;
  const Value: Integer = 0; const Offset: Integer = 0);
begin
...
procedure TVSTModuleWithMidi.MidiNoteOn(const Channel, Note, Value: Integer;
  const Offset: Integer = 0);
begin
...
procedure TVSTModuleWithMidi.MidiPitchBend(ch, val: Integer;
  const Offset: Integer = 0);
var
  a, b: Integer;
begin
...
procedure TVSTModuleWithMidi.MidiPitchBend2(const Channel, x1, x2: Integer;
  const Offset: Integer = 0);
begin
...
procedure TVSTModuleWithMidi.MidiPolyAftertouch(const Channel, Note,
  Value: Integer; const Offset: Integer = 0);
begin
...
procedure TVSTModuleWithMidi.MidiProgramChange(const Channel, Value: Integer;
  const Offset: Integer = 0);
begin
...
procedure TVSTModuleWithMidi.MidiSendSysEx(const Data: array of Byte;
  const Offset: Integer = 0);
begin
  ...
  begin
    ...
    if EventType = etSysEx then
      ReallocMem(sysexDump, dumpBytes)
    else

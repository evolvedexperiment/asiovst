...
begin
  ...
  begin
    SampleRate := Self.SampleRate;
    Frequency := Self.Frequency * FastPower2MinError4(2 * COneTwelfth32) -
      Self.Frequency;
  end;
end;
...
procedure TCustomBarberpoleFilter.ProcessBlock32
...
procedure TCustomBarberpoleFilter.ProcessBlock64
...
begin
  FLowpass.Frequency := FLFO.Frequency * FastPower2MinError4(2 * COneTwelfth32)
    - FLFO.Frequency;
  Changed;
end;
...
begin
  if Value > 0 then
    ...
    raise Exception.Create(RCStrOrderMustBeLarger0);
end;
...
begin
  ...
  begin
    MinimumFrequency := 0;
    MaximumFrequency := 2 * Self.Frequency * FastPower2MinError4
      (2 * COneTwelfth32) - Self.Frequency;
...
begin
  ...
  if Assigned(FZCTuner) then
    FZCTuner.SampleRate := SampleRate;
end;

procedure TCustomBarberpoleTuner.SetFrequency(const Value: Single);
begin
  FBarberpoleFilter.Frequency := Value;
  FZCTuner.MaximumFrequency := 2 * Frequency * FastPower2MinError4
    (2 * COneTwelfth32) - Frequency;
end;

...
{$ENDIF}
  ...
  public
    ...
    property AudioDataCollection: TCustomAudioDataCollection
      read FAudioDataCollection write SetAudioDataCollection;
    property DisplayedChannel: Integer read FDisplayedChannel
      write SetDisplayedChannel default -1;
    property DisplayChannels: TDisplayChannels read FDisplayChannels
      write SetDisplayChannels;
...
{$ENDIF}
    property WaveDrawMode: TGuiWaveDrawMode read FWaveDrawMode
      write SetWaveDrawMode default wdmLine;
...
procedure TCustomGuiAudioDataDisplay.DisplayChannelsChangedHandler
  (Sender: TObject);
begin
  //
end;
...
procedure TCustomGuiAudioDataDisplay.SetAudioData
  (const Value: TCustomAudioData);
begin
...
procedure TCustomGuiAudioDataDisplay.SetAudioDataCollection
  (const Value: TCustomAudioDataCollection);
begin
...
begin
  ...
  begin
    ...
    else if Assigned(FAudioDataCollection) then
    begin
...
procedure TCustomGuiAudioDataDisplay.SetCursor(const Value
  : TGuiAudioDataDisplayCursor);
begin
...
procedure TCustomGuiAudioDataDisplay.SetXAxis(const Value
  : TGuiAudioDataDisplayXAxis);
begin
  FXAxis.Assign(Value);
end;
...
begin
  if (ChannelCount > 0) and (ChannelCount > FDisplayedChannel) then
    if FDisplayedChannel >= 0 then
      DrawChannelData(Bitmap, FDisplayedChannel)
    else
      for ch := 0 to ChannelCount - 1 do
        DrawChannelData(Bitmap, ch);
...
procedure TCustomGuiAudioDataDisplay.DrawChannelData(Bitmap: TBitmap;
  Channel: Integer);
var
...
begin
  ...
  begin
    ...
    if Channel < FDisplayChannels.Count then
      Pen.Color := TCustomDisplayChannel(FDisplayChannels[Channel]).Color
    else
    ...
    if (FAudioDataCollection.Channels.Items[Channel] is TAudioChannel32) then
      ...
      begin
        ...
        begin
          ...
          if ChannelDataPointer^[Sample] > MaxVal then
            MaxVal := ChannelDataPointer^[Sample]
          else if ChannelDataPointer^[Sample] < MinVal then
            MinVal := ChannelDataPointer^[Sample];
          ...
          begin
            ...
            begin
              LineTo(XPixelPosAsInt,
                HlfHght * Round((1 - FScaleFactor * MinVal)));
            end
            else
            begin
              o := PenPos.Y - HlfHght;
              ...
              begin
                ...
                LineTo(XPixelPosAsInt,
                  Round((1 - FScaleFactor * MaxVal) * HlfHght));
              end
              else
              begin
                LineTo(XPixelPosAsInt,

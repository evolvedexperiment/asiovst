...
begin
  ...
  FMakeUpGain_dB := FastSqrtBab2(Sqr(Temp) + Sqr(FKnee_dB)) - Temp + CHalf32
    * FKnee_dB;
...
begin
  ...
  Result := FastPower2MinError3
    (-CHalf32 * (Result + FastSqrtBab2(Sqr(Result) + FKneeFactor)));
end;

function TLightweightSoftKneeLimiter.CharacteristicCurve_dB(const InputLevel_dB
  : Double): Double;
var
  Temp: Single;
begin
  Temp := -CHalf32 * (InputLevel_dB - FThreshold_dB);
  Result := Temp - FastSqrtBab2(Sqr(Temp) + Sqr(FKnee_dB)) + MakeUpGain_dB +
    InputLevel_dB;
end;
...
begin
  ...
  FGain := FastPower2MinError3
    (CHalf32 * (Temp - FastSqrtBab2(Sqr(Temp) + FKneeFactor)));
end;
{$ELSE}
...
begin
  if FAttack <= 0 then
    raise Exception.Create('Attack time must be larger than zero!')
  else
  begin
...
begin
  if FRelease <= 0 then
    raise Exception.Create('Release time must be larger than zero!')
  else
  begin
...
procedure TLightweightSoftKneeFeedbackLikeLimiter.
  CalculateLogScaledThresholdValue;
begin
  FThrshlddB := Threshold_dB * CdBtoAmpExpGain32;
end;
...
function TLightweightSoftKneeFeedbackLikeLimiter.TranslatePeakToGain
  (const PeakLevel: Double): Double;
begin
  Result := PeakLevel;
  Result := CHalf32 * (FThrshlddB - FastLog2ContinousError5(Result));
  Result := FastPower2MinError3
    (Result - FastSqrtBab2(Sqr(Result) + FKneeFactor));
end;
...
function TLightweightSoftKneeFeedbackLikeLimiter.GainSample
  (const Input: Double): Double;
begin
  Result := FGain * FMakeUpGain * Input;
end;

procedure TLightweightSoftKneeFeedbackLikeLimiter.InputSample
  (const Input: Double);
{$IFNDEF XPUREPASCAL}
var
...
begin
  ...
  if Temp[0] > FPeak then
    ...
    FPeak := Temp[0] + (FPeak - Temp[0]) * FReleaseFactor;

  Temp[0] := -CHalf32 * (FastLog2ContinousError5(FPeak) - FThrshlddB);
  // * FRatioFactor ???, PLEASE CHECK!!!
...
function TLightweightSoftKneeFeedbackLikeLimiter.ProcessSample64
  (Input: Double): Double;
begin
  InputSample(Input);
  Result := FGain * FMakeUpGain * Input;
...
begin
  if FAttack = 0 then
    ...
    FAttackFactor := 1 - FastPower2MinError3
      (-1 / (FAttack * 0.001 * SampleRate));
end;
...
begin
  if FRelease = 0 then
    ...
    FReleaseFactor := FastPower2MinError3(-1 / (FRelease * 0.001 * SampleRate));
end;
...
function TLightweightSoftKneeCompressor.TranslatePeakToGain(const PeakLevel
  : Double): Double;
begin
  Result := PeakLevel;
...
function TLightweightSoftKneeUpwardCompressor.TranslatePeakToGain
  (const PeakLevel: Double): Double;
begin
  Result := PeakLevel;
...
function TLightweightSoftKneeUpwardCompressor.GainSample
  (const Input: Double): Double;
begin
  Result := FGain * FMakeUpGain * Input;
end;
...
function TLightweightSoftKneeUpwardCompressor.ProcessSample64
  (Input: Double): Double;
begin
  InputSample(Input);
  Result := FGain * FMakeUpGain * Input;
...
begin
  if FAttack = 0 then
    ...
    FAttackFactor := 1 - FastPower2MinError3
      (-1 / (FAttack * 0.001 * SampleRate * FRatio));
end;
...
begin
  if FRelease = 0 then
    ...
    FReleaseFactor := FastPower2MinError3
      (-1 / (FRelease * 0.001 * SampleRate * FRatio));
end;
...
procedure TLightweightSoftKneeFeedbackCompressor.
  CalculateLogScaledThresholdValue;
begin
  FThrshlddB := Threshold_dB * CdBtoAmpExpGain32;
end;
...
function TLightweightSoftKneeFeedbackCompressor.TranslatePeakToGain
  (const PeakLevel: Double): Double;
begin
  Result := PeakLevel;
  Result := FRatioFactor * (FastLog2ContinousError5(Result) - FThrshlddB);
...
function TLightweightSoftKneeFeedbackCompressor.GainSample
  (const Input: Double): Double;
begin
  Result := FGain * FMakeUpGain * Input;
end;

procedure TLightweightSoftKneeFeedbackCompressor.InputSample
  (const Input: Double);
{$IFNDEF XPUREPASCAL}
var
  Temp: Single;
...
function TLightweightSoftKneeFeedbackCompressor.ProcessSample64
  (Input: Double): Double;
begin
  InputSample(Input);
  Result := FGain * FMakeUpGain * Input;
...
begin
  if FAttack <= 0 then
    raise Exception.Create(RCStrAttackTimeInvalid)
  else
  begin
    FAttackSampleCycle := -1 / (FAttack * 0.001 * SampleRate);
    FAttackFactor := 1 - FastPower2MinError3(FAttackSampleCycle);
...
begin
  if FRelease <= 0 then
    raise Exception.Create(RCStrReleaseTimeInvalid)
  else
  begin
    FReleaseSampleCycle := -1 / (FRelease * 0.001 * SampleRate);
    FReleaseFactor := FastPower2MinError3(FReleaseSampleCycle);
...
procedure TLightweightSoftKneeFeedbackLikeCompressor.
  CalculateLogScaledThresholdValue;
begin
  FThrshlddB := Threshold_dB * CdBtoAmpExpGain32;
end;

function TLightweightSoftKneeFeedbackLikeCompressor.TranslatePeakToGain
  (const PeakLevel: Double): Double;
begin
  Result := PeakLevel;
  Result := FRatioFactor * (FastLog2ContinousError5(Result) - FThrshlddB);
...
function TLightweightSoftKneeFeedbackLikeCompressor.GainSample
  (const Input: Double): Double;
begin
  Result := FGain * FMakeUpGain * Input;
end;

procedure TLightweightSoftKneeFeedbackLikeCompressor.InputSample
  (const Input: Double);
{$IFDEF PUREPASCAL}
var
  Temp: array [0 .. 1] of Single;
...
function TLightweightSoftKneeFeedbackLikeCompressor.ProcessSample64
  (Input: Double): Double;
begin
  InputSample(Input);
  Result := FGain * FMakeUpGain * Input;
...
RegisterDspProcessors32([TLightweightSoftKneeLimiter,
  ...
  TLightweightSoftKneeUpwardCompressor, TLightweightSoftKneeFeedbackCompressor,
  TLightweightSoftKneeFeedbackLikeCompressor]);

...
type
  ...
  TSEMp3PlayerPins = (pinFileName, pinBufferSize, pinSemitones,
    pinInterpolation, pinReset, pinOutputLeft, pinOutputRight, pinTitle,
    pinArtist, pinAlbum, pinYear, pinComment);
...
constructor TSEMp3PlayerModule.Create(AudioMaster: TSE2AudioMasterCallback;
  Reserved: Pointer);
begin
  inherited;
...
begin
  if {$IFDEF UseEmbedding}(FContainedData.Count = 0)
  {$ELSE} (not FileExists(FFileName)) {$ENDIF}
  then
    OnProcess := SubProcessBypass
  else
...
procedure TSEMp3PlayerModule.PlugStateChange(const CurrentPin: TSEPin);
begin
  inherited;
...
{$ENDIF}
          ...
          if Assigned(FBufferedPlayer.MpegAudio) then
            ...
            begin
              if Assigned(FTitle) then
                StrPCopy(FTitle, Id3Title);
              if Assigned(FArtist) then
                StrPCopy(FArtist, Id3Artist);
              if Assigned(FAlbum) then
                StrPCopy(FAlbum, Id3Album);
              if Assigned(FYear) then
                StrPCopy(FYear, Id3Year);
              if Assigned(FComment) then
                StrPCopy(FComment, Id3Comment);
            end;
    ...
    pinBufferSize:
      begin
        if FBufferSize < 1024 then
          FBufferSize := 1024
        else if FBufferSize > 65536 then
          FBufferSize := 65536;
...
begin
  ...
  begin
    if Assigned(FResourceStream) then
      FreeAndNil(FResourceStream)FResourceStream :=
        TResourceStream.Create(HInstance, FContainedData[ID], 'MP3');
...
procedure TSEMp3PlayerModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
begin
  FCriticalSection.Enter;
  try
    FBufferedPlayer.GetSamples(@FOutLeftBuffer^[BufferOffset],
      @FOutRightBuffer^[BufferOffset], SampleFrames)
  finally
    FCriticalSection.Leave;
  end;
end;
...
class procedure TSEMp3PlayerModule.GetModuleProperties
...
{$IFDEF UseEmbedding}
  ...
  try
    EnumResourceNames(HInstance, 'MP3', @EnumNamesFunc,
...
function TSEMp3PlayerModule.GetPinProperties(const Index: Integer;

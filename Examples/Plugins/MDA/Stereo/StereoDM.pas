...
type
  ...
  private
    FMod, FPhi, FDel, FDeltaPhi: Single;
    FSize: Integer;
    FBufferPos: Integer;
    FDelay: Integer;
    FRi, FRd, FLi, FLd: Single;
...
procedure TStereoDataModule.VSTModuleSampleRateChange(Sender: TObject;
...
procedure TStereoDataModule.ParameterWidthChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
...
begin
  if FMod > 0 then
    PreDefined := FloatToStrF(1000 * FMod / SampleRate, ffGeneral, 2, 2)
  else
    PreDefined := 'OFF';
end;

procedure TStereoDataModule.ParameterModChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FMod := (2100 * Power(Value, 2));
end;

procedure TStereoDataModule.ParameterRateChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  DeltaPhiChanged;
end;
...
procedure TStereoDataModule.ParameterDelayChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin

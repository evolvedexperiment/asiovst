...
type
  ...
  public
    ...
    property ThermalVoltage: Single read FThermalVoltage
      write SetThermalVoltage;
...
begin
  ...
  begin
    ...
    if FOrder - 1 >= 0 then
      SetLength(FTanhLastSample, FOrder - 1)
    else
      SetLength(FTanhLastSample, 0);
    Reset;
  end;
  inherited;
end;
...
function TLightweightSimpleClassicMoogFilter.ProcessSample64
  (Input: Double): Double;
var
  NewInput: Double;
begin
  ...
  // first stage
  FLastSample[0] := FLastSample[0] + FCoefficient *
    (FastTanhContinousError4(0.5 * NewInput * FThermalVoltageInv) -
    FTanhLastSample[0]);
  FTanhLastSample[0] := FastTanhContinousError4(0.5 * FLastSample[0] *
    FThermalVoltageInv);

  // second stage
  FLastSample[1] := FLastSample[1] + FCoefficient *
    (FTanhLastSample[0] - FTanhLastSample[1]);
  FTanhLastSample[1] := FastTanhContinousError4(0.5 * FLastSample[1] *
    FThermalVoltageInv);

  // third stage
  FLastSample[2] := FLastSample[2] + FCoefficient *
    (FTanhLastSample[1] - FTanhLastSample[2]);
  FTanhLastSample[2] := FastTanhContinousError4(0.5 * FLastSample[2] *
    FThermalVoltageInv);

  // last stage
  FLastSample[3] := FLastSample[3] + FCoefficient *
    (FTanhLastSample[2] - FastTanhContinousError4(0.5 * FLastSample[3] *
    FThermalVoltageInv));
...
function TLightweightImprovedClassicMoogFilter.ProcessSample64
  (Input: Double): Double;
var
  NewInput: Double;
begin

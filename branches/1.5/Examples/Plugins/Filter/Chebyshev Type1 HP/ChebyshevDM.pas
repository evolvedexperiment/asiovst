...
begin
  for Channel := 0 to Length(FFilter) - 1 do
    if Assigned(FFilter[Channel]) then
      FFilter[Channel].Ripple := Limit(Value, 1E-5, 10);

  // update GUI if necessary
  if EditorForm is TFmChebyshev then
    TFmChebyshev(EditorForm).UpdateRipple;
end;
...
begin
  ...
  // update GUI if necessary
  if EditorForm is TFmChebyshev then
    TFmChebyshev(EditorForm).UpdateOrder;
end;
...
begin
  ...
  // update GUI if necessary
  if EditorForm is TFmChebyshev then
    TFmChebyshev(EditorForm).UpdateFrequency;
end;


// parameter display

procedure TChebyshevHPModule.ParameterRippleDisplay(Sender: TObject;
...
procedure TChebyshevHPModule.ParameterOrderDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
...
procedure TChebyshevHPModule.StringToFrequencyParameter(Sender: TObject;

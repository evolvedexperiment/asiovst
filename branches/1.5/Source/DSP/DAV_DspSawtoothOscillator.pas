...
type
  ...
  public
    property OversamplingFactor: Integer read FOSFactor
      write SetOversamplingFactor;
  end;
  ...
  public
    ...
    property OversamplingOrder: Integer read FOsOrder
      write SetOversamplingOrder;
  end;

  TCustomOversampledFilterSawtoothOscillator = class
  ...
  TButterworthOversampledSawtoothOscillator = class
    (TCustomOversampledFilterSawtoothOscillator)
  public
    constructor Create; override;
  end;

  TChebyshevOversampledSawtoothOscillator = class
    (TCustomOversampledFilterSawtoothOscillator)
  private
    FRipple: Single;
...
begin
  inherited;
  FIntOffset := Round(0.5 * FFrequency / FOSFactor / SampleRate *
    High(Cardinal));
end;
...
begin
  inherited;
  FIntOffset := Round(0.5 * FFrequency / (1 shl FOsOrder) / SampleRate *
    High(Cardinal));
end;
...
begin
  ...
  for i := FOsOrder to Length(FDownsampler) - 1 do
    FreeAndNil(FDownsampler[i]);

  SetLength(FDownsampler, FOsOrder);

  for i := 0 to Length(FDownsampler) - 1 do
    if not Assigned(FDownsampler[i]) then

...
type
  ...
  TSEControlableChebyshevWaveshaperModule = class
  ...
  TSEAutomatableChebyshevWaveshaperModule = class
    (TSECustomChebyshevWaveshaperModule)
  protected
...
class procedure TSECustomChebyshevWaveshaperModule.GetModuleProperties
...
function TSECustomChebyshevWaveshaperModule.GetPinProperties
...
procedure TSECustomChebyshevWaveshaperModule.SubProcessStatic
...
begin
  inherited;
  for i := 0 to CHarmonicCount - 1 do
    FHarmonics[i] := 0;
end;
...
procedure TSEStaticChebyshevWaveshaperModule.PlugStateChange
...
begin
  ...
  case CurrentPin.PinID of
    ...
    2:
      FWaveShaper.Order := FOrder;
    3 .. CHarmonicCount + 1:
      if (CurrentPin.PinID - 3) < FWaveShaper.Order then
        FWaveShaper.Gain[CurrentPin.PinID - 3] :=
          FHarmonics[CurrentPin.PinID - 3];
  end;
end;

procedure TSEStaticChebyshevWaveshaperModule.SubProcess(const BufferOffset,
...
begin
  Result := inherited GetPinProperties(Index, Properties);
  if Index in [2 .. CHarmonicCount + 2] then
    with Properties^ do
      Direction := drIn;
end;
...
// e.g when the user changes a module's parameters,
// or when audio stops/starts streaming into a pin
procedure TSEAutomatableChebyshevWaveshaperModule.PlugStateChange
  (const CurrentPin: TSEPin);
begin
  inherited;
...
begin
  for Sample := 0 to SampleFrames - 1 do
  begin
    for i := 0 to FWaveShaper.Order - 1 do
      FWaveShaper.Gain[i] := FHarmonics[i, BufferOffset + Sample];
    FOutputBuffer[BufferOffset + Sample] := FWaveShaper.ProcessSample64
      (FInput1Buffer[BufferOffset + Sample]);
  end;
end;

...
{$ENDIF}
    procedure CMColorChanged(var Message:
      {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF}); message CM_COLORCHANGED;
    procedure CMEnabledChanged(var Message: TMessage);
      message CM_ENABLEDCHANGED;
    procedure CMFontChanged(var Message:
      {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF}); message CM_FONTCHANGED;
    procedure CMTextChanged(var Message:
      {$IFDEF FPC}TLMessage{$ELSE}TWmNoParams{$ENDIF}); message CM_TEXTCHANGED;
...
begin
  ...
  if not FNative then
    with Params do
      Style := (Style and not $1F) or BS_OWNERDRAW;
end;
...
procedure TCustomGuiGroup.CMTextChanged(var Message:
  {$IFDEF FPC}TLMessage{$ELSE}TWmNoParams{$ENDIF});
begin
  TextChanged;
end;
...
{$IFNDEF FPC}
  if FTransparent then
    FBackBuffer.CopyParentImage(Self)
  else
{$ENDIF}
  begin
...
begin
  ...
  if not Assigned(FBuffer) or not Assigned(FBackBuffer) then
    Exit;

  Assert((FBackBuffer.Width = FBuffer.Width) and
    (FBackBuffer.Height = FBuffer.Height));

  // copy entire back buffer to buffer
  Move(FBackBuffer.DataPointer^, FBuffer.DataPointer^,
    FBuffer.Height * FBuffer.Width * SizeOf(TPixel32));
...
begin
  ...
  begin
    FGuiFont.TextOut(Caption, PixelMap, Round(FBorderWidth + FBorderRadius),
      Round(FBorderWidth));
  end;
end;
...
begin
  ...
  begin
    ...
    RadiusFixed := ConvertToFixed24Dot8
      (Min(FBorderRadius, 0.5 * Min(Width, Height)) + 1);
    ...
    *)
    ...
    begin
      ...
      // calculate x offset
      SqrYDist := FixedSqr(FixedSub(YFixed, FixedSub(RadiusFixed,
        CFixed24Dot8One)));
      XStart.Fixed := FixedSqr(RadiusFixed).Fixed - SqrYDist.Fixed;
    ...
    for Y := FixedRoundHalfUp(RadiusFixed) to FixedRoundHalfUp
      (HeaderHeight) - 1 do
    begin
      ScnLne[0] := Scanline[Y];
      YFixed := ConvertToFixed24Dot8(Y);
      ...
      begin
        ...
        if IsUpperLowerHalf then
        begin
          if (XFixed.Fixed > ConvertToFixed24Dot8(Width).Fixed -
    ...
    for Y := FixedRoundHalfUp(HeaderHeight) to Height - 1 -
      FixedRoundHalfUp(RadiusFixed) do
    begin
      ScnLne[0] := Scanline[Y];
      YFixed := ConvertToFixed24Dot8(Y);
    ...
    for Y := FixedRoundHalfUp(RadiusFixed) to FixedRoundHalfUp(HeaderHeight) - 1
      - FixedRoundHalfUp(RadiusFixed) do
    begin
    ...
    begin
      ...
      BlendPixelLine(BorderColor, @ScnLne[1][0],
        FixedRoundHalfUp(FixedSub(HeaderWidth, RadiusFixed)) - 1);
      ...
      begin
        ...
        BlendPixelInplace(CombColor, ScnLne[0][X]);
        BlendPixelInplace(CombColor,
          ScnLne[1][FixedRoundHalfUp(HeaderWidth) - 1 - X]);
      end;

      BlendPixelLine(BorderColor, @ScnLne[0][FixedRoundHalfUp(RadiusFixed) + 1],
        FixedRoundHalfUp(FixedSub(HeaderWidth, RadiusFixed)) - 1);
      EMMS;
    end;
...
begin
  ...
  begin
    ...
    BorderWidthFixed := ConvertToFixed24Dot8(Max(FBorderWidth, 1));
    WidthMinusOne := ConvertToFixed24Dot8(Integer(Width - 1));
    HeaderWidth.Fixed := ConvertToFixed24Dot8(FHeaderWidth).Fixed + 3 *
      BorderWidthFixed.Fixed;
    ...
    begin
      ...
      begin
        ...
        // select color to be drawn
        if (XFixed.Fixed <= BorderWidthFixed.Fixed + HeaderWidth.Fixed -
          CFixed24Dot8One.Fixed) and (SqrDist.Fixed < SqrRadMinusBorder.Fixed)
        then
          CombColor := BorderColor
        else if SqrDist.Fixed <= SqrRadMinusBorderOne.Fixed then
          CombColor := PanelColor
        else if SqrDist.Fixed <= SqrRadMinusBorder.Fixed then
        begin
          Temp.Fixed := RadMinusBorder.Fixed - FixedSqrt(SqrDist).Fixed;
    ...
    for Y := FixedRoundHalfUp(RadiusFixed) to Height - 1 -
      FixedRoundHalfUp(RadiusFixed) do
    begin
      ScnLne := Scanline[Y];
      YFixed := ConvertToFixed24Dot8(Y);
      ...
      begin
        ...
        else if (XFixed.Fixed <= BorderWidthFixed.Fixed + HeaderWidth.Fixed -
          CFixed24Dot8One.Fixed) or
          (XFixed.Fixed >= WidthMinusOne.Fixed - BorderWidthFixed.Fixed +
          CFixed24Dot8One.Fixed) then
          CombColor := BorderColor
        else if (XFixed.Fixed < BorderWidthFixed.Fixed) then
        begin
...
begin
  ...
  begin
    TextSize := FGuiFont.TextExtent(Caption);
    FGuiFont.TextOut(Caption, PixelMap, Round(2 * FBorderWidth),
      (Height - TextSize.cx) div 2);
  end;
end;
...
begin
  ...
  begin
    ...
    // set other local variables
    RadiusFixed := ConvertToFixed24Dot8
      (Min(FBorderRadius, Min(FHeaderHeight, 0.5 * Min(Width, Height))) + 1);
    BorderWidthFixed := ConvertToFixed24Dot8(Max(FBorderWidth, 1));
    WidthMinusOne := ConvertToFixed24Dot8(Integer(Width - 1));
    HeaderHeight.Fixed := ConvertToFixed24Dot8(FHeaderHeight).Fixed + 2 *
      BorderWidthFixed.Fixed;
    ...
    begin
      ...
      begin
        ...
        begin
          if (XFixed.Fixed < BorderWidthFixed.Fixed - CFixed24Dot8One.Fixed) or
            (XFixed.Fixed > ConvertToFixed24Dot8(Width).Fixed -
            BorderWidthFixed.Fixed) or (YFixed.Fixed <= HeaderHeight.Fixed) then
            CombColor := BorderColor
          else if (XFixed.Fixed < BorderWidthFixed.Fixed) then
          begin
            Temp.Fixed := BorderWidthFixed.Fixed - XFixed.Fixed;
          ...
          or (XFixed.Fixed >= WidthMinusOne.Fixed - BorderWidthFixed.Fixed +
          CFixed24Dot8One.Fixed) or (YFixed.Fixed <= HeaderHeight.Fixed) then
          CombColor := BorderColor
        else if (XFixed.Fixed < BorderWidthFixed.Fixed) then
        begin
          Temp.Fixed := BorderWidthFixed.Fixed - XFixed.Fixed;
...
begin
  ...
  begin
    TextSize := FGuiFont.TextExtent(Caption);
    FGuiFont.TextOut(Caption, PixelMap, (Width - TextSize.cx) div 2,
      Round(FBorderWidth));
  end;
end;
...
begin
  ...
  begin
    ...
    // set other local variables
    BorderRadiusFixed := ConvertToFixed24Dot8
      (Min(FBorderRadius, 0.5 * Min(Width, Height)) + 1);
    BorderWidthFixed := ConvertToFixed24Dot8(Max(FBorderWidth, 1));
    ...
    begin
      YFixed := ConvertToFixed24Dot8(Y);
      SqrYDist := FixedSqr(FixedSub(YFixed, FixedSub(BorderRadiusFixed,
        CFixed24Dot8One)));
      ...
      begin
        ...
        begin
          CombColor := BorderColor;
          Temp.Fixed := CombColor.A;
          Temp := FixedMul(Temp, FixedSub(BorderRadiusFixed,
            FixedSqrt(SqrDist)));
          Assert(Temp.Fixed >= 0);
          Assert(Temp.Fixed <= $FF);
          CombColor.A := Temp.Fixed;
    ...
    for Y := 0 to FixedFloor(BorderWidthFixed) - 1 do
    begin
      BlendPixelLine(BorderColor, @Scanline[FYOffset + Y][X + FTextWidth],
        Width - 2 * X - FTextWidth);
    ...
    BlendPixelLine(CombColor, @Scanline[FYOffset + Y][X + FTextWidth],
      Width - 2 * X - FTextWidth);
    ...
    begin
      ...
      if XRange[0] > 0 then
        ...
        begin
          ...
          BlendPixelLine(BorderColor, @ScnLne[0][Width - XRange[0] - X],
            XRange[0]);
          BlendPixelLine(BorderColor, @ScnLne[1][X], XRange[0]);
          BlendPixelLine(BorderColor, @ScnLne[1][Width - XRange[0] - X],
            XRange[0]);
        end;
    end;
    ...
    if (BorderWidthFixed.Fixed > BorderRadiusFixed.Fixed) then
      for Y := FixedRoundHalfUp(BorderRadiusFixed) to FixedRoundHalfUp
        (BorderWidthFixed) - 1 do
      begin
        ScnLne[0] := Scanline[FYOffset + Y];
    ...
    CenterX.Frac := 0;
    BorderWidthFixed := FixedMul(FixedAdd(BorderWidthFixed, CFixed24Dot8One),
      CFixed24Dot8Half);

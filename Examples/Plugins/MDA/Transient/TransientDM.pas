...
begin
  if Assigned(FTransientProcessor) then
    FTransientProcessor.Output := Value;
end;
...
begin
  if Assigned(FTransientProcessor) then
    FTransientProcessor.Attack := Value;
end;
...
begin
  if Assigned(FTransientProcessor) then
    FTransientProcessor.Release := Value;
end;
...
begin
  if Assigned(FTransientProcessor) then
    FTransientProcessor.AttackHold := Value;
end;
...
begin
  if Assigned(FTransientProcessor) then
    FTransientProcessor.ReleaseHold := Value;
end;

procedure TTransientDataModule.ParameterFilterDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  if Parameter[Index] < 0.5 then
    PreDefined := 'Lowpass: ' + PreDefined
  else if Parameter[Index] > 0.5 then
    PreDefined := 'Highpass: ' + PreDefined;
end;
...
begin
  if Assigned(FTransientProcessor) then
    FTransientProcessor.Filter := Value;
end;

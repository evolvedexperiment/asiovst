...
procedure THardKneeCompressorDataModule.SLThresholdChange(Sender: TObject;
...
begin
  ...
  for ChannelIndex := 0 to 1 do
    if Assigned(FSimpleCompressors[ChannelIndex]) then
      FSimpleCompressors[ChannelIndex].Threshold_dB := Value;

  // update GUI
  if EditorForm is TFmHardKneeCompressor then
    with TFmHardKneeCompressor(EditorForm) do
      UpdateThreshold;
end;

procedure THardKneeCompressorDataModule.SLRatioChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ChannelIndex: Integer;
begin
  ...
  for ChannelIndex := 0 to 1 do
    if Assigned(FSimpleCompressors[ChannelIndex]) then
      FSimpleCompressors[ChannelIndex].Ratio := 1 / Value;
...
procedure THardKneeCompressorDataModule.SLReleaseChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ChannelIndex: Integer;
begin
  ...
  for ChannelIndex := 0 to 1 do
    if Assigned(FSimpleCompressors[ChannelIndex]) then
      FSimpleCompressors[ChannelIndex].Release := Value;
...
procedure THardKneeCompressorDataModule.SLAttackChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ChannelIndex: Integer;
begin
  ...
  for ChannelIndex := 0 to 1 do
    if Assigned(FSimpleCompressors[ChannelIndex]) then
      FSimpleCompressors[ChannelIndex].Attack := Value;
...
begin
  ...
  begin
    if Assigned(FSimpleCompressors[0]) then
      FSimpleCompressors[0].SampleRate := Abs(SampleRate);
    if Assigned(FSimpleCompressors[1]) then
      FSimpleCompressors[1].SampleRate := Abs(SampleRate);
  end;
end;

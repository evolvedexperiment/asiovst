...
begin
  ...
  begin
    FLightweightMultibandCompressor[Channel] :=
      TLightweightSoftKneeCompressor.Create;
...
procedure TLightweightMultibandCompressorDataModule.VSTModuleClose
  (Sender: TObject);
var
  Channel: Integer;
begin
  for Channel := 0 to Length(FLightweightMultibandCompressor) - 1 do
    FreeAndNil(FLightweightMultibandCompressor[Channel]);
end;
...
begin
  ...
  begin
    if Assigned(FLightweightMultibandCompressor[Index]) then
...
begin
  for Channel := 0 to Length(FLightweightMultibandCompressor) - 1 do
    if Assigned(FLinkwitzRiley[Channel, 0]) then
      FLinkwitzRiley[Channel, 0].Frequency := Value;
...
begin
  for Channel := 0 to Length(FLightweightMultibandCompressor) - 1 do
    if Assigned(FLinkwitzRiley[Channel, 1]) then
      FLinkwitzRiley[Channel, 1].Frequency := Value;
...
begin
  for Channel := 0 to Length(FLightweightMultibandCompressor) - 1 do
    if Assigned(FLinkwitzRiley[Channel, 2]) then
      FLinkwitzRiley[Channel, 2].Frequency := Value;
...
begin
  ...
  // update GUI
  if EditorForm is TFmLightweightMultibandCompressor then
    TFmLightweightMultibandCompressor(EditorForm).UpdateLimit;
end;
...
begin
  if Index in [0 .. Length(FLightweightMultibandCompressor) - 1] then
    Result := FLightweightMultibandCompressor[Index].AutoMakeUp
  else
    raise Exception.CreateFmt(RStrIndexOutOfBounds, [Index]);
end;

function TLightweightMultibandCompressorDataModule.
  GetLightweightMultibandCompressor(Index: Integer)
  : TLightweightSoftKneeCompressor;
begin
  if Index in [0 .. Length(FLightweightMultibandCompressor) - 1] then
    Result := FLightweightMultibandCompressor[Index]
  else
    raise Exception.CreateFmt(RStrIndexOutOfBounds, [Index]);
end;
...
begin
  Band := (Index - 4) div 7;
  if Assigned(FLightweightMultibandCompressor[Band]) then
    ...
    with TFmLightweightMultibandCompressor(EditorForm) do
      case Band of
        0:
          begin
            UpdateLowThreshold;
            if Parameter[Band * 7 + 10] > 0.5 then
              UpdateLowMakeUp;
          end;
        1:
          begin
...
procedure TLightweightMultibandCompressorDataModule.
...
procedure TLightweightMultibandCompressorDataModule.VSTModuleProcessMono
...
procedure TLightweightMultibandCompressorDataModule.VSTModuleProcessMonoSoftClip
  (const Inputs, Outputs: TDAVArrayOfSingleFixedArray;
  const SampleFrames: Cardinal);
var
  Sample: Integer;
...
procedure TLightweightMultibandCompressorDataModule.VSTModuleSampleRateChange
...
begin
  ...
  begin
    for Channel := 0 to Length(FLightweightMultibandCompressor) - 1 do
    begin
      if Assigned(FLightweightMultibandCompressor[Channel]) then
        FLightweightMultibandCompressor[Channel].SampleRate := SampleRate;
      if Assigned(FLinkwitzRiley[Channel, 0]) then
        FLinkwitzRiley[Channel, 0].SampleRate := SampleRate;
      if Assigned(FLinkwitzRiley[Channel, 1]) then

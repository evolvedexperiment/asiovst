...
type
  ...
  public
    ...
    procedure ProcessSample(const Input: Single;
      var Low, Band, Notch, High: Single);
    procedure ProcessBlock(Input, Low, Band, Notch, High: PDAVSingleFixedArray;
  ...
  case FFTM of
    ftmSimple:
      begin
        ...
        if FF > 17000 then
          FF1 := 2 * pi * 17000 * FSampleRateInv
        else
          FF1 := 2 * pi * FF * FSampleRateInv;
        CalculateQ;
      end;
    ftmIdeal:
      begin
        // ideal tuning:
        if FF > 17000 then
          FF1 := 2 * sin(pi * 17000 * FSampleRateInv)
        else
          FF1 := 2 * sin(pi * FF * FSampleRateInv);
        CalculateQ;
      end;
  end;
  Changed;
...
procedure TSVF.ProcessSample(const Input: Single;
  var Low, Band, Notch, High: Single);
begin
...
procedure TSVF.ProcessBlock(Input, Low, Band, Notch, High: PDAVSingleFixedArray;
  SampleFrames: Integer);
var
  c: Integer;
begin

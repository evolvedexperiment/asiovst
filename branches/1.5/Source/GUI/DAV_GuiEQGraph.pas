...
{$IFDEF FPC} LCLIntf, LMessages, {$ELSE} Windows, Messages, {$ENDIF}
  Classes, Graphics, Forms, Types, SysUtils, Controls, DAV_Types,
  DAV_FixedPoint,
...
type
  ...
  TGetFilterGainEvent = function(Sender: TObject; const Frequency: Single)
    : Single of object;
  ...
  public
    ...
    procedure ZoomLowerByPixel(StartPixel, EndPixel: Integer); virtual;
      abstract;
    ...
    property UnitPosition: TUnitPosition read FUnitPosition
      write SetUnitPosition default upValue;
  end;
  ...
  public
    ...
    property LabelPosition: TXAxisLabelPosition read FLabelPosition
      write SetLabelPosition default xlpNone;
    property LabelFrequency: TXAxisLabelFrequency read FLabelFrequency
      write SetLabelFrequency default xlfDecade;
  end;
  ...
  public
    ...
    property LabelPosition: TYAxisLabelPosition read FLabelPosition
      write SetLabelPosition default ylpNone;
    property MaximumGridLines: Integer read FMaximumGridLines
      write SetMaximumGridLines default 10;
    property AutoGranularity: Boolean read FAutoGranularity
      write SetAutoGranularity default True;
  end;
  ...
  protected
    ...
    function GetZeroHandler(Sender: TObject; PixelPosition: Integer)
      : TFixed24Dot8;
    function GetValueHandler(Sender: TObject; PixelPosition: Integer)
      : TFixed24Dot8;

    procedure PaintToGraphAntialias(const Graph: TCustomGuiEQGraph;
      const PixelMap: TGuiCustomPixelMap); virtual;
    procedure PaintToGraphDraft(const Graph: TCustomGuiEQGraph;
      const PixelMap: TGuiCustomPixelMap); virtual;
  public
  ...
  published
    ...
    property OnGetFilterGain: TGetFilterGainEvent read FOnGetFilterGain
      write SetOnGetFilterGain;
  end;
  ...
  protected
    ...
    procedure SetItem(Index: Integer;
      const Value: TGuiEQGraphSeriesCollectionItem); virtual;
    procedure Notify(Item: TCollectionItem;
      Action: TCollectionNotification); override;
    property Items[Index: Integer]: TGuiEQGraphSeriesCollectionItem read GetItem
  ...
  protected
    ...
    procedure CMFontChanged(var Message:
      {$IFDEF LCL}TLMessage{$ELSE}TMessage{$ENDIF}); message CM_FONTCHANGED;
  ...
  public
    ...
    property GraphColorDark: TColor read FGraphColorDark write SetGraphColorDark
      default $303030;
    property GraphColorLight: TColor read FGraphColorLight
      write SetGraphColorLight default $606060;
    ...
    property FilterSeries: TGuiEQGraphSeriesCollection read FFilterSeries
      write SetFilterSeries;
...
begin
  ...
  Filer.DefineProperty('LowerFrequency', ReadLowerProperty, WriteLowerProperty,
    LowerFrequency = 0);
  Filer.DefineProperty('UpperFrequency', ReadUpperProperty, WriteUpperProperty,
    UpperFrequency = 0);
end;
...
function TCustomGuiEQGraphXAxis.LogarithmicFrequencyToLinear
  (Value: Double): Double;
begin
  Result := Log2(Value * FInvLower) * FInvLog2Ratio;
end;

function TCustomGuiEQGraphXAxis.LinearToLogarithmicFrequency
  (Value: Double): Double;
begin
  Result := Power(2, Value * FLog2Ratio) * FLower;
end;

function TCustomGuiEQGraphXAxis.FastLogarithmicFrequencyToLinear
  (Value: Single): Single;
begin
  Result := FastLog2MinError3(Value * FInvLower) * FInvLog2Ratio;
end;

function TCustomGuiEQGraphXAxis.FastLinearToLogarithmicFrequency
  (Value: Single): Single;
begin
...
procedure TCustomGuiEQGraphXAxis.SetLabelPosition
  (const Value: TXAxisLabelPosition);
begin
...
begin
  if Value < 1 then
    raise Exception.Create('Value must be larger than 0!');
...
procedure TCustomGuiEQGraphYAxis.SetLabelPosition
  (const Value: TYAxisLabelPosition);
begin
  if FLabelPosition <> Value then
  begin
    FLabelPosition := Value;
...
begin
  ...
  if FLower > FUpper then
    raise Exception.Create('Lower above Upper');
...
procedure TCustomGuiEQGraphYAxis.ZoomLowerByPixel(StartPixel,
  EndPixel: Integer);
var
  Level: Single;
  Ratio: Single;
...
begin
  ...
  else if EndPixel < StartPixel then
  begin
    Level := LevelAtPixel(StartPixel);
    Ratio := FOwner.GraphHeight / ((EndPixel - Trunc(FOwner.BorderWidth)) -
      FOwner.GraphHeight);
...
begin
  ...
  FGranularity := GranularityScale *
    (Trunc(RoughGranularity / GranularityScale) + 1);
...
begin
  if Assigned(FOnGetFilterGain) then
    FESP.GeometricShape.OnGetValue := GetValueHandler
  else
    FESP.GeometricShape.OnGetValue := GetZeroHandler;
end;
...
begin
  if Collection is TGuiEQGraphSeriesCollection then
    TGuiEQGraphSeriesCollection(Collection).Changed;
end;
...
begin
  ...
  with TCustomGuiEQGraph(Collection.Owner) do
  begin
    Result := ConvertToFixed24Dot8((YAxis.UpperLevel * YAxis.RangeReciprocal) *
      GraphHeight);
  end;
end;
...
function TGuiEQGraphSeriesCollection.GetItem(Index: Integer)
  : TGuiEQGraphSeriesCollectionItem;
begin
  Result := TGuiEQGraphSeriesCollectionItem(inherited GetItem(Index));
end;

function TGuiEQGraphSeriesCollection.Insert(Index: Integer)
  : TGuiEQGraphSeriesCollectionItem;
begin
...
begin
  if Owner is TCustomGuiEQGraph then
    TCustomGuiEQGraph(Owner).BufferChanged;
end;
...
procedure TCustomGuiEQGraph.SetFilterSeries(const Value
  : TGuiEQGraphSeriesCollection);
begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                // eventually add unit
                if FXAxis.UnitPosition = upValue then
                  Txt := Txt + ' Hz';

                TextSize := FGuiFont.TextExtent(Txt);
                h := Round(Rct.Bottom - TextSize.cy - 1);
                FGuiFont.TextOut(Txt, PixelMap,
                  Round(Rct.Left + FXAxis.LogarithmicFrequencyToLinear(j * Base)
                  * Wdth) - (TextSize.cx div 2), h);
              end;
              Inc(j);
              if j >= 10 then
              begin
        ...
        begin
          ...
          begin
            ...
            with FGuiFont.Shadow do
              if Visible then
                Base := Base + Max(0, Round(Blur - OffsetX));

            FGuiFont.TextOut(Txt, PixelMap, Base,
              Round(Rct.Bottom - i - 0.5 * TextSize.cy));
            Temp := Temp + Granularity;
          end;
        end;
      ylpRight:
        ...
        begin
          ...
          begin
            ...
            FGuiFont.TextOut(Txt, PixelMap,
              Round(Rct.Right - 1 - 0.5 * TextSize.cx),
              Round(Rct.Bottom - i - 0.5 * TextSize.cy));
            Temp := Temp + Granularity;
          end;
        end;
...
begin
  if FAntiAlias then
    for SeriesIndex := 0 to FFilterSeries.Count - 1 do
      FFilterSeries[SeriesIndex].PaintToGraphAntialias(Self, PixelMap)
  else
    for SeriesIndex := 0 to FFilterSeries.Count - 1 do
      FFilterSeries[SeriesIndex].PaintToGraphDraft(Self, PixelMap)
end;
...
begin
  ...
  for SeriesIndex := 0 to FFilterSeries.Count - 1 do
    FFilterSeries[SeriesIndex].DataCacheChanged;
end;
...
procedure TCustomGuiEQGraph.CMFontChanged(var Message:
  {$IFDEF LCL}TLMessage{$ELSE}TMessage{$ENDIF});
begin
...
begin
  ...
  begin
    ...
    begin
      if (maZoom in YAxis.FMouseActions) and (ssCtrl in Shift) then
        YAxis.ZoomLowerByPixel(FLastMouseCursorPos.Y, Y)
      else if (maShift in YAxis.FMouseActions) then
        YAxis.ShiftByPixel(Y - FLastMouseCursorPos.Y);
    end;

    if (Abs(X - FLastMouseCursorPos.X) > 0) then
    begin
      if (maZoom in YAxis.FMouseActions) and (ssCtrl in Shift) then
        XAxis.ZoomAroundPixel(X, Power(1.01, (X - FLastMouseCursorPos.X)))
      else if (maShift in YAxis.FMouseActions) then
        XAxis.ShiftByPixel(X - FLastMouseCursorPos.X);
    end;
  end;
...
begin
  if (maZoom in YAxis.FMouseActions) and (MousePos.X < 32) then
    YAxis.ZoomAroundPixel(MousePos.Y, Power(1.01, WheelDelta));

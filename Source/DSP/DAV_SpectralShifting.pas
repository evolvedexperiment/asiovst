...
begin
  ...
  begin
    ...
    Bin := 1;
    Move(Spectum^[Bin], Spectum^[FBinShift + 1], (Fft.BinCount - FBinShift - 2)
      * SizeOf(TComplex32));
  ...
  else if FBinShift < 0 then
  begin
    // build new DC
    Spectum^[0].Re := Sqrt(Sqr(Spectum^[-FBinShift].Re) +
      Sqr(Spectum^[-FBinShift].Im));

    Bin := 1;
    Move(Spectum^[-FBinShift + 1], Spectum^[Bin], (Fft.BinCount + FBinShift - 2)
      * SizeOf(TComplex32));
    ...
    // fill gap with zero
    FillChar(Spectum^[(Fft.BinCount + FBinShift)],
      (-FBinShift - 1) * SizeOf(TComplex32), 0);
  end;
end;

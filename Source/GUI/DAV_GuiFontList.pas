...
type
  ...
  protected
    ...
    procedure SetItem(Index: Integer;
      const Value: TGuiCustomFontCollectionItem); virtual;
    procedure Notify(Item: TCollectionItem;
      Action: TCollectionNotification); override;
    property Items[Index: Integer]: TGuiCustomFontCollectionItem read GetItem
  ...
  public
    ...
    property OnFontClassChanged: TNotifyEvent read FOnFontClassChanged
      write FOnFontClassChanged;
  end;
  ...
  protected
    ...
    property Items[Index: Integer]: TGuiCustomFontCollectionItem
  ...
  protected
    ...
    property Items[Index: Integer]: TGuiFontCollectionItem
      read GetItems; default;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
  published
    property Fonts: TGuiFontCollection read FFontCollection
      write FFontCollection;
  end;
...
function TGuiFontCollection.GetItem(Index: Integer)
  : TGuiCustomFontCollectionItem;
begin
  Result := TGuiCustomFontCollectionItem(inherited GetItem(Index));
end;

function TGuiFontCollection.Insert(Index: Integer)
  : TGuiCustomFontCollectionItem;
begin
...
begin
  if Assigned(FOnFontClassChanged) then
    FOnFontClassChanged(Self);
end;
...
begin
  ...
  begin
    if FLinkedControls.Remove(Control) <> 0 then
...
begin
  if Assigned(FLinkedControls) then
    while FLinkedControls.Count > 0 do
      UnLinkControl(TControl(FLinkedControls[0]));
...
begin
  for Index := 0 to FLinkedControls.Count - 1 do
    with TControl(FLinkedControls[Index]) do
      Perform(GM_FontChanged, 1, Integer(Self));
end;
...
begin
  if (Value <> '') and (FFont.ClassName <> Value) and Assigned(FontClassList)
  then
  begin
...
function TGuiCustomFontList.GetItems(Index: Integer)
  : TGuiCustomFontCollectionItem;
begin
...
begin
  Assert(Assigned(FLinkedControls));
  while FLinkedControls.Count > 0 do
    TControl(FLinkedControls[0]).Perform(GM_FontListChanged, 0, 0);
end;

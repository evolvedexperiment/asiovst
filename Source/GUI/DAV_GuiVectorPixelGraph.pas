...
begin
  // ensure a geometric shape is assigned
  if not Assigned(GeometricShape.OnGetValue) then
    Exit;
  ...
  begin
    ...
    for PtIndex := 0 to Length(YValues) - 1 do
      YValues[PtIndex].Fixed := FixedAdd(GeometricShape.OnGetValue(Self,
        PtIndex - IntegerRadiusX), ConvertToFixed24Dot8(GeometricShape.MarginTop)
        ).Fixed;

    for x := GeometricShape.MarginLeft to Width -
      GeometricShape.MarginRight - 1 do
    begin
      // get next value
      YValues[Length(YValues) - 1].Fixed := FixedAdd(GeometricShape.OnGetValue(
        Self, x + IntegerRadiusX), ConvertToFixed24Dot8(GeometricShape.MarginTop)
        ).Fixed;
      ...
      if SolidRange[0] < GeometricShape.MarginTop then
        YRange[0] := GeometricShape.MarginTop
      else
        YRange[0] := SolidRange[0];

      if SolidRange[1] > Height - GeometricShape.MarginBottom - 1 then
        YRange[1] := Height - GeometricShape.MarginBottom - 1
      else
...
begin
  ...
  begin
    ...
    try
      ...
      // fill additional points
      for PtIndex := 0 to Length(FYValues) - 1 do
        FYValues[PtIndex] := ConvertFromFixed24Dot8
          (GeometricShape.OnGetValue(Self, PtIndex - IntegerRadiusX));

      for x := GeometricShape.MarginLeft to Width -
        GeometricShape.MarginRight - 1 do
      begin
        // get next value
        FYValues[Length(FYValues) - 1] :=
          ConvertFromFixed24Dot8(GeometricShape.OnGetValue(Self,
          x + IntegerRadiusX - GeometricShape.MarginLeft));
        ...
        // copy line to pixel map
        for y := GeometricShape.MarginTop to Height -
          GeometricShape.MarginBottom - 1 do
        ...
        // shift y-values
        Move(FYValues[1], FYValues[0], (Length(FYValues) - 1) *
          SizeOf(Single));
      end;
    finally
...
function TGuiPixelEquallySpacedPolyline.GetGeometricShape

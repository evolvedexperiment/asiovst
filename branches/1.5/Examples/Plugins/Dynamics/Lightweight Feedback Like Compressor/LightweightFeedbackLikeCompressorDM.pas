...
begin
  ...
  begin
    FLightweightFeedbackLikeCompressor[Channel] :=
      TLightweightSoftKneeFeedbackLikeCompressor.Create;
...
procedure TLightweightFeedbackLikeCompressorDataModule.VSTModuleClose
  (Sender: TObject);
begin
...
procedure TLightweightFeedbackLikeCompressorDataModule.VSTModuleEditOpen(
...
procedure TLightweightFeedbackLikeCompressorDataModule.
  ParameterMakeUpGainDisplay(Sender: TObject; const Index: Integer;
...
begin
  ...
  begin
    ...
    FLightweightFeedbackLikeCompressor[1].MakeUpGain_dB :=
      FLightweightFeedbackLikeCompressor[0].MakeUpGain_dB;
  end;

  // update GUI
  if EditorForm is TFmLightweightFeedbackLikeCompressor then
    TFmLightweightFeedbackLikeCompressor(EditorForm).UpdateMakeUp;
end;
...
begin
  ...
  // update GUI
  if EditorForm is TFmLightweightFeedbackLikeCompressor then
    TFmLightweightFeedbackLikeCompressor(EditorForm).UpdateStereo;
end;
...
begin
  ...
  // update GUI
  if EditorForm is TFmLightweightFeedbackLikeCompressor then
    TFmLightweightFeedbackLikeCompressor(EditorForm).UpdateLimit;
end;
...
procedure TLightweightFeedbackLikeCompressorDataModule.
  ParameterAutoMakeUpGainChange(Sender: TObject; const Index: Integer;
  var Value: Single);
begin
  if Assigned(FLightweightFeedbackLikeCompressor[0]) then
  begin
    FLightweightFeedbackLikeCompressor[0].AutoMakeUp := Boolean(Round(Value));
    if Assigned(FLightweightFeedbackLikeCompressor[1]) then
      FLightweightFeedbackLikeCompressor[1].AutoMakeUp :=
        FLightweightFeedbackLikeCompressor[0].AutoMakeUp;
  end;

  // update GUI
  if EditorForm is TFmLightweightFeedbackLikeCompressor then
    TFmLightweightFeedbackLikeCompressor(EditorForm).UpdateAutoMakeUpGain;
end;

function TLightweightFeedbackLikeCompressorDataModule.
  GetLightweightFeedbackLikeCompressor(Index: Integer): TCustomCompressor;
begin
  if Index in [0 .. Length(FLightweightFeedbackLikeCompressor) - 1] then
    Result := FLightweightFeedbackLikeCompressor[Index]
  else
...
begin
  ...
  begin
    ...
    if Assigned(FLightweightFeedbackLikeCompressor[1]) then
      FLightweightFeedbackLikeCompressor[1].Attack := FLightweightFeedbackLikeCompressor[0].Attack;
  end;

  // update GUI
  if EditorForm is TFmLightweightFeedbackLikeCompressor then
    TFmLightweightFeedbackLikeCompressor(EditorForm).UpdateAttack;
end;
...
begin
  ...
  begin
    ...
    if Assigned(FLightweightFeedbackLikeCompressor[1]) then
      FLightweightFeedbackLikeCompressor[1].Release := FLightweightFeedbackLikeCompressor[0].Release;
  end;

  // update GUI
  if EditorForm is TFmLightweightFeedbackLikeCompressor then
    TFmLightweightFeedbackLikeCompressor(EditorForm).UpdateRelease;
end;
...
begin
  ...
  begin
    ...
    if Assigned(FLightweightFeedbackLikeCompressor[1]) then
      FLightweightFeedbackLikeCompressor[1].Threshold_dB := Value;
  end;

  // update GUI
  if EditorForm is TFmLightweightFeedbackLikeCompressor then
    TFmLightweightFeedbackLikeCompressor(EditorForm).UpdateThreshold;
end;
...
begin
  ...
  begin
    ...
    if Assigned(FLightweightFeedbackLikeCompressor[1]) then
      FLightweightFeedbackLikeCompressor[1].Ratio := Value;
  end;

  // update GUI
  if EditorForm is TFmLightweightFeedbackLikeCompressor then
    TFmLightweightFeedbackLikeCompressor(EditorForm).UpdateRatio;
end;
...
begin
  ...
  begin
    ...
    if Assigned(FLightweightFeedbackLikeCompressor[1]) then
      FLightweightFeedbackLikeCompressor[1].Knee_dB := Value;
  end;

  // update GUI
  if EditorForm is TFmLightweightFeedbackLikeCompressor then
    TFmLightweightFeedbackLikeCompressor(EditorForm).UpdateKnee;
end;

procedure TLightweightFeedbackLikeCompressorDataModule.VSTModuleProcessStereo
...
procedure TLightweightFeedbackLikeCompressorDataModule.VSTModuleProcessMono
  (const Inputs, Outputs: TDAVArrayOfSingleFixedArray;
  const SampleFrames: Cardinal);
var
  Sample: Integer;
...
procedure TLightweightFeedbackLikeCompressorDataModule.
  VSTModuleProcessStereoSoftClip(const Inputs,
...
procedure TLightweightFeedbackLikeCompressorDataModule.
  VSTModuleProcessMonoSoftClip(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
  Sample: Integer;
  Temp: Single;
...
procedure TLightweightFeedbackLikeCompressorDataModule.VSTModuleSampleRateChange
  (Sender: TObject; const SampleRate: Single);
begin
  if Abs(SampleRate) <> 0 then
  begin
    if Assigned(FLightweightFeedbackLikeCompressor[0]) then
      FLightweightFeedbackLikeCompressor[0].SampleRate := SampleRate;
    if Assigned(FLightweightFeedbackLikeCompressor[1]) then
      FLightweightFeedbackLikeCompressor[1].SampleRate := SampleRate;
    if EditorForm is TFmLightweightFeedbackLikeCompressor then

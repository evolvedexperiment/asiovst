...
procedure TTanhWaveshaperModule.ParameterOrderChange(Sender: TObject;
...
begin
  for Channel := 0 to Length(FOversampler) - 1 do
  begin
    if Assigned(FOversampler[Channel].Upsampling) then
      FOversampler[Channel].Upsampling.Order := Round(Value);
    if Assigned(FOversampler[Channel].Downsampling) then
      FOversampler[Channel].Downsampling.Order := Round(Value);
  end;
end;
...
function TTanhWaveshaperModule.FastTanhMinError2Waveshaper
  (Input: Double): Double;
begin
  Result := FastTanhMinError2(FGain * Input);
end;

function TTanhWaveshaperModule.FastTanhContinousError2Waveshaper
  (Input: Double): Double;
begin
  Result := FastTanhContinousError2(FGain * Input);
end;

function TTanhWaveshaperModule.FastTanhMinError3Waveshaper
  (Input: Double): Double;
begin
  Result := FastTanhMinError3(FGain * Input);
end;

function TTanhWaveshaperModule.FastTanhContinousError3Waveshaper
  (Input: Double): Double;
begin
  Result := FastTanhContinousError3(FGain * Input);
end;

function TTanhWaveshaperModule.FastTanhMinError4Waveshaper
  (Input: Double): Double;
begin
  Result := FastTanhMinError4(FGain * Input);
end;

function TTanhWaveshaperModule.FastTanhContinousError4Waveshaper
  (Input: Double): Double;
begin
  Result := FastTanhContinousError4(FGain * Input);
end;

function TTanhWaveshaperModule.FastTanhMinError5Waveshaper
  (Input: Double): Double;
begin
  Result := FastTanhMinError5(FGain * Input);
end;

function TTanhWaveshaperModule.FastTanhContinousError5Waveshaper
  (Input: Double): Double;
begin
...
begin
  ...
  Result := Result *
    (1 + Value * (0.69314718 + Value * (0.22741128 + Value * 0.079441542)));
end;
...
begin
  Input := 2 * FGain * Input;
  Result := Input / (Abs(Input) + 4.3448473351 / (0.11932825377 - 0.15402606842
    * Abs(Input) + sqr(Input)));
end;
...
begin
  ...
  Result := Input / (3.8566590621 + 0.11306387017 * sqr(Input)) + Input /
    (0.59265249338 + 0.79772974129 * sqr(Input))
end;
  ...
  try
    case Round(Parameter[Index]) of
      0:
        FTanhWaveShaper := TanhWaveshaperMath;
      1:
        FTanhWaveShaper := TanhWaveshaper;
      2:
        FTanhWaveShaper := TanhOpt3Waveshaper;
      3:
        FTanhWaveShaper := TanhOpt4Waveshaper;
      4:
        FTanhWaveShaper := TanhOpt5Waveshaper;
      5:
        FTanhWaveShaper := TanhOpt6Waveshaper;
      6:
        FTanhWaveShaper := TanhOpt7Waveshaper;
      7:
        FTanhWaveShaper := FastTanhContinousError2Waveshaper;
      8:
        FTanhWaveShaper := FastTanhContinousError3Waveshaper;
      9:
        FTanhWaveShaper := FastTanhContinousError4Waveshaper;
      10:
        FTanhWaveShaper := FastTanhContinousError5Waveshaper;
      11:
        FTanhWaveShaper := FastTanhMinError2Waveshaper;
      12:
        FTanhWaveShaper := FastTanhMinError3Waveshaper;
      13:
        FTanhWaveShaper := FastTanhMinError4Waveshaper;
      14:
        FTanhWaveShaper := FastTanhMinError5Waveshaper;
      15:
        FTanhWaveShaper := TanhLike1Waveshaper;
      16:
        FTanhWaveShaper := TanhLike2Waveshaper;
      17:
        FTanhWaveShaper := TanhLike3Waveshaper;
      18:
        FTanhWaveShaper := TanhLike4Waveshaper;
      19:
        FTanhWaveShaper := FastTanhNollock3Waveshaper;
      20:
        FTanhWaveShaper := FastTanhMrToast3TermWaveshaper;
      21:
        FTanhWaveShaper := FastTanhMrToast4TermWaveshaper;
      22:
        FTanhWaveShaper := FastTanhToguAudioLineWaveshaper;
      23:
        FTanhWaveShaper := FastTanhTALtoastWaveshaper;
      24:
        FTanhWaveShaper := FastTanhOhmforceWaveshaper;
    end;
  finally
...
procedure TTanhWaveshaperModule.ParameterTypeDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  case Round(Parameter[Index]) of
    0:
      PreDefined := 'Tanh (Math)';
    1:
      PreDefined := 'Tanh (DAV_Types)';
    2:
      PreDefined := 'FastTanhOpt3Term';
    3:
      PreDefined := 'FastTanhOpt4Term';
    4:
      PreDefined := 'FastTanhOpt5Term';
    5:
      PreDefined := 'FastTanhOpt6Term';
    6:
      PreDefined := 'FastTanhOpt7Term';
    7:
      PreDefined := 'FastTanhContinousError2Waveshaper';
    8:
      PreDefined := 'FastTanhContinousError3Waveshaper';
    9:
      PreDefined := 'FastTanhContinousError4Waveshaper';
    10:
      PreDefined := 'FastTanhContinousError5Waveshaper';
    11:
      PreDefined := 'FastTanhMinError2Waveshaper';
    12:
      PreDefined := 'FastTanhMinError3Waveshaper';
    13:
      PreDefined := 'FastTanhMinError4Waveshaper';
    14:
      PreDefined := 'FastTanhMinError5Waveshaper';
    15:
      PreDefined := 'TanhLike1Waveshaper';
    16:
      PreDefined := 'TanhLike2Waveshaper';
    17:
      PreDefined := 'TanhLike3Waveshaper';
    18:
      PreDefined := 'TanhLike4Waveshaper';
    19:
      PreDefined := 'Nollock';
    20:
      PreDefined := 'mistertoast 3-Term';
    21:
      PreDefined := 'mistertoast 4-Term';
    22:
      PreDefined := 'ToguAudioLine';
    23:
      PreDefined := 'TAL vs. Mr.Toast';
    24:
      PreDefined := 'Ohmforce';
  end;
end;

procedure TTanhWaveshaperModule.ParameterIntegerDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := AnsiString(IntToStr(Round(Parameter[Index])));
end;

procedure TTanhWaveshaperModule.ParameterGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
...
procedure TTanhWaveshaperModule.ParameterOversamplingChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  ...
  try
    ...
    begin
      if Assigned(FOversampler[Channel].Upsampling) then
        FOversampler[Channel].Upsampling.Factor := FOSFactor;
      if Assigned(FOversampler[Channel].Downsampling) then
        FOversampler[Channel].Downsampling.Factor := FOSFactor;
    end;

    if FOSFactor = 1 then
      OnProcess := VSTModuleProcess
    else
      OnProcess := VSTModuleProcessOversampled;
...
begin
  ...
  try
    for Sample := 0 to SampleFrames - 1 do
      for Channel := 0 to Length(FOversampler) - 1 do
        Outputs[Channel, Sample] := FTanhWaveShaper(Inputs[Channel, Sample]);
  finally
    FCriticalSection.Leave;
  end;
end;
...
begin
  ...
  try
    for Sample := 0 to SampleFrames - 1 do
      for Channel := 0 to Length(FOversampler) - 1 do
      begin
        FOversampler[Channel].Upsampling.Upsample64
          (Inputs[Channel, Sample], @Temp);

        for OScnt := 0 to FOSFactor - 1 do
          Temp[OScnt] := FTanhWaveShaper(Temp[OScnt]);

        Outputs[Channel, Sample] := FOversampler[Channel]
          .Downsampling.Downsample64(@Temp);
      end;
  finally
    FCriticalSection.Leave;
  end;
...
begin
  if Abs(SampleRate) > 0 then
    ...
    begin
      ...
      try
        if Assigned(FOversampler[Channel].Upsampling) then
          FOversampler[Channel].Upsampling.SampleRate := Abs(SampleRate);
        if Assigned(FOversampler[Channel].Downsampling) then
          FOversampler[Channel].Downsampling.SampleRate := Abs(SampleRate);
      finally
        FCriticalSection.Leave;
      end;
    end;

...
begin
  ...
  for Band := 0 to Length(FFilter) - 1 do
    FFilter[Band] := TBasicGainFilter.Create;
...
begin
  for Band := 0 to Length(FDelayLine) - 1 do
    FreeAndNil(FDelayLine[Band]);
  for Band := 0 to Length(FFilter) - 1 do
    FreeAndNil(FFilter[Band]);
end;

procedure TPartyDelayDataModule.ParameterPanChange(Sender: TObject;
...
begin
  ...
  if EditorForm is TFmPartyDelay then
    TFmPartyDelay(EditorForm).UpdatePan;
end;

procedure TPartyDelayDataModule.ParameterPolarityDisplay(Sender: TObject;
...
procedure TPartyDelayDataModule.ParameterShiftFrequencyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  ...
  begin
    ...
    if Assigned(FFreqShift[Band]) then
      FFreqShift[Band].Frequency := Value;
  end;

  if EditorForm is TFmPartyDelay then
    TFmPartyDelay(EditorForm).UpdateShiftFrequency;
end;

// Limit a Value to be Lower <= Value <= Upper
function LimitInt(const Value: Integer; Lower: Integer = 0;
  Upper: Integer = 1): Integer;
begin
  if Value < Lower then
    Result := Lower
...
procedure TPartyDelayDataModule.ParameterShiftOrderChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  ...
  begin
    ...
    try
      ...
      if Assigned(FFreqShift[Band]) then
        FFreqShift[Band].CoefficientCount := FShiftOrder[Band];
    finally
...
procedure TPartyDelayDataModule.ParameterMixChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FWet := 0.01 * Value;
  FDry := Sqrt(1 - FWet);
  ...
  if EditorForm is TFmPartyDelay then
    TFmPartyDelay(EditorForm).UpdateMix;
end;
...
begin
  ...
  if EditorForm is TFmPartyDelay then
    TFmPartyDelay(EditorForm).UpdateActive;
end;

procedure TPartyDelayDataModule.ParameterDriveChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  ...
  if EditorForm is TFmPartyDelay then
    TFmPartyDelay(EditorForm).UpdateDrive;
end;

procedure TPartyDelayDataModule.ParameterBalanceChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  ...
  if EditorForm is TFmPartyDelay then
    TFmPartyDelay(EditorForm).UpdateBalance;
end;
...
procedure TPartyDelayDataModule.ParameterBandwidthChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  ...
  if Assigned(FFilter[Band]) then
    FFilter[Band].BandWidth := Value;

  if EditorForm is TFmPartyDelay then
    TFmPartyDelay(EditorForm).UpdateFilterBandwidth;
end;

procedure TPartyDelayDataModule.ParameterFilterGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  Band := Index div ParametersPerBand;
  if Assigned(FFilter[Band]) then
    FFilter[Band].Gain := Value;

  if EditorForm is TFmPartyDelay then
    TFmPartyDelay(EditorForm).UpdateFilterGain;
end;

procedure TPartyDelayDataModule.ParameterFilterFreqChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  Band := Index div ParametersPerBand;
  if Assigned(FFilter[Band]) then
    FFilter[Band].Frequency := Value;

  if EditorForm is TFmPartyDelay then
    TFmPartyDelay(EditorForm).UpdateFilterFrequency;
end;

procedure TPartyDelayDataModule.ParameterFrequencyShiftChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
  Cmplx: TComplex32;
begin
  ...
  else if Assigned(FFreqShift[Band]) then
    FreeAndNil(FFreqShift[Band]);

  if EditorForm is TFmPartyDelay then
    TFmPartyDelay(EditorForm).UpdateFrequencyShifter;
end;

procedure TPartyDelayDataModule.ParameterFilterDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
...
procedure TPartyDelayDataModule.ParameterFilterChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
...
begin
  ...
  if EditorForm is TFmPartyDelay then
    TFmPartyDelay(EditorForm).UpdateFilterType;
end;

procedure TPartyDelayDataModule.ParameterDelayChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Band: Integer;
begin
  ...
  if EditorForm is TFmPartyDelay then
    TFmPartyDelay(EditorForm).UpdateDelay;
end;

procedure TPartyDelayDataModule.ParameterFeedbackChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FFeedback[Index div ParametersPerBand] := 0.01 * Value;

  if EditorForm is TFmPartyDelay then
    TFmPartyDelay(EditorForm).UpdateFeedback;
end;

procedure TPartyDelayDataModule.ParameterPolarityChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Polarity: Boolean;
begin
  ...
  if EditorForm is TFmPartyDelay then
    TFmPartyDelay(EditorForm).UpdateInvert;
end;

procedure TPartyDelayDataModule.ParameterGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Gain: Single;
begin
  ...
  if EditorForm is TFmPartyDelay then
    TFmPartyDelay(EditorForm).UpdateGain;
end;
...
begin
  ...
  begin
    for Band := 0 to Length(FDelayLine) - 1 do
      if Assigned(FDelayLine[Band]) then
        FDelayLine[Band].SampleRate := SampleRate;
    ...
    for Band := 0 to Length(FFreqShift) - 1 do
      if Assigned(FFreqShift[Band]) then
        FFreqShift[Band].SampleRate := SampleRate;
  end;
end;
      ...
      begin
        ...
        try
          FLastOutput[Band] := FScale[Band, 0] * Inputs[0, Sample] +
            FScale[Band, 1] * Inputs[1, Sample] + FFeedback[Band] *
            FLastOutput[Band];

          if Assigned(FFilter[Band]) then
            FLastOutput[Band] := FFilter[Band].ProcessSample64
              (FLastOutput[Band]);
          ...
          if FDrive[Band] > 0 then
            FLastOutput[Band] := FastTanhContinousError4
              ((1 + FDrive[Band]) * FLastOutput[Band]);

          FLastOutput[Band] := FDelayLine[Band].ProcessSample32
            (FLastOutput[Band]);
        finally
          FCriticalSection.Leave;
        end;

...
type
  TDetuneDataModule = class(TVSTModule)
    ...
    procedure ParamDetuneDisplay(Sender: TObject; const Index: Integer; var PreDefined: AnsiString);
...
procedure TDetuneDataModule.ParamOutputChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
...
procedure TDetuneDataModule.ParamMixChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FMix := 0.01 * Value;
  MixChanged;
...
procedure TDetuneDataModule.DetuneDataModulePrograms1Initialize
  (Sender: TObject);
begin
  Programs[1].Parameter[0] := 0.20
end;

procedure TDetuneDataModule.DetuneDataModulePrograms2Initialize
  (Sender: TObject);
begin
...
begin
  ...
  begin
    ...
    p2 := p2 - d2;
    // repeat for downwards shift - can't see a more efficient way?
    if (p2 < 0) then
      p2 := p2 + lf; // output

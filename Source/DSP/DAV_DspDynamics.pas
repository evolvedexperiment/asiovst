...
{$ENDIF}
  ...
  TCustomDynamicProcessor = class(TDspPersistent, IDspProcessor32,
    IDspProcessor64)
  private
    function GetKnee_dB: Double;
  ...
  public
    ...
    function TranslatePeakToGain(const PeakLevel: Double): Double;
      virtual; abstract;
    ...
    property Threshold_dB: Double read FThreshold_dB write SetThreshold;
    // in dB
    property Knee_dB: Double read GetKnee_dB write SetKnee_dB;
  end;
...
uses
  SysUtils, Math, DAV_Common, DAV_Math, DAV_Approximations, DAV_Convert,
  DAV_Consts;
...
function TCustomDynamicProcessor.CharacteristicCurve(const InputLevel
  : Double): Double;
begin
  Result := TranslatePeakToGain(Abs(InputLevel)) * InputLevel;
end;

function TCustomDynamicProcessor.CharacteristicCurve_dB(const InputLevel_dB
  : Double): Double;
begin
...
procedure TCustomDynamicProcessor.ProcessBlock32
...
procedure TCustomDynamicProcessor.ProcessBlock64
...
begin
  Result := Power((PeakLevel * Power(Power(PeakLevel, FSoftKnee[1]) +
    FKneedThreshold, -FSoftKnee[0])),
    1 / ((Abs(PeakLevel - FThreshold) + (FSoftKnee[0] + PeakLevel -
    FThreshold))));
end;
...
begin
  ...
  FThresholdReciprocal := 1 / FThreshold;
  if FAutoMakeUp then
    FMakeUpGain := FThresholdReciprocal;
  Changed;
end;
...
procedure TCustomBrickwallLimiter.ProcessBlock32
  (const Data: PDAVSingleFixedArray; SampleCount: Integer);
var
  Sample: Integer;
begin
...
procedure TCustomBrickwallLimiter.ProcessBlock64
  (const Data: PDAVDoubleFixedArray; SampleCount: Integer);
var
  Sample: Integer;
begin
...
begin
  for Sample := 0 to SampleCount - 1 do
  begin
    if Data[Sample] > FThreshold then
      Data[Sample] := FThreshold
    else if Data[Sample] < -FThreshold then
      Data[Sample] := -FThreshold;
...
begin
  if PeakLevel > FThreshold then
    Result := FThreshold / PeakLevel
  else
    Result := 1;
  Result := FMakeUpGain * Result;
end;

function TBrickwallLimiter.CharacteristicCurve(const InputLevel
  : Double): Double;
begin
  if InputLevel > FThreshold then
    Result := FThreshold
  else if InputLevel < -FThreshold then
    Result := -FThreshold
  else
    Result := InputLevel;
  Result := FMakeUpGain * Result;
...
function TSoftBrickwallLimiter.TranslatePeakToGain(const PeakLevel
  : Double): Double;
var
  a, b: Double;
begin
...
function TSimpleSoftBrickwallLimiter.TranslatePeakToGain(const PeakLevel
  : Double): Double;
begin
  Result := Power(1 + Power(PeakLevel * FThresholdReciprocal, FSoftKnee[1]),
    -FSoftKnee[0]);
end;
...
begin
  if FAttack = 0 then
    ...
    FAttackFactor := 1 - Exp(-ln2 / (FAttack * 0.001 * SampleRate));
end;
...
begin
  if FRelease = 0 then
    ...
    FReleaseFactor := Exp(-ln2 / (FRelease * 0.001 * SampleRate));
end;
...
begin
  if Abs(Input) > FPeak then
    FPeak := FPeak + (Abs(Input) - FPeak) * FAttackFactor
  else
    FPeak := Abs(Input) + (FPeak - Abs(Input)) * FReleaseFactor;
...
begin
  ...
  if FPeak > FThreshold then
    Result := Input * FMakeUpGain * FThreshold / FPeak
  else
    Result := Input * FMakeUpGain;
end;
...
procedure TSimpleSoftKneeLimiter.ProcessBlock32
  (const Data: PDAVSingleFixedArray; SampleCount: Integer);
var
  Sample: Integer;
begin
...
procedure TSimpleSoftKneeLimiter.ProcessBlock64
  (const Data: PDAVDoubleFixedArray; SampleCount: Integer);
var
  Sample: Integer;
begin
...
begin
  ...
  Result := Input * Power2(-FSoftKnee[0] *
    Log2(1 + Power2(FSoftKnee[1] * Log2(CDenorm32 + FPeak *
    FThresholdReciprocal))));
end;
...
begin
  ...
  Result := Input * Power2(-FSoftKnee[0] *
    Log2(1 + Power2(FSoftKnee[1] * Log2(CDenorm64 + FPeak *
    FThresholdReciprocal))));
end;

function TSimpleSoftKneeLimiter.TranslatePeakToGain(const PeakLevel
  : Double): Double;
begin
  Result := Power(1 + Power(PeakLevel * FThresholdReciprocal, FSoftKnee[1]),
    -FSoftKnee[0]);
...
function TClassicSoftRangeGate.TranslatePeakToGain(const PeakLevel
  : Double): Double;
(*
  const
  cScale = 1 / 100;
...
function TClassicSoftKneeGate.TranslatePeakToGain(const PeakLevel
  : Double): Double;
begin
...
begin
  if Abs(Input) > FPeak then
    FPeak := FPeak + (Abs(Input) - FPeak) * FAttackFactor
  else if FHoldSmplCnt > FHoldSamples then
    FPeak := Abs(Input) + (FPeak - Abs(Input)) * FReleaseFactor
  else
    inc(FHoldSmplCnt);
...
begin
  if PeakLevel < FThreshold then
    Result := Power(FThreshold, 1 - FRatio) * Power(PeakLevel, FRatio - 1) *
      (1 - FRangeFactor) + FRangeFactor
  else
  begin
...
begin
  inherited InputSample(FHighCut.ProcessSample64
    (FLowCut.ProcessSample64(Input)));
end;
...
begin
  ...
  Result := Power((ScaledPeakLevel + FRangeThresholdFactor) /
    (ScaledPeakLevel + FThresholdKneeFactor), FSoftKnee[0]);
  if PeakLevel > FThreshold then
    FHoldSmplCnt := 0; // start hold phase
end;
...
begin
  ...
  if FAutoMakeUp then
    ...
    FMakeUpGain[0] := dB_to_Amp(FMakeUpGain_dB);
...
begin
  if PeakLevel < FThreshold then
    Result := FMakeUpGain[0]
  else
    Result := FMakeUpGain[1] * Power(PeakLevel, FRatio - 1);
end;
...
function TSoftKneeCompressor.TranslatePeakToGain(const PeakLevel
  : Double): Double;
begin
  if PeakLevel < FThreshold then
    Result := FMakeUpGain[0]
...
begin
  if FPreviousAbsSample > FPeak then
    FPeak := FPeak + (FPreviousAbsSample - FPeak) * FAttackFactor
  else
    FPeak := FPreviousAbsSample + (FPeak - FPreviousAbsSample) * FReleaseFactor;
...
begin
  if FAttack = 0 then
    ...
    FAttackFactor := 1 -
      Exp(-ln2 / (FAttack * 0.001 * SampleRate * FRatioReciprocal));
end;
...
begin
  if FRelease = 0 then
    ...
    FReleaseFactor :=
      Exp(-ln2 / (FRelease * 0.001 * SampleRate * FRatioReciprocal));
end;
...
begin
  ...
  if FAutoMakeUp then
    ...
    FMakeUpGains[0] := dB_to_Amp(FMakeUpGain_dB);
  FMakeUpGains[1] := Power(FThreshold, (1 - FRatio) * FRatioReciprocal);
end;

function TSimpleFeedbackCompressor.TranslatePeakToGain(const PeakLevel
  : Double): Double;
begin
  if PeakLevel < FThreshold then
    Result := 1
  else
    Result := FMakeUpGains[1] * Power(PeakLevel, 1 - FRatioReciprocal);
end;

function TSimpleFeedbackCompressor.CharacteristicCurve(const InputLevel
  : Double): Double;
begin
  Result := FMakeUpGains[0] * Power(TranslatePeakToGain(Abs(InputLevel)),
    FRatio) * InputLevel;
end;

{ TSoftKneeFeedbackCompressor }

function TSoftKneeFeedbackCompressor.TranslatePeakToGain(const PeakLevel
  : Double): Double;
(*
  var
  a, b, c : Double;
...
begin
  ...
  if i > FRMSSize then
    FillChar(FRMSBuffer[FRMSSize], (i - FRMSSize) * SizeOf(Double), 0);
  FRMSSize := i;
  FRMSFactor := 1 / FRMSSize;
  if FRMSPos > FRMSSize then
    FRMSPos := 0;
end;
...
initialization
  RegisterDspProcessors32([TSimpleDirectGate, TSoftDirectGate, TBrickwallLimiter,
    TSoftBrickwallLimiter, TLimiter, TRCLimiter, TSoftKneeLimiter,
    TSimpleSoftKneeLimiter, TClassicGate, TClassicSoftRangeGate,
    TClassicSoftKneeGate, TAdvancedGate, TSimpleCompressor, TSoftKneeCompressor,
    TSimpleFeedbackCompressor, TSoftKneeFeedbackCompressor, TSimpleRMSCompressor,
    TCompressor]);

  RegisterDspProcessors64([TSimpleDirectGate, TSoftDirectGate, TBrickwallLimiter,
    TSoftBrickwallLimiter, TLimiter, TRCLimiter, TSoftKneeLimiter,
    TSimpleSoftKneeLimiter, TClassicGate, TClassicSoftRangeGate,
    TClassicSoftKneeGate, TAdvancedGate, TSimpleCompressor, TSoftKneeCompressor,
    TSimpleFeedbackCompressor, TSoftKneeFeedbackCompressor, TSimpleRMSCompressor,

...
begin
  if Assigned(FSonogram) then
    FreeAndNil(FSonogram);
  inherited;
end;

// The most important part, processing the audio
procedure TSESonogramModule.SubProcessStatic(const BufferOffset,
...
begin
  ...
  try
    FSonogram.ProcessBlock32(PDAVSingleFixedArray(@FInputBuffer[BufferOffset]),
      SampleFrames);
  finally
...
procedure TSESonogramModule.SubProcessSleep(const BufferOffset,
  SampleFrames: Integer);
begin
  CallHost(SEAudioMasterSleepMode);
end;
...
class procedure TSESonogramModule.GetModuleProperties
...
function TSESonogramModule.GetPinProperties(const Index: Integer;

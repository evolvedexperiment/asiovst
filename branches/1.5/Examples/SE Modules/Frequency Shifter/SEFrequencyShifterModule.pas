...
type
  ...
  TSEFrequencyShifterPins = (pinInput, pinUpshift, pinDownshift, pinFrequency,
    pinCoefficientCount, pinTransitionBandwidth);
...
constructor TCustomSEFrequencyShifterModule.Create(SEAudioMaster
...
procedure TCustomSEFrequencyShifterModule.SubProcessStatic(const BufferOffset,
...
class procedure TCustomSEFrequencyShifterModule.GetModuleProperties
...
function TCustomSEFrequencyShifterModule.GetPinProperties(const Index: Integer;
...
procedure TCustomSEFrequencyShifterModule.PlugStateChange
...
procedure TSEFrequencyShifterStaticModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
var
...
begin
  ...
  for Sample := 0 to SampleFrames - 1 do
    FFrequencyShifter.ProcessSample(Inp^[Sample], Up^[Sample], Down^[Sample]);
end;

// describe the pins (plugs)
function TSEFrequencyShifterStaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
...
// An input plug has changed value
procedure TSEFrequencyShifterStaticModule.PlugStateChange
  (const CurrentPin: TSEPin);
begin
  inherited;
  case TSEFrequencyShifterPins(CurrentPin.PinID) of
    pinFrequency:
      FFrequencyShifter.Frequency := FFrequency;
    pinCoefficientCount:
      FFrequencyShifter.CoefficientCount := FCoefficientCount;
    pinTransitionBandwidth:
      FFrequencyShifter.TransitionBandwidth := FTransitionBW;
  end;
end;
...
function TSEFrequencyShifterControllableModule.GetPinProperties
  (const Index: Integer; Properties: PSEPinProperties): Boolean;
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSEFrequencyShifterPins(index) in [pinFrequency] then
    with Properties^ do
      Direction := drIn;
end;

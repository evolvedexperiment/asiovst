...
type
  ...
  public
    ...
    // sub chunks
    procedure AddSubChunk(SubChunk: TDavCustomChunk); virtual;
    procedure DeleteSubChunk(SubChunk: TDavCustomChunk); overload; virtual;
...
procedure TCustomAudioFileWAV.DeleteSubChunk(SubChunk: TDavCustomChunk);
var
  i: Integer;
begin
  i := 0;
...
procedure TCustomAudioFileWAV.AddSubChunk(SubChunk: TDavCustomChunk);
begin
...
function TCustomAudioFileWAV.GetSubChunk(Index: Integer): TDavCustomChunk;
begin
  if (Index >= 0) and (Index < FChunkList.Count) then
...
procedure TCustomAudioFileWAV.ReadUnknownChunk(const Stream: TStream;
  const ChunkName: TChunkName);
var
  ChunkClass: TDavDefinedChunkClass;
  DefinedChunk: TDavDefinedChunk;
begin

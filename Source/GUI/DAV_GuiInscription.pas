...
{$IFDEF FPC}
    ...
    procedure GMFontListChanged(var Message: TLMessage);
      message GM_FontListChanged;
{$ELSE}
    procedure GMFontChanged(var Message: TMessage); message GM_FontChanged;
    procedure GMFontListChanged(var Message: TMessage);
      message GM_FontListChanged;
{$ENDIF}
    property FontItem: TGuiCustomFontCollectionItem read FFontItem
      write SetFontItem;
  public
...
begin
  ...
  begin
    ...
    if FFontItemIndex >= 0 then
      FontItem := FFontList[FFontItemIndex];
    FontIndexChanged;
  end;
end;
...
begin
  if Assigned(FFontItem) then
    Result := FFontItem.Index
  else
    Result := -1;
end;

procedure TCustomGuiInscription.GMFontChanged(var Message:
...
procedure TCustomGuiInscription.GMFontListChanged(var Message:
  {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF});
begin
  case Message.WParam of
    0:
...
begin
  ...
  begin
    ...
    if Value < 0 then
      Value := -1
    else if Value >= FFontList.Count then
      Value := FFontList.Count - 1;

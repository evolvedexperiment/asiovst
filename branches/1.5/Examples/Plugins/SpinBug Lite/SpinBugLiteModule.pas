...
begin
  if Assigned(FHilbert[0]) then
    FreeAndNil(FHilbert[0]);
  if Assigned(FHilbert[1]) then
    FreeAndNil(FHilbert[1]);
  if Assigned(FSineLFO[0]) then
    FreeAndNil(FSineLFO[0]);
  if Assigned(FSineLFO[1]) then
    FreeAndNil(FSineLFO[1]);
end;
...
begin
  ival := Round(Value);
  if (ival < 1) or (ival > 32) then
    Exit;
  if (ival = 17) or (ival = 18) or (ival = 19) or (ival = 20) or (ival = 21) or
    (ival = 22) or (ival = 23) or (ival = 25) or (ival = 26) or (ival = 27) or
    (ival = 28) or (ival = 29) or (ival = 30) or (ival = 31) then
    Exit;
  if Assigned(FHilbert[0]) then
    FHilbert[0].NumberOfCoefficients := ival;
  if Assigned(FHilbert[1]) then
    FHilbert[1].NumberOfCoefficients := ival;
...
begin
  ...
  // update GUI
  if Assigned(EditorForm) then
    (EditorForm as TFmSpinBugLite).UpdateType;
end;
...
begin
  ...
  // update GUI
  if Assigned(EditorForm) then
    TFmSpinBugLite(EditorForm).UpdateLFO;
end;

procedure TSpinBugLiteModule.SBMTBWChange(Sender: TObject; const Index: Integer;
  var Value: Single);
begin
  if (Value <= 0) or (Value >= 0.5) then
    Exit;
  if Value <> FTBW then
  begin
    FTBW := Value;
    if Assigned(FHilbert[0]) then
      FHilbert[0].Transition := FTBW;
    if Assigned(FHilbert[1]) then
      FHilbert[1].Transition := FTBW;
  end;
end;
...
begin
  ...
  begin
    if Assigned(FSineLFO[0]) then
      FSineLFO[0].SampleRate := Abs(SampleRate);
    if Assigned(FSineLFO[1]) then
      FSineLFO[1].SampleRate := Abs(SampleRate);
  end;
end;

procedure TSpinBugLiteModule.VSTModuleProcessStereoA(const Inputs,
...
procedure TSpinBugLiteModule.VSTModuleProcessStereoB(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
  i: Integer;
  a1, b1: Single;
...
procedure TSpinBugLiteModule.VSTModuleProcessStereoC(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
  i: Integer;
  a1, b1: Single;
...
procedure TSpinBugLiteModule.VSTModuleProcessStereoD(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
  i: Integer;
  s, c: Double;
...
procedure TSpinBugLiteModule.VSTModuleProcessMono(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
  i: Integer;
  a, b: Single;
...
procedure TSpinBugLiteModule.VSTModuleProcessMonoL(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
  i: Integer;
  a, b: Single;
...
procedure TSpinBugLiteModule.VSTModuleProcessMonoR(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
  i: Integer;
  a, b: Single;
...
procedure TSpinBugLiteModule.VSTModuleProcessMS(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
  i: Integer;
  a1, b1: Single;
...
begin
  for i := 0 to SampleFrames - 1 do
  begin
    FHilbert[0].ProcessHilbertSample(0.5 * (Inputs[0, i] + Inputs[1, i]
      ), a1, b1);
    FHilbert[1].ProcessHilbertSample(0.5 * (Inputs[0, i] - Inputs[1, i]
      ), a2, b2);
...
procedure TSpinBugLiteModule.VSTModuleProcessSpecial(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
  i: Integer;
  a1, b1: Single;
...
procedure TSpinBugLiteModule.VSTModuleProcessOldOne(const Inputs,
  Outputs: TDAVArrayOfSingleFixedArray; const SampleFrames: Cardinal);
var
  i: Integer;
  a1, b1: Single;
...
begin
  for i := 0 to SampleFrames - 1 do
  begin
    FHilbert[0].ProcessHilbertSample(FastTanh2Like3Term(2.1 * Inputs[0, i]
      ), a1, b1);
    FHilbert[1].ProcessHilbertSample(FastTanh2Like3Term(2.2 * Inputs[1, i]
      ), a2, b2);
    a1 := a1 * BasicSineLFO.Sine;
    b1 := b1 * BasicSineLFO.Cosine;
    a2 := a2 * BasicSineLFO.Sine;

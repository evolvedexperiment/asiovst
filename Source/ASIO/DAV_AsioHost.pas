...
begin
  if Assigned(GAsioHost) then
    case DirectProcess of
      CAsioFalse:
        PostMessage(GAsioHost.FHandle, PMBufSwitch.Msg, AM_BufferSwitch, DoubleBufferIndex);
      CAsioTrue:
        GAsioHost.BufferSwitch(DoubleBufferIndex);
    end;
end;
...
begin
  if Assigned(GAsioHost) then
    case DirectProcess of
      ...
      CAsioTrue :
        GAsioHost.BufferSwitchTimeInfo(DoubleBufferIndex, params);
  end;
  Result := nil;
end;
...
begin
  ...
  begin
    ...
    if Assigned(GAsioHost.FOnSampleRateChanged) then
      GAsioHost.FOnSampleRateChanged(GAsioHost);
  end;
end;
...
begin
  ...
  case Selector of
    ...
    kAsioEngineVersion :
      if Assigned(GAsioHost) then
        Result := GAsioHost.FEngineVersion
      else
    ...
    kAsioSupportsTimeInfo :
      if Assigned(GAsioHost) then
        Result := Integer(assSupportsTimeInfo in GAsioHost.Supports)
      else
        Result := 1;
    ...
    kAsioSupportsInputMonitor :
      if Assigned(GAsioHost) then
        Result := Integer(assSupportsInputMonitor in GAsioHost.Supports)
      else
        Result := 1;
  end;
end;
...
begin
  ...
  begin
    if Assigned (FOnBuffersDispose) then
      FOnBuffersDispose(Self);
    FInputBuffers := nil;
    FOutputBuffers := nil;
...
begin
  ...
  case Message.WParam of
    ...
    AM_BufferSwitchTimeInfo:
      BufferSwitchTimeInfo(Message.LParam, AsioTime.FBufferTime);  // process a buffer with time
    AM_LatencyChanged:
      begin
        if Assigned(FDriver) then
          FDriver.GetLatencies(FInputLatency, FOutputLatency);
...
begin
  ...
  begin
    ...
    if FDriver.GetSamplePosition(TimeInfo.SamplePosition, TimeInfo.SystemTime) = ASE_OK then
...
begin
  if (Index < 0) or (Index >= FInputChannelCount) then
...
begin
  if (Index < 0) or (Index >= FOutputChannelCount) then
    raise Exception.CreateFmt(RCStrIndexOutOfBounds, [Index]);
...
begin
  // check if command can be transmitted
  if (FDriver = nil) and not (acdInputGain in FAsioCanDos) then
    Exit;
...
begin
  // check if command can be transmitted
  if (FDriver = nil) and not (acdOutputGain in FAsioCanDos) then
    Exit;
...
begin
  ...
  for Channel := 0 to Length(FSingleInBuffer) - 1 do
    Dispose(FSingleInBuffer[Channel]);

  // dispose single output buffers
  for Channel := 0 to Length(FSingleOutBuffer) - 1 do
    Dispose(FSingleOutBuffer[Channel]);

  // dispose double input buffers
  for Channel := 0 to Length(FDoubleInBuffer) - 1 do
    Dispose(FDoubleInBuffer[Channel]);

  // dispose double output buffers
  for Channel := 0 to Length(FDoubleOutBuffer) - 1 do
    Dispose(FDoubleOutBuffer[Channel]);
...
begin
  ...
  case ProcessorType of
    ptSSE:
      if coSSE in FConvertOptimizations then Use_SSE;
    pt3DNow:
      if co3DNow in FConvertOptimizations then Use_3DNow;
  end;
end;
...
begin
  with FAsioMixer do
  begin
    for Channel := 0 to Length(ChannelsStrips) - 1 do
      FreeAndNil(ChannelsStrips[Channel]);
      ...
      cm32:
        begin
          ...
          if Length(FSingleInBuffer) > FInputChannelCount then
            for Channel := FInputChannelCount to Length(FSingleInBuffer) - 1 do
              Dispose(FSingleInBuffer[Channel]);
          ...
          if Length(FSingleOutBuffer) > FOutputChannelCount then
            for Channel := FOutputChannelCount to Length(FSingleOutBuffer) - 1 do
              Dispose(FSingleOutBuffer[Channel]);
      ...
      cm64:
        begin
          ...
          if Length(FDoubleInBuffer) > FInputChannelCount then
            for Channel := FInputChannelCount to Length(FDoubleInBuffer) - 1 do
              Dispose(FDoubleInBuffer[Channel]);
          ...
          if Length(FDoubleOutBuffer) > FOutputChannelCount then
            for Channel := FOutputChannelCount to Length(FDoubleOutBuffer) - 1 do
              Dispose(FDoubleOutBuffer[Channel]);
...
begin
  ...
  if Assigned(FOnBufferSwitchNative) then
    FOnBufferSwitchNative(Self, @(FInputBuffers^), Index);
  ...
  begin
      ...
      bpfNoise:
        for Channel := 0 to FInputChannelCount - 1 do
          for Sample := 0 to FBufferSize - 1 do
            FDoubleInBuffer[Channel, Sample] := 2 * Random - 1;
      bpfCustom:
        if Assigned(FAsioGenerator) then
          FAsioGenerator.ProcessBuffer64(FDoubleInBuffer, False);
      else
        for Channel := 0 to FInputChannelCount - 1 do
        begin
          ...
          if Assigned(ChannelData) then
            FInConverters[Channel].ic64(ChannelData, @FDoubleInBuffer[Channel, 0], FBufferSize);
          inc(CurrentBuffer);
        end;
    end;
    ...
    if FPreventClipping <> pcNone then
      for Channel := 0 to FInputChannelCount - 1 do
        FClipPrevent.cb64(@FDoubleInBuffer[Channel, 0], FBufferSize);
    ...
    case FOutBufferPreFill of
      ...
      bpfNoise:
        for Channel := 0 to FOutputChannelCount - 1 do
          for Sample := 0 to FBufferSize - 1 do
            FDoubleOutBuffer[Channel, Sample] := 2 * Random - 1;
      bpfCustom:
        if Assigned(FAsioGenerator) then
          FAsioGenerator.ProcessBuffer64(FDoubleOutBuffer, True);
    end;

    if FInputMonitor then
      for Channel := 0 to min(FInputChannelCount, FOutputChannelCount) - 1 do
        Move(FDoubleInBuffer[Channel, 0],
    ...
    if FPreventClipping <> pcNone then
      for Channel := 0 to FOutputChannelCount - 1 do
        FClipPrevent.cb64(@FDoubleOutBuffer[Channel, 0] ,FBufferSize);
    ...
    begin
      ChannelData := CurrentBuffer^[0].Buffers[Index];
      if Assigned(ChannelData) then
        FOutConverters[Channel].oc64(@FDoubleOutBuffer[Channel, 0], ChannelData, FBufferSize);
      inc(CurrentBuffer);
    end;
  end
  else
  ...
  begin
    ...
    case FInBufferPreFill of
      ...
      bpfNoise:
        for Channel := 0 to FInputChannelCount - 1 do
          for Sample := 0 to FBufferSize - 1 do
            FSingleInBuffer[Channel, Sample] := 2 * Random - 1;
      bpfCustom:
        if Assigned(FAsioGenerator) then
          FAsioGenerator.ProcessBuffer32(FSingleInBuffer, False);
      else
      begin
        for Channel := 0 to FInputChannelCount - 1 do
        begin
          ...
          if Assigned(ChannelData) then
            FInConverters[Channel].ic32(ChannelData, @FSingleInBuffer[Channel, 0], FBufferSize);
          inc(CurrentBuffer);
        end;
      end;
    end;
    ...
    if FPreventClipping <> pcNone then
      for Channel := 0 to FInputChannelCount - 1 do
        FClipPrevent.cb32(@FSingleInBuffer[Channel, 0], FBufferSize);
    ...
    case FOutBufferPreFill of
      ...
      bpfNoise:
        ...
        begin
          Assert(FSingleOutBuffer[Channel] <> nil);
          for Sample := 0 to FBufferSize - 1 do
            FSingleOutBuffer[Channel, Sample] := 2 * Random - 1;
        end;
      bpfCustom:
        if Assigned(FAsioGenerator) then
          FAsioGenerator.ProcessBuffer32(FSingleOutBuffer, True);
    end;

    if FInputMonitor then
      for Channel := 0 to min(FInputChannelCount, FOutputChannelCount) - 1 do
        Move(FSingleInBuffer[Channel, 0],
    ...
    if FPreventClipping <> pcNone then
      for Channel := 0 to FOutputChannelCount - 1 do
        FClipPrevent.cb32(@FSingleOutBuffer[Channel, 0], FBufferSize);
    ...
    begin
      ChannelData := CurrentBuffer^[0].Buffers[Index];
      if Assigned(ChannelData) then
        FOutConverters[Channel].oc32(@FSingleOutBuffer[Channel, 0], ChannelData, FBufferSize);
      Inc(CurrentBuffer);
    end;
  end;

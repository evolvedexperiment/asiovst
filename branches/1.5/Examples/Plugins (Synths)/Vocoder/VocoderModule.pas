...
begin
  ...
  begin
    ...
    with newNote do
    begin
      MidiKey := MidiEvent.midiData[1];
      Velocity := MidiEvent.midiData[2];
      NoteOn(Midi2Pitch[MidiKey], Velocity * VeloDiv);
    end;
    Voices.Add(newNote);
  end
  ...
  begin
    for i := 0 to Voices.Count - 1 do
     if (Voices.Items[i].MidiKey = MidiEvent.midiData[1]) then
      begin
        Voices.Delete(i);
        Break;
      end;
  ...
  if (status = $B0) and (MidiEvent.midiData[1] = $7E) then
...
begin
  if Assigned(FVocoder) then
    FVocoder.SynthesisBandwidth := Value;
end;
...
begin
 if Assigned(FVocoder) then
   if Value <= -80 then
     ...
     FVocoder.InputLevel := dB_to_Amp(Value);
  if FEditorForm is TVSTGUI then
    TVSTGUI(FEditorForm).UpdateInputVolume;
end;
...
begin
  if Assigned(FVocoder) then
    if Value <= -80 then
      ...
      FVocoder.SynthLevel := dB_to_Amp(Value);
  if FEditorForm is TVSTGUI then
    TVSTGUI(FEditorForm).UpdateSynthVolume;
end;
...
begin
  if Assigned(FVocoder) then
    if Value <= -80 then
      FVocoder.VocoderLevel := 0
    else
      FVocoder.VocoderLevel := dB_to_Amp(Value);
  if FEditorForm is TVSTGUI then
    TVSTGUI(FEditorForm).UpdateVocoderVolume;
end;

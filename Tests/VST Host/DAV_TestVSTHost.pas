  ...
  with FVstHost do
    try
      ...
      with VstPlugIns.Add do
        if FileExists(FVstPluginName) then
          LoadFromFile(FVstPluginName)
        else
          raise Exception.CreateFmt(RCStrVSTPluginNotFound, [FVstPluginName]);
    except
      on E: Exception do
        Fail(E.Message);
    end;
...
begin
  if not FileExists(Value) then
    raise Exception.CreateFmt(RCStrVSTPluginDoesNotExists, [Value]);
      ...
      with FVstHost.VstPlugIns.Add do
        try
          ...
          if SequenceIndex = 1 then
            Fail(Format(RCStrErrorOpeningPluginAfterClose, [InstanceIndex]))
          else
  ...
  begin
    ...
    begin
      ...
      repeat
        ...
        if (Start = Change) and (Parameter[Param] <> Start) then
      ...
      begin
        ...
        if ppfParameterUsesIntStep in PP.Flags then
          CheckTrue(PP.LargeStepInteger >= PP.StepInteger,
            ...
            ' (' + string(PP.ParamLabel) + ', ' + string(PP.ShortLabel) +
            ') StepInteger: ' + IntToStr(PP.StepInteger) + ' LargeStepInteger: '
        ...
        if ppfParameterUsesIntegerMinMax in PP.Flags then
          CheckTrue(PP.MaxInteger >= PP.MinInteger, 'MaxInteger < MinInteger!' +
            ...
            IntToStr(PP.MaxInteger) + ' MaxInteger: ' +
            IntToStr(PP.MinInteger));

        // check future equals zero
        CheckTrue(PP.Future[0] = #0, 'Parameter ' + IntToStr(Param) +
          ': Future character value <> 0!');
      end;

      // check whether automation is possible
      if (Start = Change) and (CanBeAutomated(Param) <> 0) then
        Fail('Parameter ' + IntToStr(Param) + ': Automation');
    end;
    Active := False
  end;
end;
...
begin
  ...
  begin
    ...
    // small block sizes
    for BlockSizeIndex := 0 to 64 do
      SetBlockSize(BlockSizeIndex);

    // medium odd block sizes
    for BlockSizeIndex := 1 to 64 do
      SetBlockSize(19 * BlockSizeIndex);

    // large odd block sizes
    for BlockSizeIndex := 1 to 64 do
      SetBlockSize(1025 * BlockSizeIndex);
  end;
end;
...
begin
  ...
  // small block sizes
  for I := 0 to 64 do
    FVstHost[0].SetBlockSize(I);

  // medium odd block sizes
  for I := 1 to 64 do
    FVstHost[0].SetBlockSize(19 * I);

  // large odd block sizes
  for I := 1 to 64 do
    FVstHost[0].SetBlockSize(1025 * I);
...
begin
  ...
  TestCanDo :=
    '2zcrfo3874zrbiwrbgrvsdrgbviwztvi374vöowiurzbi4t7zviw74tvposihfopit';
  FVstHost[0].VstCanDo(TestCanDo);

  // test very long CanDo text!
  TestCanDo := '';
  for CharCnt := 0 to 1111 do
    TestCanDo := TestCanDo + AnsiChar(1 + Random(200));
  FVstHost[0].VstCanDo(TestCanDo);
end;
...
{$IFNDEF NoInvalidOpcodes}
  for I := 128 to 2000 do
    FVstHost[0].VstDispatch(TDispatcherOpcode(I));
{$ENDIF}
  with FVstHost[0] do
  begin
...
var
  ...
  Err: string;
const
  CErrorMsg: array [0 .. 13] of string = ('GetParameter(numParams)',
    'GetParameter(numParams + Random(MaxInt - numParams))',
    'SetParameter(0, 10)', 'SetParameter(numParams, 10)',
...
begin
  ...
  if Result = 'TestAbletonLiveScan' then
    Result := 'Live'
  else if Result = 'TestAbletonLive' then
    Result := 'Live'
  else if Result = 'TestCantabile' then
    Result := 'Cantabile'
  else if Result = 'TestCubaseScan' then
    Result := 'Cubase VST'
  else if Result = 'TestCubase' then
    Result := 'Cubase VST'
  else if Result = 'TestCubaseReloadPlugin' then
    Result := 'Cubase VST'
  else if Result = 'TestEnergyXT' then
    Result := 'energyXT'
  else if Result = 'TestEnergyXTBug' then
    Result := 'energyXT'
  else if Result = 'TestFL8FastScan' then
    Result := 'Fruity Wrapper'
  else if Result = 'TestFL8' then
    Result := 'Fruity Wrapper'
  else if Result = 'TestMULAB' then
    Result := 'MU.LAB'
  else if Result = 'TestReaper' then
    Result := 'REAPER'
  else if Result = 'TestSamplitude' then
    Result := 'Samplitude'
  else if Result = 'TestSoundForge10Scan' then
    Result := 'Sound Forge Pro 10.0'
  else if Result = 'TestSoundForge10' then
    Result := 'Sound Forge Pro 10.0'
  else if Result = 'TestSoundForge10Bug' then
    Result := 'Sound Forge Pro 10.0'
  else if Result = 'TestTracktion2' then
    Result := 'Tracktion 2'
  else if Result = 'TestTracktion2Scan' then
    Result := 'Tracktion 2'
  else
    Result := inherited GetProductString;
end;
...
begin
  ...
  if Result = 'TestAbletonLiveScan' then
    ...
    Result := inherited GetVendorString;
end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          CheckTrue(PP.Caption[63] = #0,
            'effGetInputProperties: Caption probably too long');
          CheckTrue(PP.Future[47] = 0,
            'effGetInputProperties: Future field <> 0');
        end;
      end;
      ...
      begin
        ...
        begin
          CheckTrue(PP.Caption[63] = #0,
            'effGetOutputProperties: Caption probably too long');
          CheckTrue(PP.Future[47] = 0,
            'effGetInputProperties: Future field <> 0');
        end;
      end;
      ...
      with TForm.Create(nil) do
        try
          ...
          // set bounds
          if Assigned(prct) then
            SetBounds(prct^.Left, prct^.Top, prct^.Right - prct^.Left,
    ...
    begin
      ...
      begin
        CheckTrue(PP.Caption[63] = #0,
          'effGetOutputProperties: Caption probably too long');
        CheckTrue(PP.Future[47] = 0,
          'effGetOutputProperties: Future field <> 0');
      end;
    end;
...
begin
  ...
  begin
    ...
    try
      ...
      // get program name indexed
      for PrgIndx := 0 to numPrograms - 1 do
        VstDispatch(effGetProgramNameIndexed, PrgIndx, -1, Data);
      ...
      // get midi key name
      for ParIndx := 0 to 127 do
        VstDispatch(effGetMidiKeyName, 0, 0, Data);
  ...
  begin
    ...
    try
      ...
      for j := 0 to numPrograms - 1 do
        VstDispatch(effGetProgramNameIndexed, j, 0, Data);
...
begin
  ...
  begin
    ...
    // get input connected
    for I := numInputs - 1 downto 0 do
      VstDispatch(effConnectInput, I);

    // get output connected
    for I := numOutputs - 1 downto 0 do
      VstDispatch(effConnectOutput, I);
    ...
    try
      ...
      begin
        // get/set program
        if VstDispatch(effGetProgram) <> j then
      ...
      // get/set program
      if VstDispatch(effGetProgram) <> 0 then
        VstDispatch(effSetProgram);
...
begin
  ...
  begin
    ...
    try
      ...
      // get program name indexed
      for j := 0 to numPrograms - 1 do
        VstDispatch(effGetProgramNameIndexed, j, -1, Data);
  ...
  begin
    ...
    with TForm.Create(nil) do
      try
        ...
        if numPrograms <= 0 then
          Fail('No programs found, Cubase will probably crash!');
...
begin
  ...
  for Channel := 0 to Length(FInput) - 1 do
    Dispose(FInput[Channel]);
  for Channel := 0 to Length(FOutput) - 1 do
    Dispose(FOutput[Channel]);
  ...
  with FVstHost[0] do
    try
      ...
      repeat
        ...
        CheckFalse(IsDen, 'Denormal found! (in program ' +
          IntToStr(CurrentProgram));
...
begin
  ...
  begin
    ...
    begin
      Process32Replacing(@FInput[0], @FOutput[0],
        FBlockSize div BlockSizeDecimation);
      for Channel := 0 to Length(FOutput) - 1 do
        for Sample := FBlockSize div 10 to FBlockSize - 1 do
          CheckTrue(FOutput[Channel, Sample] = 0,
            'Data was processed beyond blocksize');
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if not Assigned(VstEffectPointer.Process64Replacing) then
      Fail('Process() does not exists');
  ...
  for Channel := 0 to Length(FInput) - 1 do
    for Sample := 0 to FBlockSize - 1 do
      FInput[Channel, Sample] := 2 * Random - 1;

  with FVstHost[0] do
  begin
    for Cnt := 0 to 10 do
      Process(@FInput[0], @FOutput[0], Random(FBlockSize));
  end;
end;
...
begin
  ...
  begin
    ...
    if not Assigned(VstEffectPointer.Process64Replacing) then
      Fail('Process64Replacing(() does not exists');
  ...
  begin
    ...
    for ChannelIndex := 0 to Length(FInput) - 1 do
      for SampleIndex := 0 to FBlockSize - 1 do
        FInput[ChannelIndex]^[SampleIndex] := Random - Random;

    // clear output buffer
    for ChannelIndex := 0 to Length(FOutput) - 1 do
      FillChar(FOutput[ChannelIndex]^, FBlockSize * SizeOf(Single), 0);
    ...
    try
      ...
      begin
        ...
        for SampleIndex := 0 to FBlockSize - 1 do
          Input64[ChannelIndex]^[SampleIndex] := FInput[ChannelIndex]^
            [SampleIndex];
      end;
      ...
      // free memory
      for ChannelIndex := 0 to Length(Input64) - 1 do
        FreeMem(Input64[ChannelIndex]);

      for ChannelIndex := 0 to Length(Output64) - 1 do
        FreeMem(Output64[ChannelIndex]);
    end;
  end;
end;
      ...
      repeat
        ...
        begin
          ...
          begin
            ...
            if (Cnt > 3) and (Delta[Ndx] < 1E-10) then
              Break;
          end;
          StopProcess;
...
begin
  ...
  begin
    ...
    for ChannelIndex := 0 to Length(FInput) - 1 do
      for SampleIndex := 0 to FBlockSize - 1 do
        FInput[ChannelIndex]^[SampleIndex] := 2 * Random - 1;
    ...
    // clear output buffer
    for ChannelIndex := 0 to Length(FInput) - 1 do
      FillChar(FInput[ChannelIndex]^, FBlockSize * SizeOf(Single), 0);
    ...
    if (FOutput[0]^[BlockSize - 1] = 0) then
      for ChannelIndex := 0 to Length(FOutput) - 1 do
        for SampleIndex := 0 to FBlockSize - 1 do
          Check(FOutput[ChannelIndex]^[SampleIndex] = 0,
            'Potential reset issue');
...
begin
  ...
  begin
    FBlockSize := Value;
    if Assigned(FVstProcessThread) then
...
begin
  with FVstHost[0] do
    repeat
      for ParamNo := 0 to numParams - 1 do
        Parameter[ParamNo] := Random;
    until FVstProcessThread.Terminated or
      (FVstProcessThread.ProcessedBlocks > 10);

  if (not Assigned(FVstProcessThread)) or
    (FVstProcessThread.ProcessedBlocks <= 10) then
    Fail('Processing crashed!');
...
begin
  with FVstHost[0] do
    ...
    until FVstProcessThread.Terminated or
      (FVstProcessThread.ProcessedBlocks > 4);

  if (FVstProcessThread.ProcessedBlocks <= 4) then
    Fail('Processing crashed!');
end;
...
begin
  if not FileExists(Value) then
    raise Exception.Create('Specified VST plugin does not exist: ' + Value);
  if FVstPluginName <> Value then
  begin
    FVstPluginName := Value;
  end;
...
begin
  ...
  try
    ...
    begin
      TestCase := TTestCaseClass(ATestClass)
        .CreateWithName(MethodList[MethodIter]);

...
procedure TCustomSpectralFilter32.SetWindowClass(const Value
...
procedure TCustomSpectralFilter32.PerformSpectralEffect
...
procedure TAveragedSelfFiltering32.BuildFilter
  (Spectrum: PDAVComplexSingleFixedArray);
var
  Bin: Integer;
  Half: Integer;
...
begin
  ...
  // DC bin
  FFilter^[0].Re := FDecayFactor * FFilter^[0].Re + (1 - FDecayFactor) *
    Abs(Spectrum^[0].Re);
  FFilter^[0].Im := 0;
  ...
  begin
    FFilter^[Bin].Re := FDecayFactor * FFilter^[Bin].Re + (1 - FDecayFactor) *
      FastSqrtBab2(Sqr(Spectrum^[Bin].Re) + Sqr(Spectrum^[Bin].Im));
    FFilter^[Bin].Im := 0;
  end;

  // Nyquist bin
  FFilter^[Half].Re := FDecayFactor * FFilter^[Half].Re + (1 - FDecayFactor) *
    Abs(Spectrum^[Half].Re);
  FFilter^[Half].Im := 0;
...
procedure TPeakSelfFiltering32.BuildFilter
  (Spectrum: PDAVComplexSingleFixedArray);
var
  Bin: Integer;
  Half: Integer;
...
begin
  ...
  // DC bin
  if Abs(Spectrum^[0].Re) > FFilter^[0].Re then
    FFilter^[0].Re := Abs(Spectrum^[0].Re);
  ...
  // Nyquist bin
  if Abs(Spectrum^[Half].Re) > FFilter^[Half].Re then
    FFilter^[Half].Re := Abs(Spectrum^[Half].Re);
...
begin
  ...
  // other bins
  for Bin := 1 to Half - 1 do
    FMagnitudeBuffer^[Bin] := FastSqrtBab2(Sqr(Spectrum^[Bin].Re) +
      Sqr(Spectrum^[Bin].Im));
  ...
  // DC bin
  Bin := 1;
  FFilter^[0].Re := 0.5 * (FMagnitudeBuffer^[0] + 0.5 * FMagnitudeBuffer^[Bin])
    - FMagnitudeBuffer^[0];
  FFilter^[0].Im := 0;
  ...
  for Bin := 1 to Half - 1 do
  begin
    FFilter^[Bin].Re := 0.5 * (FMagnitudeBuffer^[Bin] + 0.5 *
      (FMagnitudeBuffer^[Bin + 1] + FMagnitudeBuffer^[Bin - 1])) -
      FMagnitudeBuffer^[Bin];
    FFilter^[Bin].Im := 0;
  end;

  // Nyquist bin
  FFilter^[Half].Re := 0.5 * (FMagnitudeBuffer^[Half] + 0.5 * FMagnitudeBuffer^
    [Half - 1]) - FMagnitudeBuffer^[Half];
  FFilter^[Half].Im := 0;
...
procedure TCustomSpectralFilter64.SetWindowClass(const Value
  : TWindowFunctionClass);
begin
  if FWindowClass <> Value then
  begin
...
begin
  ...
  // DC bin
  if Spectrum^[0].Re > FThreshold then
  ...
  // Nyquist bin
  if Spectrum^[Half].Re > FThreshold then
...
procedure TSpectralFilter64.PerformSpectralEffect

...
type
  ...
  public
    ...
    property Direction: TSliderDirection read FDirection write SetDirection
      default sdLeftToRight;
...
begin
  if FValue > FMaximum then
    FValue := FMaximum;
  if FDefaultValue > FMaximum then
    FDefaultValue := FMaximum;
  ControlChanged;
end;
...
begin
  if FValue < FMinimum then
    FValue := FMinimum;
  if FDefaultValue < FMinimum then
    FDefaultValue := FMinimum;
  ControlChanged;
end;
...
procedure TCustomGuiSliderGDI.MouseDown(Button: TMouseButton;
...
begin
  ...
  begin
    ...
    Value := Limit(FMinimum + MapValue(NormalizedPosition) *
      (FMaximum - FMinimum), FMinimum, FMaximum);
  end;
...
begin
  ...
  begin
    ...
    if FCaption <> '' then
      CurrentText := FCaption + ': ' + CurrentText;

    if Assigned(FOnGetText) then
      FOnGetText(Self, CurrentText);
...
begin
  ...
  begin
    ...
    XPos := Round(Width * UnmapValue((FValue - FMinimum) /
      (FMaximum - FMinimum)));
    ...
    begin
      ...
      for X := 0 to Y - 1 do
        PixelMap.FrameRect(Rect(X, X, Width - 1 - X, Height - 1 - X),
          BorderColor);
      BorderColor.A := Round($FF * (FBorderWidth - Y));
      PixelMap.FrameRect(Rect(Y, Y, Width - 1 - Y, Height - 1 - Y),
        BorderColor);
      Exit;
    end;

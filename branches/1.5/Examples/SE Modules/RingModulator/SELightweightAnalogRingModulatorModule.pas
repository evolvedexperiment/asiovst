...
type
  ...
  TSELightweightAnalogRingModulatorStaticModule = class
  ...
  TSELightweightAnalogRingModulatorControllableModule = class
...
constructor TCustomSELightweightAnalogRingModulatorModule.Create
...
procedure TCustomSELightweightAnalogRingModulatorModule.SubProcessStatic
...
class procedure TCustomSELightweightAnalogRingModulatorModule.
...
function TCustomSELightweightAnalogRingModulatorModule.GetPinProperties
...
procedure TCustomSELightweightAnalogRingModulatorModule.PlugStateChange
...
// describe your module
class procedure TSELightweightAnalogRingModulatorStaticModule.
  GetModuleProperties(Properties: PSEModuleProperties);
begin
...
procedure TSELightweightAnalogRingModulatorStaticModule.SubProcess
  (const BufferOffset, SampleFrames: Integer);
var
...
begin
  ...
  for Sample := 0 to SampleFrames - 1 do
    Outp^[Sample] := FAnalogRingModulator.ProcessSample32(Inp^[Sample]);
end;

// describe the pins (plugs)
function TSELightweightAnalogRingModulatorStaticModule.GetPinProperties
  (const Index: Integer; Properties: PSEPinProperties): Boolean;
begin
...
// An input plug has changed value
procedure TSELightweightAnalogRingModulatorStaticModule.PlugStateChange
  (const CurrentPin: TSEPin);
begin
  inherited;
  case TSELightweightAnalogRingModulatorPins(CurrentPin.PinID) of
    pinFrequency:
      FAnalogRingModulator.Frequency := FFrequency;
  end;
end;

{ TSELightweightAnalogRingModulatorControllableModule }

class procedure TSELightweightAnalogRingModulatorControllableModule.
  GetModuleProperties(Properties: PSEModuleProperties);
begin
  inherited GetModuleProperties(Properties);
  with Properties^ do
...
function TSELightweightAnalogRingModulatorControllableModule.GetPinProperties
  (const Index: Integer; Properties: PSEPinProperties): Boolean;
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSELightweightAnalogRingModulatorPins(index) in [pinFrequency] then
    with Properties^ do
      Direction := drIn;
end;

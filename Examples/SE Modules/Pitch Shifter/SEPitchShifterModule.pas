...
constructor TSEStkPitchshifterModule.Create(SEAudioMaster
...
procedure TSEStkPitchshifterModule.SubProcess(const BufferOffset,
...
begin
  ...
  for Sample := 0 to SampleFrames - 1 do
    Outp^[Sample] := FStkPitchshifter.Tick(Inp^[Sample]);
end;

procedure TSEStkPitchshifterModule.SubProcessStatic(const BufferOffset,
  SampleFrames: Integer);
begin
...
class procedure TSEStkPitchshifterModule.GetModuleProperties
...
function TSEStkPitchshifterModule.GetPinProperties(const Index: Integer;
...
begin
  ...
  case TSEStkPitchshifterPins(CurrentPin.PinID) of
    ...
    pinStkSemitones:
      FStkPitchshifter.Shift := Power(2, FSemiTones / 12);
    pinStkMix:
      FStkPitchshifter.EffectMix := 0.01 * FEffectMix;
  end;
end;

...
type
  ...
  protected
    ...
    FCubeArray: Array [0 .. CNumHistory - 1, 0 .. CNumFrequencies - 1]
      of TGLCube;
  ...
  public
    ...
    property UseDownsampling: Boolean read FUseDownsampling
      write SetUseDownsampling default True;
...
uses
  Inifiles, Registry, GLVectorGeometry, DAV_Common, DAV_Convert;
...
begin
  ...
  GLSceneViewer.Camera.TargetObject := FCubeArray[0, 16];
...
begin
  ...
  begin
    // Lowpass
    DesiredFreq := CThirdOctaveFrequencies[CNumFrequencies - Band - 1] *
      HalfThirdMulFak;
    if DesiredFreq > 0.499 * SampleRate then
      DesiredFreq := 0.499 * SampleRate;

    if UseDownsampling then
      while ((2 * DesiredFreq * FSampleRateReciprocal) * (1 shl Downsampling)) <
        FBandReserve do
        Inc(Downsampling);
  ...
  begin
    ...
    begin
      ...
      if not(ssAlt in Shift) then
        pitchNow := ClampValue(pitchNow + Math.DegToRad(FOldMousePoint.Y - Y), 0.002, PI - 0.77);
      SetVector(normalT2C, AbsoluteUp);
      RotateVector(normalT2C, normalCameraRight, -pitchNow);
      if not(ssShift in Shift) then
        RotateVector(normalT2C, AbsoluteUp,
          -Math.DegToRad(FOldMousePoint.X - X));
      ...
      case LightSource.Position.Style of
        csPoint:
          LightSource.Position.SetPoint(newPos);
        csVector:
          LightSource.Position.SetVector(newPos);
      end;
    end;
    FOldMousePoint.X := X;
    FOldMousePoint.Y := Y;
...
begin
  ...
  begin
    ...
    for i := 0 to ASIOHost.InputChannelCount - 1 do
      CbChannel.Items.Add(string(ASIOHost.InputChannelInfos[i].Name));
    with TIniFile.Create(ExtractFilePath(ParamStr(0)) + 'ASIODemo.INI') do
      try
        WriteInteger('Audio', 'Asio Driver', CbDriver.ItemIndex);
      finally
...
begin
  ...
  begin
    if FHistoryCount = 0 then
      ...
      begin
        FCubeArray[History, Band].CubeHeight := FCubeArray[History - 1, Band]
          .CubeHeight;
...
begin
  if GLSceneViewer.Align <> alClient then
...
procedure TFmAnalyser.BSNormal(Sender: TObject;
...
begin
  if SampleRate = 0 then
    raise Exception.Create('Samplerate may not be zero!');
  FSampleRateReciprocal := 1 / SampleRate;
  UpdateFilters;
end;

procedure TFmAnalyser.BSDownSampled(Sender: TObject;
  const InBuffer, OutBuffer: TDAVArrayOfSingleFixedArray);
var
  i, j: Integer;
  d, Z, s: Double;

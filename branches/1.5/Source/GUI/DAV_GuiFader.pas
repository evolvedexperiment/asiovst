...
type
  ...
  public
    ...
    property Orientation: TFaderOrientation read FOrientation
      write SetOrientation default foVertical;
    ...
    property DrawCenterLine: Boolean read FDrawCenterLine
      write SetDrawCenterLine default False;
  end;
...
begin
  ...
  Filer.DefineProperty('Max', ReadMaxProperty, WriteMaxProperty, Maximum = 0);
...
procedure TGuiCustomFader.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
      ...
      foHorizontal:
        begin
          if Assigned(FImageItem) then
            NormalizedPosition :=
              Limit((X - FImageItem.Width div 2) / (Width - FImageItem.Width
              - 1), 0, 1)
          else
            NormalizedPosition := Limit(X / (Width - 1), 0, 1);
          Value := Limit(FMinimum + MapNormalizedPositionToNormalizedValue
            (NormalizedPosition) * (FMaximum - FMinimum), FMinimum, FMaximum);
        end;
      foVertical:
        begin
          if Assigned(FImageItem) then
            NormalizedPosition :=
              1 - Limit((Y - FImageItem.Height div 2) /
              (Height - FImageItem.Height - 1), 0, 1)
          else
            NormalizedPosition := 1 - Limit(Y / (Height - 1), 0, 1);
          Value := Limit(FMinimum + MapNormalizedPositionToNormalizedValue
            (NormalizedPosition) * (FMaximum - FMinimum), FMinimum, FMaximum);
        end;
    end;
      ...
      case FOrientation of
        foHorizontal:
          begin
            ...
            begin
              FBuffer.HorizontalLine(Width div 2, Self.Width - Width div 2,
                Self.Height div 2, pxBlack32);
              FBuffer.HorizontalLine(Width div 2, Self.Width - Width div 2,
                Self.Height div 2 + 1, pxBlack32);
            end;

            Offset := (Self.Height - FImageItem.Height) div 2;
            Assert(NormalizedValue <= 1);
            YOff := Round(MapNormalizedValueToNormalizedPosition
              (NormalizedValue) * (Self.Width - Width));
        ...
        foVertical:
          begin
            if FDrawCenterLine then
            begin
              FBuffer.VerticalLine(Self.Width div 2, Height div 2,
                Self.Height - Height div 2, pxBlack32);
              FBuffer.VerticalLine(Self.Width div 2 + 1, Height div 2,
                Self.Height - Height div 2, pxBlack32);
            end;

            Offset := (Self.Width - FImageItem.Width) div 2;
            YOff := Round
              ((1 - MapNormalizedValueToNormalizedPosition(NormalizedValue)) *

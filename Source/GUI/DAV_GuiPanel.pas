...
begin
  ...
  else if Dest is TCustomGuiPanel then
...
begin
  ...
  begin
    ...
    Radius := ConvertToFixed24Dot8(Min(FRoundRadius, 0.5 * Min(Width,
      Height)) + 1);
    ...
    if RadiusMinusOne.Fixed < 0 then
      RadiusMinusOne.Fixed := 0;

    RadiusMinusBorder.Fixed := Radius.Fixed - BorderWidthFixed.Fixed;
    if RadiusMinusBorder.Fixed < 0 then
      RadiusMinusBorder.Fixed := 0;
    SqrRadMinusBorder := FixedSqr(RadiusMinusBorder);

    SqrRadMinusBorderOne.Fixed := RadiusMinusBorder.Fixed -
      CFixed24Dot8One.Fixed;
    ...
    begin
      ...
      begin
        ...
        if XFixed.Fixed < RadiusMinusOne.Fixed then
          SqrDist.Fixed := FixedSqr(FixedSub(XFixed, RadiusMinusOne)).Fixed +
            SqrYDist.Fixed
        else
        begin
          Temp.Fixed := ConvertToFixed24Dot8(Integer(Width - 1)).Fixed -
            RadiusMinusOne.Fixed;
...
begin
  ...
  if not FNative and ParentColor then
    FBufferChanged := True;

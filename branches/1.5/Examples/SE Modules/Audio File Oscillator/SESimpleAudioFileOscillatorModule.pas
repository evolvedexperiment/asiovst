...
procedure TSESimpleAudioFileOscillatorModule.SubProcess(const BufferOffset,
    ...
    if FAudioData.SampleFrames > 0 then
      ...
      begin
        FOutputBuffer[BufferOffset + Sample] :=
          FAudioData[0].ChannelDataPointer^[FPosition];
        inc(FPosition);
        if FPosition >= FAudioData[0].SampleCount then
          FPosition := 0;
      end
    else
...
class procedure TSESimpleAudioFileOscillatorModule.GetModuleProperties
...
function TSESimpleAudioFileOscillatorModule.GetPinProperties

  ...
  begin
    ...
    if PtInRect(GRectTop[0], Pt) then
      if ssCtrl in Shift then
        ...
        FEditValue := edLow
    else if PtInRect(GRectTop[1], Pt) then
      if ssCtrl in Shift then
        Parameter[1] := 2500
      else
        FEditValue := edHigh
    else if PtInRect(GRectTop[2], Pt) then
      if ssCtrl in Shift then
        Parameter[24] := 0
      else
        FEditValue := edVolume
    else if PtInRect(GRectTop[3], Pt) then
    begin
      FBackground[0].DrawMode := dmOpaque;
      FBackground[0].DrawTo(FBackground[1]);
      ...
      begin
        ...
        begin
          if ssCtrl in Shift then
            Parameter[5 + Stage * 7] := -10
          else
            FEditValue := edThreshold;
          break;
        end
        else if PtInRect(GRectStage[FStage, 1], Pt) then
        begin
          if ssCtrl in Shift then
            Parameter[6 + Stage * 7] := 4
          else
            FEditValue := edRatio;
          break;
        end
        else if PtInRect(GRectStage[FStage, 2], Pt) then
        begin
          if ssCtrl in Shift then
            Parameter[3 + Stage * 7] := 5
          else
            FEditValue := edAttack;
          break;
        end
        else if PtInRect(GRectStage[FStage, 3], Pt) then
        begin
          if ssCtrl in Shift then
            Parameter[4 + Stage * 7] := 50
          else
            FEditValue := edRelease;
          break;
        end
        else if PtInRect(GRectStage[FStage, 4], Pt) then
        begin
          if ssCtrl in Shift then
            Parameter[7 + Stage * 7] := 1
          else
            FEditValue := edKnee;
          break;
        end
        else if PtInRect(GRectStage[FStage, 5], Pt) then
        begin
          if ssCtrl in Shift then
            Parameter[8 + Stage * 7] := 3
          else
            FEditValue := edMakeUpGain;
          break;
        end
        else if PtInRect(GRectStage[FStage, 7], Pt) then
        begin
          FBackground[0].DrawMode := dmOpaque;
          FBackground[0].DrawTo(FBackground[1]);
          if bsSmooth in BandStates[Stage] then
            Parameter[9 + Stage * 7] := 0
          else
            Parameter[9 + Stage * 7] := 4;
          FAnimateFrame := -$20;
          Exit;
        end
        else if PtInRect(GRectStage[FStage, 8], Pt) and (ssCtrl in Shift) then
        begin
          FBackground[0].DrawMode := dmOpaque;
          FBackground[0].DrawTo(FBackground[1]);
...
begin
  with TSmoothMultibandCompressorDataModule(Owner) do
    case FEditValue of
      edLow:
        ...
        begin
          ...
          if ssAlt in Shift then
            s := s * Power(2, 5E-3 * (Pt.Y - Y))
          else
            s := s * Power(2, 0.01 * (Pt.Y - Y)) * Power(2, 5E-3 * (X - Pt.X));
          with ParameterProperties[0] do
            Parameter[0] := Limit(s, Min, Max);
        end;
      edHigh:
        with GRectTop[1] do
        begin
      ...
      edVolume:
        ...
        begin
          ...
          if ssAlt in Shift then
            ...
            s := s + 0.016 * (Pt.Y - Y) - 0.002 * (Pt.X - X);
          with ParameterProperties[24] do
            Parameter[24] := Limit(s, Min, Max);
        end;
      edThreshold:
        with GRectStage[FStage, 0] do
        begin
          Param := 5 + FStage * 7;
          Pt := Point((3 * Left + Right) div 4, (3 * Top + Bottom) div 4);
          s := Parameter[Param];
          if ssAlt in Shift then
            ...
            s := s + 0.05 * (Pt.Y - Y) - 0.01 * (Pt.X - X);
          if not(ssShift in Shift) then
            with ParameterProperties[Param] do
              Parameter[Param] := Limit(s, Min, Max)
          else
            for Stage := 0 to 2 do
            begin
              Param := 5 + Stage * 7;
      ...
      edRatio:
        with GRectStage[FStage, 1] do
        begin
          Param := 6 + FStage * 7;
          Pt := Point((3 * Left + Right) div 4, (3 * Top + Bottom) div 4);
          s := Parameter[Param];
          if ssAlt in Shift then
            s := s * Power(2, 2E-3 * (Pt.Y - Y))
          else
            s := s * Power(2, 0.008 * (Pt.Y - Y)) * Power(2, 2E-3 * (Pt.X - X));
          if not(ssShift in Shift) then
            with ParameterProperties[Param] do
              Parameter[Param] := Limit(s, Min, Max)
          else
      ...
      edAttack:
        ...
        begin
          ...
          if ssAlt in Shift then
            ...
            s := s * Power(2, 0.01 * (Pt.Y - Y)) * Power(2, 5E-3 * (Pt.X - X));
          if not(ssShift in Shift) then
            with ParameterProperties[Param] do
              Parameter[Param] := Limit(s, Min, Max)
          else
...
begin
  ...
  if PtInRect(GRectTop[0], MousePos) then
    FEditValue := edLow
  else if PtInRect(GRectTop[1], MousePos) then
    FEditValue := edHigh
  else if PtInRect(GRectTop[2], MousePos) then
    FEditValue := edVolume
  else
    for Stage := 0 to Length(GRectStage) - 1 do
    begin
      ...
      else if PtInRect(GRectStage[Stage, 1], MousePos) then
      begin
        FEditValue := edRatio;
        FStage := Stage;
        break;
      end
      else if PtInRect(GRectStage[Stage, 4], MousePos) then
      begin
        FEditValue := edKnee;
        FStage := Stage;
        break;
      end
      else if PtInRect(GRectStage[Stage, 5], MousePos) then
      begin
        FEditValue := edMakeUpGain;
        FStage := Stage;
        break;
      end
      else if PtInRect(GRectStage[Stage, 2], MousePos) then
      begin
        FEditValue := edAttack;
        FStage := Stage;
        break;
      end
      else if PtInRect(GRectStage[Stage, 3], MousePos) then
      begin
        FEditValue := edRelease;
        FStage := Stage;
        break;
  ...
  if WheelDelta > 0 then
    FormMouseMove(Sender, Shift + [ssAlt, ssLeft], MousePos.X,
      MousePos.Y - 10)
  else
    FormMouseMove(Sender, Shift + [ssAlt, ssLeft], MousePos.X,
      MousePos.Y + 10);
...
begin
  if FAnimateFrame >= 0 then
    FBackground[0].DrawTo(Canvas.Handle, 0, 0)
  else
  begin
...
begin
  ...
  PNG.Canvas.Lock;
  try
    for i := 0 to BMP.GlyphCount - 1 do
      BitBlt(BMP.Handle, 0, i * PNG.Height, PNG.Width, PNG.Height,
        PNG.Canvas.Handle, i * BMP.Width, 0, SRCCOPY);
  finally
    PNG.Canvas.UnLock;
  end;
end;
  ...
  with FBackground[0], TSmoothMultibandCompressorDataModule(Owner) do
    if not(bsBypass in BandStates[Stage]) then
      try
        ...
        if i < 0 then
          i := 0
        else if i >= GAutoGain.GlyphCount then
          i := GAutoGain.GlyphCount - 1;
        r := GRectStage[Stage, 7];
        Draw(r, r, GBG);
        Draw(r, Rect(0, i * GAutoGain.RealHeight, GAutoGain.RealWidth,
          (i + 1) * GAutoGain.RealHeight), GAutoGain);
      finally
        EndUpdate;
      end;
end;

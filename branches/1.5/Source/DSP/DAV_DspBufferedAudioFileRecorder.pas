...
type
  ...
  private
    ...
    procedure EncodeHandler(Sender: TObject;
  ...
  public
    ...
    procedure PutSamples(Left, Right: PDAVSingleFixedArray;
  ...
  public
    ...
    procedure PutSamples(Data: TDAVArrayOfSingleFixedArray;
      SampleFrames: Integer);
...
begin
  ...
  if (FBuffer.BufferSize - SampleInBuffer) > SampleFrames then
    FBuffer.WriteBuffer(Data, SampleFrames)
  else
  begin
    if SampleInBuffer > 0 then
      FBuffer.WriteBuffer(Data, (FBuffer.BufferSize - SampleInBuffer));
...
begin
  ...
  begin
    if ChannelCount = 0 then
      Exit
    else if ChannelCount = 1 then
      Move(FStreamBuffer^[FSubBlockPosition], ChannelPointer[0]^[0],
        SampleFrames * SizeOf(Single))
    else
    begin
      Move(FStreamBuffer^[FSubBlockPosition], ChannelPointer[0]^[0],
        SampleFrames * SizeOf(Single));
      Move(FStreamBuffer^[FSubBlockPosition], ChannelPointer[1]^[0],
        SampleFrames * SizeOf(Single));
    end;
...
begin
  ...
  begin
    while (not Terminated) and (FBuffer.SamplesInBuffer > FStreamBufSize) do
    begin
      IdleLoops := 20;
      ...
      begin
        ...
        FSubBlockPosition := 0;
        FAudioFile.Encode(FCurrentPosition * FAudioFile.ChannelCount,
          FStreamBufSize);
...
begin
  SampleInBuffer := FBuffer.SamplesInBuffer;
  if (FBuffer.BufferSize - SampleInBuffer) > SampleFrames then
    FBuffer.WriteBuffer(Left, Right, SampleFrames)
  else
  begin
    if SampleInBuffer > 0 then
      FBuffer.WriteBuffer(Left, Right, (FBuffer.BufferSize - SampleInBuffer));
    // eventually enlarge buffer here!
  end;
end;
...
begin
  ...
  begin
    if ChannelCount = 0 then
      ...
      Move(FStreamBuffer[0]^[FSubBlockPosition], ChannelPointer[0]^[0],
        SampleFrames * SizeOf(Single))
    else
    begin
      Move(FStreamBuffer[0]^[FSubBlockPosition], ChannelPointer[0]^[0],
        SampleFrames * SizeOf(Single));
      Move(FStreamBuffer[1]^[FSubBlockPosition], ChannelPointer[1]^[0],
        SampleFrames * SizeOf(Single));
    end;
    FSubBlockPosition := FSubBlockPosition + SampleFrames;
  end;
...
begin
  ...
  begin
    for Channel := Length(FStreamBuffer) - 1 downto ChannelCount do
      Dispose(FStreamBuffer[Channel]);
    SetLength(FStreamBuffer, ChannelCount);
  end
  else if ChannelCount > Length(FStreamBuffer) then
  begin
    SetLength(FStreamBuffer, ChannelCount);

...
{$IFDEF DELPHI10_UP} {$REGION 'TVstPlugIn'} {$ENDIF}
  ...
  public
    procedure ProcessAudioDataCollection(Inputs,
      Outputs: TAudioDataCollection32); overload; virtual;
    procedure ProcessAudioDataCollection(Inputs,
      Outputs: TAudioDataCollection64); overload; virtual;
    procedure ProcessAudioDataCollectionInplace
      (AudioData: TAudioDataCollection32); overload; virtual;
    procedure ProcessAudioDataCollectionInplace
      (AudioData: TAudioDataCollection64); overload; virtual;
  end;
...
{$ENDIF}
  ...
  protected
    property Items[Index: Integer]: TVstPlugInAudioData read GetItem
...
var
  Channel: Integer;
  InList, OutList: array of PDAVSingleFixedArray;
begin
  ...
  for Channel := 0 to Inputs.ChannelCount - 1 do
    InList[Channel] := Inputs[Channel].ChannelDataPointer;

  SetLength(OutList, Outputs.ChannelCount);
  for Channel := 0 to Outputs.ChannelCount - 1 do
    OutList[Channel] := Outputs[Channel].ChannelDataPointer;
...
var
  Channel: Integer;
  InList, OutList: array of PDAVDoubleFixedArray;
begin
  Assert(Inputs.SampleFrames = Outputs.SampleFrames);
  Assert(Inputs.ChannelCount >= numInputs);
  Assert(Outputs.ChannelCount >= numOutputs);
...
begin
  ...
  for Channel := 0 to AudioData.ChannelCount - 1 do
    DataList[Channel] := AudioData[Channel].ChannelDataPointer;
...
begin
  ...
  Process64Replacing(PPDouble(DataList), PPDouble(DataList),
    AudioData.SampleFrames);
end;
...
function TVstPlugInsAudioData.CloneAdd(Source: TVstPlugInAudioData)
  : TVstPlugInAudioData;
begin
  Result := TVstPlugInAudioData(inherited Add);
...
procedure TVstPlugInsAudioData.SetItem(Index: Integer;
  const Value: TVstPlugInAudioData);
begin

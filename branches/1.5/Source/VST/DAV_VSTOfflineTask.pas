...
type
  ...
  public
    ...
    constructor Create(const ExternalVstOfflineTaskRecord
      : PVstOfflineTaskRecord); overload; virtual;
    ...
    property SizeInputBuffer: LongInt read GetSizeInputBuffer
      write SetSizeInputBuffer;
    property SizeOutputBuffer: LongInt read GetSizeOutputBuffer
      write SetSizeOutputBuffer;
    ...
    property PositionToProcessFrom: Double read GetPositionToProcessFrom
      write SetPositionToProcessFrom;
    property NumFramesToProcess: Double read GetNumFramesToProcess
      write SetNumFramesToProcess;
    property MaxFramesToWrite: Double read GetMaxFramesToWrite
      write SetMaxFramesToWrite;
    ...
    property NumFramesInSourceFile: Double read GetNumFramesInSourceFile
      write SetNumFramesInSourceFile;
    property SourceSampleRate: Double read GetSourceSampleRate
      write SetSourceSampleRate;
    property DestinationSampleRate: Double read GetDestinationSampleRate
      write SetDestinationSampleRate;
    property NumSourceChannels: LongInt read GetNumSourceChannels
      write SetNumSourceChannels;
    property NumDestinationChannels: LongInt read GetNumDestinationChannels
      write SetNumDestinationChannels;
    property SourceFormat: LongInt read GetSourceFormat write SetSourceFormat;
    property DestinationFormat: LongInt read GetDestinationFormat
      write SetDestinationFormat;
  ...
  public
    property VstOfflineTaskRecord: TVstOfflineTaskRecord
      read GetVstOfflineTaskRecord;
  published
...
begin
  if Dest is TCustomVstOfflineTask then
    TCustomVstOfflineTask(Dest).FVstOfflineTaskRecord := FVstOfflineTaskRecord
  else
    inherited;
end;
...
constructor TCustomVstOfflineTask.Create(const ExternalVstOfflineTaskRecord
  : PVstOfflineTaskRecord);
begin
  inherited Create;
...
begin
  if not FIsExternal then
    Dispose(FVstOfflineTaskRecord);
  inherited;
end;
...
begin
  if Value < 0 then
    Value := 0;
  Assert(Assigned(FVstOfflineTaskRecord));
  if FVstOfflineTaskRecord.DestinationSampleRate <> Value then
    FVstOfflineTaskRecord.DestinationSampleRate := Value;
end;
...
begin
  if Value < -1 then
    Value := -1;
  Assert(Assigned(FVstOfflineTaskRecord));
  if FVstOfflineTaskRecord.MaxFramesToWrite <> Value then
    FVstOfflineTaskRecord.MaxFramesToWrite := Value;
end;
...
begin
  if Value < -1 then
    Value := -1;
  Assert(Assigned(FVstOfflineTaskRecord));
  if FVstOfflineTaskRecord.NumDestinationChannels <> Value then
    FVstOfflineTaskRecord.NumDestinationChannels := Value;
end;
...
begin
  if Value < -1 then
    Value := -1;
  Assert(Assigned(FVstOfflineTaskRecord));
  if FVstOfflineTaskRecord.NumFramesInSourceFile <> Value then
    FVstOfflineTaskRecord.NumFramesInSourceFile := Value;
end;
...
begin
  if Value < -1 then
    Value := -1;
  Assert(Assigned(FVstOfflineTaskRecord));
  if FVstOfflineTaskRecord.NumFramesToProcess <> Value then
    FVstOfflineTaskRecord.NumFramesToProcess := Value;
end;
...
begin
  if Value < -1 then
    Value := -1;
  Assert(Assigned(FVstOfflineTaskRecord));
  if FVstOfflineTaskRecord.NumSourceChannels <> Value then
    FVstOfflineTaskRecord.NumSourceChannels := Value;
end;
...
begin
  ...
  // fill rest with zero
  if TextSize < 512 then
    FillChar(FVstOfflineTaskRecord.OutputText[TextSize], 512 - TextSize, 0);
end;
...
begin
  ...
  // fill rest with zero
  if TextSize < 96 then
    FillChar(FVstOfflineTaskRecord.ProcessName[TextSize], 96 - TextSize, 0);
end;
...
begin
  ...
  // fill rest with zero
  if TextSize < 100 then
    FillChar(FVstOfflineTaskRecord.ProgressText[TextSize], 100 - TextSize, 0);
end;
...
begin
  if Value < -1 then
    Value := -1;
  if FVstOfflineTaskRecord.SizeInputBuffer <> Value then
    FVstOfflineTaskRecord.SizeInputBuffer := Value;
end;
...
begin
  if Value < -1 then
    Value := -1;
  if FVstOfflineTaskRecord.SizeOutputBuffer <> Value then
    FVstOfflineTaskRecord.SizeOutputBuffer := Value;
end;
...
begin
  if Value < 0 then
    Value := 0;
  if FVstOfflineTaskRecord.SourceSampleRate <> Value then
    FVstOfflineTaskRecord.SourceSampleRate := Value;
end;

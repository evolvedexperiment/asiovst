...
constructor TCustomHrir.Create(Azimuth, Polar, Radius: Single;
  ...
  const Left, Right: PDAVHalfFloatFixedArray);
begin
  inherited Create;
  FChunkName := GetClassChunkName;
  FillChar(FHrirHeader, SizeOf(THrirHeader), 0);
  ...
  CreateBuffers;
  Move(Left^[0], PDAVHalfFloatFixedArray(FBuffer[0])^[0],
    SampleFrames * SizeOf(THalfFloat));
  Move(Right^[0], PDAVHalfFloatFixedArray(FBuffer[1])^[0],
    SampleFrames * SizeOf(THalfFloat));
...
begin
  ...
  CreateBuffers;
  Move(Left^[0], PDAVSingleFixedArray(FBuffer[0])^[0],
    SampleFrames * SizeOf(Single));
  Move(Right^[0], PDAVSingleFixedArray(FBuffer[1])^[0],
    SampleFrames * SizeOf(Single));
  FChunkSize := GetChunkSize;
end;
...
begin
  ...
  CreateBuffers;
  Move(Left^[0], PDAVDoubleFixedArray(FBuffer[0])^[0],
    SampleFrames * SizeOf(Double));
  Move(Right^[0], PDAVDoubleFixedArray(FBuffer[1])^[0],
    SampleFrames * SizeOf(Double));
  FChunkSize := GetChunkSize;
end;

constructor TCustomHrir.Create(Azimuth, Polar: Single; const SampleRate: Single;
  const SampleFrames: Integer; const Left, Right: PDAVHalfFloatFixedArray);
begin
...
begin
  with FHrirHeader do
    Result := SizeOf(FHrirHeader) + 2 * SampleFrames * BytesPerSample;
end;
...
procedure TCustomHrir.MoveData32(Destination: PDAVSingleFixedArray;
  Index, SampleFrames: Integer);
var
  Sample: Integer;
begin
  Assert(Index in [0 .. 1]);
      ...
      case BytesPerSample of
        2:
          for Sample := 0 to SampleFrames - 1 do
            Destination^[Sample] :=
              HalfFloatToSingle(PDAVHalfFloatFixedArray(FBuffer[Index])
              ^[Sample]);
        4:
          Move(FBuffer[Index]^, Destination^[0], SampleFrames * SizeOf(Single));
        8:
          for Sample := 0 to SampleFrames - 1 do
            Destination^[Sample] := PDAVDoubleFixedArray(FBuffer[Index]
              )^[Sample];
      else
...
procedure TCustomHrir.MoveData64(Destination: PDAVDoubleFixedArray;
  Index, SampleFrames: Integer);
var
  Sample: Integer;
begin
  ...
  case Encoding of
    heFloat:
      case BytesPerSample of
        ...
        4:
          for Sample := 0 to SampleFrames - 1 do
            Destination^[Sample] := PDAVSingleFixedArray(FBuffer[Index]
              )^[Sample];
        8:
          Move(FBuffer[Index]^, Destination^[0], SampleFrames * SizeOf(Single));
      else
...
procedure TCustomHrir.AssignData32(Source: PDAVSingleFixedArray;
  Index, SampleFrames: Integer);
var
  Sample: Integer;
begin
  ...
  case Encoding of
    heFloat:
      case BytesPerSample of
        2:
          for Sample := 0 to SampleFrames - 1 do
            PDAVHalfFloatFixedArray(FBuffer[Index])^[Sample] :=
              SingleToHalfFloat(Source^[Sample]);
        4:
          Move(Source^[0], FBuffer[Index]^, SampleFrames * SizeOf(Single));
        8:
          for Sample := 0 to SampleFrames - 1 do
            PDAVDoubleFixedArray(FBuffer[Index])^[Sample] := Source^[Sample];
      else
        raise Exception.Create('not yet implemented');
      end;
    heInteger:
...
procedure TCustomHrir.AssignData64(Source: PDAVDoubleFixedArray;
  Index, SampleFrames: Integer);
var
  Sample: Integer;
begin
  ...
  case Encoding of
    heFloat:
      case BytesPerSample of
        ...
        4:
          for Sample := 0 to SampleFrames - 1 do
            PDAVSingleFixedArray(FBuffer[Index])^[Sample] := Source^[Sample];
        8:
          Move(Source^[0], FBuffer[Index]^, SampleFrames * SizeOf(Single));
      else
        raise Exception.Create('not yet implemented');
...
begin
  ...
  begin
    ...
    // check constraints
    Assert(Integer(FChunkSize) - (SizeOf(THrirHeader)) >= 2 * SampleFrames *
      FHrirHeader.BytesPerSample);
...
begin
  ...
  with FGeneralInformationRecord do
    Result := 5 + Byte(Title[0]) + SizeOf(Date) + Byte(Context[0]) +
...
begin
  case Index of
    0:
      Result := string(FGeneralInformationRecord.Title);
    1:
      Result := string(FGeneralInformationRecord.Context);
    2:
      Result := string(FGeneralInformationRecord.Copyright);
    3:
      Result := string(FGeneralInformationRecord.Author);
    4:
      Result := string(FGeneralInformationRecord.Notes);
  end;
end;
...
begin
  case Index of
    0:
      FGeneralInformationRecord.Title := ShortString(Value);
    1:
      FGeneralInformationRecord.Context := ShortString(Value);
    2:
      FGeneralInformationRecord.Copyright := ShortString(Value);
    3:
      FGeneralInformationRecord.Author := ShortString(Value);
    4:
      FGeneralInformationRecord.Notes := ShortString(Value);
  end;
end;
...
begin
  // calculate chunk size
  with FSubjectRecord do
    Result := 2 + Byte(ID[0]) + SizeOf(Sex) + Byte(Desc[0]);
end;
...
begin
  case Index of
    0:
      Result := string(FSubjectRecord.ID);
    1:
      Result := string(FSubjectRecord.Desc);
  end;
end;
...
begin
  case Index of
    0:
      FSubjectRecord.ID := ShortString(Value);
    1:
...
begin
  // calculate chunk size
  with FRoomRecord do
    Result := 3 * SizeOf(Single) + Byte(RoomType[0]) + 1;
end;
...
begin
  if Value >= 0 then
    ...
    raise Exception.Create(RCStrPositiveValueOnly);
end;
...
begin
  // calculate chunk size
  with FMicrophoneRecord do
    Result := 3 + Byte(MicType[0]) + Byte(Manufacturer[0]) + Byte(Notes[0]);
end;
...
begin
  case Index of
    0:
      Result := string(FMicrophoneRecord.MicType);
    1:
      Result := string(FMicrophoneRecord.Manufacturer);
    2:
      Result := string(FMicrophoneRecord.Notes);
  end;
end;
...
begin
  case Index of
    0:
      FMicrophoneRecord.MicType := ShortString(Value);
    1:
      FMicrophoneRecord.Manufacturer := ShortString(Value);
    2:
...
begin
  ...
  begin
    ...
    // read 'Notes' string
    Read(StringSize, 1);
    Assert(StringSize + 3 + Byte(MicType[0]) + Byte(Manufacturer[0]) <=
      FChunkSize);
    SetLength(Notes, StringSize);
    Read(Notes[1], StringSize);
  end;
...
begin
  with FOutboardRecord do
    Result := 4 + Byte(ADConverter[0]) + Byte(DAConverter[0]) +
...
begin
  with FOutboardRecord do
    case Index of
      0:
        Result := string(ADConverter);
      1:
        Result := string(DAConverter);
      2:
        Result := string(Amplifier);
      3:
        Result := string(Loudspeaker);
    end;
end;
...
begin
  with FOutboardRecord do
    case Index of
      0:
        ADConverter := ShortString(Value);
      1:
        DAConverter := ShortString(Value);
      2:
        Amplifier := ShortString(Value);
      3:
...
begin
  ...
  begin
    ...
    Read(StringSize, 1);
    Assert(StringSize + 3 + Byte(ADConverter[0]) + Byte(DAConverter[0]) <
      FChunkSize);
...
begin
  // calculate chunk size
  with FMeasurementRecord do
    Result := 2 + SizeOf(Distance) + Byte(MeasurementType[0]) +
...
begin
  case Index of
    0:
      Result := string(FMeasurementRecord.MeasurementType);
    1:
      Result := string(FMeasurementRecord.ExcitationType);
  end;
end;
...
begin
  if Value > 0 then
    FMeasurementRecord.MeasuredLength := Value
  else
    raise Exception.Create(RCStrPositiveValueOnly);
end;
...
begin
  case Index of
    0:
      FMeasurementRecord.MeasurementType := ShortString(Value);
    1:
...
begin
  if Value then
    FMeasurementRecord.Flags := 1
  else
    FMeasurementRecord.Flags := 0;
end;
...
begin
  if Assigned(FGeneralInformation) then
    Result := FGeneralInformation.Date
  else
    Result := Now;
end;
...
begin
  if Assigned(FMeasurementInformation) then
    Result := FMeasurementInformation.Distance
  else
    Result := 0;
end;
...
begin
  if Index in [0 .. (FHrirList.Count - 1)] then
    Result := TCustomHrir(FHrirList[Index])
  else
    Result := nil;
end;
...
begin
  ...
  for Index := 0 to FHrirList.Count - 1 do
    if TCustomHrir(FHrirList[Index]).SampleFrames > Result then
      Result := TCustomHrir(FHrirList[Index]).SampleFrames;
end;
...
begin
  if Assigned(FMeasurementInformation) then
    Result := FMeasurementInformation.MeasuredLength
  else
    Result := 0;
end;
...
begin
  if Assigned(FRoomInformation) then
    Result := FRoomInformation.RoomType
  else
    Result := '';
end;
...
begin
  if Assigned(FSubjectInformation) then
    Result := FSubjectInformation.Sex
  else
    Result := stUnknown;
end;
...
begin
  if not Assigned(FMeasurementInformation) then
    Result := FMeasurementInformation.Symmetric
  else
    Result := False;
end;
    ...
    itLinear, itLinear3:
      begin
        ...
        try
          ...
          for Sample := 0 to SampleFrames - 1 do
            Left^[Sample] := Scale[0] * TempData^[Sample] + Scale[1] *
              Left^[Sample];
          ...
          for Sample := 0 to SampleFrames - 1 do
            Right^[Sample] := Scale[0] * TempData^[Sample] + Scale[1] *
              Right^[Sample];
        finally
          Dispose(TempData);
        end;
    ...
    itLinear:
      begin
        ...
        try
          ...
          Hrirs[1].MoveLeft32(Left, SampleFrames);
          for Sample := 0 to SampleFrames - 1 do
            Left^[Sample] := Scale[0] * TempData[0]^[Sample] + Scale[1] *
              Left^[Sample];
          ...
          Hrirs[1].MoveRight32(Right, SampleFrames);
          for Sample := 0 to SampleFrames - 1 do
            Right^[Sample] := Scale[0] * TempData[0]^[Sample] + Scale[1] *
              Right^[Sample];
        finally
          Dispose(TempData[0]);
        end;
    ...
    itLinear3:
      begin
        ...
        begin
          ...
          AngleA[1] := GetOrthodromicAngle2D(Hrirs[0].Position,
            Hrirs[1].Position);
        ...
        try
          ...
          try
            ...
            for Sample := 0 to SampleFrames - 1 do
              Left^[Sample] := Scale[0] * TempData[0]^[Sample] + Scale[1] *
                TempData[1]^[Sample] + Scale[2] * Left^[Sample];
            ...
            for Sample := 0 to SampleFrames - 1 do
              Right^[Sample] := Scale[0] * TempData[0]^[Sample] + Scale[1] *
                TempData[1]^[Sample] + Scale[2] * Right^[Sample];
          finally
            Dispose(TempData[1]);
          end;
        finally
  ...
  case FInterpolationType of
    ...
    itLinear3:
      begin
        ...
        Angles[0] := arccos((PntA[0] - PntA[1] * PntA[2]) /
          (sqrt(1 - sqr(PntA[1])) * sqrt(1 - sqr(PntA[2]))));
        Angles[1] := arccos((PntA[1] - PntA[2] * PntA[0]) /
          (sqrt(1 - sqr(PntA[2])) * sqrt(1 - sqr(PntA[0]))));
        Angles[2] := arccos((PntA[2] - PntA[0] * PntA[1]) /
          (sqrt(1 - sqr(PntA[0])) * sqrt(1 - sqr(PntA[1]))));

        // calculate triangle angles between (using spherical trigonometry)
        HalfAng[0] := (AngleA[1] - AngleA[0] * PntA[2]);
        if HalfAng[0] <> 0 then
          HalfAng[0] :=
            arccos(Limit(HalfAng[0] / (sqrt(1 - sqr(AngleA[0])) *
            sqrt(1 - sqr(PntA[2])))));

        HalfAng[1] := (AngleA[2] - AngleA[1] * PntA[0]);
        if HalfAng[1] <> 0 then
          HalfAng[1] :=
            arccos(Limit(HalfAng[1] / (sqrt(1 - sqr(AngleA[1])) *
            sqrt(1 - sqr(PntA[0])))));

        HalfAng[2] := (AngleA[0] - AngleA[2] * PntA[1]);
        if HalfAng[2] <> 0 then
          HalfAng[2] :=
            arccos(Limit(HalfAng[2] / (sqrt(1 - sqr(AngleA[2])) *
            sqrt(1 - sqr(PntA[1])))));
...
function TCustomHrtfs.FindNearestHrirs(const SpherePos: TSphereVector2D)
  : TCustomHrir;
var
...
function TCustomHrtfs.FindNearestHrirs(const SpherePos: TSphereVector3D)
  : TCustomHrir;
var
  i: Integer;
  CurrentAngle: Single;
...
function TCustomHrtfs.FindSecondNearestHrirs(const SpherePos: TSphereVector2D;
  const Nearest: TCustomHrir): TCustomHrir;
var
  SpherePos3D: TSphereVector3D;
begin
  SpherePos3D.Azimuth := SpherePos.Azimuth;
...
function TCustomHrtfs.FindSecondNearestHrirs(const SpherePos: TSphereVector3D;
  const Nearest: TCustomHrir): TCustomHrir;
var
  i: Integer;
  CurrentAngle: Single;
...
begin
  ...
  // initialize with first HRIR
  if TCustomHrir(FHrirList[0]) <> Nearest then
    Result := TCustomHrir(FHrirList[0])
  else
    Result := TCustomHrir(FHrirList[1]);
  ...
  for i := 0 to FHrirList.Count - 1 do
    ...
    begin
      ...
      if (SpherePos.Polar = Nearest.Polar) and (TempHrir.Polar = SpherePos.Polar)
      then
      begin
...
begin
  if Assigned(FGeneralInformation) then
    FreeAndNil(FGeneralInformation);
  if Assigned(FSubjectInformation) then
    FreeAndNil(FSubjectInformation);
  if Assigned(FRoomInformation) then
    FreeAndNil(FRoomInformation);
  if Assigned(FMicrophoneInformation) then
    FreeAndNil(FMicrophoneInformation);
  if Assigned(FOutboardInformation) then
    FreeAndNil(FOutboardInformation);
  if Assigned(FMeasurementInformation) then
    FreeAndNil(FMeasurementInformation);
end;
...
procedure TCustomHrtfs.ConvertStreamToChunk(ChunkClass: TDavCustomChunkClass;
  Stream: TStream);
begin
  ...
  else if ChunkClass = TCustomHrirSubjectInformation then
  begin
    if Assigned(FSubjectInformation) then
      raise Exception.Create(RCStrChunkAlreadyExists);
    FSubjectInformation := TCustomHrirSubjectInformation.Create;
  ...
  else if ChunkClass = TCustomHrirRoomInformation then
  begin
    if Assigned(FRoomInformation) then
      raise Exception.Create(RCStrChunkAlreadyExists);
    FRoomInformation := TCustomHrirRoomInformation.Create;
  ...
  else if ChunkClass = TCustomHrirMicrophoneInformation then
  begin
    if Assigned(FMicrophoneInformation) then
      raise Exception.Create(RCStrChunkAlreadyExists);
    FMicrophoneInformation := TCustomHrirMicrophoneInformation.Create;
  ...
  else if ChunkClass = TCustomHrirOutboardInformation then
  begin
    if Assigned(FOutboardInformation) then
      raise Exception.Create(RCStrChunkAlreadyExists);
    FOutboardInformation := TCustomHrirOutboardInformation.Create;
  ...
  else if ChunkClass = TCustomHrirMeasurementInformation then
  begin
    if Assigned(FMeasurementInformation) then
      raise Exception.Create(RCStrChunkAlreadyExists);
    FMeasurementInformation := TCustomHrirMeasurementInformation.Create;

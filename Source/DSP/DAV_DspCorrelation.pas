...
{$ENDIF}{$ENDIF}
  public
    ...
    procedure CrossCorrelation(const SignalA, SignalB,
      Correlation: PDAVSingleFixedArray); overload; virtual;
    procedure CrossCorrelation(const Signal, SignalCorrelation
      : PDAVSingleFixedArray); overload; virtual;
    procedure AutoCorrelation(const Signal, Correlation: PDAVSingleFixedArray);
      overload; virtual;
    procedure AutoCorrelation(const SignalCorrelation: PDAVSingleFixedArray);
      overload; virtual;
  published
    property FFTOrder;
    property FFTSize;
...
{$ENDIF}
  public
    ...
    procedure CrossCorrelation(const SignalA, SignalB,
      Correlation: PDAVDoubleFixedArray); overload; virtual;
    procedure CrossCorrelation(const Signal, SignalCorrelation
      : PDAVDoubleFixedArray); overload; virtual;
    procedure AutoCorrelation(const Signal, Correlation: PDAVDoubleFixedArray);
      overload; virtual;
    procedure AutoCorrelation(const SignalCorrelation: PDAVDoubleFixedArray);
      overload; virtual;
  published
    property FFTOrder;
  end;
...
begin
  ...
  if Dest is TCorrelation32 then
    ...
    begin
      Move(Self.FSignalFreq^, FSignalFreq, (FFFTSizeHalf + 1) *
        SizeOf(TComplex32));
      Move(Self.FCorrelationFreq^, FCorrelationFreq,
        (FFFTSizeHalf + 1) * SizeOf(TComplex32));
    end
  else if Dest is TCorrelation64 then
...
procedure TCorrelation32.AutoCorrelation(const SignalCorrelation
...
procedure TCorrelation32.CrossCorrelation(const Signal, SignalCorrelation
  : PDAVSingleFixedArray);
begin
  ...
  if Dest is TCorrelation32 then
    ...
    begin
      Move(Self.FSignalFreq^, FSignalFreq, (FFFTSizeHalf + 1) *
        SizeOf(TComplex64));
      Move(Self.FCorrelationFreq^, FCorrelationFreq,
        (FFFTSizeHalf + 1) * SizeOf(TComplex64));
    end;
end;

procedure TCorrelation64.AutoCorrelation(const SignalCorrelation
...
procedure TCorrelation64.CrossCorrelation(const Signal, SignalCorrelation
  : PDAVDoubleFixedArray);
begin
  CrossCorrelation(Signal, Signal, SignalCorrelation);
end;

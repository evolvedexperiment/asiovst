...
type
  ...
  public
    ...
    procedure ProcessBuffer32(Buffer: TDAVArrayOfSingleFixedArray;
      IsOutput: Boolean = True); virtual;
    procedure ProcessBuffer64(Buffer: TDAVArrayOfDoubleFixedArray;
      IsOutput: Boolean = True); virtual;
  published
  ...
  public
    procedure ProcessBuffer32(Buffer: TDAVArrayOfSingleFixedArray;
      IsOutput: Boolean = True); override;
    procedure ProcessBuffer64(Buffer: TDAVArrayOfDoubleFixedArray;
      IsOutput: Boolean = True); override;
  published
...
procedure TASIOGenerator.ProcessBuffer32(Buffer: TDAVArrayOfSingleFixedArray;
  IsOutput: Boolean = True);
begin
  FillChar(Buffer[0], BlockSize * SizeOf(Single), 0);
end;

procedure TASIOGenerator.ProcessBuffer64(Buffer: TDAVArrayOfDoubleFixedArray;
  IsOutput: Boolean = True);
begin
...
procedure TASIOGeneratorNoise.ProcessBuffer32
  (Buffer: TDAVArrayOfSingleFixedArray; IsOutput: Boolean = True);
var
  ...
  if NoiseColor = ncWhite then
    for Channel := 0 to Length(Buffer) - 1 do
      for Sample := 0 to BlockSize - 1 do
        Buffer[Channel, Sample] := (2 * Random - 1)
    ...
    for Channel := 0 to Length(Buffer) - 1 do
      ...
      begin
        ...
        Buffer[Channel, Sample] := 0.1 *
          (FNoiZe[0] + FNoiZe[1] + FNoiZe[2] + FNoiZe[3] + FNoiZe[4] + FNoiZe[5]
          + FNoiZe[6] + FNoiZe[7] * 0.5362);
        FNoiZe[7] := FNoiZe[0] * 0.115926;
      end
...
procedure TASIOGeneratorNoise.ProcessBuffer64
  (Buffer: TDAVArrayOfDoubleFixedArray; IsOutput: Boolean);
var
  Channel, Sample: integer;
begin
  if NoiseColor = ncWhite then
...
procedure TASIOGeneratorTone.ProcessBuffer32
  (Buffer: TDAVArrayOfSingleFixedArray; IsOutput: Boolean = True);
var
  Channel, Sample: integer;
  CurrentPhase: Double;
begin
  ...
  begin
    case Waveform of
      wfSine:
        Buffer[Channel, Sample] := sin(FPhase * 2 * PI);
      wfSaw:
        if FPhase <= 0.5 then
          Buffer[Channel, Sample] := 2 * FPhase
        else
          Buffer[Channel, Sample] := 2 * FPhase - 2;
      wfSquare:
        if FPhase <= 0.5 then
      ...
      wfTriangle:
        if (FPhase >= 0.25) and (FPhase < 0.75) then
          ...
          Buffer[Channel, Sample] := 4 * (FPhase + 0.25) - 1
        else
          Buffer[Channel, Sample] := 4 * (FPhase + 0.25) - 5;
      wfPulse:
        if Sample = 0 then
  ...
  for Channel := 1 to Length(Buffer) - 1 do
    Move(Buffer[0, 0], Buffer[Channel, 0], BlockSize * SizeOf(Double));
end;

procedure TASIOGeneratorTone.ProcessBuffer64
  (Buffer: TDAVArrayOfDoubleFixedArray; IsOutput: Boolean);
var
  Channel, Sample: integer;
  CurrentPhase: Double;

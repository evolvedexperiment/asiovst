...
begin
  inherited;
  if Dest is TDavDefinedChunk then
    TDavDefinedChunk(Dest).FFilePosition := FFilePosition;
end;
...
begin
  ...
  begin
    {$IFDEF DELPHI5}
    for i := 0 to TDavCustomChunkContainer(Dest).FChunkList.Count - 1 do
      TDavCustomChunk(TDavCustomChunkContainer(Dest).FChunkList[i]).Assign(TDavCustomChunk(FChunkList[i]));
    {$ELSE}
...
begin
  for i := 0 to FChunkList.Count - 1 do
    RegisterChunkClass(TDavDefinedChunkClass(FChunkList[i].ClassType));
end;
...
begin
  ...
  begin
    if (ChunkName = 'RIFF') or (ChunkName = 'FORM') then
      ...
      FDataStream.Position := 0;
    while FDataStream.Position + 8 < FChunkSize do
      ConvertStreamToChunk(TDavUnknownChunkContainer, FDataStream);
  end;
end;

...
procedure ConvertReImToSquaredMagnitude32(const FrequencyDomain
  : PDAVComplexSingleFixedArray; const SqMagDomain: PDAVSingleFixedArray;
  BinCount: Integer);
procedure ConvertReImToSquaredMagnitude64(const FrequencyDomain
  : PDAVComplexDoubleFixedArray; const SqMagDomain: PDAVDoubleFixedArray;
  BinCount: Integer);
...
{$ENDIF}
  with FFT do
    try
      ...
      if (FFTSize - SampleFrames) > 0 then
        FillChar(TimeDomain[SampleFrames], (FFTSize - SampleFrames) *
          SizeOf(Single), 0);
      ...
      begin
        Move(TimeDomain[0], FilterKernel[0], (SampleFrames div 2) *
          SizeOf(Single));
        Move(TimeDomain[FFTSize - (SampleFrames div 2) - 1],
          FilterKernel[(SampleFrames div 2)], (SampleFrames div 2) *
          SizeOf(Single));
      end
      else
    ...
    try
      ...
      try
        ...
        for i := 0 to BinCount - 1 do
          Magnitude[i] := Sqrt(sqr(FreqDomain[i].Re) + sqr(FreqDomain[i].Im));
...
begin
  for i := 0 to BinCount - 1 do
    SqMagDomain[i] := sqr(FrequencyDomain[i].Re) + sqr(FrequencyDomain[i].Im);
end;

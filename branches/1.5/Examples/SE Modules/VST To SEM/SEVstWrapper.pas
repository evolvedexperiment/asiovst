...
type
  ...
  protected
    FVSTHost: TVstHost;
    FInputPtr: array of PDAVSingleFixedArray;
...
{$IFDEF UseMidi}
    ...
    FMidiOutPin: Integer;
    procedure MidiData(AClock: Cardinal; AMidiMsg: Cardinal;
      PinID: Integer); override;
...
constructor TCustomVST2SEModule.Create(SEAudioMaster: TSE2audioMasterCallback;
  Reserved: Pointer);
var
  Channel: Integer;
  RS: TResourceStream;
...
class procedure TCustomVST2SEModule.GetModuleProperties
...
function TCustomVST2SEModule.GetPinProperties(const Index: Integer;
...
begin
  ...
  else if Index - FVSTHost[0].numInputs < FVSTHost[0].numOutputs then
    with Properties^ do
    begin
...
{$IFDEF UseMidi}
  ...
  else if Index - FVSTHost[0].numInputs - FVSTHost[0].numOutputs = 0 then
    with Properties^ do
    begin
      Name := 'MIDI Input';
      Direction := drIn;
  ...
  else if Index - FVSTHost[0].numInputs - FVSTHost[0].numOutputs = 1 then
    with Properties^ do
    begin
      Name := 'MIDI Output';
      Direction := drOut;
...
procedure TCustomVST2SEModule.ProcessEvents(Sender: TObject;
  VstEvents: PVstEvents);
var
  EventIndex: Integer;
  AMidiMsg: Integer;
begin
...
procedure TCustomVST2SEModule.MidiData(AClock, AMidiMsg: Cardinal;
  PinID: Integer);
var
  IsSystemMsg: Boolean;
begin
...
procedure TStaticVST2SEModule.SubProcess(const BufferOffset,
  SampleFrames: Integer);
var
  Channel: Integer;
begin
  for Channel := 0 to Length(FVSTInputs) - 1 do
    FVSTInputs[Channel] := @FInputPtr[Channel, BufferOffset];
  for Channel := 0 to Length(FVSTOutputs) - 1 do
    FVSTOutputs[Channel] := @FOutputPtr[Channel, BufferOffset];
...
begin
  ...
  FS.DecimalSeparator := ',';
  if TryStrToFloat(S, dbl, FS) then
    Result := True;
end;
...
// describe the pins (plugs)
function TStaticVST2SEModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
var
  Channel: Integer;
  S, p: Single;
...
begin
  ...
  begin
    ...
    Param := (Index - FVSTHost[0].numInputs - FVSTHost[0].numOutputs
      {$IFDEF UseMidi} - 2 {$ENDIF});
    ...
    if Param < FVSTHost[0].numParams then
      ...
      begin
        ...
        pd[0] := FVSTHost[0].ParameterDisplay[Param] + FVSTHost[0]
          .ParameterLabel[Param];
        FVSTHost[0].Parameter[Param] := 0.01;
        pd[1] := FVSTHost[0].ParameterDisplay[Param] + FVSTHost[0]
          .ParameterLabel[Param];
        ...
        begin
          ...
          with TStringList.Create do
            try
              ...
              begin
                FVSTHost[0].Parameter[Param] := FVSTHost[0].Parameter
                  [Param] + S;
                ...
                begin
                  ...
                  un[0] := un[1];
                  Add(pd[0] + un[0]);
                  if not TryStrToFloatX(pd[0]) then
                    IsFlt := False;
                end;
              end;
              ...
              for Channel := 0 to Count - 1 do
                str := str + Strings[Channel] + '=' + IntToStr(Channel) + ',';
            finally
              Free;
            end;
...
begin
  if FVSTHost.Count >= 0 then
    with FVSTHost[0] do
    begin
      if (CurrentPin.PinID >= {$IFDEF UseMidi} 2 +
        {$ENDIF} numInputs + numOutputs) and (CurrentPin.PinID <
        {$IFDEF UseMidi} 2 + {$ENDIF} numInputs + numOutputs + numParams) then
      begin
        Param := CurrentPin.PinID {$IFDEF UseMidi} - 2
        {$ENDIF} - numInputs - numOutputs;
        assert((Param >= 0) and (Param < FVSTHost[0].numParams));
        if FParamEnum[Param] > 0 then
          Parameter[Param] := PInteger(@FParamPtr[Param])^ / FParamEnum[Param]
        else
          Parameter[Param] := FParamPtr[Param]
      end;
      ...
      for Channel := 0 to numInputs - 1 do
        if Pin[Channel].Status < InState then
          InState := Pin[Channel].Status;

      OutState := stRun;
      for Channel := numInputs to numInputs + numOutputs - 1 do
        if Pin[Channel].Status < OutState then
          OutState := Pin[Channel].Status;
      ...
      for Channel := numInputs to numInputs + numOutputs - 1 do
        Pin[Channel].TransmitStatusChange(SampleClock, OutState);
    end;
  inherited;
end;
...
constructor TAutomatableVST2SEModule.Create(SEAudioMaster
...
class procedure TAutomatableVST2SEModule.GetModuleProperties
  (Properties: PSEModuleProperties);
var
  str: string;
begin
...
begin
  ...
  if not Result then
  begin
    Param := Index {$IFDEF UseMidi} - 2
    {$ENDIF} - FVSTHost[0].numInputs - FVSTHost[0].numOutputs;
    assert((Param >= 0) and (Param <= FVSTHost[0].numParams));
...
var
  ...
  OutState: TSEStateType;
begin
  if not Assigned(CurrentPin) then
    exit;

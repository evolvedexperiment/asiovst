...
procedure TCustomBesselFilter.Complex(const Frequency: Double;
...
begin
  if Order = 0 then
    ...
    Result := (2 * Order - 1) * CalculateReverseBesselPolynomial(Order - 1, X) +
...
begin
  if Order < 2 then
    ...
    Result := (2 * Order - 1) * CalculateReverseBesselPolynomial0(Order - 1);
end;

function CalculateBesselFactor(Order, Term: Integer): Double;
begin
  Result := Factorial(Integer(2 * Order - Term)) /
    (IntPower(2, Order - Term) * Factorial(Integer(Term)) *
    Factorial(Integer(Order - Term)));
end;
...
begin
  ...
  begin
    t := 1 / ((((((K + 1 * 21) * K + 1 * 210) * K + 1 * 1260) * K + 1 * 4725) *
      K + 1 * 10395) * K + 1 * 10395);
    FB[0] := -(((((6 * K + 4 * 21) * K + 2 * 210) * K * K - 2 * 4725) * K - 4 *
      10395) * K - 6 * 10395) * t;
    FB[1] := -((((((15 * K + 5 * 21) * K - 1 * 210) * K - 3 * 1260) * K - 1 *
      4725) * K + 5 * 10395) * K + 15 * 10395) * t;
    FB[2] := -(((20 * K * K - 4 * 210) * K * K + 4 * 4725) * K * K - 20 *
      10395) * t;
    FB[3] := -((((((15 * K - 5 * 21) * K - 1 * 210) * K + 3 * 1260) * K - 1 *
      4725) * K - 5 * 10395) * K + 15 * 10395) * t;
    FB[4] := -(((((6 * K - 4 * 21) * K + 2 * 210) * K * K - 2 * 4725) * K + 4 *
      10395) * K - 6 * 10395) * t;
    FB[5] := -((((((K - 1 * 21) * K + 1 * 210) * K - 1 * 1260) * K + 1 * 4725) *
      K - 1 * 10395) * K + 1 * 10395) * t;
...
begin
  ...
  for i := 1 to FOrder - 1 do
    FState[i - 1] := FA[i] * Input + FB[i - 1] * Result + FState[i];
...
begin
  ...
  begin
    t := 1 / ((((((K + 21) * K + 210) * K + 1260) * K + 4725) * K + 10395) *
      K + 10395);
    FB[0] := -(((((6 * K + 84) * K + 420) * K * K - 9450) * K - 41580) * K -
      62370) * t;
    FB[1] := -((((((15 * K + 105) * K - 210) * K - 3780) * K - 4725) * K +
      51975) * K + 155925) * t;
    FB[2] := -(((20 * K * K - 840) * K * K + 18900) * K * K - 207900) * t;
    FB[3] := -((((((15 * K - 105) * K - 210) * K + 3780) * K - 4725) * K -
      51975) * K + 155925) * t;
    FB[4] := -(((((6 * K - 84) * K + 420) * K * K - 9450) * K + 41580) * K -
      62370) * t;
    FB[5] := -((((((K - 21) * K + 210) * K - 1260) * K + 4725) * K - 10395) * K
      + 10395) * t;
    FA[0] := 10395 * t * FGainFactor;
    FA[1] := -6 * FA[0];
    FA[2] := 15 * FA[0];
...
function TBesselHighpassFilter.MagnitudeSquared(const Frequency
  : Double): Double;
(* var
  i    : Integer;
  a,cw : Double; *)

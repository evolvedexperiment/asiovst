...
type
  ...
  private
    ...
    procedure DecodeHandler(Sender: TObject;
  ...
  protected
    ...
    procedure GetInternalSamples(Data: TDAVArrayOfSingleFixedArray;
  ...
  public
    ...
    procedure GetSamples(Data: TDAVArrayOfSingleFixedArray;
    ...
    property Interpolation: TBufferInterpolation read FInterpolation
...
constructor TReloadThread.Create(BufferedAudioPlayer
  ...
  with FBufferedAudioPlayer do
    ...
    begin
      if CheckReload then
        IdleLoops := 20;
      Dec(IdleLoops);
      if FAllowSuspend and (IdleLoops <= 0) then
        Suspended := True
      else
        Sleep(FTimeOut);
    end;
...
begin
  ...
  for ChannelIndex := 0 to Length(FStreamBuffer) - 1 do
    Dispose(FStreamBuffer[ChannelIndex]);

  for ChannelIndex := 0 to Length(FInterpolationBuffer) - 1 do
    if Assigned(FInterpolationBuffer[ChannelIndex]) then
      Dispose(FInterpolationBuffer[ChannelIndex]);
...
begin
  ...
  for ChannelIndex := 0 to Length(FStreamBuffer) - 1 do
    ReallocMem(FStreamBuffer[ChannelIndex], FStreamBufSize * SizeOf(Single));
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FSubBlockPosition := 0;
      if FCurrentPosition + FStreamBufSize < FAudioFile.SampleFrames then
        FAudioFile.Decode(FCurrentPosition * ChannelCount, FStreamBufSize)
      else
      begin
        FAudioFile.Decode(FCurrentPosition * ChannelCount,
          FAudioFile.SampleFrames - FCurrentPosition);
        FAudioFile.Decode(0, FStreamBufSize - (FAudioFile.SampleFrames -
          FCurrentPosition));
        FCurrentPosition := 0;
      end;
...
begin
  ...
  begin
    case ChannelCount of
      ...
      1:
        for ChannelIndex := 0 to Length(FStreamBuffer) - 1 do
          Move(ChannelPointer[0]^[0], FStreamBuffer[ChannelIndex]^
            [FSubBlockPosition], SampleFrames * SizeOf(Single));
    else
      for ChannelIndex := 0 to Length(FStreamBuffer) - 1 do
        Move(ChannelPointer[ChannelIndex mod ChannelCount]^[0],
          FStreamBuffer[ChannelIndex]^[FSubBlockPosition],
...
begin
  if Value > FBufferSize div 2 then
    Value := FBufferSize div 2;
...
begin
  ...
  begin
    FBufferSize := Value;
    if BlockSize > FBufferSize div 2 then
      BlockSize := FBufferSize div 2;
...
begin
  if Assigned(FAudioFile) then
    FRatio := FPitchFactor * FAudioFile.SampleRate / SampleRate
  else
    FRatio := FPitchFactor;
end;

procedure TCustomBufferedAudioPlayer.GetInternalSamples
  (Data: TDAVArrayOfSingleFixedArray; SampleFrames: Integer);
var
  Channel: Integer;
  SampleInBuffer: Integer;
begin
  SampleInBuffer := FBuffer.SamplesInBuffer;
  if SampleFrames < SampleInBuffer then
    FBuffer.ReadBuffer(Data, SampleFrames)
  else
  begin
    if SampleInBuffer > 0 then
      FBuffer.ReadBuffer(Data, SampleInBuffer);

    for Channel := 0 to Length(Data) - 1 do
      FillChar(Data[Channel]^, (SampleFrames - SampleInBuffer) *
        SizeOf(Single), 0);
  end;
end;

procedure TCustomBufferedAudioPlayer.GetSamples
  (Data: TDAVArrayOfSingleFixedArray; SampleFrames: Integer);
var
  Channel: Integer;
  Sample: Integer;
begin
  ...
  if FAllowSuspend and FBufferThread.Suspended then
    FBufferThread.Resume;
  if FRatio = 1 then
    GetInternalSamples(Data, SampleFrames)
  else
    case FInterpolation of
      biNone:
        ...
        begin
          ...
          begin
            for Channel := 0 to Min(Length(Data),
              Length(FInterpolationBuffer)) - 1 do
              Data[Channel] := PDAVSingleFixedArray
                (FInterpolationBuffer[Channel]);
            GetInternalSamples
              (TDAVArrayOfSingleFixedArray(FInterpolationBuffer), 1);
            FFractalPos := FFractalPos - 1;
          end;

          for Channel := 0 to Min(Length(Data),
            Length(FInterpolationBuffer)) - 1 do
            Data[Channel]^[Sample] := FInterpolationBuffer[Channel]^[0];
        end;
      biLinear:
        for Sample := 0 to SampleFrames - 1 do
        begin
          ...
          begin
            for Channel := 0 to Min(Length(Data),
              Length(FInterpolationBuffer)) - 1 do
            begin
              FInterpolationBuffer[Channel]^[1] := FInterpolationBuffer
                [Channel]^[0];
              Data[Channel] := PDAVSingleFixedArray
                (FInterpolationBuffer[Channel]);
            end;
          ...
          for Channel := 0 to Min(Length(Data),
            Length(FInterpolationBuffer)) - 1 do
            Data[Channel]^[Sample] := LinearInterpolation(1 - FFractalPos,
              PDAV2SingleArray(FInterpolationBuffer[Channel]));
        end;
      biHermite:
        for Sample := 0 to SampleFrames - 1 do
        begin
          ...
          begin
            for Channel := 0 to Min(Length(Data),
              Length(FInterpolationBuffer)) - 1 do
            begin
              Move(FInterpolationBuffer[Channel]^[0],
                FInterpolationBuffer[Channel]^[1], 3 * SizeOf(Single));
              Data[Channel] := PDAVSingleFixedArray
                (FInterpolationBuffer[Channel]);
            end;
            GetInternalSamples
          ...
          for Channel := 0 to Min(Length(Data),
            Length(FInterpolationBuffer)) - 1 do
            Data[Channel]^[Sample] := Hermite32_asm(1 - FFractalPos,
              PDAV4SingleArray(FInterpolationBuffer[Channel]));
        end;
      biBSpline6Point5thOrder:
        for Sample := 0 to SampleFrames - 1 do
        begin
          ...
          begin
            ...
            begin
              Move(FInterpolationBuffer[Channel]^[0],
                FInterpolationBuffer[Channel]^[1], 5 * SizeOf(Single));
              Data[Channel] := PDAVSingleFixedArray
                (FInterpolationBuffer[Channel]);
            end;
            GetInternalSamples
          ...
          for Channel := 0 to Min(Length(Data),
            Length(FInterpolationBuffer)) - 1 do
            Data[Channel]^[Sample] := BSplineInterpolation6Point5thOrder
              (1 - FFractalPos,
              PDAV6SingleArray(FInterpolationBuffer[Channel])^);
        end;
    end;
end;

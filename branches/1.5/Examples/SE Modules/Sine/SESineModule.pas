...
procedure SESineProcess(ModuleBase: TSEModuleBase; BufferOffset: Integer;
  SampleFrames: Integer); cdecl;
procedure SESineCosineProcess(ModuleBase: TSEModuleBase; BufferOffset: Integer;
  SampleFrames: Integer); cdecl;
procedure SESine2Process(ModuleBase: TSEModuleBase; BufferOffset: Integer;
  SampleFrames: Integer); cdecl;
{$ENDIF}
...
constructor TSESineModule.Create(SEAudioMaster: TSE2audioMasterCallback;
  Reserved: Pointer);
begin
...
begin
  if TSESinePins(CurrentPin.PinID) = pinFrequency then
    FrequencyChanged;
  inherited;
end;
...
class procedure TSESineModule.GetModuleProperties
...
function TSESineModule.GetPinProperties(const Index: Integer;
...
constructor TSESineCosineModule.Create(SEAudioMaster: TSE2audioMasterCallback;
  Reserved: Pointer);
begin
  inherited Create(SEAudioMaster, Reserved);
  FFrequency := 440;
...
// The most important part, processing the audio
procedure TSESineCosineModule.SubProcess(const BufferOffset,
...
// describe your module
class procedure TSESineCosineModule.GetModuleProperties
  (Properties: PSEModuleProperties);
begin
  with Properties^ do
  begin
...
// describe the pins (plugs)
function TSESineCosineModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
  Result := True;
  case Index of
...
constructor TSESine2Module.Create(SEAudioMaster: TSE2audioMasterCallback;
  Reserved: Pointer);
begin
  inherited Create(SEAudioMaster, Reserved);
  FFrequency := 440;
...
// describe your module
class procedure TSESine2Module.GetModuleProperties
  (Properties: PSEModuleProperties);
begin
  with Properties^ do
  begin
...
// describe the pins (plugs)
function TSESine2Module.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
  Result := True;
  case Index of

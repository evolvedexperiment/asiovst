...
begin
  ...
  begin
    ...
    if (Y[0] < 0) or (Y[0] >= PixelMap.Height) then
      Exit;
    ...
    if X[0] = X[1] then
      try
        BlendPixelInplace(PixelColor32,
          DataPointer[Y[1] * PixelMap.Width + X[1]]);
        Exit;
      finally
        EMMS;
      end;
    ...
    BlendPixelLine(PixelColor32, @DataPointer[Y[1] * PixelMap.Width + X[0]],
      X[1] - X[0])
  end
  else if X[0] = X[1] then
  begin
    // check whether the X[0]-coordinate is outside the pixel map
    if (X[0] < 0) or (X[0] >= PixelMap.Width) then
      Exit;
    ...
    try
      ...
      else if (X[0] >= PixelMap.Width) then
      begin
        // check if line needs to be drawn at all
      ...
      else if (X[1] >= PixelMap.Width) then
      begin
        // calculate new left offset
      ...
      else if (Y[0] >= PixelMap.Height) then
      begin
        // check if line needs to be drawn at all
      ...
      else if (Y[1] >= PixelMap.Height) then
      begin
        // recalculate new length in X[0] and Y[0] coordinates
      ...
      err := el shr 1;
      BlendPixelInplace(PixelColor32,
        DataPointer[Y[0] * PixelMap.Width + X[0]]);
...
begin
  ...
  begin
    ...
    begin
      ...
      if (XPos[0] >= 0) and (XPos[0] < PixelMap.Width) then
        BlendPixelInplace(PixelColor32, PixelMap.PixelPointer[XPos[0],
          YPos[0]]^);
      Temp.Fixed := XEnd.Frac;
      PixelColor32.A := (OriginalAlpha * FixedMul(Temp, Gap).Fixed + $80) shr 8;
      if (XPos[0] + 1 >= 0) and (XPos[0] + 1 < PixelMap.Width) then
        BlendPixelInplace(PixelColor32, PixelMap.PixelPointer[XPos[0] + 1,
          YPos[0]]^);
    end;

    Inter.Fixed := XEnd.Fixed + Gradient.Fixed;
    ...
    begin
      ...
      begin
        ...
        if (X >= 0) and (X < PixelMap.Width) then
          BlendPixelInplace(PixelColor32, PixelMap.PixelPointer[X, Y]^);
        PixelColor32.A := (OriginalAlpha * Inter.Frac + $80) shr 8;
        if (X + 1 >= 0) and (X + 1 < PixelMap.Width) then
          BlendPixelInplace(PixelColor32, PixelMap.PixelPointer[X + 1, Y]^);
      end;
    ...
    begin
      ...
      if (XPos[1] >= 0) and (XPos[1] < PixelMap.Width) then
        BlendPixelInplace(PixelColor32, PixelMap.PixelPointer[XPos[1],
          YPos[1]]^);
      Temp.Fixed := XEnd.Frac;
      PixelColor32.A := (OriginalAlpha * FixedMul(Temp, Gap).Fixed + $80) shr 8;
      if (XPos[1] + 1 >= 0) and (XPos[1] + 1 < PixelMap.Width) then
        BlendPixelInplace(PixelColor32, PixelMap.PixelPointer[XPos[1] + 1,
          YPos[1]]^);
    end;
  ...
  begin
    ...
    begin
      ...
      if (YPos[0] >= 0) and (YPos[0] < PixelMap.Height) then
        CombinePixelInplace(PixelColor32, PixelMap.PixelPointer[XPos[0], YPos[0]
          ]^, FixedMul(Temp, Gap).Fixed);
      Temp.Fixed := YEnd.Frac;
      if (YPos[0] + 1 >= 0) and (YPos[0] + 1 < PixelMap.Height) then
        CombinePixelInplace(PixelColor32, PixelMap.PixelPointer[XPos[0],
          YPos[0] + 1]^, FixedMul(Temp, Gap).Fixed);
    end;
    ...
    begin
      ...
      begin
        Y := FixedFloor(Inter);
        if (Y >= 0) and (Y < PixelMap.Height) then
          CombinePixelInplace(PixelColor32, PixelMap.PixelPointer[X, Y]^,
            $FF - Inter.Frac);
        if (Y + 1 >= 0) and (Y + 1 < PixelMap.Height) then
          CombinePixelInplace(PixelColor32, PixelMap.PixelPointer[X, Y + 1]^,
            Inter.Frac);
      end;
    ...
    begin
      Temp.Fixed := $FF - YEnd.Frac;
      if (YPos[1] >= 0) and (YPos[1] < PixelMap.Height) then
        CombinePixelInplace(PixelColor32, PixelMap.PixelPointer[XPos[1], YPos[1]
          ]^, FixedMul(Temp, Gap).Fixed);
      Temp.Fixed := YEnd.Frac;
      if (YPos[1] + 1 >= 0) and (YPos[1] + 1 < PixelMap.Height) then
        CombinePixelInplace(PixelColor32, PixelMap.PixelPointer[XPos[1],
          YPos[1] + 1]^, FixedMul(Temp, Gap).Fixed);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if X[0] = X[1] then
      try
        BlendPixelInplace(PixelColor32,
          DataPointer[(Y[1] + Index - LineWidth div 2) * PixelMap.Width
          + X[1]]);
        Exit;
      finally
        EMMS;

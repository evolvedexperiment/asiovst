...
type
  ...
  TSEDitherHighshelfNoiseshaperStaticModule = class
  ...
  TSEDitherHighshelfNoiseshaperControllableModule = class
...
constructor TCustomSEDitherHighshelfNoiseshaperModule.Create
...
procedure TCustomSEDitherHighshelfNoiseshaperModule.SubProcessStatic
...
class procedure TCustomSEDitherHighshelfNoiseshaperModule.GetModuleProperties
...
function TCustomSEDitherHighshelfNoiseshaperModule.GetPinProperties
...
procedure TCustomSEDitherHighshelfNoiseshaperModule.PlugStateChange
...
procedure TSEDitherHighshelfNoiseshaperStaticModule.SubProcess
  (const BufferOffset, SampleFrames: Integer);
var
...
begin
  ...
  for Sample := 0 to SampleFrames - 1 do
    Outp^[Sample] := FDitherNoiseshaper.ProcessFloat(Inp^[Sample]);
end;

// describe the pins (plugs)
function TSEDitherHighshelfNoiseshaperStaticModule.GetPinProperties
  (const Index: Integer; Properties: PSEPinProperties): Boolean;
begin
...
// An input plug has changed value
procedure TSEDitherHighshelfNoiseshaperStaticModule.PlugStateChange
  (const CurrentPin: TSEPin);
begin
  inherited;
  case TSEDitherNoiseshaperPins(CurrentPin.PinID) of
    pinBitDepth:
      FDitherNoiseshaper.BitDepth := FBitDepth;
    pinLimit:
      FDitherNoiseshaper.Limit := FLimit;
    pinDitherType:
      FDitherNoiseshaper.DitherType := FDitherType;
    pinDitherAmplitude:
      FDitherNoiseshaper.DitherAmplitude := FDitherAmplitude;
  end;
end;

{ TSEDitherHighshelfNoiseshaperControllableModule }

class procedure TSEDitherHighshelfNoiseshaperControllableModule.
...
function TSEDitherHighshelfNoiseshaperControllableModule.GetPinProperties
  (const Index: Integer; Properties: PSEPinProperties): Boolean;
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSEDitherNoiseshaperPins(index) in [pinBitDepth .. pinNoiseshaperType] then
    with Properties^ do
      Direction := drIn;
end;

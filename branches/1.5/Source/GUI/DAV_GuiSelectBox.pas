...
{$ENDIF}
  public
    ...
    property SelectBoxColor: TColor read FSelectBoxColor write SetSelectBoxColor
      default clBtnShadow;
...
procedure TCustomGuiSelectBox.CMFontChanged(var Message:
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (X < FArrowButtonWidth) or (X > Width - 1 - FArrowButtonWidth) then
          BgColor := BackColor
        else
          BgColor := ButtonColor;
        ...
        begin
          ...
          CombColor := CombinePixel(BorderColor, BgColor,
            Round($FF - Temp * $FF));
        end
        ...
        // lines
        if ((X >= FArrowButtonWidth) and (X < FArrowButtonWidth + FBorderWidth))
          or ((X > Width - 1 - FArrowButtonWidth - FBorderWidth) and
          (X <= Width - 1 - FArrowButtonWidth)) then
          CombColor := BorderColor;
      ...
      begin
        ...
        if (Y < BorderWidth - 1) or (Y > Height - 1 - BorderWidth + 1) then
          ...
          begin
            ...
            else if ((X >= FArrowButtonWidth) and
              (X < FArrowButtonWidth + FBorderWidth)) or
              ((X > Width - 1 - FArrowButtonWidth - FBorderWidth) and
              (X <= Width - 1 - FArrowButtonWidth)) then
              CombColor := BorderColor
            else
              CombColor := BgColor;
    ...
    begin
      ...
      BlendPixelInplace(ApplyAlpha(ArrowColor, XFixed.Frac),
        ScnLn[0]^[Offset - X - 1]);
      BlendPixelLine(ArrowColor, @ScnLn[0]^[Width - Offset], X);
      BlendPixelInplace(ApplyAlpha(ArrowColor, XFixed.Frac),
        ScnLn[0]^[Width - Offset + X]);
...
begin
  ...
  else if Loop and (FItemIndex >= FItems.Count - 1) then
    ItemIndex := 0;
end;
...
begin
  if FItemIndex > 0 then
    ItemIndex := ItemIndex - 1
  else if Loop and (FItemIndex = 0) then
    ...
    case FAlignment of
      taLeftJustify:
        ...
        begin
          ...
          FGuiFont.TextOut(FItems[FItemIndex], PixelMap, Offset,
...
procedure TGuiSelectBox.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
    ...
    case FAlignment of
      taLeftJustify:
        begin
          if (X > Width - FArrowButtonWidth) then
            PreviousItem
          else if (X > Width - 2 * FArrowButtonWidth) then
            NextItem;
      ...
      taCenter:
        begin
          if (X < FArrowButtonWidth) then
            PreviousItem
          else if (X > Width - FArrowButtonWidth) then
            NextItem;
      ...
      taRightJustify:
        begin
          if (X < FArrowButtonWidth) then
            PreviousItem
          else if (X < 2 * FArrowButtonWidth) then
            NextItem;
...
procedure TGuiIntegerBox.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  if Button = mbLeft then
    case FAlignment of
    ...
    case FAlignment of
      taLeftJustify:
        begin
          ...
          FGuiFont.TextOut(IntToStr(FValue), PixelMap, Offset,
            (Height - TextSize.cy) div 2);
        end;
      taCenter:
        begin

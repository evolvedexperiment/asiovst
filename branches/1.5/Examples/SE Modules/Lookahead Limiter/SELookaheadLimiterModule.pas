...
type
  TCustomLookaheadLimiterSEModuleClass = class of
    TCustomLookaheadLimiterSEModule;
...
procedure TCustomLookaheadLimiterSEModule.PlugStateChange
...
begin
  ...
  case CurrentPin.PinID of
    ...
    3:
      FLookaheadLimiter.LookAhead := 2 * Round(0.5 * FLookaheadSamples);
  end;
end;

// describe your module
class procedure TCustomLookaheadLimiterSEModule.GetModuleProperties
...
procedure TCustomLookaheadLimiterSEModule.SubProcessStatic(const BufferOffset,
...
function TCustomLookaheadLimiterSEModule.GetPinProperties(const Index: Integer;
...
begin
  ...
  case CurrentPin.PinID of
    4:
      FLookaheadLimiter.Input_dB := FInputGain;
    5:
      FLookaheadLimiter.Output_dB := FOutputGain;
    6:
      FLookaheadLimiter.Release := FRelease;
  end;
  inherited;
end;
...
function TLookaheadLimiterParamStaticSEModule.GetPinProperties
...
procedure TLookaheadLimiterAutomatableSEModule.PlugStateChange
  (const CurrentPin: TSEPin);
begin
  case CurrentPin.PinID of
    4 .. 6:
      if (Pin[4].Status <> stRun) and (Pin[5].Status <> stRun) and
        (Pin[6].Status <> stRun) then
        OnProcess := SubProcess
      else
...
procedure TLookaheadLimiterAutomatableSEModule.SubProcessAutomated

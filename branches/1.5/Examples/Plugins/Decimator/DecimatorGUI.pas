...
procedure TVSTGUI.ShSHRateBgMouseDown(Sender: TObject; Button: TMouseButton;
...
procedure TVSTGUI.ShSHRateBgMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  if (ssLeft in Shift) and (FMouseContext = mcSHRate) then
    with TVSTDecimator(Owner) do
    begin
      if ShSHRateBg.Height - Y > ShSHRateBg.Height then
        Parameter[0] := 44100
      else if ShSHRateBg.Height - Y < 0 then
        Parameter[0] := 44.1
      else
        Parameter[0] := FreqLinearToLog(1 - Y / ShSHRateBg.Height) * 2.205;
      LbRate.Caption := FloatToStrF(Parameter[0], ffGeneral, 5, 2);
    end;
end;

procedure TVSTGUI.ShBitsBgMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  if (ssLeft in Shift) and (FMouseContext = mcBits) then
    with TVSTDecimator(Owner) do
    begin
      if ShBitsBg.Height - Y > ShBitsBg.Height then
        Parameter[1] := 24
      else if ShBitsBg.Height - Y < 0 then
        Parameter[1] := 1
      else
        Parameter[1] := Round(23 * (1 - (Y / ShBitsBg.Height)) + 1);
      LbBits.Caption := IntToStr(Round(Parameter[1]));
    end;
end;

procedure TVSTGUI.ShCutBgMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  if (ssLeft in Shift) and (FMouseContext = mcCut) and (Y >= -10) then
    with TVSTDecimator(Owner) do
    begin
      if ShCutBg.Height - Y > ShCutBg.Height then
        Parameter[2] := 20000
      else if ShCutBg.Height - Y < 0 then
        Parameter[2] := 20
      else
        Parameter[2] := FreqLinearToLog(1 - Y / ShCutBg.Height);
      LbCut.Caption := FloatToStrF(Parameter[2], ffGeneral, 5, 2);
    end;
end;

procedure TVSTGUI.ShResBgMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  if (ssLeft in Shift) and (FMouseContext = mcRes) then
    with TVSTDecimator(Owner) do
    begin
      if ShResBg.Height - Y > ShResBg.Height then
        Parameter[3] := 8
      else if ShResBg.Height - Y <= 0 then
        Parameter[3] := 0.1
      else
        Parameter[3] := 8 * (1 - Y / ShResBg.Height);
      LbRes.Caption := FloatToStrF(Parameter[3], ffGeneral, 2, 2);
    end;
end;

procedure TVSTGUI.ShMixBgMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  if (ssLeft in Shift) and (FMouseContext = mcMix) then
    with TVSTDecimator(Owner) do
    begin
      if ShMixBg.Height - Y > ShMixBg.Height then
        Parameter[5] := 100
      else if ShMixBg.Height - Y <= 0 then
        Parameter[5] := 0
      else
        Parameter[5] := 100 * (1 - Y / ShMixBg.Height);
      LbMix.Caption := IntToStr(Round(Parameter[5]));
    end;
end;

procedure TVSTGUI.shVolBgMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  if (ssLeft in Shift) and (FMouseContext = mcVol) then
    with TVSTDecimator(Owner) do
    begin
      if shVolBg.Height - Y > shVolBg.Height then
        Parameter[6] := -24
      else if shVolBg.Height - Y <= 0 then
        Parameter[6] := 6
      else
        Parameter[6] := 6 - 30 * Y / shVolBg.Height;
      LbVol.Caption := IntToStr(Round(Parameter[6]));
    end;
end;

procedure TVSTGUI.ShSHRateMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  ShSHRateBgMouseMove(Sender, Shift, X, ShSHRate.Top - ShSHRateBg.Top + Y);
end;

procedure TVSTGUI.ShBitsMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  ShBitsBgMouseMove(Sender, Shift, X, ShBits.Top - ShBitsBg.Top + Y);
end;

procedure TVSTGUI.ShCutMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  ShCutBgMouseMove(Sender, Shift, X, ShCut.Top - ShCutBg.Top + Y);
end;

procedure TVSTGUI.ShResMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  ShResBgMouseMove(Sender, Shift, X, ShRes.Top - ShResBg.Top + Y);
end;

procedure TVSTGUI.ShMixMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  ShMixBgMouseMove(Sender, Shift, X, ShMix.Top - ShMixBg.Top + Y);
end;

procedure TVSTGUI.ShVolMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  shVolBgMouseMove(Sender, Shift, X, ShVol.Top - shVolBg.Top + Y);
end;
...
begin
  with TVSTDecimator(Owner) do
  begin
    BitTopPos := Round((1 - BitDepth * COne24th) * ShBitsBg.Height +
      ShBitsBg.Top);
...
begin
  with TVSTDecimator(Owner) do
  begin
    FreqTopPos := Round((1 - 2 * CutoffNormalizedFrequency) * ShCutBg.Height +
      ShCutBg.Top);
...
begin
  with TVSTDecimator(Owner) do
  begin
    SRTopPos := Round((1 - SampleHoldRate) * ShSHRateBg.Height +
      ShSHRateBg.Top);

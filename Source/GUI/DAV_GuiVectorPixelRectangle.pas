...
begin
  ...
  begin
    ...
    begin
      ...
      for X := XRange[0] to XRange[1] - 1 do
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        PixelColor32.A := Fixed24Dot8Mul($FF - GeometricShape.Left.Frac,
          CurrentAlpha);
      ...
      for X := XRange[0] + XAntiAlias[0] to XRange[1] - XAntiAlias[1] do
        BlendPixelInplace(PixelColor32, ScnLne[X]);

      // draw last pixel of this first scanline
      if XAntiAlias[1] <> 0 then
      begin
        PixelColor32.A := Fixed24Dot8Mul(GeometricShape.Right.Frac,
          CurrentAlpha);
  ...
  begin
    ...
    begin
      ...
      if (ClipRect.Left >= Width) or (ClipRect.Right < 0) or
        (ClipRect.Left >= ClipRect.Right) then
        Exit;
      ...
      if (ClipRect.Top >= Height) or (ClipRect.Bottom < 0) or
        (ClipRect.Top >= ClipRect.Bottom) then
        Exit;

      // set y-range
      if ClipRect.Top > 0 then
        YRange[0] := ClipRect.Top
      else
        YRange[0] := 0;

      if ClipRect.Bottom < Height - 1 then
        YRange[1] := ClipRect.Bottom
      else
        YRange[1] := Height - 1;

      // set x-range
      if ClipRect.Left > 0 then
        XRange[0] := ClipRect.Left
      else
        XRange[0] := 0;

      if ClipRect.Right < Width - 1 then
        XRange[1] := ClipRect.Right
      else
        XRange[1] := Width - 1;

      // eventually limit round radius
      if 2 * RoundedRadius > ClipRect.Right - ClipRect.Left then
        RoundedRadius := (ClipRect.Right - ClipRect.Left) div 2;
      if 2 * RoundedRadius > ClipRect.Bottom - ClipRect.Top then
        RoundedRadius := (ClipRect.Bottom - ClipRect.Top) div 2;
    end;
    ...
    begin
      ...
      if ClipRect.Bottom - Y < MinYDistance then
        MinYDistance := ClipRect.Bottom - Y;
      ...
      begin
        SqrYDist :=
          FixedSqr(ConvertToFixed24Dot8(Integer(RoundedRadius - MinYDistance)));
        SqrDist := FixedSub(ConvertToFixed24Dot8(Sqr(RoundedRadius)), SqrYDist);
        Assert(SqrDist.Fixed >= 0);
        XOffset := RoundedRadius - FixedRound(FixedSub(FixedSqrt(SqrDist),
          CFixed24Dot8Half));

        // eventually change left offset
        if XPos[0] - ClipRect.Left < RoundedRadius then
        begin
          if XPos[0] - ClipRect.Left < XOffset then
            XPos[0] := ClipRect.Left + XOffset;
        end;

        // eventually change right offset
        if ClipRect.Right - XPos[1] < RoundedRadius then
        begin
          if ClipRect.Right - XPos[1] < XOffset then
            XPos[1] := ClipRect.Right - XOffset;
        end;
      end;
  ...
  begin
    ...
    begin
      ...
      if (FixedFloor(OriginalLeft) >= Width) or (FixedCeil(OriginalRight) < 0)
        or (FixedFloor(OriginalLeft) >= FixedCeil(OriginalRight)) then
        Exit;
      ...
      if (FixedFloor(OriginalTop) >= Height) or (FixedCeil(OriginalBottom) < 0)
        or (FixedFloor(OriginalTop) >= FixedCeil(OriginalBottom)) then
        Exit;
    end;

    // set y-range
    if OriginalTop.Fixed > 0 then
      YRange[0] := FixedFloor(OriginalTop)
    else
      YRange[0] := 0;

    if OriginalBottom.Fixed < ConvertToFixed24Dot8(Integer(Height - 1)).Fixed
    then
      YRange[1] := FixedCeil(OriginalBottom)
    else
      YRange[1] := Height - 1;

    // set x-range
    if OriginalLeft.Fixed > 0 then
      XRange[0] := OriginalLeft
    else
      XRange[0].Fixed := 0;

    if OriginalRight.Fixed < ConvertToFixed24Dot8(Integer(Width - 1)).Fixed then
      XRange[1] := OriginalRight
    else
      XRange[1] := ConvertToFixed24Dot8(Integer(Width - 1));

    // eventually limit round radius
    if FixedAdd(RoundedRadius, RoundedRadius).Fixed > FixedSub(OriginalRight,
      OriginalLeft).Fixed then
      RoundedRadius := FixedDiv(FixedSub(OriginalRight, OriginalLeft),
        CFixed24Dot8Two);
    if FixedAdd(RoundedRadius, RoundedRadius).Fixed > FixedSub(OriginalBottom,
      OriginalTop).Fixed then
      RoundedRadius := FixedDiv(FixedSub(OriginalBottom, OriginalTop),
        CFixed24Dot8Two);
    ...
    begin
      ...
      // calculate y alpha
      if Y = YRange[0] then
        YAlpha := $FF - (OriginalTop.Fixed - YFixed.Fixed)
      else if Y = YRange[1] then
        YAlpha := $FF - (YFixed.Fixed - OriginalBottom.Fixed)
      else
        YAlpha := $FF;

      // calculate minimal y distance (either distance to upper or lower bound)
      MinYDistance := FixedSub(YFixed, OriginalTop);
      if FixedSub(OriginalBottom, YFixed).Fixed < MinYDistance.Fixed then
        MinYDistance := FixedSub(OriginalBottom, YFixed);
      ...
      begin
        ...
        if SqrDistPx2Ctr.Fixed < 0 then
          XDistance.Fixed := 0
        else
          XDistance := FixedSqrt(SqrDistPx2Ctr);
        XOffset := FixedSub(RoundedRadius, XDistance);

        // eventually change left offset
        if FixedSub(XPos[0], OriginalLeft).Fixed < RoundedRadius.Fixed then
        begin
          if FixedSub(XPos[0], OriginalLeft).Fixed < XOffset.Fixed then
            XPos[0] := FixedAdd(OriginalLeft, XOffset);
        end;

        // eventually change right offset
        if FixedSub(OriginalRight, XPos[1]).Fixed < RoundedRadius.Fixed then
        begin
          if FixedSub(OriginalRight, XPos[1]).Fixed < XOffset.Fixed then
            XPos[1] := FixedSub(OriginalRight, XOffset);
        end;
        ...
        begin
          ...
          repeat
            ...
            begin
              ...
              if Temp.Fixed >= $FF then
                Break;
              if YAlpha < $FF then
                TempColor32.A := (PixelColor32.A * YAlpha * XAlpha) div $FE01
              else
                TempColor32.A := (PixelColor32.A * XAlpha) shr 8;
            end;
          ...
          repeat
            // calculate squared distance
            Temp.Fixed := X[1] shl 8 -
              (OriginalRight.Fixed - RoundedRadius.Fixed);
            if Temp.Fixed < 0 then
              Break;
            SqrDistPx2Ctr.Fixed := FixedSqr(Temp).Fixed + SqrYDistPx2Ctr.Fixed;
            if SqrDistPx2Ctr.Fixed >= SqrRadMinusOne.Fixed then
          ...
          TempColor32 := PixelColor32;
          if YAlpha < $FF then
            TempColor32.A := (PixelColor32.A * YAlpha + $7F) shr 8;

          if X[1] >= X[0] then
            BlendPixelLine(TempColor32, @ScnLn[X[0]], X[1] - X[0] + 1);
        end;
      end
      else
      begin
        ...
        begin
          ...
          if X[1] - X[0] - 1 > 0 then
            BlendPixelLine(TempColor32, @ScnLn[X[0] + 1], X[1] - X[0] - 1);
        end;
      end;
    end;
...
function TGuiPixelFilledRoundedRectangle.GetGeometricShape
  : TGuiRoundedRectangle;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SqrYDist :=
          FixedSqr(ConvertToFixed24Dot8(Integer(RoundedRadius - MinYDistance)));
        SqrDist := FixedSub(ConvertToFixed24Dot8(Sqr(RadiusMinusOne)),
          SqrYDist);
        if SqrDist.Fixed < 0 then
          SqrDist.Fixed := 0;
        XCount := XOffset - FixedRound(FixedSub(FixedSqrt(SqrDist),
          CFixed24Dot8Half));
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if X[1] >= X[0] then
          begin
            if (Y = YRange[0]) or (Y = YRange[1]) then
              BlendPixelLine(TempColor32, @ScnLn[X[0]], X[1] - X[0] + 1);
          end;
        end;
      end

...
procedure TmdaAmbienceDataModule.ParamHFDampChange(Sender: TObject;
...
procedure TmdaAmbienceDataModule.ParamSizeChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  temp: Double;
begin
  temp := 0.025 + 0.2665 * Value;
  if (FRoomsize <> temp) then
    FReady := False; // need to flush buffer
  FRoomsize := temp;
end;
...
begin
  if Assigned(FBuffers[0]) then
    Dispose(FBuffers[0]);
  if Assigned(FBuffers[1]) then
    Dispose(FBuffers[1]);
  if Assigned(FBuffers[2]) then
    Dispose(FBuffers[2]);
  if Assigned(FBuffers[3]) then
    Dispose(FBuffers[3]);
end;

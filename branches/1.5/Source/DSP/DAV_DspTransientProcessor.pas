...
type
  ...
  public
    constructor Create; override;
    procedure ProcessSample(const InLeft, InRight: Single;
...
begin
  ...
  begin
    ...
    FAttackFactors[1] := Power(10, 1 - 0.025 * (FAttack + 100) *
      CSampleRateScaling * SampleRate);
  end
  else
  begin
    FAttackFactors[0] := Power(10, -4 + 0.025 * (FAttack + 100) *
      CSampleRateScaling * SampleRate);
...
begin
  FAttackHoldFactor := 1 - Power(10, -2 - 0.04 * FAttackHold *
    CSampleRateScaling * SampleRate);
end;
...
begin
  ...
  begin
    ...
    FReleaseFactors[1] := 1 - Power(10, -5.85 + 0.0135 * (FRelease + 100) *
      CSampleRateScaling * SampleRate);
  end
  else
  begin
    FReleaseFactors[0] := 1 - Power(10, -3.15 - 0.0135 * (FRelease + 100) *
      CSampleRateScaling * SampleRate);
...
begin
  FReleaseHoldFactor := Power(10, -0.04 * FReleaseHold * CSampleRateScaling *
    SampleRate);
end;
...
begin
  ...
  if Gain > FEnv[0] then
    ...
    FEnv[0] := FEnv[0] * FAttackHoldFactor;
  if Gain > FEnv[1] then
    FEnv[1] := FEnv[1] + FAttackFactors[1] * (Gain - FEnv[1])
  else
    FEnv[1] := FEnv[1] * FAttackHoldFactor;
  if Gain > FEnv[2] then
    FEnv[2] := FEnv[2] + FReleaseHoldFactor * (Gain - FEnv[2])
  else
    FEnv[2] := FEnv[2] * FReleaseFactors[0];
  if Gain > FEnv[3] then
    FEnv[3] := FEnv[3] + FReleaseHoldFactor * (Gain - FEnv[3])
  else
    FEnv[3] := FEnv[3] * FReleaseFactors[1];
...
procedure TCustomStereoTransientProcessor.ProcessSample(const InLeft,
  InRight: Single; out OutLeft, OutRight: Single);
var
  Gain, Mono: Single;
begin
  ...
  if Mono > FEnv[0] then
    ...
    FEnv[0] := FEnv[0] * FAttackHoldFactor;
  if Mono > FEnv[1] then
    FEnv[1] := FEnv[1] + FAttackFactors[1] * (Mono - FEnv[1])
  else
    FEnv[1] := FEnv[1] * FAttackHoldFactor;
  if Mono > FEnv[2] then
    FEnv[2] := FEnv[2] + FReleaseHoldFactor * (Mono - FEnv[2])
  else
    FEnv[2] := FEnv[2] * FReleaseFactors[0];
  if Mono > FEnv[3] then
    FEnv[3] := FEnv[3] + FReleaseHoldFactor * (Mono - FEnv[3])
  else
    FEnv[3] := FEnv[3] * FReleaseFactors[1];
  Gain := (FEnv[0] - FEnv[1] + FEnv[2] - FEnv[3]);
...
procedure TCustomStereoTransientProcessor.ProcessStereo(var Left,
  Right: Single);
begin

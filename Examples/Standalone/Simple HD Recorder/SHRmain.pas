...
type
  ...
  public
    ...
    property InputChannelOffset: Integer read FChannelOffsets[0]
      write FChannelOffsets[0];
    property OutputChannelOffset: Integer read FChannelOffsets[1]
      write FChannelOffsets[1];
  end;
...
begin
  ...
  begin
    ...
    begin
      AsioChannel := (FChannelOffsets[0] + Channel)
        mod ASIOHost.InputChannelCount;
      FPeaks[Channel] := 0.999 * FPeaks[Channel];
      if Abs(InBuffer[AsioChannel, Sample]) > FPeaks[Channel] then
        FPeaks[Channel] := Abs(InBuffer[AsioChannel, Sample]);
      FDataBuffers[FDataBufferIndex, Channel]^[FDataBufferPos] :=
        InBuffer[AsioChannel, Sample];
    end;
    Inc(FDataBufferPos);
    if FDataBufferPos >= FDataBufferSize then
...
begin
  ...
  begin
    if FStorageThread.Suspended then
...
begin
  ...
  FileBaseName := 'Rec' + IntToStr(y) + '-' + IntToStr(MO) + '-' + IntToStr(D) +
    '-' + IntToStr(h) + '-' + IntToStr(M) + '-' + IntToStr(s);
  SaveWAVFile(FileBaseName + '-Ch1.wav', @FDataBuffers[Index, 0]^[0],
    Round(ASIOHost.SampleRate), 1, 16, FDataBufferSize);
  SaveWAVFile(FileBaseName + '-Ch2.wav', @FDataBuffers[Index, 1]^[0],
    Round(ASIOHost.SampleRate), 1, 16, FDataBufferSize);
  SaveWAVFile(FileBaseName + '-Ch3.wav', @FDataBuffers[Index, 2]^[0],
    Round(ASIOHost.SampleRate), 1, 16, FDataBufferSize);
  SaveWAVFile(FileBaseName + '-Ch4.wav', @FDataBuffers[Index, 3]^[0],
    Round(ASIOHost.SampleRate), 1, 16, FDataBufferSize);
end;
...
begin
  ...
  FDataBufferPos := 0;
  if not Assigned(FStorageThread) then
    StoreData(FDataBufferIndex)
  else
    FStorageThread.Resume;
end;
...
begin
  ...
  DecodeDate(Now, y, MO, D);
  FileBaseName := 'Rec' + IntToStr(y) + '-' + IntToStr(MO) + '-' + IntToStr(D) +
    '-' + IntToStr(h) + '-' + IntToStr(M);
  SaveWAVFile(FileBaseName + '-Ch1.wav', FDataBuffers[FDataBufferIndex, 0],
    Round(ASIOHost.SampleRate), 1, 16, FDataBufferIndex);
  SaveWAVFile(FileBaseName + '-Ch2.wav', FDataBuffers[FDataBufferIndex, 1],
    Round(ASIOHost.SampleRate), 1, 16, FDataBufferIndex);
  SaveWAVFile(FileBaseName + '-Ch3.wav', FDataBuffers[FDataBufferIndex, 2],
    Round(ASIOHost.SampleRate), 1, 16, FDataBufferIndex);
  SaveWAVFile(FileBaseName + '-Ch4.wav', FDataBuffers[FDataBufferIndex, 3],
    Round(ASIOHost.SampleRate), 1, 16, FDataBufferIndex);
end;

...
type
  ...
  public
    ...
    property AttackShape: TAttackShape read FAttackShape write FAttackShape
      default asParabolic;
  ...
  TDspFeedforwardLookaheadLimiter32 = class
    (TCustomDspFeedforwardLookaheadLimiter, IDspProcessor32)
  protected
    FHoldValue: Single;
    FPeak: Single;
  ...
  TDspFeedbackLikeLookaheadLimiter32 = class
    (TCustomDspFeedbackLikeLookaheadLimiter, IDspProcessor32)
  protected
    FHoldValue: Single;
    FPeak: Single;
    FSampleBuffer32: PDAVSingleFixedArray;
...
begin
  ...
  if FRelease = 0 then
    ...
    FReleaseFactor := Power(2, -1 / FReleaseSamples);
...
begin
  ...
  begin
    ...
    if FHoldCounter >= FBufferPos[0] then
      FHoldCounter := FHoldCounter - FBufferPos[0]
    else
      FHoldCounter := FHoldCounter - FBufferPos[0] + FLookAhead;
  ...
  Inc(FBufferPos[0]);
  if FBufferPos[0] >= FLookAhead then
    FBufferPos[0] := 0;
  ...
  case FAttackShape of
    asLinear:
      begin
        ...
        FWindowBuffer32[1]^[FBufferPos[1]] := FWindowBuffer32[0]^
          [FBufferPos[1]];
  ...
  Inc(FBufferPos[1]);
  if FBufferPos[1] >= FLookAhead div 2 then
    FBufferPos[1] := 0;
...
function TDspFeedforwardLookaheadLimiter32.ProcessSample32
  (Input: Single): Single;
var
  Pos: Integer;
  Temp: Single;
...
function TDspFeedbackLikeLookaheadLimiter32.ProcessSample32
  (Input: Single): Single;
var
  Pos: Integer;
  Temp: Single;

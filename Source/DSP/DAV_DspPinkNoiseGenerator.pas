...
procedure TCustomPinkNoiseGenerator.ProcessBlock32
...
procedure TCustomPinkNoiseGenerator.ProcessBlock64
  (const Data: PDAVDoubleFixedArray; SampleCount: Integer);
var
  Sample: Integer;
begin
  for Sample := 0 to SampleCount - 1 do
    Data[Sample] := ProcessSample64;
end;
...
const
  ...
  pSUM: array [0 .. 4] of Single = (0.00198, 0.01478, 0.06378, 0.23378,
    0.91578);
begin
  ur1 := Random;
  if (ur1 <= pSUM[0]) then
  ...
  Result := FAmplitude * (FContribution[0] + FContribution[1] + FContribution[2]
    + FContribution[3] + FContribution[4]);
...
begin
  ur1 := Random;
  if (ur1 <= pSUM[0]) then
    FContribution[0] := FastRandom * pA[0]
  else if (ur1 <= pSUM[1]) then
    FContribution[1] := FastRandom * pA[1]
  else if (ur1 <= pSUM[2]) then
    FContribution[2] := FastRandom * pA[2]
  else if (ur1 <= pSUM[3]) then
    FContribution[3] := FastRandom * pA[3]
  else if (ur1 <= pSUM[4]) then
    FContribution[4] := FastRandom * pA[4];

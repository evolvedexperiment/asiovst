...
constructor TSELabelGui.Create(SEGuiCallback: TSEGuiCallback;
  AHostPtr: Pointer);
begin
  inherited;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Line[0, x].B := (Line[1, 4 * x].B + Line[1, 4 * x + 1].B + Line[1,
          4 * x + 2].B + Line[1, 4 * x + 3].B + Line[2, 4 * x].B + Line[2,
          4 * x + 1].B + Line[2, 4 * x + 2].B + Line[2, 4 * x + 3].B + Line[3,
          4 * x].B + Line[3, 4 * x + 1].B + Line[3, 4 * x + 2].B + Line[3,
          4 * x + 3].B + Line[4, 4 * x].B + Line[4, 4 * x + 1].B + Line[4,
          4 * x + 2].B + Line[4, 4 * x + 3].B) div 16;
        Line[0, x].G := (Line[1, 4 * x].G + Line[1, 4 * x + 1].G + Line[1,
          4 * x + 2].G + Line[1, 4 * x + 3].G + Line[2, 4 * x].G + Line[2,
          4 * x + 1].G + Line[2, 4 * x + 2].G + Line[2, 4 * x + 3].G + Line[3,
          4 * x].G + Line[3, 4 * x + 1].G + Line[3, 4 * x + 2].G + Line[3,
          4 * x + 3].G + Line[4, 4 * x].G + Line[4, 4 * x + 1].G + Line[4,
          4 * x + 2].G + Line[4, 4 * x + 3].G) div 16;
        Line[0, x].R := (Line[1, 4 * x].R + Line[1, 4 * x + 1].R + Line[1,
          4 * x + 2].R + Line[1, 4 * x + 3].R + Line[2, 4 * x].R + Line[2,
          4 * x + 1].R + Line[2, 4 * x + 2].R + Line[2, 4 * x + 3].R + Line[3,
          4 * x].R + Line[3, 4 * x + 1].R + Line[3, 4 * x + 2].R + Line[3,
          4 * x + 3].R + Line[4, 4 * x].R + Line[4, 4 * x + 1].R + Line[4,
          4 * x + 2].R + Line[4, 4 * x + 3].R) div 16;
      end;
    end;
  end;
end;
...
begin
  if FText <> '' then
  begin
    if FBitmap.Width <> 4 * wi.Width then
      FBitmap.Width := 4 * wi.Width;
    if FBitmap.Height <> 4 * wi.Height then
      FBitmap.Height := 4 * wi.Height;
    with TCanvas.Create do
      try
        Handle := hDC;
        Brush.Style := bsClear;
        BitBlt(FBitmap.Canvas.Handle, 0, 0, wi.Width, wi.Height, Handle, 0,
          0, SRCCOPY);
  ...
  case CurrentPin.PinIndex of
    ...
    5:
      begin
        ...
        begin
          if B then
            FBitmap.Canvas.Font.Style := FBitmap.Canvas.Font.Style + [fsItalic]
          else
            FBitmap.Canvas.Font.Style := FBitmap.Canvas.Font.Style - [fsItalic];
          CallHost(seGuiHostRequestRepaint);
        end;
...
begin
  ...
  if Pin[4].ValueAsBoolean then
    FBitmap.Canvas.Font.Style := FBitmap.Canvas.Font.Style + [fsBold]
  else
    FBitmap.Canvas.Font.Style := FBitmap.Canvas.Font.Style - [fsBold];
  if Pin[5].ValueAsBoolean then
    FBitmap.Canvas.Font.Style := FBitmap.Canvas.Font.Style + [fsItalic]
  else
    FBitmap.Canvas.Font.Style := FBitmap.Canvas.Font.Style - [fsItalic];

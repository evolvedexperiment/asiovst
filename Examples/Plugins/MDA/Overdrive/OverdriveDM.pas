...
procedure TOverdriveDataModule.ParameterMuffleChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FFilter := Power(10, -1.6 * 0.01 * Value);
end;

procedure TOverdriveDataModule.ParameterOutputChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
...
begin
  ...
  begin
    ...
    if (Inputs[1, Sample] > 0) then
      Outp[1] := sqrt(Inp[1])
    else
      Outp[1] := -sqrt(-Inp[1]);

    //filter
    State[0] := State[0] + FFilter * (FDrive * (Outp[0] - Inp[0]) + Inp[0] - State[0]);

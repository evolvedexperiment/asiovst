...
{$A4}
  ...
  CNoiseShaperCoefficients3MEc: array [0 .. 2] of Single = (1.652,
    -1.049, 0.1382);
  ...
  // F-weighted
  CNoiseShaperCoefficients3Fc: array [0 .. 2] of Single = (1.623,
    -0.982, 0.109);
  CNoiseShaperCoefficients9Fc: array [0 .. 8] of Single = (2.412, -3.370, 3.937,
    -4.174, 3.353, -2.205, 1.281, -0.569, 0.0847);
  ...
  CNoiseShaperCoefficients15kSharpEx88200: array [0 .. 7] of Single =
    ...
    -0.68687433463692, -0.686840576074174, 0.888606037761557,
    -0.298648374809776);
...
type
  TNoiseShaperType = (nsNone, nsEFB, ns2Sc, ns2MEc, ns3MEc, ns9MEc, ns5IEc,
    ns9IEc, ns3Fc, ns9Fc, nsSBM, nsSBMr, nsSharp14k7thOrder, nsSharp15k8thOrder,
    nsSharp16k9thOrder, nsExperimental);
  ...
  public
    ...
    property DitherType: TDitherType read FDitherType write SetDitherType
      default dtTriangular;
  end;
  ...
  public
    ...
    property NoiseshaperType: TNoiseShaperType read FNoiseshaperType
      write SetNoiseshaperType default ns9Fc;
  end;
  ...
  published
    property BitDepth;
    property DitherAmplitude: Single read FDitherAmplitude
      write FDitherAmplitude;
    property DitherType;
    property Limit;
  ...
  published
    property BitDepth;
    property DitherAmplitude: Double read FDitherAmplitude
      write FDitherAmplitude;
    property DitherType;
    property Limit;
    property NoiseshaperType;
...
begin
  ...
  else if Dest is TDitherNoiseShaper32 then
    with TDitherNoiseShaper32(Dest) do
    begin
      inherited;
      FBitMul := Self.FBitMul;
  ...
  case FNoiseshaperType of
    ...
    nsEFB:
      begin
        ...
        Move(CNoiseShaperCoefficientsEFB[0], FCoefficients[0],
          FOrder * SizeOf(Single));
      end;
    ns2Sc:
      begin
        Order := Length(CNoiseShaperCoefficients2Sc);
        Move(CNoiseShaperCoefficients2Sc[0], FCoefficients[0],
          FOrder * SizeOf(Single));
      end;
    ns2MEc:
      begin
        Order := Length(CNoiseShaperCoefficients2MEc);
        Move(CNoiseShaperCoefficients2MEc[0], FCoefficients[0],
          FOrder * SizeOf(Single));
      end;
    ns3MEc:
      begin
        Order := Length(CNoiseShaperCoefficients3MEc);
        Move(CNoiseShaperCoefficients3MEc[0], FCoefficients[0],
          FOrder * SizeOf(Single));
      end;
    ns9MEc:
      begin
        Order := Length(CNoiseShaperCoefficients9MEc);
        Move(CNoiseShaperCoefficients9MEc[0], FCoefficients[0],
          FOrder * SizeOf(Single));
      end;
    ns5IEc:
      begin
        Order := Length(CNoiseShaperCoefficients5IEc);
        Move(CNoiseShaperCoefficients5IEc[0], FCoefficients[0],
          FOrder * SizeOf(Single));
      end;
    ns9IEc:
      begin
        Order := Length(CNoiseShaperCoefficients9IEc);
        Move(CNoiseShaperCoefficients9IEc[0], FCoefficients[0],
          FOrder * SizeOf(Single));
      end;
    ns3Fc:
      begin
        Order := Length(CNoiseShaperCoefficients3Fc);
        Move(CNoiseShaperCoefficients3Fc[0], FCoefficients[0],
          FOrder * SizeOf(Single));
      end;
    ns9Fc:
      begin
        Order := Length(CNoiseShaperCoefficients9Fc);
        Move(CNoiseShaperCoefficients9Fc[0], FCoefficients[0],
          FOrder * SizeOf(Single));
      end;
    nsSBM:
      begin
        Order := Length(CNoiseShaperCoefficientsSBM);
        Move(CNoiseShaperCoefficientsSBM[0], FCoefficients[0],
          FOrder * SizeOf(Single));
      end;
    nsSBMr:
      begin
        Order := Length(CNoiseShaperCoefficientsSBMr);
        Move(CNoiseShaperCoefficientsSBMr[0], FCoefficients[0],
          FOrder * SizeOf(Single));
      end;
    nsSharp14k7thOrder:
      begin
        Order := Length(CNoiseShaperCoefficients14kSharp44100);
        Move(CNoiseShaperCoefficients14kSharp44100[0], FCoefficients[0],
          FOrder * SizeOf(Single));
      end;
    nsSharp15k8thOrder:
      begin
        Order := Length(CNoiseShaperCoefficients15kSharp44100);
        Move(CNoiseShaperCoefficients15kSharp44100[0], FCoefficients[0],
          FOrder * SizeOf(Single));
      end;
    nsSharp16k9thOrder:
      begin
        Order := Length(CNoiseShaperCoefficients16kSharp44100);
        Move(CNoiseShaperCoefficients16kSharp44100[0], FCoefficients[0],
          FOrder * SizeOf(Single));
      end;
    nsExperimental:
      begin
        Order := Length(CNoiseShaperCoefficientsEX);
        Move(CNoiseShaperCoefficientsEX[0], FCoefficients[0],
          FOrder * SizeOf(Single));
      end;
  end;
end;
...
{$IFDEF RenderFIR}
  // render FIR filter
  Result := Input;
  for Coef := 0 to FOrder - 1 do
    Result := Result - FCoefficients[Coef] * FHistory
      [(FOrder + FHistoryPos - Coef) mod FOrder];
...
begin
  ...
  for Coef := 0 to FOrder - 1 do
    Input := Input - FCoefficients[Coef] * FHistory
      [(FOrder + FHistoryPos - Coef) mod FOrder];
  ...
  case FDitherType of
    dtNone:
      Result := Round(Input - CHalf32);
    dtEqual:
      Result := Round(Input - CHalf32 + FDitherAmplitude * FastRandom);
    dtTriangular:
      Result := Round(Input - CHalf32 + FDitherAmplitude * (random - random));
    dtGauss:
      Result := Round(Input - CHalf32 + FDitherAmplitude * RandomGauss);
    dtFastGauss:
      Result := Round(Input - CHalf32 + FDitherAmplitude * FastRandomGauss);
  else
    Result := 0;
  end;

  if FLimit then
    if Result < FLimits[0] then
      Result := FLimits[0]
    else if Result > FLimits[1] then
      Result := FLimits[1];
...
begin
  ...
  else if Dest is TDitherNoiseShaper64 then
...
begin
  case FNoiseshaperType of
    nsEFB:
      begin
        Order := Length(CNoiseShaperCoefficientsEFB);
        for Coef := 0 to Order - 1 do
          FCoefficients[Coef] := CNoiseShaperCoefficients2Sc[Coef];
      end;
    ns2Sc:
      begin
        Order := Length(CNoiseShaperCoefficients2Sc);
    ...
    ns2MEc:
      begin
        Order := Length(CNoiseShaperCoefficients2MEc);
        for Coef := 0 to Order - 1 do
          FCoefficients[Coef] := CNoiseShaperCoefficients2MEc[Coef];
      end;
    ns3Fc:
      begin
        Order := Length(CNoiseShaperCoefficients3Fc);
        for Coef := 0 to Order - 1 do
          FCoefficients[Coef] := CNoiseShaperCoefficients3Fc[Coef];
      end;
    ns3MEc:
      begin
        Order := Length(CNoiseShaperCoefficients3MEc);
        for Coef := 0 to Order - 1 do
          FCoefficients[Coef] := CNoiseShaperCoefficients3MEc[Coef];
      end;
    ns5IEc:
      begin
        Order := Length(CNoiseShaperCoefficients5IEc);
        for Coef := 0 to Order - 1 do
          FCoefficients[Coef] := CNoiseShaperCoefficients5IEc[Coef];
      end;
    ns9Fc:
      begin
        Order := Length(CNoiseShaperCoefficients9Fc);
        for Coef := 0 to Order - 1 do
          FCoefficients[Coef] := CNoiseShaperCoefficients9Fc[Coef];
      end;
    ns9MEc:
      begin
        Order := Length(CNoiseShaperCoefficients9MEc);
        for Coef := 0 to Order - 1 do
          FCoefficients[Coef] := CNoiseShaperCoefficients9MEc[Coef];
      end;
    ns9IEc:
      begin
        Order := Length(CNoiseShaperCoefficients9IEc);
        for Coef := 0 to Order - 1 do
          FCoefficients[Coef] := CNoiseShaperCoefficients9IEc[Coef];
      end;
    nsSBM:
      begin
        Order := Length(CNoiseShaperCoefficientsSBM);
        for Coef := 0 to Order - 1 do
          FCoefficients[Coef] := CNoiseShaperCoefficientsSBM[Coef];
      end;
    nsSBMr:
      begin
        Order := Length(CNoiseShaperCoefficientsSBMr);
        for Coef := 0 to Order - 1 do
          FCoefficients[Coef] := CNoiseShaperCoefficientsSBMr[Coef];
      end;
    nsSharp14k7thOrder:
      begin
        Order := Length(CNoiseShaperCoefficients14kSharp44100);
        for Coef := 0 to Order - 1 do
          FCoefficients[Coef] := CNoiseShaperCoefficients14kSharp44100[Coef];
      end;
    nsSharp15k8thOrder:
      begin
        Order := Length(CNoiseShaperCoefficients15kSharp44100);
        for Coef := 0 to Order - 1 do
          FCoefficients[Coef] := CNoiseShaperCoefficients15kSharp44100[Coef];
      end;
    nsSharp16k9thOrder:
      begin
        Order := Length(CNoiseShaperCoefficients16kSharp44100);
        for Coef := 0 to Order - 1 do
          FCoefficients[Coef] := CNoiseShaperCoefficients16kSharp44100[Coef];
      end;
    nsExperimental:
      begin
        Order := Length(CNoiseShaperCoefficientsEX);
        for Coef := 0 to Order - 1 do
          FCoefficients[Coef] := CNoiseShaperCoefficientsEX[Coef];
      end;
  end;
end;
...
begin
  ...
  case FDitherType of
    dtNone:
      Result := Round(Input - CHalf64);
    dtEqual:
      Result := Round(Input - CHalf64 + FDitherAmplitude * (2 * random - 1));
    dtTriangular:
      Result := Round(Input - CHalf64 + FDitherAmplitude * (random - random));
    dtGauss:
      Result := Round(Input - CHalf64 + FDitherAmplitude * RandomGauss);
    dtFastGauss:
      Result := Round(Input - CHalf64 + FDitherAmplitude * FastRandomGauss);
  else
    Result := 0;
  end;
...
begin
  if Samplerate < 41000 then
    Move(CNoiseShaperCoefficients15kSharpEx40000[0], FCoefficients[0],
      FOrder * SizeOf(Single))
  else if Samplerate < 46000 then
    Move(CNoiseShaperCoefficients15kSharpEx44100[0], FCoefficients[0],
      FOrder * SizeOf(Single))
  else if Samplerate < 55000 then
    Move(CNoiseShaperCoefficients15kSharpEx48000[0], FCoefficients[0],
      FOrder * SizeOf(Single))
  else if Samplerate < 75100 then
    Move(CNoiseShaperCoefficients15kSharpEx64000[0], FCoefficients[0],
      FOrder * SizeOf(Single))
  else
    Move(CNoiseShaperCoefficients15kSharpEx96000[0], FCoefficients[0],
      FOrder * SizeOf(Single));
end;

...
const
  CNumFrequencies = 32;
  CThirdOctaveFrequencies: array [0 .. CNumFrequencies - 1] of Single = (16, 20,
...
function TSimpleThirdOctaveVocoder.ProcessSample(Input,
...
begin
  ...
  for Band := 0 to Length(FAnalysisFilters) - 1 do
    FAnalysisFilters[Band].SampleRate := SampleRate;

  for Band := 0 to Length(FSynthesisFilters) - 1 do
    FSynthesisFilters[Band].SampleRate := SampleRate;
end;
  ...
  for Band := 0 to Length(CBarkFrequencyScale) - 1 do
    ...
    begin
      if FDownSampler >= 0 then
        while (2 shl DS) * CBarkFrequencyScale[Band] < FTransitionBandwidth *
          SampleRate do
          DS := DS + 1;
...
begin
  ...
  for Band := 0 to CNumFrequencies - 1 do
    with FBands[Band] do
    begin
      if (FDownSampler mod DownsampleFactor) <> 0 then
        Break;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        SampleRate := Self.SampleRate;
        SetFilterValues(min(0.5 * SampleRate, HalfThirdOctaveMulFak64 *
          (CThirdOctaveFrequencies[CNumFrequencies - Band - 1])), 0, 0.1);
        if FDownSampler >= 0 then
          while (2 shl DS) * Frequency < 0.1 * SampleRate do
            Inc(DS);
        CalculateCoefficients;
      end;
      ...
      begin
        SampleRate := Self.SampleRate;
        SetFilterValues((CThirdOctaveFrequencies[CNumFrequencies - Band - 1]) /
          HalfThirdOctaveMulFak64, 0, 0.1);
        CalculateCoefficients;
      end;

...
type
  ...
  public
    ...
    property OnLoudnessChanged: TLoudnessUpdate read FOnLoudnessChanged
      write FOnLoudnessChanged;
    property OnPeakLoudnessChanged: TLoudnessUpdate read FOnPeakChanged
      write FOnPeakChanged;
  end;
...
begin
  case FTime of
    ltMomentary:
      FUpdateSampleCount := Round(0.1 * SampleRate);
    ltShort:
      FUpdateSampleCount := Round(SampleRate);
    ltIntegrated:
      FUpdateSampleCount := Round(SampleRate);
  end;
end;
...
begin
  if Assigned(FOnLoudnessChanged) then
    FOnLoudnessChanged(Self, GetLoudness);
end;
...
begin
  case FTime of
    ltMomentary:
      Result := GetLoudnessMomentary;
    ltShort:
      Result := GetLoudnessShort;
    ltIntegrated:
      Result := GetLoudnessIntegration;
  else
    raise Exception.Create('Undefined');
  end;
end;
...
begin
  ...
  if FAbsoluteGatedCount <> 0 then
    Value := Value + FAbsoluteGatedValue / FAbsoluteGatedCount;
  Assert(Value > 0);
  ...
  begin
    if LLRec^.Loudness < Loudness then
      Break;
  ...
  if ItemCount = 0 then
    ...
    Value := CMeanSquareBias + Value / ItemCount;
...
begin
  ...
  repeat
    ...
    else if (LinkPointer^)^.Loudness < Loudness then
    begin
...
begin
  ...
  if FMomIntValue < CMeanSquareBias then
    FMomIntValue := CMeanSquareBias;
  ...
  begin
    FSampleCount := 0;
    if FIsRunning then
      ProcessLongTermSample(FMomIntValue * FMomIntScale);
  end;
  ...
  begin
    ...
    if Assigned(FOnPeakChanged) then
      FOnPeakChanged(Self, -0.691 + 10 * FastLog10ContinousError3(FPeakHold));
  end;
...
begin
  ...
  begin
    ...
    FDelayLine400ms[ChannelIndex] := TDelayLineSamples32.Create
      (F400msSampleCount);
    FDelayLine2600ms[ChannelIndex] := TDelayLineSamples32.Create
      (F2600msSampleCount);
...
begin
  ...
  if FAbsoluteGatedCount[0] <> 0 then
    Value[0] := Value[0] + FAbsoluteGatedValue[0] / FAbsoluteGatedCount[0];
  Value[1] := CMeanSquareBias;
  if FAbsoluteGatedCount[1] <> 0 then
    Value[1] := Value[1] + FAbsoluteGatedValue[1] / FAbsoluteGatedCount[1];
  ...
  begin
    ...
    if ItemCount = 0 then
      Value[ChannelIndex] := CMeanSquareBias
    else
      Value[ChannelIndex] := CMeanSquareBias + Value[ChannelIndex] / ItemCount;
  end;
...
begin
  ...
  if FMomIntValue[0] < CMeanSquareBias then
    FMomIntValue[0] := CMeanSquareBias;

  // calculate short integration (3 seconds)
  FShortIntValue[0] := FShortIntValue[0] + Sqr(CurrentValue) -
    Sqr(FDelayLine2600ms[0].ProcessSample32(Value400ms));
  if FShortIntValue[0] < CMeanSquareBias then
    FShortIntValue[0] := CMeanSquareBias;
...
begin
  ...
  if FMomIntValue[1] < CMeanSquareBias then
    FMomIntValue[1] := CMeanSquareBias;

  // calculate short integration (3 seconds)
  FShortIntValue[1] := FShortIntValue[1] + Sqr(CurrentValue) -
    Sqr(FDelayLine2600ms[1].ProcessSample32(Value400ms));
  if FShortIntValue[1] < CMeanSquareBias then
    FShortIntValue[1] := CMeanSquareBias;

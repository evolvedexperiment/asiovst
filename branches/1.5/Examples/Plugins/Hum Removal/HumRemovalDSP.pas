...
begin
  ...
  for Channel := 0 to Length(FHumRemoval) - 1 do
    FHumRemoval[Channel] := TDspHumRemoval.Create;
...
begin
  for Channel := 0 to Length(FHumRemoval) - 1 do
    FreeAndNil(FHumRemoval[Channel]);

  for Index := 0 to Length(FGoertzel) - 1 do
    FreeAndNil(FGoertzel[Index]);
end;

procedure THumRemovalModule.ParameterHighpassOrderDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := AnsiString(IntToStr(Round(Parameter[Index])));
end;

procedure THumRemovalModule.ParameterHighpassTypeChange(Sender: TObject;
...
begin
  for Channel := 0 to Length(FHumRemoval) - 1 do
    if Assigned(FHumRemoval[Channel]) then
      case Round(Value) of
        0:
          FHumRemoval[Channel].HighpassFilterType := TButterworthLowCutFilter;
        1:
          FHumRemoval[Channel].HighpassFilterType := TChebyshev1LowCutFilter;
        2:
          FHumRemoval[Channel].HighpassFilterType := TChebyshev2LowCutFilter;
      end;

  // eventually update GUI
  if EditorForm is TFmHumRemoval then
    with TFmHumRemoval(EditorForm) do
      UpdateHighpassType;
end;

procedure THumRemovalModule.ParameterHighpassFrequencyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to Length(FHumRemoval) - 1 do
    if Assigned(FHumRemoval[Channel]) then
      if Assigned(FHumRemoval[Channel].HighpassFilter) then
        FHumRemoval[Channel].HighpassFilter.Frequency := Value;
...
procedure THumRemovalModule.ParameterHighpassOrderChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to Length(FHumRemoval) - 1 do
    if Assigned(FHumRemoval[Channel]) then
      FHumRemoval[Channel].HighpassFilter.Order := Round(Value);
...
begin
  for Channel := 0 to Length(FHumRemoval) - 1 do
    if Assigned(FHumRemoval[Channel]) then
      FHumRemoval[Channel].HighpassFilterActive := Value > 0.5;
...
procedure THumRemovalModule.ParameterCaptureHumProfileChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Ndx: Integer;
begin
  if Value > 0.5 then
    OnProcess := VSTModuleProcessDetect
  else
  ...
  for Ndx := 0 to Length(FGoertzel) - 1 do
    if Assigned(FGoertzel[Ndx]) then
      FGoertzel[Ndx].Reset;

  // eventually update GUI
  if EditorForm is TFmHumRemoval then
    with TFmHumRemoval(EditorForm) do
      UpdateCaptureHumProfile;
end;

procedure THumRemovalModule.ParameterAttenuationChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to Length(FHumRemoval) - 1 do
    if Assigned(FHumRemoval[Channel]) then
      FHumRemoval[Channel].Attenuation := Value;
end;

function THumRemovalModule.Magnitude_dB(Frequency: Single): Single;
begin
  if Assigned(FHumRemoval[0]) then
    Result := FHumRemoval[0].Magnitude_dB(Frequency)
  else
    Result := 1;
end;

procedure THumRemovalModule.ParameterBandwidthChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to Length(FHumRemoval) - 1 do
    if Assigned(FHumRemoval[Channel]) then
      FHumRemoval[Channel].Bandwidth := Value;
...
procedure THumRemovalModule.ParameterFundamentalFrequencyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  Channel: Integer;
begin
  for Channel := 0 to Length(FHumRemoval) - 1 do
    if Assigned(FHumRemoval[Channel]) then
      FHumRemoval[Channel].FundamentalFrequency := Value;
...
begin
  case Round(Parameter[Index]) of
    ...
    1:
      PreDefined := 'Chebyshev I';
    2:
      PreDefined := 'Chebyshev II';
  end;
end;
...
begin
  if Abs(SampleRate) > 0 then
    for Channel := 0 to Length(FHumRemoval) - 1 do
      if Assigned(FHumRemoval[Channel]) then
        FHumRemoval[Channel].HighpassFilter.SampleRate := Abs(SampleRate);
end;
...
begin
  for Channel := 0 to Length(FHumRemoval) - 1 do
    for Sample := 0 to SampleFrames - 1 do
      Outputs[Channel, Sample] := FHumRemoval[Channel].ProcessSample32
...
begin
  ...
  begin
    ...
    for Index := 0 to Length(FGoertzel) - 1 do
      FGoertzel[Index].ProcessSample64(Sum);

...
uses
  Windows, Classes, Forms, SysUtils, {$IFDEF UseCriticalSection}SyncObjs,
  {$ENDIF} DAV_Types, DAV_VstHost, DAV_DspPolyphaseUpsampler,
  DAV_DspPolyphaseDownsampler, WinAmpDspVstGui;
...
type
  ...
  TWinAmpConvert = procedure(const Data: Pointer;
    const ChannelCount, SampleFrames: Integer) of object;
  ...
  private
    ...
    procedure ConvertDummy(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertFloatToInterleaved8bit(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertFloatToInterleaved16bit(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertFloatToInterleaved24bit(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertFloatToInterleaved32bit(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertInterleaved8bitToFloat(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertInterleaved16bitToFloat(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertInterleaved24bitToFloat(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertInterleaved32bitToFloat(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
{$IFDEF UseFloatConvert}
    procedure ConvertFloatToInterleaved(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertInterleavedToFloat(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertFloatToInterleavedOversampled(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertInterleavedToFloatOversampled(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
{$ENDIF}
    procedure ConvertFloatToInterleaved8bitOversampled(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertFloatToInterleaved16bitOversampled(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertFloatToInterleaved24bitOversampled(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertFloatToInterleaved32bitOversampled(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertInterleaved8bitToFloatOversampled(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertInterleaved16bitToFloatOversampled(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertInterleaved24bitToFloatOversampled(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
    procedure ConvertInterleaved32bitToFloatOversampled(const Data: Pointer;
      const ChannelCount, SampleFrames: Integer);
...
{$ENDIF}
  ...
  public
    constructor CreateNew(AOwner: TComponent;
      const AWinAmpDspModule: PWinampDSPModule); reintroduce; virtual;
    destructor Destroy; override;
    procedure Config;
    procedure Quit;
    procedure LoadVSTDLL(const VSTDLL: TFileName);
    function ModifySamples(const Samples: Pointer;
      const SampleFrames, BitPerSample, ChannelCount,
...
function ModifySamples(const This_Mod: PWinampDSPModule; const Samples: Pointer;
  const SampleFrames, BitPerSample, ChannelCount, SampleRate: Integer)
  : Integer; cdecl;
...
var
  WADSPHeader: TWinAmpDSPheader = (Version: $20;
    Description: 'VST Host DSP v1.1.5 for WinAmp'#0; GetModule: GetModule;
    Key: $21);

  WADSPModule: TWinampDSPModule = (Description
    : 'VST Host DSP v1.1.5 for WinAmp'#0; HwndParent: 0; HDLLinstance: 0;
    Config: Config; Init: Init; ModifySamples: ModifySamplesDummy; Quit: Quit;
    UserData: nil);
...
begin
  ...
  try
    ...
    if (ContainedVSTPlugins.Count > 0) and (ContainedVSTPlugins[0] <> 'DLL')
    then
    begin
...
const
  GenericFileMapping: TGenericMapping = (GenericRead: FILE_GENERIC_READ;
...
begin
  ...
  // make sure a pointer to the TWinAmpDSPModule exists
  if not Assigned(This_Mod) then
    Exit;

  // make sure a TWinAmpObject instance exists
  if not Assigned(This_Mod^.UserData) then
    Exit;
...
constructor TWinAmpObject.CreateNew(AOwner: TComponent;
  const AWinAmpDspModule: PWinampDSPModule);
var
  ContainedVSTPlugins: TStringList;
  RS: TResourceStream;
begin
...
{$ENDIF}
  try
    ...
    if FRegistryKey = 'dsp_vst.dll' then
      FRegistryKey := 'Software\WinAmp\VST Host DSP Plugin'
    else
      FRegistryKey := 'Software\WinAmp\' + Copy(FRegistryKey, 1,
        Pos('.dll', FRegistryKey) - 1);
    FFxpName := ExpandUNCFileName(Copy(FxpName, 1, Pos('.dll', FFxpName) - 1)
      + '.fxp');
  except
    FRegistryKey := 'Software\WinAmp\VST Host DSP Plugin';
...
begin
  if Assigned(FEditorForm) then
    FEditorForm.CBPreset.ItemIndex := FVstHost[0].CurrentProgram;
end;
...
{$ENDIF}
      if not Assigned(FEditorForm) then
        FEditorForm := TFmWinAmpVST.Create(Self);
...
{$ENDIF}
      ...
      with FVstHost[0] do
        try
          ...
          else if CheckAccessToFile(GENERIC_WRITE, ExtractFilePath(FFxpName))
          then
            SavePreset(FFxpName);
...
begin
  for Channel := 0 to min(Length(FInputBuffer), ChannelCount) - 1 do
    for Sample := 0 to SampleFrames - 1 do
      FInputBuffer[Channel]^[Sample] := I8^[Sample * ChannelCount + Channel]
        * DivFak8;
end;

procedure TWinAmpObject.ConvertInterleaved8bitToFloatOversampled
...
begin
  for Channel := 0 to min(Length(FInputBuffer), ChannelCount) - 1 do
    for Sample := 0 to SampleFrames - 1 do
      FUpsampler[Channel].ProcessSample(I8^[Sample * ChannelCount + Channel] *
        DivFak8, PDAV2SingleArray(@FInputBuffer[Channel]^[2 * Sample])^);
end;
...
begin
  for Channel := 0 to min(Length(FInputBuffer), ChannelCount) - 1 do
    for Sample := 0 to SampleFrames - 1 do
      FInputBuffer[Channel]^[Sample] := I16^[Sample * ChannelCount + Channel]
        * DivFak16;
end;

procedure TWinAmpObject.ConvertInterleaved16bitToFloatOversampled
  (const Data: Pointer; const ChannelCount, SampleFrames: Integer);
var
  I16: PSmallIntArray absolute Data;
  Channel, Sample: Integer;
...
begin
  for Channel := 0 to min(Length(FInputBuffer), ChannelCount) - 1 do
    for Sample := 0 to SampleFrames - 1 do
      FUpsampler[Channel].ProcessSample(I16^[Sample * ChannelCount + Channel] *
        DivFak16, PDAV2SingleArray(@FInputBuffer[Channel]^[2 * Sample])^);
end;
...
procedure TWinAmpObject.ConvertInterleaved24bitToFloatOversampled
  (const Data: Pointer; const ChannelCount, SampleFrames: Integer);
var
  I24: P3ByteArray absolute Data;
  Channel, Sample: Integer;
  ...
  for Channel := 0 to min(Length(FInputBuffer), ChannelCount) - 1 do
    ...
    begin
      TempData := (ShortInt(I24^[Sample * ChannelCount + Channel][2]) shl 16) +
        (I24^[Sample * ChannelCount + Channel][1] shl 8) +
        (I24^[Sample * ChannelCount + Channel][0]);
      FUpsampler[Channel].ProcessSample(TempData,
        PDAV2SingleArray(@FInputBuffer[Channel]^[2 * Sample])^);
    end;
end;
...
begin
  for Channel := 0 to min(Length(FInputBuffer), ChannelCount) - 1 do
    for Sample := 0 to SampleFrames - 1 do
      FInputBuffer[Channel]^[Sample] := I32^[Sample * ChannelCount + Channel]
        * DivFak32;
end;
...
begin
  for Channel := 0 to min(Length(FInputBuffer), ChannelCount) - 1 do
    for Sample := 0 to SampleFrames - 1 do
      FUpsampler[Channel].ProcessSample(I32^[Sample * ChannelCount + Channel],
        PDAV2SingleArray(@FInputBuffer[Channel]^[2 * Sample])^);
end;
...
begin
  for Channel := 0 to min(Length(FInputBuffer), ChannelCount) - 1 do
    for Sample := 0 to SampleFrames - 1 do
      FInputBuffer[Channel]^[Sample] := Interleaved^
        [Sample * ChannelCount + Channel];
end;

procedure TWinAmpObject.ConvertInterleavedToFloatOversampled
  (const Data: Pointer; const ChannelCount, SampleFrames: Integer);
var
  Interleaved: PDAVSingleFixedArray absolute Data;
  Channel, Sample: Integer;
begin
  for Channel := 0 to min(Length(FInputBuffer), ChannelCount) - 1 do
    for Sample := 0 to SampleFrames - 1 do
      FUpsampler[Channel].ProcessSample
...
procedure TWinAmpObject.ConvertFloatToInterleaved16bitOversampled
  (const Data: Pointer; const ChannelCount, SampleFrames: Integer);
var
  I16: PSmallIntArray absolute Data;
  Channel, Sample: Integer;
...
procedure TWinAmpObject.ConvertFloatToInterleaved24bitOversampled
  (const Data: Pointer; const ChannelCount, SampleFrames: Integer);
var
  I24: P3ByteArray absolute Data;
  Channel, Sample: Integer;
...
procedure TWinAmpObject.ConvertFloatToInterleaved32bitOversampled
  (const Data: Pointer; const ChannelCount, SampleFrames: Integer);
var
  I32: PIntegerArray absolute Data;
  Channel, Sample: Integer;
...
begin
  for Channel := 0 to min(Length(FOutputBuffer), ChannelCount) - 1 do
    for Sample := 0 to SampleFrames - 1 do
      Interleaved^[Sample * ChannelCount + Channel] :=
        Limit(FOutputBuffer[Channel]^[Sample]);
end;

procedure TWinAmpObject.ConvertFloatToInterleavedOversampled
  (const Data: Pointer; const ChannelCount, SampleFrames: Integer);
var
  Interleaved: PDAVSingleFixedArray absolute Data;
  Channel, Sample: Integer;
begin
  for Channel := 0 to min(Length(FOutputBuffer), ChannelCount) - 1 do
    for Sample := 0 to SampleFrames - 1 do
      Interleaved^[Sample * ChannelCount + Channel] :=
        Limit(FDownsampler[Channel].ProcessSample
        (PDAV2SingleArray(@FOutputBuffer[Channel]^[2 * Sample])^));
end;
{$ENDIF}

function TWinAmpObject.ModifySamples(const Samples: Pointer;
...
begin
  ...
  if not Assigned(FVstHost) or not FVstHost[0].Active or FBypass then
    Exit;
      ...
      case BitPerSample of
        ...
        32:
          ...
          begin
            ...
            FWinAmpConvertOut := ConvertFloatToInterleaved32bitOversampled;
            // ConvertFloatToInterleavedOversampled;
          end
          else
          begin
            FWinAmpConvertIn := ConvertInterleaved32bitToFloat;
            // ConvertInterleavedToFloat;
            FWinAmpConvertOut := ConvertFloatToInterleaved32bit;
            // ConvertFloatToInterleaved;
          end;
      else
        begin
...
{$ENDIF}
  if Assigned(FEditorForm) then
    FEditorForm.UpdatePluginInformation;
end;

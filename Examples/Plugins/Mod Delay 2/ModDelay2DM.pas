...
begin
  ...
  try
    for Channel := 0 to Length(FModDelay) - 1 do
      FreeAndNil(FModDelay[0]);
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TModDelay2Module.ParameterGainDisplay(Sender: TObject;
...
procedure TModDelay2Module.ParameterMixChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
    if Assigned(FModDelay[Index div 7]) then
      FModDelay[Index div 7].Mix := Value;
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TModDelay2Module.ParameterDepthChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
    if Assigned(FModDelay[Index div 7]) then
      FModDelay[Index div 7].Depth := Value;
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TModDelay2Module.ParameterRateChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
    if Assigned(FModDelay[Index div 7]) then
      FModDelay[Index div 7].Rate := Value;
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TModDelay2Module.ParameterFeedbackChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
    if Assigned(FModDelay[Index div 7]) then
      FModDelay[Index div 7].Feedback := Value;
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TModDelay2Module.ParameterDelayChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
    if Assigned(FModDelay[Index div 7]) then
      FModDelay[Index div 7].Delay := Value;
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TModDelay2Module.ParameterGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FGain[Index div 7] := dB_to_Amp(Value);
end;

procedure TModDelay2Module.ParameterLowpassChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
    if Assigned(FModDelay[Index div 7]) then
      FModDelay[Index div 7].LowpassFrequency := Value;
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TModDelay2Module.ParameterLowpassLabel(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
var
  Freq: Single;
begin
  ...
  if Freq < 1000 then
    PreDefined := 'Hz'
  else if Freq <= 20000 then
    PreDefined := 'kHz'
  else
    PreDefined := '';
end;

procedure TModDelay2Module.ParameterLowpassDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
var
  Freq: Single;
begin
  Freq := FModDelay[Index div 7].LowpassFrequency;
  if Freq < 1000 then
    PreDefined := AnsiString(FloatToStrF(Freq, ffGeneral, 3, 3))
  else if Freq < 20000 then
    PreDefined := AnsiString(FloatToStrF(0.001 * Freq, ffGeneral, 3, 3))
  else
    PreDefined := 'off';
end;
...
begin
  ...
  try
    for Channel := 0 to Length(FModDelay) - 1 do
      if Assigned(FModDelay[Channel]) then
        FModDelay[Channel].SampleRate := SampleRate;
  finally
    FCriticalSection.Leave;
  end;
...
begin
  ...
  try
    ...
    begin
      Outputs[0, Sample] := FModDelay[0].ProcessSample32
        (FGain[0] * Inputs[0, Sample]);
      Outputs[1, Sample] := FModDelay[1].ProcessSample32
        (FGain[1] * Inputs[1, Sample]);
    end;
  finally
    FCriticalSection.Leave;
  end;

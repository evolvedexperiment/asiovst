...
type
  ...
  protected
    procedure FilterSub(CurrentRow, PreviousRow: PByteArray;
      BytesPerRow, PixelByteSize: Integer);
    procedure FilterUp(CurrentRow, PreviousRow: PByteArray;
      BytesPerRow, PixelByteSize: Integer);
    procedure FilterAverage(CurrentRow, PreviousRow: PByteArray;
      BytesPerRow, PixelByteSize: Integer);
    procedure FilterPaeth(CurrentRow, PreviousRow: PByteArray;
      BytesPerRow, PixelByteSize: Integer);

    procedure DecodeFilterRow(FilterMethod: TAdaptiveFilterMethod;
      CurrentRow, PreviousRow: PByteArray; BytesPerRow, PixelByteSize: Integer);
  public
    procedure DecodeToScanline(Bitmap: TObject;
      ScanLineCallback: TScanLineCallback); virtual; abstract;
  end;
  ...
  protected
    procedure FilterSub(CurrentRow, PreviousRow, OutputRow: PByteArray;
      BytesPerRow, PixelByteSize: Integer);
    procedure FilterUp(CurrentRow, PreviousRow, OutputRow: PByteArray;
      BytesPerRow, PixelByteSize: Integer);
    procedure FilterAverage(CurrentRow, PreviousRow, OutputRow: PByteArray;
      BytesPerRow, PixelByteSize: Integer);
    procedure FilterPaeth(CurrentRow, PreviousRow, OutputRow: PByteArray;
      BytesPerRow, PixelByteSize: Integer);

    procedure EncodeFilterRow(CurrentRow, PreviousRow, OutputRow,
  ...
  protected
    ...
    procedure TransferData(Source: Pointer; Destination: Pointer);
      virtual; abstract;
  public
    constructor Create(Stream: TStream; Header: TChunkPngImageHeader;
  ...
  protected
    procedure TransferData(const Pass: Byte; Source: Pointer;
      Destination: Pointer); virtual; abstract;
  public
    constructor Create(Stream: TStream; Header: TChunkPngImageHeader;
      Gamma: TChunkPngGamma = nil; Palette: TChunkPngPalette = nil); override;
    destructor Destroy; override;
  ...
  TPngAdam7GrayscaleAlpha8bitBGRADecoder =
    TPngAdam7GrayscaleAlpha8bitRGBADecoder;
  TPngAdam7GrayscaleAlpha16bitBGRADecoder =
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Palette[Index].R :=
            Round(Power((Color.R * COne255th), PreCalcGamma) * 255);
          Palette[Index].G :=
            Round(Power((Color.G * COne255th), PreCalcGamma) * 255);
          Palette[Index].B :=
            Round(Power((Color.B * COne255th), PreCalcGamma) * 255);
        end;
      end
      else
        for Index := 0 to FPalette.Count - 1 do
  ...
  begin
    ...
    begin
      ...
      for Index := 0 to $FF do
        FMappingTable[Index] :=
          Round(Power((Index * COne255th), PreCalcGamma) * 255);
    end
    else
      for Index := 0 to $FF do
...
begin
  for Index := PixelByteSize + 1 to BytesPerRow do
    CurrentRow[Index] :=
      (CurrentRow[Index] + CurrentRow[Index - PixelByteSize]) and $FF;
{$ELSE}
asm
  ADD     EDX, 1
  MOV     EAX, EDX
...
begin
  ...
  for Index := PixelByteSize + 1 to BytesPerRow do
    CurrentRow[Index] :=
      (CurrentRow[Index] + (CurrentRow[Index - PixelByteSize] + PreviousRow[
      Index]) shr 1) and $FF;
end;

function PaethPredictor(a, B, c: Byte): Integer; {$IFNDEF CPUx86_64} pascal;
{$ENDIF}
{$IFDEF PUREPASCAL}
var
...
begin
  ...
  for Index := PixelByteSize + 1 to BytesPerRow do
    CurrentRow[Index] :=
      (CurrentRow[Index] + PaethPredictor(CurrentRow[Index - PixelByteSize],
      PreviousRow[Index], PreviousRow[Index - PixelByteSize])) and $FF;
end;
...
begin
  case FilterMethod of
    afmNone:
      ;
    afmSub:
      FilterSub(CurrentRow, PreviousRow, BytesPerRow, PixelByteSize);
    afmUp:
      FilterUp(CurrentRow, PreviousRow, BytesPerRow, PixelByteSize);
    afmAverage:
      FilterAverage(CurrentRow, PreviousRow, BytesPerRow, PixelByteSize);
    afmPaeth:
      FilterPaeth(CurrentRow, PreviousRow, BytesPerRow, PixelByteSize);
  else
    raise Exception.Create(RCStrUnsupportedFilter);
  end;
end;

{ TCustomPngEncoder }

procedure TCustomPngEncoder.FilterSub(CurrentRow, PreviousRow,
  OutputRow: PByteArray; BytesPerRow, PixelByteSize: Integer);
var
  Index: Integer;
begin
  // copy first pixel
...
procedure TCustomPngEncoder.FilterUp(CurrentRow, PreviousRow,
  OutputRow: PByteArray; BytesPerRow, PixelByteSize: Integer);
var
  Index: Integer;
begin
...
procedure TCustomPngEncoder.FilterAverage(CurrentRow, PreviousRow,
  OutputRow: PByteArray; BytesPerRow, PixelByteSize: Integer);
var
  Index: Integer;
begin
...
procedure TCustomPngEncoder.FilterPaeth(CurrentRow, PreviousRow,
  OutputRow: PByteArray; BytesPerRow, PixelByteSize: Integer);
var
  Index: Integer;
begin
...
procedure TCustomPngEncoder.EncodeFilterRow(CurrentRow, PreviousRow, OutputRow,
  TempBuffer: PByteArray; BytesPerRow, PixelByteSize: Integer);
var
  PixelIndex: Integer;
  CurrentSum: Cardinal;
...
begin
  ...
  for PixelIndex := 1 to BytesPerRow do
    CurrentSum := CurrentSum + TempBuffer[PixelIndex];
  ...
  FilterAverage(CurrentRow, PreviousRow, TempBuffer, BytesPerRow,
    PixelByteSize);
  for PixelIndex := 1 to BytesPerRow do
    CurrentSum := CurrentSum + TempBuffer[PixelIndex];
...
begin
  ...
  begin
    ...
    then
      raise Exception.Create(RCStrDataIncomplete);
...
procedure TPngGrayscale16bitRGBADecoder.TransferData(Source: Pointer;
  Destination: Pointer);
var
  Index: Integer;
...
begin
  for Index := 0 to FHeader.Width - 1 do
  begin
    Dest^.R := FMappingTable[Src^ and $FF];
    INC(Src);
    Dest^.G := Dest^.R;
    Dest^.B := Dest^.R;
    Dest^.a := 255;
...
procedure TPngTrueColor16bitRGBADecoder.TransferData(Source: Pointer;
  Destination: Pointer);
var
  Index: Integer;
...
procedure TPngGrayscaleAlpha8bitRGBADecoder.TransferData(Source: Pointer;
  Destination: Pointer);
var
  Index: Integer;
  Src: PByte absolute Source;
...
procedure TPngGrayscaleAlpha16bitRGBADecoder.TransferData(Source: Pointer;
  Destination: Pointer);
var
  Index: Integer;
  Src: PWord absolute Source;
...
procedure TPngTrueColorAlpha8bitRGBADecoder.TransferData(Source: Pointer;
  Destination: Pointer);
var
  Index: Integer;
...
procedure TPngTrueColorAlpha16bitRGBADecoder.TransferData(Source: Pointer;
  Destination: Pointer);
var
  Index: Integer;
...
procedure TPngTrueColor16bitBGRADecoder.TransferData(Source: Pointer;
  Destination: Pointer);
var
  Index: Integer;
  Src: PRGB24Word absolute Source;
...
procedure TPngTrueColorAlpha8bitBGRADecoder.TransferData(Source: Pointer;
  Destination: Pointer);
var
  Index: Integer;
  Src: PRGB32 absolute Source;
...
procedure TPngTrueColorAlpha16bitBGRADecoder.TransferData(Source: Pointer;
  Destination: Pointer);
var
  Index: Integer;
  Src: PRGB32Word absolute Source;
...
function TCustomPngNonInterlacedEncoder.ColorInPalette(Color: TBGR32): Integer;
var
  Palette: PPalette24;
  Color24: TRGB24;
begin
  ...
  begin
    ...
    if (Color.R = Color24.R) and (Color.G = Color24.G) and (Color.B = Color24.B)
    then
      Exit;
  end;
  Result := -1;

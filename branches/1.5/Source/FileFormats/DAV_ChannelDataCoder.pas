...
begin
  ...
  begin
    ...
    Move(FBlockBuffer[0], TCustomChannelDataCoder(Dest).FBlockBuffer[0],
      FBlockSize);
  end
  else
    inherited;
...
function TCustomChannelDataCoder.CorrectBlocksize(const Value: Cardinal)
  : Cardinal;
begin
  Result := Value;
end;
...
begin
  ...
  begin
    Assert(Length(FChannelArray) = Length(TCustomChannel32DataCoder(Dest)
      .FChannelArray));
    for Channel := 0 to Length(FChannelArray) - 1 do
    begin
      Move(FChannelArray[Channel, 0], TCustomChannel32DataCoder(Dest)
        .FChannelArray[Channel, 0], FSampleFrames * SizeOf(Single));
    end;
  end
  else if Dest is TCustomChannel64DataCoder then
  begin
    Assert(Length(FChannelArray) = Length(TCustomChannel64DataCoder(Dest)
      .FChannelArray));
    for Channel := 0 to Length(FChannelArray) - 1 do
      for Sample := 0 to FSampleFrames - 1 do
        TCustomChannel64DataCoder(Dest).FChannelArray[Channel, Sample] :=
          FChannelArray[Channel, Sample];
  end;
end;
...
begin
  ...
  if FChannelCount < Cardinal(Length(FChannelArray)) then
    for Channel := FChannelCount to Length(FChannelArray) - 1 do
      Dispose(FChannelArray[Channel]);
...
function TCustomChannel32DataCoder.GetChannelPointer(Index: Integer)
...
procedure TCustomChannel32DataCoderFixedPoint.SetByteAlign
  (const Value: TByteAlign);
begin
...
procedure TCustomChannel32DataCoderFixedPoint.SetDitherType
  (const Value: TDitherType);
begin
...
function TCustomChannel32DataCoderFixedPoint.CorrectBlocksize
...
begin
  ...
  else if Dest is TCustomChannel64DataCoderFixedPoint then
...
function TCustomChannel32DataCoderFixedPoint.SampleToByte(Sample: Cardinal)
  : Cardinal;
begin
...
begin
  if (Bits > 8 * SampleSize) then
    raise Exception.Create('Number of bits must fit into the sample size!');

  FBits := Bits;
  FSampleSize := SampleSize;

  CalculateBlockSize;
  if BlockSize <> CorrectBlocksize(BlockSize) then
    BlockSize := CorrectBlocksize(BlockSize);
...
begin
  ...
  case SampleSize of
    ...
    2:
      for Channel := 0 to FChannelCount - 1 do
        for Sample := 0 to FSampleFrames - 1 do
          FChannelArray[Channel]^[Sample] := PDAVSmallIntArray(FBlockBuffer)
            ^[Sample * FChannelCount + Channel] * FScaleFactor[1];
    3:
      for Channel := 0 to FChannelCount - 1 do
        for Sample := 0 to FSampleFrames - 1 do
          FChannelArray[Channel]^[Sample] :=
            (ShortInt(PDAVByteArray(FBlockBuffer)
            ^[3 * (Sample * FChannelCount + Channel) + 2]) shl 16 +
            PDAVByteArray(FBlockBuffer)^[3 * (Sample * FChannelCount + Channel)
            + 1] shl 8 + PDAVByteArray(FBlockBuffer)
            ^[3 * (Sample * FChannelCount + Channel)]) * FScaleFactor[1];
    4:
      for Channel := 0 to FChannelCount - 1 do
        for Sample := 0 to FSampleFrames - 1 do
          FChannelArray[Channel]^[Sample] := PIntegerArray(FBlockBuffer)
            ^[Sample * FChannelCount + Channel] * FScaleFactor[1];
  end;
end;
...
begin
  ...
  for Sample := 0 to FSampleFrames - 1 do
    for Channel := 0 to FChannelCount - 1 do
      PDAVByteArray(FBlockBuffer)^[Sample * FChannelCount + Channel] :=
        EncodeALaw(FChannelArray[Channel]^[Sample]);
end;
...
const
  CALawDecompressTable: array [0 .. 255] of Smallint = (-5504, -5248, -6016,
...
begin
  ...
  for Channel := 0 to FChannelCount - 1 do
    for Sample := 0 to FSampleFrames - 1 do
      FChannelArray[Channel]^[Sample] := CALawDecompressTable
        [PDAVByteArray(FBlockBuffer)^[Sample * FChannelCount + Channel]
        ] * CScale;
end;
...
const
  CBias: Integer = $84;
  CClip: Integer = 32635;
  CMuLawCompressTable: array [0 .. 255] of Byte = (0, 0, 1, 1, 2, 2, 2, 2, 3, 3,
...
begin
  ...
  for Sample := 0 to FSampleFrames - 1 do
    for Channel := 0 to FChannelCount - 1 do
      PDAVByteArray(FBlockBuffer)^[Sample * FChannelCount + Channel] :=
        LinearToMuLawSample(FChannelArray[Channel]^[Sample]);
end;

const
  CMuLawDecompressTable: array [0 .. 255] of Smallint = (-32124, -31100, -30076,
    -29052, -28028, -27004, -25980, -24956, -23932, -22908, -21884, -20860,
    -19836, -18812, -17788, -16764, -15996, -15484, -14972, -14460, -13948,
    -13436, -12924, -12412, -11900, -11388, -10876, -10364, -9852, -9340, -8828,
    -8316, -7932, -7676, -7420, -7164, -6908, -6652, -6396, -6140, -5884, -5628,
    -5372, -5116, -4860, -4604, -4348, -4092, -3900, -3772, -3644, -3516, -3388,
    -3260, -3132, -3004, -2876, -2748, -2620, -2492, -2364, -2236, -2108, -1980,
    -1884, -1820, -1756, -1692, -1628, -1564, -1500, -1436, -1372, -1308, -1244,
    -1180, -1116, -1052, -988, -924, -876, -844, -812, -780, -748, -716, -684,
    -652, -620, -588, -556, -524, -492, -460, -428, -396, -372, -356, -340,
    -324, -308, -292, -276, -260, -244, -228, -212, -196, -180, -164, -148,
    -132, -120, -112, -104, -96, -88, -80, -72, -64, -56, -48, -40, -32, -24,
    -16, -8, 0, 32124, 31100, 30076, 29052, 28028, 27004, 25980, 24956, 23932,
    22908, 21884, 20860, 19836, 18812, 17788, 16764, 15996, 15484, 14972, 14460,
...
begin
  ...
  for Channel := 0 to FChannelCount - 1 do
    for Sample := 0 to FSampleFrames - 1 do
      FChannelArray[Channel]^[Sample] := CMuLawDecompressTable
        [(PDAVByteArray(FBlockBuffer)^[Sample * FChannelCount + Channel])
        ] * CScale;
end;
...
function TChannel32DataCoderFloat16.CorrectBlocksize(const Value: Cardinal)
  : Cardinal;
var
  Granularity: Cardinal;
begin
...
begin
  ...
  for Channel := 0 to FChannelCount - 1 do
    for Sample := 0 to FSampleFrames - 1 do
      PDAVHalfFloatFixedArray(FBlockBuffer)^[Sample * FChannelCount + Channel]
        := SingleToHalfFloat(FChannelArray[Channel]^[Sample]);
end;
...
begin
  ...
  for Channel := 0 to FChannelCount - 1 do
    for Sample := 0 to FSampleFrames - 1 do
      FChannelArray[Channel]^[Sample] :=
        HalfFloatToSingle(PDAVHalfFloatFixedArray(FBlockBuffer)
        ^[Sample * FChannelCount + Channel]);
end;
...
function TChannel32DataCoderFloat32.CorrectBlocksize(const Value: Cardinal)
  : Cardinal;
var
  Granularity: Cardinal;
begin
...
begin
  ...
  for Channel := 0 to FChannelCount - 1 do
    for Sample := 0 to FSampleFrames - 1 do
      PDAVSingleFixedArray(FBlockBuffer)^[Sample * FChannelCount + Channel] :=
        FChannelArray[Channel]^[Sample];
end;
...
begin
  ...
  for Channel := 0 to FChannelCount - 1 do
    for Sample := 0 to FSampleFrames - 1 do
      FChannelArray[Channel]^[Sample] := PDAVSingleFixedArray(FBlockBuffer)
        ^[Sample * FChannelCount + Channel];
end;
...
function TChannel32DataCoderFloat64.CorrectBlocksize(const Value: Cardinal)
  : Cardinal;
var
  Granularity: Cardinal;
begin
...
begin
  ...
  for Channel := 0 to FChannelCount - 1 do
    for Sample := 0 to FSampleFrames - 1 do
      PDAVDoubleFixedArray(FBlockBuffer)^[Sample * FChannelCount + Channel] :=
        FChannelArray[Channel]^[Sample];
end;
...
begin
  ...
  for Channel := 0 to FChannelCount - 1 do
    for Sample := 0 to FSampleFrames - 1 do
      FChannelArray[Channel]^[Sample] := PDAVDoubleFixedArray(FBlockBuffer)
        ^[Sample * FChannelCount + Channel];
end;
...
begin
  ...
  begin
    ...
    for Channel := 0 to Length(FChannelArray) - 1 do
      for Sample := 0 to FSampleFrames - 1 do
        TCustomChannel32DataCoder(Dest).FChannelArray[Channel, Sample] :=
          FChannelArray[Channel, Sample];
  end
  else if Dest is TCustomChannel64DataCoder then
  begin
    ...
    for Channel := 0 to Length(FChannelArray) - 1 do
    begin
      Move(FChannelArray[Channel, 0], TCustomChannel64DataCoder(Dest)
        .FChannelArray[Channel, 0], FSampleFrames * SizeOf(Double));
    end;
  end;
end;
...
function TCustomChannel64DataCoder.GetChannelPointer(Index: Integer)
...
procedure TCustomChannel64DataCoderFixedPoint.SetByteAlign
  (const Value: TByteAlign);
begin
  if FByteAlign <> Value then
  begin
...
procedure TCustomChannel64DataCoderFixedPoint.SetDitherType
  (const Value: TDitherType);
begin
  if FDitherType <> Value then
  begin
...
function TCustomChannel64DataCoderFixedPoint.CorrectBlocksize
  (const Value: Cardinal): Cardinal;
var
  Granularity: Cardinal;
begin
...
function TCustomChannel64DataCoderFixedPoint.SampleToByte(Sample: Cardinal)
  : Cardinal;
begin
  Result := Sample * FSampleSize;
end;
...
function TChannel64DataCoderFloat32.CorrectBlocksize(const Value: Cardinal)
  : Cardinal;
var
  Granularity: Cardinal;
begin
...
function TChannel64DataCoderFloat64.CorrectBlocksize(const Value: Cardinal)
  : Cardinal;
var
  Granularity: Cardinal;
begin

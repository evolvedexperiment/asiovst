...
type
  ...
  public
    property GranularPitchShifter[Index: Integer]: TDspGranularPitchShifter32
      read GetGranularPitchShifter;
  end;
...
begin
  ...
  try
    if Assigned(FGranularPitchShifter[0]) then
      FGranularPitchShifter[0].Semitones := Value;
    if Assigned(FGranularPitchShifter[1]) then
      FGranularPitchShifter[1].Semitones := Value;
  finally
    FCriticalSection.Leave;
  end;
  if EditorForm is TFmGranularPitchShifter then
    with TFmGranularPitchShifter(EditorForm) do
      UpdateSemitones;
end;

procedure TGranularPitchShifterModule.ParamStagesDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  PreDefined := AnsiString(IntToStr(Round(Parameter[Index])));
end;

procedure TGranularPitchShifterModule.ParamStagesChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
    if Assigned(FGranularPitchShifter[0]) then
      FGranularPitchShifter[0].Stages := Round(Value);
    if Assigned(FGranularPitchShifter[1]) then
      FGranularPitchShifter[1].Stages := Round(Value);
  finally
    FCriticalSection.Leave;
  end;
  if EditorForm is TFmGranularPitchShifter then
...
procedure TGranularPitchShifterModule.ParamGranularityChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
    if Assigned(FGranularPitchShifter[0]) then
      FGranularPitchShifter[0].Granularity := 1E-3 * Value;
    if Assigned(FGranularPitchShifter[1]) then
      FGranularPitchShifter[1].Granularity := 1E-3 * Value;
  finally
    FCriticalSection.Leave;
  end;
  if EditorForm is TFmGranularPitchShifter then
...
procedure TGranularPitchShifterModule.ParamMixChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  Value := 100;
  FCriticalSection.Enter;
...
begin
  ...
  try
    if Assigned(FGranularPitchShifter[0]) then
      FGranularPitchShifter[0].SampleRate := SampleRate;
    if Assigned(FGranularPitchShifter[1]) then
      FGranularPitchShifter[1].SampleRate := SampleRate;
  finally
    FCriticalSection.Leave;
  end;
end;

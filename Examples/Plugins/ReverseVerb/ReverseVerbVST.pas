...
begin
  ...
  begin
    ...
    FDelay[ChannelIndex, 1] := TDelayLineSamples32.Create
      (FConvolution[ChannelIndex].Latency);
  end;
...
{$ENDIF}
  if Assigned(FImpulseResponse[0]) then
    Dispose(FImpulseResponse[0]);
  if Assigned(FImpulseResponse[1]) then
    Dispose(FImpulseResponse[1]);
  if Assigned(FBackwardIR[0]) then
    Dispose(FBackwardIR[0]);
  if Assigned(FBackwardIR[1]) then
    Dispose(FBackwardIR[1]);
  if Assigned(FTempBuffer) then
    Dispose(FTempBuffer);

  for ChannelIndex := 0 to Length(FConvolution) - 1 do
    FreeAndNil(FConvolution[ChannelIndex]);
  for ChannelIndex := 0 to Length(FDelay) - 1 do
  begin
...
procedure TReverseVerbDataModule.ParameterDampingChange(Sender: TObject;
...
begin
  ...
  if not Assigned(FDampingFilter) then
    Exit;
  ...
  begin
    ...
    begin
      ...
      Assert(Weight <= 1);
      IR[SampleIndex] := Weight * Current + (1 - Weight) *
        FDampingFilter.ProcessSample64(Current);
    ...
    begin
      ...
      Current := 0.5 * (Weight + Sqr(Sqr(Weight))) * TailGain * ExpGain *
        (Random - Random);

      // calculate early reflections
      if SampleIndex = NextSample then
      begin
        Current := Current + (1 - Sqr(Weight)) * ERGain * ExpGain *
          (Random - Random);
        Offset := Random((TailStart - SampleIndex) div 3) div 3;
        NextSample := SampleIndex + 1 + Offset;
      end;
    ...
    begin
      ...
      Weight := 0.1 * Sqr(Sqr((FIRLength - SampleIndex) * Scale));
      Assert(Weight <= 1);
      IR[SampleIndex] := CDenorm32 + Weight * Current + (1 - Weight) *
        FDampingFilter.ProcessSample64(Current);
      BackIR^[FIRLength - 1 - SampleIndex] := IR[SampleIndex];
  ...
  try
    for ChannelIndex := 0 to Length(FConvolution) - 1 do
      if Assigned(FConvolution[ChannelIndex]) then
        if FIsBackward then
          FConvolution[ChannelIndex].LoadImpulseResponse
            (FBackwardIR[ChannelIndex], FIRLength)
        else
          FConvolution[ChannelIndex].LoadImpulseResponse
            (FImpulseResponse[ChannelIndex], FIRLength);
  finally
...
begin
  ...
  if Temp <> 0 then
    Temp := -3 / (SampleRate * FT60)
  else
    Temp := -20;
  FT60Factor := FastPower10ContinousError3(Temp);
end;

procedure TReverseVerbDataModule.ParameterDirectChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ChannelIndex: Integer;
begin
  ...
  try
    for ChannelIndex := 0 to Length(FCompressor) - 1 do
      if Assigned(FCompressor[ChannelIndex]) then
        FCompressor[ChannelIndex].MakeUpGain_dB := Value;
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TReverseVerbDataModule.ParameterERGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
...
procedure TReverseVerbDataModule.ParameterTailChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
...
procedure TReverseVerbDataModule.ParameterThresholdChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ChannelIndex: Integer;
begin
  ...
  try
    for ChannelIndex := 0 to Length(FCompressor) - 1 do
      if Assigned(FCompressor[ChannelIndex]) then
        FCompressor[ChannelIndex].Threshold_dB := Value;
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TReverseVerbDataModule.ParameterCompressorRatioChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ChannelIndex: Integer;
begin
  ...
  try
    for ChannelIndex := 0 to Length(FCompressor) - 1 do
      if Assigned(FCompressor[ChannelIndex]) then
        FCompressor[ChannelIndex].Ratio := Value;
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TReverseVerbDataModule.ParameterOutputGainChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FOutputGain := dB_to_Amp(Value);
end;

procedure TReverseVerbDataModule.ParameterERTimeChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ChannelIndex: Integer;
begin
  ...
  try
    for ChannelIndex := 0 to Length(FCompressor) - 1 do
      if Assigned(FCompressor[ChannelIndex]) then
        FCompressor[ChannelIndex].Attack := 0.1 * Value;
  finally
    FCriticalSection.Leave;
  end;
end;

procedure TReverseVerbDataModule.ParameterDirectionChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  ...
  if FIsBackward then
    OnProcess := VSTModuleProcessBackward
  else
    OnProcess := VSTModuleProcessForward;
  OnProcess32Replacing := OnProcess;
end;

procedure TReverseVerbDataModule.ParameterDirectionDisplay(Sender: TObject;
...
procedure TReverseVerbDataModule.ParameterDampingLabel(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
...
procedure TReverseVerbDataModule.ParameterDampingDisplay(Sender: TObject;
  const Index: Integer; var PreDefined: AnsiString);
begin
  if Parameter[Index] >= 1000 then
    PreDefined := AnsiString(FloatToStrF(1E-3 * Parameter[Index],
      ffGeneral, 4, 4))
  else
    PreDefined := AnsiString(FloatToStrF(Parameter[Index], ffGeneral, 4, 4));
end;

procedure TReverseVerbDataModule.ParameterT60Change(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ChannelIndex: Integer;
begin
  ...
  try
    for ChannelIndex := 0 to Length(FCompressor) - 1 do
      if Assigned(FCompressor[ChannelIndex]) then
        FCompressor[ChannelIndex].Release := Value;
  finally
    FCriticalSection.Leave;
  end;
end;
...
{$ELSE}
  FIRChanged := True;
  if Assigned(FIRUpdateThread) and FIRUpdateThread.Suspended then
    FIRUpdateThread.Resume;
{$ENDIF}
end;
  ...
  try
    for ChannelIndex := 0 to CNumChannels - 1 do
      if Assigned(FDelay[ChannelIndex, 0]) and
        Assigned(FConvolution[ChannelIndex]) then
      begin
        ...
        begin
          Outputs[ChannelIndex, SampleIndex] := FOutputGain *
            (Outputs[ChannelIndex, SampleIndex] + FDelay[ChannelIndex,
            0].ProcessSample32(FDelay[ChannelIndex,
            1].ProcessSample32(FCompressor[ChannelIndex].ProcessSample32(Inputs
  ...
  try
    for ChannelIndex := 0 to CNumChannels - 1 do
      ...
      begin
        ...
        begin
          Outputs[ChannelIndex, SampleIndex] := FOutputGain *
            (FDelay[ChannelIndex, 0].ProcessSample32(Outputs[ChannelIndex,
            SampleIndex] + FDelay[ChannelIndex,
            1].ProcessSample32(FCompressor[ChannelIndex].ProcessSample32(Inputs
            [ChannelIndex, SampleIndex]))));
        end;
      end;

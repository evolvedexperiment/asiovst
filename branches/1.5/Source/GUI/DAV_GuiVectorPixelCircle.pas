...
begin
  ...
  begin
    ...
    if YRange[1] < 0 then
      Exit;
    if FixedRound(FixedSub(CenterX, Radius)) >= Width then
      Exit;
    if FixedRound(FixedAdd(CenterX, Radius)) < 0 then
      Exit;
...
begin
  ...
  begin
    ...
    if YRange[1] < 0 then
      Exit;
    if Center.Re - Radius >= Width then
      Exit;
    if Center.Re + Radius < 0 then
      Exit;
    ...
    begin
      ...
      begin
        ...
        if SqrDist >= SqrRadMinusOne then
          CombColor.A := Round(CombColor.A * (Radius - FastSqrtBab2(SqrDist)));
...
begin
  ...
  begin
    ...
    if Radius.Fixed > 0 then
      SqrRadMinusOne := FixedSqr(FixedSub(Radius, CFixed24Dot8One))
    else
...
{$ELSE}
          PixelLineCount :=
            FixedFloor(FixedAdd(FixedSub(CenterX, ConvertToFixed24Dot8(X)),
            FixedSqrt(FixedSub(SqrRadMinusOne, SqrYDist)))) + 1;
...
begin
  ...
  begin
    ...
    begin
      ...
      // inner radius
      SqrMinusWidth := Sqr(BranchlessClipPositive(SingleRadius - LineWidth.Fixed
        * CFixed24Dot8ToFloat));
    end;
    ...
    if YRange[1] < 0 then
      Exit;
    if Center.Re - SingleRadius >= Width then
      Exit;
    if Center.Re + SingleRadius < 0 then
      Exit;
...
begin
  ...
  begin
    ...
    if YRange[1] < 0 then
      Exit;
    if FixedRound(FixedSub(CenterX, OuterRadius)) >= Width then
      Exit;
    if FixedRound(FixedAdd(CenterX, OuterRadius)) < 0 then
      Exit;
    ...
    if OuterRadius.Fixed > 0 then
      SqrOuterRad := FixedSqr(FixedSub(OuterRadius, CFixed24Dot8One))
    else
      SqrOuterRad.Fixed := 0;
    ...
    begin
      ...
      begin
        ...
        else if SqrDist.Fixed >= SqrInnerRadOne.Fixed then
        begin
          BlendPixelInplace(PixelColor32, ScnLne[X]);
          Inc(X);
        end
        else if SqrDist.Fixed > SqrInnerRad.Fixed then
        begin
          SqrDist.Fixed := InnerRadius.Fixed - FixedSqrt(SqrDist).Fixed;
          if SqrDist.Fixed < $FF then
            CombColor.A := $FF - ((SqrDist.Fixed * CombColor.A + $7F) shr 8)
          else
            CombColor.A := 0;
          BlendPixelInplace(CombColor, ScnLne[X]);
          Inc(X);
...
{$ELSE}
          PixelLineCount :=
            FixedFloor(FixedAdd(FixedSub(CenterX, ConvertToFixed24Dot8(X)),
            FixedSqrt(FixedSub(SqrInnerRad, SqrYDist)))) + 1;

...
constructor TSEHRTF3DModule.Create(SEAudioMaster: TSE2audioMasterCallback;
  Reserved: Pointer);
begin
...
procedure TSEHRTF3DModule.SubProcessStatic(const BufferOffset,
...
begin
  if (Pin[Integer(pinInputL)].Status = stRun) and
    (Pin[Integer(pinInputR)].Status = stRun) then
    OnProcess := SubProcess
  else
  begin
...
class procedure TSEHRTF3DModule.GetModuleProperties
...
function TSEHRTF3DModule.GetPinProperties(const Index: Integer;

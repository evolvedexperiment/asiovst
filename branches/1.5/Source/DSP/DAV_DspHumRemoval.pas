...
type
  ...
  public
    ...
    procedure ProcessBlock32(const Data: PDAVSingleFixedArray;
      SampleCount: Integer);
    procedure ProcessBlock64(const Data: PDAVSingleFixedArray;
      SampleCount: Integer);
    function Magnitude_dB(const Frequency: Double): Double;
    function MagnitudeSquared(const Frequency: Double): Double;

    property HighpassFilterActive: Boolean read FDcFilterActive
      write SetDcFilterActive;
    property HighpassFilterType: TOrderFilterClass read FHighpassFilterType
      write SetHighpassFilterType;
    property HighpassFilter: TCustomOrderFilter read FHighpassFilter;
    property NotchFilterCount: Integer read FNotchFilterCount
      write SetNotchFilterCount;
    property FundamentalFrequency: Single read FFundamentalFrequency
      write SetFundamentalFrequency;
...
begin
  ...
  for FilterIndex := 0 to Length(FNotchFilters) - 1 do
    FreeAndNil(FNotchFilters[FilterIndex]);
  inherited;
end;
...
begin
  ...
  for FilterIndex := 0 to Length(FNotchFilters) - 1 do
    FNotchFilters[FilterIndex].ResetStates;
end;
...
begin
  for FilterNo := 0 to Length(FNotchFilters) - 1 do
    FNotchFilters[FilterNo].Bandwidth := Sqr(FBandwidth);
  Changed;
end;
...
begin
  for FilterNo := 0 to Length(FNotchFilters) - 1 do
    FNotchFilters[FilterNo].Gain := -FAttenuation + FAttenuation * FilterNo /
      Length(FNotchFilters);
  Changed;
end;
...
var
  FilterNo: Integer;
begin
  for FilterNo := 1 to Length(FNotchFilters) do
    FNotchFilters[FilterNo - 1].Frequency := FFundamentalFrequency * FilterNo;
  Changed;
end;
...
begin
  // free unused filters
  for FilterNo := FNotchFilterCount to Length(FNotchFilters) - 1 do
    FreeAndNil(FNotchFilters[FilterNo]);
...
begin
  ...
  else if FHighpassFilter is TCustomChebyshev2Filter then
...
var
  FilterNo: Integer;
begin
  if FDcFilterActive then
    Result := FHighpassFilter.MagnitudeSquared(Frequency)
  else
    Result := 1;

  for FilterNo := 0 to Length(FNotchFilters) - 1 do
    Result := Result * FNotchFilters[FilterNo].MagnitudeSquared(Frequency);
end;
...
begin
  if FDcFilterActive then
    Result := FHighpassFilter.ProcessSample64(Input)
  else
    Result := Input;
  for FilterNo := 0 to Length(FNotchFilters) - 1 do
    Result := FNotchFilters[FilterNo].ProcessSample64(Result);
end;

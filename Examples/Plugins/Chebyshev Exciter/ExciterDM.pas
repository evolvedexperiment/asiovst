...
type
  ...
  private
    ...
    FSplitterHighpassFilter: array [0 .. 1, 0 .. 1]
      of TButterworthHighPassFilter;
...
begin
  for ChannelIndex := 0 to numInputs - 1 do
    ...
    begin
      FSourceLowpassFilter[ChannelIndex, BandIndex] :=
        TButterworthLowPassFilter.Create;
      FSourceHighpassFilter[ChannelIndex, BandIndex] :=
        TButterworthHighPassFilter.Create;
      FSplitterHighpassFilter[ChannelIndex, BandIndex] :=
        TButterworthHighPassFilter.Create;
    end;
...
procedure TExciterDataModule.ParamMixChange(Sender: TObject;
...
begin
  ...
  if EditorForm is TFmExciter then
    TFmExciter(EditorForm).UpdateMix;
end;

procedure TExciterDataModule.ParamShapeChange(Sender: TObject;
  const Index: Integer; var Value: Single);
begin
  FCriticalSection.Enter;
  try
  ...
  // update GUI
  if EditorForm is TFmExciter then
    TFmExciter(EditorForm).UpdateShape;
end;

procedure TExciterDataModule.ParameterOrderChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
...
begin
  ...
  try
    for ChannelIndex := 0 to numInputs - 1 do
      for BandIndex := 0 to 1 do
      begin
        if Assigned(FSourceLowpassFilter[ChannelIndex, BandIndex]) then
          FSourceLowpassFilter[ChannelIndex, BandIndex].Order := Round(Value);
        if Assigned(FSourceHighpassFilter[ChannelIndex, BandIndex]) then
          FSourceHighpassFilter[ChannelIndex, BandIndex].Order := Round(Value);
        if Assigned(FSplitterHighpassFilter[ChannelIndex, BandIndex]) then
          FSplitterHighpassFilter[ChannelIndex, BandIndex].Order :=
            Round(Value);
      end;
    InvertMix;
  finally
  ...
  // update GUI
  if EditorForm is TFmExciter then
    TFmExciter(EditorForm).UpdateOrder;
end;

procedure TExciterDataModule.ParamFrequencyChange(Sender: TObject;
  const Index: Integer; var Value: Single);
var
  ChannelIndex: Integer;
  BandIndex: Integer;
begin
  ...
  begin
    FCriticalSection.Enter;
    try
      FChebyshevWaveshaper.Order :=
        Round(Min(22000, 0.48 * SampleRate) / Value + 0.5);
      for ChannelIndex := 0 to numInputs - 1 do
        for BandIndex := 0 to 1 do
        begin
  ...
  // update GUI
  if EditorForm is TFmExciter then
    TFmExciter(EditorForm).UpdateTune;
end;
  ...
  begin
    ...
    try
      for ChannelIndex := 0 to numInputs - 1 do
        ...
        begin
          if Assigned(FSourceLowpassFilter[ChannelIndex, BandIndex]) then
            FSourceLowpassFilter[ChannelIndex, BandIndex].SampleRate :=
              Abs(SampleRate);
          if Assigned(FSourceHighpassFilter[ChannelIndex, BandIndex]) then
            FSourceHighpassFilter[ChannelIndex, BandIndex].SampleRate :=
              Abs(SampleRate);
          if Assigned(FSplitterHighpassFilter[ChannelIndex, BandIndex]) then
            FSplitterHighpassFilter[ChannelIndex, BandIndex].SampleRate :=
              Abs(SampleRate);
        end;
      if Assigned(FChebyshevWaveshaper) then
        FChebyshevWaveshaper.Order :=
          Round(Min(22000, 0.48 * Abs(SampleRate)) / ParameterByName
          ['Tune'] + 0.5);
    finally
      FCriticalSection.Leave;
    end;
  ...
  try
    for SampleIndex := 0 to Min(FBlockSize, SampleFrames) - 1 do
      ...
      begin
        ...
        Outputs[ChannelIndex, SampleIndex] := Low + FMix[0] * High +
          FMix[1] * Source;
      end;
  finally
    FCriticalSection.Leave;
...
procedure TExciterDataModule.VSTModuleProcessDoubleReplacing(const Inputs,
  ...
  try
    for SampleIndex := 0 to Min(FBlockSize, SampleFrames) - 1 do
      ...
      begin
        ...
        Source := FChebyshevWaveshaper.ProcessSample64(FOverdriveGain * Low);
        Source := FSourceHighpassFilter[ChannelIndex, 1].ProcessSample64
          (FSourceHighpassFilter[ChannelIndex, 0].ProcessSample64(CDenorm
          + Source));
...
begin
  ...
  try
    for ChannelIndex := 0 to 1 do
      ...
      begin
        if Assigned(FSourceLowpassFilter[ChannelIndex, BandIndex]) then
          FSourceLowpassFilter[ChannelIndex, BandIndex].ResetStates;
        if Assigned(FSourceHighpassFilter[ChannelIndex, BandIndex]) then
          FSourceHighpassFilter[ChannelIndex, BandIndex].ResetStates;
        if Assigned(FSplitterHighpassFilter[ChannelIndex, BandIndex]) then
          FSplitterHighpassFilter[ChannelIndex, BandIndex].ResetStates;
      end;
  finally
    FCriticalSection.Leave;
  end;

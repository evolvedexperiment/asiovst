...
type
  ...
  TSEBarkVocoderPins = (pinInput, pinCarrier, pinOutput, pinBandwidth,
...
procedure TCustomSEBarkVocoderModule.SubProcessStatic(const BufferOffset,
  SampleFrames: Integer);
begin
...
class procedure TCustomSEBarkVocoderModule.GetModuleProperties
...
function TCustomSEBarkVocoderModule.GetPinProperties(const Index: Integer;
...
// describe the pins (plugs)
function TSEBarkVocoderStaticModule.GetPinProperties(const Index: Integer;
  Properties: PSEPinProperties): Boolean;
begin
...
begin
  Result := inherited GetPinProperties(Index, Properties);
  if TSEBarkVocoderPins(index) in [pinBandwidth .. pinRelease] then
    with Properties^ do
      Direction := drIn;
end;
...
procedure TSEBarkVocoderAutomatableModule.SubProcessAutomated
